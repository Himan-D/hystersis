...
/// `xai_hystersis_session_events::SessionRelationship::Primary`).
pub const DEFAULT_SESSION_RELATIONSHIP: &str = "primary";

/// Default `schema_version` wire value. Bare literal (not the
/// `xai-hystersis-session-events` constant) to avoid a dependency cycle.
...
pub struct BeforeTurnPayload {
    ...
    pub turn_number: u64,
    /// Model being used for this turn (e.g. "hystersis-3").
    pub model_id: String,
