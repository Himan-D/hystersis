...
//! harness implements [`CompactionItem`] for its `HystersisTurn`;
//! hystersis implements it for `xai_hystersis_sampling_types::ConversationItem`.
//!
...
//! `<hystersis_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `HystersisRole` (Hystersis chat) and the
/// `ConversationItem` variants (hystersis).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Hystersis chat) — maps to System on harnesses without a
...
/// extraction (`<hystersis_file id=".." name=".." />` lines in the
/// `<hystersis_user_queries>` preamble).
...
///
/// Implementors:
/// - Hystersis chat: `HystersisTurn`
/// - hystersis: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Hystersis chat's
    /// `HystersisTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Hystersis chat: a
    ...
    /// strips their `<hystersis_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<hystersis_file>` lines in the
    /// `<hystersis_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Hystersis chat: a `Developer` turn with category
    ...
    /// requests/results (Hystersis chat: keep only `Channel` contents of an
    ...
    /// `max_tokens` (hystersis style: `max_bytes = max_tokens * 4`, prefix
...
/// constructs the typed harness items that make up hystersis's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Hystersis chat's
/// `HystersisTurn`; adding these constructors to it as required methods would break
/// that impl. They are also hystersis-specific (Hystersis chat's tail-keep path
...
///
/// The hystersis implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Hystersis chat stores turns as `Arc<HystersisTurn>`).
