//! hystersis's full-replace compaction pass.
//!
//! hystersis does not select a tail to keep; it summarizes the whole
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `xai-hystersis-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). hystersis keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (hystersis's
...
/// Run hystersis's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam hystersis's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // hystersis appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce hystersis's exact compacted
    /// history shape, with the LLM output cleaned and the agent-state reminder
    /// carried through as the final item.
    #[tokio::test]
    async fn full_replace_produces_hystersis_build_history_shape() {
        let llm_turns = vec![
