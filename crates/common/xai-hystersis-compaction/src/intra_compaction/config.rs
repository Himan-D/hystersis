...
///
/// - `FullReplace` (default): hystersis's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from hystersis-shell's
...
/// time (token threshold only, matching hystersis) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as hystersis full-replace auto-compact). Worthless early passes are
    ...
    /// `<hystersis_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "hystersis-4.20";
