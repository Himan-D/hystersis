...
pub enum CompactionTarget {
    ...
    /// with a single summary — hystersis's full-replace strategy. No tail is
...
/// stream processor. Implemented by Hystersis chat's
/// `StreamProcessor` (`Item = Arc<HystersisTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<HystersisTurn>`) already satisfies it.
