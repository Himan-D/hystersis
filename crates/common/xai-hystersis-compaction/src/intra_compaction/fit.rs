...
//!                          that alone exceed budget (hystersis style:
...
//!   (one-shot, same `tokens * 4` budget as hystersis)
...
/// One-shot payload shrink (hystersis: `max_bytes = max_tokens * 4`).
///
...
///
/// Matches hystersis's focus: in-place truncate is for oversized tool
...
pub fn fit_turns_for_summarizer<T: CompactionItemBuilder>(
    ...
    // One-shot prefix clip on tool results only (hystersis style).
    ...
    // hystersis: when even the newest unit alone exceeds budget, keep it
...
///
/// Mirrors hystersis `truncate_text_to_bytes` / `truncate_item_to_tokens`:
