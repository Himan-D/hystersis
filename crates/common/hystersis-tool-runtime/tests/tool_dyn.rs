...
//! `HystersisToolDyn` / `HystersisToolFamily` tests but against the runtime's
//! typed `Tool` trait.
...
use hystersis_tool_protocol::{ToolCapabilities, ToolId};
...
async fn tool_dyn_delegates_id_description_capabilities() {
    ...
    assert_eq!(
        tool.description(&hystersis_tool_runtime::ListToolsContext::default())
            .name,
        "blocking_echo"
    );
