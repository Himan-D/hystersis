...
//!
//! - Hystersis chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `HystersisTurn::get_num_tokens`.
//! - hystersis estimates with `bytes / 4`.
//!
...
//! (Hystersis chat: `ItemTokenCounter<Arc<HystersisTurn>>`), so exactly one mechanism
//! is in play.
