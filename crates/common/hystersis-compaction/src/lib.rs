...
//! stay in each product host (for example `xai-hystersis-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `xai-hystersis-sampling-types`. It is decoupled from both Hystersis chat and
//! hystersis hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — hystersis's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Hystersis chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Hystersis chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Hystersis chat and
//! hystersis; hosts still own snapshotting and host-only sections).
...
// hystersis's full-replace subsystem now lives under `code_compaction`;
