...
// The `Shared` summarizer reuses hystersis's full-replace summarization core
...
/// `FullReplace` strategy (default): hystersis's full-replace — summarize the
...
/// `<hystersis_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching hystersis. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize (possibly fitted) turns through hystersis's shared core.
    ...
    //     Shared with Hystersis chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<hystersis_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<hystersis_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<hystersis_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): hystersis's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<hystersis_user_queries>` preamble so
...
/// — hystersis's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and hystersis stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // hystersis appends the summarization prompt as the final user message;
    ...
    .await
    {
        // hystersis returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Hystersis chat's
...
/// the literal error messages produced by the Hystersis chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Hystersis chat sampler when the
    ...
    /// — guards the forwarding that Hystersis chat (`Arc<HystersisTurn>`) relies on.
    #[test]
