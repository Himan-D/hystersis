...
//!
//! The canonical `sample → classify → retry` loop, used by **both** hystersis's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Hystersis chat's intra `Shared` summarizer
...
//! the winning summary (hystersis cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the hystersis host uses to step down its input size).
