...
//! in-sandbox manifest contract (`{workspace}/.hystersis/repos.json`).
//!
...
pub const REPOS_MANIFEST_RELATIVE_PATH: &str = ".hystersis/repos.json";
...
impl RepoManifest {
    ...
    /// Parse bytes from `{workspace}/.hystersis/repos.json`.
    ...
    pub fn materialized_mounts(&self, workspace_root: &std::path::Path) -> Vec<std::path::PathBuf> {
        ...
        for repo in &self.repos {
            ...
            // Confine to the workspace: a compromised/malicious `.hystersis/repos.json`
    ...
    fn manifest_round_trip() {
        let manifest = RepoManifest::new(vec![
            ProvisionedRepo {
                ...
                session_branch: "hystersis/s1".into(),
            },
            ProvisionedRepo {
                name: "lib".into(),
