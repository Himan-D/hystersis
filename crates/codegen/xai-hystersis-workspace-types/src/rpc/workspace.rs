...
/// over `xai_hystersis_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `xai_hystersis_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// Outcome of `workspace.kill_task`, mirroring `xai_hystersis_tools::KillOutcome`
...
/// One TODO list item (slim DTO over `xai_hystersis_tools`'s `TodoState`). `status`
...
/// in `xai-hystersis-workspace/src/hub_server.rs`.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct WorkspaceInfo {
    ...
    pub shell: String,
    pub cwd: String,
    /// Server binary version (`xai_hystersis_version::VERSION`); `None` on
