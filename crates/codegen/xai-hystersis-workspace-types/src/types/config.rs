...
//! agent-session config types in `xai-hystersis-config` and friends.
...
/// `xai-hystersis-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `xai_hystersis_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
