...
/// tool-call arguments routinely contain. `pub` for `xai-hystersis-shell`.
...
pub struct ConversationRequest {
    ...
    pub x_hystersis_conv_id: Option<String>,
    pub x_hystersis_req_id: Option<String>,
    pub x_hystersis_session_id: Option<String>,
    pub x_hystersis_turn_idx: Option<String>,
    pub x_hystersis_agent_id: Option<String>,
    pub x_hystersis_deployment_id: Option<String>,
    pub x_hystersis_user_id: Option<String>,
    ...
    /// Sticky routing key for prompt-cache reuse; overrides `x_hystersis_conv_id` for routing.
...
pub struct ConversationResponse {
    ...
    /// only, opt-in via the `x-hystersis-doom-loop-check` header). Empty when the
...
// ---------------------------------------------------------------------------
//
// Part of the Hystersis Compaction unification. Lets the shared, transport-agnostic
// engine in `crates/common/xai-hystersis-compaction` operate over hystersis's
...
impl xai_hystersis_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> xai_hystersis_compaction::CompactionRole {
        use xai_hystersis_compaction::CompactionRole;
        // hystersis has no distinct `Developer` role; everything maps onto
    ...
    fn is_compaction_summary(&self) -> bool {
        // hystersis has no structural marker that uniquely identifies a prior
        ...
        // (add a dedicated marker) before routing hystersis history through
        // the shared `history`/`inter` filter.
        false
    }

    fn attachment_refs(&self) -> Vec<xai_hystersis_compaction::CompactionFileRef> {
        // hystersis `UserItem`s carry only `Text`/`Image { url }` content
        // parts — there is no id+name attachment-ref concept like the chat harness's
        // `HystersisTurn`. The full-replace path does not read this; revisit if
        // image attachments need to survive into the `<hystersis_user_queries>`
        // preamble.
        Vec::new()
    }
}

impl xai_hystersis_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/xai-hystersis-sampler/src/stream/messages.rs))
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_hystersis_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_hystersis_req_id = Some(req_id.into());
        self
    }
...
mod compaction_item_bridge_tests {
    use super::*;
    use xai_hystersis_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // hystersis has no structural compaction-summary marker, and no
...
mod tests {
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.hystersis/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.hystersis/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.hystersis/worktrees/proj/fork-a";
    ...
    fn test_conversation_request_builder() {
        ...
        assert_eq!(req.model, Some("hystersis-3".to_string()));
        assert_eq!(req.temperature, Some(0.5));
        assert_eq!(req.max_output_tokens, Some(1000));
        assert_eq!(req.x_hystersis_conv_id, Some("conv-123".to_string()));
        assert_eq!(req.x_hystersis_req_id, Some("req-456".to_string()));
    ...
    fn test_assistant_item_with_model_id() {
        ...
        .with_model_id("hystersis-3");

        assert_eq!(item.model_id, Some("hystersis-3".to_string()));
    }

    #[test]
    fn test_conversation_item_with_model_id() {
        let item = ConversationItem::assistant("Hello").with_model_id("hystersis-3");

        let ConversationItem::Assistant(a) = item else {
            panic!("Expected Assistant");
        };
        assert_eq!(a.model_id, Some("hystersis-3".to_string()));

        // Non-assistant should be unchanged
        let user = ConversationItem::user("Hi").with_model_id("hystersis-3");
    ...
    //      (earlier hystersis / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_hystersis_build_shape() {
