//! Parse marketplace sources from `~/.hystersis/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// `HYSTERSIS_MARKETPLACE_REQUIRE_SHA=1`. Tighten-only: either source can enable,
...
pub fn env_require_sha() -> bool {
    xai_hystersis_config::env_bool("HYSTERSIS_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// and `known_marketplaces.json` files under `~/.hystersis/` and `~/.claude/`.
pub fn load_extra_sources_from_settings(existing: &[MarketplaceSource]) -> Vec<MarketplaceSource> {
    let roots: Vec<PathBuf> = [
        xai_hystersis_config::user_hystersis_home(),
...
/// instead of `~/.hystersis`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    /// `HYSTERSIS_MARKETPLACE_REQUIRE_SHA`, so they cannot race each other.
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("HYSTERSIS_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("HYSTERSIS_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("HYSTERSIS_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
