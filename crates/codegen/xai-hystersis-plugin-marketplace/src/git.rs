...
pub fn sync_source_cache_with_mode(
    ...
    let url = xai_hystersis_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(xai_hystersis_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    xai_hystersis_config::hystersis_home().join("marketplace-cache")
}
