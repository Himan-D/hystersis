...
//!
//! Catalog file lookup, in order: `.hystersis-plugin/marketplace.json` (preferred),
//! `.hystersis-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.hystersis-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.hystersis-plugin/plugin.json`
...
pub fn load_index(marketplace_root: &Path) -> Result<Option<MarketplaceIndex>, String> {
    let hystersis_dir = marketplace_root.join(".hystersis-plugin");
    let claude_dir = marketplace_root.join(".claude-plugin");
    let candidates = [
        hystersis_dir.join("marketplace.json"),
        hystersis_dir.join("plugin.json"),
...
mod tests {
    ...
    #[test]
    fn load_index_valid_hystersis_dir() {
        let dir = tempfile::tempdir().unwrap();
        let hystersis_dir = dir.path().join(".hystersis-plugin");
        std::fs::create_dir_all(&hystersis_dir).unwrap();
        std::fs::write(
            hystersis_dir.join("marketplace.json"),
            r#"{"name": "hystersis", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "hystersis");
    }

    #[test]
    fn load_index_hystersis_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".hystersis-plugin", "hystersis"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "hystersis");
    }
