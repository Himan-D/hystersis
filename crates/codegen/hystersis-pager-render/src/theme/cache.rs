...
//! `hystersis_shell::util::config::set_theme()` (and friends), invoked
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    hystersis_markdown::set_color_level_cap(if locked {
        hystersis_markdown::ColorLevel::Basic
    } else {
        hystersis_markdown::ColorLevel::TrueColor
    });
    hystersis_markdown::set_polarity_safe_syntax(locked);
}
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `hystersis_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = hystersis_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = hystersis_config::load_effective_config_disk_only() else {
...
mod tests {
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!hystersis_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                hystersis_markdown::polarity_safe_syntax(),
