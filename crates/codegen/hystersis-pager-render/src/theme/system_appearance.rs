...
//! 2. Explicit env stamps — `HYSTERSIS_APPEARANCE` / `LC_HYSTERSIS_APPEARANCE`
...
/// 1. Desktop APIs
/// 2. Explicit wrap/SSH stamps (`HYSTERSIS_APPEARANCE` / `LC_HYSTERSIS_APPEARANCE`)
...
///
/// Used by `hystersis wrap` to stamp the *local* OS theme into the child env
...
/// in `config.toml`. When `None`, defaults to `HystersisNight` / `HystersisDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::HystersisDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::HystersisNight),
    }
}
