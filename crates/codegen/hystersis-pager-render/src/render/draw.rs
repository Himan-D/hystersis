...
//! [`hystersis_ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
use hystersis_ratatui_inline::LinkSpan;
...
pub type PagerTerminal = hystersis_ratatui_inline::Terminal<CrosstermBackend<TermWriter>>;
#[derive(Debug)]
...
pub fn spawn_writer_thread() -> (
    ...
    let test_delay = std::env::var("HYSTERSIS_TEST_FRAME_WRITE_DELAY_MS")
        .ok()
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || -> std::io::Result<()> {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = hystersis_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(payload) = rx.recv() {
                ...
                let result = {
                    let _guard = hystersis_shared::stderr::stderr_lock();
