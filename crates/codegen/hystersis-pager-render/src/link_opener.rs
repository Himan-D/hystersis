...
//!
//! Extracted from the `OpenSuperhystersisUrl` dispatch handler so that any
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("HYSTERSIS_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "HYSTERSIS_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
...
fn spawn_url_opener(url: &str) -> bool {
    ...
    hystersis_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// The returned command is TTY-guarded via [`hystersis_tty_utils::detach_std_command`]
...
fn build_open_path_command(path: &std::path::Path) -> std::process::Command {
    ...
    command
        ...
        .stdout(std::process::Stdio::null())
        .stderr(std::process::Stdio::null());
    hystersis_tty_utils::detach_std_command(&mut command);
    command
}
...
///
/// Used by the HystersisPro upsell flow to attribute clicks to `referrer=hystersis`,
/// matching the OAuth consent screen and hystersis.com/cli marketing links regardless of
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/hystersis session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param("https://hystersis.com/upgrade", "referrer", "hystersis");
        assert_eq!(out, "https://hystersis.com/upgrade");
    }

    #[test]
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://hystersis.com/upgrade?referrer=other",
            "referrer",
            "hystersis",
        );
        assert_eq!(out, "https://hystersis.com/upgrade?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://hystersis.com/upgrade?heavy=1",
            "referrer",
            "hystersis",
        );
        assert_eq!(
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`hystersis.com/#hystersispro`); we still want the referrer attached.
        let out = ensure_query_param("https://hystersis.com/#hystersispro", "referrer", "hystersis");
        assert_eq!(out, "https://hystersis.com/upgrade");
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "hystersis");
        assert_eq!(out, "not a url");
    }

    #[test]
    fn ensure_query_param_url_encodes_value() {
        let out = ensure_query_param("https://hystersis.com/upgrade", "referrer", "hystersis build");
        assert_eq!(out, "https://hystersis.com/upgrade+build");
    }
