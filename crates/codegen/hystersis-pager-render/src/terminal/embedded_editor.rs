//! Detects whether hystersis is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` hystersis is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `xai-hystersis-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
