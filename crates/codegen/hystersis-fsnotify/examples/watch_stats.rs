...
//! startup latency, under either strategy (`HYSTERSIS_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
...
//! HYSTERSIS_FSNOTIFY_PER_DIR=0 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! HYSTERSIS_FSNOTIFY_PER_DIR=1 cargo run --release -p xai-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "hystersis holds 55k inotify watches" report).
...
use hystersis_fsnotify::{FsConfig, FsEventSource};
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("HYSTERSIS_FSNOTIFY_PER_DIR").unwrap_or_default();
