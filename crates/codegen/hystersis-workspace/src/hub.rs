...
use url::Url;
use hystersis_computer_hub_sdk::{
...
use hystersis_tool_protocol::ToolId;
...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        session_handler_resolver: Option<hystersis_computer_hub_sdk::SessionHandlerResolver>,
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(hystersis_version::VERSION)
            .image_capabilities(
...
struct CallCompletedGuard {
    ...
    outcome: hystersis_session_events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: hystersis_session_events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: hystersis_session_events::ToolOutcome) {
...
impl ToolServerHandler for SessionRoutedToolHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let hub_session = ctx
            .extensions
            .get::<hystersis_tool_runtime::SessionContext>()
            .map(|s| s.0.clone());
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        tool = %self.name(),
                        session = %session_id,
                        "HYSTERSIS_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    ToolStreamItem::Terminal(Ok(run_result)) => {
                        ...
                        _guard.set_outcome(hystersis_session_events::ToolOutcome::Success);
                    ...
                    ToolStreamItem::Terminal(Err(e)) => {
                        ...
                        _guard.set_outcome(hystersis_session_events::ToolOutcome::Error);
                        let e = match &virt {
...
fn parse_server_id(id: &str) -> Result<hystersis_tool_protocol::ServerId, ClientError> {
    hystersis_tool_protocol::ServerId::new(id)
...
mod tests {
    ...
    use futures::StreamExt;
    use hystersis_tool_runtime::{SessionContext, ToolCallId};
    ...
    async fn handle_call_terminal_matches_non_streaming_call() {
        ...
        match reference {
            Ok(run_result) => {
                ...
                assert_eq!(
                    typed.model_output,
                    vec![hystersis_tool_runtime::ContentBlock::Text {
    ...
    use hystersis_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    impl hystersis_tool_runtime::Tool for GateStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> hystersis_tool_protocol::ToolId {
            hystersis_tool_protocol::ToolId::new("gate_streaming_stub").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::hystersis_tool_runtime::ListToolsContext,
        ) -> hystersis_tool_types::ToolDescription {
            hystersis_tool_types::ToolDescription::new("gate_streaming_stub", "gate streaming stub")
        }
        async fn run(
            &self,
        ...
        async fn execute(
            ...
            use hystersis_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    async fn drain_counts<T>(mut stream: hystersis_tool_runtime::ToolStream<T>) -> (usize, usize, bool) {
        let mut progress = 0;
        let mut terminal = 0;
    ...
    use hystersis_tools::notification::types::{ToolNotification, ToolNotificationHandle};
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("HystersisBuild:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    async fn wait_until(
        ...
        pred: impl Fn(&hystersis_tool_protocol::ToolServerStatusPayload) -> bool,
        timeout: Duration,
    ) -> hystersis_tool_protocol::ToolServerStatusPayload {
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use hystersis_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use hystersis_tools::computer::types::{TaskKind, TaskSnapshot};
    ...
    impl hystersis_tool_runtime::Tool for PathEchoStub {
        type Args = serde_json::Value;
        type Output = serde_json::Value;
        fn id(&self) -> hystersis_tool_protocol::ToolId {
            hystersis_tool_protocol::ToolId::new("path_echo").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::hystersis_tool_runtime::ListToolsContext,
        ) -> hystersis_tool_types::ToolDescription {
            hystersis_tool_types::ToolDescription::new("path_echo", "path echo stub")
        }
        async fn run(
            &self,
            _ctx: hystersis_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            input: serde_json::Value,
        ) -> hystersis_tool_runtime::ToolStream<serde_json::Value> {
            use hystersis_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    async fn drain_terminal(
        stream: hystersis_tool_runtime::ToolStream<hystersis_tool_runtime::TypedToolOutput>,
    ) -> (
        Vec<hystersis_tool_runtime::ToolProgress>,
    ...
    async fn handle_call_virtualizes_inbound_outbound_and_progress() {
        ...
        match progress.first() {
            Some(hystersis_tool_runtime::ToolProgress::Text { text }) => {
    ...
    impl hystersis_tool_runtime::Tool for PathErrorStub {
        type Args = serde_json::Value;
        type Output = serde_json::Value;
        fn id(&self) -> hystersis_tool_protocol::ToolId {
            hystersis_tool_protocol::ToolId::new("path_error").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::hystersis_tool_runtime::ListToolsContext,
        ) -> hystersis_tool_types::ToolDescription {
            hystersis_tool_types::ToolDescription::new("path_error", "path error stub")
        }
        async fn run(
            &self,
            _ctx: hystersis_tool_runtime::ToolCallContext,
            ...
            Err(hystersis_tool_runtime::ToolError::new(
                hystersis_tool_runtime::ToolErrorKind::Execution,
    ...
    impl hystersis_tool_runtime::Tool for BashCcoPathStub {
        type Args = serde_json::Value;
        type Output = hystersis_tools::types::output::ToolOutput;
        fn id(&self) -> hystersis_tool_protocol::ToolId {
            hystersis_tool_protocol::ToolId::new("bash_cco_path").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::hystersis_tool_runtime::ListToolsContext,
        ) -> hystersis_tool_types::ToolDescription {
            hystersis_tool_types::ToolDescription::new("bash_cco_path", "bash cco path stub")
        }
        async fn run(
            &self,
            _ctx: hystersis_tool_runtime::ToolCallContext,
        ...
        ) -> Result<hystersis_tools::types::output::ToolOutput, hystersis_tool_runtime::ToolError>
        {
            ...
            Ok(hystersis_tools::types::output::ToolOutput::Bash(
                hystersis_tools::types::output::BashOutput {
                    ...
                    output_for_prompt:
                        hystersis_tools::types::output::BashOutput::make_output_for_prompt(stdout),
                    exit_code: 0,
