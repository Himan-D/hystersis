...
//!
//! Persists per-folder trust decisions to `~/.hystersis/trusted_folders.toml`.
...
//!
//! The store is rooted at [`hystersis_config::user_hystersis_home`] — the **Option**
//! home that resolves to `None` (rather than a cwd-relative `./.hystersis`) when
//! neither `$HYSTERSIS_HOME` nor a home directory is set (e.g. a minimal container /
...
//! a `./.hystersis/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.hystersis/`.
...
impl TrustStore {
    /// Load the trust store from `<user_hystersis_home>/trusted_folders.toml`.
    ///
    ...
    /// Default on-disk path: `<user_hystersis_home>/trusted_folders.toml`, or `None`
    /// when no user home resolves.
    ///
    /// Resolves via [`hystersis_config::user_hystersis_home`], never
    /// [`hystersis_config::hystersis_home`], so it never falls back to a cwd-relative
    /// `./.hystersis` — that fallback would let an untrusted cloned repo's `.hystersis`
    /// masquerade as the user-global store and self-trust the checkout.
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(hystersis_config::user_hystersis_home())
    }

    /// Map a resolved user-hystersis-home to the store path, preserving "no home" as
    ...
    fn default_path_in(user_hystersis_home: Option<PathBuf>) -> Option<PathBuf> {
        Some(user_hystersis_home?.join(TRUST_FILE_NAME))
    }
    ...
    fn record_decision(&mut self, workspace_key: &Path, trusted: bool) -> io::Result<()> {
        ...
        let Some(path) = self.path.as_deref() else {
            tracing::warn!(
                path = %canonical.display(),
                trusted,
                "folder trust: no user hystersis home resolved; trust decision not recorded"
            );
            return Ok(());
        };
...
///
/// A hystersis-managed worktree first collapses onto its recorded source repo's git
/// ROOT (via the `~/.hystersis/worktrees.db` registry), so every `hystersis -w` worktree
...
/// `hystersis -w` was launched from (the recorded source repo may be a repo subdir).
...
/// `hystersis -w` worktree of a repo shares one trust key. The collapse fires ONLY
...
fn git_derived_workspace_key(cwd: &Path) -> PathBuf {
    // A hystersis-managed worktree (any creation mode, incl. standalone clones git
    ...
    if let Some(source_repo) = crate::worktree::source_repo_for_cwd(&cwd.to_string_lossy()) {
        ...
        // ONE key regardless of the subdir hystersis -w was launched from (parity with
...
/// sharing `~/.hystersis/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.hystersis/trusted-hook-projects` file listed one canonical project
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = hystersis_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
/// is testable without the process-global hystersis-home cache. Returns the number
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match hystersis_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.hystersis");
        assert_eq!(
        ...
        // cwd-relative `./.hystersis` that hystersis_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.hystersis/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
        assert_eq!(TrustStore::default_path_in(None), None);
    }

    #[test]
    fn default_path_sources_from_user_hystersis_home() {
        // Thin source-pin: the production accessor reads user_hystersis_home()
        // (Option, no cwd fallback), not hystersis_home(). The real regression guard
        // is the seam test above (default_path_in(None) == None).
        assert_eq!(
            TrustStore::default_path(),
            hystersis_config::user_hystersis_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
    ...
    fn workspace_key_collapses_linked_worktrees_onto_main_checkout() {
        // Every linked `hystersis -w` worktree of a repo must share ONE trust key:
    ...
    // ── workspace_key registry collapse (hystersis-managed worktrees) ─────────
    ...
    /// Point `HYSTERSIS_HOME` at an isolated tempdir and register one hystersis-managed
    ...
    /// dir)`; the [`LockedTestEnv`] holds the lock and restores `HYSTERSIS_HOME` on
    /// drop (before releasing the lock), so the caller may bind it any way.
    fn register_hystersis_worktree(
        temp: &tempfile::TempDir,
        name: &str,
        source_repo: &Path,
        creation_mode: &str,
    ) -> (LockedTestEnv, PathBuf) {
        use hystersis_fast_worktree::{WorktreeDb, WorktreeKind, WorktreeRecord, WorktreeStatus};
    ...
    #[test]
    fn workspace_key_collapses_standalone_hystersis_worktree_onto_source_repo() {
        ...
        let (_env, wt) = register_hystersis_worktree(&temp, "wt", &source_repo, "standalone");

        let expected = canonicalize_or_owned(&source_repo);
        assert_eq!(
            workspace_key(&wt),
            expected,
            "a standalone hystersis worktree must collapse onto its recorded source repo"
        );
    ...
    fn workspace_key_collapses_worktree_onto_source_repo_git_root() {
        ...
        let (_env, wt) = register_hystersis_worktree(&temp, "wt", &subdir, "standalone");
    ...
    fn workspace_key_ignores_registry_for_cwd_outside_worktrees_dir() {
        ...
        // `<hystersis_home>/worktrees`: `worktree_record_for_cwd` skips the registry
        ...
        // worktree cwd, and `outside` is its OWN git repo (under hystersis HOME but not
        ...
        let (_env, _wt) = register_hystersis_worktree(&temp, "wt", &source_repo, "standalone");

        // Under hystersis HOME but NOT under `<home>/worktrees`, and its own git repo.
        let outside = root.join("hystersis-home").join("not-worktrees").join("proj");
        ...
        assert_eq!(
            ...
            "a cwd outside <hystersis_home>/worktrees keys on its own repo root"
        );
        assert_ne!(
            key,
