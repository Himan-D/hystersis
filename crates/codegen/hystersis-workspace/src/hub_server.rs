...
use hystersis_computer_hub_sdk::ToolServerHandler;
use hystersis_tools::computer::types::KillOutcome;
use hystersis_tools::computer::types::TaskKind;
use hystersis_tools::implementations::hystersis_build::scheduler::interval::interval_to_human;
use hystersis_tools::implementations::hystersis_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use hystersis_tools::registry::types::FinalizedToolset;
use hystersis_tools::types::resources::Terminal;
...
use hystersis_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "hystersis_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "hystersis_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_REQUESTS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "hystersis_workspace_rpc_requests_total",
...
static WORKSPACE_RPC_ERRORS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "hystersis_workspace_rpc_errors_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "hystersis_workspace_rpc_duration_seconds",
...
) -> Vec<hystersis_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use hystersis_tools::computer::types::TaskKind;
    use hystersis_tools::types::resources::Terminal;
    use hystersis_tools::types::tool::ToolKind;
    use hystersis_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &hystersis_tools::registry::types::FinalizedToolset,
) -> Vec<hystersis_workspace_types::rpc::workspace::TodoSummaryWire> {
    use hystersis_tools::implementations::hystersis_build::todo::{TodoState, TodoStatus};
    use hystersis_tools::types::resources::State;
    use hystersis_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use hystersis_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use hystersis_workspace_types::rpc::presence::PresenceNoteReq;
        use hystersis_workspace_types::rpc::search::FuzzyStatusReq;
        use hystersis_workspace_types::rpc::skills::DiscoverPluginsReq;
        use hystersis_workspace_types::rpc::workspace::{
        ...
        use hystersis_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
        ...
        match method {
            <WorkspaceInfoReq as WorkspaceRpc>::METHOD => {
                ...
                let info = WorkspaceInfo {
                    ...
                    version: Some(hystersis_version::VERSION.to_owned()),
                };
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<hystersis_tool_runtime::SessionContext>();
    ...
    async fn handle_hook(&self, session_id: SessionId, frame: HookFrame) {
        match frame.event {
            ...
            HookEvent::Custom { kind, payload } => {
                use hystersis_tool_protocol::turn_hook::{
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use hystersis_tool_protocol::turn_hook::{self, TurnHookRequest};
