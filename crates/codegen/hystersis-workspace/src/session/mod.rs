...
use hystersis_computer_hub_mcp_adapter::McpBridgeHandle;
use hystersis_mcp::servers::McpState;
use hystersis_tools::notification::AcknowledgedToolNotification;
use hystersis_tools::notification::types::ToolNotificationHandle;
use hystersis_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
use hystersis_hunk_tracker::HunkTrackerHandle;
use hystersis_tool_protocol::ToolId;
use hystersis_tool_runtime::WorkspaceViewerContext;
...
pub struct WorkspaceSession {
    ...
    ///
    /// [`HunkTrackerActor`]: hystersis_hunk_tracker::HunkTrackerActor
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, hystersis_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn set_cwd_for_virtualization(&self, cwd: PathBuf) -> Result<(), String> {
        ...
        res.insert(hystersis_tools::types::resources::Cwd(cwd));
        Ok(())
    }
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<hystersis_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<hystersis_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<hystersis_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<hystersis_hooks::error::HookError>,
    ...
    pub(crate) auth_provider: Option<hystersis_computer_hub_sdk::SharedAuthProvider>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<hystersis_tools::notification::types::ToolNotificationHandle>>,
    /// Sink for workspace-originated ext-notifications to the client (e.g.
    /// `hystersis.com/search/fuzzy/status`). Mode-agnostic: the shell wires it to the
    ...
    pub(crate) local_registry: hystersis_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn hystersis_tools::implementations::lsp::LspBackend>>,
    ...
    /// (from `HYSTERSIS_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$HYSTERSIS_WORKSPACE_HOME` — the workspace-owned on-disk state root
    /// (`<hystersis_home>/workspace` by default). The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<hystersis_file_utils::queue::UploadQueue>>,
    ...
    /// (`HYSTERSIS_WORKSPACE_EVENTS_ENABLED=true`). When `false`, every
    /// [`session_event_writer`](Self::session_event_writer) hands back an
    /// [`EventWriter::noop()`](hystersis_session_events::EventWriter::noop) and
    ...
    /// enabled (`HYSTERSIS_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, hystersis_session_events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<hystersis_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<hystersis_file_utils::queue::UploadQueue>> {
    ...
    /// [`EventWriter::noop()`](hystersis_session_events::EventWriter::noop)
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> hystersis_session_events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<hystersis_session_events::EventWriter> {
    ...
    pub fn auth_provider(&self) -> Option<&hystersis_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](hystersis_computer_hub_sdk::ToolServer)
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<hystersis_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, hystersis_session_events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> hystersis_session_events::EventWriter {
    use hystersis_session_events::EventWriter;
    if !enabled {
    ...
    let create = hystersis_config::create_dir_all_owner_only(&dir);
    hystersis_config::set_dir_owner_only(&sessions_root);
    if let Err(e) = create {
        tracing::warn!(
...
mod tests {
    ...
    use dashmap::DashMap;
    use hystersis_session_events::{Event, EventWriter};
    ...
    /// Session-derived content outside ~/.hystersis/sessions: same owner-only rule,
