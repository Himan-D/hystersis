...
#[tokio::test]
async fn get_worktree_info_standalone_hystersis_marker() {
    let tmp = tempfile::tempdir().unwrap();
    ...
    std::fs::write(
        clone.join(".git").join("hystersis-worktree-source"),
        main.display().to_string(),
    )
    .unwrap();
    assert!(clone.join(".git").is_dir());
    assert!(clone.join(".git").join("hystersis-worktree-source").is_file());
...
fn register_db_worktree(home: &Path, wt: &Path, source: &Path, label: &str) {
    let db = hystersis_fast_worktree::db::WorktreeDb::open(home).unwrap();
...
fn get_worktree_info_db_record_without_marker() {
    let tmp = tempfile::tempdir().unwrap();
    let home = dunce::canonicalize(tmp.path()).unwrap().join("hystersis-home");
    std::fs::create_dir_all(&home).unwrap();
    let _env = crate::LockedTestEnv::lock().set("HYSTERSIS_HOME", &home);
...
fn test_effective_worktree_cwd_empty_offset() {
    let result = effective_worktree_cwd("/home/user/.hystersis/worktrees/repo/ab-123-a", Path::new(""));
    assert_eq!(result, "/home/user/.hystersis/worktrees/repo/ab-123-a");
}

#[test]
fn test_effective_worktree_cwd_single_level_offset() {
    let result =
        effective_worktree_cwd("/home/user/.hystersis/worktrees/repo/ab-123-a", Path::new("src"));
    assert_eq!(result, "/home/user/.hystersis/worktrees/repo/ab-123-a/src");
}

#[test]
fn test_effective_worktree_cwd_nested_offset() {
    let result = effective_worktree_cwd(
        "/home/user/.hystersis/worktrees/repo/ab-123-b",
        Path::new("packages/frontend/src"),
    );
    assert_eq!(
        result,
        "/home/user/.hystersis/worktrees/repo/ab-123-b/packages/frontend/src"
    );
}
...
fn test_effective_cwd_roundtrip_with_compute_offset() {
    ...
    let worktree_root = "/home/user/.hystersis/worktrees/myrepo/ab-test-a";
