...
///
/// ```
/// use hystersis_workspace::session::git::normalize_repo_url;
///
/// assert_eq!(
...
/// Payload for the `hystersis.com/git_head_changed` ACP extension notification.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub const RESTORE_CODE_LOG: &str = "hystersis_restore_code";
...
///
/// 1. `supplied_cwd` is a hystersis-managed worktree (`~/.hystersis/worktrees/...`).
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = hystersis_tools::util::hystersis_home::hystersis_home().join("worktrees");
...
const REWIND_GIT_ENV: &str = "HYSTERSIS_WORKSPACE_REWIND_GIT";
...
pub fn git_rewind_enabled() -> bool {
    hystersis_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
...
const DEFAULT_EXCLUDES_MARKER: &str = "hystersis default excludes";
...
# hystersis default excludes (local-only; seeded by the workspace git_commit op)
.hystersis/
node_modules/
.env
.env.*
...
async fn seed_default_gitignore(git_root: &Path) -> Result<()> {
    ...
    tokio::fs::write(&path, hystersis_workspace_types::binding::DEFAULT_GITIGNORE).await?;
