...
use hystersis_computer_hub_mcp_adapter::{
...
use hystersis_mcp::servers::{McpClient, parse_mcp_qualified_name};
use hystersis_tool_protocol::ToolId;
use hystersis_tool_runtime::{ToolCallContext, ToolStream, TypedToolOutput};
...
impl McpTransport for McpClientTransportAdapter {
    async fn initialize(&self) -> Result<McpServerInfo, hystersis_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
            .map_err(|e| hystersis_computer_hub_mcp_adapter::McpError::Transport(e.to_string()))?;
        let info = service.peer_info().ok_or_else(|| {
            hystersis_computer_hub_mcp_adapter::McpError::Transport("no peer info after init".into())
        })?;
        Ok(McpServerInfo {
    ...
    async fn list_tools(
        &self,
    ) -> Result<Vec<McpToolDefinition>, hystersis_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn call_tool(
        ...
        name: &str,
        arguments: Value,
    ) -> Result<McpCallResult, hystersis_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn close(&self) -> Result<(), hystersis_computer_hub_mcp_adapter::McpError> {
...
/// Extract a server name from an [`McpError`](hystersis_mcp::servers::McpError),
/// falling back to `"unknown"`.
pub(crate) fn server_name_from_mcp_error(e: &hystersis_mcp::servers::McpError) -> &str {
    e.server_name().unwrap_or("unknown")
}

/// Bridge config factory for MCP bridge connections.
pub(crate) fn make_bridge_config(
    session_id: hystersis_tool_protocol::SessionId,
    server_name: &str,
) -> McpBridgeConfig {
    McpBridgeConfig {
        session_id,
...
mod tests {
    use super::*;
    use hystersis_computer_hub_mcp_adapter::{McpBridge, McpError};
