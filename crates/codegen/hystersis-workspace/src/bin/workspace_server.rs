...
//!
//! Reads OIDC credentials from `~/.hystersis/auth.json`, connects to a
...
use url::Url;
use hystersis_diag_server::{self as diag_server, DiagHandle, ErrorClass};
use hystersis_workspace::config::WorkspaceServerMetadata;
use hystersis_workspace::error::WorkspaceError;
use hystersis_workspace_daemon::daemonize;
use hystersis_workspace_daemon::preview_supervisor::{
...
const SERVICE_NAME: &str = "prod_hystersis_workspace";
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<hystersis_tool_protocol::ServerId>()
        .err()
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    #[arg(long, default_value = "wss://computer-hub.hystersis.com/v1/tools")]
    hub_url: String,
    #[arg(long)]
    ...
    /// `HYSTERSIS_WORKSPACE_UPLOAD_QUEUE_ENABLED` env var to `false`) to fall back to
    /// the legacy inline path. Accepts `true`/`false`.
    #[arg(
        long,
        env = "HYSTERSIS_WORKSPACE_UPLOAD_QUEUE_ENABLED",
    ...
    /// Trust project-scoped LSP servers from `<repo>/.hystersis/lsp.json`.
    /// Defaults off; sandbox opts in only after workspace trust is established.
    #[arg(
        long,
        env = "HYSTERSIS_WORKSPACE_PROJECT_LSP_TRUSTED",
        default_value_t = false,
        action = clap::ArgAction::Set,
    )]
    project_lsp_trusted: bool,
    /// Confine `hystersis.com/fs/*` resolution to the workspace root (reject `..`,
    ...
    /// Override with `HYSTERSIS_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "HYSTERSIS_WORKSPACE_CONFINE_FS_TO_ROOT",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    /// to -900, and force `HYSTERSIS_TOOLS_RESET_CHILD_OOM` so shell/pty children
...
/// `/usr/local/bin/xai-hystersis-preview-proxy` child (see `cli.rs` for the proxy's
...
///
/// `xai-hystersis-workspace-daemon` deliberately does not depend on
/// `xai-hystersis-workspace`, so this binary owns the one adapter between them.
struct TrackerSink(std::sync::Arc<hystersis_workspace::activity::ActivityTracker>);
...
fn main() -> anyhow::Result<()> {
    ...
    let oom_protection = hystersis_tty_utils::protect_from_oom_kill();
    ...
    if should_set_reset_child_oom(oom_protection.is_ok(), args.oom_protect) {
        unsafe { std::env::set_var(hystersis_tty_utils::RESET_CHILD_OOM_ENV, "1") };
    }
    let mut builder = tokio::runtime::Builder::new_multi_thread();
    builder
        .worker_threads(hystersis_tty_utils::runtime::capped_worker_threads().get())
        .enable_all();
    let rt = hystersis_tty_utils::runtime::build_with_blocking_pool(&mut builder)?;
    rt.block_on(run(args, cwd, oom_protection, oom_protect_applied))
}
/// Whether to arm `HYSTERSIS_TOOLS_RESET_CHILD_OOM` after the always-on protect attempt.
///
...
async fn run(
    ...
    let donating = hystersis_computer_hub_sdk::DonatingLogLayer::new_inert();
    ...
    let direct_otlp = match std::env::var("HYSTERSIS_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match hystersis_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
    ...
    {
        use hystersis_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("HYSTERSIS_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(value = %val,
                        "Unrecognized HYSTERSIS_SANDBOX_PROFILE, defaulting to workspace");
                    ProfileName::Workspace
                } else {
                    parsed
                }
            }
            None if hystersis_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let profile_name = profile.to_string();
        if profile == ProfileName::Off {
            tracing::info!(profile = %profile_name, "Sandbox explicitly disabled via HYSTERSIS_SANDBOX_PROFILE=off");
        } else {
            ...
            sandbox.install();
            let active = hystersis_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = hystersis_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let mut status_config = hystersis_workspace::StatusConfig::from_env();
    status_config.preview_control_port = args.preview.preview_control_port;
    let auth_provider = hystersis_workspace::hub_auth::provider(
        &url,
    ...
    let session_id = std::env::var("HYSTERSIS_SESSION_ID").ok();
    ...
    {
        let caps = hystersis_workspace::image_capabilities::image_capabilities();
    ...
    hystersis_workspace::init_metrics();
    let ws_handle = match hystersis_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = hystersis_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            hystersis_workspace::handle::DrainReason::Sigterm,
        )
        .await;
    ...
    hystersis_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn classify_from_client_error_display_round_trip() {
        let handshake = WorkspaceError::HubError(
            hystersis_computer_hub_sdk::ClientError::HandshakeAuthFailed { status: 401 }.to_string(),
        );
        ...
        let auth = WorkspaceError::HubError(
            hystersis_computer_hub_sdk::ClientError::AuthError("token rejected".into()).to_string(),
        );
        assert_eq!(
        ...
        let network = WorkspaceError::HubError(
            hystersis_computer_hub_sdk::ClientError::NetworkError("connection refused".into())
                .to_string(),
        );
        assert_eq!(
    ...
    fn project_lsp_trust_defaults_off_and_is_opt_in() {
        unsafe { std::env::remove_var("HYSTERSIS_WORKSPACE_PROJECT_LSP_TRUSTED") };
        let args = Args::try_parse_from(["xai-workspace-server"]).unwrap();
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        let args = Args::try_parse_from([
            ...
            "https://hystersis.com/preview-auth",
        ...
        assert_eq!(
            ...
            Some("https://hystersis.com/preview-auth")
        );
    ...
    /// scraper's own behavior is tested in `xai-hystersis-workspace-daemon`, and the
    /// tracker's in `hystersis_workspace::activity`; this covers the seam.
    #[test]
    fn tracker_sink_forwards_every_signal_to_the_activity_tracker() {
        use hystersis_workspace::activity::ActivityTracker;
