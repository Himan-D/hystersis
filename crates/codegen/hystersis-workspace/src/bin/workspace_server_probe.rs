...
//! access token from `~/.hystersis/auth.json` does (same identity).
...
use url::Url;
use uuid::Uuid;
use hystersis_computer_hub_sdk::pool::HubConnectionPool;
use hystersis_computer_hub_sdk::{AuthCredential, ToolHarnessBuilder};
...
async fn call_tool(
    harness: &hystersis_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<hystersis_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "HystersisBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
