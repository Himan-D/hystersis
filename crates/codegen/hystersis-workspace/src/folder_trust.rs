...
use hystersis_config_types::{BoolFlag, RemoteSettings};
...
fn is_local_build() -> bool {
    ...
    if std::env::var(hystersis_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
...
fn feature_enabled_for_build(remote: Option<&RemoteSettings>, is_local_build: bool) -> bool {
    ...
    let user = hystersis_config::load_from_disk().ok();
    let managed = hystersis_config::load_managed_config().ok();
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = hystersis_agent::repo::RepoDirChain::resolve(cwd);
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = hystersis_config::load_config_file(&path) else {
            continue;
        };
    ...
    if !hystersis_agent::plugins::project_plugin_dirs_in(&chain.dirs).is_empty() {
        hit!("plugins");
    }
    ...
    if !hystersis_agent::discovery::project_agent_dirs_in(&chain.dirs).is_empty() {
        hit!("agents");
    }
...
mod tests {
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(hystersis_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn is_local_build_honors_test_version_override() {
        ...
        {
            let _sim = EnvVarGuard::set(hystersis_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
        ...
        let _unset = EnvVarGuard::unset(hystersis_version::TEST_VERSION_ENV);
