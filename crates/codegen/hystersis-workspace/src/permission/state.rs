...
use hystersis_tools::util::hystersis_home::hystersis_home;
...
fn permission_scope_root(cwd: &AbsPathBuf) -> std::path::PathBuf {
    match hystersis_agent::repo::RepoDirChain::resolve(cwd.as_path()).git_root {
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    hystersis_config::sessions_cwd_dir(&permission_scope_root(cwd).to_string_lossy())
}
...
fn legacy_state_dir(cwd: &AbsPathBuf, dir: &std::path::Path) -> Option<std::path::PathBuf> {
    let legacy = hystersis_config::sessions_cwd_dir(cwd.as_str());
...
async fn resolve_store_dirs(cwd: &AbsPathBuf, ensure: bool) -> StoreDirs {
    let cwd = cwd.clone();
    let fallback_dir = hystersis_config::sessions_cwd_dir(cwd.as_str());
    tokio::task::spawn_blocking(move || {
        ...
        let dir = if ensure {
            ...
            hystersis_config::ensure_sessions_cwd_dir(&root).unwrap_or_else(|e| {
                tracing::warn!(?e, "failed ensuring sessions cwd dir for permission state");
                hystersis_config::sessions_cwd_dir(&root)
            })
        } else {
            hystersis_config::sessions_cwd_dir(&root)
        };
...
async fn try_load_state(path: &std::path::Path) -> Option<PermissionState> {
    try_load_state_with_writer(path, |path, contents| {
        hystersis_config::fs_atomic::write_atomically(path, contents, None)
    })
    .await
}
...
async fn persist_state_to_dir(
    ...
    let result = persist_state_to_path_with_writer(&path, state, move |path, contents| {
        hystersis_config::create_dir_all_owner_only(&dir)?;
        hystersis_config::fs_atomic::write_atomically(path, contents, None)
    })
    .await;
    if let Err(e) = result {
...
"#,
    ...
    fn scope_root_outside_a_repo_stays_the_cwd() {
        ...
        if hystersis_agent::repo::RepoDirChain::resolve(&dir)
            .git_root
            .is_none()
        {
