...
/// `hystersis_config::hystersis_home()` is a process-wide `OnceLock`, so under
...
"#,
    ...
    let global_live = hystersis_config::user_hystersis_home()
...
async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
    ...
    let layers = hystersis_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
    ...
    .unwrap();
    assert_eq!(
        hystersis_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
        1
    );
