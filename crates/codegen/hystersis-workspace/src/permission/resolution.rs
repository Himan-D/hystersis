...
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    hystersis_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    if let Some(global_path) = hystersis_config::user_hystersis_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match hystersis_config::load_config_file(&global_path) {
    ...
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match hystersis_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[hystersis_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
    ...
    fn live(project_trusted: bool) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &hystersis_config::managed_config_layers(),
            ),
            project_trusted,
        }
    }
...
/// [`hystersis_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = hystersis_config::claude_managed_settings_path() else {
...
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = hystersis_config::requirements_layers();
