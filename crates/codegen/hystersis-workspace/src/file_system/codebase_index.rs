...
use hystersis_codebase_graph::{IndexManager, IndexManagerConfig, IndexManagerHandle};

use hystersis_tools::util::hystersis_home::hystersis_home;

/// Get the cache path for a cwd's index.
///
/// Cache is stored in: `~/.hystersis/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `hystersis.com/code/status` reporting `reason: notStarted`.
    ...
    /// In `hystersis.com/code/status`, `resolve_index_handle()` calls
