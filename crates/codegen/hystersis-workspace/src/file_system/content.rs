...
pub use hystersis_workspace_types::rpc::search::{
...
fn build_ripgrep_command(root: &Path, params: &ContentSearchParams) -> Command {
    ...
    hystersis_tty_utils::detach_search_command(&mut cmd);
    ...
    F: Fn(ContentSearchBatch) + Send + 'static,
{
    ...
    if hit_limit {
        ...
        hystersis_tools::util::reap_killed_search_child(&mut child).await;
    } else {
