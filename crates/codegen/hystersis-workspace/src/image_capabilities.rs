...
//! files under `/usr/share/hystersis/capabilities.d/<token>`.
//!
//! ADVISORY ONLY: never an authorization input. Guest processes run as root
//! and can create files here at will. Deliberately NOT under `/etc/hystersis`,
...
/// `/usr/share/hystersis/bundled-skills`).
pub const DEFAULT_CAPABILITIES_DIR: &str = "/usr/share/hystersis/capabilities.d";
/// Override for tests / local dev / non-Linux hosts.
pub const CAPABILITIES_DIR_ENV: &str = "HYSTERSIS_IMAGE_CAPABILITIES_DIR";
/// Re-exported so the reader and the bind reply that carries its output
/// cannot drift on the spelling.
pub use hystersis_tool_protocol::IMAGE_CAPABILITIES_V1;
// The gate and the caps live beside the wire field they bound: every hop that
// validates the set must apply identical rules.
use hystersis_tool_protocol::{MAX_IMAGE_CAPABILITIES, is_image_capability_token};
...
mod tests {
    ...
    use std::fs;
    use std::path::Path;
    use hystersis_tool_protocol::MAX_IMAGE_CAPABILITY_LEN;
    ...
    fn missing_dir_is_unknown() {
        ...
        assert_eq!(caps.state("hystersis-files.occ"), None);
        assert!(!caps.has("hystersis-files.occ"));
    }
    ...
    fn self_token_makes_the_read_authoritative() {
        ...
        touch(tmp.path(), "hystersis-files.occ");
        let caps = load(tmp.path());
        assert!(caps.is_declared());
        assert_eq!(caps.state("hystersis-files.occ"), Some(true));
        assert!(caps.has("hystersis-files.occ"));
        // Declared-and-absent, not unknown.
        assert_eq!(caps.state("vercel.cli"), Some(false));
        assert!(!caps.has("vercel.cli"));
        assert_eq!(caps.wire(), ["capabilities.v1", "hystersis-files.occ"]);
    }
    ...
    fn only_plausible_tokens_are_noteworthy() {
        for name in ["Grove.Daemon", "notes.txt", "hystersis_files.occ", "trailing."] {
