...
//! run) — and can be forced with `HYSTERSIS_VOICE_CAPTURE=inprocess`.
...
/// Env escape hatch: `HYSTERSIS_VOICE_CAPTURE=inprocess` forces the legacy
/// in-process cpal backend (accepting its permanent footprint cost).
const CAPTURE_BACKEND_ENV: &str = "HYSTERSIS_VOICE_CAPTURE";
