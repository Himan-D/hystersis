...
/// `[endpoints].hystersis_api_base_url` so enterprise proxies need no second knob.
...
impl Default for VoiceConfig {
    fn default() -> Self {
        Self {
            api_base: "https://api.hystersis.com".into(),
...
impl VoiceConfig {
    ...
    /// `api_base`: non-empty `[voice].api_base`, else `[endpoints].hystersis_api_base_url`
    /// from `root`, else `resolved_endpoints_base`, else `https://api.hystersis.com`.
    ///
    ...
    pub fn from_config_table(root: &toml::Table, resolved_endpoints_base: Option<&str>) -> Self {
        ...
        // makes "unset" and an explicit `https://api.hystersis.com` indistinguishable.
        ...
        .or_else(|| {
            non_empty_str(
                root.get("endpoints")
                    .and_then(|e| e.get("hystersis_api_base_url"))
                    .and_then(|v| v.as_str()),
            )
        })
...
mod tests {
    ...
    fn default_stt_ws_uses_wss() {
        assert_eq!(
            ...
            "wss://api.hystersis.com/v1/stt"
        );
    }

    #[test]
    fn scheme_less_and_wss_bases() {
        for base in ["api.hystersis.com", "wss://api.hystersis.com", "HTTPS://api.hystersis.com"] {
            let cfg = VoiceConfig {
                api_base: base.into(),
                ..VoiceConfig::default()
            };
            assert_eq!(cfg.stt_ws_url().unwrap(), "wss://api.hystersis.com/v1/stt");
        }
    }
    ...
    #[test]
    fn hystersis_v1_base_preserves_prefix() {
        let cfg = VoiceConfig {
    ...
    fn inherits_endpoints_when_voice_api_base_unset() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
hystersis_api_base_url = "https://proxy.example.com/xai/v1"
"#,
        )
        .unwrap();
...
"#,
    ...
    fn table_endpoints_beat_resolved_endpoints_base() {
        let table: toml::Table = toml::from_str(
            r#"
[endpoints]
hystersis_api_base_url = "https://config.example.com"
"#,
        )
        .unwrap();
...
hystersis_api_base_url = "https://proxy.example.com/xai/v1"
[voice]
api_base = "https://api.hystersis.com"
language = "es"
"#,
        )
        .unwrap();
        let cfg = VoiceConfig::from_config_table(&table, None);
        assert_eq!(cfg.api_base, "https://api.hystersis.com");
