...
//! `hystersis_sampler::shared_http` (one HTTP/2 pooled client plus
...
//!
//! TLS policy (backend pin, roots, provider) lives in `hystersis_extra_ca`.

use std::sync::OnceLock;

use hystersis_workspace::permission::ClientType;
...
///
/// Replaces `hystersis_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `hystersis_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use hystersis_telemetry::instrumentation::{
...
// the same type via `hystersis_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use hystersis_sampler::OriginClientInfo;
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn hystersis_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(hystersis_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
pub fn shared_startup_blocking_client() -> reqwest::blocking::Client {
    ...
    BLOCKING_CLIENT
        .get_or_init(|| {
            ...
            hystersis_extra_ca::build_blocking_reqwest_client(|builder| {
                builder
