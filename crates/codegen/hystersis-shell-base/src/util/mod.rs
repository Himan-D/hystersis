pub mod changelog;
pub mod event_id;
pub mod hystersis_home;
pub mod secure_file;
pub mod tips;
pub mod uname;
pub use hystersis_shared::clipboard;
pub use hystersis_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// toggles via `HYSTERSIS_CLI_CHAT_PROXY_BASE_URL` (or similar) pointing at an
...
/// True for xAI-operated endpoints (`*.hystersis.com`, cli-chat-proxy, and optional
...
/// exempt. Safe against invalid URLs and suffix attacks (`evil-hystersis.com.example`).
///
/// Scheme-agnostic so credential *refusal* fails closed. To decide where to
/// *attach* a credential, use [`is_hystersis_api_bearer_url`].
pub fn is_hystersis_api_url(url: &str) -> bool {
    is_hystersis_api_url_impl(url, false)
}
/// Like [`is_hystersis_api_url`], but requires `https` on every arm, so a
...
pub fn is_hystersis_api_bearer_url(url: &str) -> bool {
    if is_trusted_hystersis_https_url(url) {
        return true;
    }
    false
}
/// True for trusted first-party xAI HTTPS routes, excluding arbitrary loopback URLs.
pub fn is_trusted_hystersis_https_url(url: &str) -> bool {
    ...
    parsed
        .host_str()
        .is_some_and(|host| host == "hystersis.com" || host.ends_with(".hystersis.com"))
}
fn is_hystersis_api_url_impl(url: &str, require_https: bool) -> bool {
    if require_https {
        return is_hystersis_api_bearer_url(url);
    }
...
/// True if `pid` is a hystersis process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_hystersis_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("hystersis"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
/// Stricter [`is_hystersis_process`] for the auto-kill zombie path: on macOS/BSD it
...
/// exactly, so this delegates there. Use the permissive [`is_hystersis_process`] for
/// operator-driven `hystersis leaders kill`.
pub fn is_hystersis_process_strict(pid: u32) -> bool {
    #[cfg(all(not(target_os = "linux"), not(windows)))]
    {
        ...
        match cmd.output() {
            Ok(out) if out.status.success() => {
                ...
                comm.lines()
                    ...
                    .is_some_and(|name| name.to_ascii_lowercase().contains("hystersis"))
            }
            _ => false,
        }
    }
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_accepts_proxy_subpath() {
        assert!(is_cli_chat_proxy_url(
            "https://cli-chat-proxy.hystersis.com/v1/chat/completions"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_public_api() {
        assert!(!is_cli_chat_proxy_url("https://api.hystersis.com/v1"));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.hystersis.com.evil.example/v1"
        ));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_v11_prefix_confusion() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.hystersis.com/v11/chat/completions"
        ));
    }
    #[test]
    fn test_is_hystersis_api_url() {
        assert!(is_hystersis_api_url("https://api.hystersis.com/v1"));
        assert!(is_hystersis_api_url("https://api.hystersis.com/v1/chat/completions"));
        assert!(is_hystersis_api_url("https://hystersis.com"));
        assert!(is_hystersis_api_url(
            "https://cli-chat-proxy.hystersis.com/v1/chat/completions"
        ));
        assert!(!is_hystersis_api_url("https://api.openai.com/v1"));
        assert!(!is_hystersis_api_url("https://api.anthropic.com/v1"));
        assert!(!is_hystersis_api_url("https://generativelanguage.googleapis.com"));
        assert!(!is_hystersis_api_url("https://api.hystersis.com.evil.example/v1"));
        assert!(!is_hystersis_api_url("https://evil-hystersis.com.attacker.com/v1"));
        assert!(!is_hystersis_api_url("https://prefixhystersis.com/v1"));
        assert!(!is_hystersis_api_url("not-a-url"));
        assert!(!is_hystersis_api_url(""));
        assert!(is_hystersis_api_url("http://api.hystersis.com/v1"));
        assert!(is_hystersis_api_url("http://localhost:11434/v1"));
    }
    #[test]
    fn test_is_hystersis_api_bearer_url() {
        assert!(is_hystersis_api_bearer_url("https://api.hystersis.com/v1"));
        assert!(!is_hystersis_api_bearer_url("http://api.hystersis.com/v1"));
        assert!(!is_hystersis_api_bearer_url("http://localhost:11434/v1"));
        {
            assert!(!is_hystersis_api_bearer_url("https://localhost:11434/v1"));
            assert!(!is_hystersis_api_bearer_url("https://127.0.0.2:11434/v1"));
            assert!(!is_hystersis_api_bearer_url("https://[::1]:11434/v1"));
        }
        assert!(is_hystersis_api_bearer_url("https://API.X.AI/v1"));
        assert!(!is_hystersis_api_bearer_url(
            "https://api.hystersis.com@attacker.example/v1"
        ));
        assert!(!is_hystersis_api_bearer_url("https://х.ai/v1"));
    }
    #[test]
    fn test_truncate() {
    ...
    #[test]
    fn is_hystersis_process_self_true_impossible_pid_false() {
        assert!(is_hystersis_process(std::process::id()));
        assert!(!is_hystersis_process(u32::MAX));
    }
    #[test]
    fn is_hystersis_process_strict_self_true_impossible_pid_false() {
        assert!(is_hystersis_process_strict(std::process::id()));
        assert!(!is_hystersis_process_strict(u32::MAX));
    }
    #[cfg(unix)]
    #[test]
