...
fn registered_settings() {
    ...
    assert_eq!(
        rows,
        BTreeMap::from([
            ("session_search", ("HYSTERSIS_SESSION_SEARCH", true)),
            ("lsp_tools", ("HYSTERSIS_LSP_TOOLS", false)),
            ("web_fetch", ("HYSTERSIS_WEB_FETCH", false)),
            ("session_recap", ("HYSTERSIS_SESSION_RECAP", true)),
            ("ask_user_question", ("HYSTERSIS_ASK_USER_QUESTION", true)),
            ("voice_mode", ("HYSTERSIS_VOICE_MODE", true)),
            ("write_file", ("HYSTERSIS_WRITE_FILE", true)),
            ("feedback", ("HYSTERSIS_FEEDBACK_ENABLED", true)),
            ("feedback_trace_card", ("HYSTERSIS_FEEDBACK_TRACE_CARD", false)),
            ("turn_summary", ("HYSTERSIS_TURN_SUMMARY", true)),
            ("cancel_rewind", ("HYSTERSIS_CANCEL_REWIND", true)),
            (
                "compaction_verbatim_input",
                ("HYSTERSIS_COMPACTION_VERBATIM_INPUT", true),
            ),
            ("two_pass_compaction", ("HYSTERSIS_TWO_PASS_COMPACTION", true)),
            ("backend_tools", ("HYSTERSIS_BACKEND_SEARCH", true)),
            ("auto_wake", ("HYSTERSIS_AUTO_WAKE", true)),
            (
                "subagent_worktree_snapshot",
                ("HYSTERSIS_SUBAGENT_WORKTREE_SNAPSHOT", false),
            ),
        ]),
    );
}
...
fn off_reason_names_the_setting_that_turned_it_off() {
    ...
    for (sources, reason) in [
        ...
        (
            ...
            "the HYSTERSIS_SESSION_SEARCH environment variable",
        ),
        (
            FeatureSources {
                config: Some(false),
