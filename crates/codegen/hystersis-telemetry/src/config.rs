...
//!
//! Extracted from `xai-hystersis-shell::agent::config` so the data-collector
...
mod telemetry_mode_tests {
    ...
    /// `HYSTERSIS_TELEMETRY_ENABLED={mode}` (Display), so every Display output
...
pub struct TelemetryConfig {
    ...
    /// External OTEL master switch (`= HYSTERSIS_EXTERNAL_OTEL`, env wins).
...
/// Fork-isolated defaults: no baked endpoint, telemetry off unless env enables it.
...
impl Default for TelemetryConfig {
    fn default() -> Self {
        // Default mode is Disabled (Off) unless HYSTERSIS_TELEMETRY_ENABLED env overrides.
        // Endpoints are None unless build-time env or runtime env (apply_env_overrides) sets them.
        let (baked_url, baked_key, baked_token, baked_enabled) = internal_defaults();
        let build_url = build_env_default(option_env!("HYSTERSIS_TELEMETRY_BUILD_EVENTS_URL"));
        let build_key = build_env_default(option_env!("HYSTERSIS_TELEMETRY_BUILD_EVENTS_API_KEY"));
        let build_token = build_env_default(option_env!("HYSTERSIS_TELEMETRY_BUILD_MIXPANEL_TOKEN"));
...
impl TelemetryConfig {
    pub fn apply_env_overrides(&mut self) {
        self.normalize();
        if let Some(value) = Self::env_override("HYSTERSIS_TELEMETRY_EVENTS_URL")
            .or_else(|| Self::env_override("HYSTERSIS_TELEMETRY_ENDPOINT"))
        {
            self.events_url = value;
        }
        if let Some(value) = Self::env_override("HYSTERSIS_TELEMETRY_EVENTS_API_KEY") {
            self.events_api_key = value;
        }
        if let Some(value) = Self::env_override("HYSTERSIS_TELEMETRY_MIXPANEL_TOKEN") {
            self.mixpanel_token = value;
        }
        if let Some(value) = env_bool("HYSTERSIS_TELEMETRY_MIXPANEL_ENABLED") {
            self.mixpanel_enabled = value;
        }
        if let Some(value) = env_bool("HYSTERSIS_TELEMETRY_TRACE_UPLOAD") {
            self.trace_upload = Some(value);
        }
        // Endpoint is env-configurable (fork-isolated): HYSTERSIS_TELEMETRY_OTEL_ENDPOINT wins,
        // with HYSTERSIS_OTEL_ENDPOINT and generic HYSTERSIS_TELEMETRY_ENDPOINT as fallbacks.
        if let Some(value) = Self::env_override("HYSTERSIS_TELEMETRY_OTEL_ENDPOINT")
            .or_else(|| Self::env_override("HYSTERSIS_OTEL_ENDPOINT"))
            .or_else(|| Self::env_override("OTEL_EXPORTER_OTLP_ENDPOINT"))
        {
            self.otel_endpoint = value;
        }
    }
    fn normalize(&mut self) {
        ...
        self.otel_endpoint = Self::normalize_optional_string(self.otel_endpoint.take());
    }
...
///
/// Local copy of `hystersis_shell::agent::config::env_bool` so this crate
...
mod tests {
    ...
    fn default_is_build_env_layer_when_feature_off() {
        ...
        let url = build_env_default(option_env!("HYSTERSIS_TELEMETRY_BUILD_EVENTS_URL"));
        let key = build_env_default(option_env!("HYSTERSIS_TELEMETRY_BUILD_EVENTS_API_KEY"));
        let token = build_env_default(option_env!("HYSTERSIS_TELEMETRY_BUILD_MIXPANEL_TOKEN"));
