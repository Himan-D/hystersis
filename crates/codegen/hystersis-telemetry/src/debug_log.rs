...
use hystersis_config::hystersis_home;
...
// until added here.
const FIREHOSE_BASE_DIRECTIVES: &str = "info,hystersis_pager=debug,hystersis_shell=debug,hystersis_tools=debug,hystersis_telemetry=debug,hystersis_agent=debug,hystersis_mcp=debug,hystersis_session_search=debug,hystersis_acp_lib=debug,sampling_log=off";
...
mod tests {
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "hystersis_shell", "inside session");
            });
            tracing::info!(target: "hystersis_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `hystersis_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "hystersis_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "hystersis_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "hystersis_shell", "one first");
                tracing::info!(target: "hystersis_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "hystersis_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
