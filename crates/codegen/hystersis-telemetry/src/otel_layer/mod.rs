...
//!
//! `xai-hystersis-pager` uses this module to set up OTLP
...
const ENV_OTEL_FILTER: &str = "HYSTERSIS_OTEL_FILTER";
...
/// (`AuthManager`, `EndpointsConfig`, `HystersisComConfig`).
///
/// Built by the binaries (`xai-hystersis-pager`) from their own
/// configuration. The credentials provider is constructed by shell's
/// `hystersis_shell::auth::credential_provider::build_otel_credential_provider`.
...
pub struct OtelClientInfo {
    /// Binary name (`hystersis-pager`) -> `client.name`.
...
pub struct OtelExporterConfig {
    /// Full OTLP traces endpoint URL (e.g. `https://cli-chat-proxy.hystersis.com/v1/traces`).
...
///
/// - `client_name`: binary name (e.g. `"hystersis-tui"`, `"hystersis-pager"`) -- stored as
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let otel_filter = tracing_subscriber::filter::EnvFilter::try_new(&otel_filter)
        .unwrap_or_else(|e| {
            eprintln!(
                "[otel] Invalid HYSTERSIS_OTEL_FILTER '{}': {}. Using default '{}'.",
...
fn build_export_headers(
    ...
    snapshot: &hystersis_auth::CredentialSnapshot,
...
mod tests {
    ...
    use std::sync::Mutex;
    use hystersis_auth::{AuthCredentialProvider, CredentialSnapshot, HttpAuth};
