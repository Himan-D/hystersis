...
//!
//! Extracted from `xai-hystersis-shell::agent::telemetry`.
...
pub enum EmitterOrigin {
    /// `xai-hystersis-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `xai-hystersis-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "hystersis-shell-",
            EmitterOrigin::Workspace => "hystersis-workspace-",
        }
    }
}
...
/// event-name prefix (`hystersis-shell-*` vs `hystersis-workspace-*`).
///
...
mod tests {
    ...
    /// What a command exiting right after emitting (`hystersis login`) relies on.
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "hystersis-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "hystersis-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical
    /// `format!("hystersis-shell-{suffix}")` event name byte-for-byte, since every
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("hystersis-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "hystersis-workspace-turn");
    }
