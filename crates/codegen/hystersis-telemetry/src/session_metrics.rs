...
pub enum TraceUploadReason {
    ...
    FeatureOff,
    /// No hystersis.com auth or deployment key.
    NoCredentials,
    /// Direct-to-bucket S3 upload.
    DirectS3,
    /// Proxy mode via hystersis.com auth.
    Proxy,
...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<hystersis_file_utils::UploadMethod>) -> Self {
        match method {
            Some(hystersis_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(hystersis_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(hystersis_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
#[cfg(test)]
mod tests {
    use hystersis_file_utils::UploadMethod;

    use super::TraceUploadReason;

    /// The `hystersis-shell-doom_loop_recovery` Mixpanel event's name and
