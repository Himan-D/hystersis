...
//!
//! Provides a dedicated tracing target (`hystersis_memory`) with an optional
//! file logger that writes to `~/.hystersis/logs/memory.log`.
//!
//! ## When to use
//!
...
//! HYSTERSIS_MEMORY_LOG=0 hystersis                # disable even when enabled
//! tail -f ~/.hystersis/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "hystersis_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "HYSTERSIS_MEMORY_LOG";
    ...
    ///
    /// Writes to `~/.hystersis/logs/memory.log`. Filters to `hystersis_memory=trace`.
    /// Set `HYSTERSIS_MEMORY_LOG=0` to disable, `HYSTERSIS_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || hystersis_home().join("logs").join("memory.log");
