...
//!
//! Extracted from `xai-hystersis-shell::agent::telemetry::track`. The HTTP client is
...
use serde_json::json;
use hystersis_mixpanel::Mixpanel;
...
/// whichever [`EmitterOrigin`] prefix it carries (`hystersis-shell-` /
/// `hystersis-workspace-`). Unprefixed names pass through unchanged. Kept in
...
/// format for Mixpanel. Handles both CCP display names ("HystersisPro Heavy")
/// and JWT-derived keys ("hystersispro_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "HystersisPro Heavy" | "hystersispro_heavy" => "hystersispro_heavy",
        "HystersisPro Plus" | "hystersispro_plus" => "hystersispro_plus",
        "HystersisPro" | "hystersispro" => "hystersispro",
        "HystersisPro Lite" | "hystersispro_lite" => "hystersispro_lite",
...
impl EventEnrichment {
    fn capture() -> Self {
        ...
        Self {
            ...
            dev_build: hystersis_version::IS_DEV_BUILD,
...
pub async fn track(event_name: &str, request_id: &str, ctx: &UserContext, mut metadata: Metadata) {
    ...
    if let Some(ref mixpanel) = client.mixpanel {
        ...
        props.insert("app_name".into(), json!("Hystersis Code"));
...
mod tests {
    ...
    /// identical to the previous `strip_prefix("hystersis-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("hystersis-shell-turn"), "turn");
        assert_eq!(
            event_value("hystersis-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("hystersis-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("hystersis-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("HystersisPro"), "hystersispro");
        assert_eq!(normalize_tier("HystersisPro Heavy"), "hystersispro_heavy");
        assert_eq!(normalize_tier("hystersispro_heavy"), "hystersispro_heavy");
        ...
        assert_eq!(normalize_tier("HystersisPro Lite"), "hystersispro_lite");
        assert_eq!(normalize_tier("HystersisPro Plus"), "hystersispro_plus");
        assert_eq!(normalize_tier("hystersispro_plus"), "hystersispro_plus");
    ...
    /// (e.g. a future `"hystersis-shell-ext-"` origin), an earlier `ALL` entry could
