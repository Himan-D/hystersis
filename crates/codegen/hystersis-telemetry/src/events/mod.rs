...
//!
//! These structs were extracted from `xai-hystersis-shell` so they can be
...
pub enum ContextualTipKind {
    ...
    WordSelect,
    /// SSH session without `hystersis wrap` → tip to wrap the ssh command locally.
    SshWrap,
}
...
///
/// Volume caveat: one-shot `hystersis update` resolves telemetry from disk+env
...
pub enum CliUpdateTrigger {
    /// A human ran `hystersis update` or accepted an update prompt.
    UserCommand,
...
impl CompactionScope {
    pub fn begin(params: CompactionBeginParams) -> Self {
        ...
        let percentage = hystersis_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct SubagentCompleted {
    ...
    // `hystersis_code_subagent_spawn_*` taxonomy); absent when a phase did not run.
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: hystersis_session_events::types::ToolOutcome,
    pub duration_ms: u64,
    #[serde(skip_serializing_if = "Option::is_none")]
...
// ---------------------------------------------------------------------------
// Pager events (called from xai-hystersis-pager via log_event)
// ---------------------------------------------------------------------------
...
#[derive(Serialize)]
pub struct HystersisProUpsellShown {
    pub source: HystersisProUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct HystersisProUpsellClicked {
    pub source: HystersisProUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`HystersisProUpsell`]; lets the funnel attribute the click to the
...
pub struct ClipboardCopy {
    ...
    /// An explicit `hystersis wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
/// is lifted — i.e. they subscribed (externally on hystersis.com) and came back.
/// This is the actual conversion signal for HystersisPro Heavy subscriptions
/// attributed to Hystersis: the user saw the gate in Hystersis, went and
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Hystersis's upsell surface.
...
/// `hystersis-shell-manual_auth` (the `manual_auth` binding gets the `hystersis-shell-`
...
/// One attempt to download + activate a new `hystersis` binary. Analytics name:
/// `hystersis-shell-cli_update`. Emitted on failure too; failures carry the
...
// ─────────────────────────────────────────────────────────────────────────────
// hystersis clone (utility process)
// ─────────────────────────────────────────────────────────────────────────────
...
/// Whether `hystersis clone` finished the mount.
#[derive(Serialize, Clone, Copy, Debug, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Where a failed `hystersis clone` stopped. Closed set — no freeform strings.
#[derive(Serialize, Clone, Copy, Debug, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// One `hystersis clone` attempt. Content-free: no URL, dest, store, or repo name.
#[derive(Serialize, Debug, Clone, PartialEq)]
...
telemetry_event!(HystersisProUpsellShown, "hystersispro_upsell_shown");
telemetry_event!(HystersisProUpsellClicked, "hystersispro_upsell_clicked");
...
mod tests {
    ...
    fn compaction_completed_name_and_shape() {
        ...
        let with_model = serde_json::to_value(CompactionCompleted {
            ...
            tokens_after: 15_000,
            model_id: Some("hystersis-4".into()),
            compaction_id: "cid-1".into(),
            compaction_mode: CompactionModeLabel::Summary,
