...
use hystersis_telemetry::client;
use hystersis_telemetry::config::{TelemetryConfig, TelemetryMode};
...
async fn manual_auth_posts_to_events_endpoint_as_hystersis_shell_manual_auth() {
    ...
    hystersis_telemetry::log_event(ManualAuth {
    ...
    for (key, expected) in [
        ...
        (
            "dev_build",
            serde_json::json!(hystersis_version::IS_DEV_BUILD),
        ),
    ...
    for key in hystersis_telemetry::client::RESERVED_EVENT_KEYS {
        assert!(
