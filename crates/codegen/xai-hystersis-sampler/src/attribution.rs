...
//!
//! This crate stays decoupled from `xai-hystersis-shell`: no shell types, no
//! auth-manager dependency.

use std::sync::Arc;

pub use xai_hystersis_auth::bearer_fragment::BEARER_SUFFIX_LEN;
...
/// Sampler endpoints only — tool clients use `xai_hystersis_tools::ToolConsumer`.
