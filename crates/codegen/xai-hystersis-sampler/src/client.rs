...
use xai_hystersis_sampling_types::error::{
...
pub use xai_hystersis_sampling_types::ApiBackend;

/// Process-level fallback for the `x-grok-client-identifier` header.
const DEFAULT_CLIENT_IDENTIFIER: &str = "hystersis-shell";

/// Product identifier baked into User-Agent strings.
const AGENT_PRODUCT: &str = "hystersis-shell";
const ANTHROPIC_DEFAULT_MAX_TOKENS: u32 = 128_000;

/// Per-request `x-hystersis-*` headers. Optional fields are skipped when empty/`None`.
struct HystersisRequestHeaders<'a> {
    conv_id: &'a str,
    req_id: &'a str,
    model_id: &'a str,
    session_id: &'a str,
...
impl HystersisRequestHeaders<'_> {
    fn apply(&self, builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
        let mut b = builder
            .header("x-hystersis-conv-id", self.conv_id)
            .header("x-hystersis-req-id", self.req_id)
            .header("x-hystersis-model-override", self.model_id)
            .header("x-hystersis-session-id", self.session_id)
            .header("x-hystersis-agent-id", self.agent_id);
        if let Some(idx) = self.turn_idx {
            b = b.header("x-hystersis-turn-idx", idx);
        }
        if let Some(id) = self.deployment_id.filter(|s| !s.is_empty()) {
            b = b.header("x-hystersis-deployment-id", id);
        }
        if let Some(id) = self.user_id.filter(|s| !s.is_empty()) {
            b = b.header("x-hystersis-user-id", id);
        }
        b
    }
}
...
fn apply_terminal_event_overrides(event: &mut rs::ResponseStreamEvent, data: &str) {
    ...
    if let Some(ticks) = xai_hystersis_sampling_types::reported_cost_ticks(
        value
...
fn extract_model_metadata(headers: &reqwest::header::HeaderMap) -> Option<ResponseModelMetadata> {
    let context_window = headers
        .get("x-hystersis-context-window")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<u64>().ok());

    let max_completion_tokens = headers
        .get("x-hystersis-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
struct ClientDefaults {
    ...
    doom_loop_recovery: Option<xai_hystersis_sampling_types::DoomLoopRecoveryPolicy>,
}
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        {
            headers.insert(
                HeaderName::from_static("x-hystersis-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-hystersis-user-id"), header_value);
        }
    ...
    pub async fn chat_completion(
        ...
        let x_hystersis_conv_id = &payload.x_hystersis_conv_id.clone().unwrap_or_default();
        let x_hystersis_req_id = &payload.x_hystersis_req_id.clone().unwrap_or_default();
        ...
        let hystersis_headers = HystersisRequestHeaders {
            conv_id: x_hystersis_conv_id,
            req_id: x_hystersis_req_id,
            model_id: &model_id,
            session_id: payload.x_hystersis_session_id.as_deref().unwrap_or_default(),
            turn_idx: payload.x_hystersis_turn_idx.as_deref(),
            agent_id: payload.x_hystersis_agent_id.as_deref().unwrap_or_default(),
            deployment_id: payload.x_hystersis_deployment_id.as_deref(),
            user_id: payload.x_hystersis_user_id.as_deref(),
        };
        let SentRequest {
            builder,
            sent_bearer,
        } = self.post(self.endpoint("chat/completions"));
        let http_request = hystersis_headers.apply(builder).json(&payload);
    ...
    pub async fn chat_completion_stream(
        ...
        let http_request = hystersis_headers
            .apply(builder)
    ...
    pub async fn create_response(
        ...
        let x_hystersis_conv_id = request.x_hystersis_conv_id.as_deref().unwrap_or_default();
        let x_hystersis_req_id = request.x_hystersis_req_id.as_deref().unwrap_or_default();
        ...
        let hystersis_headers = HystersisRequestHeaders {
            ...
            req_id: x_hystersis_req_id,
            model_id: &model_id,
            session_id: request.x_hystersis_session_id.as_deref().unwrap_or_default(),
            turn_idx: request.x_hystersis_turn_idx.as_deref(),
            agent_id: request.x_hystersis_agent_id.as_deref().unwrap_or_default(),
            deployment_id: request.x_hystersis_deployment_id.as_deref(),
            user_id: request.x_hystersis_user_id.as_deref(),
        };
        ...
        xai_hystersis_sampling_types::patch_reasoning_text_types(&mut request_body);
        let SentRequest {
            builder,
            sent_bearer,
        } = self.post(self.endpoint("responses"));
        let http_request = hystersis_headers.apply(builder).json(&request_body);
    ...
    /// gate that adds the opt-in `x-hystersis-doom-loop-check` request header, so
    ...
    pub async fn create_response_stream(
        ...
        let mut http_request = hystersis_headers
            .apply(builder)
            .header(ACCEPT, HeaderValue::from_static("text/event-stream"));
    ...
    pub async fn create_message(
        ...
        let http_request = hystersis_headers.apply(builder).json(&request.inner);
    ...
    pub async fn conversation_stream_responses(
        ...
        let trace = request.trace.take();
        let x_hystersis_conv_id = request.x_hystersis_conv_id.clone();
        let x_hystersis_req_id = request.x_hystersis_req_id.clone();
        let x_hystersis_session_id = request.x_hystersis_session_id.clone();
        let x_hystersis_turn_idx = request.x_hystersis_turn_idx.clone();
        let x_hystersis_agent_id = request.x_hystersis_agent_id.clone();
        ...
        let extra_tools = xai_hystersis_sampling_types::extra_tool_entries(&request.hosted_tools);

        let responses_request: rs::CreateResponse = (&request).into();

        let mut wrapper = CreateResponseWrapper::new(responses_request);
        wrapper.x_hystersis_conv_id = x_hystersis_conv_id;
        wrapper.x_hystersis_req_id = x_hystersis_req_id;
        wrapper.x_hystersis_session_id = x_hystersis_session_id;
        wrapper.x_hystersis_turn_idx = x_hystersis_turn_idx;
        wrapper.x_hystersis_agent_id = x_hystersis_agent_id;
...
mod tests {
    ...
    use xai_hystersis_sampling_types::ApiErrorCode;
    use xai_hystersis_sampling_types::types::ChatRequestMessage;
    ...
    fn stream_collect_error_preserves_should_retry() {
        let info = SamplingErrorInfo {
            ...
            credential: xai_hystersis_sampling_types::SentCredential::Unknown,
        };
    ...
    fn streaming_chat_request_serializes_correctly() {
        let request = ChatCompletionRequest {
            ...
            reasoning_effort: None,
            x_hystersis_conv_id: None,
            x_hystersis_req_id: None,
    ...
    fn user_agent_omits_origin_version_when_absent() {
        ...
        let ua = user_agent_string_for(&origin);
        // No slash between product and the hystersis-shell agent product.
        assert!(ua.starts_with("my-client hystersis-shell/"));
    }

    #[test]
    fn user_agent_collapses_when_origin_matches_agent() {
        let agent_version = xai_hystersis_version::VERSION.to_string();
        let origin = OriginClientInfo {
            product: AGENT_PRODUCT.to_string(),
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "hystersis", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
