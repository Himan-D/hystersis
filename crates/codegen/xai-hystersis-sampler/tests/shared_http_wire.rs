...
use xai_hystersis_sampler::SamplingClient;
use xai_hystersis_test_support::spawn_counting_server;

/// Pin the env these assertions depend on before any client is built, so
/// ambient shell exports (`HYSTERSIS_SAMPLER_SHARED_CLIENT=0`,
/// `HYSTERSIS_POOL_MAX_IDLE=0`) cannot flip the expected pooling behavior.
fn pin_env() {
    ...
    PIN.call_once(|| {
        ...
        unsafe {
            std::env::remove_var("HYSTERSIS_SAMPLER_SHARED_CLIENT");
            std::env::set_var("HYSTERSIS_POOL_MAX_IDLE", "2");
            std::env::set_var("HYSTERSIS_POOL_IDLE_TIMEOUT_SECS", "90");
        }
    });
}
