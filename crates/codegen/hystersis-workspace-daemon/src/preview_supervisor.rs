...
//! spawns the unchanged `/usr/local/bin/xai-hystersis-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/xai-hystersis-preview-proxy";
...
/// `hystersis_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "hystersis_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `xai-hystersis-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`HYSTERSIS_SERVER_KEY` /
/// `HYSTERSIS_SESSION_ID`) reach the proxy by env inheritance — never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT hystersis_tty_utils::detach_command: the proxy must stay in
        ...
        let oom_protect = std::env::var_os(hystersis_tty_utils::RESET_CHILD_OOM_ENV).is_some();
        ...
        unsafe {
            cmd.pre_exec(move || {
                ...
                if oom_protect {
                    ...
                    hystersis_tty_utils::reset_oom_score_adj()?;
                }
                Ok(())
            });
        }
...
/// `xai-hystersis-preview-proxy`'s `/__control/activity` route).
...
/// the default in `xai-hystersis-preview-proxy/src/cli.rs`).
...
/// than the concrete tracker so this crate stays free of `xai-hystersis-workspace`:
...
pub async fn supervise_preview_metrics(control_port: Option<u16>, shutdown: watch::Receiver<bool>) {
    scrape_metrics_loop(
        ...
        shutdown,
        |body| {
            if let Some(sink) = hystersis_computer_hub_sdk::metric_donate::active_metrics_sink() {
...
mod tests {
    ...
    /// accounting is covered by `hystersis_workspace::activity`, and the wiring
    /// between the two by the `xai-workspace-server` binary's own tests.
    struct TestSink {
        window_ms: u64,
    ...
    fn sample_cfg() -> PreviewArgs {
        PreviewArgs {
            ...
            auth_redirect: Some("https://hystersis.com/preview-auth".to_owned()),
            allow_public: true,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match xai-hystersis-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "HYSTERSIS_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
