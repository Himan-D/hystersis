...
use xai_hystersis_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use xai_hystersis_pager::client_identity::PAGER_CLIENT_VERSION;
use xai_hystersis_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use xai_hystersis_shell::agent::config::Config as AgentConfig;
use xai_hystersis_shell::leader::{
...
use xai_hystersis_telemetry::process_info::{
...
fn process_identity(command: Option<&Command>, is_interactive: bool) -> Option<ProcessIdentity> {
    use xai_hystersis_telemetry::process_info::LeaderMode::Standalone;
...
use std::env;
use xai_hystersis_update::{UpdateConfig, auto_update, enforce_version_policy_or_exit};
...
fn apply_agent_endpoint_args(
    agent_args: &xai_hystersis_pager::app::AgentArgs,
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use xai_hystersis_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(xai_hystersis_telemetry::sampling_log::layer())
        .with(xai_hystersis_telemetry::instrumentation::layer())
        .with(xai_hystersis_telemetry::hooks_log::layer())
        .with(xai_hystersis_telemetry::otel_layer::build_otel_layer(
            xai_hystersis_telemetry::otel_layer::OtelClientInfo {
                client_name: "grok-pager",
                client_version: xai_hystersis_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            xai_hystersis_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    xai_hystersis_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    xai_hystersis_telemetry::external::init(
        xai_hystersis_shell::agent::config::resolve_external_otel_config(
            xai_hystersis_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: xai_hystersis_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `grok setup`: rendering + exit codes only; fetch logic lives in `xai_hystersis_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use xai_hystersis_shell::managed_config::{self, SetupOutcome};
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = xai_hystersis_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !xai_hystersis_shell::util::is_hystersis_process(pid) {
        ...
        if let Err(e) = xai_hystersis_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(xai_hystersis_shell::env::HystersisBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, xai_hystersis_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = xai_hystersis_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = xai_hystersis_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&xai_hystersis_shell::leader::ControlPayload>,
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&xai_hystersis_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<xai_hystersis_shell::util::config::RemoteSettings> {
    join_early_prefetch(xai_hystersis_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = xai_hystersis_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
...
async fn workspace_control(
    ...
    let agent_config = xai_hystersis_shell::config::load_agent_config_disk_only()
...
async fn workspace_start(
    ...
    use xai_hystersis_shell::auth::ensure_authenticated;
    xai_hystersis_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = xai_hystersis_shell::config::load_effective_config()
    ...
    let (use_leader, _) = resolve_use_leader(
        ...
        true,
        xai_hystersis_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    xai_hystersis_telemetry::sentry::flush_on_shutdown();
    xai_hystersis_telemetry::otel_layer::shutdown_otel();
    xai_hystersis_telemetry::debug_log::flush();
...
async fn run_agent_command(
    agent_args: Box<xai_hystersis_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use xai_hystersis_pager::app::signal_handler::next_signal_code;
    ...
    if matches!(
        ...
        xai_hystersis_shell::agent::app::suppress_otel();
    }
    init_tracing_simple("agent");
    let _otel_guard = xai_hystersis_telemetry::otel_layer::otel_guard();
    ...
    xai_hystersis_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => xai_hystersis_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(error = %e, "--trust: failed to resolve cwd; folder not trusted")
            }
        }
    }
    let early_prefetch = xai_hystersis_shell::agent::models::start_early_prefetch(None);
    xai_hystersis_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Grok Build (pager) - v{}",
            xai_hystersis_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                xai_hystersis_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(xai_hystersis_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = xai_hystersis_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = xai_hystersis_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
        xai_hystersis_shell::util::config::PermissionMode::Ask,
    );
    ...
    agent_config.resolve_runtime_fields(&xai_hystersis_shell::agent::config::RuntimeResolutionContext {
    ...
    let requested_confinement = xai_hystersis_sandbox::requested_confinement_profile();
    let LeaderMode {
        use_leader,
    ...
    use xai_hystersis_telemetry::process_info::LeaderMode::{Attached, Standalone};
    ...
    let managed_install = is_managed_install(
        ...
        &xai_hystersis_shell::util::hystersis_home::hystersis_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = xai_hystersis_shell::leader::LeaderEnvUrls::from(&agent_config.hystersis_com_config);
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = xai_hystersis_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            xai_hystersis_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(xai_hystersis_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = xai_hystersis_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = xai_hystersis_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !xai_hystersis_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<xai_hystersis_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(xai_hystersis_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<xai_hystersis_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(xai_hystersis_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    xai_hystersis_shell::heap_profile::install(xai_hystersis_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "grok {}\n",
        xai_hystersis_version::display_version_with_commit(
            xai_hystersis_version::full_version(),
            channel_label,
        )
    )
}
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = xai_hystersis_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    xai_hystersis_version::set_full_version(env!("VERSION_WITH_COMMIT"));
    xai_hystersis_telemetry::startup::mark_process_start();
    if let Some(code) = xai_hystersis_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = xai_hystersis_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    set_release_channel(ReleaseChannel::from_label(
        xai_hystersis_update::channel_name().unwrap_or_default(),
    ));
    ...
    xai_hystersis_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    xai_hystersis_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        xai_hystersis_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        xai_hystersis_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    xai_hystersis_pager::memory_trace::start(xai_hystersis_pager::memory_trace::default_dir());
    raise_fd_limit();
    if let Err(e) = xai_hystersis_config::validate_requirements() {
    ...
    let _sentry_guard = xai_hystersis_telemetry::sentry::init(xai_hystersis_telemetry::sentry::Config {
        client: "grok-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: xai_hystersis_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    xai_hystersis_pager::docs::extract_user_guide_docs(&xai_hystersis_shell::util::hystersis_home::hystersis_home());
    xai_crash_handler::install_terminal_restore_only();
    if xai_hystersis_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = xai_hystersis_shell::util::hystersis_home::hystersis_home().join("crash");
    ...
    let crashed = xai_hystersis_active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    if let Err(e) = result {
        ...
        match e.downcast_ref::<xai_hystersis_pager::app::StartupFailure>() {
...
async fn async_main(args: PagerArgs) -> Result<()> {
    xai_hystersis_extra_ca::ensure_default_crypto_provider();
    ...
    if args.chat() {
        unsafe {
            std::env::set_var(xai_hystersis_shell::agent::chat_modes::HYSTERSIS_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(xai_hystersis_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    ...
    if let Some(Command::Completions { shell }) = &args.command {
        xai_hystersis_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return xai_hystersis_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        xai_hystersis_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        xai_hystersis_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    xai_hystersis_shell::config::apply_sandbox(
        None,
    ...
    xai_hystersis_shell::http::set_client_name(if is_interactive {
        xai_hystersis_workspace::permission::ClientType::HystersisPager
    } else {
        xai_hystersis_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": xai_hystersis_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
            ...
            Command::Inspect { json } => {
                ...
                xai_hystersis_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return xai_hystersis_pager::mcp_cmd::run(mcp_args).await;
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
                ...
                return xai_hystersis_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = xai_hystersis_telemetry::otel_layer::otel_guard();
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = xai_hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_hystersis_pager::models::list_available_models(&agent_config).await;
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = xai_hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_hystersis_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
                ...
                return xai_hystersis_pager::disk_usage_cmd::run(disk_usage_args);
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = xai_hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_hystersis_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = xai_hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_hystersis_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return xai_hystersis_pager::export_cmd::run(export_args);
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = xai_hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return xai_hystersis_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return xai_hystersis_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = xai_hystersis_pager::configure_cmd::run().await;
                ...
                let config = xai_hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_hystersis_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                xai_hystersis_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = xai_hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                xai_hystersis_shell::auth::run_cli_logout(&config)?;
                xai_hystersis_shell::instrumentation::finalize_and_exit(0);
            }
...
Command::Dashboard => {
            ...
            Command::Configure => {
                xai_hystersis_pager::configure_cmd::run().await?;
                return Ok(());
            }
        }
    }
    let headless_prompt = xai_hystersis_pager::headless::HeadlessPrompt::from_args(
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(xai_hystersis_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == xai_hystersis_pager::headless::OutputFormat::Plain
        {
            args.output_format = xai_hystersis_pager::headless::OutputFormat::Json;
        }
        return xai_hystersis_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            xai_hystersis_pager::headless::HeadlessOptions {
    ...
    let result = xai_hystersis_pager::app::run(args, bg_update_rx).await;
    xai_hystersis_sandbox::flush();
    match result {
        Ok(true) => {
...
fn build_update_config() -> UpdateConfig {
    let environment = xai_hystersis_shell::env::HystersisBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                xai_hystersis_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("GROK_NPM_REGISTRY"))
        .ok()
        .or_else(xai_hystersis_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = xai_hystersis_shell::config::load_effective_config_disk_only()
        && let Some(ch) = xai_hystersis_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn is_managed_install(exe: Option<std::path::PathBuf>, hystersis_home: &std::path::Path) -> bool {
    ...
    let managed = xai_hystersis_config::hystersis_application_in(hystersis_home);
...
async fn run_update_command(
    ...
    let telemetry_cfg = xai_hystersis_shell::config::load_agent_config_disk_only()
        .map_err(|e| tracing::warn!("grok update: telemetry init skipped (agent config: {e})"))
        .ok();
    if let Some(agent_cfg) = telemetry_cfg {
        let auth_manager = std::sync::Arc::new(xai_hystersis_shell::auth::AuthManager::new(
            &xai_hystersis_shell::util::hystersis_home::hystersis_home(),
            agent_cfg.hystersis_com_config.clone(),
        ));
        xai_hystersis_shell::agent::init::update_telemetry_config(&agent_cfg, &auth_manager);
    }
    ...
    xai_hystersis_telemetry::session_ctx::drain_pending(xai_hystersis_telemetry::session_ctx::CLI_DRAIN)
        .await;
    result?;
    Ok(())
}
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in xai_hystersis_shell::leader::discover_leaders().await {
        if d.classification != xai_hystersis_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match xai_hystersis_shell::leader::LeaderClient::connect(
            socket_path,
            "grok-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(xai_hystersis_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(xai_hystersis_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            xai_hystersis_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: xai_hystersis_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            xai_hystersis_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!xai_hystersis_shell::heap_profile::prof_available());
            return;
        }
        assert!(xai_hystersis_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(xai_hystersis_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(xai_hystersis_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        xai_hystersis_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    ...
    fn workspace_command_gate_resolution() {
        use xai_hystersis_shell::util::config::RemoteSettings;
