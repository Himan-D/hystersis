//! `hystersis update` is a recovery command: a config failure must not block it.
//!
...
fn pager_binary() -> std::path::PathBuf {
    ...
    option_env!("CARGO_BIN_EXE_xai-hystersis-pager")
...
/// Run `hystersis update` in an isolated home against the local pointer base.
fn run_update(base: &str, config_toml: &str, extra_args: &[&str]) -> std::process::Output {
    ...
    Command::new(pager_binary())
        ...
        .env_clear()
        .env("HOME", home.path())
        .env("HYSTERSIS_HOME", home.path())
        .env("PATH", std::env::var("PATH").unwrap_or_default())
        .env("HYSTERSIS_CLI_BASE_URL", base)
        .output()
        .expect("spawn hystersis update")
}
...
fn corrupt_config_never_changes_update_outcome() {
    ...
    *body.lock().unwrap_or_else(|e| e.into_inner()) = current;
    ...
    assert!(
        ...
        "healthy hystersis update against the local base must exit 0\nstdout:\n{}\nstderr:\n{}",
    ...
    assert!(
        ...
        "a corrupt config.toml must not block hystersis update\nstdout:\n{}\nstderr:\n{}",
