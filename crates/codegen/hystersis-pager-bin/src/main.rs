...
use hystersis_pager::app::{WorkspaceMgmtArgs, WorkspaceMgmtCommand, WorkspaceStartArgs};
use hystersis_pager::client_identity::PAGER_CLIENT_VERSION;
use hystersis_shell::agent::app::{run_headless, run_leader, run_stdio_agent};
use hystersis_shell::agent::config::Config as AgentConfig;
use hystersis_shell::leader::{
...
use hystersis_telemetry::process_info::{
...
fn process_identity(command: Option<&Command>, is_interactive: bool) -> Option<ProcessIdentity> {
    use hystersis_telemetry::process_info::LeaderMode::Standalone;
    let (entrypoint, interactivity) = match command {
        ...
        Some(Command::Configure) => (Entrypoint::Headless, Interactivity::Interactive),
        Some(Command::Info) => (Entrypoint::Headless, Interactivity::Unattended),
    };
...
use std::env;
use hystersis_update::{UpdateConfig, auto_update, enforce_version_policy_or_exit};
...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.hystersis_ws_origin {
        config.hystersis_com_config.hystersis_ws_origin = v.clone();
    }
    if let Some(v) = &args.hystersis_ws_url {
        config.hystersis_com_config.hystersis_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(
    agent_args: &hystersis_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.hystersis_api_base_url {
        config.endpoints.hystersis_api_base_url = v.clone();
    }
}
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    use hystersis_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(hystersis_telemetry::sampling_log::layer())
        .with(hystersis_telemetry::instrumentation::layer())
        .with(hystersis_telemetry::hooks_log::layer())
        .with(hystersis_telemetry::otel_layer::build_otel_layer(
            hystersis_telemetry::otel_layer::OtelClientInfo {
                client_name: "grok-pager",
                client_version: hystersis_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            hystersis_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    hystersis_telemetry::debug_log::install_firehose(registry, app_entrypoint);
    hystersis_telemetry::external::init(
        hystersis_shell::agent::config::resolve_external_otel_config(
            hystersis_telemetry::external::config::ExternalClientInfo {
                service_version: env!("VERSION_WITH_COMMIT").to_owned(),
                client_version: hystersis_version::VERSION.to_owned(),
                app_entrypoint: app_entrypoint.to_owned(),
            },
        ),
    );
}
/// `grok setup`: rendering + exit codes only; fetch logic lives in `hystersis_shell::managed_config`.
/// `json` prints the served configuration instead of installing it.
async fn run_setup_command(json: bool) {
    use hystersis_shell::managed_config::{self, SetupOutcome};
        ...
        match managed_config::fetch_setup_report().await {
            Ok(report) => {
                ...
                if !report.configured {
                    eprintln!(
                        "Your team doesn't have a managed configuration yet. A team admin can set one up at hystersis.com."
                    );
                }
            }
            Err(e) => {
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = hystersis_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !hystersis_shell::util::is_hystersis_process(pid) {
        ...
        if let Err(e) = hystersis_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(hystersis_shell::env::HystersisBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, hystersis_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = hystersis_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = hystersis_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&hystersis_shell::leader::ControlPayload>,
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&hystersis_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
fn fetch_remote_settings() -> Option<hystersis_shell::util::config::RemoteSettings> {
    join_early_prefetch(hystersis_shell::agent::models::start_early_prefetch(None))
}
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = hystersis_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.hystersis_com_config.hystersis_ws_url;
...
async fn workspace_control(
    ...
    let agent_config = hystersis_shell::config::load_agent_config_disk_only()
...
async fn workspace_start(
    ...
    use hystersis_shell::auth::ensure_authenticated;
    hystersis_shell::util::config::set_remote_campaigns_from_settings(remote_settings.as_ref());
    let raw_config = hystersis_shell::config::load_effective_config()
    ...
    let (use_leader, _) = resolve_use_leader(
        ...
        true,
        hystersis_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
    ...
    ensure_authenticated(
        &agent_config.hystersis_com_config,
        false,
        Some("No cached credentials found. Run `grok login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.hystersis_com_config);
...
struct StdioReplayState {
    ...
    /// `hystersis.com/leader_reconnected` as the primary restored session.
...
fn cache_outgoing_acp_state(msg: &str, state: &std::sync::Mutex<StdioReplayState>) {
    ...
    match method {
        ...
        "session/close" | "hystersis.com/session/close" | "_hystersis.com/session/close" => {
...
const REPLAY_LOAD_REQUEST_ID: &str = "hystersis.com/leader-replay/session-load";
...
/// `hystersis.com/leader_reconnected` with empty params in that case, signalling the
...
fn shutdown_and_flush_telemetry(exit_code: i32) -> ! {
    hystersis_telemetry::sentry::flush_on_shutdown();
    hystersis_telemetry::otel_layer::shutdown_otel();
    hystersis_telemetry::debug_log::flush();
...
async fn run_agent_command(
    agent_args: Box<hystersis_pager::app::AgentArgs>,
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use hystersis_pager::app::signal_handler::next_signal_code;
    ...
    if matches!(
        ...
        hystersis_shell::agent::app::suppress_otel();
    }
    init_tracing_simple("agent");
    let _otel_guard = hystersis_telemetry::otel_layer::otel_guard();
    let agent_config = hystersis_shell::config::load_agent_config_disk_only().unwrap_or_default();
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
    }
    hystersis_telemetry::instrumentation::install_panic_hook();
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => hystersis_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(error = %e, "--trust: failed to resolve cwd; folder not trusted")
            }
        }
    }
    let early_prefetch = hystersis_shell::agent::models::start_early_prefetch(None);
    hystersis_shell::agent::mvp_agent::warm_async_http_client();
    ...
    if !is_stdio && !is_leader {
        eprintln!(
            "Grok Build (pager) - v{}",
            hystersis_version::display_version_with_commit(
                env!("VERSION_WITH_COMMIT"),
                hystersis_update::channel_label(),
            )
        );
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(hystersis_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = hystersis_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    agent_config.default_auto_mode = hystersis_shell::util::config::effective_auto_for_launch(
        agent_args.yolo,
        permission_mode_flag.as_deref(),
        None,
        hystersis_shell::util::config::PermissionMode::Ask,
    );
    ...
    agent_config.resolve_runtime_fields(&hystersis_shell::agent::config::RuntimeResolutionContext {
    ...
    let requested_confinement = hystersis_sandbox::requested_confinement_profile();
    let LeaderMode {
        use_leader,
    ...
    use hystersis_telemetry::process_info::LeaderMode::{Attached, Standalone};
    ...
    let managed_install = is_managed_install(
        ...
        &hystersis_shell::util::hystersis_home::hystersis_home(),
    );
    ...
    if use_leader {
        ...
        let env_urls = hystersis_shell::leader::LeaderEnvUrls::from(&agent_config.hystersis_com_config);
        ...
        match mode {
            ClientMode::Stdio => {
                if let Err(error) = hystersis_tty_utils::kill_current_process_on_parent_death() {
                    tracing::warn!(
                        %error,
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = hystersis_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                            biased;
                                    ...
                                    .await
                                {
                                    Ok((new_tx, mut new_rx, _disconnect_rx)) => {
                                        ...
                                        let notification = format!(
                                            r#"{{"jsonrpc":"2.0","method":"hystersis.com/leader_reconnected","params":{params}}}"#
                                        );
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = hystersis_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            hystersis_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(hystersis_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = hystersis_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = hystersis_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !hystersis_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<hystersis_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(hystersis_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<hystersis_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(hystersis_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    hystersis_shell::heap_profile::install(hystersis_shell::heap_profile::HeapProfileHooks {
...
fn version_text(channel_label: &str) -> String {
    format!(
        "grok {}\n",
        hystersis_version::display_version_with_commit(
            hystersis_version::full_version(),
            channel_label,
        )
    )
}
...
fn dispatch_doctor_if_requested(args: &PagerArgs) -> bool {
    ...
    if let Err(error) = hystersis_pager::doctor_cmd::run(doctor_args.clone()) {
        eprintln!("Error: {error:#}");
        std::process::exit(1);
    }
    true
...
fn main() {
    hystersis_version::set_full_version(env!("VERSION_WITH_COMMIT"));
    hystersis_telemetry::startup::mark_process_start();
    if let Some(code) = hystersis_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    if let Some(code) = hystersis_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    set_release_channel(ReleaseChannel::from_label(
        hystersis_update::channel_name().unwrap_or_default(),
    ));
    let args = PagerArgs::parse_cli();
    if dispatch_version_if_requested(&args) || dispatch_doctor_if_requested(&args) || dispatch_info_if_requested(&args) {
        return;
    }
    hystersis_pager_minimal::install();
    #[cfg(all(feature = "jemalloc", unix))]
    hystersis_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        hystersis_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        hystersis_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    hystersis_pager::memory_trace::start(hystersis_pager::memory_trace::default_dir());
    raise_fd_limit();
    if let Err(e) = hystersis_config::validate_requirements() {
    ...
    let _sentry_guard = hystersis_telemetry::sentry::init(hystersis_telemetry::sentry::Config {
        client: "grok-pager",
        client_version: PAGER_CLIENT_VERSION,
        release: env!("VERSION_WITH_COMMIT"),
        disabled: hystersis_shell::agent::config::is_error_reporting_disabled_sync(),
    });
    hystersis_pager::docs::extract_user_guide_docs(&hystersis_shell::util::hystersis_home::hystersis_home());
    hystersis_crash_handler::install_terminal_restore_only();
    if hystersis_shell::util::config::load_crash_handler_enabled_sync() {
        let crash_dir = hystersis_shell::util::hystersis_home::hystersis_home().join("crash");
        if let Some(report) = hystersis_crash_handler::check_previous_crash(&crash_dir) {
        ...
        if !hystersis_crash_handler::install(hystersis_crash_handler::CrashHandlerConfig {
    ...
    let crashed = hystersis_active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
    ...
    let runtime =
        hystersis_tty_utils::runtime::build_with_blocking_pool(&mut builder).unwrap_or_else(|e| {
    ...
    hystersis_telemetry::debug_log::flush();
    if let Err(e) = result {
        hystersis_tty_utils::restore_native_stderr();
        match e.downcast_ref::<hystersis_pager::app::StartupFailure>() {
...
fn dispatch_info_if_requested(args: &PagerArgs) -> bool {
    if !matches!(args.command, Some(Command::Info)) {
        return false;
    }
    println!("Hystersis CLI v{} - The Developer's Agent", env!("CARGO_PKG_VERSION"));
    println!("");
    println!("   ██╗  ██╗██╗   ██╗███████╗████████╗███████╗██████╗ ███████╗██╗███████╗");
    println!("   ██║  ██║╚██╗ ██╔╝██╔════╝╚══██╔══╝██╔════╝██╔══██╗██╔════╝██║██╔════╝");
    println!("   ███████║ ╚████╔╝ ███████╗   ██║   █████╗  ██████╔╝███████╗██║███████╗");
    ...
    println!("  Config: ~/.hystersis/config.toml");
    println!("");
    println!("Core Integrations:");
    println!("  [✓] OpenAI Compatible");
    println!("  [✓] Anthropic Compatible");
    println!("  [✓] OpenRouter Compatible");
    println!("  [✓] Offline Local Models (Llama / Qwen)");
    println!("  [✓] Model Context Protocol (MCP) Support");
    println!("  [✓] Python & Bash Plugin Engine");
    println!("");
    println!("Type `hystersis` to open the interactive dashboard.");
    true
}

async fn async_main(args: PagerArgs) -> Result<()> {
    hystersis_extra_ca::ensure_default_crypto_provider();
    ...
    if args.chat() {
        unsafe {
            std::env::set_var(hystersis_shell::agent::chat_modes::HYSTERSIS_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(hystersis_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    ...
    if let Some(Command::Completions { shell }) = &args.command {
        hystersis_pager::completions_cmd::run(*shell);
        return Ok(());
    }
    if let Some(Command::Wrap(ref wrap_args)) = args.command {
        return hystersis_pager::wrap_cmd::run(wrap_args);
    }
    ...
    let sandbox_profile_arg = match args.startup_sandbox_profile(saved_profile.as_deref()) {
        hystersis_pager::app::cli::SandboxStartup::Apply(profile) => profile,
        hystersis_pager::app::cli::SandboxStartup::Conflict { requested, saved } => {
            eprintln!(
    ...
    hystersis_shell::config::apply_sandbox(
        None,
    ...
    hystersis_shell::http::set_client_name(if is_interactive {
        hystersis_workspace::permission::ClientType::HystersisPager
    } else {
        hystersis_workspace::permission::ClientType::Generic
    });
    ...
    if let Some(command) = args.command.take() {
        match command {
            Command::Version { json } => {
                if json {
                    let payload = serde_json::json!({
                        ...
                        "channel": hystersis_update::channel_name().unwrap_or("unknown"),
                    });
                    println!("{}", serde_json::to_string(&payload)?);
                } else {
                    write_version(
            ...
            Command::Info => {
                unreachable!("info was consumed before runtime startup")
            }
            Command::Doctor(_) => {
            ...
            Command::Inspect { json } => {
                ...
                hystersis_shell::inspect::inspect(&cwd, json).await?;
                return Ok(());
            }
            Command::Setup { json } => {
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
            ...
            Command::Mcp(mcp_args) => {
                init_tracing_simple("cli");
                return hystersis_pager::mcp_cmd::run(mcp_args).await;
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
                ...
                return hystersis_pager::plugin_cmd::run(plugin_args).await;
            }
            Command::Models => {
                init_tracing_simple("cli");
                let _otel_guard = hystersis_telemetry::otel_layer::otel_guard();
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return hystersis_pager::models::list_available_models(&agent_config).await;
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return hystersis_pager::worktree_cmd::run(worktree_args, &agent_config).await;
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
                ...
                return hystersis_pager::disk_usage_cmd::run(disk_usage_args);
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return hystersis_pager::sessions_cmd::run(sessions_args, &agent_config).await;
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return hystersis_pager::share_cmd::run(share_args, &agent_config).await;
            }
            Command::Export(export_args) => {
                init_tracing_simple("cli");
                return hystersis_pager::export_cmd::run(export_args);
            }
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
                ...
                let agent_config = hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return hystersis_pager::trace_cmd::run(trace_args, &agent_config).await;
            }
            Command::Memory(memory_args) => {
                return hystersis_pager::memory_cmd::run(memory_args);
            }
            Command::Update {
                check,
                json,
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        let _ = hystersis_pager::configure_cmd::run().await;
                ...
                let config = hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                hystersis_shell::auth::run_cli_login(&config, oauth, device_auth, devbox).await?;
                println!();
                hystersis_shell::instrumentation::finalize_and_exit(0);
            }
            Command::Logout => {
                init_tracing_simple("cli");
                let config = hystersis_shell::config::load_agent_config_disk_only()
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                hystersis_shell::auth::run_cli_logout(&config)?;
                hystersis_shell::instrumentation::finalize_and_exit(0);
            }
...
Command::Dashboard => {
            ...
            Command::Configure => {
                hystersis_pager::configure_cmd::run().await?;
                return Ok(());
            }
        }
    }
    let headless_prompt = hystersis_pager::headless::HeadlessPrompt::from_args(
    ...
    if agent_config.model_providers.is_empty() && agent_config.config_models.is_empty() {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(hystersis_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some()
            && args.output_format == hystersis_pager::headless::OutputFormat::Plain
        {
            args.output_format = hystersis_pager::headless::OutputFormat::Json;
        }
        return hystersis_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            hystersis_pager::headless::HeadlessOptions {
    ...
    let result = hystersis_pager::app::run(args, bg_update_rx).await;
    hystersis_sandbox::flush();
    match result {
        Ok(true) => {
...
fn build_update_config() -> UpdateConfig {
    let environment = hystersis_shell::env::HystersisBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                hystersis_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("GROK_NPM_REGISTRY"))
        .ok()
        .or_else(hystersis_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = hystersis_shell::config::load_effective_config_disk_only()
        && let Some(ch) = hystersis_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
/// True when `exe` is the binary `<hystersis_home>/bin/grok` resolves to, the
...
fn is_managed_install(exe: Option<std::path::PathBuf>, hystersis_home: &std::path::Path) -> bool {
    if hystersis_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = hystersis_config::hystersis_application_in(hystersis_home);
...
async fn run_update_command(
    ...
    let telemetry_cfg = hystersis_shell::config::load_agent_config_disk_only()
        .map_err(|e| tracing::warn!("grok update: telemetry init skipped (agent config: {e})"))
        .ok();
    if let Some(agent_cfg) = telemetry_cfg {
        let auth_manager = std::sync::Arc::new(hystersis_shell::auth::AuthManager::new(
            &hystersis_shell::util::hystersis_home::hystersis_home(),
            agent_cfg.hystersis_com_config.clone(),
        ));
        hystersis_shell::agent::init::update_telemetry_config(&agent_cfg, &auth_manager);
    }
    ...
    hystersis_telemetry::session_ctx::drain_pending(hystersis_telemetry::session_ctx::CLI_DRAIN)
        .await;
    result?;
    Ok(())
}
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in hystersis_shell::leader::discover_leaders().await {
        if d.classification != hystersis_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match hystersis_shell::leader::LeaderClient::connect(
            socket_path,
            "grok-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(hystersis_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(hystersis_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            hystersis_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: hystersis_shell::heap_profile::JemallocStats) {
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            hystersis_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!hystersis_shell::heap_profile::prof_available());
            return;
        }
        assert!(hystersis_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(hystersis_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(hystersis_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        hystersis_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    ...
    fn workspace_command_gate_resolution() {
        use hystersis_shell::util::config::RemoteSettings;
    ...
    fn cache_session_close_stops_replaying_it() {
        ...
        cache_outgoing_acp_state(
            r#"{"jsonrpc":"2.0","id":3,"method":"_hystersis.com/session/close","params":{"sessionId":"s1"}}"#,
            &state,
        );
        let s = state.lock().unwrap();
    ...
    async fn replay_waits_for_load_response_through_notifications() {
        ...
        let responder = tokio::spawn(async move {
            ...
            response_tx
                .send(
                    r#"{"jsonrpc":"2.0","method":"hystersis.com/leader/version_mismatch","params":{}}"#
                        .to_string(),
                )
                .unwrap();
            response_tx
    ...
    /// `hystersis.com/leader_reconnected` with empty params and the external client
