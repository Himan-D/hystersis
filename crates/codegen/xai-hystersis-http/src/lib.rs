...
//! `xai_hystersis_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `HYSTERSIS_POOL_*` /
//! `HYSTERSIS_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `HYSTERSIS_SAMPLER_SHARED_CLIENT=0` falls back to
//! a fresh client per `SamplingClient`.
//!
//! TLS policy (backend pin, roots, provider) lives in `xai_hystersis_extra_ca`.

use std::sync::OnceLock;

use xai_hystersis_workspace::permission::ClientType;
...
///
/// Replaces `xai_hystersis_shell::instrumentation_timer!`, which cannot be referenced
/// here (it lives in the shell crate, which now depends on this one). This is a
/// behavior-preserving copy: it routes to the same
/// `xai_hystersis_telemetry::instrumentation` API and keeps the Chrome trace
/// span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use xai_hystersis_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `xai-hystersis-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `xai-hystersis-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `xai-hystersis-telemetry` consumes
// the same type via `xai_hystersis_sampler::OriginClientInfo`. The shell-specific
...
/// `HYSTERSIS_CLIENT_VERSION` env vars. Returns `None` when
/// `HYSTERSIS_CLIENT_NAME` is unset.
...
/// Process-level client identifier (`HYSTERSIS_CLIENT_NAME` env var, default `"hystersis-shell"`).
...
/// (`hystersis -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-grok-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `hystersis -p`, `run_headless_inner` for
/// `hystersis agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn xai_hystersis_auth::AuthCredentialProvider>,
) -> reqwest_middleware::ClientWithMiddleware {
    reqwest_middleware::ClientBuilder::new(client)
        .with(xai_hystersis_auth::AuthRetryMiddleware::new(credentials, 1))
        .build()
}
...
pub fn shared_startup_blocking_client() -> reqwest::blocking::Client {
    ...
    BLOCKING_CLIENT
        .get_or_init(|| {
            ...
            xai_hystersis_extra_ca::build_blocking_reqwest_client(|builder| {
                builder
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "hystersis-desktop".to_string(),
    ...
    fn origin_client_info_from_meta_uses_client_type_when_identifier_absent() {
        ...
        assert_eq!(
            origin_client_info_from_meta(Some(&meta)),
            Some(OriginClientInfo {
                product: "hystersis-pager".to_string(),
    ...
    fn merge_origin_client_info_preserves_primary_product_and_backfills_version() {
        let merged = merge_origin_client_info(
            Some(OriginClientInfo {
                product: "hystersis-web".to_string(),
                version: None,
            }),
            Some(OriginClientInfo {
                product: "hystersis-desktop".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("hystersis-desktop/1.2.3 hystersis-shell/"));
        ...
        assert!(without_version.starts_with("hystersis-web hystersis-shell/"));
        assert!(!without_version.starts_with("hystersis-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        let ua = UserAgent {
            origin: OriginClientInfo {
                product: "hystersis-shell".to_string(),
        ...
        assert_eq!(ua.render(), "hystersis-shell/0.1.171 (macos; aarch64)");
    }
}
