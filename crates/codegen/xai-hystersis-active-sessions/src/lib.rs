//! Tracks open TUI sessions in `~/.hystersis/active_sessions.json` for crash
...
// -- Public API (delegates to `_in` variants with default hystersis home) --------

/// Register a session as active (idempotent by session_id).
pub fn register(session: ActiveSession) -> io::Result<()> {
    register_in(&xai_hystersis_config::hystersis_home(), session)
}
...
pub fn try_unregister(session_id: &acp::SessionId) -> io::Result<bool> {
    try_unregister_in(&xai_hystersis_config::hystersis_home(), session_id)
}

/// Remove entries with dead PIDs and return them.
pub fn collect_crashed() -> io::Result<Vec<ActiveSession>> {
    collect_crashed_in(&xai_hystersis_config::hystersis_home())
}
