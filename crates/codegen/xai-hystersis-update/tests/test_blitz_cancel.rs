...
//!
//! > `~/.hystersis/bin/hystersis` resolves to a binary that passes the smoke-test, OR it
...
use xai_hystersis_update::auto_update::install_internal_from_base;
...
/// (`hystersis` and `agent` — see `swap_managed_bin_links`). Returns the
...
/// What the active `hystersis` should resolve to after an install attempt.
...
/// to both `hystersis` and `agent` — `swap_managed_bin_links` moves them together.
...
/// `cargo nextest run -p xai-hystersis-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("HYSTERSIS_BLITZ_ITERS")
        .ok()
