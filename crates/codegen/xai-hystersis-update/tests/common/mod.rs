...
//! its own `OnceLock<HYSTERSIS_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the HYSTERSIS_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes HYSTERSIS_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// HYSTERSIS_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `HYSTERSIS_HOME`, initialized exactly once per test
/// binary. Once initialized, `xai_hystersis_config::hystersis_home()` will resolve to
...
pub fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe {
            std::env::set_var("HYSTERSIS_HOME", &path);
            std::env::remove_var("HYSTERSIS_TEST_VERSION");
            std::env::remove_var("NPM_TOKEN");
            std::env::remove_var("HYSTERSIS_INSTALLER");
            std::env::remove_var("HYSTERSIS_MANAGED_BY_NPM");
            std::env::remove_var("HYSTERSIS_MANAGED_BY_INTERNAL");
        }
        path
    })
}

/// Wipe state in `HYSTERSIS_HOME` between tests so each test sees a clean home.
...
/// Override the version reported by `get_installed_hystersis_version()` for the
/// duration of the test (until [`reset_home`] or process exit).
pub fn set_test_version(v: &str) {
    // SAFETY: tests using this helper must be `#[serial]`.
    unsafe { std::env::set_var("HYSTERSIS_TEST_VERSION", v) };
}
...
/// Minimal [`xai_hystersis_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> xai_hystersis_update::UpdateConfig {
    xai_hystersis_update::UpdateConfig {
...
/// Backdate every file in `HYSTERSIS_HOME/downloads` by ~2 hours.
///
