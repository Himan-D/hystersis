...
use crate::version::get_installed_hystersis_version;
use semver::Version;
use tracing::warn;
use xai_hystersis_shell::util::config::VersionPolicy;
...
pub(crate) enum VersionPolicyError {
    #[error(
        "Cannot install Hystersis {target}: the minimum allowed version is {minimum}. \
         Run `hystersis update` to install the latest allowed version."
    )]
