...
use crate::version::{
    UpdateConfig, fetch_latest_version, get_installed_hystersis_version, get_latest_version,
    is_version_cache_fresh, try_fetch_stable_pointer, write_version_cache,
};
use xai_hystersis_shell::util::config;
use xai_hystersis_shell::util::hystersis_home::{hystersis_application, hystersis_home};
pub use xai_hystersis_telemetry::events::CliUpdateTrigger;
use xai_hystersis_telemetry::events::{
...
const MSG_RUN_UPDATE_MANUAL: &str = "Run `hystersis update` to get the latest version.";
/// An empty or `"stable"` channel means stable — the installers' default
/// (`CHANNEL="${HYSTERSIS_CHANNEL:-stable}"` in install.sh).
...
fn manual_install_cmd(channel: &str) -> String {
    ...
    if channel == "enterprise" {
        ...
        return if cfg!(windows) {
            "irm https://hystersis.com/cli/enterprise-install.ps1 | iex".to_string()
        } else {
            "curl -fsSL https://hystersis.com/cli/enterprise-install.sh | bash".to_string()
        };
    }
    if is_stable_channel(channel) || !safe {
        return if cfg!(windows) {
            "irm https://hystersis.com/cli/install.ps1 | iex".to_string()
        } else {
            "curl -fsSL https://hystersis.com/cli/install.sh | bash".to_string()
        };
    }
    if cfg!(windows) {
        format!("$env:HYSTERSIS_CHANNEL='{channel}'; irm https://hystersis.com/cli/install.ps1 | iex")
    } else {
        format!("curl -fsSL https://hystersis.com/cli/install.sh | HYSTERSIS_CHANNEL='{channel}' bash")
    }
}
...
fn reinstall_hint(installer: &str, channel: &str) -> String {
    match installer {
        "npm" => "Please reinstall via npm:\n  npm i -g @xai-official/hystersis".to_string(),
        "gh-release" => "Please reinstall via GitHub Releases:\n  gh release download --repo xai-org-shared/hystersis --pattern 'hystersis-*' --output hystersis && chmod +x hystersis".to_string(),
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of Hystersis is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of Hystersis is available.");
        }
        return Ok(());
    }
    ...
    println!("Hystersis - v{}{}", status.current_version, channel_label);
    Ok(())
}
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_hystersis_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_hystersis_version();
...
/// background download, explicit `hystersis update`) is reused as-is. This both
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_hystersis_version);
    if needs_update(
        &effective_current,
        &target,
        &update_config.channel,
    ...
    let running = get_installed_hystersis_version();
...
/// managed `~/.hystersis/bin/hystersis` symlink.
///
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("HYSTERSIS_INSTALLER") {
    ...
    if std::env::var_os("HYSTERSIS_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("HYSTERSIS_MANAGED_BY_INTERNAL").is_some() {
...
pub struct BackgroundUpdateCheck {
    ...
    /// Handle to the background `hystersis update` child, `Some` only when a
...
/// (spawns `hystersis update` as a detached child process) so the new binary is
...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    ...
    // restart hint". The quit-for-update path's `hystersis update` child resolves
...
/// Launch "hystersis update" in blocking or non-blocking mode.
///
...
async fn run_update_subcommand(
    ...
    // delay an update.
    if let Some(mode) = xai_hystersis_telemetry::client::current_mode() {
        cmd.env("HYSTERSIS_TELEMETRY_ENABLED", mode.to_string());
    }
    match run_mode {
        UpdateRunMode::Blocking => {
            ...
            if !status.success() {
                anyhow::bail!("hystersis update failed with {}", status);
            }
            Ok(None)
        }
        UpdateRunMode::NonBlocking => {
            ...
            xai_hystersis_tools::util::detach_command(&mut cmd);
...
/// Resolve the hystersis binary path for re-execution after an update.
///
/// `current_exe()` resolves symlinks via `/proc/self/exe` (see proc(5)),
/// so it returns the old versioned target after a symlink swap.
/// Prefer `~/.hystersis/bin/hystersis` which always points to the latest version.
fn resolve_restart_exe() -> Result<std::path::PathBuf> {
    let canonical = hystersis_application();
    if canonical.exists() {
        return Ok(canonical);
    }
...
/// Restart hystersis with the original command-line arguments to pick up the update.
pub fn restart_hystersis() -> Result<()> {
    ...
    cmd.env_clear();
    cmd.envs(std::env::vars_os().filter(|(k, _)| k != "HYSTERSIS_AUTO_UPDATE"));
    eprintln!("Restarting Hystersis...");
...
pub async fn run_install_script(
    ...
    xai_hystersis_telemetry::session_ctx::log_event(CliUpdate {
        outcome,
        trigger,
        from_version,
        to_version,
...
/// interactive `hystersis update`, background `--auto` children, the leader's
...
fn download_client() -> reqwest::Result<reqwest::Client> {
    xai_hystersis_extra_ca::build_reqwest_client(|builder| builder.timeout(DOWNLOAD_REQUEST_TIMEOUT))
}
...
/// versioned name as the extension (`hystersis-0.1.181-linux-x86_64` →
/// `hystersis-0.1.tmp`) and therefore collides for every `0.1.x` version. The PID
...
/// versioned base like `hystersis-0.1.181` doesn't collide via `with_extension`.
...
/// Delete `~/.hystersis/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = hystersis_home().join("models_cache.json");
...
/// Remove the stale `hystersis-pager` symlink/binary from `~/.hystersis/bin/` left by
...
/// config and writes to `~/.hystersis/bin/`, so callers must isolate
/// `HYSTERSIS_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.hystersis/downloads/`, not yet
/// activated as the managed `hystersis`/`agent`.
...
async fn download_verified_from_base(
    ...
    let hystersis_home = hystersis_home();
    let download_dir = hystersis_home.join("downloads");
    tokio::fs::create_dir_all(&download_dir).await?;

    let binary_name = format!("hystersis-{}-{}", version, platform);
    let binary_path = download_dir.join(&binary_name);

    eprintln!("  Downloading hystersis v{} ({})...", version, platform);
    ...
    // corrupt download is caught here and never becomes the active hystersis.
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let hystersis_home = hystersis_home();
    let download_dir = hystersis_home.join("downloads");
    let bin_dir = hystersis_home.join("bin");
    tokio::fs::create_dir_all(&bin_dir).await?;

    // Atomic swap of ~/.hystersis/bin/{hystersis,agent} -> downloaded binary.
    ...
    cleanup_old_downloads(&download_dir, "hystersis", &download.version).await;
    cleanup_old_downloads(&download_dir, "hystersis-pager", &download.version).await;
    ...
    regenerate_completions(&link_path, &hystersis_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, hystersis_home: &std::path::Path) {
    // Derive $HOME independently — hystersis_home may be overridden via HYSTERSIS_HOME
    // env var, so hystersis_home.parent() isn't necessarily the user's home dir.
    #[allow(deprecated)]
    let user_home = std::env::home_dir().unwrap_or_default();

    let completions: &[(&str, std::path::PathBuf)] = &[
        ("bash", hystersis_home.join("completions/bash/hystersis.bash")),
        ("zsh", hystersis_home.join("completions/zsh/_hystersis")),
        ("fish", user_home.join(".config/fish/completions/hystersis.fish")),
    ];
...
///
/// When both paths share a grandparent (e.g. `~/.hystersis/bin/hystersis` and
/// `~/.hystersis/downloads/hystersis-0.1.203-linux-x86_64`), returns a relative path
/// like `../downloads/hystersis-0.1.203-linux-x86_64`.  When they share the same
/// parent directory, returns just the filename.  Falls back to the absolute
/// `target` path for any other layout.
///
/// Relative symlinks survive Docker bind-mounts where `~/.hystersis/` is mapped
...
fn relative_symlink_target(target: &std::path::Path, link: &std::path::Path) -> std::path::PathBuf {
    ...
    // Same directory — just the filename (e.g. hystersis-latest -> hystersis-0.1.203-…)
    if target_parent == link_parent
        && let Some(name) = target.file_name()
    {
        return std::path::PathBuf::from(name);
    }
    // Sibling directories — ../target_dir/filename (e.g. bin/hystersis -> ../downloads/hystersis-…)
...
/// Swap `~/.hystersis/bin/{hystersis,agent}` to point at `binary_path`. Returns the
/// `hystersis` link path (for [`regenerate_completions`]).
///
/// `hystersis` and `agent` are first-class entry points that the bootstrap
/// installers (`install.sh`, `install.ps1`, `install-enterprise.sh`)
/// maintain in lockstep, and so must the updater — otherwise `hystersis update`
/// leaves `agent` pinned at the previous version.
///
/// Unix: atomic symlink swap with relative target (survives Docker
/// bind-mounts of `~/.hystersis/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let hystersis_name = if cfg!(windows) { "hystersis.exe" } else { "hystersis" };
    let agent_name = if cfg!(windows) { "agent.exe" } else { "agent" };
    let hystersis_link = bin_dir.join(hystersis_name);
    let agent_link = bin_dir.join(agent_name);
    let link_paths: [std::path::PathBuf; 2] = [hystersis_link.clone(), agent_link];
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running hystersis sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
///
/// `bin_prefix` is the binary name prefix, e.g. `"hystersis"` or `"hystersis-pager"`.
/// Files must match `{bin_prefix}-{digit}*` to be considered versioned binaries
/// (this avoids `hystersis-*` matching `hystersis-pager-*` or `hystersis-latest`).
///
...
async fn cleanup_old_downloads(dir: &std::path::Path, bin_prefix: &str, current_version: &str) {
    ...
    while let Ok(Some(entry)) = entries.next_entry().await {
        ...
        // Skip symlinks (e.g. hystersis-latest).
        ...
        // binary (avoids `hystersis-latest`, `hystersis-pager-*` when prefix is `hystersis`).
        let suffix = &name[prefix.len()..];
        if !suffix.starts_with(|c: char| c.is_ascii_digit()) {
            continue;
        }
        // Extract the version portion via the shared parser (handles the
        // internal `hystersis-0.1.150-macos-aarch64`, pre-release, and npm
        // `hystersis-0.1.150` layouts — see `version_from_versioned_binary_name`).
...
async fn heal_managed_install(installer: &str) {
    ...
    #[cfg(any(unix, windows))]
    {
        ...
        #[cfg(unix)]
        reconcile_agent_to_hystersis(&bin_dir).await;

        #[cfg(windows)]
        reconcile_agent_exe_to_hystersis(&bin_dir).await;
    }
}

#[cfg(unix)]
async fn reconcile_agent_to_hystersis(bin_dir: &std::path::Path) {
    let hystersis_link = bin_dir.join("hystersis");
    let agent_link = bin_dir.join("agent");

    let Ok(hystersis_target) = tokio::fs::read_link(&hystersis_link).await else {
        return;
    };
    if tokio::fs::metadata(&hystersis_link).await.is_err() {
        return;
    }
    if let Ok(agent_target) = tokio::fs::read_link(&agent_link).await
        && agent_target == hystersis_target
    {
        return;
    }
    match atomic_symlink_swap(&hystersis_target, &agent_link).await {
        Ok(()) => tracing::info!(
            hystersis_target = %hystersis_target.display(),
            "reconciled agent bin symlink to hystersis target"
        ),
...
#[cfg(windows)]
async fn reconcile_agent_exe_to_hystersis(bin_dir: &std::path::Path) {
    let hystersis_exe = bin_dir.join("hystersis.exe");
    let agent_exe = bin_dir.join("agent.exe");

    if tokio::fs::metadata(&hystersis_exe).await.is_err() {
        return;
    }
    match agent_exe_differs(&hystersis_exe, &agent_exe).await {
        Ok(true) => {}
        Ok(false) => return,
        Err(e) => {
    ...
    match windows_replace_exe(&hystersis_exe, &agent_exe).await {
        Ok(()) => tracing::info!("reconciled agent.exe to hystersis.exe"),
        Err(e) => tracing::warn!("failed to reconcile agent.exe to hystersis.exe: {e:#}"),
    }
}

#[cfg(windows)]
async fn agent_exe_differs(
    hystersis: &std::path::Path,
    agent: &std::path::Path,
) -> std::io::Result<bool> {
    use tokio::io::{AsyncReadExt, BufReader};
    let hystersis_len = tokio::fs::metadata(hystersis).await?.len();
    match tokio::fs::metadata(agent).await {
        Ok(m) if m.len() != hystersis_len => return Ok(true),
        Ok(_) => {}
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(true),
        Err(e) => return Err(e),
    }
    let mut rg = BufReader::new(tokio::fs::File::open(hystersis).await?);
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    xai_hystersis_tools::util::detach_command(&mut cmd);
    cmd.envs(xai_hystersis_tools::util::pager_env());
...
/// Download and install hystersis from GitHub Releases (xai-org-shared/hystersis).
///
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    eprintln!(
        "  Downloading hystersis v{} ({}) from GitHub Releases...",
        version, platform
    );
    ...
    // Update hystersis-latest -> versioned binary so any existing symlinks that route
    // through it (e.g. /usr/local/bin/hystersis -> ~/.hystersis/downloads/hystersis-latest)
    // resolve to the newly installed version.
    #[cfg(unix)]
    {
        let latest_path = download_dir.join("hystersis-latest");
        ...
        if let Err(e) = atomic_symlink_swap(&rel_target, &latest_path).await {
            tracing::warn!("Failed to update hystersis-latest symlink: {e}");
        }
    }

    // Also update /usr/local/bin/{hystersis,agent} if either points directly into
    // ~/.hystersis/downloads/ (legacy layout — skips the hystersis-latest indirection).
    // Permission errors ignored.
    #[cfg(unix)]
    for name in ["hystersis", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".hystersis/downloads/") && !target_str.ends_with("hystersis-latest") {
    ...
    // Clean up old versioned binaries (keeps current + 1 previous).
    cleanup_old_downloads(&download_dir, "hystersis", &version).await;
    cleanup_old_downloads(&download_dir, "hystersis-pager", &version).await;
...
/// Check if other hystersis processes are running (macOS only).
///
/// On macOS, `npm i -g` replaces the vendored binary in node_modules in-place.
/// Any hystersis process running from that vendored path will be SIGKILL'd by the
...
/// inode is unlinked.
///
/// While our postinstall.js now uses versioned binaries under ~/.hystersis/bin/
...
#[cfg(target_os = "macos")]
fn warn_if_other_hystersis_processes_running() {
    let my_pid = std::process::id().to_string();
    let mut cmd = Command::new("pgrep");
    cmd.args(["-f", "hystersis"])
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    xai_hystersis_tools::util::detach_std_command(&mut cmd);
    if let Ok(output) = cmd.output() {
        ...
        if !other_pids.is_empty() {
            eprintln!(
                "  ⚠ Warning: {} other hystersis process(es) detected.",
                other_pids.len()
            );
            ...
            eprintln!("    Consider closing other hystersis sessions before updating.");
            eprintln!();
        }
    }
}
...
fn install_npm(target: Option<&str>, channel: &str, npm_registry: Option<&str>) -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    warn_if_other_hystersis_processes_running();

    let version_arg = match target {
        Some(ver) => format!("@xai-official/hystersis@{ver}"),
        None => {
...
/// Run the `hystersis update` command. Returns `Ok(Some(version))` when the target
...
pub async fn run_update(
    ...
    if let Some(version) = pinned_version {
        ...
        eprintln!("  ✓ hystersis v{} installed successfully!", version);
        eprintln!("  Please restart Hystersis.");
    ...
    let (latest_version, install_target) = match plan {
        UpdatePlan::Skip { latest } => {
            // Cache so an explicit `hystersis update` doesn't re-prompt every run.
            let stable_ptr = try_fetch_stable_pointer().await;
    ...
    {
        eprintln!(
            "Forcing reinstall of Hystersis {} (already up to date)",
            effective_current
        );
        &effective_current
    } else {
        eprintln!("Updating Hystersis {} → {}", effective_current, install_target);
        &install_target
    };
    ...
    write_version_cache(target_version, stable_ptr.as_deref()).await;
    refresh_deployment_config().await;
    eprintln!("  ✓ hystersis v{} installed successfully!", target_version);

    if !force && std::env::var_os("HYSTERSIS_AUTO_UPDATE").is_none() {
        eprintln!("  Please restart Hystersis.");
    }
...
async fn refresh_deployment_config() {
    if !xai_hystersis_shell::managed_config::has_principal() {
        return;
    }
    if !xai_hystersis_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    xai_hystersis_shell::managed_config::clear_orphan();
    if !xai_hystersis_shell::config::is_managed_config_stale_for(
        &xai_hystersis_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match xai_hystersis_shell::managed_config::sync().await {
        Ok(true) => eprintln!("  Applied managed configuration."),
        Ok(false) => tracing::debug!("no managed configuration to apply"),
        // Auth issues aren't actionable mid-update: quiet here, loud on `hystersis setup`.
        Err(e) if e.is_auth_rejection() => tracing::debug!("managed config not applied: {e}"),
        Err(e) if e.is_retryable() => {
            tracing::debug!("managed config refresh failed: {e}");
            eprintln!("  Couldn't apply managed configuration. Run `hystersis setup` to retry.");
        }
