...
use hystersis_announcements::RemoteAnnouncement;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-hystersis-doom-loop-check` header, parse the reported
    ...
    /// Detector window sent as the value of `x-hystersis-doom-loop-check`
...
/// Consent notice from `hystersis_build_settings.consent_gate`. Which accounts see it is a targeting
...
pub struct RemoteSettings {
    ...
    /// `app::subscription` module). Forwarded from the `hystersis_build_settings`
    ...
    /// OAuth2 provider issuer URL (e.g., "https://auth.hystersis.com"). When present
    /// together with `oauth2_client_id`, the client uses OAuth2 authorization code
    /// flow. Controlled via remote settings for gradual rollout. Generic fork
    /// default is `https://auth.hystersis.com` — override via `oauth2_issuer`.
    #[serde(default)]
    pub oauth2_issuer: Option<String>,
    /// OAuth2 client_id for the CLI. Paired with `oauth2_issuer`.
    #[serde(default)]
    pub oauth2_client_id: Option<String>,
    /// When `Some(true)`, enable the default OAuth2 provider (https://auth.hystersis.com).
    /// Enterprise OIDC (user's own IdP via `oidc` config) always wins.
    /// Controlled via remote settings; `--oauth` CLI flag overrides.
    /// Default is enabled when no explicit setting is present (fork-generic).
    #[serde(default)]
    pub hystersis_oauth_enabled: Option<bool>,
    #[serde(default)]
    ...
    /// default (on). Sits below env `HYSTERSIS_FOLDER_TRUST`, user
    ...
    /// remote settings `hystersis_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `hystersis_build_settings` remote settings key) to enable it. See
    ...
    /// `hystersis_telemetry::external::apply_remote_policy`.
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `hystersis_build_settings`).
    ...
    /// ghost text), from the `hystersis_build_settings` remote settings flag. Sits below
    /// env (`HYSTERSIS_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `hystersis-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `hystersis-imagine-image`) instead of the default quality model
    /// (`hystersis-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `HYSTERSIS_VOICE_MODE`. Free-tier HystersisPro upsell
    ...
    /// remain coding-data opted-out. From `hystersis_build_settings`.
    ...
    /// User's subscription tier from remote settings `hystersis_build_access_gate`.
    /// E.g. "free", "premium", "hystersispro", "hystersispro_heavy".
    ...
    /// Whether the user is allowed to use Hystersis. Set by remote settings
    /// `hystersis_build_access_gate` targeting rules. `None` = no server response
    ...
    /// (e.g. "HystersisPro", "X Premium+", "Free", "API Key"). Set by CCP
    ...
    /// Server-controlled via the remote settings `hystersis_build_usage_redirect_url`
    ...
    /// `hystersis_build_settings`. Per-model override on `ModelInfo`
    /// (`hystersis_build_models`) takes precedence; user config and env var
    /// further override per the resolver chain.
    #[serde(default)]
    pub auto_compact_threshold_percent: Option<u8>,
    /// Max subagent nesting depth (`hystersis_build_settings.subagents_max_depth`).
    #[serde(default)]
    ...
    /// (`hystersis_build_settings.max_parallel_image_gen_calls`).
    #[serde(default)]
    pub max_parallel_image_gen_calls: Option<u32>,
    /// Max parallel video-gen tool calls in one model step
    /// (`hystersis_build_settings.max_parallel_video_gen_calls`).
    #[serde(default)]
    ...
    /// `0` disables. Env (`HYSTERSIS_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`HYSTERSIS_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`HYSTERSIS_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`HYSTERSIS_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `hystersis workspace` CLI command (Computer Hub
    /// workspace exposure), from `hystersis_build_settings.workspace_command_enabled`.
    ...
    /// `hystersis_build_settings.keep_text_selection_default`. Applied only when the user has set no
...
pub struct ContextualHintsRemote {
    ...
    /// SSH wrap session-load tip (recommend `hystersis wrap ssh` for remote sessions).
    #[serde(default)]
...
/// only guaranteed to work with a compatible harness (cursor vs hystersis).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoalRoleModel {
    /// Model id, e.g. "hystersis-4". Resolved against available models at
    /// spawn time; unknown/unauthorized ⇒ fail-open to current model.
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "hystersis-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-hystersis toolset), applied REGARDLESS of the
...
mod tests {
    ...
    fn remote_settings_image_description_model_round_trip() {
        let json = r#"{"image_description_model": "hystersis"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.image_description_model.as_deref(), Some("hystersis"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_prompt_suggestion_model_round_trip() {
        let json = r#"{"prompt_suggestion_model": "hystersis-0.1"}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.prompt_suggestion_model.as_deref(), Some("hystersis-0.1"));
        let out = serde_json::to_string(&s).unwrap();
        let s2: RemoteSettings = serde_json::from_str(&out).unwrap();
    ...
    fn remote_settings_consent_gate_round_trip() {
        let json = r#"{
            "consent_gate": {
                ...
                "version": 3,
                "title": "Updated terms",
                "body": "Review our [Terms of Service](https://hystersis.com/legal/tos) before continuing.",
        ...
        assert!(
            gate.body
                .as_deref()
                .is_some_and(|b| b.contains("https://hystersis.com/legal/tos"))
        );
    }
    ...
    fn remote_settings_goal_planner_model_round_trip() {
        let json =
            r#"{"goal_planner_model": {"model": "hystersis-4", "agent_type": "general-purpose"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_strategist_model_round_trip() {
        let json = r#"{"goal_strategist_model": {"model": "hystersis-4.5", "agent_type": "cursor"}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_fully_valid_pool_round_trips() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "hystersis-4", "agent_type": "general-purpose"},
            {"model": "hystersis-3", "agent_type": "cursor"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
    ...
    fn remote_settings_goal_skeptic_models_extra_unknown_fields_kept() {
        let json = r#"{"goal_skeptic_models": [
            {"model": "hystersis-4", "agent_type": "general-purpose", "reasoning_effort": "high"}
        ]}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_skeptic_models,
    ...
    fn remote_settings_goal_role_models_malformed_pair_does_not_drop_other_fields() {
        let json = r#"{
            ...
            "goal_strategist_model": {"model": "hystersis-4.5", "agent_type": "cursor"},
            "default_model": "hystersis-4"
        }"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(s.goal_planner_model, None);
        assert_eq!(
        ...
        assert_eq!(s.default_model.as_deref(), Some("hystersis-4"));
    }
    #[test]
    fn remote_settings_goal_role_model_extra_unknown_fields_kept_single_pair() {
        let json = r#"{"goal_planner_model": {"model": "hystersis-4", "agent_type": "general-purpose", "future": true}}"#;
        let s: RemoteSettings = serde_json::from_str(json).unwrap();
        assert_eq!(
            s.goal_planner_model,
            Some(GoalRoleModel {
