...
//! `.hystersis-plugin/plugin-index.json` (preferred), then
...
use hystersis_hooks_plugins_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_hystersis_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".hystersis-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_hystersis_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".hystersis-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".hystersis-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".hystersis-plugin",
            r#"{
                "$schema": "https://hystersis.com/hystersis/plugin-index.schema.json",
                "version": 1,
