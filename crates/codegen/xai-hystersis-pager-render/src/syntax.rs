...
//! Dark themes (HystersisNight, TokyoNight) share `hystersis-night.tmTheme`;
//! HystersisDay uses `hystersis-day.tmTheme` with deepened colors for light backgrounds.
//!
...
//! `current_kind()` is a nominal `HystersisNight` (so leftover kind-keyed paths
...
pub use xai_hystersis_markdown::Syntect;
...
static SYNTECT_HYSTERSISNIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_HYSTERSISDAY: OnceLock<Syntect> = OnceLock::new();
...
/// reports a nominal `HystersisNight`, so this returns the night theme. Token
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        | ThemeKind::Auto => SYNTECT_HYSTERSISNIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/hystersis-night.tmTheme"))),
        ThemeKind::TokyoNight => SYNTECT_TOKYONIGHT
            .get_or_init(|| Syntect::new(include_bytes!("../assets/tokyo-night.tmTheme"))),
        ThemeKind::HystersisDay => SYNTECT_HYSTERSISDAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/hystersis-day.tmTheme"))),
    }
}
