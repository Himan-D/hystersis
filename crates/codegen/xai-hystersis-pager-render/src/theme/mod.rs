...
//! The default theme is HystersisNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! HystersisNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::HystersisNight, ThemeKind::HystersisDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::HystersisNight => "hystersisnight",
            Self::TokyoNight => "tokyonight",
            Self::HystersisDay => "hystersisday",
    ...
    /// when quantized to 256 or 16 colors. HystersisNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::HystersisNight => false,
            Self::TokyoNight => true,
            Self::HystersisDay => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "hystersisnight" | "hystersis-night" | "dark" => Some(Self::HystersisNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "hystersisday" | "hystersis-day" | "light" | "day" => Some(Self::HystersisDay),
...
/// `"hystersisnight"` → `"Hystersis Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "hystersisnight" => "Hystersis Night",
        "hystersisday" => "Hystersis Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.hystersis/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::HystersisNight => Self::hystersisnight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::HystersisDay => Self::hystersisday(),
            ...
            // if reached, fall back to HystersisNight.
            ThemeKind::Auto => Self::hystersisnight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `HYSTERSIS_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on hystersisnight, erasing every state signal). We pin each
    ...
    /// theme (e.g., HystersisNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // HystersisNight bg field quantizes to Color::Black and these
            // become invisible.
            ...
            // Markdown content: naive Basic quantize lands HystersisNight
...
pub fn apply_cursor_color() {
    ...
    xai_hystersis_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::HystersisNight.is_auto());
        assert!(!ThemeKind::HystersisDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        // — see `ansi16_overrides_gray_hierarchy_collapses_to_two_slots`.
        use ratatui::style::Color;
        let t = Theme::hystersisday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::hystersisnight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::hystersisday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. HystersisNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::HystersisNight => Theme::hystersisnight(),
                ThemeKind::HystersisDay => Theme::hystersisday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_hystersisnight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark HystersisNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::hystersisnight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("hystersisnight"),
            Some(ThemeKind::HystersisNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::HystersisNight));
        assert_eq!(ThemeKind::from_name("hystersisday"), Some(ThemeKind::HystersisDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::HystersisDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("hystersisnight", ThemeKind::HystersisNight),
            ("hystersis-night", ThemeKind::HystersisNight),
            ("dark", ThemeKind::HystersisNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("hystersisday", ThemeKind::HystersisDay),
            ("hystersis-day", ThemeKind::HystersisDay),
            ("light", ThemeKind::HystersisDay),
            ("day", ThemeKind::HystersisDay),
