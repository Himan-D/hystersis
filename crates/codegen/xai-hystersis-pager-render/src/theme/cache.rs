...
//! lookup must be cheaper than re-loading from `~/.hystersis/config.toml`.
...
//! `xai_hystersis_shell::util::config::set_theme()` (and friends), invoked
...
static CURRENT: AtomicU8 = AtomicU8::new(ThemeKind::HystersisNight as u8);
...
/// back to `HystersisNight` if the byte is somehow out of range (which
...
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::HystersisNight as u8 => ThemeKind::HystersisNight,
        x if x == ThemeKind::HystersisDay as u8 => ThemeKind::HystersisDay,
...
/// When `None`, `to_theme_kind()` defaults to `HystersisNight` / `HystersisDay`.
...
///
/// On the first call, reads from `~/.hystersis/config.toml` (via the shell's
...
pub fn current_kind() -> ThemeKind {
    ...
    if terminal_native_locked() {
        return ThemeKind::HystersisNight;
    }
...
pub fn set_terminal_native_lock(locked: bool) {
    ...
    xai_hystersis_markdown::set_color_level_cap(if locked {
        xai_hystersis_markdown::ColorLevel::Basic
    } else {
        xai_hystersis_markdown::ColorLevel::TrueColor
    });
    xai_hystersis_markdown::set_polarity_safe_syntax(locked);
}
...
///
/// Precedence:
/// 1. Environment variable (`HYSTERSIS_THEME` / `LC_HYSTERSIS_THEME`)
/// 2. Config file (`[ui].theme`)
/// 3. Default: `HystersisNight`
#[must_use]
...
fn env_theme_name_from(env: &HashMap<String, String>) -> Option<&str> {
    for key in ["HYSTERSIS_THEME", "LC_HYSTERSIS_THEME"] {
        let Some(raw) = env
            .get(key)
            .map(String::as_str)
...
fn resolve_from_config(config_theme: Option<ThemeKind>, osc11_fallback: bool) -> ThemeKind {
    ...
    // Default: HystersisNight
    ThemeKind::HystersisNight
}
...
fn resolve_from_appearance(appearance: Option<system_appearance::SystemAppearance>) -> ThemeKind {
    ...
    appearance
        ...
        .unwrap_or(ThemeKind::HystersisNight)
}
...
/// and the user's dark/light theme mapping. Falls back to `HystersisNight`
...
// -- Disk reads --------------------------------------------------------------
//
// All writes go through `xai_hystersis_shell::util::config::set_theme()` (and
...
fn load_from_disk() -> Option<ThemeKind> {
    let root = xai_hystersis_config::load_effective_config_disk_only().ok()?;
...
fn load_auto_theme_config() -> AutoThemeConfig {
    let Ok(root) = xai_hystersis_config::load_effective_config_disk_only() else {
...
pub fn reset_for_test() {
    ...
    CURRENT.store(ThemeKind::HystersisNight as u8, Ordering::Relaxed);
...
mod tests {
    ...
    fn terminal_native_lock_pins_kind_and_blocks_apply_kind() {
        with_test_env(|| {
            set(ThemeKind::HystersisDay);
            set_terminal_native_lock(true);
            assert!(terminal_native_locked());
            assert_eq!(current_kind(), ThemeKind::HystersisNight, "nominal kind");

            let applied = super::super::Theme::apply_kind(ThemeKind::HystersisDay);
            assert_eq!(applied, ThemeKind::HystersisNight, "apply_kind must no-op");
            assert_eq!(current_kind(), ThemeKind::HystersisNight);

            set_terminal_native_lock(false);
            assert_eq!(
                current_kind(),
                ThemeKind::HystersisDay,
    ...
    fn terminal_native_lock_serves_terminal_default_palette() {
        with_test_env(|| {
            ...
            assert_ne!(
                theme.text_primary,
                super::super::Theme::hystersisday().text_primary,
                "must not serve the cached (HystersisDay) theme"
            );
        });
    }
    ...
    fn terminal_native_lock_enables_polarity_safe_syntax() {
        with_test_env(|| {
            assert!(!xai_hystersis_markdown::polarity_safe_syntax());
            set_terminal_native_lock(true);
            assert!(
                xai_hystersis_markdown::polarity_safe_syntax(),
    ...
    fn terminal_native_lock_caps_quantize_at_ansi16() {
        ...
        with_test_env(|| {
            ...
            for input in [
                Color::Rgb(0x26, 0x26, 0x26), // hystersisday text_primary
    ...
    fn resolve_no_osc11_explicit_auto_and_default() {
        with_test_env(|| {
            assert_eq!(
                resolve_from_config(Some(ThemeKind::HystersisDay), false),
                ThemeKind::HystersisDay
            );
            assert!(!is_auto_mode());
            ...
            assert_eq!(resolve_from_config(None, false), ThemeKind::HystersisNight);
        });
    }
    ...
    #[test]
    fn resolve_auto_dark_system_returns_hystersisnight() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Dark));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::HystersisNight);
        });
    }

    #[test]
    fn resolve_auto_light_system_returns_hystersisday() {
        with_test_env(|| {
            system_appearance::set_mock(Some(system_appearance::SystemAppearance::Light));
            let result = resolve_auto();
            assert_eq!(result, ThemeKind::HystersisDay);
        });
    }

    #[test]
    fn resolve_auto_detection_failure_returns_hystersisnight() {
        with_test_env(|| {
    ...
    #[test]
    fn resolve_from_config_no_config_returns_hystersisnight() {
        with_test_env(|| {
    ...
    fn resolve_from_config_explicit_theme_returns_it() {
        with_test_env(|| {
            let result = resolve_from_config(Some(ThemeKind::HystersisDay), true);
            assert_eq!(result, ThemeKind::HystersisDay);
            assert!(
                !is_auto_mode(),
    ...
    fn env_theme_overrides_config() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(Some("hystersisday"), Some(ThemeKind::TokyoNight), false),
                ThemeKind::HystersisDay
            );
            assert!(!is_auto_mode());
        });
    ...
    fn unknown_env_theme_falls_through_to_config() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(Some("not-a-theme"), Some(ThemeKind::HystersisDay), false),
                ThemeKind::HystersisDay
            );
        });
    }
    ...
    #[test]
    fn hystersis_theme_wins_over_lc_and_config() {
        with_test_env(|| {
            let env = theme_env(&[("HYSTERSIS_THEME", "hystersisday"), ("LC_HYSTERSIS_THEME", "tokyonight")]);
            assert_eq!(
    ...
    #[test]
    fn empty_or_absent_hystersis_theme_falls_through_to_lc() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[
                        ("HYSTERSIS_THEME", ""),
                        ("LC_HYSTERSIS_THEME", "hystersisday")
                    ])),
                    Some(ThemeKind::TokyoNight),
                    false,
                ),
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("LC_HYSTERSIS_THEME", "hystersisday")])),
                    Some(ThemeKind::TokyoNight),
                    true,
                ),
                ThemeKind::HystersisDay
    ...
    #[test]
    fn unknown_hystersis_theme_falls_through_to_valid_lc() {
        with_test_env(|| {
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[
                        ("HYSTERSIS_THEME", "not-a-theme"),
                        ("LC_HYSTERSIS_THEME", "hystersisday"),
                    ])),
                    Some(ThemeKind::TokyoNight),
                    false,
    ...
    fn unknown_or_empty_theme_env_falls_through_to_config() {
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("HYSTERSIS_THEME", ""), ("LC_HYSTERSIS_THEME", "")])),
                    Some(ThemeKind::HystersisDay),
                    true,
                ),
                ThemeKind::HystersisDay
    ...
    fn env_theme_system_alias_arms_auto_on_both_resolve_paths() {
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("HYSTERSIS_THEME", "system")])),
                    Some(ThemeKind::TokyoNight),
                    false,
                ),
                ThemeKind::HystersisDay
        ...
        with_test_env(|| {
            ...
            assert_eq!(
                resolve_initial_theme_from(
                    env_theme_name_from(&theme_env(&[("LC_HYSTERSIS_THEME", "auto")])),
                    Some(ThemeKind::TokyoNight),
                    true,
                ),
                ThemeKind::HystersisNight
    ...
    fn set_then_current_kind_round_trips() {
        with_test_env(|| {
            ...
            set(ThemeKind::HystersisDay);
            assert_eq!(current_kind(), ThemeKind::HystersisDay);
        });
    }
