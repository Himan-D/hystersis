...
pub use xai_hystersis_config::hystersis_home;
pub use xai_hystersis_tools::util::format_bytes;

/// A closed stdout (`hystersis du | head`) is a clean stop, not a failure.
...
/// Path to `$HYSTERSIS_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    hystersis_home().join("pager.toml")
}

/// `~/.hystersis` or `$HYSTERSIS_HOME`, decided by the resolved home rather than by
/// whether `HYSTERSIS_HOME` is set in the environment.
pub fn display_hystersis_home_prefix() -> String {
    display_hystersis_home_prefix_for(&hystersis_home())
}

pub fn display_hystersis_home_prefix_for(home: &Path) -> String {
    let default = xai_hystersis_config::default_hystersis_home();
    if home == default || home == dunce::canonicalize(&default).unwrap_or(default) {
        "~/.hystersis".to_string()
    } else {
        "$HYSTERSIS_HOME".to_string()
    }
}

/// User-facing path under [`hystersis_home()`], e.g. ``~/.hystersis/config.toml``.
pub fn display_user_hystersis_path(relative: impl AsRef<Path>) -> String {
    display_user_hystersis_path_for(&hystersis_home(), relative)
}

fn display_user_hystersis_path_for(home: &Path, relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_hystersis_home_prefix_for(home);
...
/// Abbreviate an absolute path for display: prefer [`hystersis_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let hystersis = hystersis_home();
    if let Ok(rest) = path_buf.strip_prefix(&hystersis) {
        let prefix = display_hystersis_home_prefix();
...
/// True when `path` is under user [`hystersis_home()`] (not project `{cwd}/.hystersis`).
pub fn is_under_user_hystersis_home(path: &Path) -> bool {
    path.starts_with(hystersis_home())
}
...
mod tests {
    ...
    #[test]
    fn display_hystersis_home_prefix_default_install() {
        if std::env::var("HYSTERSIS_HOME").is_ok() {
            return;
        }
        assert_eq!(display_hystersis_home_prefix(), "~/.hystersis");
    }

    #[test]
    fn display_user_hystersis_path_joins_relative() {
        let path = display_user_hystersis_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".hystersis") || path.contains("$HYSTERSIS_HOME"));
    }

    #[test]
    fn display_user_hystersis_path_for_custom_home_uses_override_label() {
        let custom = std::env::temp_dir().join("hystersis-home-display-regression");
        assert_eq!(
            display_user_hystersis_path_for(&custom, "config.toml"),
            "$HYSTERSIS_HOME/config.toml"
        );
        assert_eq!(
            display_user_hystersis_path_for(&custom, "sandbox.toml"),
            "$HYSTERSIS_HOME/sandbox.toml"
        );
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_hystersis() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.hystersis/memory/MEMORY.md");
