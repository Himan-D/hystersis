...
pub enum TerminalName {
    ...
    JetBrains,
    /// Hystersis Desktop (Electron app).
    #[strum(to_string = "Hystersis Desktop")]
    HystersisDesktop,
...
impl TerminalName {
    ...
    pub fn is_xtermjs_embed(self) -> bool {
        matches!(
            self,
            Self::VsCode | Self::Cursor | Self::Windsurf | Self::HystersisDesktop
        )
    }
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` hystersis is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches hystersis, which
    ...
    pub fn telemetry_snapshot(&self) -> xai_hystersis_telemetry::events::TerminalTelemetry {
        ...
        xai_hystersis_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: xai_hystersis_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> xai_hystersis_shared::session::FeedbackTerminalInfo {
        use xai_hystersis_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(xai_hystersis_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `hystersis ssh` can
    // forward env vars.
    let mut ctx = build_terminal_context_from_env(&env);
...
/// `xai-hystersis-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "hystersisdesktop" => Some(TerminalName::HystersisDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.hystersis/pager.toml` and
