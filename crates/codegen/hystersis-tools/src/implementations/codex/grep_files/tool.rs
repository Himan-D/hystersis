...
use crate::implementations::hystersis_build::grep::ripgrep::rg_path;
...
///
/// Shares `ToolKind::Search` with the hystersis `GrepTool`. These tools are
/// namespace-exclusive — consumers enable either `HystersisBuild` or `Codex` search,
...
impl hystersis_tool_runtime::Tool for CodexGrepFilesTool {
    type Args = CodexGrepFilesInput;
    type Output = CodexGrepFilesOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new("grep_files").expect("valid tool id")
    }
    ...
    ) -> Result<CodexGrepFilesOutput, hystersis_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn test_ctx(cwd: &Path) -> hystersis_tool_runtime::ToolCallContext {
    ...
    async fn tool_reports_empty_pattern_error() {
        ...
        let result = hystersis_tool_runtime::Tool::run(&tool, test_ctx(tmp.path()), input)
            .await
            .unwrap();
        match result {
