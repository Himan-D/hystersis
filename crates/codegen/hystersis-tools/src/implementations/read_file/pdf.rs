...
fn extract_pdf_plain_text(bytes: Vec<u8>, style: PageTextStyle) -> Result<String, String> {
    ...
    let style = match style {
        PageTextStyle::HystersisBuild => PageTextStyle::HystersisBuild,
...
pub(crate) fn extract_pdf_plain_text_all(bytes: Vec<u8>) -> Result<String, String> {
    extract_pdf_plain_text(bytes, PageTextStyle::HystersisBuild)
}
...
pub(crate) fn extract_pdf_text(
    ...
    let text = extract_page_texts(&doc, &page_indices, PageTextStyle::HystersisBuild)?;
