//! LSP server configuration from `.hystersis/lsp.json`.
...
/// This is the canonical merge function — both session startup and `hystersis inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::hystersis_home::hystersis_home().join("lsp.json");
    let project_path = cwd.join(".hystersis").join("lsp.json");
...
/// Load LSP server configs from `~/.hystersis/lsp.json` and `<cwd>/.hystersis/lsp.json`.
