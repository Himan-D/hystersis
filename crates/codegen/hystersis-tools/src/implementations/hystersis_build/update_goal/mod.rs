...
pub use hystersis_tools_api::slash_commands::UPDATE_GOAL_TOOL_NAME;
...
/// access from `xai-hystersis-shell`.
#[doc(hidden)]
...
impl hystersis_tool_runtime::ToolOutput for UpdateGoalOutput {}
...
impl hystersis_tool_runtime::Tool for UpdateGoalTool {
    type Args = UpdateGoalInput;
    type Output = UpdateGoalOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new(UPDATE_GOAL_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<UpdateGoalOutput, hystersis_tool_runtime::ToolError> {
...
pub fn render_ack_into_output(
    ...
    match ack {
        ...
        UpdateGoalAck::Rejected { reason, detail } => Err(hystersis_tool_runtime::ToolError::custom(
            reason.error_code(),
            detail,
        )),
    }
