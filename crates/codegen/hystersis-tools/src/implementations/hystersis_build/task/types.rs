...
use hystersis_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"hystersis-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`hystersis_tool_types::is_not_sentinel`] for existing call sites.
pub use hystersis_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `xai-hystersis-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "HystersisBuild:run_terminal_cmd" | "HystersisBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("hystersis_build", "SubagentEventSender", SubagentEventSender);
...
register_resource!("hystersis_build", "SubagentDepthCounter", SubagentDepthCounter);

/// Host-injected max nesting depth; absent → [`super::MAX_SUBAGENT_DEPTH`].
#[derive(Debug, Clone, Copy)]
pub struct MaxSubagentDepth(pub u32);

register_resource!("hystersis_build", "MaxSubagentDepth", MaxSubagentDepth);
...
register_resource!("hystersis_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("hystersis_build", "SessionIdResource", SessionIdResource);
...
///
/// Set by xai-hystersis-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by xai-hystersis-shell at turn start.
...
register_resource!("hystersis_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("HystersisBuild:run_terminal_cmd", ToolKind::Execute),
                tc("HystersisBuild:read_file", ToolKind::Read),
                tc("HystersisBuild:list_dir", ToolKind::List),
                tc("HystersisBuild:grep", ToolKind::Search),
                tc("HystersisBuild:kill_task", ToolKind::KillTaskAction),
                tc("HystersisBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("HystersisBuild:kill_task", ToolKind::KillTaskAction),
                tc("HystersisBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("HystersisBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("HystersisBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
