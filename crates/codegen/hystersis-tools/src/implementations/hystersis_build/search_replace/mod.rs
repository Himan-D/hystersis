...
register_resource!("hystersis_build", "SearchReplace", SearchReplaceParams);
            ...
            let output = match e.io_error_kind() {
                ...
                _ => {
                    return Err(hystersis_tool_runtime::ToolError::execution(
                        hystersis_tool_protocol::ToolId::new("search_replace").expect("valid"),
                        e.to_string(),
                    ));
                }
            };
...
impl hystersis_tool_runtime::Tool for SearchReplaceTool {
    type Args = SearchReplaceInput;
    type Output = SearchReplaceOutput;
    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new("search_replace").expect("valid tool id")
    }
    fn description(
        &self,
...
mod tests {
    ...
    async fn consecutive_edits_succeed_without_prior_read() {
        ...
        let result1 = hystersis_tool_runtime::Tool::run(
            &tool,
        ...
        let result2 = hystersis_tool_runtime::Tool::run(
            &tool,
            test_ctx(shared),
