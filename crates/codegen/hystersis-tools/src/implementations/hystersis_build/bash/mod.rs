...
use regex::Regex;
use hystersis_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<hystersis_tool_protocol::ToolCapabilities> =
    LazyLock::new(|| hystersis_tool_protocol::ToolCapabilities {
        is_read_only: false,
        tool_scope: Some(hystersis_tool_protocol::ToolScope::Write),
        streaming: Some(hystersis_tool_protocol::StreamingSpec {
...
fn bash_output_chunk_progress(
    spec: &hystersis_tool_protocol::StreamingSpec,
    chunk: &BashOutputChunk,
    last_total: &mut usize,
) -> Option<hystersis_tool_runtime::ToolProgress> {
    // `stream_chunk` counts in `u64`; convert at the boundary.
    let mut cursor = *last_total as u64;
    let progress = hystersis_tool_runtime::stream_chunk(
        spec,
        &chunk.base.output,
...
pub struct BashParams {
    ...
    /// hystersis opts up to 10h via xai-hystersis-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "hystersis_build.Bash";
...
impl hystersis_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<hystersis_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => hystersis_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production hystersis sets it to 10h in xai-hystersis-shell's
...
/// [`hystersis_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `HYSTERSIS_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("HYSTERSIS_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in hystersis_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the hystersis_build
...
impl BashTool {
    ...
    /// (xai-hystersis-cursor) uses this ceiling to report the true FG wait in
    ...
    ///   the terminal backend default applies (`HYSTERSIS_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `HYSTERSIS_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = hystersis_config::shell::chain_separator();
...
impl hystersis_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: hystersis_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> hystersis_tool_runtime::ToolStream<BashToolOutput> {
        ...
        if input.is_background {
            ...
            return Box::pin(async_stream::stream! {
                yield hystersis_tool_runtime::ToolStreamItem::Terminal(this.run(ctx, input).await);
            });
        }
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield hystersis_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield hystersis_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, hystersis_tool_runtime::ToolError> {
        ...
        let ampersand = hystersis_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(hystersis_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing hystersis_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (hystersis_build backend) and can surface an educational
...
mod tests {
    ...
    fn read_chunk_progress(p: &hystersis_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
            hystersis_tool_runtime::ToolProgress::Custom { subkind, payload } => {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let hystersis_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = hystersis_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, hystersis_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                hystersis_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn foreground_command_success() {
        ...
        let result = hystersis_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = hystersis_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn errors_when_terminal_not_in_resources() {
        ...
        let result =
            hystersis_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), make_input("ls"))
                .await;
        assert!(result.is_err());
        assert!(
            result
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = hystersis_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(hystersis_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // hystersis-shell opts up to 10h via max_timeout_secs. Background unaffected.
    ...
    mod versioned_background_check {
        ...
        ) -> hystersis_tool_runtime::ToolCallContext {
            let mut ctx =
                hystersis_tool_runtime::ToolCallContext::new(hystersis_tool_protocol::ToolCallId::new_v7());
            ctx.extensions.insert(resources);
            ctx.extensions.insert(hystersis_tool_runtime::BehaviorVersion(
