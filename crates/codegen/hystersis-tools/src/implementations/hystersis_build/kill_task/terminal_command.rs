...
use crate::implementations::hystersis_build::task_output::background_bash_requires_exprs;
use crate::types::requirements::{Expr, ToolRequirement};
use crate::types::tool::{ToolKind, ToolNamespace};
use hystersis_tool_types::{KillTaskOutput, KillTaskToolInput};
...
impl hystersis_tool_runtime::Tool for KillTerminalCommandTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new("kill_terminal_command").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        hystersis_tool_runtime::Tool::run(&KillTaskTool, ctx, input).await
    }
}
