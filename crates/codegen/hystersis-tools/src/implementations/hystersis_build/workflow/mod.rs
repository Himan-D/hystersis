...
pub use hystersis_tools_api::slash_commands::WORKFLOW_TOOL_NAME;

/// Short name of a tool id (`HystersisBuild:workflow` → `workflow`).
...
pub enum WorkflowSource {
    Name {
        #[schemars(
            description = "Name of a registered workflow (built-in, or discovered from the project `.hystersis/workflows/` or user `~/.hystersis/workflows/`)."
        )]
        name: String,
    },
    Script {
...
impl hystersis_tool_runtime::ToolOutput for WorkflowToolOutput {}
...
A started run gets a session-unique display name (e.g. `review-changes`, `review-changes-2`) — the handle to show the user and use with `/workflow pause|resume|stop <name>`; keep run IDs internal. Each launch persists an editable `script_path`; edit it and launch as a new run to iterate. Use the `resume` source only for a same-process paused run (process restarts are terminal); it reuses the run's original immutable source and args, and a budget-limited run resumes only with a higher `agent_budget`. Save reusable scripts to `.hystersis/workflows/<name>.rhai`."##
    }
...
impl hystersis_tool_runtime::Tool for WorkflowTool {
    type Args = WorkflowToolInput;
    type Output = WorkflowToolOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new(WORKFLOW_TOOL_NAME).expect("valid tool id")
    }
    ...
    async fn run(
    ...
    ) -> Result<WorkflowToolOutput, hystersis_tool_runtime::ToolError> {
        ...
        match ack_rx.await {
            ...
            Ok(WorkflowLaunchAck::Rejected { code, detail }) => {
                Err(hystersis_tool_runtime::ToolError::custom(code, detail))
            }
            Err(_) => Err(hystersis_tool_runtime::ToolError::custom(
...
mod tests {
    ...
    fn workflow_id_matches_kind_or_short_name() {
        ...
        assert!(is_workflow_tool_id("HystersisBuild:workflow"));
        assert!(!is_workflow_tool_id("web_search"));
        assert!(is_workflow_tool(Some(ToolKind::Workflow), "anything"));
        assert!(is_workflow_tool(None, "HystersisBuild:workflow"));
