...
//! `LspTool` (Tool trait impl) under the `HystersisBuild` namespace.
...
impl hystersis_tool_runtime::ToolOutput for LspToolOutput {}
...
impl hystersis_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, hystersis_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    hystersis_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.hystersis/lsp.json or <cwd>/.hystersis/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
