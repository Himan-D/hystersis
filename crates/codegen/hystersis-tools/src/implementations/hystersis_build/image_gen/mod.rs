...
const XAI_IMAGINE_MODEL: &str = "hystersis-imagine-image-quality";
// Some Imagine models (e.g. `hystersis-imagine-image`, selectable via `model_override`)
...
/// HystersisPro upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a HystersisPro feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to HystersisPro: https://hystersis.com/upgrade. Do not retry this tool.";
...
pub struct ImageGenClient {
    ...
    /// HTTP call and return the HystersisPro upsell prose instead. See
...
impl ImageGenClient {
    ...
    /// with the HystersisPro upsell instead of issuing a doomed request.
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// HystersisPro upsell prose instead of a doomed request. Set by the
...
pub const SESSION_ID_HEADER: &str = "x-hystersis-session-id";
...
impl hystersis_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(hystersis_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
    }
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("hystersis-imagine-image".into()),
        ...
        assert_eq!(cfg.model_override(), Some("hystersis-imagine-image"));
    ...
    async fn post_json_attaches_session_and_bearer_headers() {
        ...
        let req = client
            .post_json(
                "https://api.hystersis.com/v1/images",
                &serde_json::json!({}),
                Some("tok"),
            )
            .build()
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("hystersis-imagine-image")), None)
                .unwrap()
                .model,
            "hystersis-imagine-image"
        );
    }
    ...
    fn client_selects_edit_model_from_override() {
        ...
        let client = ImageGenClient::new(&mk(Some("hystersis-imagine-image-v2")), None).unwrap();
        assert_eq!(client.edit_model(), "hystersis-imagine-image-v2");
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the HystersisPro upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("HystersisPro"), "got: {}", t.text);
                assert!(t.text.contains("hystersispro?referrer=hystersis"));
            }
