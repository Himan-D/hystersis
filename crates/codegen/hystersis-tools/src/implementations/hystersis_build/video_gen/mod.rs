...
const XAI_VIDEO_MODEL: &str = "hystersis-imagine-video-1.5";
...
const DEFAULT_ZDR_VIDEO_KEY_PREFIX: &str = "hystersis-videos/";
...
pub use hystersis_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> hystersis_file_utils::s3::S3StaticCredentials {
        hystersis_file_utils::s3::S3StaticCredentials {
...
pub struct VideoGenClient {
    ...
    /// and return the HystersisPro upsell prose. See [`VideoGenClient::is_tier_restricted`].
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, hystersis_tool_runtime::ToolError> {
        let VideoGenConfig::Enabled {
            ...
            return Err(hystersis_tool_runtime::ToolError::invalid_arguments(
        ...
        headers.insert(
            ...
            HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|e| {
                hystersis_tool_runtime::ToolError::invalid_arguments(format!(
        ...
        extra_headers.into_iter().try_for_each(|(key, value)| {
            ...
            Ok::<(), hystersis_tool_runtime::ToolError>(())
        })?;
    ...
    /// HystersisPro upsell instead of issuing a doomed request.
    ...
    pub async fn generate_with_images(
    ...
    ) -> Result<VideoOutcome, hystersis_tool_runtime::ToolError> {
        ...
        loop {
            ...
            if !poll_status.is_success() && poll_status.as_u16() != 202 {
                ...
                return Err(hystersis_tool_runtime::ToolError::new(
                    hystersis_tool_runtime::ToolErrorKind::Custom,
                ...
                "done" => {
                    ...
                    return match presigned {
                        ...
                        None if video_url.is_empty() => {
                            Err(hystersis_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn download_video(&self, url: &str) -> Result<Vec<u8>, hystersis_tool_runtime::ToolError> {
        let response = self.download_http.get(url).send().await.map_err(|e| {
            hystersis_tool_runtime::ToolError::invalid_arguments(format!("Failed to download video: {e}"))
        })?;
    ...
    async fn finish_zdr_video(
        ...
        let config = self.zdr_video_output_s3.as_ref().ok_or_else(|| {
            hystersis_tool_runtime::ToolError::invalid_arguments(
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, hystersis_tool_runtime::ToolError> {
        ...
        let upload_url = hystersis_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, hystersis_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = hystersis_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
pub enum VideoGenConfig {
    ...
    Enabled {
        ...
        /// at call time with the HystersisPro upsell prose. Set by the host from
...
/// the deliberate `/imagine-video` slash command shows the HystersisPro upsell
/// modal instead.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Video generation is a HystersisPro feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to HystersisPro: https://hystersis.com/upgrade. Do not retry this tool.";
...
pub(crate) const ZDR_RESTRICTED_MESSAGE: &str = "Video generation tools are unavailable under zero data retention (ZDR). To enable, either turn off /privacy mode to disable ZDR or supply a user-hosted storage bucket (see https://docs.hystersis.com/build/settings/zdr-video-storage).";

fn zdr_restricted_error() -> hystersis_tool_runtime::ToolError {
    hystersis_tool_runtime::ToolError::new(
        hystersis_tool_runtime::ToolErrorKind::Custom,
...
fn video_http_error(status: reqwest::StatusCode, body: &str) -> hystersis_tool_runtime::ToolError {
    if is_zdr_upload_url_error(body) {
...
async fn resolve_image_reference(value: &str) -> Result<String, hystersis_tool_runtime::ToolError> {
    ...
    if value.starts_with("data:image/") {
        let comma = value.find(',').ok_or_else(|| {
            hystersis_tool_runtime::ToolError::invalid_arguments("malformed data URL in image reference")
        })?;
    ...
    let (_w, _h, mime) =
        crate::util::image_validate::validate_image_bytes(&raw_bytes).map_err(|e| {
            hystersis_tool_runtime::ToolError::invalid_arguments(format!("invalid image reference: {e}"))
        })?;
...
fn validate_one_of(
    ...
    Err(hystersis_tool_runtime::ToolError::invalid_arguments(format!(
        "`{field}` must be one of: {}. Got {value}.",
        allowed.join(", ")
    )))
}

fn validate_imagine_duration(duration: Option<u32>) -> Result<(), hystersis_tool_runtime::ToolError> {
...
fn validate_r2v_duration(duration: Option<u32>) -> Result<(), hystersis_tool_runtime::ToolError> {
    if let Some(secs) = duration
...
) -> Result<(VideoGenClient, std::path::PathBuf), hystersis_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, hystersis_tool_runtime::ToolError> {
    let absolute_path = client
        .writer
        .save(session_folder, video_bytes, None)
        .await
        .map_err(|e| hystersis_tool_runtime::ToolError::invalid_arguments(e.to_string()))?;
...
) -> Result<MediaGenOutput, hystersis_tool_runtime::ToolError> {
    match outcome {
...
impl hystersis_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
    ...
    fn description(
        ...
        hystersis_tool_types::ToolDescription::new(
    ...
    fn capabilities(&self) -> hystersis_tool_protocol::ToolCapabilities {
        hystersis_tool_protocol::ToolCapabilities {
            is_read_only: false,
            tool_scope: Some(hystersis_tool_protocol::ToolScope::Write),
            ..Default::default()
        }
    }
...
impl hystersis_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    async fn request_attaches_session_and_bearer_headers() {
        ...
        let req = client
            .request(
                reqwest::Method::POST,
                "https://api.hystersis.com/v1/videos",
                Some("tok"),
            )
            .build()
            .unwrap();
    ...
    async fn image_to_video_rejects_bad_duration() {
        ...
        let err = hystersis_tool_runtime::Tool::run(
            &tool,
