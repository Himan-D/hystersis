...
crate::register_resource!("hystersis_build", "UseTool", UseToolParams);
...
async fn dispatch_local_mcp(
    ...
    let tool_id = hystersis_tool_protocol::ToolId::new(tool_name).map_err(|_| {
        hystersis_tool_runtime::ToolError::invalid_arguments(format!("invalid tool name: '{tool_name}'"))
    })?;
...
fn is_local_tool_id_rejection(err: &hystersis_tool_runtime::ToolError, tool_name: &str) -> bool {
    err.kind == hystersis_tool_runtime::ToolErrorKind::InvalidArguments
    ...
    if let Some(source) = gateway_source {
            ...
            && let Some(dispatch) = dispatch.clone()
        {
            match dispatch_local_mcp(dispatch, tool_name, tool_input.clone(), ctx.clone()).await {
                ...
                Err(err)
                    if err.kind != hystersis_tool_runtime::ToolErrorKind::NotFound
...
impl hystersis_tool_runtime::Tool for UseTool {
    type Args = UseToolInput;
    type Output = ToolOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new(USE_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    impl hystersis_tool_runtime::ToolDispatch for MockToolDispatch {
        async fn call(
            &self,
            ...
            hystersis_tool_runtime::terminal_only(Ok(hystersis_tool_runtime::TypedToolOutput::from_value(
                tool_id, value,
            )))
        }
    }
    ...
    #[async_trait::async_trait]
    impl hystersis_tool_runtime::ToolDispatch for CapturingDispatch {
        async fn call(
            &self,
            tool_id: hystersis_tool_protocol::ToolId,
            ...
            if !matches!(
                ...
                return hystersis_tool_runtime::terminal_only(Err(
                    hystersis_tool_runtime::ToolError::not_found(tool_id, "Tool not found"),
                ));
            }
    ...
    fn ctx_capturing() -> (hystersis_tool_runtime::ToolCallContext, SharedArgs) {
    ...
    #[async_trait::async_trait]
    impl hystersis_tool_runtime::ToolDispatch for NotFoundDispatch {
        async fn call(
            &self,
            tool_id: hystersis_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            hystersis_tool_runtime::terminal_only(Err(hystersis_tool_runtime::ToolError::not_found(
                tool_id,
                "Tool not found",
            )))
        }
    ...
    #[async_trait::async_trait]
    impl hystersis_tool_runtime::ToolDispatch for InvalidArgumentsDispatch {
        async fn call(
            &self,
            _tool_id: hystersis_tool_protocol::ToolId,
            _args: serde_json::Value,
            _ctx: hystersis_tool_runtime::ToolCallContext,
        ) -> hystersis_tool_runtime::ToolStream<hystersis_tool_runtime::TypedToolOutput> {
            hystersis_tool_runtime::terminal_only(Err(hystersis_tool_runtime::ToolError::invalid_arguments(
    ...
    #[async_trait::async_trait]
    impl hystersis_tool_runtime::ToolDispatch for ErrorToolDispatch {
        async fn call(
            &self,
            _tool_id: hystersis_tool_protocol::ToolId,
            _args: serde_json::Value,
            ...
            let tid = hystersis_tool_protocol::ToolId::new(&self.error)
    ...
    fn new_ctx() -> hystersis_tool_runtime::ToolCallContext {
        let call_id = hystersis_tool_protocol::ToolCallId::new_v7();
        hystersis_tool_runtime::ToolCallContext::new(call_id)
    }

    fn ctx_with_dispatch(
        dispatch: impl hystersis_tool_runtime::ToolDispatch + 'static,
    ...
    async fn rejects_builtin_tool_names() {
        ...
        assert_eq!(err.kind, hystersis_tool_runtime::ToolErrorKind::InvalidArguments);
    ...
    async fn gateway_null_arguments_default_to_object() {
        ...
        hystersis_tool_runtime::Tool::run(
            &UseTool,
            ctx,
            UseToolInput {
                tool_name: "grafana__search_dashboards".into(),
    ...
    fn classify_python_repr_single_line_is_long_line_text() {
        ...
        let row = "{'id': 0, 'name': 'user0', 'email': 'u0@hystersis.com', 'age': 20}";
    ...
    fn classify_csv_stays_plain_text() {
        ...
        for i in 0..50 {
            csv.push_str(&format!("{i},user{i},u{i}@hystersis.com,{}\n", 20 + i));
        }
    ...
    fn ctx_capturing_with_resources(
        resources: crate::types::resources::SharedResources,
    ) -> (hystersis_tool_runtime::ToolCallContext, SharedArgs) {
        let args: SharedArgs = Arc::new(std::sync::Mutex::new(None));
        let mut ctx = new_ctx();
        ctx.extensions
