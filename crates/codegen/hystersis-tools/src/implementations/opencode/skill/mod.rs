...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.hystersis/skills/ or .hystersis/skills/)
${%- endif %}
</available_skills>"#;
...
impl hystersis_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> hystersis_tool_protocol::ToolId {
        hystersis_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, hystersis_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(hystersis_tool_runtime::Tool::id(&tool).as_str(), "skill");
