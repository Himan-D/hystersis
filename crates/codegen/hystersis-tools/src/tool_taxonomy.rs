...
/// object (mirroring `hystersis.com/mcp_tool`). Consumers deserialize it into
/// [`CanonicalToolMeta`].
pub const TOOL_META_KEY: &str = "hystersis.com/tool";
...
impl ToolKind {
    ...
    /// `hystersis.com/tool`. Exhaustive, so a new `ToolKind` must add a label to compile.
...
///   share it (hystersis `read_file` → `"Read"`).
...
///   should read `namespace` loosely (as a string) and, on any `hystersis.com/tool`
...
impl CanonicalToolMeta {
    ...
    /// (`bash_mode`, `backend`, `hystersis.com/mcp_tool`, …).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::HystersisBuild,
    ...
    fn writing_tool_kind_matches_definition_sites() {
        ...
        covered(crate::implementations::hystersis_build::SearchReplaceTool);
        covered(crate::implementations::hystersis_build::BashTool);
        covered(crate::implementations::hystersis_build::TodoWriteTool);
        covered(crate::implementations::hystersis_build::WorkflowTool);
        covered(crate::implementations::hystersis_build::ImageGenTool);
        covered(crate::implementations::hystersis_build::ImageEditTool);
        covered(crate::implementations::hystersis_build::ImageToVideoTool);
        covered(crate::implementations::hystersis_build::ReferenceToVideoTool);
        covered(crate::implementations::hystersis_build::AskUserQuestionTool);
        ...
        covered(crate::implementations::hystersis_build_hashline::HashlineEditTool);
    }
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::HystersisBuild => ("hystersis_build", "HystersisBuild"),
                ToolNamespace::HystersisBuildConcise => ("hystersis_build_concise", "HystersisBuildConcise"),
                ToolNamespace::HystersisBuildHashline => ("hystersis_build_hashline", "HystersisBuildHashline"),
