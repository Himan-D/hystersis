//! ToolBridge: adapter that wraps `xai-hystersis-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the hystersis_build agent and `None` for
    ...
    pub async fn kill_background_task(
        ...
        task_id: &str,
        source: KillSource,
    ) -> Result<KillOutcome, hystersis_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn list_scheduled_tasks(
        &self,
    ) -> Vec<crate::implementations::hystersis_build::scheduler::types::ScheduledTask> {
        use crate::implementations::hystersis_build::scheduler::types::{
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, hystersis_tool_runtime::ToolError> {
        use crate::implementations::hystersis_build::scheduler::types::{
            SchedulerCommand, SchedulerHandle,
        };
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    hystersis_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        sender
            ...
            .map_err(|_| {
                hystersis_tool_runtime::ToolError::custom("process_manager", "Scheduler actor stopped")
            })?;
        reply_rx
            .await
            .map_err(|_| {
            ...
            .map_err(crate::implementations::hystersis_build::scheduler::types::scheduler_tool_error)
    }
...
mod tests {
    ...
    impl hystersis_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> hystersis_tool_protocol::ToolId {
            hystersis_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::hystersis_tool_runtime::ListToolsContext,
        ) -> hystersis_tool_types::ToolDescription {
            hystersis_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + hystersis_build's snake_case in one registry
