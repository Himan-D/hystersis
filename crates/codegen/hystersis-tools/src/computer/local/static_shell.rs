...
pub use hystersis_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__HYSTERSIS_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    hystersis_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = hystersis_config::shell::detect_unix_shell_kind();
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias hystersis_alias_probe='echo ALIAS_OK'\nhystersis_fn_probe() { echo FN_OK; }\n",
            "hystersis_alias_probe && hystersis_fn_probe",
        )
        .await;
