...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.hystersis/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.hystersis/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.hystersis/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.hystersis/ or ~/.claude/.
    User { path: PathBuf },
