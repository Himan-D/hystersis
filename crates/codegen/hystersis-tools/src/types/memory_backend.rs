...
//!
//! `MemoryBackend` is defined in `xai-hystersis-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `xai-hystersis-tools`.
/// Mirrors `hystersis_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "hystersis_memory";
