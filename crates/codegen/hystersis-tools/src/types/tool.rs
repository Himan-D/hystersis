...
//!
//! The tool runtime contract (`Tool` trait) lives in `hystersis_tool_runtime`.
...
///
/// Serializes to snake_case (`hystersis_build`, `mcp`, …) for the
...
/// (e.g. `"HystersisBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "HystersisBuild")]
    HystersisBuild,
    #[serde(alias = "HystersisBuildConcise")]
    HystersisBuildConcise,
    #[serde(alias = "HystersisBuildHashline")]
    HystersisBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
