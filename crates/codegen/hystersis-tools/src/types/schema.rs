...
///
/// Use with `#[schemars(with = "HystersisIntegerSchema")]` on `Option<usize>` fields.
pub struct HystersisIntegerSchema;
impl schemars::JsonSchema for HystersisIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "hystersis_integer_schema".into()
    }
...
/// `z.coerce.string()` used by the TypeScript hystersis-computer tools, where
...
pub use hystersis_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
