...
//! after every tool call to check for SKILL.md files in `.hystersis/skills/`,
...
/// Directories that contain skill definitions (`.hystersis/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".hystersis", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.hystersis/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.hystersis/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.hystersis/) are outside the git root so the walk breaks early.
