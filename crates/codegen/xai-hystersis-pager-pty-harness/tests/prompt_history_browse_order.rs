...
use xai_hystersis_pager_pty_harness::{ContentController, PtyHarness, keys, pager_binary};
...
async fn run() -> Result<()> {
    ...
    std::fs::create_dir_all(&hystersis_home).context("create sandbox .hystersis")?;
    std::fs::write(
