...
/// the shell's `HystersisAuth::is_zdr_team` trigger) — locks the settings modal's
...
/// names must match the shell's `HystersisAuth` serde names in
/// `xai-hystersis-shell/src/auth/model.rs`).
fn seed_fake_oauth_raw(
    ...
    std::fs::write(
        ...
        format!(
            r#"{{
  "https://auth.hystersis.com::b1a00492-073a-47ea-816f-4c329264a828": {{
