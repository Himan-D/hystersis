//! Scripted TUI scenario runner for xai-hystersis-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$HYSTERSIS_HOME`
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&hystersis_home)
                .with_context(|| format!("create scenario HYSTERSIS_HOME {}", hystersis_home.display()))?;
            fs::write(hystersis_home.join("config.toml"), config_toml)
...
mod tests {
    ...
    fn workspace_rejects_paths_outside_the_tempdir() {
        ...
        let sandbox = xai_hystersis_test_support::TestSandbox::new();
