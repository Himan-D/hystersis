//! `pty-bench` — PTY benchmark CLI for `xai-hystersis-pager`.
//!
...
//! ```bash
//! cargo bench -p xai-hystersis-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/hystersis-${VERSION}-linux-x86_64 \
//!   cargo bench -p xai-hystersis-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for xai-hystersis-pager",
    long_about = None,
)]
struct Cli {
