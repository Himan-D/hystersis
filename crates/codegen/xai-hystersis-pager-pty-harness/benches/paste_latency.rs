//! `paste-latency` — clipboard paste latency benchmark for `xai-hystersis-pager`.
//!
...
//!
//! ```bash
//! cargo bench -p xai-hystersis-pager-pty-harness --bench paste_latency -- --iterations 10
//!
//! # Compare an old release artifact, text mode only, JSON to a file:
//! cargo bench -p xai-hystersis-pager-pty-harness --bench paste_latency -- \
//!   --binary ~/Downloads/hystersis-old --mode text --json /tmp/paste-old.json
//! ```
...
#[command(
    name = "paste-latency",
    about = "Clipboard paste latency benchmark for xai-hystersis-pager (macOS, real pasteboard)",
    long_about = None,
)]
struct Cli {
