...
use hystersis_sampling_types::{ContentPart, ConversationItem, SyntheticReason, ToolResultItem};
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = hystersis_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = hystersis_sampling_types::repair_dangling_tool_calls(
        items,
        hystersis_sampling_types::DanglingToolCallReason::HarnessHalted {
