...
use hystersis_compaction_transcript::CompactionDetail;
...
impl CompactionMode {
    ...
    /// resolver attach the separately-resolved `HYSTERSIS_COMPACTION_DETAIL`.
    ...
    pub fn transcript_hint(self, location: Option<&str>) -> Option<String> {
        use hystersis_compaction_transcript::INDEX_FILE;
        let loc = location?;
        Some(match self {
