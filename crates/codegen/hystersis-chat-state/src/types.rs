...
/// emitter in `xai-hystersis-shell` and the upsert/detection here — a drift would
...
pub enum AuthType {
    /// From AuthManager (hystersis login, OIDC, external binary). Refreshable.
    #[default]
    SessionToken,
...
/// `hystersis_sampling_types::SamplingConfig` (which is secret-free).
