...
use tokio::sync::mpsc;
use hystersis_sampling_types::{ConversationItem, SamplingConfig};
...
async fn record_last_turn_usage_round_trip() {
    use hystersis_sampling_types::TokenUsage;
...
async fn provider_counted_model_output_persists_without_bumping_estimate() {
    ...
    h.handle.push_model_output(ConversationItem::Reasoning(
        hystersis_sampling_types::synthesized_reasoning_item("r".repeat(4_000)),
    ));
...
async fn strip_conversation_images_replaces_only_listed_urls_and_persists() {
    ...
    assert!(
        u.content
            .iter()
            .all(|p| !matches!(p, hystersis_sampling_types::ContentPart::Image { .. })),
    ...
    assert!(
        survivor
            .content
            .iter()
            .any(|p| matches!(p, hystersis_sampling_types::ContentPart::Image { .. })),
...
async fn build_request_with_tool_definitions() {
    use hystersis_sampling_types::ToolSpec;
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools =
        ConversationItem::Assistant(hystersis_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use hystersis_sampling_types::{ContentPart, UserItem};
...
async fn sampling_config_survives_compaction_replacement() {
    use hystersis_sampling_types::ApiBackend;
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            hystersis_sampling_types::AssistantItem {
...
fn serialize_via_public_api(
    req: &hystersis_sampling_types::ConversationRequest,
) -> serde_json::Value {
    use hystersis_sampling_types::rs;
    let create_response: rs::CreateResponse = req.into();
    let mut body = serde_json::to_value(&create_response).unwrap();
    hystersis_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &hystersis_sampling_types::ConversationRequest,
    extended: &hystersis_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    use hystersis_sampling_types::ContentPart;
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(hystersis_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(hystersis_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn build_request_budgets_tool_images_on_request_copy_only() {
    use hystersis_sampling_types::{ContentPart, ToolCall};
...
async fn prefix_stable_with_backend_tool_calls() {
    use hystersis_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
