//! Config file loading for Hystersis.
//!
//! Merge order (lowest → highest priority):
//! 1. `/etc/hystersis/managed_config.toml`
//! 2. `$HYSTERSIS_HOME/managed_config.toml`
//! 3. `$HYSTERSIS_HOME/config.toml`
//! 4. `$HYSTERSIS_HOME/requirements.toml` (cloud cache; Ed25519-signed at rest once a
//!    key is embedded — see [`signed_policy`] — below the OS-protected layers)
//! 5. `/etc/hystersis/requirements.toml`
//! 6. macOS MDM managed preferences (`ai.x.hystersis`, admin-forced) — macOS only
//!
...
pub use global_hook_sources::{
    ...
    ensure_hystersis_hook_slots, existing_ancestor_chain, is_direct_hook_json_name,
...
pub use env_overlay::{
    HYSTERSIS_CONFIG_ENV, HYSTERSIS_CONFIG_PATH_ENV, OverlaySource, ResolvedOverlay, resolved_env_overlay,
};
#[cfg(unix)]
pub use global_hook_sources::{
...
pub use paths::{
    ...
    decode_cwd_from_dirname, default_hystersis_home, encode_cwd_dirname, ensure_sessions_cwd_dir,
    ensure_sessions_cwd_dir_in, hystersis_application, hystersis_application_in, hystersis_home, sessions_cwd_dir,
    sessions_cwd_dir_in, set_dir_owner_only, system_config_dir, user_hystersis_home,
};
pub use validation::{
