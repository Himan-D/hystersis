//! Filesystem locations for hystersis config files and binaries.

use std::path::PathBuf;

pub use xai_hystersis_home::{default_hystersis_home, hystersis_home, user_hystersis_home};
...
/// Canonical hystersis application path: `$HYSTERSIS_HOME/bin/hystersis` (Unix) or `hystersis.exe` (Windows).
pub fn hystersis_application() -> PathBuf {
    hystersis_application_in(&hystersis_home())
}

/// [`hystersis_application`] under an explicit home instead of `$HYSTERSIS_HOME`.
pub fn hystersis_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "hystersis.exe" } else { "hystersis" };
    home.join("bin").join(name)
}

/// System-wide config directory: `/etc/hystersis/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/hystersis"))
    } else {
        None
    }
}
...
/// `hystersis_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    sessions_cwd_dir_in(&hystersis_home(), cwd)
}

/// [`sessions_cwd_dir`] with an injectable hystersis home — the single source of
/// truth for the `sessions/<encoded-cwd>` path shape.
pub fn sessions_cwd_dir_in(hystersis_home: &std::path::Path, cwd: &str) -> PathBuf {
    hystersis_home.join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ensure_sessions_cwd_dir_in(&hystersis_home(), cwd)
}

/// [`ensure_sessions_cwd_dir`] with an injectable hystersis home.
pub fn ensure_sessions_cwd_dir_in(
    hystersis_home: &std::path::Path,
    cwd: &str,
) -> std::io::Result<PathBuf> {
    let encoded_name = encode_cwd_dirname(cwd);
    let dir = sessions_cwd_dir_in(hystersis_home, cwd);
    ...
    set_dir_owner_only(&hystersis_home.join("sessions"));
...
mod tests {
    ...
    fn ensure_sessions_cwd_dir_retightens_existing_loose_dirs() {
        ...
        let dir = ensure_sessions_cwd_dir_in(home.path(), "/some/project").unwrap();
        // Simulate dirs created by an older hystersis with umask-default perms.
        std::fs::set_permissions(&dir, std::fs::Permissions::from_mode(0o755)).unwrap();
