...
//! `HYSTERSIS_CONFIG` overlay, and campaign resolution.
...
/// Whether a layer merge includes the `HYSTERSIS_CONFIG` overlay.
...
pub struct ConfigLayers {
    ...
    pub user: toml::Value,
    /// `HYSTERSIS_CONFIG` / `HYSTERSIS_CONFIG_PATH` overlay, above user but below
    ...
    /// constructs this field directly, and `hystersis inspect`). Construct this field
    ...
    /// `auth_provider`, `model_providers`, `plugins`, `auth`, `hystersis_com_config`,
    ...
    /// `HYSTERSIS_SANDBOX` / `OTEL_*`); `[features] telemetry` is the master switch
    /// and is allowlisted.
    ///
...
impl ConfigLayers {
    ...
    /// Layer merge (no campaigns), including the `HYSTERSIS_CONFIG` overlay.
    ///
    ...
    /// Layer merge excluding the `HYSTERSIS_CONFIG` overlay, for security gates.
    ...
    /// wraps this with the `HYSTERSIS_CAMPAIGNS_OVERRIDE` env layer.
    ...
    /// Apply campaign patches, re-apply the `HYSTERSIS_CONFIG` overlay, then restore requirements.
...
/// `HYSTERSIS_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("HYSTERSIS_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`xai_hystersis_shell::util::config::load_effective_config`) when remote campaigns
/// or `HYSTERSIS_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// Path to `$HYSTERSIS_HOME/campaigns_state.json` under `home`.
pub fn campaigns_state_path(home: &std::path::Path) -> std::path::PathBuf {
    home.join(CAMPAIGNS_STATE_FILE)
}

/// Fail-open dismissed ids from `$HYSTERSIS_HOME/campaigns_state.json`.
pub fn load_dismissed_ids_from_home() -> std::collections::HashSet<String> {
    let Some(home) = crate::user_hystersis_home() else {
...
mod tests {
    ...
    /// `HYSTERSIS_CAMPAIGNS=0` disables campaign application regardless of config.
    /// `HYSTERSIS_CAMPAIGNS` is process-global, so this test serializes itself with a
    ...
    fn kill_switch_env_var_disables() {
        ...
        let prior = std::env::var_os("HYSTERSIS_CAMPAIGNS");
        let empty = toml::Value::Table(Default::default());

        // SAFETY: ENV_GUARD serializes this against itself; no other test in the
        // crate mutates or reads HYSTERSIS_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("HYSTERSIS_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("HYSTERSIS_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("HYSTERSIS_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("HYSTERSIS_CAMPAIGNS") },
        }
    }
