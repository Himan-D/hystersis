...
use crate::paths::{system_config_dir, user_hystersis_home};
...
/// hystersis config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_hystersis_home().as_deref(), USER_CONFIG_FILENAME)
}

/// User config filename (`$HYSTERSIS_HOME/config.toml`), shared by the loaders here.
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_hystersis_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.hystersis/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.hystersis` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/hystersis`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
pub fn managed_config_layers() -> Vec<ManagedConfigLayer> {
    managed_config_layers_at(system_config_dir().as_deref(), user_hystersis_home().as_deref())
}
...
/// A hook's origin (held by `xai_hystersis_hooks::HookSpec::layer`). Defined here, not
/// in `xai-hystersis-hooks`, since the dep direction is `xai-hystersis-hooks -> xai-hystersis-config`;
/// this crate sets the config tiers, `File`/`Plugin` are set downstream.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum HookProvenance {
    /// `/etc/hystersis/managed_config.toml`.
    SystemManaged,
    /// `$HYSTERSIS_HOME/managed_config.toml` (server-synced).
    Managed,
...
pub fn hook_config_layers() -> Vec<HookConfigLayer> {
    hook_config_layers_at(system_config_dir().as_deref(), user_hystersis_home().as_deref())
}
...
/// can't be parsed (broken `HYSTERSIS_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match xai_hystersis_version::installed_semver() {
...
mod tests {
    ...
    fn load_user_config_layer_reads_file_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("hystersis-load-layer-{}", std::process::id()));
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("hystersis-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
