//! Hystersis-owned direct global hook paths shared by shell discovery and sandbox
//! write-deny: `$HYSTERSIS_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$HYSTERSIS_HOME/hooks/` (discovered + protected).
    HookDirectory,
    /// `$HYSTERSIS_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked HYSTERSIS_HOME is not allowed under sandbox write-deny: {path}")]
    SymlinkedHystersisHome { path: PathBuf },
    ...
    #[error("Hystersis hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("Hystersis hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create Hystersis hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create Hystersis hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$HYSTERSIS_HOME/hooks` dir + `hooks-paths` file (create if missing).
/// Race-resistant create (`create_dir` / `create_new`+`O_NOFOLLOW`); never
/// truncates an existing registry; rejects symlinks/wrong types.
pub fn ensure_hystersis_hook_slots(hystersis_home: &Path) -> Result<(), GlobalHookSourceError> {
    if path_has_symlink_component(hystersis_home) {
        return Err(GlobalHookSourceError::SymlinkedHystersisHome {
            path: hystersis_home.to_path_buf(),
        });
    }

    match std::fs::create_dir(hystersis_home) {
        Ok(()) => {}
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {}
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(hystersis_home).map_err(|source| {
                GlobalHookSourceError::CreateHooksDir {
                    path: hystersis_home.to_path_buf(),
                    source,
                }
    ...
    let hystersis_meta = std::fs::symlink_metadata(hystersis_home).map_err(|source| {
        GlobalHookSourceError::CreateHooksDir {
            path: hystersis_home.to_path_buf(),
            source,
        }
    })?;
    if hystersis_meta.file_type().is_symlink() || !hystersis_meta.file_type().is_dir() {
        return Err(GlobalHookSourceError::SymlinkedHystersisHome {
            path: hystersis_home.to_path_buf(),
        });
    }

    let hooks = hystersis_home.join("hooks");
    ...
    let registry = hystersis_home.join("hooks-paths");
...
/// Resolve Hystersis-owned direct global hook sources (`reject_symlinks` for sandbox).
pub fn resolve_global_hook_sources(
    ...
    if let Some(hystersis) = hystersis_home {
        if reject_symlinks && path_has_symlink_component(hystersis) {
            return Err(GlobalHookSourceError::SymlinkedHystersisHome {
                path: hystersis.to_path_buf(),
            });
        }

        let hooks = hystersis.join("hooks");
        let hooks_paths = hystersis.join("hooks-paths");
        if reject_symlinks {
