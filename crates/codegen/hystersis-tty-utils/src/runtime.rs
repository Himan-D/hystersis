...
//! against per-user ceilings (`pids.max` / `RLIMIT_NPROC`). Hystersis runtimes
...
//! runtime (the `hystersis` binary, `workspace_server`) derives its worker count
//! from here.
...
/// Maximum runtime worker threads for any hystersis process.
#[allow(
    clippy::unwrap_used,
