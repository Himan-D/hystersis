//! Canonical external-settings tool name ↔ Hystersis tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`xai-hystersis-hooks`) needs the
//! Hystersis tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`xai-hystersis-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `hystersis` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Hystersis, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Hystersis [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Hystersis tool names this Claude tool maps to (empty when there is no direct
    /// Hystersis tool — the entry then only contributes a `kind`).
    hystersis: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, hystersis: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        hystersis,
...
const fn match_only(claude: &'static str, hystersis: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        hystersis,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (hystersis reads SKILL.md)
...
/// The Hystersis [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Hystersis tool names a Claude matcher term fires on.
pub fn hystersis_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.hystersis)
...
/// The Claude names that map to `hystersis_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(hystersis_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.hystersis.contains(&hystersis_name))
        .map(|t| t.claude)
}

/// Every distinct Hystersis name the table references, for the `xai-hystersis-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn hystersis_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.hystersis.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Hystersis name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.hystersis.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
