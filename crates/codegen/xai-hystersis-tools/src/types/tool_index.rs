...
//!
//! `ToolSearchIndex` is defined in `xai-hystersis-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `xai-hystersis-shell`
...
/// in `xai-hystersis-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `xai-hystersis-shell` after MCP initialization.
#[derive(Clone)]
