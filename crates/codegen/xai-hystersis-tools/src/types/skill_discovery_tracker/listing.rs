...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (hystersis build harness) ─────────────
...
///
/// `"/path/.hystersis/skills/my-skill/SKILL.md"` -> `"/path/.hystersis/skills/"`
...
/// Used by the budgeted (hystersis build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Hystersis build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the hystersis
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // hystersis drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: hystersis build harness (budgeted XML) ───────────
