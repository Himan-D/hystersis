...
impl ShellKind {
    ...
    pub fn detect() -> Self {
        match xai_hystersis_config::shell::detect_unix_shell_kind() {
            xai_hystersis_config::shell::UnixShellKind::Bash => Self::Bash,
            xai_hystersis_config::shell::UnixShellKind::Zsh => Self::Zsh,
        }
    }

    /// Resolved absolute path to the shell binary. Falls back from `$SHELL` →
    /// `which` → common dirs → `/bin/<name>`. Result is cached process-wide
    /// in `xai_hystersis_config::shell::unix_shell_path`. See that function for
    /// the full cascade. Returns `&'static str`.
    pub fn binary_path(&self) -> &'static str {
        let kind = match self {
            Self::Bash => xai_hystersis_config::shell::UnixShellKind::Bash,
            Self::Zsh => xai_hystersis_config::shell::UnixShellKind::Zsh,
        };
        xai_hystersis_config::shell::unix_shell_path(kind)
    }
