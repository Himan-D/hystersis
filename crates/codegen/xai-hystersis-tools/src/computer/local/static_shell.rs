...
pub use xai_hystersis_config::shell::UnixShellKind;
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    xai_hystersis_config::shell::unix_shell_path(shell)
}
...
impl StaticShellSnapshot {
    ...
    pub async fn init(cwd: &Path) -> Self {
        let shell = xai_hystersis_config::shell::detect_unix_shell_kind();
