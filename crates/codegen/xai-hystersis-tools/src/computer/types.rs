...
///
/// Serialized over the wire in the `hystersis.com/task/kill` ext response
/// (`xai-hystersis-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (xai-hystersis-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in xai-hystersis-tools, spawns processes)
/// - `AcpTerminalBackend` (in xai-hystersis-shell, calls ACP protocol)
#[async_trait::async_trait]
