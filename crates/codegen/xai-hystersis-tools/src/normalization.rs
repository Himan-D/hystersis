...
/// Resolve `wire_name` in `toolset` and merge the canonical `hystersis.com/tool` object
...
/// the `hystersis.com/tool` `_meta` object. Equivalent tools across toolsets emit the
...
mod tests {
    ...
    fn canonical_omits_absent_options_not_null() {
        let hystersis = parse(serde_json::json!({"variant":"ReadFile","target_file":"/a"}));
        let g = canonical_input(&hystersis).unwrap();
