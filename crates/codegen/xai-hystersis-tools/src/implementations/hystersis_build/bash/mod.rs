...
use regex::Regex;
use xai_hystersis_config::shell::AmpersandSemantics;
...
/// [`xai_hystersis_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = xai_hystersis_config::shell::chain_separator();
...
impl xai_tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = xai_hystersis_config::shell::ampersand_semantics();
