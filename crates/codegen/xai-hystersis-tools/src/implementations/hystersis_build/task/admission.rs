...
impl SubagentLimits {
    ...
    fn from_lookup(lookup: impl Fn(&str) -> Option<String>) -> Self {
        ...
        let max_concurrent = parse_positive_env(
            "HYSTERSIS_MAX_CONCURRENT_SUBAGENTS",
            lookup("HYSTERSIS_MAX_CONCURRENT_SUBAGENTS"),
        )
        .unwrap_or(default.max_concurrent);
        let behavior = match lookup("HYSTERSIS_SUBAGENT_LIMIT_BEHAVIOR") {
            ...
            Some(value) => {
                tracing::warn!(
                    %value,
                    "HYSTERSIS_SUBAGENT_LIMIT_BEHAVIOR is neither `queue` nor `fail`; keeping `queue`"
                );
                LimitBehavior::Queue
            }
        };
...
impl AdmissionError {
    ...
    /// into `HYSTERSIS_SUBAGENT_LIMIT_BEHAVIOR=fail`.
