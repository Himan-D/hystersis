...
//!
//! - **`xai-hystersis-tools`** — tool blocks on a oneshot, formats the result.
//! - **`xai-hystersis-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`xai-hystersis-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `xai-hystersis-tools`.
...
register_resource!("hystersis_build", "UserQuestionSender", UserQuestionSender);
