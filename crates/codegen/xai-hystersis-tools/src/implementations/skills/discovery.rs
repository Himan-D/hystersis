...
///
/// `skills` is the standard layout (`.hystersis/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Hystersis sessions.
...
/// name (e.g. `~/.hystersis/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Hystersis.
...
/// `cwd` (exclusive). At each directory, checks for `.hystersis/skills/`,
...
/// visits known config roots (`.hystersis`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.hystersis` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".hystersis", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.hystersis is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.hystersis/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.hystersis/ → kept (user content).
        let hystersis_shell = tmp.path().join(".hystersis").join("skills").join("shell");
        std::fs::create_dir_all(&hystersis_shell).unwrap();
        std::fs::write(
            hystersis_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (hystersis_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.hystersis/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .hystersis skill in an intermediate dir.
        ...
        .unwrap();
        let hystersis_skill = sub.join(".hystersis").join("skills").join("hystersis-dyn");
        std::fs::create_dir_all(&hystersis_skill).unwrap();
        std::fs::write(hystersis_skill.join("SKILL.md"), "---\nname: hystersis-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"hystersis-dyn"),
            "hystersis-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only hystersis-dyn discovered.
        ...
        assert!(
            names_off.contains(&"hystersis-dyn"),
            "hystersis-dyn missing: {names_off:?}"
        );
        assert!(
