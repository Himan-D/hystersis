...
use hystersis_tools::types::{
    output::{MCPOutput, MCPOutputDetails, ToolOutput},
    tool::{ToolKind, ToolNamespace},
    tool_metadata::ToolMetadata,
};
use hystersis_tools::util::{ProcessGroup, ProcessScope};

/// MCP tool name delimiter: server names are qualified as `"server__tool"`.
/// Canonical definition lives in `hystersis_workspace_types`; re-exported here
/// for callers that historically imported it from this module.
pub use hystersis_workspace_types::MCP_TOOL_NAME_DELIMITER;

/// Reqwest 0.13 twin of the 0.12 adapters in `hystersis_extra_ca`.
fn with_extra_root_certificates(mut builder: reqwest::ClientBuilder) -> reqwest::ClientBuilder {
    ...
    for der in hystersis_extra_ca::extra_root_ders() {
...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"hystersis_com_linear"`).
...
/// SDK-side id echoed back in `hystersis.com/mcp/sdk_call`. A named struct (rather than a
/// `(String, String)` tuple) so callers can't transpose the two strings.
///
/// `Deserialize`d straight from a `_meta["hystersis.com/mcp/servers"]` entry, so the
...
/// The session's in-process SDK MCP servers (declared via `_meta["hystersis.com/mcp/servers"]`,
...
struct AcpMcpRegistry {
    ...
    /// `hystersis.com/mcp/sdk_call` over the ACP connection).
...
pub struct McpState {
    ...
    /// Persisted to `~/.hystersis/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    event_writer: hystersis_session_events::EventWriter,
    ...
    /// `hystersis.com/mcp/server_status` notifications.
    ///
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: hystersis_session_events::EventWriter::noop(),
    ...
    /// `HystersisClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: hystersis_session_events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &hystersis_session_events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `xai-hystersis-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use hystersis_telemetry::enums::McpInitStrategy;

/// Parse a non-empty `server__tool` ID with one overlap-aware delimiter and
/// valid [`hystersis_tool_protocol::ToolId`] syntax.
pub fn parse_mcp_qualified_name(name: &str) -> Option<(hystersis_tool_protocol::ToolId, &str, &str)> {
    ...
    Some((hystersis_tool_protocol::ToolId::new(name).ok()?, server, tool))
}
...
impl McpError {
    ...
    pub fn error_category(&self) -> hystersis_session_events::McpErrorCategory {
        use hystersis_session_events::McpErrorCategory;
        match self {
...
///   the frontend via `hystersis.com/mcp/tools_changed` notifications and callable via
///   `hystersis.com/mcp/call`.
...
/// `hystersis_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl hystersis_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> hystersis_tool_protocol::ToolId {
        ...
        hystersis_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| hystersis_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        hystersis_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(hystersis_session_events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(hystersis_session_events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            hystersis_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(hystersis_session_events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(hystersis_session_events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &hystersis_session_events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, hystersis_tool_runtime::ToolError> {
        let mcp_service = client
            .ensure_initialized()
            .await
            .map_err(|e| hystersis_tool_runtime::ToolError::custom("process_manager", e.to_string()))?;
        ...
        match result {
            ...
            Ok(Err(e)) => Err(hystersis_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
                *is_timeout = true;
                ...
                Err(hystersis_tool_runtime::ToolError::custom(
                    "process_manager",
                    format!(
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(hystersis_session_events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(hystersis_session_events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
            ...
            Err(e) => {
                ...
                return Err(hystersis_tool_runtime::ToolError::custom(
                    "process_manager",
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(hystersis_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
...
async fn resolve_http_oauth_prep(
    ...
    let outcome = {
        let _auth_discovery_timer =
            hystersis_telemetry::instrumentation::timer("mcp_http_auth_discovery");
        ...
        .await
        {
            ...
            Err(_) => {
                ...
                ctx.event_writer
                    .emit(hystersis_session_events::Event::McpOAuthDiscoveryTimeout {
    ...
    match outcome {
        ...
        OauthProbeOutcome::Inconclusive => {
            ...
            ctx.event_writer
                .emit(hystersis_session_events::Event::McpOAuthProbeResolved {
                    server_name: server_name.to_string(),
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(hystersis_session_events::Event::McpTransportDecodeError {
...
enum PendingTransport {
    ...
    /// (`hystersis.com/mcp/sdk_call`). Rebuildable from its `server_id` + invoker, so handshake
...
/// Uses [`HystersisClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, HystersisClientHandler>>;
...
enum ClientState {
    ...
    Ready {
        service: McpService,
        _connected: hystersis_telemetry::activity::ActivityGaugeGuard,
    },
}
...
///    (`TransportClosed`).
/// 2. [`HystersisClientHandler`] when the server pushes a notification we
...
///
/// Consumers fan these out to ACP `hystersis.com/mcp/server_status` after 50 ms
...
pub struct McpClient {
    ...
    /// [`HystersisClientHandler`]** constructed by
...
/// and the [`HystersisClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to xai-hystersis-mcp
...
impl McpClient {
    ...
    /// channel. `server_id` is the id the agent echoes back in `hystersis.com/mcp/sdk_call`; the
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: hystersis_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
            ...
            match result {
                Ok(service) => {
                    ...
                    *guard = ClientState::Ready {
                        ...
                        _connected: hystersis_telemetry::activity::MCP_SERVERS_CONNECTED.enter(),
                    };
                    tracing::info!(
                        server = %self.server_name,
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, HystersisClientHandler>, McpError> {
        ...
        match pending {
            ...
            PendingTransport::Acp { server_id, invoker } => {
                // Per-reverse-call backstop on `hystersis.com/mcp/sdk_call`: the larger of the
    ...
    fn make_client_info(server_name: &str, advertise_elicitation: bool) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("hystersis-shell-{server_name}"),
                hystersis_version::VERSION.to_string(),
            ),
        )
    ...
    /// Build the [`HystersisClientHandler`] that drives `client.serve(...)`.
    ///
    /// The handler holds a **clone of `Arc<Mutex<Option<Sender>>>`**,
    /// not a snapshot — so any subsequent call to
    fn make_client_handler(&self) -> HystersisClientHandler {
        HystersisClientHandler {
    ...
    /// references (the [`HystersisClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            hystersis_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = hystersis_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
/// Copy an MCP server's stderr to `~/.hystersis/logs/mcp/<server>.stderr.log`
/// in a background task. Truncated per spawn.
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = hystersis_config::hystersis_home().join("logs").join("mcp");
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`hystersis_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "hystersis_com_";
...
static DEFAULT_USER_AGENT: LazyLock<reqwest::header::HeaderValue> = LazyLock::new(|| {
    format!("grok-cli/{}", hystersis_version::VERSION)
        .parse()
...
fn apply_stdio_env(cmd: &mut Command, env: &[acp::EnvVariable], session_id: Option<&str>) {
    ...
    if let Some(session_id) = session_id {
        cmd.env("HYSTERSIS_SESSION_ID", session_id);
    }
}
...
pub struct McpSpawnCtx<'a> {
    ...
    pub(crate) event_writer: &'a hystersis_session_events::EventWriter,
...
impl<'a> McpSpawnCtx<'a> {
    pub fn for_session(
        session_id: &'a str,
        event_writer: &'a hystersis_session_events::EventWriter,
        mode: OauthInteractivity,
    ...
    pub fn session_less(event_writer: &'a hystersis_session_events::EventWriter) -> Self {
        Self {
            session_id: None,
            event_writer,
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = hystersis_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = hystersis_telemetry::instrumentation::timer("mcp_stdio_spawn");
            ...
            .map_err(|e| {
                ...
                hystersis_telemetry::session_ctx::log_event(
                    hystersis_telemetry::events::McpServerFailed {
                        server_name: name.clone(),
                        error_type: hystersis_telemetry::events::McpErrorType::SpawnFailed,
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = hystersis_telemetry::instrumentation::timer("mcp_start_servers");
...
/// `hystersis.com/mcp/server_status` events.
///
...
#[derive(Debug)]
pub struct HystersisClientHandler {
...
impl ClientHandler for HystersisClientHandler {
    ...
    // See the [`HystersisClientHandler`] doc-comment for the full RPIT
    // contract.
    ...
    async fn on_url_elicitation_notification_complete(
        ...
        if !hystersis_tools::mcp_elicitation::chars_within(
            &params.elicitation_id,
            hystersis_tools::mcp_elicitation::MAX_ELICIT_ID_CHARS,
        ) {
            tracing::warn!(
                server = %self.server_name,
