//! Single source of truth for the `hystersis.com/mcp/*` ACP wire strings.
//!
...
/// Forward tool-invocation method (client -> agent): `hystersis.com/mcp/call`.
///
...
pub const MCP_CALL: &str = "hystersis.com/mcp/call";

/// Reverse zero-IPC tool-invocation method (agent -> client): `hystersis.com/mcp/sdk_call`.
///
...
pub const MCP_SDK_CALL: &str = "hystersis.com/mcp/sdk_call";

/// `session/new` `_meta` key listing in-process SDK MCP servers: `hystersis.com/mcp/servers`.
pub const MCP_SERVERS: &str = "hystersis.com/mcp/servers";

/// `initialize` `_meta` capability flag advertising in-process SDK MCP support
/// (enables the SDK's `transport="acp"`): `hystersis.com/mcp/sdk`.
pub const MCP_SDK: &str = "hystersis.com/mcp/sdk";

/// Reverse elicitation method (agent -> client): `hystersis.com/mcp/elicit`.
///
...
pub const MCP_ELICIT: &str = "hystersis.com/mcp/elicit";

/// Elicitation-complete notification (agent -> client): `hystersis.com/mcp/elicit_complete`.
///
/// Forwards a server's `notifications/elicitation/complete` so the client can
/// dismiss the popup for the given `elicitationId`.
pub const MCP_ELICIT_COMPLETE: &str = "hystersis.com/mcp/elicit_complete";
