...
//! image bytes.
//!
//! Both the TUI ([`hystersis_pager::prompt_images`]) and the server-side
...
//! [`hystersis_pager::prompt_images::display_text`]. The shape of this
...
/// [`AttachedImages`](hystersis_tools::types::resources::AttachedImages))
...
///   separator — see [`hystersis_pager::prompt_images::display_text`].
...
/// [`hystersis_pager::prompt_images::read_image_at_path`], which has no
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    hystersis_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
mod tests {
    ...
    /// `hystersis_pager::prompt_images::display_text`. A
