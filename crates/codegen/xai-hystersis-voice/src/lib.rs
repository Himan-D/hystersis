//! Voice input for Hystersis CLI: an xAI streaming STT client and the
...
mod intercept_tests {
    ...
    fn capture_subcommand_matches_only_argv1() {
        assert!(is_capture_subcommand(&argv(&["hystersis", "__mic-capture"])));
        ...
        assert!(!is_capture_subcommand(&argv(&["hystersis"])));
        assert!(!is_capture_subcommand(&argv(&["hystersis", "chat"])));
        assert!(!is_capture_subcommand(&argv(&[
            "hystersis",
            "chat",
            "__mic-capture"
