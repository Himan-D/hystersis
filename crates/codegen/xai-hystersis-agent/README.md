...
This crate extracts a first-class `Agent` type from `xai-hystersis-shell`.
...
`xai-hystersis-shell`, another in-process host, or a headless batch runner.
...
in `.hystersis/agents/` (project-level) or `~/.hystersis/agents/` (user-level).

```rust
use xai_hystersis_agent::{AgentDefinition, AgentBuilder};
use xai_hystersis_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".hystersis/agents/code-reviewer.md")?;
...
```rust
use xai_hystersis_agent::discovery;

// Find all .md files in .hystersis/agents/ directories
...
```
┌──────────────────┐
│  xai-hystersis-agent  │  ← This crate
│  (Agent, Builder, │
│   Definition)     │
└────────┬─────────┘
         │ depends on
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  xai-hystersis-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`xai-hystersis-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `xai-hystersis-agent` depends on it for tool setup.
- **`xai-hystersis-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `xai-hystersis-agent` (AGENTS.md
...
|---|---|---|
| `hystersis` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p xai-hystersis-agent

# Test
cargo test -p xai-hystersis-agent

# Clippy
cargo clippy -p xai-hystersis-agent --fix --allow-dirty
