...
//! - `Default`: the legacy Hystersis prefix (built by the shell layer).
...
use xai_hystersis_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Hystersis prefix (`<user_info>` + optional `<git_status>`), built directly by the
...
/// One discovered rule file (AGENTS.md / Claude.md / .hystersis/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `xai_hystersis_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.hystersis/, ~/.claude/).
    ...
    /// (`~/.hystersis/projects/<encoded-cwd>/mcps`). Surfaced in
...
mod tests {
    ...
    fn format_rules_section_workspace_then_user() {
        ...
        let user = [
            ...
            RuleEntry {
                path: "/home/dev/.hystersis/AGENTS.md".into(),
    ...
    fn format_rules_section_keeps_markdown_heading_off_the_open_tag() {
        let user = [RuleEntry {
            path: "/home/dev/.hystersis/rules/personal.md".into(),
