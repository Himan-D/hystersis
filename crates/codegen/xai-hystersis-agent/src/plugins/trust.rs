...
//!
//! Plugins from project directories (`.hystersis/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.hystersis/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.hystersis/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.hystersis/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_hystersis_home() so a project's `.hystersis/trusted-plugins` is never
        // read as the user trust store when neither HYSTERSIS_HOME nor a home dir resolves.
        let Some(hystersis) = xai_hystersis_config::user_hystersis_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = hystersis.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.hystersis/trusted-plugins`.
    ...
    /// rewrites `~/.hystersis/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.hystersis/trusted-plugins`.
