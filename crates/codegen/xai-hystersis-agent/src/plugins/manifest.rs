...
//! 1. `.hystersis-plugin/plugin.json`
...
///
/// Replaces `${HYSTERSIS_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${HYSTERSIS_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`xai_hystersis_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    xai_hystersis_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".hystersis-plugin")).unwrap();

        // Write manifest in .hystersis-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".hystersis-plugin/plugin.json"),
    ...
    fn substitute_env_vars_replaces_all() {
        let input = "${HYSTERSIS_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${HYSTERSIS_PLUGIN_DATA}/cache";
