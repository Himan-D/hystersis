...
use xai_hystersis_tools::implementations::codex;
use xai_hystersis_tools::implementations::hystersis_build;
use xai_hystersis_tools::implementations::hystersis_build_concise;
use xai_hystersis_tools::implementations::memory;
use xai_hystersis_tools::implementations::opencode;
use xai_hystersis_tools::implementations::search_tool;
use xai_hystersis_tools::implementations::use_tool;
use xai_hystersis_tools::registry::types::{ToolConfig, ToolServerConfig};
...
pub fn hystersis_build_hashline_toolset(
    hashline_tools: Vec<xai_hystersis_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<xai_hystersis_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
...
pub struct AgentDefinition {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<xai_hystersis_sampling_types::ToolOverrides>,
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: xai_hystersis_config::hystersis_home()
                    .join("agent-memory")
                    .join(agent_name),
...
/// these into `xai_hystersis_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let hystersis = xai_hystersis_config::user_hystersis_home();
...
impl AgentDefinition {
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<xai_hystersis_tools::registry::types::ToolConfig>,
    ) {
...
mod tests {
    ...
    fn task_tool_rename_matches_task_tool_id_predicate() {
        ...
        assert!(xai_hystersis_tools::is_task_tool_id(&name));
    }
    ...
    fn bash_tool_rename_matches_writing_tool_kind() {
        ...
        assert_eq!(
            xai_hystersis_tools::tool_taxonomy::writing_tool_kind(&name),
            Some(xai_hystersis_tools::types::tool::ToolKind::Execute)
        );
    }
    /// Native presets only.
    #[test]
