...
//! primitive consumed cross-crate by `xai-hystersis-workspace`, not agent-definition
//! discovery.
...
/// code-exec markers (`.mcp.json`, `.hystersis/config.toml`, `.claude/settings.json`,
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // home-level `.hystersis`/`.mcp.json`/plugins would look repo-local. Drop
...
/// from `xai-hystersis-workspace`, which depends on THIS crate) to keep the dep edge
