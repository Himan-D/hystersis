...
//!
//! Searches `.hystersis/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.hystersis/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.hystersis/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".hystersis/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.hystersis/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.hystersis/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), hystersis.as_deref())
}
...
fn all_subagents_with_home(
    ...
    home: Option<&Path>,
    hystersis_home: Option<&Path>,
) -> Vec<SubagentEntry> {
    let discovered = discover_with_home(cwd, home, hystersis_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.hystersis/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.hystersis/agents/` walking from `cwd` up to repo root
/// 2. `~/.hystersis/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.hystersis/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user hystersis agents, `.claude`
/// compat agents, then bundled. `.hystersis` dirs resolve from `hystersis_home`
/// (HYSTERSIS_HOME-aware) plus the legacy literal `~/.hystersis` when HYSTERSIS_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    hystersis_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.hystersis, included only when it differs from hystersis_home
    // (i.e. HYSTERSIS_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_hystersis = home
        .map(|h| h.join(".hystersis"))
        .filter(|legacy| hystersis_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = hystersis_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_hystersis {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let hystersis = xai_hystersis_config::user_hystersis_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), hystersis.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, hystersis_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let hystersis = xai_hystersis_config::user_hystersis_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), hystersis.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, hystersis_home);
...
///
/// Project-level `.hystersis/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let hystersis = xai_hystersis_config::user_hystersis_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), hystersis.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, hystersis_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, hystersis_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Hystersis
...
/// Load project agent definitions from every `.hystersis/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectHystersis,
            PluginScope::User => PluginOrigin::UserHystersis,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_hystersis_when_hystersis_home_differs() {
        let home = Path::new("/home/u");
        let hystersis = Path::new("/custom/hystersishome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(hystersis))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&hystersis.join("agents")));
        assert!(paths.contains(&home.join(".hystersis").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&hystersis.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".hystersis").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_hystersis_home_is_dot_hystersis() {
        let home = Path::new("/home/u");
        let hystersis = home.join(".hystersis");
        let count = user_agent_dirs(Some(home), Some(&hystersis))
            .into_iter()
            .filter(|(p, _)| *p == hystersis.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.hystersis/agents when hystersis_home == ~/.hystersis"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_hystersis_build() {
        let def = by_name("hystersis");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "hystersis");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".hystersis").join("agents");
        let agents_dir_2 = inner_dir.join(".hystersis").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".hystersis").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".hystersis")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_uses_bundled_when_no_higher_priority_match_exists() {
        ...
        let def =
            by_name_in_cwd_with_home("bundled-only", &cwd, Some(&home), Some(&home.join(".hystersis")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".hystersis").join("agents");
        let bundled_dir = home.join(".hystersis").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".hystersis")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def = by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".hystersis")))
            .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".hystersis").join("agents");
        let bundled_dir = home.join(".hystersis").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "hystersis" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "hystersis.md",
            "hystersis",
        ...
        let def = by_name_in_cwd("hystersis", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "hystersis");
        assert_eq!(def.description, "Custom hystersis");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .hystersis/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("hystersis-orchestrator")
            .expect("from_str must resolve hystersis-orchestrator");
        assert_eq!(variant, BuiltinAgentName::HystersisBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "hystersis-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("hystersis-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find hystersis-orchestrator");
        assert_eq!(def.name, "hystersis-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.hystersis/agents/) agent named "explore" should NOT shadow
