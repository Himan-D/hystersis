...
pub use hystersis_config::HookProvenance;
...
pub fn parse_hooks_from_config_layers(
    layers: &[hystersis_config::HookConfigLayer],
) -> (Vec<HookSpec>, Vec<HookError>) {
    let home = hystersis_config::user_hystersis_home();
...
mod tests {
    ...
    fn config_layer(source_name: &str, toml_src: &str) -> hystersis_config::HookConfigLayer {
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let hooks = value.get("hooks").cloned().unwrap();
        hystersis_config::HookConfigLayer::new(
            hystersis_config::HookProvenance::Managed,
            source_name,
            hooks,
        )
    }
    ...
    fn config_layers_additive_and_dedup_keeps_higher_authority() {
        ...
        use hystersis_config::HookProvenance::{Managed, User};
