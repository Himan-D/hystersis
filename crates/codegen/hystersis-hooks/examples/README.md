...
Sample hooks for Hystersis. Copy to `~/.hystersis/hooks/` to enable globally, or to `<project>/.hystersis/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.hystersis/hooks/bin
cp examples/hooks/safe-shell.json ~/.hystersis/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.hystersis/hooks/bin/
chmod +x ~/.hystersis/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.hystersis/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.hystersis/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.hystersis/hooks/bin/
chmod +x ~/.hystersis/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.hystersis/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.hystersis/hooks/bin
cp examples/hooks/session-log.json ~/.hystersis/hooks/
cp examples/hooks/bin/session-log.sh ~/.hystersis/hooks/bin/
chmod +x ~/.hystersis/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.hystersis/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.hystersis/hooks/bin
cp examples/hooks/tool-logger.json ~/.hystersis/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.hystersis/hooks/bin/
chmod +x ~/.hystersis/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.hystersis/hooks/bin
cp examples/hooks/stop-verify.json ~/.hystersis/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.hystersis/hooks/bin/
chmod +x ~/.hystersis/hooks/bin/stop-verify.sh
```
...
- **Event names:** `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, `SessionEnd` (see the [user guide](../../xai-hystersis-pager/docs/user-guide/10-hooks.md) for the full set)
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Hystersis names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.hystersis/hooks/`. The hook stops running on the next session.