...
//! over ACP (`hystersis.com/log` notifications); shell receives them in
...
pub const LOG_METHOD: &str = "hystersis.com/log";
...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "hystersis-pager")]
    #[serde(rename = "hystersis-pager")]
    HystersisPager,
    #[strum(serialize = "hystersis-desktop")]
    #[serde(rename = "hystersis-desktop")]
    HystersisDesktop,
}
...
/// Wire format for the `hystersis.com/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// `~/.hystersis/logs/unified.jsonl` (those bursts inflate exactly the counters
...
fn log_path() -> PathBuf {
    ...
    hystersis_home().join(LOG_DIR).join(LOG_FILE)
}
...
fn test_log_dir() -> &'static PathBuf {
    ...
    TEST_LOG_DIR.get_or_init(|| {
        ...
        let dir = std::env::temp_dir().join(format!(
            "hystersis-unified-log-test-{}-{nanos}",
            std::process::id()
        ));
...
/// healing a stale handle rather than re-resolving `$HYSTERSIS_HOME` — which also
...
///
/// Called by the `hystersis.com/log` notification handler. Entries from
...
mod tests {
    ...
    /// writer onto the developer's real `~/.hystersis/logs/unified.jsonl`.
    #[ctor::ctor]
    ...
    /// an emit lands in a per-process temp file, never under `hystersis_home()`.
    #[test]
    fn redirect_routes_writes_and_snapshots_to_process_temp_file() {
        ...
        assert!(
            ...
            "the shared file must live under the temp dir, not hystersis_home(): {}",
            log_path().display()
        );
    }
