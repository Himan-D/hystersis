...
fn build_resource(cfg: &ExternalOtelConfig) -> opentelemetry_sdk::Resource {
    let mut attrs = vec![
        ...
        opentelemetry::KeyValue::new("hystersis_code.schema.version", super::schema::SCHEMA_VERSION),
    ];
...
fn grpc_tls_candidates(
    ...
    // Process-wide `HYSTERSIS_EXTRA_CA_BUNDLE` roots (fail-open by that crate's
    // contract), matching the HTTP transport's client policy — the same
    // corporate CA must work on both transports.
    if let Some(extra_pem) = ders_to_pem_bundle(xai_hystersis_extra_ca::extra_root_ders()) {
...
mod tests {
    ...
    /// The DER→PEM re-encode used for `HYSTERSIS_EXTRA_CA_BUNDLE` must produce a
