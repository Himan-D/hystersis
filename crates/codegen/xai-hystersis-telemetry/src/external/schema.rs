...
/// Wire schema version, exported as resource attr `hystersis_code.schema.version`.
/// Additive changes (new events/attrs) do not bump it; renames/removals do.
pub const SCHEMA_VERSION: &str = "v1";

/// Meter/logger instrumentation scope name.
pub const SCOPE_NAME: &str = "ai.xai.hystersis_code";
...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "hystersis_code.session_start",
            Self::SessionEnd => "hystersis_code.session_end",
            Self::UserPrompt => "hystersis_code.user_prompt",
            Self::TurnCompleted => "hystersis_code.turn_completed",
            Self::ApiRequest => "hystersis_code.api_request",
            Self::ApiError => "hystersis_code.api_error",
            Self::ToolResult => "hystersis_code.tool_result",
            Self::ToolDecision => "hystersis_code.tool_decision",
            Self::McpServerConnection => "hystersis_code.mcp_server_connection",
            Self::PermissionModeChanged => "hystersis_code.permission_mode_changed",
            Self::SkillActivated => "hystersis_code.skill_activated",
            Self::PluginLoaded => "hystersis_code.plugin_loaded",
            Self::Compaction => "hystersis_code.compaction",
            Self::Subagent => "hystersis_code.subagent",
            Self::Auth => "hystersis_code.auth",
            Self::InternalError => "hystersis_code.internal_error",
            Self::ModelSwitched => "hystersis_code.model_switched",
            Self::ContextualTip => "hystersis_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    /// `hystersis_code.session.count`.
    SessionCount,
    /// `hystersis_code.token.usage` — `token_type` ∈ `input|output|reasoning|cache_read`.
    TokenUsage {
        token_type: &'static str,
        model: String,
        count: u64,
    },
    /// `hystersis_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `hystersis_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `hystersis_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `hystersis_code.error.count`.
    ErrorCount {
        error_category: String,
        model: String,
    },
    /// `hystersis_code.startup.timeout` (`stuck_in` = phase label).
    StartupTimeout { stuck_in: String, auth_mode: String },
    /// `hystersis_code.startup.phase_duration` (ms; `phase` = phase label).
    ...
    /// `hystersis_code.startup.total` (ms from process start to a usable session).
    StartupTotal {
        duration_ms: u64,
        outcome: String,
        auth_mode: String,
...
pub(crate) const METRIC_SESSION_COUNT: &str = "hystersis_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "hystersis_code.token.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "hystersis_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "hystersis_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "hystersis_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "hystersis_code.error.count";
pub(crate) const METRIC_STARTUP_PHASE_DURATION: &str = "hystersis_code.startup.phase_duration";
pub(crate) const METRIC_STARTUP_TIMEOUT: &str = "hystersis_code.startup.timeout";
pub(crate) const METRIC_STARTUP_TOTAL: &str = "hystersis_code.startup.total";
...
/// Built-in tool names (closed enum surface in `xai-hystersis-tools`) that pass
...
fn tool_outcome_label(o: &xai_hystersis_session_events::types::ToolOutcome) -> &'static str {
    use xai_hystersis_session_events::types::ToolOutcome;
    match o {
...
/// `SessionHarness` → `hystersis_code.session_start`. Emitted from a spawn outside
...
/// `SessionNew` → `hystersis_code.session.count` (metric only; the flagship
...
/// `SessionEnded` → `hystersis_code.session_end`.
...
/// `PromptSubmitted` → `hystersis_code.user_prompt`. Prompt text rides the
...
/// `TurnCompleted` → `hystersis_code.turn_completed` + `turn.count`
...
/// `ModelResponseReceived` → `hystersis_code.api_request` + `token.usage`.
...
/// `RateLimitHit` → `hystersis_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` → `hystersis_code.api_error`. Category/class enums only — no
...
/// `ToolCallCompleted` → `hystersis_code.tool_result` + `tool.usage`.
...
/// `PermissionDecisionPayload` → `hystersis_code.tool_decision` + `tool.decision`.
...
/// `McpServerConnected` → `hystersis_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` → `hystersis_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` → `hystersis_code.permission_mode_changed`.
...
/// `ContextualTip` → `hystersis_code.contextual_tip`. Labels only (no user
...
/// `YoloToggled` → `hystersis_code.permission_mode_changed`.
...
/// `SkillDispatched` → `hystersis_code.skill_activated`. Skill names are details-gated; source and trigger export by default.
...
/// `PluginInstalled` → `hystersis_code.plugin_loaded`.
...
/// `PluginUsed` → `hystersis_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` → `hystersis_code.compaction`.
...
/// `CompactionTriggered` → `hystersis_code.compaction` trigger attrs ride on the
/// completion event instead (one event per compaction); not mapped.
/// `SubagentLaunched` → `hystersis_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` → `hystersis_code.subagent` (`phase=completed`).
...
/// `InternalError` → `hystersis_code.internal_error`. Error class only — no
...
/// `ModelSwitched` → `hystersis_code.model_switched`.
