...
//! file logger that writes to `~/.hystersis/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `xai_hystersis_hooks` or `xai_hystersis_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! HYSTERSIS_HOOKS_LOG=1 hystersis              # enable, write to ~/.hystersis/logs/hooks.log
//! HYSTERSIS_HOOKS_LOG=/tmp/h.log hystersis     # write to custom path
//! HYSTERSIS_HOOKS_LOG=0 hystersis              # explicitly disable
//! tail -f ~/.hystersis/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "HYSTERSIS_HOOKS_LOG";
...
///
/// Writes to `~/.hystersis/logs/hooks.log` (or custom path via `HYSTERSIS_HOOKS_LOG`).
/// Filters to hooks (`xai_hystersis_hooks`) and plugins (`xai_hystersis_agent::plugins`) targets.
/// Set `HYSTERSIS_HOOKS_LOG=0` to disable, `HYSTERSIS_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "xai_hystersis_hooks=debug,xai_hystersis_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || hystersis_home().join("logs").join("hooks.log");
