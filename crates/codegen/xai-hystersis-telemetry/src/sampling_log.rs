//! Tracing layer for `target: "sampling_log"` → `~/.hystersis/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `HYSTERSIS_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "HYSTERSIS_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = hystersis_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
