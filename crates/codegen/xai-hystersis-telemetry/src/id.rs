...
const ENV_AGENT_ID: &str = "HYSTERSIS_AGENT_ID";

static AGENT_ID: OnceLock<String> = OnceLock::new();
static AGENT_INSTANCE_ID: OnceLock<String> = OnceLock::new();

/// Returns the stable agent ID: `HYSTERSIS_AGENT_ID` if set, else the value cached
/// in `$HYSTERSIS_HOME/agent_id`, else a machine-derived UUID computed once and
...
fn load_or_compute_agent_id() -> String {
    ...
    let cache_path = xai_hystersis_config::hystersis_home().join("agent_id");
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    xai_hystersis_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
...
/// Opt-in special-user gate for telemetry (`HYSTERSIS_TELEMETRY_SPECIAL_USER`).
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("HYSTERSIS_TELEMETRY_SPECIAL_USER").as_deref(),
