...
//! `hystersis-shell-manual_auth` with the `reason`/`trigger`/`token_kind`/`principal`
...
use xai_hystersis_telemetry::client;
use xai_hystersis_telemetry::config::{TelemetryConfig, TelemetryMode};
...
async fn manual_auth_posts_to_events_endpoint_as_hystersis_shell_manual_auth() {
    ...
    xai_hystersis_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "hystersis-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no hystersis-shell-manual_auth POST received"
        );
    ...
    for (key, expected) in [
        ...
        (
            "dev_build",
            serde_json::json!(xai_hystersis_version::IS_DEV_BUILD),
        ),
    ...
    for key in xai_hystersis_telemetry::client::RESERVED_EVENT_KEYS {
        assert!(
