...
use xai_hystersis_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
const SECRET_MODEL: &str = "hystersis-4-sk-LEAKmodel1234567890abcd";
...
fn external_stream_gates_on_end_to_end() {
    ...
    let mut cfg = external::ExternalOtelConfig::resolve_with(
        |name| match name {
            "HYSTERSIS_EXTERNAL_OTEL" => Some("1".into()),
    ...
    assert!(!xai_hystersis_telemetry::is_enabled());

    xai_hystersis_telemetry::log_event(xai_hystersis_telemetry::events::SessionHarness {
        ...
        permission_mode: xai_hystersis_telemetry::enums::PermissionMode::Ask,
        ...
        memory_retrieval_mode: xai_hystersis_telemetry::events::MemoryRetrievalMode::Disabled,
        is_git_repo: true,
        auto_update: None,
    });
    xai_hystersis_telemetry::log_event(xai_hystersis_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "hystersis-4".into(),
    ...
    xai_hystersis_telemetry::log_event(xai_hystersis_telemetry::events::ModelResponseReceived {
    ...
    xai_hystersis_telemetry::log_event(xai_hystersis_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "hystersis_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.xai.hystersis_code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
    ...
    assert_eq!(
        harness
            .resource
            .get("hystersis_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "hystersis_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "hystersis_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "hystersis_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "hystersis_code.session.count");
