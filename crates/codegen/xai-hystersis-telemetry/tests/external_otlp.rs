...
fn external_stream_end_to_end() {
    ...
    let mut cfg = xai_hystersis_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = xai_hystersis_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    xai_hystersis_telemetry::external::init(Some(cfg));
    assert!(xai_hystersis_telemetry::external::is_active());
    ...
    xai_hystersis_telemetry::log_event(xai_hystersis_telemetry::events::SessionNew {
    ...
    for expected in [
        "hystersis_code.session_start",
        "hystersis_code.user_prompt",
        "hystersis_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "hystersis_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "hystersis_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "hystersis_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "hystersis_code.token.usage"));
    assert_eq!(
    ...
    xai_hystersis_telemetry::external::shutdown();
    assert!(
