...
pub enum SideQuestionError {
    ...
    Sampling(#[from] hystersis_sampling_types::SamplingError),
...
pub fn meta_category_str(
    category: hystersis_session_events::types::CancellationCategory,
) -> &'static str {
...
pub enum SessionCommand {
    ...
    GetToolOverrides {
        respond_to: oneshot::Sender<Option<hystersis_sampling_types::ToolOverrides>>,
    },
    ...
    SetToolOverrides {
        overrides: hystersis_sampling_types::ToolOverrides,
    },
    Prompt {
        prompt_id: String,
        ...
        tool_overrides_update: Option<hystersis_sampling_types::ToolOverridesUpdate>,
    ...
    SetSessionModel {
        sampling_config: hystersis_sampler::SamplerConfig,
        use_concise: bool,
    ...
    /// rather than the agent-level default (e.g. `hystersis-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-hystersis-model-override` header and merges
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<hystersis_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<hystersis_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.hystersis/hooks`,
    ...
    /// Out-of-band history repair (`hystersis.com/session/repair`): fix tool-pairing
    ...
    RepairHistory {
        dry_run: bool,
        respond_to:
            oneshot::Sender<anyhow::Result<hystersis_chat_state::compaction_utils::HistoryRepairReport>>,
    },
    GetRewindPoints {
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, hystersis_chat_state::UsageTotals)>,
    ...
    /// mid-session via the `hystersis.com/session/update_mcp_servers` extension method).
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<hystersis_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        ...
        respond_to: oneshot::Sender<Result<hystersis_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<hystersis_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<hystersis_hooks_plugins_types::HooksListResponse>,
    },
    /// Execute a hooks management action from the pager modal.
    HooksAction {
        action: hystersis_hooks_plugins_types::HooksAction,
        respond_to: oneshot::Sender<hystersis_hooks_plugins_types::ActionOutcome>,
    },
    ...
    /// This session's plugin registry, as served by `hystersis.com/plugins/list`.
    PluginsList {
        respond_to:
            oneshot::Sender<Option<std::sync::Arc<hystersis_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// from the notification bridge, which does not go through `send_hystersis_notification`).
    ...
    /// serialized mailbox; the rebroadcast of `hystersis.com/queue/changed` is the
    ...
    /// `hystersis.com/queue/changed` is the truth signal for every attached client.
    ...
    /// tool-free model call (default `hystersis-0.1` when available via
    ...
    /// `hystersis` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    Interject {
        ...
        /// `hystersis.com/session/interjection` so the originating pager can dedup its
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<hystersis_chat_state::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<hystersis_sampling_types::conversation::ConversationItem>>>,
    },
