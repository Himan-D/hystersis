...
) -> hystersis_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> hystersis_tool_protocol::session_event::ToolCallOutcome {
    use crate::session::events::ToolOutcome;
    use hystersis_tool_protocol::session_event::ToolCallOutcome;
    match outcome {
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> hystersis_hooks::runner::RunContext<'_> {
        hystersis_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_annotation(&self, message: &str) {
        self.send_hystersis_notification(XaiSessionUpdate::HookAnnotation {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[hystersis_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, hystersis_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
        ...
        self.send_hystersis_notification(XaiSessionUpdate::HookExecution {
    ...
    pub(super) async fn dispatch_hook(
        ...
        payload: hystersis_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            hystersis_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                hystersis_hooks::result::HookRunResult::Success {
                    ...
                    hook_name,
                    elapsed,
                    hystersis_telemetry::events::HookOutcome::Success,
                ),
                hystersis_hooks::result::HookRunResult::Blocked {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    hystersis_telemetry::events::HookOutcome::Blocked,
                ),
                hystersis_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    hystersis_telemetry::events::HookOutcome::Error,
                ),
                hystersis_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = XaiSessionUpdate::TaskCompleted {
            task_snapshot: hystersis_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
