...
impl SessionActor {
    pub(super) async fn handle_set_session_model(
                ...
                if let ConversationItem::System(sys) = item {
                    if use_concise {
                        sys.content = std::sync::Arc::<str>::from(
                            hystersis_agent::prompt::template::COMPACT_SYSTEM_PROMPT,
                        );
                    } else {
                        sys.content =
        ...
        if is_family_switch && !turn_in_flight && self.history_has_model_minted_items().await {
            ...
            let trigger_info = compaction::AutoCompactTriggerInfo {
                ...
                context_window,
                percentage: hystersis_token_estimation::usage_percentage_u8(
    ...
    ///
    /// Builds a fresh [`hystersis_agent::Agent`] from the cached
    /// [`crate::session::agent_rebuild::AgentRebuildSpec`] + the supplied
    /// [`hystersis_agent::AgentDefinition`], replaces `self.agent`,
