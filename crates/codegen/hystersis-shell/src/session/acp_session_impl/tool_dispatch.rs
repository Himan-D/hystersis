...
/// - `file_path` — hystersis_build (`search_replace`), opencode (`EditTool`,
///   `WriteTool`, `ReadTool`), codex (`read_file`), hystersis_build_hashline
///   (`hashline_edit`)
/// - `path` — alternate edit/read tools
/// - `target_file` — hystersis_build (`read_file`, via `#[serde(rename)]`)
///
...
/// `list_dir`=`target_directory`. Grammar lives in `hystersis_compaction_transcript`.
pub(super) fn compaction_artifact_read(
    args: &serde_json::Value,
) -> Option<hystersis_compaction_transcript::CompactionArtifact> {
    let path = str_arg(
        args,
        &["target_file", "file_path", "path", "target_directory"],
    )?;
    hystersis_compaction_transcript::classify_compaction_path(path)
}
...
impl SessionActor {
    ...
    pub(super) async fn handle_direct_bash_command(
        ...
        let title_command = hystersis_tools::util::strip_redundant_session_cd(
            &command,
        ...
        // execute tool by kind so the hystersis.com/tool identity still stamps.
        ...
        let exec_wire = {
            ...
            agent
                .tool_bridge()
                .toolset()
                .tool_name_for_kind(hystersis_tools::types::tool::ToolKind::Execute)
        };
...
// Re-use the UTF-8-safe truncation helper from xai-hystersis-sampling-types rather
...
///    [`MAX_ARGS_IN_ERROR`] bytes).  Without this, hystersis-shell would sanitize
