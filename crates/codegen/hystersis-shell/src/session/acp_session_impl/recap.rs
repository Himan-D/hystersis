...
use hystersis_sampling_types::SamplingError;
...
fn build_side_question_attempt(base: &ConversationRequest) -> ConversationRequest {
    ...
    request.x_hystersis_req_id = Some(format!("xai-btw-{}", uuid::Uuid::new_v4()));
    request
}
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(
        ...
        if super::side_call::should_strip_side_call_reasoning(
            ...
            items = hystersis_chat_state::compaction_utils::strip_reasoning_blocks(items);
        }
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        let x_hystersis_conv_id = format!("recap-{}", uuid::Uuid::new_v4());
        let x_hystersis_req_id = format!("xai-recap-{}", uuid::Uuid::new_v4());
        let request = self
            .side_call_request(&setup, items, x_hystersis_conv_id.clone(), x_hystersis_req_id.clone())
            .await;
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. Hystersis
    ...
    fn persist_recap_request_artifact(
        ...
        reminder_tag: &str,
        x_hystersis_req_id: &str,
        x_hystersis_conv_id: &str,
        started_at: String,
        summary: Option<&str>,
        ...
        let artifact = RecapRequestFile {
            ...
            x_hystersis_req_id: x_hystersis_req_id.to_owned(),
            x_hystersis_conv_id: x_hystersis_conv_id.to_owned(),
            strip_reasoning,
    ...
    /// (`HYSTERSIS_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`hystersis-0.1`). Every
    ...
    /// `hystersis-0.1` for OAuth users) the request is **skipped
    ...
    /// a reasoning model's response, and some models (e.g. `hystersis`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            temperature: None,
            x_hystersis_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_hystersis_req_id: Some(format!("xai-promptsuggest-{}", uuid::Uuid::new_v4())),
            x_hystersis_session_id: Some(self.session_info.id.to_string()),
        ...
        tracing::debug!(
            raw_preview = %hystersis_tools::util::truncate_str(raw.trim(), 60),
...
mod tests {
    ...
    fn side_question_attempts_get_fresh_request_ids() {
        let base = ConversationRequest {
            x_hystersis_conv_id: Some("btw-test".into()),
            ..Default::default()
        };
        let a = build_side_question_attempt(&base);
        let b = build_side_question_attempt(&base);
        let (a_id, b_id) = (a.x_hystersis_req_id.unwrap(), b.x_hystersis_req_id.unwrap());
        ...
        assert_eq!(a.x_hystersis_conv_id, base.x_hystersis_conv_id);
    }
}
