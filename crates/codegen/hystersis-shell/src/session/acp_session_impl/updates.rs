...
mod inbound_summary_persist_scrub_tests {
    ...
    use crate::session::persistence::MAX_TITLE_SCALARS;
    /// Drive inbound `_hystersis.com/session/update` through `handle_hystersis_session_notification`
    ...
    async fn persist_path_scrubs_title_and_drops_manual_meta() {
        ...
        local
            .run_until(async {
                ...
                actor
                    .handle_hystersis_session_notification(XaiSessionNotification {
...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, hystersis_chat_state::UsageTotals)],
    ...
    /// AutoCompactCompleted, etc.), use `send_hystersis_notification` instead.
    ///
    /// The frequency-based split (`send_buffered_hystersis_update` vs `send_hystersis_notification`)
    /// mirrors the ACP-side split between `send_update` (high-frequency,
    /// buffered) and `emit_notification_direct` (low-frequency, direct).
    pub(super) async fn send_buffered_hystersis_update(&self, update: XaiSessionUpdate) {
        self.close_rewind_window().await;
    ...
    pub(super) async fn emit_buffered(&self, notification: SessionNotification) {
        match notification {
            ...
            SessionNotification::Xai(n) => {
                self.log_outbound_hystersis_buffered(&n);
                if self
                    .notifications
                    .gateway_enabled
    ...
    fn log_outbound_hystersis_buffered(&self, notification: &XaiSessionNotification) {
        if !matches!(
            notification.update,
        ...
        tracing::info!(
            target: "acp_event",
            event = "hystersis_buffered_notification_sent",
    ...
    /// [`Self::send_hystersis_notification`] minus persistence: broadcast-only, for
    ...
    pub(super) fn send_hystersis_notification_transient(&self, update: XaiSessionUpdate) {
        let notification = XaiSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
            meta: Some(serde_json::json!({
    ...
    pub(super) async fn handle_hystersis_session_notification(
        &self,
        ...
        match &notification.update {
            XaiSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    hystersis_hooks::event::HookEventName::SubagentStart,
                    None,
                    hystersis_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = hystersis_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        hystersis_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
    ...
    pub(super) fn persist_hystersis_update_only(&self, update: XaiSessionUpdate) {
        let notification = XaiSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            hystersis_hooks::event::HookEventName::Notification,
            None,
            hystersis_hooks::event::HookPayload::Notification {
    ...
    pub(super) async fn send_hystersis_notification(&self, update: XaiSessionUpdate) {
        self.send_hystersis_notification_with_extra_meta(update, None)
            .await;
    }
    ...
    pub(super) fn response_completed_update(
        &self,
        response: &hystersis_sampling_types::ConversationResponse,
    ) -> XaiSessionUpdate {
        let usage =
            response
                .usage
    ...
    /// [`Self::send_hystersis_notification`] with caller-supplied `_meta` keys merged
    /// into the standard eventId/timestamp meta. Caller keys win on collision.
    #[tracing::instrument(skip_all)]
    pub(super) async fn send_hystersis_notification_with_extra_meta(
        &self,
...
#[cfg(test)]
mod hystersis_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
    fn persisted_hystersis_event_id(
    ...
    /// `send_hystersis_notification` (own emission) and
    /// `handle_hystersis_session_notification` (inbound/forwarded, meta-less) —
    ...
    #[tokio::test]
    async fn actor_persisted_hystersis_lines_carry_event_id() {
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                ...
                let actor = create_test_actor(0, 256_000, 85, gateway_tx, persistence_tx).await;
                actor
                    .send_hystersis_notification(XaiSessionUpdate::HookAnnotation {
                        message: "own emission".into(),
                    })
                    .await;
                let own_id = persisted_hystersis_event_id(&mut prx);
                ...
                actor
                    ...
                    .await;
                let inbound_id = persisted_hystersis_event_id(&mut prx);
                assert!(inbound_id.starts_with("test-actor-"));
                assert_ne!(own_id, inbound_id);
                actor.persist_hystersis_update_only(XaiSessionUpdate::HookAnnotation {
                    message: "persist-only".into(),
                });
                let persist_only_id = persisted_hystersis_event_id(&mut prx);
