...
pub(super) struct MemoryFlushSnapshot {
    counts: hystersis_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &hystersis_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                hystersis_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                hystersis_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        hystersis_telemetry::session_ctx::log_event(
            hystersis_telemetry::memory_telemetry::MemorySessionSummary {
                ...
                match &result {
                    crate::session::memory::hooks::SessionEndResult::Written(path_str) => {
                        ...
                        self.reindex_and_embed(std::path::Path::new(path_str), "session")
                            .await;
                        self.send_hystersis_notification(XaiSessionUpdate::MemorySessionSaved {
                            path: path_str.clone(),
                        })
                        .await;
                    }
    ...
    async fn run_dream_inner(
        ...
        self.send_hystersis_notification(XaiSessionUpdate::MemoryDreamCompleted {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            x_hystersis_conv_id: Some(format!("dream-{}", uuid::Uuid::new_v4())),
            x_hystersis_req_id: Some(format!("xai-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: hystersis_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        let flush_start = std::time::Instant::now();

        self.send_hystersis_notification(XaiSessionUpdate::MemoryFlushStarted)
            .await;
        ...
        let result = async {
            ...
            hystersis_telemetry::session_ctx::log_event(
                hystersis_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            items.extend(
                hystersis_chat_state::compaction_utils::ModelRequestHistory::from_raw(recent).into_items(),
            );
            ...
            let request = ConversationRequest {
                items,
                model: Some(model),
                x_hystersis_conv_id: Some(format!("flush-{}", uuid::Uuid::new_v4())),
                x_hystersis_req_id: Some(format!("xai-flush-{}", uuid::Uuid::new_v4())),
                x_hystersis_session_id: Some(session_id.clone()),
                x_hystersis_agent_id: Some(hystersis_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: hystersis_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        hystersis_telemetry::session_ctx::log_event(
            hystersis_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => hystersis_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => hystersis_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => hystersis_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => hystersis_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::MemoryFlushed {
        ...
        self.send_hystersis_notification(XaiSessionUpdate::MemoryFlushCompleted {
            result: outcome,
            path: flush_path,
        })
        .await;
    ...
    pub(super) async fn snapshot_memory_flush_state(&self) -> MemoryFlushSnapshot {
        ...
        let chat_history =
            hystersis_chat_state::compaction_utils::prepare_conversation_for_summarization(conversation);
        MemoryFlushSnapshot {
            counts,
            chat_history,
        }
    ...
    /// LLM call using the `hystersis` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    hystersis_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                hystersis_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = hystersis_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = hystersis_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                hystersis_sampler::collect_response(events).await
            }
        };
