...
impl SessionActor {
    ...
    /// Emit `hystersis.com/git_head_changed` after an edit/shell command that may have
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch, commit) = tokio::join!(
            hystersis_workspace::session::git::get_worktree_info(cwd),
            hystersis_workspace::session::git::get_branch(cwd),
            hystersis_workspace::session::git::get_current_commit(cwd),
        );
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification = acp::ExtNotification::new("hystersis.com/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<hystersis_tools::implementations::hystersis_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) async fn handle_completion(
        ...
        // replayed `_hystersis.com/session/update` rail so a viewer that re-attaches
    ...
    /// persists + forwards via `send_hystersis_notification`. Retiring
    ...
    pub(super) async fn emit_turn_completed(
        ...
        self.send_hystersis_notification_with_extra_meta(
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use hystersis_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> hystersis_hooks::event::StopFailureKind {
        use hystersis_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &hystersis_agent::plugins::install_registry::InstallError,
    ) -> String {
