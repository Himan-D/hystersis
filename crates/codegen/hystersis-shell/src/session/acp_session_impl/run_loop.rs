...
pub(super) async fn fire_session_end_hooks(session: &SessionActor, reason: &str) {
    let envelope = session.fire_hook(
        hystersis_hooks::event::HookEventName::SessionEnd,
        None,
        hystersis_hooks::event::HookPayload::SessionEnd {
    ...
    if let Some(registry) = session.hook_registry.borrow().clone() {
        ...
        let results = hystersis_hooks::dispatcher::dispatch_non_blocking(
            &registry,
            hystersis_hooks::event::HookEventName::SessionEnd,
            &envelope,
            &ctx,
...
async fn log_session_ended(session: &SessionActor) {
    ...
    if let Some(signals) = session.signals_handle().snapshot().await {
        hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::SessionEnded {
...
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<hystersis_chat_state::ChatStateEvent>,
    ...
    let _elicitation_coordinator = if !session.startup_hints.is_subagent {
        let elicit_inbox = hystersis_mcp::elicitation::ElicitationInbox::new();
        {
    ...
    if !session.startup_hints.is_subagent {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<hystersis_mcp::servers::McpClientEvent>();
        {
            let mut mcp_state = session.mcp_state.lock().await;
        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: hystersis_telemetry::memory_log::TARGET,
                        ...
                        tracing::debug!(target: hystersis_telemetry::memory_log::TARGET,
                ...
                event = chat_state_event_rx.recv() => {
                    match event {
                        Some(hystersis_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
                        ...
                        Some(hystersis_chat_state::ChatStateEvent::ImageBudget {
                            body_bytes,
                            trigger_bytes,
                        ...
                        Some(hystersis_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
                        Some(hystersis_chat_state::ChatStateEvent::TokensUpdated { .. }) => {
                    ...
                    match cmd {
                        ...
                        SessionCommand::OverrideModelName { model_name, extra_headers, context_window } => {
                            ...
                            if let Some(mut cfg) = session.chat_state_handle.get_sampling_config().await {
                                ...
                                // the agent-level default (e.g. "hystersis-4.5").
                                ...
                                if let Some(r) = crate::agent::config::try_resolve_model_credentials(model_name.as_str(), existing.api_key.as_deref()) {
                                    session.chat_state_handle.update_credentials(hystersis_chat_state::Credentials {
                                        api_key: r.api_key,
                        ...
                        SessionCommand::GetHooksList { respond_to } => {
                            ...
                            let _ = respond_to.send(hystersis_hooks_plugins_types::HooksListResponse {
                                hooks,
                                project_trusted,
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        hystersis_tools::implementations::hystersis_build::monitor::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = hystersis_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::XaiSessionNotification { notification } => {
                            session.handle_hystersis_session_notification(notification).await;
                        }
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            // also fires for `<cwd>/.hystersis/config.toml` edits,
                            ...
                            {
                                let _ = tx.send(
                                    hystersis_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::ToggleMcpServer { server_name, enabled, server_config, respond_to } => {
                            session.events.emit(hystersis_session_events::Event::McpServerToggled {
                            ...
                            if is_managed_gateway {
                                ...
                                tokio::task::spawn_local(async move {
                                    ...
                                    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
                                        notifications.forward_fire_and_forget(acp::ExtNotification::new("hystersis.com/mcp/tools_changed", params.into()));
                                    }
                                    let _ = respond_to.send(Ok(()));
                                });
                                continue;
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                hystersis_hooks::event::HookEventName::SessionStart,
                                None,
                                hystersis_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[hystersis_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = hystersis_chat_state::compaction_utils::extract_user_query(&raw);
