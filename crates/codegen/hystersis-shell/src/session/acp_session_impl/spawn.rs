...
fn drop_cli_catchall_allows(
    rules: Vec<hystersis_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<hystersis_workspace::permission::types::PermissionRule>,
    Vec<hystersis_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if hystersis_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
/// process-lifetime (`hystersis_tty_utils::runtime`).
pub(crate) fn build_session_runtime() -> std::io::Result<tokio::runtime::Runtime> {
    let mut builder = tokio::runtime::Builder::new_current_thread();
    hystersis_tty_utils::runtime::apply_blocking_pool(builder.enable_all()).build()
}
fn configured_memory_retrieval_mode(
    config: Option<&crate::config::MemoryConfig>,
) -> hystersis_telemetry::events::MemoryRetrievalMode {
    use hystersis_telemetry::events::MemoryRetrievalMode::*;
...
/// ([`hystersis_sampler::RATE_LIMIT_RETRY_DISABLED`]). If the pacer is disabled
/// (`pacer_max_attempts == 0`) the subagent falls back to the sampler's own 429
/// retry ([`hystersis_sampler::RATE_LIMIT_RETRY_THRESHOLD`]) so disabling the
...
fn subagent_sampler_rate_limit_threshold(is_subagent: bool, pacer_max_attempts: u32) -> u32 {
    if is_subagent && pacer_max_attempts > 0 {
        hystersis_sampler::RATE_LIMIT_RETRY_DISABLED
    } else {
        hystersis_sampler::RATE_LIMIT_RETRY_THRESHOLD
    }
}
#[cfg(all(test, unix))]
...
mod cli_catchall_drop_tests {
    ...
    use hystersis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use hystersis_workspace::permission::rules::parse_permission_rule;
    use hystersis_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
    ...
    fn disabled_memory_config_has_disabled_retrieval_mode() {
        assert_eq!(
            ...
            hystersis_telemetry::events::MemoryRetrievalMode::Disabled
        );
    }
...
mod subagent_rate_limit_threshold_tests {
    ...
    use hystersis_sampler::{RATE_LIMIT_RETRY_DISABLED, RATE_LIMIT_RETRY_THRESHOLD};
    #[test]
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: hystersis_chat_state::Credentials,
    auth_method_id: crate::agent::auth_method::SharedAuthMethodId,
    auth_manager: Option<Arc<AuthManager>>,
    attribution_callback: Option<hystersis_sampler::SharedAttributionCallback>,
    ...
    compaction_mode: hystersis_chat_state::CompactionMode,
    ...
    web_search_sampling_config: Option<hystersis_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: hystersis_tools::implementations::hystersis_build::app_builder::AppBuilderDeployerConfig,
    ...
    media_gen_batch_limits: hystersis_tools::media_gen_limits::MediaGenBatchLimits,
    ...
    prompt_audience: hystersis_agent::prompt::context::PromptAudience,
    ...
    plugin_registry: Option<std::sync::Arc<hystersis_agent::plugins::PluginRegistry>>,
    plugin_registry_handle: Option<hystersis_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<hystersis_workspace::permission::PermissionHandle>,
    api_key_provider: Option<hystersis_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<hystersis_hooks::discovery::HookRegistry>>,
    workspace_ops: hystersis_workspace::WorkspaceOps,
    cli_permission_rules: Vec<hystersis_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn hystersis_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        hystersis_tools::implementations::hystersis_build::scheduler::types::SchedulerHandle,
    >,
    max_turns: Option<usize>,
    forked_tool_override: Option<Vec<ToolSpec>>,
    is_chat_kind: bool,
    spawn_timer: Option<hystersis_telemetry::subagent_spawn::SharedSubagentSpawnTimer>,
    ...
    hystersis_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(hystersis_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            hystersis_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
                project_trusted,
            )
            .await;
        let yolo_pin = hystersis_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("hystersis: --allow catch-all ignored: {reason}");
            }
        }
            ...
            match &mut permission_config {
                ...
                None => {
                    permission_config = Some(
                        hystersis_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(hystersis_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if hystersis_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    hystersis_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn hystersis_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            hystersis_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        hystersis_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            hystersis_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("HYSTERSIS_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "HYSTERSIS_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    let chat_state_sampling_config = hystersis_sampling_types::SamplingConfig {
    ...
    let actor_pruning_config = hystersis_chat_state::PruningConfig {
    ...
    let chat_state_handle = hystersis_chat_state::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    let task_completion_reservations =
        hystersis_tools::reminders::task_completion::TaskCompletionReservations::default();
    let task_wake_suppressed =
        hystersis_tools::reminders::task_completion::TaskWakeSuppressed::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        hystersis_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        hystersis_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    let resolve_policy = || crate::util::config::resolve_shell_env_policy(effective_cfg.as_ref());
    let terminal_backend: std::sync::Arc<dyn hystersis_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn hystersis_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn hystersis_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(hystersis_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(hystersis_tools::computer::local::LocalFs)
        };
    ...
    let compaction_policy = hystersis_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        hystersis_tools::implementations::hystersis_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn hystersis_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target: hystersis_telemetry::memory_log::TARGET,
                error = %e,
        ...
        let backend: std::sync::Arc<dyn hystersis_tools::types::memory_backend::MemoryBackend> =
        ...
        hystersis_telemetry::session_ctx::log_event(
            hystersis_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        hystersis_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    let mcp_state = {
        ...
        if !acp_mcp_servers.is_empty() {
            ...
            tracing::info!(
                ...
                "Registered in-process SDK MCP servers (hystersis.com/mcp/sdk_call)"
            );
        }
    ...
    let harness_metrics = if !startup_hints.is_subagent
        && (telemetry_enabled || hystersis_telemetry::external::is_active())
    {
        ...
        Some(super::telemetry::SessionHarnessMetrics {
            ...
            permission_mode: if session_yolo_mode {
                hystersis_telemetry::enums::PermissionMode::AlwaysApprove
            } else if session_auto_mode
                && crate::util::config::auto_permission_mode_enabled_from_disk()
            {
                hystersis_telemetry::enums::PermissionMode::Auto
            } else {
                hystersis_telemetry::enums::PermissionMode::Ask
            },
    ...
    let resolved_task_output =
        hystersis_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        hystersis_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        hystersis_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<hystersis_tools::implementations::hystersis_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    if let Some(ref timer) = spawn_timer {
        use hystersis_telemetry::subagent_spawn::SubagentSpawnPhase;
    ...
    let feedback_client_type = match client_type {
        ClientType::HystersisTUI => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
        ClientType::HystersisWeb => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Web,
        ...
        ClientType::HystersisPager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = hystersis_workspace::session::git::find_git_root_from_path(
    ...
    let sampler_retry_policy = hystersis_sampler::RetryPolicy {
    ...
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<hystersis_sampler::SamplingEvent>();
    let sampler_handle = hystersis_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<hystersis_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<hystersis_hooks::discovery::HookRegistry>> =
        if let Some(override_reg) = hook_registry_override {
            ...
            let git_root = hystersis_workspace::session::git::find_git_root_from_path(cwd_path).ok();
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        hystersis_tools::implementations::hystersis_build::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        hystersis_tools::implementations::hystersis_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use hystersis_tools::implementations::hystersis_build::workflow::WorkflowLaunchAck;
            while let Some((req, ack)) = workflow_launch_rx.recv().await {
                if !background_workflows_enabled {
                    let _ = ack.send(WorkflowLaunchAck::Rejected {
                        ...
                        detail: "Background workflows are disabled for this session \
                                 ([workflows] enabled = false / HYSTERSIS_WORKFLOWS=0 / remote flag)."
                            .into(),
                    });
                    continue;
                }
                ...
                use hystersis_tools::implementations::hystersis_build::workflow::WorkflowSource;
                ...
                if input.validate_only {
                    ...
                    let agent_budget = input
                        .agent_budget
                        .unwrap_or(hystersis_workflow::DEFAULT_AGENT_BUDGET);
                    tokio::spawn(async move {
                        let verdict = tokio::task::spawn_blocking(move || {
                            hystersis_workflow::validate_script_with_agent_budget(
                                &script,
                                probe_args,
                                agent_budget,
                            )
    ...
    let obs_bridge = {
        let sid = hystersis_tool_protocol::SessionId::new(&*session_info.id.0)
            .unwrap_or_else(|_| hystersis_tool_protocol::SessionId::new("unknown").expect("valid"));
        hystersis_computer_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<hystersis_sampling_types::ToolOverrides>,
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: hystersis_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match hystersis_workspace::session::git::discover_git_root(root) {
                hystersis_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    hystersis_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => hystersis_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(hystersis_tools::types::tool_index::ToolIndex(
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(hystersis_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            hystersis_tools::implementations::hystersis_build::workflow::WorkflowLaunchHandle(
    ...
    if !background_workflows_enabled {
        session
            ...
            .tool_bridge()
            .update_resource(
                hystersis_tools::implementations::hystersis_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                hystersis_telemetry::session_ctx::log_event(
                    hystersis_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        use hystersis_tools::implementations::hystersis_build::ask_user_question::{
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = hystersis_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            hystersis_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    let hosting = hystersis_telemetry::activity::SESSIONS_ACTIVE.enter();
    tokio::task::spawn_local(async move {
        let _hosting = hosting;
        hystersis_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, hystersis_agent::AgentBuildError>,
    >();
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx.send(Err(hystersis_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
            ...
            let actor_main = async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent": tp })
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let span = hystersis_file_utils::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
            ...
            rt.block_on(hystersis_telemetry::session_ctx::drain_at_session_exit());
        });
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    hystersis_sampling_types::conversation::ContentPart::Text { text }
