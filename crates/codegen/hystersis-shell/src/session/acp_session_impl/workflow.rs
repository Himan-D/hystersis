        ...
        match op {
            ...
            ManageOp::Resume => {
                ...
                if status == WorkflowRunStatus::BudgetLimited {
                    ...
                    if used >= hystersis_workflow::MAX_AGENT_BUDGET {
                        return format!(
                            "Run '{name}' exhausted the maximum agent budget ({used}{limit} agents) \
                             and cannot be resumed. Start a new run instead."
                        );
                    }
                    let suggested = used.saturating_add(64).min(hystersis_workflow::MAX_AGENT_BUDGET);
                    return format!(
    ...
    for run in &runs {
        ...
        if !objective.is_empty() {
            let _ = write!(
                out,
                "\n  Objective: {}",
                hystersis_tools::util::truncate_str(
                    &objective,
