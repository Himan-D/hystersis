...
fn partition_rules_by_scope(
    files: Vec<hystersis_agent::prompt::agents_md::AgentConfigFile>,
    hystersis_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_roots: &[&std::path::Path],
) -> (
    Vec<hystersis_agent::prompt::user_message::RuleEntry>,
    Vec<hystersis_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = hystersis_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use hystersis_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[hystersis_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
    ...
    fn partitioned_snapshot_bodies_match_rules_and_old_reminder() {
        ...
        let rules =
            hystersis_agent::prompt::user_message::format_rules_section(&workspace, &user).unwrap();
        let reminder = hystersis_agent::prompt::agents_md::format_agents_md_section(&files).unwrap();
        for body in [
            "repo-agents-body",
            "repo-claude-body",
            "home-hystersis-body",
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == Some(hystersis_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    pub(super) async fn build_user_message_prefix(&self) -> String {
        ...
        if matches!(template, UserMessageTemplate::Default) && include_verification {
            ...
            user_rules.splice(
                0..0,
                hystersis_agent::prompt::browser_verification::synthetic_user_rules(),
            );
            hystersis_agent::prompt::user_message::append_rules_section(
                &mut out,
                &workspace_rules,
                &user_rules,
            );
    ...
    async fn build_templated_user_message(
        ...
        use hystersis_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use hystersis_workspace::file_system::{git_status_short, jj_status};
        use hystersis_workspace::session::git::VcsKind;
    ...
    ) -> Vec<hystersis_agent::prompt::user_message::McpServerEntry> {
        use hystersis_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let current_query = crate::session::image_describe::strip_template_context_tags(
            &hystersis_chat_state::compaction_utils::extract_user_query(&original_user_message),
        );
        ...
        let client = hystersis_sampler::SamplingClient::new(sampler_config).map_err(|e| {
