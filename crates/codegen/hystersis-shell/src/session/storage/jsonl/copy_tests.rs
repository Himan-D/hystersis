...
#[tokio::test]
async fn copy_session_data_transforms_hystersis_updates() {
    use crate::extensions::notification::{
    ...
    adapter
        .append_update(
            &source_info,
            &SessionUpdate::Xai(Box::new(hystersis_notification)),
        )
        .await
        .unwrap();
...
async fn copy_session_data_with_model_override() {
    ...
    let loaded = adapter.load_session(&target_info).await.unwrap();
    assert_eq!(loaded.summary.current_model_id.0.as_ref(), "hystersis-3");
    assert_eq!(
        loaded.summary.parent_session_id,
