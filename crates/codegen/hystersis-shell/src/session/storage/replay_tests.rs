...
fn hystersis_envelope(session_update_json: &str) -> String {
    format!(
        r#"{{"timestamp":1,"method":"_hystersis.com/session/update","params":{{"sessionId":"s","update":{session_update_json}}}}}"#
    )
}
...
fn stream_replay_updates_at_missing_session_is_empty() {
    ...
    std::fs::create_dir_all(hystersis_home.path().join("sessions")).unwrap();

    let mut count = 0usize;
    let emission =
        stream_replay_updates_at("does-not-exist", hystersis_home.path(), |_| count += 1).unwrap();
...
fn stream_replay_updates_at_surfaces_read_errors() {
    let hystersis_home = tempfile::tempdir().unwrap();
    let session_dir = hystersis_home.path().join("sessions").join("cwd").join("sess");
    std::fs::create_dir_all(&session_dir).unwrap();
    std::fs::write(session_dir.join(SUMMARY_FILE), "{}").unwrap();
    std::fs::create_dir(session_dir.join(UPDATES_FILE)).unwrap();

    let result = stream_replay_updates_at("sess", hystersis_home.path(), |_| {});
    assert!(
        result.is_err(),
...
fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
    ...
    let old_xai = r#"{"timestamp":2,"method":"_hystersis.com/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"}}}"#;
    let raw = format!("{a1}\n{old_xai}\n");
    ...
    let new_xai = r#"{"timestamp":2,"method":"_hystersis.com/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"},"_meta":{"eventId":"ev2"}}}"#;
    let raw = format!("{a1}\n{new_xai}\n");
    let prepared = prepare_replay_lines(&raw, Some("ev1"));
    assert!(!prepared.mark_replay);
    assert_eq!(prepared.lines.len(), 1);
...
fn prepare_replay_reports_spawn_without_finish() {
    let spawn = |id: &str, child: &str| {
        format!(
            r#"{{"method":"_hystersis.com/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_spawned","subagent_id":"{id}","parent_session_id":"s","child_session_id":"{child}","subagent_type":"general-purpose","description":"task"}},"_meta":{{"eventId":"s-1"}}}}}}"#
        )
    };
    let finish = |id: &str| {
        format!(
            r#"{{"method":"_hystersis.com/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_finished","subagent_id":"{id}","child_session_id":"c{id}","status":"completed","tool_calls":0,"turns":0,"duration_ms":0}},"_meta":{{"eventId":"s-2"}}}}}}"#
        )
    };
...
fn persist_hystersis_update(update: crate::extensions::notification::SessionUpdate) -> String {
...
fn stream_replay_forwards_completed_tool_call_update_without_base() {
    ...
    let encoded = hystersis_config::encode_cwd_dirname(cwd);
...
#[test]
fn stream_replay_forwards_hystersis_updates_in_file_order() {
    let home = tempfile::tempdir().unwrap();
    let sid = "child-xai";
    let dir = home.path().join("sessions").join("cwd").join(sid);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join(SUMMARY_FILE), "{}").unwrap();
    let compact = persist_hystersis_update(
...
#[test]
fn hystersis_only_transcript_forwards_but_stays_empty() {
    let home = tempfile::tempdir().unwrap();
    let sid = "child-xai-only";
    let dir = home.path().join("sessions").join("cwd").join(sid);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join(SUMMARY_FILE), "{}").unwrap();
    let retry = persist_hystersis_update(crate::extensions::notification::SessionUpdate::RetryState(
...
fn replay_would_emit_requires_an_emitting_acp_line() {
    ...
    let hystersis_only = persist_hystersis_update(
        crate::extensions::notification::SessionUpdate::AutoCompactCompleted {
            tokens_before: Some(1_000),
            tokens_after: 100,
            elapsed_ms: Some(5),
...
fn child_fast_path_finds_updates_under_child_cwd() {
    ...
    let encoded = hystersis_config::encode_cwd_dirname(child_cwd);
    let sid = "child-wt";
    let dir = home.path().join("sessions").join(&encoded).join(sid);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join(SUMMARY_FILE), "{}").unwrap();
...
fn child_lookup_falls_back_when_fast_path_misses() {
    let home = tempfile::tempdir().unwrap();
    let other = hystersis_config::encode_cwd_dirname("/other/cwd");
