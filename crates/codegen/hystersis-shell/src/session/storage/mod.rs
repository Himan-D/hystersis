...
pub use hystersis_session_search::fts as search_fts;
...
pub(crate) const XAI_SESSION_UPDATE_METHOD: &str = "_hystersis.com/session/update";
...
pub(crate) struct SessionUpdateEnvelope {
    ...
    /// Either "session/update" for ACP or "_hystersis.com/session/update" for xAI extensions.
    pub method: String,
...
pub enum AppendCwdSwitchError {
    NotCommitted(io::Error),
    Committed {
        acknowledgement: hystersis_chat_state::StrictAppendAck,
        source: io::Error,
    },
}
...
pub trait StorageAdapter: Send + Sync {
    ...
    ) -> Result<hystersis_chat_state::StrictAppendAck, AppendCwdSwitchError> {
...
mod tests {
    ...
    #[test]
    fn hystersis_rewind_marker_yields_rewind_to() {
        let line = hystersis_envelope(
    ...
    #[test]
    fn hystersis_rewind_to_zero_yields_rewind_to_zero() {
        let line = hystersis_envelope(
    ...
    #[test]
    fn hystersis_diff_review_yields_not_user() {
        let line = hystersis_envelope(r#"{"sessionUpdate":"diff_review","content":[]}"#);
        assert_eq!(
            parse_prompt_extract_event(&line),
            PromptExtractEvent::NotUserMessage
        );
    ...
    fn filter_rewind_ignores_a_malformed_middle_line() {
        ...
        let rewind_to_1 = hystersis_envelope(
            r#"{"sessionUpdate":"rewind_marker","target_prompt_index":1,"created_at":"2024-01-01"}"#,
        );
    ...
    #[test]
    fn from_str_unknown_hystersis_variant_deserializes_via_envelope() {
        ...
        let line = hystersis_envelope(r#"{"sessionUpdate":"git_branch_update","branch":"main"}"#);
    ...
    #[test]
    fn from_str_known_hystersis_variant_still_works() {
        let line = hystersis_envelope(r#"{"sessionUpdate":"memory_flush_started"}"#);
        let update = SessionUpdateEnvelope::from_str(&line).unwrap();
        match update {
            SessionUpdate::Xai(notif) => {
                assert_eq!(
