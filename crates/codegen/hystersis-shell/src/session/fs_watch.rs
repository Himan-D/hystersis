//! Session-level fs-watch policy over [`hystersis_fsnotify`].
//!
//! Mechanism (OS watch, coalesce, refcount) lives in `hystersis_fsnotify`. This module
...
use hystersis_workspace::file_system::{CodebaseIndexManager, FileIndex, WalkOptions};
...
/// Dedup key for `hystersis.com/git_head_changed`, shared by the watcher's `GitHead`
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> hystersis_workspace::file_system::FileIndexDelta {
    use hystersis_workspace::file_system::FileIndexDelta;
...
pub(crate) struct CapabilityInputs {
    ...
    pub hunk_tracking: bool,
    pub code_nav: bool,
    /// `hystersis.com/gitHeadChanged`; opt-in (absent => off).
...
impl ClientNotify {
    async fn on_change(&self, paths: &[PathBuf], kind: FsEventKind) {
        ...
        match self.mode {
            ClientFsMode::Events => {
                // Present-tense strings are the `hystersis.com/fs_notify` wire protocol;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = hystersis_workspace::session::git::get_branch(&self.cwd).await;
        let commit = hystersis_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = hystersis_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = hystersis_workspace::session::git::GitHeadChanged {
            ...
            if let Ok(raw) = serde_json::value::to_raw_value(&params) {
                self.gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "hystersis.com/git_head_changed",
                        raw.into(),
                    ));
            }
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: hystersis_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root =
                hystersis_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || hystersis_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = hystersis_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(hystersis_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: hystersis_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
