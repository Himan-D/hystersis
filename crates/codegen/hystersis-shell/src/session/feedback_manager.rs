...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(hystersis_version::VERSION.to_string());
    s
}
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if let Some(user_meta) = crate::agent::mvp_agent::parse_json_object_env("HYSTERSIS_USER_METADATA") {
    ...
    if telemetry_enabled {
        ...
        hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `HYSTERSIS_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `HYSTERSIS_TELEMETRY_ENABLED`. These are analytics data that
    ...
    /// [`SHUTDOWN_DRAIN_CAP`] (or `HYSTERSIS_SESSION_EXIT_DRAIN_SECS`) so a
...
pub struct FeedbackManager {
    ...
    upload_queue_stats: std::sync::OnceLock<Arc<hystersis_file_utils::queue::UploadQueueStats>>,
}
...
impl FeedbackManager {
    ...
    pub(crate) fn set_upload_queue_stats(
        &self,
        stats: Arc<hystersis_file_utils::queue::UploadQueueStats>,
    ) {
    ...
    /// `hystersis.com/debug/trigger_feedback` ACP extension method to exercise
    ...
    /// `HYSTERSIS_SESSION_EXIT_DRAIN_SECS` up to hard max 7s). Incomplete drains
    /// leave durable pairs on disk for next-session recovery.
    pub async fn shutdown(&self, queue: Option<&hystersis_file_utils::queue::UploadQueue>) {
        let pending = queue
...
/// residual time for hooks/memory. Override with `HYSTERSIS_SESSION_EXIT_DRAIN_SECS`
...
/// on slow networks may raise it with `HYSTERSIS_SESSION_EXIT_DRAIN_SECS` up to
...
fn nonempty_drain_budget(config_timeout: Duration) -> Duration {
    let cap = std::env::var("HYSTERSIS_SESSION_EXIT_DRAIN_SECS")
        .ok()
...
mod tests {
    ...
    async fn test_shutdown_with_upload_queue_drains() {
        ...
        use std::sync::Arc;
        use hystersis_file_utils::queue::{TraceExportSource, UploadQueue, UploadRetryPolicy};
    ...
    fn test_shutdown_budgets_fit_under_session_flush_grace() {
        ...
        let _unset = hystersis_test_support::env::EnvGuard::unset("HYSTERSIS_SESSION_EXIT_DRAIN_SECS");
        assert!(
    ...
    fn test_nonempty_drain_budget_env_raises_cap() {
        {
            let _guard =
                hystersis_test_support::env::EnvGuard::set("HYSTERSIS_SESSION_EXIT_DRAIN_SECS", "7");
            assert_eq!(
                nonempty_drain_budget(Duration::from_secs(30)),
        ...
        {
            let _guard =
                hystersis_test_support::env::EnvGuard::set("HYSTERSIS_SESSION_EXIT_DRAIN_SECS", "99");
            assert_eq!(
                nonempty_drain_budget(Duration::from_secs(30)),
    ...
    async fn test_is_auth_permanently_failed_reads_auth_manager() {
        ...
        use crate::auth::{AuthManager, HystersisAuth, HystersisComConfig};
        use std::sync::Arc;
...
mod author_identity_tests {
    ...
    fn text_submission() -> FeedbackSubmission {
        ...
        s.model_id = Some("hystersis-4".to_string());
        s
    }
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        let _email =
            hystersis_test_support::env::EnvGuard::set("HYSTERSIS_TEST_WORK_EMAIL", "ada@corp.example");
        let _name =
            hystersis_test_support::env::EnvGuard::set("HYSTERSIS_TEST_WORK_NAME", "Ada Lovelace");

        // The loader expands `$VAR` at load, exactly as a trusted config tier ships it.
        let mut value = toml::from_str::<toml::Value>(
            r#"
[feedback.user]
name = ["$HYSTERSIS_TEST_WORK_NAME"]
email = ["$HYSTERSIS_TEST_WORK_EMAIL"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(body["modelId"], "hystersis-4");
        ...
        assert_eq!(persisted.model_id.as_deref(), Some("hystersis-4"));
    }

    /// `HYSTERSIS_USER_METADATA` is merged into the submission and travels with it:
    ...
    async fn workflow_merges_user_metadata_into_submission() {
        let _guard = hystersis_test_support::env::EnvGuard::set(
            "HYSTERSIS_USER_METADATA",
