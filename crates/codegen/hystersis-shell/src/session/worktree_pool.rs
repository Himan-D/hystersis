...
//!   under `~/.hystersis/worktree_pool/<instance_id>/` with a `.pid` liveness
...
use hystersis_fast_worktree::{WorktreeBuilder, WorktreeSync};

const WORKTREE_POOL_LOG: &str = "hystersis_worktree_pool";
use crate::util::config::PoolConfig;
use crate::util::hystersis_home::hystersis_home;

use hystersis_tty_utils::git_command;
...
/// `~/.hystersis/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.hystersis/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.hystersis/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.hystersis/worktree_pool/`, reads each `.pid` file, and checks
...
pub fn cleanup_stale_pool_worktrees(source_git_root: Option<&Path>) {
    ...
    if has_adoptable {
        ...
        REGISTRATION_CLEANUP_ONCE.call_once(move || {
            let removed = hystersis_fast_worktree::remove_stale_worktree_registrations_under(
                &root,
                &pool_base_directory(),
            );
            tracing::info!(
...
async fn warm_source_repo(repo_path: &Path) {
    ...
    let _ = tokio::task::spawn_blocking(move || {
        ...
        for &(key, val) in hystersis_tty_utils::GIT_AUTH_SUPPRESSION_ENVS.iter() {
            cmd.env(key, val);
        }
        let result = cmd
...
/// `CleanAll` mode. Placed under `~/.hystersis/worktree_pool/<instance_id>/<uuid>/`.
async fn create_pooled_worktree(
    ...
    let result = tokio::task::spawn_blocking(move || {
        WorktreeBuilder::new(&source, &dest_clone)
            .creation_mode(hystersis_fast_worktree::CreationMode::GitCheckout)
            .parallelism(parallelism)
            .worktree_kind(hystersis_fast_worktree::WorktreeKind::Pool)
            .create()
    })
    .await?;
...
fn schedule_cleanup(path: PathBuf) {
    ...
    tokio::task::spawn(async move {
        ...
        if path.exists() {
            ...
            let result = tokio::task::spawn_blocking(move || {
                hystersis_fast_worktree::remove_worktree(&path_clone)
            })
            .await;
...
async fn remove_worktree_registration(path: &Path) {
    ...
    let result =
        tokio::task::spawn_blocking(move || hystersis_fast_worktree::remove_worktree(&path_owned)).await;
