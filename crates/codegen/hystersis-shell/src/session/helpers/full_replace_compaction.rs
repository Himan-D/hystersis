//! hystersis's L5 wiring onto the shared full-replace engine
//! (`hystersis_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](hystersis_compaction::sample_full_replace_summary);
//! this module adapts hystersis's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](hystersis_compaction::FullReplaceError).
...
use hystersis_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use hystersis_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
...
/// [`CompactionSampler`] for hystersis's full-replace pass.
///
...
/// else the structured hystersis prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the hystersis prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured hystersis), ignoring the shared
...
/// Map hystersis's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces hystersis's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    hystersis_telemetry::session_ctx::log_event(CompactionRetryDegraded {
