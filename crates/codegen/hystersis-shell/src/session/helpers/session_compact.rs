...
pub use hystersis_chat_state::compaction_utils::{
...
pub(crate) use hystersis_sampling_types::is_context_length_error;
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_hystersis_agent_id = Some(hystersis_telemetry::id::agent_id());
                    ...
                    Ok(chunk) => {
                        if let Some(choice) = chunk.choices.first() {
                            ...
                            if let Some(fr) = choice.finish_reason {
                                let sr = hystersis_sampling_types::StopReason::from(fr);
                                truncated =
                                    matches!(sr, hystersis_sampling_types::StopReason::Length);
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        if !matches!(
                            &event,
                            hystersis_sampling_types::messages::MessageStreamEvent::Ping
                        ) {
                            last_progress_at = std::time::Instant::now();
                        }
                        match event {
                        hystersis_sampling_types::messages::MessageStreamEvent::ContentBlockDelta {
                            delta: hystersis_sampling_types::messages::StreamDelta::TextDelta { text },
                            ..
                        } => {
                            timing.record_delta();
                            content.push_str(&text);
                        }
                        hystersis_sampling_types::messages::MessageStreamEvent::MessageDelta { delta, .. } => {
                            if let Some(sr) = delta.stop_reason {
                                truncated = matches!(
                                    sr,
                                    hystersis_sampling_types::messages::StopReason::MaxTokens
                                        | hystersis_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                );
                                stop_reason = Some(
                                    match sr {
                                        hystersis_sampling_types::messages::StopReason::EndTurn => "end_turn".to_string(),
                                        hystersis_sampling_types::messages::StopReason::MaxTokens => "max_tokens".to_string(),
                                        hystersis_sampling_types::messages::StopReason::ToolUse => "tool_use".to_string(),
                                        hystersis_sampling_types::messages::StopReason::StopSequence => "stop_sequence".to_string(),
                                        hystersis_sampling_types::messages::StopReason::Refusal => "refusal".to_string(),
                                        hystersis_sampling_types::messages::StopReason::PauseTurn => "pause_turn".to_string(),
                                        hystersis_sampling_types::messages::StopReason::ModelContextWindowExceeded => "model_context_window_exceeded".to_string(),
                                        // Record the wire value, not a fabricated sentinel.
                                        hystersis_sampling_types::messages::StopReason::Unknown(s) => s,
                                    },
                                );
                            }
                        }
