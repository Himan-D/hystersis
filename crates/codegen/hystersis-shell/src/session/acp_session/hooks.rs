...
//! both matched by the agent ([`hystersis_hooks::matcher::HookMatcher`], shared with
//! file hooks):
...
use hystersis_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use hystersis_telemetry::events::ClientHookGateOutcome;
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| {
            hystersis_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value)
        })
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::HookBlocked {
    ...
    > {
        let default_timeout =
            if envelope.hook_event_name.traits().gate == hystersis_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                hystersis_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    hystersis_telemetry::session_ctx::log_event(
                        hystersis_telemetry::events::ClientHookGate {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> hystersis_hooks::dispatcher::StopDispatchResult {
        use hystersis_hooks::result::HookRunResult;

        let mut out = hystersis_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed, gate_outcome)) in ordered {
            ...
            let detail = hystersis_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                hystersis_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    hystersis_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.may_have_hooks_for(HookEventName::Stop));
