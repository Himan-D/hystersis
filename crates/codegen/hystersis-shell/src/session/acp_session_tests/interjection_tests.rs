...
mod interjection_broadcast_tests {
    ...
    /// `handle_interjection` depends on: method `hystersis.com/session/interjection`
            ...
            .run_until(async {
                ...
                while let Ok(msg) = gateway_rx.try_recv() {
                    if let hystersis_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                        && args.request.method.as_ref() == "hystersis.com/session/interjection"
                    {
                        payload =
                ...
                let payload = payload.expect("an hystersis.com/session/interjection broadcast");
                assert_eq!(
