...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(hystersis_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                hystersis_acp_lib::AcpClientMessage,
            >();
            ...
            let cwd = AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
            let fs = Arc::new(
                hystersis_workspace::file_system::MockFs::new(cwd.to_path_buf()),
            );
            let terminal = Arc::new(DummyTerminal {});
            let (hunk_tx, _hunk_rx) = tokio::sync::mpsc::unbounded_channel();
            let hunk_tracker_handle = hystersis_hunk_tracker::HunkTrackerActor::spawn(
            ...
            agent
                .tool_bridge()
                .update_resource(
                    hystersis_tools::implementations::hystersis_build::task::types::CurrentPromptIdResource(
                        "running".to_string(),
                    ),
                )
                .await;
...
/// user's next queued prompt) and rebroadcasts `hystersis.com/queue/changed`. The
    ...
    local
        .run_until(async {
            ...
            assert_eq!(
                user.text_content(),
                frame_user_turn(
                    hystersis_interjection_core::INTERJECTION_NOTE,
                    &expected_assembled
                )
            );
            prompt_task.abort();
    ...
    local
        .run_until(async {
            ...
            let cfg = hystersis_sampler::SamplerConfig {
                ...
                temperature: None,
                top_p: None,
                api_backend: hystersis_sampler::ApiBackend::Responses,
                auth_scheme: Default::default(),
                extra_headers: Default::default(),
                extra_response_includes: Vec::new(),
                query_params: Default::default(),
            ...
            let (sampler_event_tx, _sampler_event_rx) = tokio::sync::mpsc::unbounded_channel::<
                hystersis_sampler::SamplingEvent,
            >();
            let sampler_handle = hystersis_sampler::SamplerActor::spawn(
                cfg,
                hystersis_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                hystersis_acp_lib::AcpClientMessage,
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(
                        hystersis_sampling_types::UserItem {
                            content: vec![hystersis_sampling_types::ContentPart::Text {
                                text: "hi".into(),
                            }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use hystersis_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    hystersis_sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
