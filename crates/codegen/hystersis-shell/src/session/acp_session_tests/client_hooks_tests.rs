...
fn install_client_hook(
    actor: &SessionActor,
    event: hystersis_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
...
fn spawn_deny_responder(
    gateway_rx: tokio::sync::mpsc::UnboundedReceiver<hystersis_acp_lib::AcpClientMessage>,
    reason: &'static str,
) {
    ...
    tokio::task::spawn_local(async move {
        while let Some(msg) = gateway_rx.recv().await {
            match msg {
                hystersis_acp_lib::AcpClientMessage::ExtMethod(args) => {
    ...
    local
        .run_until(async {
            ...
            actor.fire_hook(
                hystersis_hooks::event::HookEventName::Stop,
                None,
                hystersis_hooks::event::HookPayload::Stop {
            ...
            let hystersis_acp_lib::AcpClientMessage::ExtNotification(args) = msg else {
                panic!("expected an hystersis.com/hooks/event ext notification");
            };
            assert_eq!(args.request.method.as_ref(), "hystersis.com/hooks/event");
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                hystersis_tools::registry::types::ToolConfig::for_tool::<
                    hystersis_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        hystersis_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
async fn post_tool_use_and_failure_never_double_fire() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_hystersis_build_agent_with_todo().await;
                ...
                |rx: &mut tokio::sync::mpsc::UnboundedReceiver<hystersis_acp_lib::AcpClientMessage>| {
                    ...
                    while let Ok(msg) = rx.try_recv() {
                        if let hystersis_acp_lib::AcpClientMessage::ExtNotification(args) = msg
                            && args.request.method.as_ref() == "hystersis.com/hooks/event"
                        {
                            let params: serde_json::Value =
                                serde_json::from_str(args.request.params.get()).unwrap();
...
pub(super) fn file_registry_with_spec(
    event: hystersis_hooks::event::HookEventName,
    script: &str,
) -> hystersis_hooks::discovery::HookRegistry {
...
fn file_registry(
    ...
    let (mut registry, _) = hystersis_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![hystersis_hooks::config::HookSpec {
...
/// but still delivers the observe `hystersis.com/hooks/event` notification.
#[tokio::test(flavor = "current_thread")]
            ...
            *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry_with_spec(
            ...
            tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    match msg {
                        hystersis_acp_lib::AcpClientMessage::ExtMethod(args) => {
                            if args.request.method.as_ref() == "hystersis.com/hooks/run" {
                                runs.set(runs.get() + 1);
                            }
                        ...
                        hystersis_acp_lib::AcpClientMessage::ExtNotification(args) => {
                            if args.request.method.as_ref() == "hystersis.com/hooks/event" {
...
/// `"subagent_stop"` on the wire, matching `HYSTERSIS_HOOK_EVENT`.
#[tokio::test(flavor = "current_thread")]
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                hystersis_hooks::event::HookEventName::SubagentEnd,
                None,
                hystersis_hooks::event::HookPayload::SubagentStop {
                    phase: hystersis_hooks::event::SubagentStopPhase::Observe,
...
async fn an_unanswered_client_gate_leaves_the_report_unspent() {
    ...
    local
        .run_until(async {
            ...
            tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    if let hystersis_acp_lib::AcpClientMessage::ExtMethod(args) = msg {
...
async fn only_a_completed_stop_hook_is_the_turns_report() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(
                &actor,
                hystersis_hooks::event::HookEventName::StopFailure,
                &["cb_fail"],
            );
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry(
                ...
                let reported = actor.claim_and_queue(
                    ...
                    super::turn_end_hooks::TurnEnd::Failed {
                        error: hystersis_hooks::event::StopFailureKind::Unknown,
                        error_details: None,
                        last_assistant_message: None,
                    },
                );
