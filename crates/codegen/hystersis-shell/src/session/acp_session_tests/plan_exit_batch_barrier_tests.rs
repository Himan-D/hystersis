...
use hystersis_tools::implementations::hystersis_build::exit_plan_mode::ExitPlanModeExtRequest;
...
async fn seeded_active_plan_actor_with_edit_tools() -> (
    ...
    *actor.agent.borrow_mut() = test_agent_with_tools(vec![
        ToolConfig::from_id("HystersisBuild:read_file"),
        ToolConfig {
            id: "HystersisBuild:search_replace".into(),
            params: Some(
    ...
    actor
        ...
        .borrow()
        .tool_bridge()
        .update_resource(hystersis_tools::types::resources::PlanFilePath(
            plan_path.clone(),
        ))
        .await;
...
fn spawn_exit_capture(
    mut gateway_rx: tokio::sync::mpsc::UnboundedReceiver<hystersis_acp_lib::AcpClientMessage>,
) -> (
        ...
        while let Some(msg) = gateway_rx.recv().await {
            match msg {
                hystersis_acp_lib::AcpClientMessage::ExtMethod(args) => {
                    if args.request.method.as_ref() == "hystersis.com/exit_plan_mode" {
...
async fn assert_mixed_batch_snapshot(write_first: bool) {
    ...
    let snapshot = captured
        ...
        .unwrap()
        .clone()
        .expect("gateway must receive hystersis.com/exit_plan_mode with plan content");
...
async fn mixed_permission_cancel_skips_exit_reverse_request() {
    ...
    local
        .run_until(async {
            ...
            use hystersis_tools::implementations::hystersis_build::exit_plan_mode::ExitPlanModeTool;
            use hystersis_tools::registry::types::ToolConfig;
            use hystersis_workspace::permission::{ClientType, spawn_permission_manager};
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                ToolConfig {
                    id: "HystersisBuild:run_terminal_cmd".into(),
                    params: Some(
                        serde_json::from_value(serde_json::json!({
            ...
            let responder = tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    match msg {
                        hystersis_acp_lib::AcpClientMessage::RequestPermission(args) => {
                            let _ = args
                                .response_tx
