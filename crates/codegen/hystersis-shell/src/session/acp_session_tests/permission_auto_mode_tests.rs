...
use hystersis_workspace::permission::{AccessKind, ClientType, spawn_permission_manager};
    ...
    local
        .run_until(async {
            ...
            let d = session
                .permissions
                .request(
                    AccessKind::Bash("cargo test -p xai-hystersis-workspace".into()),
                    dummy_update,
                    None,
                    None,
                    None,
            ...
            assert!(
                matches!(d, hystersis_workspace::permission::Decision::Allow),
            ...
            let d2 = session
                ...
                .await;
            assert!(
                !matches!(d2, hystersis_workspace::permission::Decision::Allow),
    ...
    local
        .run_until(async {
            ...
            actor.permissions.set_classifier_transcript(vec![
                hystersis_workspace::permission::ClassifierTurn::UserText("please run tests".into()),
            ]);
...
fn build_classifier_turns_captures_tool_use_excludes_text_and_results() {
    use hystersis_workspace::permission::ClassifierTurn;
    let conv = vec![
        ...
        super::ConversationItem::assistant_tool_calls(vec![
            hystersis_sampling_types::conversation::ToolCall {
...
fn build_classifier_turns_projects_full_filtered_resident_prefix() {
    use hystersis_sampling_types::synthesized_reasoning_item;
    use hystersis_workspace::permission::ClassifierTurn;
...
fn build_classifier_turns_filters_non_user_carriers() {
    ...
    let conv = vec![
        ...
        tool_image,
        user_image,
        super::ConversationItem::User(hystersis_sampling_types::UserItem {
