use super::support::*;
use super::*;
use hystersis_tools::reminders::task_completion::consumed_completion_ids;
use hystersis_tools::types::output::{BashOutput, TextOutput, ToolOutput};
...
async fn cancel_barrier_rejects_task_completion_wake_without_reporting_it() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                res.get::<hystersis_tools::types::resources::State<
                    hystersis_tools::reminders::task_completion::ReportedTaskCompletions,
                >>()
                .is_none(),
                "declined admission must not report before user re-engagement"
            );
            drop(res);
            let reminder = hystersis_tools::reminders::TaskCompletionReminder;
            let reminders = hystersis_tools::types::tool::Reminder::collect_reminders(
                &reminder,
                resources,
        ...
        .run_until(async {
            ...
            assert!(
                resources
                    .lock()
                    .await
                    .get::<hystersis_tools::types::resources::State<
                        hystersis_tools::reminders::task_completion::ReportedTaskCompletions,
                    >>()
                    .is_none(),
...
async fn genuine_user_start_consumes_deferred_completions_without_notification_turn() {
    ...
    local
        .run_until(async {
            ...
            let body = hystersis_tools::reminders::task_completion::format_monitor_completion(
                &hystersis_tools::types::TaskSnapshot {
                    ...
                    signal: None,
                    completed: true,
                    kind: hystersis_tools::computer::types::TaskKind::Monitor,
                    block_waited: false,
...
async fn already_reported(actor: &SessionActor, task_id: &str) -> bool {
    use hystersis_tools::reminders::task_completion::ReportedTaskCompletions;
...
struct OneTaskTerminal {
    tasks: Vec<hystersis_tools::computer::types::TaskSnapshot>,
}
#[async_trait::async_trait]
impl hystersis_tools::computer::types::TerminalBackend for OneTaskTerminal {
    async fn run(
        &self,
        _: hystersis_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        hystersis_tools::computer::types::TerminalRunResult,
        hystersis_tools::computer::types::ComputerError,
    > {
        unimplemented!()
    }
    ...
    async fn get_task(&self, _: &str) -> Option<hystersis_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn kill_task(&self, _: &str) -> hystersis_tools::computer::types::KillOutcome {
        hystersis_tools::computer::types::KillOutcome::NotFound
    }
    async fn wait_for_completion(
        &self,
        _: &str,
        _: Option<std::time::Duration>,
    ) -> Option<hystersis_tools::computer::types::TaskSnapshot> {
        None
    }
    async fn list_tasks(&self) -> Vec<hystersis_tools::computer::types::TaskSnapshot> {
        self.tasks.clone()
    }
}
fn completed_bash_task(id: &str) -> hystersis_tools::computer::types::TaskSnapshot {
    hystersis_tools::computer::types::TaskSnapshot {
        task_id: id.into(),
