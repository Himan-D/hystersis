...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> hystersis_computer_hub_sdk::ObservabilityBridge {
    hystersis_computer_hub_sdk::ObservabilityBridge::new(
        None,
        hystersis_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> hystersis_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<hystersis_sampling_types::HostedTool>,
) -> hystersis_agent::Agent {
    let base = test_agent_default().await;
    hystersis_agent::Agent::new(
        base.definition().clone(),
        hystersis_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        hystersis_agent::ReminderPolicy::default(),
        hystersis_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> hystersis_agent::Agent {
    use hystersis_tools::implementations::hystersis_build::update_goal::UpdateGoalTool;
    use hystersis_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// Hystersis-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_hystersis_build_agent_with_todo() -> hystersis_agent::Agent {
    use hystersis_tools::implementations::hystersis_build::todo::TodoWriteTool;
    use hystersis_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> hystersis_agent::Agent {
    use hystersis_tools::implementations::hystersis_build::enter_plan_mode::EnterPlanModeTool;
    use hystersis_tools::implementations::hystersis_build::exit_plan_mode::ExitPlanModeTool;
    use hystersis_tools::registry::types::ToolConfig;
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<hystersis_tools::registry::types::ToolConfig>,
) -> hystersis_agent::Agent {
    test_agent_from_config(
        hystersis_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        hystersis_agent::AgentDefinition::default_hystersis_build(),
        std::sync::Arc::new(hystersis_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_user_message_template(
    template: hystersis_agent::prompt::user_message::UserMessageTemplate,
) -> hystersis_agent::Agent {
    let mut definition = hystersis_agent::AgentDefinition::default_hystersis_build();
...
async fn test_agent_from_config(
    ...
    definition: hystersis_agent::AgentDefinition,
    backend: std::sync::Arc<dyn hystersis_tools::computer::types::TerminalBackend>,
) -> hystersis_agent::Agent {
    use hystersis_tools::computer::local::LocalFs;
    use hystersis_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("hystersis-test"),
...
pub(crate) async fn create_test_actor_ex(
    ...
    gateway_tx: tokio::sync::mpsc::UnboundedSender<hystersis_acp_lib::AcpClientMessage>,
...
pub(crate) async fn create_test_actor_with_terminal(
    ...
    let cwd = hystersis_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
    let fs = Arc::new(hystersis_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    let hunk_tracker_handle = hystersis_hunk_tracker::HunkTrackerActor::spawn(
        ...
        cwd.to_path_buf(),
        hunk_tx,
        hystersis_hunk_tracker::TrackingMode::AgentOnly,
        tokio_util::sync::CancellationToken::new(),
    );
    let mut tool_context =
        ToolContext::new(cwd.clone(), None, None, fs, terminal, hunk_tracker_handle);
    tool_context.task_completion_reservations =
        Some(hystersis_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(hystersis_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let chat_state_handle = hystersis_chat_state::ChatStateActor::spawn(
        vec![],
        hystersis_sampling_types::SamplingConfig {
        ...
        Box::new(hystersis_chat_state::NullChatPersistence),
        chat_event_tx,
        tokio_util::sync::CancellationToken::new(),
    );
    ...
    let actor = SessionActor {
        ...
        permissions: hystersis_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
        ...
        compaction: crate::session::compaction_config::CompactionConfig {
            ...
            compaction_mode: hystersis_chat_state::CompactionMode::Transcript,
            verbatim_input: true,
        ...
        extension_registry: hystersis_agent_lifecycle::LocalExtensionRegistry::default(),
        ...
        vcs_kind: hystersis_workspace::session::git::VcsKind::Git,
        ...
        sampler_handle: hystersis_sampler::SamplerHandle::noop(),
        sampling_gate: None,
...
pub(crate) async fn build_actor() -> (
    ...
    let (gateway_tx, gateway_rx) =
        tokio::sync::mpsc::unbounded_channel::<hystersis_acp_lib::AcpClientMessage>();
