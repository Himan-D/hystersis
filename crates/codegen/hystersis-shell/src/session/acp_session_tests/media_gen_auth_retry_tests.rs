...
use hystersis_tools::types::output::{ToolOutput, ToolRunResult};
...
fn ok_result(text: &str) -> Result<ToolRunResult, hystersis_tool_runtime::ToolError> {
    Ok(ToolRunResult {
...
fn err(msg: &str) -> Result<ToolRunResult, hystersis_tool_runtime::ToolError> {
...
fn http_err(status: u16, msg: &str) -> Result<ToolRunResult, hystersis_tool_runtime::ToolError> {
    Err(
        hystersis_tool_runtime::ToolError::new(hystersis_tool_runtime::ToolErrorKind::Custom, msg.to_owned())
            .with_details(
...
fn is_auth_tool_error_classification() {
    ...
    let cases: Vec<(bool, hystersis_tool_runtime::ToolError)> = vec![
        ...
        // the inference path's gate in xai-hystersis-sampling-types/src/error.rs:
        ...
        (
            true,
            hystersis_tool_runtime::ToolError::invalid_arguments("response: invalid api key for project"),
        ),
        // Fallback path: OAuth 2.0 `invalid_token` payload (RFC 6749)
        // surfaced as raw JSON without a structured status code.
        (
            true,
            hystersis_tool_runtime::ToolError::invalid_arguments(r#"{"error":"invalid_token"}"#),
        ),
        // Fallback path: case-insensitive "unauthorized" anywhere in
        // the message body.
        (
            true,
            hystersis_tool_runtime::ToolError::invalid_arguments("UNAUTHORIZED"),
        ),
        // Negative: transport failure must not trigger a token refresh.
        (
            false,
            hystersis_tool_runtime::ToolError::invalid_arguments("Image generation timed out after 60s"),
        ),
        // Negative: structural not-found error; not a network response.
        (
            false,
            hystersis_tool_runtime::ToolError::not_found(
                hystersis_tool_protocol::ToolId::new("image_gen").expect("valid"),
        ...
        (
            false,
            hystersis_tool_runtime::ToolError::invalid_arguments("request id req_401abc failed"),
        ),
    ];
