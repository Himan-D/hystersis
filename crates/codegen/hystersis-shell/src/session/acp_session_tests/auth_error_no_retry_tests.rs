...
/// attribution.
fn auth_error() -> hystersis_sampler::SamplingErrorInfo {
    hystersis_sampler::SamplingErrorInfo {
        kind: hystersis_sampler::SamplingErrorKind::Auth,
...
async fn make_actor_with_auth_and_credentials(
    auth_manager: Option<Arc<AuthManager>>,
    auth_type: hystersis_chat_state::AuthType,
    api_key: String,
) -> (Arc<SessionActor>, mpsc::UnboundedReceiver<PersistenceMsg>) {
    let method_id = match auth_type {
        hystersis_chat_state::AuthType::SessionToken => "cached_token",
        hystersis_chat_state::AuthType::ApiKey => "xai.api_key",
    };
...
async fn make_actor_with_method_and_credentials(
    ...
    actor
        .chat_state_handle
        .update_credentials(hystersis_chat_state::Credentials {
...
fn model_not_found_error() -> hystersis_sampler::SamplingErrorInfo {
    hystersis_sampler::SamplingErrorInfo {
            kind: hystersis_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model hystersis does not exist or your team does not have access".into(),
...
async fn legacy_auth_hint_on_404_model_not_found() {
    ...
    local
        .run_until(async {
            ...
            assert!(
                msg.contains("hystersis update"),
                "hint must mention `hystersis update` before re-login, got: {msg}"
            );
            assert!(
                msg.contains("hystersis logout"),
                "hint must mention `hystersis logout`, got: {msg}"
            );
            assert!(
                msg.contains("hystersis login"),
                "hint must mention `hystersis login`, got: {msg}"
            );
            let update_at = msg.find("hystersis update").expect("hystersis update");
            let logout_at = msg.find("hystersis logout").expect("hystersis logout");
            assert!(
                update_at < logout_at,
...
fn unauthorized_401_error() -> hystersis_sampler::SamplingErrorInfo {
    hystersis_sampler::SamplingErrorInfo {
            kind: hystersis_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.hystersis.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_hystersis_token_auth=xai-grok-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            should_retry: None,
...
async fn set_session_model_invalidates_byok_memo_for_same_model_id() {
    ...
    local
        .run_until(async {
            ...
            let cfg = hystersis_sampler::SamplerConfig {
...
/// never ride a later request to `api.hystersis.com`. Mirrors the forward direction in
...
async fn switch_to_first_party_model_drops_minted_provider_token() {
    ...
    local
        .run_until(async {
            ...
            let (actor, _rx) =
                make_actor_with_auth_and_credentials(None, hystersis_chat_state::AuthType::ApiKey, token)
                    .await;
...
async fn sampler_non_auth_kind_401_on_provider_model_still_recovers() {
    ...
    local
        .run_until(async {
            ...
            error.kind = hystersis_sampler::SamplingErrorKind::Api;
