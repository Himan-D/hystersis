...
//! content blocks, the session calls a vision-capable Hystersis model
...
use hystersis_chat_state::compaction_utils::{extract_real_user_queries, extract_user_query};
use hystersis_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
use hystersis_tools::util::truncate::truncate_middle;
...
impl ImageDescribeCache {
    ...
    pub(crate) async fn get_or_describe(
        &self,
        client: hystersis_sampler::SamplingClient,
        model: &str,
        raw_bytes: &[u8],
        mime_type: &str,
        outline: Option<&str>,
...
pub(crate) fn persist_user_images(
    ...
    crate::util::hystersis_home::create_dir_all_owner_only(&assets_dir)?;
...
mod tests {
    use super::*;
    use hystersis_sampling_types::conversation::{ConversationItem, UserItem};
    #[test]
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![hystersis_sampling_types::conversation::ContentPart::Text {
                text: text.into(),
            }],
            synthetic_reason: None,
            ..Default::default()
