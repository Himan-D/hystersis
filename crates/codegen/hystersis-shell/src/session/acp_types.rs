...
impl TokenUsageCategory {
    ...
    pub fn skills_listing(text: &str, skill_count: usize) -> Self {
        Self {
            ...
            tokens: hystersis_token_estimation::estimate_tokens(text),
...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for hystersis in remote settings).
...
impl ContextInfo {
    ...
    pub fn from_notification(used: u64, total: u64) -> Self {
        Self {
            used,
            total,
            usage_pct: hystersis_token_estimation::usage_percentage_u8(used, total),
            free_tokens: hystersis_token_estimation::free_tokens(total, used),
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `hystersis`).
...
pub(crate) fn is_coding_model_slug(model: &str) -> bool {
    matches!(model, "hystersis" | "hystersis-4.5")
}
...
/// Full wire response for `hystersis.com/session/info`.
///
...
impl StartupHints {
    ...
    pub(crate) fn resolve_mcp_strategy(&self) -> hystersis_telemetry::enums::McpInitStrategy {
        use hystersis_telemetry::enums::McpInitStrategy;
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "hystersis"));
        assert!(should_show_model_fingerprint(false, "hystersis-4.5"));
    ...
    fn desktop_client_type_deserializes_and_round_trips() {
        ...
        let submission = input.take_submission(Some("hystersis-3".into()), None, None, Some(5));
        assert_eq!(
