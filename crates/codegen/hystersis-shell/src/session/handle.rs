...
use hystersis_file_utils::queue::UploadQueue;
...
pub const SCHEDULER_BACKGROUND_LOOPS_META_KEY: &str = "hystersis.com/schedulerBackgroundLoops";
...
pub struct SessionHandle {
    ...
    pub chat_state_handle: hystersis_chat_state::ChatStateHandle,
    ...
    /// `hystersis.com/codeNavigation.enabled`.  Stored per-session so that in leader
    ...
    /// Exposed so the `hystersis.com/toggle_plan_mode` handler can toggle plan mode
    ...
    /// Set via `hystersis.com/debug/arm_auto_compact`.
    pub force_compact: std::sync::Arc<std::sync::atomic::AtomicBool>,
    pub permission_handle: hystersis_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn hystersis_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    pub(crate) async fn get_model_metadata(&self) -> hystersis_chat_state::ModelMetadata {
            ...
            .is_ok()
        {
            ...
            hystersis_chat_state::ModelMetadata::default()
        }
    }
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<hystersis_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<hystersis_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
