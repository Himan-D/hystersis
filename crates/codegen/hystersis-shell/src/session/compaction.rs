...
use std::sync::Arc;
use hystersis_chat_state::compaction_utils::{
...
use hystersis_sampling_types::{ApiBackend, ConversationItem};
...
/// Override with `HYSTERSIS_PREFIRE_LEAD_PERCENT`.
const DEFAULT_PREFIRE_LEAD_PERCENT: u64 = 10;
fn prefire_lead_percent() -> u64 {
    std::env::var("HYSTERSIS_PREFIRE_LEAD_PERCENT")
        .ok()
...
impl SessionActor {
    ...
    async fn two_pass_sample(&self, history: Vec<ConversationItem>) -> Option<CompactOutput> {
        ...
        let compaction_tool_tokens = hystersis_chat_state::estimate_tool_specs_tokens(&tools);
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        hystersis_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("HYSTERSIS_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target: "two_pass",
                "two_pass: DEBUG HYSTERSIS_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(hystersis_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
impl SessionActor {
    ...
    async fn maybe_pre_compaction_flush(
        ...
        if hystersis_memory::flush::should_flush(
            total_tokens,
            context_window,
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                hystersis_telemetry::events::CompactionTrigger::Manual,
                false,
            )
            .await
        {
        ...
        self.send_hystersis_notification(XaiSessionUpdate::AutoCompactCompleted {
    ...
    async fn emit_compact_cancelled(&self, auto_trigger: bool) -> Result<(), acp::Error> {
        if auto_trigger {
            use crate::extensions::notification::SessionUpdate as XaiSessionUpdate;
            self.send_hystersis_notification(XaiSessionUpdate::AutoCompactCancelled {
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            hystersis_telemetry::session_ctx::log_event(
                hystersis_telemetry::events::AutoCompactSuppressed {
            ...
            self.send_hystersis_notification(
    ...
    pub(crate) async fn surface_compact_auth_failure(&self, err: acp::Error) -> acp::Error {
        ...
        acp::Error::auth_required().data(crate::sampling::error::terminal_error_data(
            message,
            Some(401),
            hystersis_sampler::SamplingErrorKind::Auth,
        ))
    }
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    hystersis_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    hystersis_chat_state::estimate_conversation_tokens(&full_conv),
                );
                if hystersis_token_estimation::exceeds_threshold(
                    projected_preserved,
                    context_window,
                    self.compaction.threshold_percent.get(),
                ) {
    ...
    async fn run_compact_inner(
        ...
        trigger: hystersis_telemetry::events::CompactionTrigger,
        lossy_input: bool,
    ) -> Result<(), acp::Error> {
        let (cancel, _cancel_scope) = self.compaction.cancel.enter();
        ...
        let trigger_str = match trigger {
            hystersis_telemetry::events::CompactionTrigger::Manual => "manual",
            hystersis_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = hystersis_telemetry::events::CompactionScope::begin(
            hystersis_telemetry::events::CompactionBeginParams {
                trigger,
                ...
                compaction_mode: match self.compaction.compaction_mode {
                    hystersis_chat_state::CompactionMode::Summary => {
                        hystersis_telemetry::events::CompactionModeLabel::Summary
                    }
                    hystersis_chat_state::CompactionMode::Transcript => {
                        hystersis_telemetry::events::CompactionModeLabel::Transcript
                    }
                    hystersis_chat_state::CompactionMode::Segments(_) => {
                        hystersis_telemetry::events::CompactionModeLabel::Segments
                    }
                },
        ...
        self.dispatch_hook(
            hystersis_hooks::event::HookEventName::PreCompact,
            hystersis_hooks::event::HookPayload::PreCompact {
        ...
        let segment_messages = if self.compaction.compaction_mode.writes_segments() {
            hystersis_chat_state::compaction_utils::prepare_conversation_for_segment(
        ...
        let mut simplified_messages = if verbatim_input_enabled {
            hystersis_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
                summary_strips_reasoning,
            )
        } else {
            hystersis_chat_state::compaction_utils::prepare_conversation_for_summarization(
                full_conversation,
            )
        };
        ...
        let effective_tool_defs: Vec<hystersis_sampling_types::ToolDefinition> = self
        ...
        let compaction_tool_tokens =
            hystersis_chat_state::estimate_tool_definitions_tokens(&effective_tool_defs);
        let compaction_tools: Vec<hystersis_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(hystersis_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<hystersis_sampling_types::HostedTool> =
            self.hosted_tools_for_turn();
        if lossy_input {
            simplified_messages = hystersis_chat_state::compaction_utils::fit_conversation_to_budget(
                simplified_messages,
        ...
        let estimated_input_tokens =
            hystersis_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, hystersis_telemetry::events::CompactionTrigger::Auto);
        let wall_clock_budget_secs = self
            .agent
            .borrow()
            .compaction_policy()
        ...
        let fr_config = hystersis_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match hystersis_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(hystersis_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(hystersis_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(hystersis_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    ...
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            hystersis_telemetry::session_ctx::log_event(
                                hystersis_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = hystersis_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    hystersis_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim,
                                        budget,
                                    )
                                }
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                hystersis_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                hystersis_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use hystersis_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn hystersis_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn hystersis_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        hystersis_sampling_types::ContentPart::Text { text } => {
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: hystersis_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        self.dispatch_hook(
            hystersis_hooks::event::HookEventName::PostCompact,
            hystersis_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
        ...
        compaction.complete(
            hystersis_telemetry::events::CompactionCompleteStats {
                tokens_after,
                two_pass_used,
                segments_queued,
            ...
            hystersis_telemetry::events::CompactionTiming {
    ...
    pub(crate) fn should_auto_compact(
        ...
        if hystersis_token_estimation::exceeds_threshold(
            ...
            let percentage = hystersis_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &hystersis_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = hystersis_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::AutoCompactFired {
            tokens_before: trigger_info.tokens_used,
            percentage: trigger_info.percentage,
        });
        self.signals_handle()
            .record_compaction(trigger_info.tokens_used);
        self.send_hystersis_notification(XaiSessionUpdate::AutoCompactStarted {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                hystersis_telemetry::events::CompactionTrigger::Auto,
                lossy_input,
            )
            .await;
            ...
            Err(e) => {
                ...
                if !cancelled
                    && self
                        ...
                        == SUPPRESS_NONE
                {
                    self.send_hystersis_notification(XaiSessionUpdate::AutoCompactFailed {
                        error: String::new(),
                    })
                    .await;
                }
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<hystersis_sampling_types::ToolSpec>,
        user_context: Option<&str>,
    ...
    fn persist_compaction_checkpoint(
        ...
        self.persist_hystersis_update_only(XaiSessionUpdate::CompactionCheckpoint(Box::new(info)));
        tracing::info!(
            prompt_index_at_compaction,
