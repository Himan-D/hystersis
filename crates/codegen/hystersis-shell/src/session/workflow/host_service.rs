...
use hystersis_workflow::{AgentOpts, AgentResult, BudgetState, HostError, WorkflowHostRequest};
...
pub(crate) const WORKFLOW_MAX_AGENT_RUNS: u32 =
    (hystersis_workflow::MAX_AGENT_BUDGET as u32) * (SCHEMA_CONTRACT_RETRIES + 1);
...
pub(crate) struct WorkflowHostParams {
    ...
    pub effort: Option<hystersis_sampling_types::ReasoningEffort>,
...
impl HostService {
        ...
        match self.agent_slots.try_acquire() {
            ...
            Err(tokio::sync::TryAcquireError::NoPermits) => {
                ...
                hystersis_telemetry::session_ctx::log_event(
                    hystersis_telemetry::events::SubagentLimitHit::workflow_run_concurrent(
    ...
    async fn spawn_agent(&self, mut opts: AgentOpts) -> Result<AgentResult, HostError> {
        ...
        let reasoning_effort = opts
            ...
            .map(|effort| {
                effort
                    .parse::<hystersis_sampling_types::ReasoningEffort>()
                    .map_err(|error| {
        ...
        let isolation = opts
            .isolation_worktree
            .then_some(hystersis_tool_types::SubagentIsolationMode::Worktree);
    ...
    async fn git_diff_since(&self, commit: &str) -> Result<String, HostError> {
        ...
        hystersis_tty_utils::detach_command(&mut cmd);
...
mod tests {
    ...
    async fn a_run_keeps_at_most_max_concurrent_agents_live() {
        ...
        let succeed = |spawn: hystersis_tools::implementations::hystersis_build::task::types::SubagentSpawnRequest| {
            spawn
                .respond_with(|request| {
                    hystersis_tools::implementations::hystersis_build::task::types::SubagentResult {
                        success: true,
