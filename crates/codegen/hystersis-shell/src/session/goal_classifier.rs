...
use hystersis_tools::implementations::hystersis_build::task::backend::{ChannelBackend, SubagentBackend};
...
/// `HYSTERSIS_GOAL_CLASSIFIER_MAX` or remote `goal_classifier_max_runs` to
/// raise it arbitrarily (only the `GOAL_CLASSIFIER_MAX_RUNS_MIN` floor
/// is enforced).
pub(crate) const GOAL_CLASSIFIER_MAX_RUNS_DEFAULT: u32 = 10;

/// Floor for `HYSTERSIS_GOAL_CLASSIFIER_MAX` / remote `goal_classifier_max_runs`.
...
/// attempt. Override via `HYSTERSIS_GOAL_VERIFIER_N` (clamped 1..=5) or the
...
/// Lower/upper bounds for `HYSTERSIS_GOAL_VERIFIER_N` / remote
...
/// (via [`hystersis_tool_types::format_resume_footer`]): trace tooling
...
pub(crate) fn build_subagent_trace_items(
    ...
    prompt: &str,
    output: &str,
) -> Vec<hystersis_sampling_types::conversation::ConversationItem> {
    use hystersis_sampling_types::conversation::{ConversationItem, ToolCall};
    ...
    let footer = hystersis_tool_types::format_resume_footer(subagent_id, subagent_type, None);
...
/// the phase via [`hystersis_chat_state::ChatStateHandle::flush_harness_trace_turn`]
...
pub(crate) fn record_subagent_trace(
    sink: Option<&(hystersis_chat_state::ChatStateHandle, String)>,
    subagent_id: &str,
    subagent_type: &str,
    description: &str,
    prompt: Option<&str>,
...
pub(crate) struct ChannelSpawner {
    ...
    pub(crate) foreground_wait:
        Option<hystersis_tools::implementations::hystersis_build::task::types::SubagentForegroundWait>,
    ...
    pub(crate) trace_sink: Option<(hystersis_chat_state::ChatStateHandle, String)>,
