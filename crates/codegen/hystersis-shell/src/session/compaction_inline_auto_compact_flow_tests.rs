...
use hystersis_workspace::file_system::MockFs;
use hystersis_workspace::permission::PermissionHandle;
#[derive(Debug)]
...
async fn create_test_actor(
    ...
    gateway_tx: mpsc::UnboundedSender<hystersis_acp_lib::AcpClientMessage>,
    ...
    local
        .run_until(async {
            let (gateway_tx, _gateway_rx) =
                mpsc::unbounded_channel::<hystersis_acp_lib::AcpClientMessage>();
...
fn switch_target_config(model: &str, base_url: String) -> hystersis_sampler::SamplerConfig {
    hystersis_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.replace_conversation(vec![
                ...
                ConversationItem::Reasoning(hystersis_sampling_types::rs::ReasoningItem {
                ...
                ConversationItem::assistant_tool_calls(vec![hystersis_sampling_types::ToolCall {
                    id: std::sync::Arc::<str>::from("call_hystersis_minted_id"),
                    name: "run_terminal_command".to_string(),
                    arguments: std::sync::Arc::<str>::from(r#"{"command":"ls"}"#),
                }]),
                ConversationItem::ToolResult(hystersis_sampling_types::ToolResultItem {
                    tool_call_id: "call_hystersis_minted_id".to_string(),
...
fn classify_suppress_reason_maps_error_text() {
    ...
    assert_eq!(
        classify("API error (status 402 Payment Required): Hystersis usage balance exhausted"),
        SuppressReason::CreditBlock
    );
    assert_eq!(
        classify("Hystersis usage limit reached"),
        SuppressReason::CreditBlock
    );
    assert_eq!(
...
fn api_error_with_context_window(context_window: u64) -> hystersis_sampler::SamplingErrorInfo {
    hystersis_sampler::SamplingErrorInfo {
        kind: hystersis_sampler::SamplingErrorKind::Api,
...
async fn test_compact_on_error_triggers_when_tokens_exceed_new_window() {
    ...
    local
        .run_until(async {
            let (gateway_tx, _) = mpsc::unbounded_channel::<hystersis_acp_lib::AcpClientMessage>();
...
async fn test_compact_on_error_noop_without_model_metadata() {
    ...
    local
        .run_until(async {
            ...
            let err = hystersis_sampler::SamplingErrorInfo {
                kind: hystersis_sampler::SamplingErrorKind::Api,
                status_code: Some(400),
                message: "prompt is too long".to_string(),
                is_retryable: false,
...
async fn get_transcript_path_returns_some_when_file_exists() {
    ...
    local
        .run_until(async {
            ...
            actor.compaction.compaction_mode = hystersis_chat_state::CompactionMode::Transcript;
            ...
            actor.compaction.compaction_mode = hystersis_chat_state::CompactionMode::Summary;
