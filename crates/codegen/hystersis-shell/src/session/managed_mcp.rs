...
//! `hystersis_shell_session_support::managed_mcp` and is re-exported here so
//! `crate::session::managed_mcp::…` paths keep resolving unchanged.

pub use hystersis_shell_session_support::managed_mcp::*;
...
impl McpDisabledReason {
    ...
    pub(crate) fn for_blocked_server(
        policy: &hystersis_workspace::permission::resolution::McpServerAllowlist,
...
fn apply_mcp_server_policy(
    ...
    allowlist: &hystersis_workspace::permission::resolution::McpServerAllowlist,
) -> Vec<McpServerWithPolicy> {
    merged
        .into_iter()
        .filter_map(|server| {
...
pub(crate) fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: hystersis_tools::util::hystersis_home::hystersis_home().join("config.toml"),
    };
...
pub(crate) struct McpDiscoveryInputs<'a> {
    ...
    pub plugin_registry: Option<&'a hystersis_agent::plugins::PluginRegistry>,
    pub compat: &'a hystersis_tools::types::compat::CompatConfig,
}
...
fn load_plugin_mcp_servers_from_value(
    ...
    let normalized = hystersis_agent::plugins::manifest::normalize_inline_mcp_servers(root);
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = hystersis_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
"#,
    ...
    fn merge_drops_denied_server_and_classifies_as_denylist() {
        use hystersis_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
...
"#,
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use hystersis_agent::plugins::PluginRegistry;
        use hystersis_agent::plugins::PluginScope;
        use hystersis_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use hystersis_agent::plugins::manifest::{PathOrInline, PluginManifest};
        ...
        let dp = DiscoveredPlugin {
            ...
            origin: hystersis_agent::plugins::PluginOrigin::UserHystersis,
            trusted: true,
            skill_dirs: vec![],
            command_dirs: vec![],
            agent_dirs: vec![],
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use hystersis_agent::plugins::PluginScope;
        use hystersis_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use hystersis_agent::plugins::manifest::PluginManifest;
