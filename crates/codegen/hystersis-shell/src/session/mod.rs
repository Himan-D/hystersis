...
pub use hystersis_fsnotify::{FsConfig, FsEvent, FsEventKind, FsEventSource, FsNotifyError, GitMetaKind};
...
pub(crate) fn is_cursor_user_template(
    _template: &hystersis_agent::prompt::user_message::UserMessageTemplate,
) -> bool {
    false
}
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub(crate) struct CompactionPins {
    pub mode: hystersis_chat_state::CompactionMode,
    pub two_pass: bool,
}
pub(crate) fn cursor_compaction_pins(
    ...
    if is_cursor {
        CompactionPins {
            mode: hystersis_chat_state::CompactionMode::Summary,
            two_pass: false,
        }
    } else {
        CompactionPins {
...
pub(crate) fn is_cursor_system_template(
    _template: &hystersis_agent::prompt::context::TemplateOverride,
) -> bool {
    false
}
...
pub use hystersis_agent_lifecycle::{
...
/// Client-requested fs notification mode (was hystersis_fsnotify::FsNotifyMode).
...
pub use hystersis_shared::session::info;
pub mod managed_mcp;
...
pub mod persistence;
pub use hystersis_shared::placeholder_images;
pub mod plan_mode;
