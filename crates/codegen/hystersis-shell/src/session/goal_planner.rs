...
///
/// [`SubagentRuntimeOverrides::harness_agent_type`]: hystersis_tools::implementations::hystersis_build::task::types::SubagentRuntimeOverrides::harness_agent_type
...
pub(crate) struct RoleSpawnOverride {
    ...
    /// Resolved harness `agent_type` (e.g. `"hystersis-plan"`)
...
mod tests {
    ...
    async fn fail_open_retry_emits_spawn_failed_event() {
        ...
        let writer = hystersis_session_events::EventWriter::open(dir.path());
        let ov = RoleSpawnOverride {
            model: Some("m".into()),
            agent_type: Some("t".into()),
        };
