...
/// Env lane (desktop `HYSTERSIS_SESSION_LIST_CONVERSATIONS`) OR process-wide
/// `--chat` (`HYSTERSIS_CHAT_MODE`); hard-off in release builds.
...
/// Parse `hystersis.com/session/list` params and, under process-wide chat mode, force
...
fn client_sent_kind_filter(req: &ListReq) -> bool {
    let Some(kind) = req
        .meta
        .as_ref()
        .and_then(|m| m.get("hystersis.com/facetFilters"))
...
pub struct ListReq {
    ...
    /// A relaxed response sets `_meta["hystersis.com/listScope"]`, re-evaluated per page.
    #[serde(
        default,
        rename = "allowRelax",
...
/// `as_str` value (`hystersis.com/listScope`), so no serde derive is needed.
...
impl ParsedMeta {
    fn parse(meta: Option<&serde_json::Value>) -> Self {
        ...
        let facet_filters = meta
            .get("hystersis.com/facetFilters")
            ...
            .unwrap_or_default();
        let query = meta
            .get("hystersis.com/query")
            .and_then(|v| v.as_str())
            .map(str::to_owned);
        let limit = meta
            .get("hystersis.com/limit")
...
pub(crate) fn force_kind(req: &mut ListReq, kind: SessionKind) {
    ...
    let mut filters = match meta.remove("hystersis.com/facetFilters") {
        Some(serde_json::Value::Object(map)) => map,
        _ => serde_json::Map::new(),
    };
    filters.insert(
    ...
    meta.insert(
        "hystersis.com/facetFilters".to_owned(),
...
pub(crate) struct ExtListResponseMeta {
    #[serde(rename = "hystersis.com/facets")]
    pub facets: FacetSummary,
    #[serde(rename = "hystersis.com/partial")]
    pub partial: PartialInfo,
    /// Present only when the listing relaxed beyond the cwd.
    #[serde(rename = "hystersis.com/listScope", skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["hystersis.com/session"]["kind"], "build");
    ...
    fn parsed_meta_reads_facet_filters_query_and_limit() {
        let meta = serde_json::json!({
            "hystersis.com/facetFilters": { "kind": ["build"], "starred": true },
            "hystersis.com/query": "antelope",
            "hystersis.com/limit": 5,
        });
    ...
    fn forced_kind_replaces_client_build_filter() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({
                "hystersis.com/facetFilters": { "kind": ["build"] },
            })),
            ..ListReq::default()
        };
        force_kind_chat(&mut req);
    ...
    fn forced_kind_preserves_other_facets() {
        let mut req = ListReq {
            meta: Some(serde_json::json!({
                "hystersis.com/facetFilters": { "kind": ["build"], "starred": [true], "workspace": ["w1"] },
                "hystersis.com/query": "antelope",
                "hystersis.com/limit": 5,
            })),
            ..ListReq::default()
    ...
    fn hystersis_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
        let am = std::sync::Arc::new(crate::auth::AuthManager::new(
            dir,
            crate::auth::HystersisComConfig::default(),
        ));
        am.hot_swap(crate::auth::HystersisAuth {
            auth_mode: crate::auth::AuthMode::Oidc,
            oidc_issuer: Some(crate::auth::hystersis_oauth2_issuer().to_owned()),
    ...
    async fn forced_kind_serves_conversations_only() {
        let addr = spawn_conversations_stub(
            ...
            .await;
        let _env = hystersis_test_support::EnvGuard::set(
            "HYSTERSIS_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
        let home = tempfile::tempdir().expect("tempdir");
        let client = ConversationsClient::new(hystersis_auth_manager(home.path()));
        let mut req = ListReq {
            meta: Some(serde_json::json!({
                "hystersis.com/facetFilters": { "kind": ["build"] },
            })),
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = hystersis_test_support::EnvGuard::unset("HYSTERSIS_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = hystersis_test_support::EnvGuard::set("HYSTERSIS_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = hystersis_test_support::EnvGuard::set("HYSTERSIS_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::HYSTERSIS_CHAT_MODE_ENV;
        let _chat_off = hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_MODE_ENV);
        let _desktop_off =
            hystersis_test_support::EnvGuard::unset("HYSTERSIS_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _desktop =
                hystersis_test_support::EnvGuard::set("HYSTERSIS_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = hystersis_test_support::EnvGuard::set(HYSTERSIS_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        let raw = serde_json::json!({
            "_meta": { "hystersis.com/facetFilters": { "kind": ["build"], "starred": [true] } },
        })
        .to_string();
        {
            let _off = hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_MODE_ENV);
        ...
        {
            let _on = hystersis_test_support::EnvGuard::set(HYSTERSIS_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
            ...
            for bad in [
                serde_json::json!({ "_meta": { "hystersis.com/facetFilters": { "kind": [] } } }),
                serde_json::json!({ "_meta": { "hystersis.com/facetFilters": { "kind": null } } }),
                serde_json::json!({ "_meta": { "hystersis.com/facetFilters": { "kind": ["other"] } } }),
            ] {
    ...
    /// Wire pin for the cross-crate `hystersis.com/partial` envelope the pager parses:
    ...
    fn ext_list_response_serializes_partial_reasons() {
        for (reason, wire) in [
            ...
            .expect("serialize");
            assert_eq!(
                value["_meta"]["hystersis.com/partial"],
        ...
        .expect("serialize");
        assert_eq!(
            healthy["_meta"]["hystersis.com/partial"],
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = hystersis_workspace::session::git::normalize_repo_url(this_repo).unwrap();
    ...
    /// Send-side wire pin: `hystersis.com/listScope` present iff the scope relaxed.
    #[test]
    fn ext_list_response_serializes_scope() {
        ...
        assert_eq!(with["_meta"]["hystersis.com/listScope"], serde_json::json!("repo"));
        let without =
            serde_json::to_value(ext_list_response(result(ListScope::Cwd))).expect("serialize");
        assert!(without["_meta"].get("hystersis.com/listScope").is_none());
    }
}
