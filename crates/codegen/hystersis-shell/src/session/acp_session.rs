...
use hystersis_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use hystersis_agent::prompt::skills::SkillsConfig;
use hystersis_sampler::SamplerConfig as SamplingConfig;
use hystersis_sampling_types::truncate_bytes;
...
use hystersis_tools::types::compat::CompatConfig;
use hystersis_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use hystersis_workspace::file_system::CodebaseIndexManager;
use hystersis_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use hystersis_workspace::session::file_state::{FileStateHandle, FileStateTracker};
const SESSION_LOG: &str = "hystersis_session";
#[path = "compaction.rs"]
mod compaction;
...
pub(crate) use spawn::*;
/// Client-registered hook gates (the `hystersis.com/hooks/run` reverse request).
mod hooks;
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<hystersis_sampling_types::ToolOverridesUpdate>,
...
impl hystersis_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        hystersis_tools::types::resources::ManagedGatewayToolCallResponse,
        hystersis_tool_runtime::ToolError,
    > {
        let auth_key = self
            .auth_manager
            .get_valid_token()
            ...
            .ok_or_else(|| hystersis_tool_runtime::ToolError::unauthorized("no auth token available"))?;
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                hystersis_tool_runtime::ToolError::unauthorized(detail)
            } else if status == reqwest::StatusCode::FORBIDDEN {
                hystersis_tool_runtime::ToolError::permission_denied(detail)
            } else {
                let tool_id = hystersis_tool_protocol::ToolId::new(caller)
                    .unwrap_or_else(|_| hystersis_tool_protocol::ToolId::new("use_tool").expect("valid"));
                hystersis_tool_runtime::ToolError::execution(tool_id, detail)
            };
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            hystersis_tool_runtime::ToolError::network_error(format!(
                "Managed MCP gateway tool call failed: {}",
                e.without_url()
            ))
        }
        crate::session::managed_mcp::ManagedMcpFetchError::NoAuth => {
            hystersis_tool_runtime::ToolError::unauthorized("no auth token available")
        }
    }
}
...
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, hystersis_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn forbidden_status_maps_to_permission_denied_and_carries_status() {
        ...
        assert_eq!(err.kind, hystersis_tool_runtime::ToolErrorKind::PermissionDenied);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, hystersis_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, hystersis_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// `xai-hystersis-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<hystersis_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: hystersis_chat_state::ChatStateHandle,
    ...
    pub(crate) tool_overrides: std::cell::RefCell<Option<hystersis_sampling_types::ToolOverrides>>,
    ...
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<hystersis_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<hystersis_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<hystersis_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-hystersis-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<hystersis_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<hystersis_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) agent: std::cell::RefCell<hystersis_agent::Agent>,
    /// Dedup slot for `hystersis.com/git_head_changed`, shared with the fs-watch
    /// `GitHead` consumer (see `git_head_dedup_key`).
    pub(crate) last_reported_branch: Arc<parking_lot::Mutex<Option<String>>>,
    /// Client opted into `hystersis.com/gitHeadChanged`. When false (headless/SDK),
    /// `maybe_notify_git_branch` no-ops — no git subprocess.
    git_head_enabled: bool,
    /// A client that will draw a status row has attached (`hystersis.com/statusLine`).
    ...
    /// Kill-switch (`HYSTERSIS_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<hystersis_tools::implementations::hystersis_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) extension_registry: hystersis_agent_lifecycle::LocalExtensionRegistry,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<hystersis_hooks::discovery::HookRegistry>>>,
    ...
    /// Client hooks from `session/new` `_meta["hystersis.com/hooks"]`; gated in
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<hystersis_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<hystersis_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) observability_bridge: hystersis_computer_hub_sdk::ObservabilityBridge,
    ...
    pub(crate) pending_image_strip:
        parking_lot::Mutex<Option<(hystersis_sampler::RequestId, Vec<std::sync::Arc<str>>)>>,
    /// Handle to the per-session `xai-hystersis-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: hystersis_sampler::SamplerHandle,
    ...
    /// Cached recipe for constructing this session's [`hystersis_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`hystersis_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: hystersis_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    /// Fire-and-forget — failures are logged but do not interrupt the turn.
    async fn send_after_turn_event(&self, payload: hystersis_tool_protocol::turn_hook::AfterTurnPayload) {
        self.workspace_ops
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == hystersis_tools::implementations::hystersis_build::SCHEDULER_CREATE_TOOL_NAME
            }),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == hystersis_tools::implementations::hystersis_build::workflow::WORKFLOW_TOOL_NAME
            }),
    ...
    async fn send_feedback_notification(&self, request: crate::session::feedback::FeedbackRequest) {
        self.send_hystersis_notification(XaiSessionUpdate::FeedbackRequest(request.into()))
            .await;
    }
}
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<hystersis_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `hystersis prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &hystersis_agent::PromptContext) {
...
) -> Option<hystersis_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use hystersis_tools::types::output::{MCPOutput, ToolOutput};
    use hystersis_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl hystersis_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl hystersis_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> hystersis_tool_protocol::ToolId {
            hystersis_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::hystersis_tool_runtime::ListToolsContext,
        ) -> hystersis_tool_types::ToolDescription {
            hystersis_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            _ctx: hystersis_tool_runtime::ToolCallContext,
            _args: serde_json::Value,
        ) -> Result<ToolOutput, hystersis_tool_runtime::ToolError> {
...
mod tool_meta_stamp_tests {
    //! Pin the `hystersis.com/tool` stamps on the harness emission paths: the early
    ...
    use super::*;
    use tokio::sync::mpsc;
    use hystersis_tools::registry::types::ToolConfig;
    use hystersis_tools::tool_taxonomy::TOOL_META_KEY;
    use hystersis_workspace::permission::PermissionCommand;
    ...
    /// The `hystersis.com/tool` object from an event's `_meta`, if present.
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "HystersisBuild:read_file".to_string(),
                    )])
                    .await,
                );
                ...
                let t = tool_meta(early.as_ref()).expect("early ToolCall carries hystersis.com/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["namespace"], "hystersis_build");
                assert!(t.get("input").is_none(), "identity-only before parse");
                let refined = refined.expect("refinement ToolCallUpdate emitted");
                let t = tool_meta(refined.as_ref()).expect("refinement carries hystersis.com/tool");
        ...
        local
            .run_until(async {
                        ...
                        } = cmd
                        {
                            *captured_in_task.lock().await = Some(tool_call_update);
                            let _ = respond_to.send(
                                hystersis_workspace::permission::PermissionResolution {
                ...
                let t = tool_meta(update.meta.as_ref())
                    .expect("permission-request ToolCallUpdate carries hystersis.com/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["input"]["path"], "/tmp/stamp.txt");
            })
...
mod managed_gateway_tool_tests {
    ...
    use hystersis_tools::types::output::{MCPOutput, ToolOutput};
    use hystersis_tools::types::tool::{ToolKind, ToolNamespace};
    use hystersis_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<hystersis_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
