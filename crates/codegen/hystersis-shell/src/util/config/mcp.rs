...
use hystersis_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use hystersis_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
...
pub use hystersis_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use hystersis_config_types::PoolConfig;
...
pub(crate) fn mcp_preferences_path() -> PathBuf {
    hystersis_config::hystersis_home().join("mcp_preferences.json")
}
...
pub(crate) fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&hystersis_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    hystersis_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = hystersis_mcp::credentials::McpCredentialStore::load_default() {
...
pub use hystersis_workspace::project_config::{
...
/// registry snapshot.
pub fn load_cli_plugin_registry(cwd: &std::path::Path) -> hystersis_agent::plugins::PluginRegistry {
    ...
    let discovered = hystersis_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    hystersis_agent::plugins::PluginRegistry::from_discovered(
        discovered,
...
pub(crate) fn session_registry_from_env_opt() -> Option<bool> {
    hystersis_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = hystersis_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = hystersis_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = hystersis_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
    ...
    fn load_cli_plugin_registry_includes_project_config_path_plugins() {
        ...
        let _env = hystersis_test_support::EnvGuard::set("HYSTERSIS_HOME", home.path());
        ...
        let agents = hystersis_agent::discovery::plugin_agents(&registry);
        assert!(
            agents
                .iter()
