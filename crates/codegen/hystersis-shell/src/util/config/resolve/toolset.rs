...
/// [`hystersis_config::env_bool`] parser (`HYSTERSIS_TOOLS_FIND_BFS` /
...
/// [`hystersis_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub(crate) fn resolve_search_tools_enabled(
    ...
    let disable = hystersis_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = hystersis_config::env_bool(primary).or_else(|| hystersis_config::env_bool(alias));
...
) -> Option<hystersis_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<hystersis_tools::util::ShellEnvironmentPolicy>() {
...
/// the shared [`hystersis_config::env_bool`] via `BoolFlag`). The secs env var
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            hystersis_tools::implementations::hystersis_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        hystersis_tools::implementations::hystersis_build::ask_user_question::response_timeout_env_secs(),
...
) -> hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
fn cap_web_search_domains(list: Option<Vec<String>>, field: &str) -> Option<Vec<String>> {
    const MAX: usize = hystersis_sampling_types::MAX_WEB_SEARCH_DOMAINS;
...
///
/// [`WebSearchOptions`]: hystersis_sampling_types::WebSearchOptions
pub(crate) fn resolve_web_search_domains_from_disk()
-> Option<hystersis_sampling_types::WebSearchOptions> {
...
fn web_search_options_from_section(
    ...
    let opts = hystersis_sampling_types::WebSearchOptions {
    ...
    match opts.validate() {
        ...
        Err(e) => {
            ...
            Some(hystersis_sampling_types::WebSearchOptions {
...
mod web_search_domains_tests {
    ...
    // (see `hystersis_config::loader` normalization tests). These cover only the
...
mod ask_user_question_timeout_tests {
    ...
    use hystersis_tools::implementations::hystersis_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = hystersis_tools::implementations::hystersis_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod shell_env_policy_tests {
    use super::*;
    use hystersis_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
