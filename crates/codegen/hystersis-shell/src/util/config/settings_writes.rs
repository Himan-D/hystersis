...
/// canonical theme name (`hystersisnight`, `tokyonight`, `auto`, etc.).
...
pub async fn set_openai_api_key(value: String) -> Result<()> {
    let _guard = super::persist::lock_config_writes().await;
    let path = crate::util::hystersis_home::hystersis_home().join("config.toml");
    let mut doc = tokio::fs::read_to_string(&path).await.unwrap_or_default().parse::<toml_edit::DocumentMut>()?;
    ...
    if let Some(providers) = doc.get_mut("model_providers").and_then(|i| i.as_table_mut()) {
        ...
        if let Some(openai) = providers.get_mut("openai").and_then(|i| i.as_table_mut()) {
            openai.insert("api_key", toml_edit::value(value));
        }
    }
...
pub async fn set_anthropic_api_key(value: String) -> Result<()> {
    let _guard = super::persist::lock_config_writes().await;
    let path = crate::util::hystersis_home::hystersis_home().join("config.toml");
    let mut doc = tokio::fs::read_to_string(&path).await.unwrap_or_default().parse::<toml_edit::DocumentMut>()?;
    if doc.get("model_providers").is_none() {
    ...
    if let Some(providers) = doc.get_mut("model_providers").and_then(|i| i.as_table_mut()) {
        if providers.get("anthropic").is_none() {
            let mut table = toml_edit::Table::new();
            table.set_implicit(true);
            providers.insert("anthropic", toml_edit::Item::Table(table));
        }
        if let Some(anthropic) = providers.get_mut("anthropic").and_then(|i| i.as_table_mut()) {
            anthropic.insert("api_key", toml_edit::value(value));
        }
    }
    tokio::fs::write(&path, doc.to_string()).await?;
    Ok(())
...
pub async fn set_openrouter_api_key(value: String) -> Result<()> {
    let _guard = super::persist::lock_config_writes().await;
    let path = crate::util::hystersis_home::hystersis_home().join("config.toml");
    let mut doc = tokio::fs::read_to_string(&path).await.unwrap_or_default().parse::<toml_edit::DocumentMut>()?;
    if doc.get("model_providers").is_none() {
    ...
    if let Some(providers) = doc.get_mut("model_providers").and_then(|i| i.as_table_mut()) {
        if providers.get("openrouter").is_none() {
            let mut table = toml_edit::Table::new();
            table.set_implicit(true);
            providers.insert("openrouter", toml_edit::Item::Table(table));
        }
        if let Some(openrouter) = providers.get_mut("openrouter").and_then(|i| i.as_table_mut()) {
            openrouter.insert("api_key", toml_edit::value(value));
        }
    }
    tokio::fs::write(&path, doc.to_string()).await?;
    Ok(())
...
pub async fn set_ollama_api_base(value: String) -> Result<()> {
    let _guard = super::persist::lock_config_writes().await;
    let path = crate::util::hystersis_home::hystersis_home().join("config.toml");
    let mut doc = tokio::fs::read_to_string(&path).await.unwrap_or_default().parse::<toml_edit::DocumentMut>()?;
    if doc.get("model_providers").is_none() {
    ...
    if let Some(providers) = doc.get_mut("model_providers").and_then(|i| i.as_table_mut()) {
        if providers.get("ollama").is_none() {
            let mut table = toml_edit::Table::new();
            table.set_implicit(true);
            providers.insert("ollama", toml_edit::Item::Table(table));
        }
        if let Some(ollama) = providers.get_mut("ollama").and_then(|i| i.as_table_mut()) {
            ollama.insert("api_base", toml_edit::value(value));
        }
    }
    tokio::fs::write(&path, doc.to_string()).await?;
    Ok(())
...
pub async fn set_lm_studio_api_base(value: String) -> Result<()> {
    let _guard = super::persist::lock_config_writes().await;
    let path = crate::util::hystersis_home::hystersis_home().join("config.toml");
    let mut doc = tokio::fs::read_to_string(&path).await.unwrap_or_default().parse::<toml_edit::DocumentMut>()?;
    if doc.get("model_providers").is_none() {
    ...
    if let Some(providers) = doc.get_mut("model_providers").and_then(|i| i.as_table_mut()) {
        if providers.get("lm_studio").is_none() {
            let mut table = toml_edit::Table::new();
            table.set_implicit(true);
            providers.insert("lm_studio", toml_edit::Item::Table(table));
        }
        if let Some(lm_studio) = providers.get_mut("lm_studio").and_then(|i| i.as_table_mut()) {
            lm_studio.insert("api_base", toml_edit::value(value));
        }
    }
    tokio::fs::write(&path, doc.to_string()).await?;
    Ok(())
...
pub async fn set_trinetra_api_key(value: String) -> Result<()> {
    set_provider_api_key("trinetra", "api_key", value).await
}

pub async fn set_provider_api_key(provider: &str, key: &str, value: String) -> Result<()> {
    let _guard = super::persist::lock_config_writes().await;
    let path = crate::util::hystersis_home::hystersis_home().join("config.toml");
    let mut doc = tokio::fs::read_to_string(&path).await.unwrap_or_default().parse::<toml_edit::DocumentMut>()?;
    if doc.get("model_providers").is_none() {
    ...
    if let Some(providers) = doc.get_mut("model_providers").and_then(|i| i.as_table_mut()) {
        if providers.get(provider).is_none() {
            let mut table = toml_edit::Table::new();
            table.set_implicit(true);
            providers.insert(provider, toml_edit::Item::Table(table));
        }
        if let Some(tbl) = providers.get_mut(provider).and_then(|i| i.as_table_mut()) {
