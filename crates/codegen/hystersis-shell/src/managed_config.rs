...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    ...
    hystersis_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    hystersis_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            hystersis_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", hystersis_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            hystersis_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
pub fn clear_orphan() {
    ...
    if hystersis_config::fail_closed_policy_armed_at(&home) {
        tracing::info!(
    ...
    match failure.kind {
        TransportFailureKind::CertificateUntrusted => {
            ManagedConfigError::CertificateUntrusted(certificate_detail(
                failure.detail,
                hystersis_extra_ca::configured_bundle_env(),
                hystersis_extra_ca::extra_root_ders().len(),
            ))
        }
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = hystersis_config::signed_policy::stored_envelope_nonce(
        &crate::util::hystersis_home::hystersis_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            hystersis_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
fn apply_fetched(
    ...
    let verified = if hystersis_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(hystersis_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        hystersis_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(hystersis_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            hystersis_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(hystersis_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &hystersis_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<hystersis_config::signed_policy::SignatureEnvelope> {
    use hystersis_config::signed_policy::now_unix;
    ...
    let claim = match hystersis_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &hystersis_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
pub fn classify_auth_mode() -> hystersis_telemetry::startup::AuthMode {
    auth_mode(
...
) -> hystersis_telemetry::startup::AuthMode {
    use hystersis_telemetry::startup::AuthMode;
...
pub async fn ensure_managed_policy_present(
    ...
    hystersis_telemetry::startup::enter(hystersis_telemetry::startup::StartupPhase::ManagedPolicy);
    ...
    hystersis_telemetry::startup::set_auth_mode(auth_mode(has_deployment_key, &signed_in_team));
...
fn bump_managed_rollback_floor() {
    ...
    if !hystersis_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::hystersis_home::hystersis_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            hystersis_config::bump_rollback_floor(&home);
        }
