...
fn main_cli_tools_override_preserves_profile_injection_policy() {
    ...
    let mut cases = vec![(AgentDefinition::default_hystersis_build(), true)];
...
fn auto_mode_config_parses_from_toml_and_json_equivalently() {
    use hystersis_workspace::permission::ClassifierPromptType;
    let toml_src = r#"
enabled = true
prompt_type = "no_user_tool_prefix"
classifier_model = "hystersis-4.5"
...
"#;
    ...
    for cfg in [&from_toml, &from_json] {
        ...
        assert_eq!(cfg.classifier_model.as_deref(), Some("hystersis-4.5"));
...
fn inject_url_derived_headers_skips_proxy_headers_for_external_url() {
    let mut headers = IndexMap::new();
    inject_url_derived_headers(&mut headers, None, "https://api.hystersis.com/v1");
...
fn session_resolver_is_not_stamped_onto_third_party_samplers() {
    ...
    impl hystersis_sampler::BearerResolver for SessionResolver {
...
fn shell_environment_policy_known_keys_track_the_policy_struct() {
    let hystersis_tools::util::ShellEnvironmentPolicy {
        inherit: _,
        ignore_default_excludes: _,
        exclude: _,
        set: _,
        include_only: _,
    } = hystersis_tools::util::ShellEnvironmentPolicy::default();
...
fn parses_model_api_key() {
    ...
    assert_eq!(model.info.model, "hystersis-4.5");
...
async fn static_key_shadows_defined_provider_through_pipeline() {
    ...
    assert_eq!(creds.auth_type, hystersis_chat_state::AuthType::ApiKey);
...
async fn resolve_credentials_serves_cached_provider_token() {
    use hystersis_chat_state::AuthType;
...
async fn set_env_key_shadows_warm_provider_at_resolve_time() {
    use hystersis_test_support::EnvGuard;
    let var = "HYSTERSIS_TEST_ENVKEY_SHADOW";
...
fn sampling_config_uses_fallback_when_no_model_api_key() {
    ...
    let sampling_config = sampling_config_for_model(
        ...
        ResolvedCredentials {
            ...
            auth_type: hystersis_chat_state::AuthType::ApiKey,
...
fn sampling_config_scopes_no_inline_citations_include() {
    for (supports_search, backend, base_url, expected) in [
        ...
        (true, ApiBackend::Responses, "https://api.hystersis.com/v1", true),
        (false, ApiBackend::Responses, "https://api.hystersis.com/v1", false),
        (
            true,
...
fn default_models_dual_endpoint_routing() {
    ...
    for (model_id, entry) in default_model_entries(&endpoints) {
        ...
        assert_eq!(
            api_key_creds.base_url, endpoints.hystersis_api_base_url,
            "{model_id}: ExternalApiKey must route to api.hystersis.com"
        );
    }
}
#[test]
...
fn env_keys_resolve_first_set_wins() {
    let keys = EnvKeys::new(["HYSTERSIS_TEST_ENV_KEY_PRIMARY", "HYSTERSIS_TEST_ENV_KEY_FALLBACK"]);
    assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
    assert_eq!(
        keys.resolve_value_with(
            |n| (n == "HYSTERSIS_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
        ),
        Some("from-fallback".into())
    );
    assert_eq!(
        keys.resolve_value_with(|n| match n {
            "HYSTERSIS_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
            "HYSTERSIS_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
            _ => None,
        }),
    ...
    assert_eq!(
        keys.resolve_value_with(|n| match n {
            "HYSTERSIS_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
            "HYSTERSIS_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
            _ => None,
        }),
        Some("from-fallback".into())
...
fn env_keys_resolve_skips_whitespace_only_value() {
    let keys = EnvKeys::new(["HYSTERSIS_TEST_WS_PRIMARY", "HYSTERSIS_TEST_WS_FALLBACK"]);
    assert_eq!(
        keys.resolve_value_with(|n| match n {
            "HYSTERSIS_TEST_WS_PRIMARY" => Some("   ".into()),
            "HYSTERSIS_TEST_WS_FALLBACK" => Some("real".into()),
            _ => None,
        }),
        Some("real".into())
    );
    assert_eq!(
        EnvKeys::single("HYSTERSIS_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
        None
    );
    assert_eq!(
        EnvKeys::single("HYSTERSIS_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
        Some("  tok  ".into())
    );
}
#[test]
...
fn first_own_credential_empty_api_key_falls_through_to_env_key() {
    use hystersis_test_support::EnvGuard;
    let var = "HYSTERSIS_TEST_FIRST_OWN_CRED_ENV";
    let _guard = EnvGuard::set(var, "env-token");
...
fn resolve_credentials_multi_env_key_uses_lc_alias() {
    use hystersis_chat_state::AuthType;
    let primary = "HYSTERSIS_TEST_MULTI_ENV_PRIMARY";
    let alias = "HYSTERSIS_TEST_MULTI_ENV_LC_ALIAS";
    unsafe {
...
fn resolve_credentials_empty_env_key_falls_through_to_session() {
    use hystersis_chat_state::AuthType;
    use hystersis_test_support::EnvGuard;
    let primary = "HYSTERSIS_TEST_EMPTY_ENV_PRIMARY";
    let alias = "HYSTERSIS_TEST_EMPTY_ENV_LC_ALIAS";
...
fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
    ...
    let primary = "HYSTERSIS_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
    let alias = "HYSTERSIS_TEST_EMPTY_ENV_GLOBAL_ALIAS";
    let _primary = EnvGuard::set(primary, "");
    let _alias = EnvGuard::set(alias, "");
...
fn enforce_disable_api_key_auth_blocks_first_party_only() {
    use hystersis_chat_state::AuthType;
    let mut creds = api_key_creds("https://api.hystersis.com/v1");
    ...
    let mut creds = ResolvedCredentials {
        ...
        ..api_key_creds("https://api.hystersis.com/v1")
    };
    enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
    assert_eq!(creds.auth_type, AuthType::SessionToken);
}
...
/// (non-hystersis.com) own keys are preserved. (`try_resolve_model_credentials`
...
fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
    ...
    assert_eq!(config.api_backend, ApiBackend::Messages);
    let client = hystersis_sampler::SamplingClient::new(config).expect("client should build");
...
fn byok_from_lookup_classifies_all_states() {
    ...
    let session = test_model_entry("m", "https://api.hystersis.com/v1", None, None, None);
    assert_eq!(
...
fn user_override_adds_api_key_to_default_model() {
    ...
    assert_eq!(
        model.info.base_url, "https://cli-chat-proxy.hystersis.com/v1",
...
fn user_override_parses_compaction_at_tokens_from_toml() {
    use hystersis_sampling_types::CompactionAtTokens;
    let dm = crate::models::default_model();
    let raw_config: toml::Value = toml::from_str(&format!(
        r#"
            [model."{dm}"]
...
fn user_override_parses_compactions_remaining_from_toml() {
    use hystersis_sampling_types::CompactionsRemaining;
    let dm = crate::models::default_model();
    let raw_config: toml::Value = toml::from_str(&format!(
        r#"
            [model."{dm}"]
...
fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
    ...
    assert_eq!(
        ...
        CompactionMode::Segments(hystersis_chat_state::CompactionDetail::default())
    );
    assert_eq!(
...
fn sampling_config_context_window_from_entry_or_default() {
    let model = test_model_entry("any-model", "https://api.hystersis.com/v1", None, None, None);
    let config = sampling_config_for_model(
        &model,
        resolve_credentials(&model, None),
        None,
    ...
    let mut model = test_model_entry("any-model", "https://api.hystersis.com/v1", None, None, None);
...
/// xai-hystersis-shell/src/agent/models.rs:857 for any BYOK Claude config.
#[test]
...
#[test]
fn model_agent_type_defaults_to_hystersis_build() {
    let raw_config: toml::Value = toml::from_str(
        r#"
            [model.my-model]
            model = "my-model"
...
fn hidden_model_excluded_from_acp_but_kept_in_catalog() {
    ...
    let raw_config: toml::Value = toml::from_str(
        r#"
            [model.visible-model]
            model = "visible-model"
            base_url = "https://api.hystersis.com/v1"
            context_window = 200000
...
fn invalid_glob_is_rejected_by_validation() {
    ...
    assert!(ModelGlobSet::compile(Some(&vec!["hystersis[".to_string()])).is_err());
    let raw: toml::Value = toml::from_str(
        r#"
            [models]
            allowed_models = ["hystersis["]
            "#,
    )
    .unwrap();
...
#[test]
fn auth_alias_maps_to_hystersis_com_config() {
    let raw: toml::Value = toml::from_str(
        r#"
            [auth.oidc]
    ...
    let oidc = cfg.hystersis_com_config.oidc.expect("oidc should be set");
    assert_eq!(oidc.issuer, "https://example.okta.com");
    assert_eq!(oidc.client_id, "test-id");
}
#[test]
fn hystersis_com_config_still_works() {
    let raw: toml::Value = toml::from_str(
        r#"
...
fn disable_api_key_auth_parses_from_auth_alias() {
    ...
    assert_eq!(absent.hystersis_com_config.disable_api_key_auth, None);
    let raw: toml::Value = toml::from_str(
        r#"
            [auth]
    ...
    .unwrap();
    let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
    assert_eq!(cfg.hystersis_com_config.disable_api_key_auth, Some(true));
}
...
fn force_login_team_uuid_parses_string_and_array() {
    ...
    let _g = crate::env::EnvVarGuard::remove("HYSTERSIS_FORCE_LOGIN_TEAM_ID");
    assert!(
    ...
    let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
    assert_eq!(absent.hystersis_com_config.force_login_team_uuid, None);
    let raw: toml::Value = toml::from_str(
        r#"
            [auth]
    ...
    let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
    assert_eq!(
        cfg.hystersis_com_config.force_login_team_uuid,
...
fn force_login_team_id_env_overrides_user_config() {
    use crate::auth::ForceLoginTeam;
    let _guard = crate::env::EnvVarGuard::set("HYSTERSIS_FORCE_LOGIN_TEAM_ID", "env-team");
    assert!(
        crate::auth::force_login_team_from_requirements().is_none(),
        "clear the force_login_team_uuid pin in requirements.toml to run this test",
    );
    ...
    assert_eq!(
        from_env.hystersis_com_config.force_login_team_uuid,
    ...
    assert_eq!(
        overridden.hystersis_com_config.force_login_team_uuid,
        Some(ForceLoginTeam::Single("env-team".into())),
    );
}
...
fn force_login_team_id_env_unset_keeps_config_value() {
    use crate::auth::ForceLoginTeam;
    let _guard = crate::env::EnvVarGuard::remove("HYSTERSIS_FORCE_LOGIN_TEAM_ID");
    assert!(
        crate::auth::force_login_team_from_requirements().is_none(),
        "clear the force_login_team_uuid pin in requirements.toml to run this test",
    );
...
fn force_login_team_uuid_implies_api_key_auth_disabled() {
    use crate::auth::{ForceLoginTeam, HystersisComConfig};
    let base = HystersisComConfig {
...
fn e2e_user_overrides_default_model_key_with_custom_endpoint() {
    ...
    assert_eq!(
        ...
        "should route to the user's custom endpoint, not api.hystersis.com"
    );
...
fn config_models_default_is_not_overwritten_by_default_models_json() {
    ...
    let resolved = resolve_string_flag(
        None,
        "HYSTERSIS_DEFAULT_MODEL_TEST_NONEXISTENT",
        config_default,
...
fn config_models_default_custom_model_is_in_resolved_model_list() {
    ...
    assert!(
        models.contains_key("acme-hystersis"),
        "user-defined model must be in the resolved model list"
    );
    let model = models.get("acme-hystersis").unwrap();
    assert_eq!(model.info.model, "hystersis-4.5");
    assert_eq!(model.info.base_url, "https://inference.example.com/v1");
}
#[test]
fn e2e_default_model_with_session_routes_to_proxy() {
    ...
    assert_eq!(
        sampling.base_url, "https://cli-chat-proxy.hystersis.com/v1",
        "session auth should route to cli-chat-proxy, not api.hystersis.com"
    );
}
#[test]
#[serial]
fn e2e_default_model_with_external_api_key_routes_to_api_xai() {
    ...
    assert_eq!(
        sampling.base_url, "https://api.hystersis.com/v1",
        "external API key should route to api.hystersis.com via api_base_url"
    );
    unsafe { std::env::remove_var("XAI_API_KEY") };
}
#[test]
fn e2e_user_config_overrides_prefetched_model() {
    ...
    prefetched.insert(
        dm.to_string(),
        test_model_entry(dm, "https://cli-chat-proxy.hystersis.com/v1", None, None, None),
    );
    let (_, models) = resolve_models_from_toml(
        &format!(
            r#"
...
fn e2e_credential_priority_model_key_beats_session_beats_env() {
    ...
    let model_no_key = test_model_entry(
        ...
        None,
        None,
        Some("https://api.hystersis.com/v1"),
    );
...
fn e2e_duplicate_model_field_both_entries_survive() {
    ...
    let user = models.get("acme-hystersis").unwrap();
    ...
    assert_eq!(sampling.base_url, "https://cli-chat-proxy.hystersis.com/v1",);
}
#[test]
fn e2e_enterprise_custom_endpoint_skips_hystersis_defaults() {
    let mut cfg = Config::default();
...
fn e2e_acp_model_info_no_dedup_on_model_field() {
    let mut models = IndexMap::new();
    models.insert(
        "default-hystersis".to_string(),
        test_model_entry(
            crate::models::default_model(),
            "https://cli-chat-proxy.hystersis.com/v1",
            None,
            None,
            Some("https://api.hystersis.com/v1"),
        ),
    ...
    assert!(
        acp_models.contains_key(&acp::ModelId::new("default-hystersis")),
        "default entry should be addressable by map key"
    );
    assert!(
        acp_models.contains_key(&acp::ModelId::new("acme-hystersis")),
...
fn e2e_enterprise_endpoints_plus_partial_model_override() {
    ...
    let (_, models) = resolve_models_from_toml(
        &format!(
            ...
            hystersis_api_base_url = "https://enterprise-api.acme.com/v1"
...
fn e2e_enterprise_endpoints_only_no_model_override() {
    ...
    assert_eq!(
        model.api_base_url.as_deref(),
        Some("https://enterprise-api.acme.com/v1"),
        "default model should use enterprise hystersis_api_base_url"
    );
}
...
fn unset_endpoint_env_vars() {
    for k in [
        "HYSTERSIS_CLI_CHAT_PROXY_BASE_URL",
        "HYSTERSIS_XAI_API_BASE_URL",
        "HYSTERSIS_FEEDBACK_BASE_URL",
        "HYSTERSIS_TRACE_UPLOAD_URL",
        "HYSTERSIS_MANAGED_CONFIG_URL",
        "HYSTERSIS_MODELS_BASE_URL",
        "HYSTERSIS_MODELS_LIST_URL",
        "OTEL_EXPORTER_OTLP_ENDPOINT",
        "OTEL_EXPORTER_OTLP_TRACES_ENDPOINT",
        "OTEL_EXPORTER_OTLP_HEADERS",
        "HYSTERSIS_INTERNAL_OTLP_TRACES_ENDPOINT",
        "HYSTERSIS_INTERNAL_OTLP_HEADERS",
        "HYSTERSIS_EXTERNAL_OTEL",
    ] {
        unsafe { std::env::remove_var(k) };
    }
}
/// INVARIANT: auxiliary-service resolvers resolve to the cli-chat-proxy, never
/// `hystersis_api_base_url` — overriding ONLY inference keeps every aux endpoint on
...
fn aux_endpoints_resolve_to_proxy_never_inference() {
    ...
    let cfg = EndpointsConfig {
        hystersis_api_base_url: inference.to_string(),
    ...
    assert_eq!(cfg.hystersis_api_base_url, inference);
...
/// REGRESSION: the managed-config URL never follows `hystersis_api_base_url`
...
fn loader_managed_config_url_never_follows_inference_endpoint() {
    ...
    let cfg = Config::new_from_toml_cfg(
        &toml::from_str(
            r#"[endpoints]
                hystersis_api_base_url = "https://inference.acme-corp.example/xai/v1""#,
        )
        .unwrap(),
    )
...
fn resolve_title_refresh_defaults_to_turn_summary_but_decouples() {
    unsafe { std::env::remove_var("HYSTERSIS_TITLE_REFRESH") };
    unsafe { std::env::remove_var("HYSTERSIS_TURN_SUMMARY") };
    ...
    unsafe { std::env::set_var("HYSTERSIS_TITLE_REFRESH", "0") };
    let r = decoupled.resolve_title_refresh();
    assert!(!r.value, "HYSTERSIS_TITLE_REFRESH env wins");
    assert_eq!(r.source, ConfigSource::Env);
    unsafe { std::env::remove_var("HYSTERSIS_TITLE_REFRESH") };
}
/// A `turn_summary` pin lands in the title's default slot, so it moves the title
/// with it. Only the default slot, so `HYSTERSIS_TITLE_REFRESH` still outranks it and a
...
fn a_turn_summary_pin_moves_the_title_default_and_the_environment_lifts_it() {
    let _env = EnvGuard::set("HYSTERSIS_TURN_SUMMARY", "1");
    let mut cfg = Config::default();
    ...
    {
        let _title = EnvGuard::unset("HYSTERSIS_TITLE_REFRESH");
        assert!(
            !cfg.resolve_title_refresh().value,
            "the pin outranks HYSTERSIS_TURN_SUMMARY, and the title default follows the pin"
        );
    }
    let _title = EnvGuard::set("HYSTERSIS_TITLE_REFRESH", "1");
...
fn a_title_refresh_pin_outranks_the_environment() {
    let _env = EnvGuard::set("HYSTERSIS_TITLE_REFRESH", "1");
    let mut cfg = Config::default();
    cfg.requirements
        .title_refresh
        .pin(false, crate::config::RequirementSource::Unknown);
    let r = cfg.resolve_title_refresh();
    assert!(!r.value, "the pin lost to HYSTERSIS_TITLE_REFRESH");
...
fn resolve_doom_loop_recovery_precedence() {
    ...
    unsafe { std::env::remove_var("HYSTERSIS_DOOM_LOOP_RECOVERY") };
    ...
    unsafe { std::env::set_var("HYSTERSIS_DOOM_LOOP_RECOVERY", "0") };
    assert!(
...
fn worktree_auto_gc_section_parses_from_toml() {
    unsafe { hystersis_fast_worktree::clear_auto_gc_env_for_test() };
    let raw: toml::Value = toml::from_str(
        r#"
            [worktree.auto_gc]
            enabled = true
    ...
    assert_eq!(
        p.max_age_by_kind
            .get(&hystersis_fast_worktree::WorktreeKind::Subagent),
        Some(&Some(3600))
    );
    assert_eq!(
        p.max_age_by_kind
            .get(&hystersis_fast_worktree::WorktreeKind::Manual),
        Some(&None)
    );
}
...
fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
    unsafe { std::env::remove_var("HYSTERSIS_TELEMETRY_ENABLED") };
    unsafe { std::env::remove_var("HYSTERSIS_TELEMETRY_TRACE_UPLOAD") };
    let mut cfg = Config::default();
...
fn resolve_goal_defaults_to_true_when_unset() {
    unsafe { std::env::remove_var("HYSTERSIS_GOAL") };
    let cfg = Config::default();
...
fn resolve_goal_env_overrides_config_without_remote_kill_switch() {
    unsafe { std::env::set_var("HYSTERSIS_GOAL", "1") };
    let mut cfg = Config::default();
...
fn background_workflows_default_on_without_affecting_goal() {
    unsafe { std::env::remove_var("HYSTERSIS_WORKFLOWS") };
    let cfg = Config::default();
...
fn resolve_workflows_remote_false_kills_local_opt_in() {
    unsafe { std::env::set_var("HYSTERSIS_WORKFLOWS", "1") };
    let mut cfg = Config::default();
...
fn resolve_workflows_env_wins() {
    unsafe { std::env::set_var("HYSTERSIS_WORKFLOWS", "0") };
    let cfg = Config::default();
    let r = cfg.resolve_workflows();
    assert_eq!(r.source, ConfigSource::Env);
    assert!(
...
fn resolve_image_gen_model_override_remote_settings_or_config() {
    unsafe { std::env::remove_var("HYSTERSIS_IMAGE_GEN_MODEL_OVERRIDE") };
    ...
    assert_eq!(
        with(None, Some("hystersis-imagine-image")).resolve_image_gen_model_override(),
        Some("hystersis-imagine-image".to_owned())
    );
    assert_eq!(
        with(Some("hystersis-imagine-image-pro"), Some("hystersis-imagine-image"))
            .resolve_image_gen_model_override(),
        Some("hystersis-imagine-image-pro".to_owned())
    );
}
#[test]
#[serial]
fn resolve_image_edit_model_override_remote_settings_or_config() {
    unsafe { std::env::remove_var("HYSTERSIS_IMAGE_EDIT_MODEL_OVERRIDE") };
    let with = |config: Option<&str>, gb: Option<&str>| Config {
        features: Features {
    ...
    assert_eq!(
        with(None, Some("hystersis-imagine-image")).resolve_image_edit_model_override(),
        Some("hystersis-imagine-image".to_owned())
    );
    assert_eq!(
        with(Some("hystersis-imagine-image-pro"), Some("hystersis-imagine-image"))
    ...
    let gen_only = Config {
        remote_settings: Some(crate::util::config::RemoteSettings {
            image_gen_model_override: Some("hystersis-imagine-image".to_owned()),
            ..Default::default()
        }),
        ..Default::default()
    };
...
fn imagine_tools_disabled_gates_image_edit() {
    unsafe { std::env::remove_var("HYSTERSIS_IMAGE_EDIT") };
    ...
    unsafe { std::env::set_var("HYSTERSIS_IMAGE_EDIT", "1") };
...
fn resolve_image_gen_gates() {
    unsafe { std::env::remove_var("HYSTERSIS_IMAGE_GEN") };
    ...
    unsafe { std::env::set_var("HYSTERSIS_IMAGE_GEN", "1") };
    let denied = Config {
        remote_settings: Some(crate::util::config::RemoteSettings {
...
fn resolve_video_gen_gates() {
    unsafe { std::env::remove_var("HYSTERSIS_VIDEO_GEN") };
...
fn clear_goal_envs() {
    unsafe {
        std::env::remove_var("HYSTERSIS_GOAL");
        std::env::remove_var("HYSTERSIS_GOAL_CLASSIFIER");
        std::env::remove_var("HYSTERSIS_GOAL_PLANNER");
        std::env::remove_var("HYSTERSIS_GOAL_SUMMARY");
        std::env::remove_var("HYSTERSIS_GOAL_VERIFIER_N");
        std::env::remove_var("HYSTERSIS_GOAL_CLASSIFIER_MAX");
        std::env::remove_var("HYSTERSIS_GOAL_STRATEGIST_EVERY");
        std::env::remove_var("HYSTERSIS_GOAL_REVERIFY_AFTER");
    }
}
...
fn resolve_goal_classifier_env_overrides_default_and_remote() {
    clear_goal_envs();
    unsafe { std::env::set_var("HYSTERSIS_GOAL_CLASSIFIER", "0") };
    let r = cfg_with_goal_and_remote(true, remote_classifier(true))
        .resolve_goal_classifier_enabled(true);
    assert!(!r.value);
    assert_eq!(r.source, ConfigSource::Env);
    unsafe { std::env::set_var("HYSTERSIS_GOAL_CLASSIFIER", "1") };
...
fn resolve_goal_planner_env_overrides_default_and_remote() {
    clear_goal_envs();
    unsafe { std::env::set_var("HYSTERSIS_GOAL_PLANNER", "0") };
    let r = cfg_with_goal_and_remote(true, remote_planner(true)).resolve_goal_planner_enabled(true);
    assert!(!r.value);
    assert_eq!(r.source, ConfigSource::Env);
    unsafe { std::env::set_var("HYSTERSIS_GOAL_PLANNER", "1") };
    let r =
...
fn resolve_goal_summary_env_overrides_default_and_remote() {
    clear_goal_envs();
    unsafe { std::env::set_var("HYSTERSIS_GOAL_SUMMARY", "0") };
...
const GOAL_USE_CURRENT_ENV: &str = "HYSTERSIS_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "hystersis", agent_type = "hystersis-plan" }
...
"#;
    ...
    let cfg = Config::new_from_toml_cfg(&raw).unwrap();
    assert_eq!(cfg.goal.planner_model.as_ref().unwrap().model, "hystersis");
    assert_eq!(
        cfg.goal.strategist_model.as_ref().unwrap().agent_type,
        "cursor"
    );
    assert_eq!(cfg.goal.skeptic_models.len(), 2);
    assert_eq!(cfg.goal.skeptic_models[0].model, "hystersis");
    assert_eq!(
...
planner_model = { agent_type = "hystersis-plan" }
"#;
    let raw: toml::Value = toml::from_str(toml_str).unwrap();
...
"#,
    ...
    let hystersis_build = crate::util::config::GoalRoleModel {
        model: "hystersis".into(),
        agent_type: "hystersis-plan".into(),
    };
    ...
    assert_eq!(
        planner.value,
        GoalRoleModelChoice::Explicit(hystersis_build.clone())
    );
    ...
    assert_eq!(
        ...
        vec![
            GoalRoleModelChoice::Explicit(hystersis_build),
...
fn config_accepts_all_known_sections() {
    let unused = unused_keys_from_toml(
        r#"
            ...
            command = "/opt/bin/hystersis-identity"
            [repo_changes_dedup]
            enabled = false
            [relay]
            enabled = false
...
fn internal_otlp_test_config() -> EndpointsConfig {
    EndpointsConfig {
        ...
        hystersis_internal_otlp_traces_endpoint: None,
        hystersis_internal_otlp_headers: None,
        external_otel_master_switch: false,
        ..Default::default()
    }
}
/// `hystersis_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*`
/// fields regardless of the master switch.
#[test]
fn internal_otlp_endpoint_hystersis_internal_wins_regardless_of_switch() {
    for switch in [false, true] {
        let cfg = EndpointsConfig {
            hystersis_internal_otlp_traces_endpoint: Some(
        ...
        assert_eq!(
            ...
            "switch={switch}: HYSTERSIS_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
        );
    }
}
...
fn internal_otlp_consumed_standard_vars_cases() {
    ...
    for case in cases {
        let cfg = EndpointsConfig {
            ...
            hystersis_internal_otlp_traces_endpoint: case
                .internal_ep
                .then(|| "https://internal.example/traces".to_string()),
            hystersis_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
            ..internal_otlp_test_config()
        };
        assert_eq!(
...
/// Headers precedence: `hystersis_internal_otlp_headers` wins; legacy
/// `otel_exporter_otlp_headers` only when the master switch is unset.
#[test]
fn internal_otlp_headers_precedence() {
    for switch in [false, true] {
        let cfg = EndpointsConfig {
            hystersis_internal_otlp_headers: Some("x-debug=1".to_string()),
...
fn ext_client() -> hystersis_telemetry::external::config::ExternalClientInfo {
    hystersis_telemetry::external::config::ExternalClientInfo::default()
}
#[test]
fn external_otel_default_off_and_double_opt_in() {
    ...
    assert!(
        resolve_external_otel_config_with(
            None,
            None,
            ext_env(&[("HYSTERSIS_EXTERNAL_OTEL", "1")]),
            ext_client(),
            false,
        )
        .is_none()
...
fn external_otel_file_table_layered_under_env() {
    ...
    assert!(
        resolve_external_otel_config_with(
            Some(&effective),
            None,
            ext_env(&[("HYSTERSIS_EXTERNAL_OTEL", "0")]),
            ext_client(),
            false,
        )
        .is_none()
...
fn external_otel_requirements_pin_wins_over_env() {
    ...
    assert!(
        resolve_external_otel_config_with(
            None,
            Some(&req),
            ext_env(&[("HYSTERSIS_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
            ext_client(),
            false,
        )
        .is_none()
...
/// (managed config / requirements — no `HYSTERSIS_EXTERNAL_OTEL` env var) must
...
fn clear_runtime_env_vars() {
    unsafe {
        std::env::remove_var("HYSTERSIS_SUBAGENTS");
        std::env::remove_var("HYSTERSIS_RESPECT_GITIGNORE");
        std::env::remove_var("HYSTERSIS_WEB_SEARCH_MODEL");
        std::env::remove_var("HYSTERSIS_SESSION_SUMMARY_MODEL");
        std::env::remove_var("HYSTERSIS_CURSOR_SKILLS_ENABLED");
        std::env::remove_var("HYSTERSIS_CURSOR_RULES_ENABLED");
        std::env::remove_var("HYSTERSIS_CURSOR_AGENTS_ENABLED");
        std::env::remove_var("HYSTERSIS_CLAUDE_SKILLS_ENABLED");
        std::env::remove_var("HYSTERSIS_CLAUDE_RULES_ENABLED");
        std::env::remove_var("HYSTERSIS_CLAUDE_AGENTS_ENABLED");
    }
}
fn clear_managed_mcp_env_vars() {
    unsafe {
        std::env::remove_var("HYSTERSIS_MANAGED_MCPS_ENABLED");
        std::env::remove_var("HYSTERSIS_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
    }
}
...
fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
    let _env = isolate_compat_env();
    let _codex = EnvGuard::set("HYSTERSIS_CODEX_SESSIONS_ENABLED", "true");
    let resolved = resolve_compat_sessions_from_raw(Err(()), None);
    assert!(!resolved.cursor.sessions);
    assert!(!resolved.claude.sessions);
    assert!(resolved.codex.sessions);
...
fn resolve_compat_env_sessions_disable_independently() {
    ...
    for (vendor, env_var) in [
        (CompatVendor::Cursor, "HYSTERSIS_CURSOR_SESSIONS_ENABLED"),
        (CompatVendor::Claude, "HYSTERSIS_CLAUDE_SESSIONS_ENABLED"),
        (CompatVendor::Codex, "HYSTERSIS_CODEX_SESSIONS_ENABLED"),
    ] {
...
fn resolve_compat_precedence_and_reserved_codex_hook() {
    ...
    let _session = EnvGuard::set("HYSTERSIS_CURSOR_SESSIONS_ENABLED", "true");
    let _hook = EnvGuard::set("HYSTERSIS_CODEX_HOOKS_ENABLED", "true");
    let resolved = resolve_compat_config(&config, Some(&remote));
    assert!(resolved.cursor.sessions);
    assert!(resolved.codex.hooks);
}
...
fn resolve_runtime_fields_compat_asymmetric_sources() {
    let _env = isolate_compat_env();
    let _cursor = EnvGuard::set("HYSTERSIS_CURSOR_SESSIONS_ENABLED", "false");
    let raw: toml::Value =
...
fn resolve_runtime_fields_gitignore_from_env() {
    clear_runtime_env_vars();
    unsafe { std::env::set_var("HYSTERSIS_RESPECT_GITIGNORE", "0") };
    let raw = empty_config();
    let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
    cfg.resolve_runtime_fields(&RuntimeResolutionContext {
        raw_config: &raw,
...
fn is_telemetry_explicitly_disabled_sync_env_signals() {
    unsafe { std::env::set_var("HYSTERSIS_TELEMETRY_ENABLED", "0") };
    unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
    assert!(is_telemetry_explicitly_disabled_sync());
    unsafe { std::env::set_var("HYSTERSIS_TELEMETRY_ENABLED", "1") };
    assert!(!is_telemetry_explicitly_disabled_sync());
    unsafe { std::env::remove_var("HYSTERSIS_TELEMETRY_ENABLED") };
...
default = "hystersis-4.5"
"#,
    )
    .unwrap();
    let v = semver::Version::parse("1.8.0").unwrap();
    hystersis_config::apply_version_overrides(&mut value, &v).unwrap();
    let cfg = Config::new_from_toml_cfg(&value).unwrap();
    assert_eq!(cfg.models.default.as_deref(), Some("hystersis-4.5"));
}
/// Reproduce the enterprise managed config bug: [model.hystersis] sets
/// context_window=500k for model="hystersis-4.5", but
/// [models].default="hystersis-4.5" resolves to the bare
/// prefetched entry (256k) because Layer 3 only overrides key
/// "hystersis", not key "hystersis-4.5".
///
...
fn slug_propagation_enterprise_managed_config_key_mismatch() {
    ...
    prefetched.insert("hystersis-4.5".to_owned(), entry);
    let resolved = resolve_model_list(&cfg, Some(prefetched));
    let by_key = resolved
        .get("hystersis")
        .expect("hystersis key must exist");
    assert_eq!(by_key.info.context_window.get(), 500_000);
    assert_eq!(by_key.info.model, "hystersis-4.5");
    let by_latest = resolved.get("hystersis-4.5").expect("hystersis-4.5 key must exist");
    assert_eq!(
        by_latest.info.context_window.get(),
        500_000,
        "BUG: prefetched 'hystersis-4.5' should inherit 500k from \
         sibling 'hystersis' (same model slug), not stay at {default_cw}"
    );
}
...
fn slug_propagation_inherits_api_backend_but_not_agent_type() {
    ...
    let raw: toml::Value = toml::from_str(
        r#"
            ...
            api_backend = "responses"
            agent_type = "hystersis"
            "#,
    )
    .unwrap();
    let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
    let mut prefetched = IndexMap::new();
    let mut entry = test_model_entry("hystersis-4.5", "https://test.example.com/v1", None, None, None);
    entry.info.context_window = NonZeroU64::new(default_cw).unwrap();
    entry.info.agent_type = default_agent_type();
    entry.info.api_backend = ApiBackend::default();
    prefetched.insert("hystersis-4.5".to_owned(), entry);
    let resolved = resolve_model_list(&cfg, Some(prefetched));
    let latest = resolved.get("hystersis-4.5").unwrap();
    assert_eq!(
...
fn resolve_model_list_config_reasoning_efforts_beats_remote() {
    ...
    .unwrap();
    let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
    let mut entry = prefetch_model_entry("hystersis-x", 200_000, ApiBackend::default());
    entry.info.reasoning_efforts = vec![ReasoningEffortOption {
        id: "high".to_string(),
        value: ReasoningEffort::High,
        label: "High".to_string(),
    ...
    let mut prefetched = IndexMap::new();
    prefetched.insert("hystersis-x".to_owned(), entry);
    let resolved = resolve_model_list(&cfg, Some(prefetched));
...
fn mcp_liveness_watchers_default_is_true() {
    unsafe { std::env::remove_var("HYSTERSIS_MCP_LIVENESS_WATCHERS") };
...
fn mcp_liveness_watchers_requirement_wins_over_everything() {
    unsafe { std::env::set_var("HYSTERSIS_MCP_LIVENESS_WATCHERS", "true") };
    let r =
...
fn mcp_liveness_watchers_env_wins_over_config_and_below() {
    unsafe { std::env::set_var("HYSTERSIS_MCP_LIVENESS_WATCHERS", "false") };
...
fn mcp_auto_restart_default_is_true() {
    unsafe { std::env::remove_var("HYSTERSIS_MCP_AUTO_RESTART") };
...
fn mcp_auto_restart_requirement_wins_over_everything() {
    unsafe { std::env::set_var("HYSTERSIS_MCP_AUTO_RESTART", "false") };
...
fn mcp_auto_restart_env_wins_over_config_and_below() {
    unsafe { std::env::set_var("HYSTERSIS_MCP_AUTO_RESTART", "true") };
...
fn mcp_push_server_status_default_is_true() {
    unsafe { std::env::remove_var("HYSTERSIS_MCP_PUSH_SERVER_STATUS") };
...
fn mcp_push_server_status_requirement_wins_over_everything() {
    unsafe { std::env::set_var("HYSTERSIS_MCP_PUSH_SERVER_STATUS", "true") };
    let r =
...
fn mcp_push_server_status_env_wins_over_config_and_below() {
    unsafe { std::env::set_var("HYSTERSIS_MCP_PUSH_SERVER_STATUS", "false") };
...
fn mcp_recursive_config_watch_default_is_true() {
    unsafe { std::env::remove_var("HYSTERSIS_MCP_RECURSIVE_CONFIG_WATCH") };
...
fn mcp_recursive_config_watch_requirement_wins_over_everything() {
    unsafe { std::env::set_var("HYSTERSIS_MCP_RECURSIVE_CONFIG_WATCH", "true") };
...
fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
    unsafe { std::env::set_var("HYSTERSIS_MCP_RECURSIVE_CONFIG_WATCH", "false") };
...
fn remote_settings_disarm_managed_config_signatures() {
    ...
    assert!(hystersis_config::signed_policy::verification_active());
...
fn remote_settings_disarm_requires_prod_proxy_when_keys_embedded() {
    ...
    unsafe {
        std::env::remove_var("HYSTERSIS_CLI_CHAT_PROXY_BASE_URL");
    }
    apply_remote_settings_side_effects(Some(&settings));
    assert!(
        !hystersis_config::signed_policy::verification_active(),
...
#[test]
fn a_status_line_the_parser_could_not_read_in_full_reaches_hystersis_inspect() {
    use super::super::config_model_override_parse::{ConfigWarningKind, WarningTarget};
    let raw_config: toml::Value = toml::from_str(
        r#"
            [ui]
