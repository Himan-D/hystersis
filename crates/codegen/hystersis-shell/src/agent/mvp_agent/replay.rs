...
type ReplayCompletionRx = tokio::sync::oneshot::Receiver<hystersis_acp_lib::AcpResult<()>>;
...
impl MvpAgent {
    ...
    /// - xAI updates (`"_hystersis.com/session/update"`) → `ExtNotification`.
    ///
    ...
    pub(super) fn forward_raw_replay_line(
        ...
        // updates.jsonl only persists `_hystersis.com/session/update` and `session/update`.
        ...
        let is_xai = method == "_hystersis.com/session/update";
            ...
            if let Some(obj) = params.as_object_mut() {
                ...
                if let Some(m) = meta.as_object_mut() {
                    ...
                    if let Some(pd) = persist_data {
                        m.insert("hystersis.com/persist".to_string(), pd.clone());
                    }
                    if let Some(tid) = target_client_id {
                        m.insert("hystersis.com/leaderClientId".to_string(), tid.clone());
                    }
                }
            }
        ...
        // the loading client.
        if let Some(tid) = target_client_id {
            stamp_meta_value(&mut notification.meta, "hystersis.com/leaderClientId", tid);
        }
    ...
    pub(super) async fn replay_session_updates(
        ...
        replay_timer.with_subphase(hystersis_telemetry::startup::Subphase::SessionReplay);
...
mod drain_tests {
    ...
    async fn completion_window_awaits_oldest_before_exceeding_cap() {
        ...
        let (overflow_tx, overflow_rx) =
            tokio::sync::oneshot::channel::<hystersis_acp_lib::AcpResult<()>>();
        {
