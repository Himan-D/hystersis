...
//! verdict is [`hystersis_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
impl MvpAgent {
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = hystersis_workspace::trust::workspace_key(cwd);
        ...
        self.session_registry.for_each_resident(|_, h| {
            if hystersis_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key {
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a hystersis_agent::plugins::SharedPluginRegistryHandle,
    compat: &'a hystersis_tools::types::CompatConfig,
