...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        hystersis_telemetry::unified_log::info("agent initialized", None, None);
        hystersis_telemetry::startup::mark_agent_serving();
        ...
        let hystersis_home = hystersis_fast_worktree::resolve_hystersis_home();
        tokio::task::spawn_blocking(move || {
            crate::session::worktree_pool::cleanup_stale_pool_worktrees(None);
            if !remote_settled {
                return;
            }
            Self::reclaim_worktrees(hystersis_home, auto_gc_policy);
        });
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    hystersis_workspace::permission::cleanup_stale_permission_state(
        ...
        hystersis_workspace::trust::migrate_legacy_hook_trust();
        if let Some(auth) = self.auth_manager.current() {
            ...
            hystersis_telemetry::unified_log::info(
                ...
                Some(
                    ...
                    "key_prefix": hystersis_auth::bearer_suffix(&auth.key),
                    "rt_prefix": auth.refresh_token.as_deref().map(hystersis_auth::bearer_suffix),
                }),
                ),
            );
        ...
        if client_type == ClientType::Generic {
            match client_identifier.as_deref() {
                Some("hystersis-web") => client_type = ClientType::HystersisWeb,
                Some("nebula") => client_type = ClientType::Nebula,
                Some("hystersis-code-extension") => client_type = ClientType::Extension,
                Some("hystersis-desktop") => client_type = ClientType::Desktop,
                _ => {}
            }
        }
        *self.client_type.borrow_mut() = client_type;
        ...
        tracing::info!(
            ...
            "code-nav capability initialized from initialize request; \
             index will start lazily on first hystersis.com/code/* request if eligible"
        );
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let pre = self
            ...
            .current()
            .map(|a| (
                hystersis_auth::bearer_suffix(&a.key).to_owned(),
                a
                    .refresh_token
                    .as_deref()
                    .map(|t| hystersis_auth::bearer_suffix(t).to_owned()),
            ));
        ...
        if !self.cfg.borrow().hystersis_com_config.api_key_auth_disabled()
            && auth_method::read_hystersis_api_key_env().is_err()
            ...
            .api_key_auth_disabled();
        {
            ...
            let gc = &cfg.hystersis_com_config;
        ...
        let preferred_method_early = self.cfg.borrow().hystersis_com_config.preferred_method;
        let hystersis_api_base_url = self.cfg.borrow().endpoints.hystersis_api_base_url.clone();
        let has_byok = self
            .models_manager
            .models()
            .values()
        ...
        let has_external_api_key = auth_method::should_advertise_hystersis_api_key_with_env_ok(
            disable_api_key_auth,
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.hystersis_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.hystersis_com_config.auth_provider_label.clone(),
                cfg.hystersis_com_config.auth_provider_command.is_some(),
                cfg.hystersis_com_config.oidc.is_some(),
                issuer,
            )
        };
        if has_enterprise_oidc {
            ...
            tracing::info!(
                label = ?login_label,
                has_auth_provider,
                "auth: advertising hystersis.com auth method",
            );
        }
        ...
        hystersis_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!({
                "hystersis_home": crate::util::hystersis_home::hystersis_home().display().to_string(),
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("HYSTERSIS_AGENT_METADATA");
                    serde_json::json!({
                    "hystersisShell": true,
                    ...
                    // channel (`hystersis.com/mcp/sdk_call`); the SDK reads this to enable transport="acp".
                    (hystersis_mcp::wire::MCP_SDK): true,
                    // `session/new` / `session/load` accept per-session plugin roots in
                    // `_meta.pluginDirs`; the SDKs gate `HystersisOptions.plugins` on this.
                    (SESSION_PLUGIN_DIRS_CAPABILITY_KEY): true,
                    "currentWorkingDirectory": current_working_directory.to_string_lossy().to_string(),
                    "agentVersion": hystersis_version::VERSION,
                    "agentId": agent_id(),
                    ...
                    // disabled feature produces zero `hystersis.com/recap` traffic.
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().hystersis_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                if sampling_config.api_key.is_none() {
                    if let Ok(api_key) = auth_method::read_hystersis_api_key_env() {
                        ...
                        .any(|m| m.has_own_credentials())
                    {
                        ...
                        return Err(
                            acp::Error::auth_required()
                                .data(
                                    "Run `hystersis configure` to set up your API keys.",
                                ),
                        );
                    }
                }
                ...
                log_event(hystersis_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().hystersis_com_config.preferred_method,
                ...
                self.enforce_hystersis_code_access(&auth).await;
                self.maybe_sync_bundle_in_background(false);
            ...
            auth_method::HYSTERSIS_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let hystersis_ctx = self.auth_manager.hystersis_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_hystersis_oauth(cli_oauth);
                    ...
                    })?;
                {
                    ...
                    tracing::debug!("auth: hystersis.com/oidc handler set api_key (SessionToken)");
                    hystersis_telemetry::unified_log::debug(
                        "auth: hystersis.com/oidc handler set api_key (SessionToken)",
                        None,
                        None,
                    );
                }
    ...
    async fn prompt(
        ...
        if let Some(meta) = arguments.meta.as_ref() {
            hystersis_file_utils::trace_context::link_current_span_to_meta(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match hystersis_sampling_types::ToolOverridesUpdate::parse(value) {
        ...
        handle
            ...
            .send(SessionCommand::Prompt {
                ...
                verbatim,
                traceparent: hystersis_file_utils::trace_context::current_traceparent(),
                json_schema,
                send_now,
                admission: None,
            ...
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new(
                        "hystersis.com/session/prompt_complete",
                        params.into(),
                    ),
                );
        }
        ...
        match stop_result {
            Ok(turn_ok) => {
                ...
                let turn_messages: Option<hystersis_chat_state::TurnCapture> = {
                    let (tx, rx) = oneshot::channel();
                    if handle
                        .cmd_tx
                ...
                if let Some(ctx) = trace_context {
                        ...
                        && let Some(client) = self.session_registry_client()
                    {
                        ...
                        tokio::spawn(async move {
                            let git_out = |args: &[&str]| -> Option<String> {
                                hystersis_tty_utils::git_command()
                                    .current_dir(&cwd_str)
                                    .args(args)
                                    .output()
                                    .ok()
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = hystersis_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = hystersis_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = hystersis_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            hystersis_file_utils::trace_context::link_current_span_to_meta(meta);
        }
        ...
        let result = match method.as_ref() {
            "hystersis.com/getApiKey" | "hystersis.com/setApiKey" => {
                crate::extensions::auth::handle(self, &args).await
            }
            "hystersis.com/session/info" | "hystersis.com/session/close" | "hystersis.com/session/list"
            | "hystersis.com/sessions/list" => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            "hystersis.com/workspaces/list" => {
                crate::agent::handlers::workspaces::handle(self, &args).await
            }
            "hystersis.com/models/list" => {
                crate::agent::handlers::models::handle(self, &args).await
            }
            "hystersis.com/session/updates" => {
                crate::extensions::session_updates::handle(&args, &self.gateway).await
            }
            "hystersis.com/session/state" => {
                crate::extensions::session_state::handle_state(&args).await
            }
            "hystersis.com/session/import" => {
                crate::extensions::session_state::handle_import(&args).await
            }
            "hystersis.com/session/load_history" => {
                crate::extensions::chat_conversation_history::handle(self, &args).await
            }
            "hystersis.com/session/search" => {
                crate::extensions::session_search::handle(self, &args).await
            }
            "hystersis.com/session/resolve_local_for_worktree_resume"
            | "hystersis.com/session/rehydrate" => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            #[cfg(feature = "local-workspace")]
            "hystersis.com/session/add_local_workspace" => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            "hystersis.com/session/rename" | "hystersis.com/session/delete"
            | "hystersis.com/session/update_mcp_servers" | "hystersis.com/session/fork"
            | "hystersis.com/plugins/reload" | "hystersis.com/commands/list" => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            m if InternalMethod::from_name(m).is_some() => {
                crate::extensions::session_admin::handle(self, &args).await
            }
            "hystersis.com/session/repair" => crate::extensions::repair::handle(self, &args).await,
            "hystersis.com/session/usage" => crate::extensions::usage::handle(self, &args).await,
            "hystersis.com/memory/flush" | "hystersis.com/memory/rewrite" => {
                crate::extensions::memory::handle(self, &args).await
            }
            "hystersis.com/skills/refresh-baseline" => {
                self.refresh_skill_baseline_for_all_sessions();
                crate::extensions::to_ext_response(
                    Ok(serde_json::json!({"ok": true})),
                )
            }
            "hystersis.com/interject" => crate::extensions::interject::handle(self, &args).await,
            "hystersis.com/feedback" | "hystersis.com/feedback/dismiss" | "hystersis.com/feedback/upload-trace"
            | "hystersis.com/btw" => crate::extensions::feedback::handle(self, &args).await,
            "hystersis.com/recap" => crate::extensions::recap::handle(self, &args).await,
            "hystersis.com/cloud/terminate" => {
                crate::extensions::auth_gate::require_hystersis_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `hystersis login` to authenticate.",
                )?;
            ...
            "hystersis.com/cloud/env/create" => {
                crate::extensions::auth_gate::require_hystersis_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `hystersis login` to authenticate.",
            ...
            "hystersis.com/cloud/env/update" => {
                crate::extensions::auth_gate::require_hystersis_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `hystersis login` to authenticate.",
            ...
            "hystersis.com/cloud/env/delete" => {
                crate::extensions::auth_gate::require_hystersis_auth(
                    &self.auth_manager,
                    "Authentication required",
                    "Run `hystersis login` to authenticate.",
            ...
            "hystersis.com/billing" => crate::extensions::billing::handle(self, &args).await,
            "hystersis.com/auto-topup-rule" => {
                crate::extensions::billing::handle(self, &args).await
            }
            "hystersis.com/share_session" => crate::extensions::share::handle(self, &args).await,
            "hystersis.com/privacy/setCodingDataRetention" => {
                crate::extensions::privacy::handle(self, &args).await
            }
            "hystersis.com/consent/record" => {
                crate::extensions::consent::handle(self, &args).await
            }
            "hystersis.com/rollout/survey" => {
                crate::extensions::rollout::handle(self, &args).await
            }
            "hystersis.com/prompt_history" => {
                crate::extensions::prompt_history::handle(self, &args).await
            }
            "hystersis.com/suggest" => crate::extensions::suggest::handle(self, &args).await,
            "hystersis.com/suggestPrompt" => crate::extensions::suggest::handle(self, &args).await,
            s if s.starts_with("hystersis.com/auth/") => {
                crate::extensions::auth::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/session_summaries/") => {
                crate::agent::handlers::session::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/git/worktree/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::worktree::handle(self, &ops, &args).await
            }
            s if s.starts_with("hystersis.com/git/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::git::handle(self, &ops, &args).await
            }
            s if s.starts_with("hystersis.com/compact_conversation") => {
                crate::extensions::memory::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/plugins/") => {
                crate::extensions::plugins::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/marketplace/") => {
                crate::extensions::marketplace::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/hooks/") => {
                crate::extensions::hooks::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/hunk-tracker/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::hunk_tracker::handle(self, &ops, &args).await
            }
            s if s.starts_with("hystersis.com/pr/") => {
            ...
            s if s.starts_with("hystersis.com/task/") => {
                crate::extensions::task::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/scheduler/") => {
                crate::extensions::task::handle_scheduler(self, &args).await
            }
            s if s.starts_with("hystersis.com/subagent/") => {
                crate::extensions::task::handle_subagent(self, &args).await
            }
            s if s.starts_with("hystersis.com/terminal/") => {
            ...
            s if s.starts_with("hystersis.com/search/") => {
                crate::extensions::search::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/bundle/") => {
                crate::extensions::bundle::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/code/") => {
                let ops = self.resolve_workspace_ops()?;
                crate::extensions::code_nav::handle(self, &ops, &args).await
            }
            s if s.starts_with("hystersis.com/skills/") || s == "hystersis.com/workflows/list" => {
            ...
            s if s.starts_with("hystersis.com/review") => {
                crate::extensions::feedback::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/debug/") => {
                crate::extensions::debug::handle(self, &args).await
            }
            s if s.starts_with("hystersis.com/rewind") => {
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "hystersis.com/yolo_mode_changed"
        ...
        if args.method.as_ref() == "hystersis.com/permissions/reset" {
            let mut updated = 0;
            self.session_registry
        ...
        if args.method.as_ref() == "hystersis.com/toggle_plan_mode"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref().starts_with("hystersis.com/queue/")
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref() == "hystersis.com/terminal/pty/input"
            && let Ok(params) = serde_json::from_str::<
                serde_json::Value,
            >(args.params.get())
        {
        ...
        if args.method.as_ref() == "_hystersis.com/session/update" {
        ...
        if args.method.as_ref() == "hystersis.com/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "hystersis.com/telemetry/multi_agent_followup" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| hystersis_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "hystersis.com/telemetry/multi_agent_apply" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "hystersis.com/telemetry/multi_agent_discard" {
            #[derive(serde::Deserialize)]
            ...
            >(args.params.get()) {
                ...
                hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| hystersis_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == hystersis_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                hystersis_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            hystersis_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
