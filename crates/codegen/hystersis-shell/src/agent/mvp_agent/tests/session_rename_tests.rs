//! `hystersis.com/session/rename` ext-handler coverage: resident `ManualTitleRenamed`
...
fn isolate_hystersis_home() -> IsolatedHome {
    let dir = tempfile::tempdir().unwrap();
    let env = EnvGuard::set("HYSTERSIS_HOME", dir.path());
    IsolatedHome {
        _dir: dir,
        _env: env,
    }
...
async fn rename_enqueues_manual_title_on_resident_persistence_tx() {
    let _home = isolate_hystersis_home();
...
async fn rename_fanout_stamps_title_is_manual_meta() {
    ...
    use hystersis_acp_lib::{AcpAgentGatewaySender as GatewaySender, AcpClientMessage};
    ...
    assert!(
        saw_manual_meta,
        "drive_rename must stamp _meta.hystersis.com/titleIsManual on SessionSummaryGenerated"
    );
}
...
async fn reset_fanout_stamps_title_is_manual_false() {
    ...
    assert!(
        saw_unpin_ext,
        "reset must stamp _meta.hystersis.com/titleIsManual=false on SessionSummaryGenerated"
    );
    assert!(
        saw_unpin_siu,
