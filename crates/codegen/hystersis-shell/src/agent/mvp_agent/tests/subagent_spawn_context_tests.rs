...
async fn subagent_spawn_context_inherits_parent_permission_handle() {
    use hystersis_workspace::permission::types::{
    ...
    local
        .run_until(async {
            ...
            let cwd = hystersis_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp"))
                .expect("absolute cwd");
            let (permission_handle, _events_rx) = hystersis_workspace::permission::spawn_permission_manager(
                sid.clone(),
                gateway,
                cwd,
                hystersis_workspace::permission::types::ClientType::Generic,
                Some(
            ...
            for access in [
                hystersis_workspace::permission::AccessKind::Read(Some(".env".into())),
                hystersis_workspace::permission::AccessKind::Bash("cat .env".into()),
            ] {
                ...
                assert!(
                    matches!(
                        decision,
                        hystersis_workspace::permission::Decision::PolicyDeny(_)
                    ),
...
async fn subagent_spawn_context_inherits_parent_configured_cutoff() {
    ...
    let cutoff = hystersis_sampling_types::ToolOverrides {
        x_search: Some(hystersis_sampling_types::XSearchOptions {
            date_bound: Some(
                hystersis_sampling_types::SearchDateBound::new(None, Some("2020-01-01".to_string()))
                    .unwrap(),
            ),
        }),
        web_search: None,
...
async fn subagent_spawn_context_inherits_parent_process_scope() {
    ...
    let mut handle = make_test_handle("test-model", false, None);
    let parent_scope = hystersis_tty_utils::ProcessScope::new();
    handle.tool_context.process_scope = Some(parent_scope.clone());
    agent.insert_resident(&sid, handle);
    let owner = std::sync::Arc::new(hystersis_tty_utils::ProcessGroup::new().expect("process group"));
...
fn subagent_spawn_context_resolves_compaction_mode_like_parent() {
    ...
    use hystersis_chat_state::{CompactionDetail, CompactionMode};
    use hystersis_test_support::EnvGuard;
    let _mode = EnvGuard::unset("HYSTERSIS_COMPACTION_MODE");
    let _detail = EnvGuard::unset("HYSTERSIS_COMPACTION_DETAIL");
    ...
    let _env_mode = EnvGuard::set("HYSTERSIS_COMPACTION_MODE", "transcript");
    assert_eq!(
        ctx.resolve_compaction_mode(),
        CompactionMode::Transcript,
        "HYSTERSIS_COMPACTION_MODE must win over parent config and remote"
    );
}
#[test]
