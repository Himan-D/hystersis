...
fn insert_applied_tool_overrides(
    ...
    echo: Option<&hystersis_sampling_types::ToolOverrides>,
) {
...
struct RestoredSignals {
    ...
    turn_count: u64,
    tool_call_count: u64,
    plan_mode_state: hystersis_telemetry::events::PlanModeState,
...
impl RestoredSignals {
    fn read(
        ...
        use hystersis_telemetry::events::PlanModeState as Reported;
        Self {
...
impl MvpAgent {
    pub(super) async fn new_session_inner(
        ...
        let spawn_res = {
            ...
            spawn_timer.with_subphase(hystersis_telemetry::startup::Subphase::SessionSpawn);
        ...
        if product_analytics || hystersis_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = hystersis_telemetry::context::collect_git_context(&cwd_str);
                let ev = hystersis_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                hystersis_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) =
            match hystersis_workspace::session::git::discover_git_root(cwd.as_path()) {
    ...
    async fn attach_session(
        ...
        let git_root =
            hystersis_workspace::session::git::find_git_root_from_path(cwd.as_path()).ok();
        ...
        persistence_timer.with_subphase(hystersis_telemetry::startup::Subphase::SessionLoad);
        ...
        let envrc = if !load_envrc {
            Some(hystersis_workspace::envrc::spawn_envrc_load(
        ...
        let (model_state, response_meta) = self
            ...
            .await;
        hystersis_telemetry::unified_log::info("session loaded", Some(session_id.0.as_ref()), None);
        ...
        if self.product_analytics_enabled() {
            log_event(hystersis_telemetry::events::SessionLoad {
    ...
    async fn restore_session_code(
        ...
        if restore_code_requested && registry_client_for_restore.is_none() {
            hystersis_workspace::session::git::warn_registry_disabled_restore(session_id.0.as_ref());
        }
        let restore_checkout_allowed =
            hystersis_workspace::session::git::restore_code_checkout_allowed(
                cwd.as_path(),
        ...
        {
            tracing::warn!(
                target: hystersis_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
        ...
        if restore_code_requested
            ...
            && let Some(ref target_sha) = summary.head_commit
        {
            use hystersis_workspace::session::git::RestoreKind;
            let outcome = hystersis_workspace::session::git::checkout_session_commit(
                cwd.as_path(),
                target_sha,
                true,
                session_id.0.as_ref(),
    ...
    async fn build_attach_response_meta(
        ...
        if summary.head_commit.is_some()
            ...
            && summary.git_root_dir.as_deref().is_none_or(|root| {
                hystersis_workspace::session::git::find_git_root_from_path(std::path::Path::new(
                    cwd.as_str(),
                ))
                .ok()
            ...
            })
        {
            ...
            git_scan_timer.with_subphase(hystersis_telemetry::startup::Subphase::SessionGitScan);
            let cwd_path = std::path::Path::new(cwd.as_str());
            let current_head =
                hystersis_workspace::session::git::git_cli(cwd_path, &["rev-parse", "HEAD"])
                    .await
                    .ok();
            if let Some(divergence) = hystersis_workspace::session::git::detect_head_divergence(
