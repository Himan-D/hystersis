...
use hystersis_telemetry::id::{agent_id, agent_instance_id};
use hystersis_telemetry::session_ctx::log_event;
use hystersis_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use hystersis_workspace::permission::{ClientType, PermissionEvent};
...
use hystersis_paths::AbsPathBuf;
use hystersis_workspace::session::git::GitDiscoveryResult;
use hystersis_hunk_tracker::HunkTrackerActor;
/// Hard-error message for legacy Direct hub-bind sessions (`hystersis.com/cloud_server_id`).
...
pub(crate) fn reject_direct_hub_cloud_meta(
    ...
    if session_meta.and_then(|m| m.get("hystersis.com/cloud_server_id")).is_some() {
...
/// If `persist_data` is provided, it will be included in the meta under `hystersis.com/persist`.
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::HystersisAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `HystersisProPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "HystersisPro" => jwt_claim == "hystersispro",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "HystersisProPro" => jwt_claim == "hystersispro_heavy",
        "HystersisProLite" => jwt_claim == "hystersispro_lite",
        "HystersisProPlus" => jwt_claim == "hystersispro_plus",
        _ => jwt_claim.parse::<u64>().is_ok_and(|n| n != 0),
    }
}
/// ACP `_meta` key for chat+local workspace intent (pager stamps on chat create).
#[cfg(feature = "local-workspace")]
const LOCAL_WORKSPACE_META_KEY: &str = "hystersis.com/local_workspace";
...
pub(crate) struct SessionSpawnOptions<'a> {
    ...
    pub envrc: Option<hystersis_workspace::envrc::EnvrcLoad>,
...
/// `_meta["hystersis.com/session"].kind` → [`SessionKind`]; absent/unknown/malformed → `Build`.
fn parse_session_kind(
    meta: Option<&acp::Meta>,
) -> crate::session::unified_list::SessionKind {
    use crate::session::unified_list::SessionKind;
    use serde::Deserialize;
    meta.and_then(|m| m.get("hystersis.com/session"))
...
pub(crate) const SESSION_PLUGIN_DIRS_CAPABILITY_KEY: &str = "hystersis.com/pluginDirs";
...
/// Used to stamp `hystersis.com/leaderClientId` onto replay notifications so the leader can
...
fn mark_as_replay(
    ...
    if let Some(persist) = persist_data {
        obj.insert("hystersis.com/persist".to_string(), persist.clone());
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a hystersis_sampling_types::TokenUsage>,
...
/// Typed payload for the `hystersis.com/settings/update` notification sent to pager
...
fn announcements_push_payload(
    stored: Option<&[hystersis_announcements::RemoteAnnouncement]>,
    last_emitted: &[hystersis_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<hystersis_announcements::RemoteAnnouncement>> {
    let current = hystersis_announcements::filter_expired_at(
...
/// Override with `HYSTERSIS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("HYSTERSIS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
/// gates fails.  Used in `hystersis.com/code/status` responses and to generate
...
pub(crate) enum CodeNavEligibility {
    ...
    ClientNotWeb,
    /// Client did not advertise `hystersis.com/codeNavigation.enabled`.
...
struct ResidentResources {
    ...
    codebase_index: Option<std::sync::Arc<hystersis_codebase_graph::IndexManagerHandle>>,
    is_headless: bool,
...
pub struct MvpAgent {
    ...
    /// hystersis.com chat-product catalog (`/rest/modes`) for chat sessions; distinct
    ...
    /// channels; a new `authenticate` or `hystersis.com/auth/cancel` cancels the
    /// prior attempt.
    ...
    /// Whether the current client advertised `hystersis.com/codeNavigation.enabled`.
    ...
    /// Whether the current client advertised `hystersis.com/folderTrust.interactive` (it
    ...
    /// list. Set by `enforce_hystersis_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.hystersis/`).
    ///
    ...
    /// Local session-registry override: `HYSTERSIS_SESSION_REGISTRY` env, else
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        hystersis_tools::implementations::hystersis_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: hystersis_tools::implementations::hystersis_build::monitor::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: hystersis_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled hystersis-desktop's first
    ...
    workspace_ops: RefCell<Option<hystersis_workspace::WorkspaceOps>>,
    ...
    /// Announcements list last actually emitted via `hystersis.com/announcements/update`
    ...
    last_emitted_announcements: RefCell<Vec<hystersis_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &hystersis_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<hystersis_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !hystersis_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = hystersis_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// unchanged — compatible stock switches (e.g. `hystersis` →
/// `hystersis-plan`) intentionally preserve the session's original ACP
/// `agentProfile`.
...
/// default wire format and toolset, so switching e.g. `hystersis` →
/// `hystersis-plan` doesn't require rebuilding the harness and would
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use hystersis_agent::config::is_strict_harness_agent_type;
    match (
...
use hystersis_chat_state::conversation_util::replace_or_insert_system_head;
...
struct AuthRequestMeta {
    ...
    /// Pager auth `request_seq` for this attempt. Scopes `hystersis.com/auth/cancel`
...
pub(crate) fn subagent_rate_limit_max_attempts_env() -> Option<u32> {
    parse_subagent_rate_limit_max_attempts(
        std::env::var("HYSTERSIS_SUBAGENT_RATE_LIMIT_MAX_ATTEMPTS").ok().as_deref(),
    )
}
...
fn parse_subagent_rate_limit_max_attempts(raw: Option<&str>) -> Option<u32> {
    ...
    match value.parse::<u32>() {
        ...
        Err(_) => {
            tracing::warn!(
                value,
                "ignoring invalid HYSTERSIS_SUBAGENT_RATE_LIMIT_MAX_ATTEMPTS"
            );
            None
        }
    }
...
/// Parse the client-advertised `hystersis.com/hunkTracker.mode` string. Case-insensitive
/// and trimmed. Absent/blank/`off`/`disabled` => `None`; unknown => `AllDirty`.
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<hystersis_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(hystersis_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<hystersis_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        session_id: &acp::SessionId,
        updates_file_path: &Option<PathBuf>,
    ) -> Vec<tokio::sync::oneshot::Receiver<hystersis_acp_lib::AcpResult<()>>> {
        ...
        for task in &orphaned {
            let snapshot = hystersis_tools::types::TaskSnapshot {
    ...
    pub(super) async fn enforce_hystersis_code_access(&self, auth: &crate::auth::HystersisAuth) {
        ...
        *self.allow_access_resolved_for.borrow_mut() = Some(auth.user_id.clone());
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings hystersis_build_access_gate)"
            );
            self.retry_subscription_check().await;
        }
    }
    /// Single-shot subscription check called by the pager's "Check
    /// subscription" button (`hystersis.com/auth/check_subscription`). The pager
    ...
    pub(super) fn start_search_index_once(&self) {
        ...
        if let crate::session::storage::search::IndexDecision::On(index) = self
            .search_index()
        {
            index.bootstrap_once(crate::util::hystersis_home::hystersis_home());
        }
    }
    ...
    /// The caller resolves the home: read here, $HYSTERSIS_HOME would be read when the
    /// blocking thread starts, and this deletes worktrees under what it finds.
    pub(super) fn reclaim_worktrees(
        hystersis_home: anyhow::Result<std::path::PathBuf>,
        policy: hystersis_fast_worktree::ResolvedWorktreeAutoGc,
    ) {
        if let Err(e) = hystersis_home
            .and_then(|home| hystersis_fast_worktree::WorktreeDb::open(&home))
            .and_then(|db| hystersis_fast_worktree::maybe_auto_gc(&db, &policy))
        {
            tracing::warn!(error = %e, "auto worktree gc failed");
        }
    }
    /// Fire-and-forget `hystersis.com/settings/update` from the current remote snapshot.
        ...
        if let Ok(params) = serde_json::value::to_raw_value(&payload) {
            self.gateway
                .forward_fire_and_forget(
                    acp::ExtNotification::new("hystersis.com/settings/update", params.into()),
                );
        }
    }
    ...
    pub(super) fn spawn_tier_recheck(&self) {
        ...
        tokio::task::spawn_local(async move {
            ...
            agent_ref.get().enforce_hystersis_code_access(&auth).await;
        });
    }
    /// Spawn a best-effort bundle sync. Re-fires on every call site (init,
    /// cached_token, hystersis.com/oidc); the cheap pre-checks below absorb repeats
    ...
    ///    against `~/.hystersis/bundled/` and the manifest.
