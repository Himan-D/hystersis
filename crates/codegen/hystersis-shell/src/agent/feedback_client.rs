...
// ============================================================================
// Turn delta wire types (local to xai-hystersis-shell until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::hystersis_auth_credentials::HystersisAuthCredentials,
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials = crate::util::hystersis_auth_credentials::HystersisAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(hystersis_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::hystersis_auth_credentials::HystersisAuthCredentials,
    ) -> Arc<dyn hystersis_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(hystersis_auth::StaticAuthCredentialProvider::new(
    ...
    fn record_401_attribution_if_needed(
        ...
        stamp: Option<&hystersis_auth::StampedBearerSuffix>,
        op: &str,
    ) {
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, hystersis_version::VERSION)
            .header(
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = hystersis_file_utils::trace_context::inject_trace_context_into_request(request);
        let req = request.build().context(context)?;
        let (response, stamp) = hystersis_auth::execute_with_stamp(&self.client, req)
            .await
            .context(context)?;
...
mod tests {
    ...
    fn test_signals_to_update() {
        let signals = crate::session::signals::SessionSignals {
            ...
            models_used: vec!["hystersis-3".to_string()],
            primary_model_id: Some("hystersis-3".to_string()),
        ...
        assert_eq!(update.primary_model_id, Some("hystersis-3".to_string()));
