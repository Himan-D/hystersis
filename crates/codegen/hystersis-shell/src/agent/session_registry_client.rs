...
impl SessionRegistryClient {
    pub fn new(base_url: impl Into<String>, user_token: impl Into<String>) -> Self {
        ...
        Self {
            ...
            credentials: crate::util::hystersis_auth_credentials::HystersisAuthCredentials::new(Some(
                user_token.into(),
            )),
            session_id: None,
        }
    ...
    async fn send_authed(
        ...
        reqwest::Response,
        Option<hystersis_auth::StampedBearerSuffix>,
    )> {
        let builder = hystersis_file_utils::trace_context::inject_trace_context_into_request(builder);
        let request = builder.build().context(op)?;
        hystersis_auth::execute_with_stamp(&self.client, request)
            .await
            .map_err(|e| match e {
    ...
    /// [`hystersis_auth::StampedBearerSuffix`] for why never a re-resolution).
    fn record_401_attribution(&self, op: &str, stamp: Option<&hystersis_auth::StampedBearerSuffix>) {
