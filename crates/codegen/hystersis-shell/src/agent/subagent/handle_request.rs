...
use hystersis_tools::implementations::{hystersis_build, opencode};
...
/// `xai-hystersis-tools` coordinator actor and reached only through `reporter`.
#[tracing::instrument(
...
pub(crate) async fn run_shell_child(
    run: hystersis_build::task::coordinator::ChildRunRequest<ShellChildRuntime>,
    mut ctx: SubagentSpawnContext,
    gateway: GatewaySender,
) -> ChildRunOutput<ShellCompletionData> {
    let hystersis_build::task::coordinator::ChildRunRequest {
        mut request,
    ...
    let spawn_timer = hystersis_telemetry::subagent_spawn::SubagentSpawnTimer::new_shared();
    ...
    let mut effective_runtime = hystersis_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = hystersis_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != hystersis_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != hystersis_tool_types::SubagentIsolationMode::None {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(&source_cwd) {
            ...
            Err(e) => {
                ...
                std::env::temp_dir()
                    .join("hystersis-subagent-worktrees")
                    .join(&request.id)
            }
        };
        let source_clone = source_cwd;
        let subagent_id = request.id.clone();
        let creation_mode: hystersis_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
            let mut builder = hystersis_fast_worktree::WorktreeBuilder::new(&source_clone, &dest)
                .working_tree_mode(hystersis_fast_worktree::WorkingTreeMode::PreserveWorkingTree)
                .creation_mode(creation_mode)
                .worktree_kind(hystersis_fast_worktree::WorktreeKind::Subagent)
    ...
    effective_runtime.capability_mode = hystersis_subagent_resolution::intersect_capability_modes(
    ...
    hystersis_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    if let Err(e) = crate::util::hystersis_home::ensure_sessions_cwd_dir(&child_session_info.cwd) {
    ...
    {
        let reminder = hystersis_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let covered_by_parent = hystersis_fsnotify::watch_root_covers(&ctx.parent_cwd, &child_cwd);
    ...
    let child_cwd_abs = hystersis_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        hystersis_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let parent_traceparent = hystersis_file_utils::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = hystersis_chat_state::estimate_conversation_tokens(&forked_conversation);
    ...
    let attribution_callback: Option<hystersis_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = hystersis_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<hystersis_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                hystersis_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == hystersis_hooks::event::HookEventName::Stop {
                            s.event = hystersis_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if !agent_owned_mcp_servers_allowed(is_plugin_agent) {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    hystersis_agent::config::McpServerRef::Named(name) => {
                    ...
                    hystersis_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            hystersis_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    let _active = hystersis_telemetry::activity::SUBAGENTS_ACTIVE.enter();
    debug_assert!(
        hystersis_telemetry::activity::SUBAGENTS_ACTIVE.get() >= 1,
        "SubagentLaunched must stamp a self-inclusive count"
    );
    hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        hystersis_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
        pins.mode,
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            hystersis_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        ...
        if inherit_skills {
            ...
            hystersis_agent::prompt::skills::SkillsConfig::default()
        },
        if inherit_skills {
        ...
        ctx.yolo_mode
            || matches!(
                ...
                hystersis_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        hystersis_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        hystersis_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        hystersis_telemetry::events::Outcome::Cancelled
    } else {
        hystersis_telemetry::events::Outcome::Error
    };
    let mut completed = hystersis_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        owner: telemetry_owner_kind(&request),
        workflow_run_id: request.owner.workflow_run_id().map(str::to_string),
    ...
    hystersis_telemetry::session_ctx::log_event(completed);
    match (
...
pub(crate) async fn dispose_worktree_after_completion(
    ...
    let ref_name = format!("refs/hystersis/subagents/{subagent_id}");
    ...
    let reclaim = tokio::task::spawn_blocking(move || {
        hystersis_fast_worktree::reclaimable_after_snapshot(
            &checked_path,
    ...
    .await;
    match reclaim {
        Ok(hystersis_fast_worktree::Reclaim::Now { .. }) => {}
        Ok(hystersis_fast_worktree::Reclaim::Keep(reason)) => {
            tracing::info!(
                subagent_id = %subagent_id,
                worktree_path = %worktree.display(),
                %reason,
        ...
        Ok(hystersis_fast_worktree::Reclaim::Unnamed(error)) => {
            tracing::warn!(
                subagent_id = %subagent_id,
                worktree_path = %worktree.display(),
                %error,
