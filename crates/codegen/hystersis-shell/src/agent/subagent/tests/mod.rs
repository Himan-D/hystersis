...
use hystersis_subagent_resolution::resolve_effective_overrides;
use hystersis_tools::implementations::hystersis_build::task::coordinator::{
    ChildCompletion, CompletionDisposition,
};
#[test]
fn canonical_total_tokens_does_not_double_count_reasoning() {
    let totals = hystersis_chat_state::UsageTotals {
        input_tokens: 100,
        output_tokens: 40,
        reasoning_tokens: 25,
        ..Default::default()
...
async fn usage_ack_precedes_terminal_presentation() {
    ...
    let by_model = vec![(
            ...
            hystersis_chat_state::UsageTotals {
                input_tokens: 10,
                output_tokens: 4,
                ..Default::default()
            },
...
async fn subagent_inherits_session_cli_overrides() {
    use hystersis_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    use hystersis_agent::config::PermissionMode;
    const PIN: &str = hystersis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
async fn emit_subagent_notification_stamps_one_event_id_on_both_paths() {
    ...
    let broadcast_id = match gateway_rx.try_recv().expect("broadcast must fire") {
        hystersis_acp_lib::AcpClientMessage::ExtNotification(args) => {
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
            resume_worktree_action(true, Some("refs/hystersis/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
            resume_worktree_action(false, Some("refs/hystersis/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let reservations = hystersis_tools::reminders::task_completion::TaskCompletionReservations::default();
...
fn partial_override_fills_from_role() {
    ...
    let role = hystersis_subagent_resolution::config::SubagentRole {
    ...
    assert_eq!(
            ...
            Some(hystersis_tool_types::SubagentCapabilityMode::Execute)
        );
}
#[test]
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            hystersis_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = hystersis_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = hystersis_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
            ...
            ConversationItem::Reasoning(hystersis_sampling_types::synthesized_reasoning_item(
                "deliberating",
            )),
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use hystersis_sampling_types::conversation::{
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use hystersis_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
            ConversationItem::system("parent system"),
...
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("HystersisBuild:read_file");
...
/// Regression: on the DEFAULT hystersis host —
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    ...
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::HystersisBuildPlan.as_ref().to_string(),
    );
...
async fn cancel_pending_at_promote_removes_fresh_worktree_preserves_resumed() {
    ...
    hystersis_fast_worktree::WorktreeBuilder::new(&repo, &fresh)
        .standalone(true)
        .create()
        .unwrap();
    assert!(fresh.exists());
    ...
    hystersis_fast_worktree::WorktreeBuilder::new(&repo, &resumed)
        .standalone(true)
        .create()
        .unwrap();
...
fn subagent_auth_type_rule() {
    ...
    let byok = byok_model_entry("hystersis-byok");
    let plain = test_model_entry("hystersis-plain");
    assert_eq!(
...
fn fresh_tool_model_accepts_visible_key_and_internal_id() {
    ...
    models.insert("hystersis-3".to_string(), test_model_entry("hystersis-3-2025-02-15"));
    assert!(
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(!error.contains("hystersis models"));
    }
    assert!(
            super::handle_request::task_model_override_error(
                Some("oauth-only"),
...
fn test_sampling_config(model_slug: &str) -> hystersis_sampling_types::SamplingConfig {
...
fn spawn_test_parent_chat_state(model_slug: &str) -> hystersis_chat_state::ChatStateHandle {
    let (mock, _persistence_rx) = hystersis_chat_state::MockChatPersistence::new();
    let (event_tx, _event_rx) = mpsc::unbounded_channel();
    let token = tokio_util::sync::CancellationToken::new();
    hystersis_chat_state::ChatStateActor::spawn(
        vec![],
