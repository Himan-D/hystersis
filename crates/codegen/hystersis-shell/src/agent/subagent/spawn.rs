...
//!    here, which drives the coordinator (living in `xai-hystersis-tools`).
...
pub(crate) use hystersis_tools::implementations::hystersis_build::task::coordinator::{
    self, ChildCompletion, ChildControl, ChildRunOutput, LocalBoxFuture, StartedChild,
    SubagentProgress,
};
use hystersis_tools::implementations::hystersis_build::task::types::{SubagentRequest, SubagentResult};
...
/// `HYSTERSIS_SUBAGENT_WORKER_THREADS` overrides.
...
fn build_worker_runtime() -> std::io::Result<tokio::runtime::Runtime> {
    let workers = std::env::var("HYSTERSIS_SUBAGENT_WORKER_THREADS")
        .ok()
...
impl coordinator::ChildRunner for ShellChildRunner {
    ...
    type ValidateFuture = coordinator::LocalBoxFuture<
        hystersis_tools::implementations::hystersis_build::task::types::SubagentValidateTypeOutcome,
    >;
    type DescribeFuture = coordinator::LocalBoxFuture<
        hystersis_tools::implementations::hystersis_build::task::types::SubagentDescribeOutcome,
    >;
        ...
        Box::pin(async move {
            ...
            let Some(mut ctx) = this.try_build_subagent_spawn_context(&parent_sid) else {
                ...
                return coordinator::ChildRunOutput {
                    result: hystersis_tools::implementations::hystersis_build::task::types::SubagentResult {
                        success: false,
                        error: Some(
        ...
        Box::pin(async move {
            ...
            match this.try_build_subagent_spawn_context(&parent_session_id) {
                ...
                None => {
                    ...
                    hystersis_tools::implementations::hystersis_build::task::types::SubagentDescribeOutcome::Unavailable
                }
            }
        })
    }
...
fn log_limit_notice(notice: coordinator::SubagentLimitNotice) {
    ...
    hystersis_telemetry::session_ctx::log_event(SubagentLimitHit::session_concurrent(
...
pub(crate) fn spawn_subagent_coordinator(
    ...
    >,
    limits: hystersis_tools::implementations::hystersis_build::task::admission::SubagentLimits,
) {
    ...
    let config = coordinator::CoordinatorConfig {
        foreground_budget:
            hystersis_tools::implementations::hystersis_build::task::backend::env_duration_or(
                "HYSTERSIS_SUBAGENT_AWAIT_BUDGET_MS",
...
pub(crate) struct InjectParams<'a> {
    ...
    pub task_completion_reservations:
        &'a Option<hystersis_tools::reminders::task_completion::TaskCompletionReservations>,
...
pub(crate) fn inject_subagent_completed_prompt(params: InjectParams) {
    ...
    let summary =
        hystersis_tools::implementations::hystersis_build::task::completion_summary(request, result);
    let message = hystersis_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = hystersis_tools::reminders::wrap_reminder(&message);
