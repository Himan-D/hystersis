...
//! `xai-hystersis-tools` coordinator actor. This module keeps shell-specific
...
use hystersis_acp_lib::AcpAgentGatewaySender as GatewaySender;
use hystersis_agent::config::{McpInheritance, ModelOverride, PermissionMode};
use hystersis_sampling_types::conversation::ConversationItem;
use hystersis_session_events::types::CancellationCategory;
use hystersis_subagent_resolution::ResumeSourceData;
use hystersis_tools::implementations::hystersis_build::monitor::types::MonitorEventBuffer;
use hystersis_tools::implementations::hystersis_build::task::types::*;
use hystersis_tools::types::tool::ToolKind;
use hystersis_workspace::file_system::AsyncFileSystem;
use hystersis_hunk_tracker::HunkTrackerHandle;
mod attempt_runner;
mod spawn;
pub(crate) use spawn::{
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn hystersis_tools::implementations::lsp::LspBackend>>,
    ...
    /// (not an intermediate parent's) because xai-hystersis-tools task/coordinator.rs
    ...
    pub process_scope: Option<hystersis_tty_utils::ProcessScope>,
    ...
    pub sampling_config: hystersis_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::HystersisAuth>,
    pub parent_cwd: PathBuf,
    pub parent_session_id: String,
    /// The parent's cutoff at spawn, applied to the child's first turn. `None` if unset.
    pub inherited_tool_overrides: Option<hystersis_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    ...
    pub parent_terminal_backend: Option<Arc<dyn hystersis_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<hystersis_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<hystersis_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: hystersis_tools::implementations::hystersis_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: hystersis_tools::implementations::hystersis_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: hystersis_tools::implementations::hystersis_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<hystersis_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<hystersis_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<hystersis_tools::registry::types::ToolConfig>>,
    ...
    pub hook_registry: Option<std::sync::Arc<hystersis_hooks::discovery::HookRegistry>>,
    pub permission_handle: Option<hystersis_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<hystersis_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: hystersis_workspace::WorkspaceOps,
    ...
    pub attribution_callback: Option<hystersis_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "hystersis").
    ...
    pub parent_tool_definitions: Option<Vec<hystersis_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<hystersis_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: hystersis_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: hystersis_tools::types::compat::CompatConfig,
    /// Shared completion reservations held by auto-wake prompts.
    pub task_completion_reservations:
        Option<hystersis_tools::reminders::task_completion::TaskCompletionReservations>,
...
pub(crate) fn strip_ask_user_question_tool(tools: &mut Vec<hystersis_sampling_types::ToolSpec>) {
    tools.retain(|tool| tool.name != "ask_user_question");
}
pub(crate) fn strip_workflow_tool(tools: &mut Vec<hystersis_sampling_types::ToolSpec>) {
    tools.retain(|tool| {
        !hystersis_tools::implementations::hystersis_build::is_workflow_tool_id(&tool.name)
    });
}
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut hystersis_agent::config::AgentDefinition) {
    ...
    pub(crate) fn snapshot_parent_compaction_pins(
        resolved_mode: hystersis_chat_state::CompactionMode,
        ...
        let current = parent_agent_name
            .and_then(|name| hystersis_agent::discovery::by_name_in_cwd(name, parent_cwd))
    ...
    pub(crate) fn compaction_pins_for_child(
        &self,
        child_template: &hystersis_agent::prompt::user_message::UserMessageTemplate,
    ) -> crate::session::CompactionPins {
        crate::session::cursor_compaction_pins(
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> hystersis_chat_state::CompactionMode {
        crate::agent::config::resolve_compaction_mode_from(
            crate::agent::config::env_string("HYSTERSIS_COMPACTION_MODE").as_deref(),
            self.agent_config
                .as_ref()
        ...
        .with_segment_detail(crate::agent::config::resolve_compaction_detail_from(
            crate::agent::config::env_string("HYSTERSIS_COMPACTION_DETAIL").as_deref(),
            self.agent_config
                .as_ref()
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &hystersis_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (hystersis_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &hystersis_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (hystersis_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &hystersis_sampler::SamplerConfig,
    resolved_id: &acp::ModelId,
    parent: &hystersis_sampler::SamplerConfig,
) {
...
async fn read_parent_sampling_config(
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let inherited = hystersis_sampler::SamplerConfig {
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(hystersis_sampler::SamplerConfig, acp::ModelId)> {
    ...
    config.bearer_resolver = if !ctx.would_strip_fallback_key(config.api_key.as_deref())
        && resolved_auth_type == hystersis_chat_state::AuthType::SessionToken
    {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[hystersis_sampling_types::conversation::ConversationItem],
) -> usize {
    conversation
        .iter()
...
struct InitialContext {
    ...
    conversation: Vec<hystersis_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<hystersis_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        hystersis_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[hystersis_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<hystersis_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = hystersis_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        hystersis_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
    ...
    if let Some(source) = resume_source {
            ...
            .await
        {
            Ok(result) => {
                ...
                let estimated_tokens = hystersis_chat_state::estimate_conversation_tokens(&conversation);
...
fn resolve_inherited_mcp_pool(
    ...
    inheritance: &hystersis_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<hystersis_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = hystersis_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def = hystersis_subagent_resolution::discover_agent_definition(
        subagent_type,
        &resolution_context,
    )?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    hystersis_subagent_resolution::available_agent_names(
        &hystersis_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<hystersis_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = hystersis_subagent_resolution::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match hystersis_subagent_resolution::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(hystersis_subagent_resolution::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(hystersis_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(hystersis_subagent_resolution::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            hystersis_subagent_resolution::ResolutionError::PersonaResolution(_)
            | hystersis_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match hystersis_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(hystersis_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            hystersis_subagent_resolution::ResolutionError::Unknown { .. }
            | hystersis_subagent_resolution::ResolutionError::PersonaResolution(_)
            | hystersis_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    hystersis_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
/// Apply the harness-dependent toolset/prompt re-selection to a resolved
/// agent definition.
///
/// The harness flavor (alternate vs hystersis) normally follows the PARENT
/// agent: `HystersisBuildOrchestrator` parents give children
...
/// (so a hystersis session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut hystersis_agent::config::AgentDefinition,
) {
    let resolution_context = hystersis_subagent_resolution::HarnessToolsetContext {
    ...
    hystersis_subagent_resolution::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
...
fn summarize_tool_config(
    config: &hystersis_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_permission_mode(
    requested: hystersis_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> hystersis_agent::config::PermissionMode {
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    hystersis_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
fn cancellation_error_message(
    category: Option<hystersis_session_events::types::CancellationCategory>,
...
fn telemetry_owner_kind(
    request: &SubagentRequest,
) -> hystersis_telemetry::events::SubagentOwnerKind {
    if request.owner.is_workflow() {
        hystersis_telemetry::events::SubagentOwnerKind::Workflow
    } else if request.from_scheduler_loop() {
        hystersis_telemetry::events::SubagentOwnerKind::SchedulerLoop
    } else {
        hystersis_telemetry::events::SubagentOwnerKind::Task
    }
}
...
/// `hystersis.com/subagent/list_running` on reconnect.
#[allow(clippy::too_many_arguments)]
...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    backend: &hystersis_tools::implementations::hystersis_build::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    gateway: &GatewaySender,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
