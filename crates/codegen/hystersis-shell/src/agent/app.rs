...
use crate::auth::{AuthManager, HystersisAuth, HystersisComConfig, run_auth_flow};
...
/// or live subagent). The second signal covers relay-driven (hystersis.com
...
fn spawn_agent_local(
    ...
    tokio::task::spawn_local(
        GatewayReceiver::new(gw_rx, conn)
            .with_on_meta(hystersis_file_utils::trace_context::span_from_meta_traceparent)
            .run(),
    );
    handle_io
}
...
/// Start a skills file watcher and wire it to inject `hystersis.com/internal/reload_skills`
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = hystersis_agent::prompt::workspace_user::optional_workspace_user_dir();
...
/// ([`hystersis_fsnotify::shared`]) run their event loops on a runtime that outlives
...
fn register_fs_watch_runtime() {
    hystersis_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
pub async fn run_stdio_agent(
    ...
    hystersis_telemetry::unified_log::set_version(hystersis_version::VERSION);
    hystersis_file_utils::queue::cleanup_orphaned_uploads(
        &hystersis_home::hystersis_home(),
        hystersis_file_utils::queue::DEFAULT_MAX_AGE,
    );
    if let Ok(version) = std::env::var("HYSTERSIS_CLIENT_VERSION") {
        crate::unified_log::info(
            "HYSTERSIS_CLIENT_VERSION",
            None,
    ...
    let result = local_set
        .run_until(async move {
            ...
            apply_otel_config(&auth_manager, &agent_config.hystersis_com_config);
...
pub async fn run_headless(
    ...
    const HEADLESS_NO_SESSION: &str = "Headless mode requires a hystersis.com session. \
        Run `hystersis login` to sign in, or use `hystersis agent stdio` for API-key access.";
    hystersis_file_utils::queue::cleanup_orphaned_uploads(
        &hystersis_home::hystersis_home(),
        hystersis_file_utils::queue::DEFAULT_MAX_AGE,
    );
    ...
    let ctx = &agent_config.hystersis_com_config;
    let (mut auth, did_browser_flow) = if reauthenticate {
        let auth_manager = Arc::new(AuthManager::new(&hystersis_home::hystersis_home(), ctx.clone()));
        run_auth_flow(
            &auth_manager,
            ctx,
            true,
        ...
        let auth_manager = Arc::new(AuthManager::new(&hystersis_home::hystersis_home(), ctx.clone()));
        if crate::agent::auth_method::has_hystersis_api_key_env()
    ...
    let hystersis_code_url = format!("{}/build", ctx.hystersis_ws_origin);
    let on_first_connect: Box<dyn FnOnce() + Send + 'static> = Box::new(move || {
        if !did_browser_flow {
            eprintln!();
            eprintln!(
                "Open Hystersis: {} (press Enter to open in browser)",
                hystersis_code_url
            );
            eprintln!();
            let url_for_open = hystersis_code_url.clone();
        ...
        .run_until(async move {
            let _agent_handle = tokio::task::spawn_local(async move {
                ...
                tokio::task::spawn_local(
                    GatewayReceiver::new(gw_rx, conn)
                        .with_on_meta(
                            hystersis_file_utils::trace_context::span_from_meta_traceparent,
                        )
                        .run(),
                );
...
fn should_seed_shared_session(existing: Option<&HystersisAuth>, session: &HystersisAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
/// Start the leader's hystersis.com relay connection according to the start policy,
/// parking the [`RelayHandle`](crate::agent::relay::RelayHandle) in `slot`
/// once the connection task is running.
///
/// * `relay_on_demand == false` (default — explicit `hystersis agent leader`
...
///   duplication of mirroring every agent message to hystersis.com.
///
...
/// Everything needed to arm the leader's hystersis.com relay *after* startup.
///
...
struct DeferredRelayArm {
    ...
    slot: Rc<std::cell::RefCell<Option<crate::agent::relay::RelayHandle>>>,
    hystersis_com_config: crate::auth::HystersisComConfig,
...
impl DeferredRelayArm {
    ...
    /// [`spawn_leader_relay`]).
    fn arm_if_eligible(self, session: &HystersisAuth, auth_manager: &Arc<AuthManager>) -> Option<Self> {
        ...
        info!("Relay-eligible auth token appeared after startup — arming hystersis.com relay");
        spawn_leader_relay(
            self.slot,
            relay_config,
...
/// the agent boundary.
pub fn apply_otel_config(auth_manager: &AuthManager, hystersis_com_config: &HystersisComConfig) {
    suppress_otel();
    ...
    if crate::agent::otel_gate::should_open_at_startup(crate::agent::otel_gate::StartupGate {
        ...
        has_session,
        session_pending: crate::agent::otel_gate::is_session_pending(has_session, hystersis_com_config),
    }) {
        crate::agent::otel_gate::open_at_startup();
    }
}
/// Run the agent in leader mode, accepting IPC connections from multiple clients.
/// When a hystersis.com session is present, the leader connects to the websocket relay
...
/// * `relay_on_demand` - If true, defer the hystersis.com relay WebSocket until the
...
pub async fn run_leader(
    ...
    let control_state = LeaderServerControlState::new(LeaderServerMetadata {
        ...
        leader_binary_version: hystersis_version::VERSION.to_string(),
    })
    ...
    let ctx = &agent_config.hystersis_com_config;
    suppress_otel();
    let auth: Option<HystersisAuth> = crate::auth::try_noninteractive_auth_no_mint(ctx).await;
    let has_session = auth.is_some()
        || agent_config
            ...
            .is_some();
    let session_pending =
        crate::agent::otel_gate::is_session_pending(has_session, &agent_config.hystersis_com_config);
    let policy_channel =
    ...
    local_set
        .run_until(async move {
                ...
                tokio::task::spawn_local(async move {
                    tokio::select! {
                        ...
                        minted = crate::auth::mint_session_noninteractive(&mint_auth_manager)
                            => match minted {
                            Some(session) => info!(
                                is_xai = session.is_hystersis_auth(),
            ...
            if let Some(relay_config) = relay_config {
                ...
                info!(
                    "Relay not started: no hystersis.com session token \
                ...
                deferred_relay_arm = Some(DeferredRelayArm {
                    ...
                    hystersis_com_config: agent_config.hystersis_com_config.clone(),
            ...
            let auth_scope = agent_config.hystersis_com_config.auth_scope();
            let initial_auth_key_hash = hystersis_config::user_hystersis_home()
                    ...
                    match update {
                        ...
                        ConfigUpdate::Ui { theme, yolo, fork_secondary_model } => {
                            ...
                            let notification = serde_json::json!({
                                "jsonrpc": "2.0",
                                "method": "hystersis.com/config_changed",
                                "params": {
                                    "section": "ui",
                                    "changes": {
                                        "theme": theme,
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> HystersisAuth {
        HystersisAuth {
            key: key.into(),
    ...
    /// a relay-eligible hystersis.com OIDC session.
    fn test_relay_config(addr: std::net::SocketAddr) -> crate::agent::relay::RelayConfig {
        ...
        let cfg = crate::auth::HystersisComConfig {
            hystersis_ws_url: format!("ws://{addr}"),
            hystersis_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
        crate::agent::relay::RelayConfig::for_session(&auth, &cfg, None, None)
            .expect("hystersis.com OIDC session must be relay-eligible")
    }
    ...
    fn embedded_otel_gate_keeps_a_session_user_fail_closed() {
        ...
        use hystersis_telemetry::external::{
        ...
        const PROXY_ENV_VAR: &str = "HYSTERSIS_CLI_CHAT_PROXY_BASE_URL";
        ...
        let with_session = {
            ...
            let am = Arc::new(AuthManager::new(dir.path(), HystersisComConfig::default()));
            am.hot_swap(session);
            am
        };
    ...
    /// `hystersis agent leader` (devbox/systemd — no local IPC clients,
    /// `relay_on_demand == false`) must connect the hystersis.com relay eagerly.
    ...
    async fn deferred_arm_connects_relay_when_auth_appears() {
        ...
        let (_demand_tx, demand_rx) = watch::channel(false);
        let slot = Rc::new(std::cell::RefCell::new(None));
        let hystersis_com_config = crate::auth::HystersisComConfig {
            hystersis_ws_url: format!("ws://{addr}"),
            hystersis_ws_origin: format!("http://{addr}"),
            ..Default::default()
        };
        let tmp = tempfile::tempdir().unwrap();
        let auth_manager = Arc::new(AuthManager::new(tmp.path(), hystersis_com_config.clone()));
        ...
        local
            .run_until(async {
                let ineligible = HystersisAuth::test_default();
                let arm = arm
    ...
    fn internal_reload_request_line_carries_id_params_and_newline() {
        ...
        assert_eq!(
            msg["method"], "_hystersis.com/internal/reload_models",
        ...
        let msg: serde_json::Value = serde_json::from_str(line.trim_end()).unwrap();
        assert_eq!(msg["method"], "_hystersis.com/internal/auth_cleared");
    }
    #[tokio::test]
