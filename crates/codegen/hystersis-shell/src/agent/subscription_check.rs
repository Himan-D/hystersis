...
//!
//! The pager drives the polling via `hystersis.com/auth/check_subscription`: the 5s
...
/// Whether a `/user?include=subscription` tier qualifies for Hystersis
...
async fn fetch_user_info(
    ...
    let request = http_client
        ...
        .header(
            "X-XAI-Token-Auth",
            auth_manager.hystersis_com_config().token_header.as_str(),
        )
...
/// the paywall is shown (`hystersis.com/auth/check_subscription`).
///
