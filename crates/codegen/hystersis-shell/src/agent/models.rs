...
use hystersis_sampling_types::{ReasoningEffort, ReasoningEffortOption};
...
impl ModelFetchAuth {
    ...
    pub(crate) fn resolve(endpoints: &config::EndpointsConfig, has_cached_session: bool) -> Self {
        if endpoints.has_custom_endpoint() {
            ...
            Self::Deployment
        } else if crate::agent::auth_method::has_hystersis_api_key_env() {
            Self::ApiKey
        } else {
            Self::Session
        }
...
struct Inner {
    ...
    gateway: RwLock<Option<hystersis_acp_lib::AcpAgentGatewaySender>>,
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&hystersis_home, HystersisComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: hystersis_acp_lib::AcpAgentGatewaySender) {
    ...
    /// Hot-reload the catalog from `~/.hystersis/models_cache.json` after an external write (config-watcher detected).
    ...
    async fn bounded_startup_auth(auth_manager: &Arc<AuthManager>) -> Option<HystersisAuth> {
    ...
    /// without a live IdP.
    async fn bounded_auth_refresh<F>(fut: F) -> Option<HystersisAuth>
    where
        F: std::future::Future<Output = Option<HystersisAuth>>,
    {
