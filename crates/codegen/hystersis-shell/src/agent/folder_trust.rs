...
//! interactive prompt, and the durable [`hystersis_workspace::trust::TrustStore`]
...
//! on [`hystersis_workspace::folder_trust::decide`]; the consume-side nuance is
...
use hystersis_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};
...
// `hystersis_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use hystersis_workspace::folder_trust::grant_folder_trust;
use hystersis_workspace::folder_trust::{
...
// (`hystersis_agent::plugins::TrustStore` at `~/.hystersis/trusted-plugins`, plus the
...
/// [`hystersis_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub(crate) fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = hystersis_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// ([`hystersis_workspace::folder_trust::repo_config_kinds`] /
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    hystersis_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn agent_inline_hooks_allowed(
    scope: hystersis_agent::config::AgentScope,
    trusted: impl FnOnce() -> bool,
) -> bool {
    scope != hystersis_agent::config::AgentScope::Project || trusted()
}
...
pub(crate) fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            hystersis_tools::implementations::lsp::config::LspServerConfig,
            hystersis_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, hystersis_tools::implementations::lsp::config::LspServerConfig>
{
    hystersis_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use hystersis_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(hystersis_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        let env = hystersis_workspace::envrc::load_envrc_or_empty(tmp.path());
        assert_eq!(
            env.get("GATED_ENVRC"),
            Some(&"1".to_string()),
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // HYSTERSIS_FOLDER_TRUST unset so the default-on feature flag applies.
        use hystersis_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("HYSTERSIS_HOME", home.path());
        let _flag = EnvGuard::unset("HYSTERSIS_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn project_agent_inline_hooks_gated_when_untrusted_but_user_kept() {
        ...
        use hystersis_agent::config::AgentScope;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("HYSTERSIS_HOME", home.path());
        let _flag = EnvGuard::unset("HYSTERSIS_FOLDER_TRUST");
        let tmp = repo_tmp();
        ...
        .unwrap();
        let def = hystersis_agent::discovery::by_name_in_cwd("explore", tmp.path())
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        let _unset_ver = EnvGuard::unset(hystersis_version::TEST_VERSION_ENV);
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        use hystersis_agent::plugins::discovery::DiscoveryConfig;
        use hystersis_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("HYSTERSIS_HOME", home.path());
        let _flag = EnvGuard::unset("HYSTERSIS_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        let has_project_hook = |reg: &hystersis_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = hystersis_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
        let (reg, _errs) = crate::util::hooks::discover_hooks(
            git_root.as_deref(),
            &hystersis_tools::types::compat::CompatConfig::default(),
            untrusted,
        );
        assert!(
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        ...
        use hystersis_tools::implementations::lsp::config::LspServerConfig;
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use hystersis_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use hystersis_tools::types::config_source::ConfigSource;
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(hystersis_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var(hystersis_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(hystersis_version::TEST_VERSION_ENV) };
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        let _sim = EnvGuard::unset(hystersis_version::TEST_VERSION_ENV);
        if option_env!("HYSTERSIS_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        let _sim = EnvGuard::set(hystersis_version::TEST_VERSION_ENV, "0.0.0-sim");
        let _flag = EnvGuard::unset("HYSTERSIS_FOLDER_TRUST");
        let remote = RemoteSettings {
