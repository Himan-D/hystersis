...
//! connection to the hystersis.com relay server with automatic reconnection.
...
/// Config for the hystersis.com WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a hystersis.com first-party session
    /// (`is_hystersis_auth`: hystersis.com-issuer OIDC or external credential) with a
    /// non-empty bearer. BYOK/ApiKey, non-hystersis.com issuers (enterprise OIDC,
    /// third-party external providers), and deprecated WebLogin get `None`
    /// (relay-off; the leader still serves clients over IPC).
    pub(crate) fn for_session(
        session: &HystersisAuth,
        ctx: &HystersisComConfig,
        alpha_test_key: Option<String>,
        auth_manager: Option<Arc<AuthManager>>,
    ) -> Option<Self> {
        if !session.is_hystersis_auth() || session.key.is_empty() {
            return None;
        }
        let _ = alpha_test_key;
        Some(Self {
            ws_url: ctx.hystersis_ws_url.clone(),
            ws_origin: ctx.hystersis_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to hystersis.com with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `hystersis login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    match recovered {
        Ok(new_auth) if new_auth.key == config.auth.key => {
            ...
            hystersis_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "key_prefix": hystersis_auth::bearer_suffix(&new_auth.key),
                })),
            );
            false
        }
        Ok(new_auth) => {
            ...
            hystersis_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "new_key_prefix": hystersis_auth::bearer_suffix(&new_auth.key),
                })),
            );
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Hystersis WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(hystersis_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    tokio::select! {
        ...
        result = tokio::time::timeout(connect_timeout, async {
            if let Some(proxy_url) = proxy_url {
                ...
                let connector =
                    tokio_tungstenite::Connector::Rustls(hystersis_extra_ca::rustls_client_config());
...
mod tests {
    ...
    /// Helper to create a test HystersisAuth with the given key.
    fn test_auth(key: &str) -> HystersisAuth {
        HystersisAuth {
            key: key.to_string(),
    ...
    #[test]
    fn for_session_builds_only_for_hystersis_issuer() {
        use crate::auth::XAI_OAUTH2_ISSUER;
        let cfg = HystersisComConfig::default();
        let builds = |a: &HystersisAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let xai = HystersisAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
            ..test_auth("xai-bearer")
        };
        assert!(xai.is_hystersis_auth(), "precondition: is_hystersis_auth");
        assert!(builds(&xai));
        let external_xai = HystersisAuth {
            auth_mode: AuthMode::External,
            oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
            ..test_auth("ext-bearer")
        };
        assert!(external_xai.is_hystersis_auth(), "precondition: is_hystersis_auth");
        assert!(builds(&external_xai));
        assert!(!builds(&HystersisAuth {
            key: String::new(),
            ..xai.clone()
        }));
        assert!(!builds(&HystersisAuth {
    ...
    /// Helper: write a HystersisAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &HystersisAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        let cfg = crate::auth::HystersisComConfig::default();
        let scope = cfg.auth_scope();
        let am = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_session = HystersisAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
        ...
        let mut config = RelayConfig::for_session(&expired_session, &cfg, None, Some(am.clone()))
            .expect("hystersis.com OIDC session is relay-eligible");
        let cancel = CancellationToken::new();
    ...
    async fn attempt_auth_recovery_same_key_backs_off_without_cancel() {
        ...
        let fresh_session = HystersisAuth {
            auth_mode: AuthMode::Oidc,
            oidc_issuer: Some(XAI_OAUTH2_ISSUER.to_string()),
