...
pub const LEGACY_XAI_API_KEY_ENV_VAR: &str = "HYSTERSIS_CODE_XAI_API_KEY";

/// Read the API key from the environment.
///
/// Checks `XAI_API_KEY` first, then falls back to the legacy
/// `HYSTERSIS_CODE_XAI_API_KEY` for backward compatibility.
pub(crate) fn read_hystersis_api_key_env() -> Result<String, std::env::VarError> {
    std::env::var(XAI_API_KEY_ENV_VAR).or_else(|_| std::env::var(LEGACY_XAI_API_KEY_ENV_VAR))
}

/// Returns `true` if either `XAI_API_KEY` or `HYSTERSIS_CODE_XAI_API_KEY` is set.
pub fn has_hystersis_api_key_env() -> bool {
    read_hystersis_api_key_env().is_ok()
}
...
///
/// `disable_api_key_auth` (`[hystersis_com_config] disable_api_key_auth` /
/// `HYSTERSIS_DISABLE_API_KEY_AUTH`) is the admin kill switch: when true the
...
/// [`should_advertise_hystersis_api_key_with_env_ok`] with the probe result instead.
pub(crate) fn should_advertise_hystersis_api_key<'a, I>(disable_api_key_auth: bool, models: I) -> bool
where
    I: IntoIterator<Item = &'a ModelEntry>,
{
    should_advertise_hystersis_api_key_with_env_ok(disable_api_key_auth, models, true)
}
...
pub(crate) fn should_advertise_hystersis_api_key_with_env_ok<'a, I>(
    ...
    I: IntoIterator<Item = &'a ModelEntry>,
{
    ...
    has_byok || (has_hystersis_api_key_env() && first_party_env_ok)
}
...
pub struct AuthMethodsBuildInputs<'a> {
    ...
    /// callers compute via [`should_advertise_hystersis_api_key_with_env_ok`] after
    /// the validity probe; presence-only paths may use
    /// [`should_advertise_hystersis_api_key`]. When `preferred_method` is `Oidc`,
    ...
    /// Optional display label for the login method (`hystersis.com` or `oidc`).
    pub login_label: Option<&'a str>,
    /// True if `hystersis_com_config.auth_provider_command` is configured (sets
    /// `meta.external_provider = true` on the `hystersis.com` method).
...
fn build_pinned_api_key(has_external_api_key: bool) -> BuiltAuthMethods {
    ...
    BuiltAuthMethods {
        methods: vec![hystersis_api_key_auth_method()],
...
fn build_unpinned(
    ...
    if has_external_api_key {
        methods.push(hystersis_api_key_auth_method());
        default_auth_method_id = Some(acp::AuthMethodId::new(XAI_API_KEY_METHOD_ID));
    }
...
fn push_interactive_login(
    ...
    if has_enterprise_oidc {
        ...
        // the same `cfg.hystersis_com_config.oidc` Option, so the inconsistent
        // `(true, None)` combination is a programmer error -- panic loudly
        // (matches the original `cfg.hystersis_com_config.oidc.as_ref().unwrap()`
        ...
        methods.push(hystersis_com_auth_method(login_label, has_auth_provider_command));
    }
}
...
impl AuthMethodKind {
    pub fn from_id(id: &acp::AuthMethodId) -> Self {
        match id.0.as_ref() {
            ...
            HYSTERSIS_COM_METHOD_ID => Self::HystersisCom,
    ...
    /// `true` for session-based methods (cached_token, hystersis.com, oidc).
    pub(crate) fn is_session_based(self) -> bool {
        matches!(self, Self::CachedToken | Self::HystersisCom | Self::Oidc)
    }

    /// Requires user interaction (browser, OIDC redirect, or external auth command).
    pub fn needs_interactive_login(self) -> bool {
        matches!(self, Self::HystersisCom | Self::Oidc)
    }
}

/// `true` for session-based ACP methods (cached_token, hystersis.com, oidc).
...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `hystersis login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `hystersis login`, set XAI_API_KEY, or add api_key to ~/.hystersis/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `hystersis login` to authenticate.";

pub const XAI_API_KEY_METHOD_ID: &str = "xai.api_key";
pub(crate) fn hystersis_api_key_auth_method() -> acp::AuthMethod {
...
pub(crate) fn cached_token_auth_method() -> acp::AuthMethod {
    acp::AuthMethod::Agent(
        ...
        .description(Some("Cached token from ~/.hystersis/auth.json".to_string())),
    )
}

pub const HYSTERSIS_COM_METHOD_ID: &str = "hystersis.com";

/// xAI OAuth2/OIDC auth. Method id `"hystersis.com"` kept for ACP wire-compat.
pub(crate) fn hystersis_com_auth_method(
    label: Option<&str>,
    has_auth_provider_command: bool,
) -> acp::AuthMethod {
    let name = label.unwrap_or("Hystersis");
    ...
    acp::AuthMethod::Agent(
        acp::AuthMethodAgent::new(acp::AuthMethodId::new(HYSTERSIS_COM_METHOD_ID), name.to_string())
...
mod tests {
    ...
    /// No advertiseable API-key credentials → interactive `hystersis.com`.
    #[test]
    fn after_cached_token_unavailable_falls_to_hystersis_com_without_api_key() {
        assert_eq!(
    ...
    #[test]
    fn enterprise_byok_first_method_is_hystersis_api_key() {
    ...
    #[test]
    fn byok_with_cached_token_keeps_hystersis_api_key_first() {
        let inputs = AuthMethodsBuildInputs {
    ...
    /// `hystersis.com` — `auth_methods.first()` does NOT need interactive login,
    ...
    /// Brand-new user (no API key, no cached token): only `hystersis.com` is
    ...
    #[test]
    fn fresh_user_only_advertises_hystersis_com_and_requires_login() {
        let built = build_auth_methods(default_inputs());

        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::HystersisCom));
        assert!(built.default_auth_method_id.is_none());
        assert_eq!(built.methods.len(), 1);
    }

    /// Enterprise OIDC replaces `hystersis.com` (mutually exclusive). xai.api_key,
    /// when present, still leads.
    #[test]
    fn enterprise_oidc_replaces_hystersis_com_but_hystersis_api_key_still_first() {
        let inputs = AuthMethodsBuildInputs {
            has_external_api_key: true,
            has_cached_token: false,
        ...
        assert!(
            !built
                .methods
                .iter()
                .any(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::HystersisCom),
            "hystersis.com and oidc are mutually exclusive",
        );
    }

    /// `has_auth_provider_command` is plumbed through to the `hystersis.com` method
    ...
    fn auth_provider_command_sets_external_provider_meta() {
        ...
        let hystersis = built
            .methods
            .iter()
            .find(|m| AuthMethodKind::from_id(m.id()) == AuthMethodKind::HystersisCom)
            .expect("hystersis.com must be advertised");
        assert_eq!(hystersis.name(), "Acme Corp");
        let meta = hystersis.meta().expect("meta should be set");
        assert_eq!(
    ...
    /// `~/.hystersis/config.toml` skeleton from the bug report, walks it through
    /// the same predicate (`should_advertise_hystersis_api_key`) and the same
    ...
    fn enterprise_byok_config_does_not_require_login() {
        ...
        // login method. Confirms the predicate isn't trivially true.
        {
            ...
            let has_external_api_key = should_advertise_hystersis_api_key(false, models.values());
        ...
        // pager's `startup_auth_metadata()` returns `needs_login = false`.
        {
            ...
            let built = build_auth_methods(AuthMethodsBuildInputs {
                ...
                // hystersis.com login (no enterprise OIDC).
                has_cached_token: false,
                ..default_inputs()
            });
            assert_eq!(
    ...
    #[test]
    #[serial]
    fn global_external_api_key_advertises_hystersis_api_key_first() {
    ...
    #[test]
    #[serial]
    fn disable_api_key_auth_suppresses_hystersis_api_key_method() {
        let _set = EnvGuard::set(XAI_API_KEY_ENV_VAR, "xai-external-key");
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);

        // Flag off: today's behavior (advertised first).
        assert!(should_advertise_hystersis_api_key(false, models.values()));

        // Flag on: never advertised, regardless of credentials.
        let has_external_api_key = should_advertise_hystersis_api_key(true, models.values());
        assert!(!has_external_api_key);
        let built = build_auth_methods(AuthMethodsBuildInputs {
            has_external_api_key,
            ..default_inputs()
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::HystersisCom),
    ...
    fn env_key_probe_unusable_suppresses_advertise_without_byok() {
        ...
        let models = resolve_model_list(&cfg, None);
        assert!(
            should_advertise_hystersis_api_key(false, models.values()),
            "presence-only helper still sees the env key"
        );
        assert!(
            !should_advertise_hystersis_api_key_with_env_ok(false, models.values(), false),
    ...
    fn env_key_probe_ok_still_advertises() {
        ...
        let cfg = Config::default();
        let models = resolve_model_list(&cfg, None);
        assert!(should_advertise_hystersis_api_key_with_env_ok(
            false,
            models.values(),
            true
        ));
    ...
    /// Legacy `HYSTERSIS_CODE_XAI_API_KEY` env var is accepted as a fallback
    /// when `XAI_API_KEY` is not set, ensuring existing deployments keep working.
    #[test]
    #[serial]
    fn legacy_env_var_fallback_advertises_hystersis_api_key() {
        let _unset_new = EnvGuard::unset(XAI_API_KEY_ENV_VAR);
        let _set_legacy = EnvGuard::set(LEGACY_XAI_API_KEY_ENV_VAR, "xai-legacy-key");
        assert!(has_hystersis_api_key_env());
        assert_eq!(read_hystersis_api_key_env().unwrap(), "xai-legacy-key");
    ...
    /// When both `XAI_API_KEY` and `HYSTERSIS_CODE_XAI_API_KEY` are set,
    ...
    fn new_env_var_takes_precedence_over_legacy() {
        ...
        assert_eq!(read_hystersis_api_key_env().unwrap(), "new-key");
    }

    // -- hystersis login --legacy regression coverage ------------------------
    //
    // `hystersis login --legacy` produces a HystersisAuth with `auth_mode: WebLogin`,
    // `oidc_issuer: None`, and no `expires_at` (30-day hardcoded TTL).
    // When this token is present via the `HYSTERSIS_AUTH` env var (or via legacy
    ...
    // to-end. A regression in HYSTERSIS_AUTH JSON parsing or in auth method
    // ordering would send legacy-token users to the login screen.

    /// END-TO-END REGRESSION TEST: a legacy auth token (WebLogin, no
    /// expires_at) present in the `HYSTERSIS_AUTH` env var, with no other auth
    ...
    #[test]
    #[serial]
    fn hystersis_login_legacy_token_does_not_require_login() {
        use crate::auth::{AuthManager, AuthMode, HystersisAuth, HystersisComConfig};

        // Ensure clean slate for "no other auth available".
        let _g1 = EnvGuard::unset("HYSTERSIS_AUTH_PATH");
        let _g2 = EnvGuard::unset(XAI_API_KEY_ENV_VAR);

        // Construct a legacy-style token exactly as `hystersis login --legacy`
        // produces: WebLogin mode, no OIDC fields, no refresh_token, no
        // expires_at (is_expired falls back to 30-day age check).
        let legacy_token = HystersisAuth {
        ...
        // Provide it via HYSTERSIS_AUTH env var (highest priority code path in
        ...
        let _g = EnvGuard::set("HYSTERSIS_AUTH", &legacy_json);
        ...
        assert!(
            current.is_some(),
            "legacy token in HYSTERSIS_AUTH env MUST be loaded directly -- if this fails, \
    ...
    fn no_legacy_token_means_no_cached_token_advertised() {
        ...
        let _g1 = EnvGuard::unset("HYSTERSIS_AUTH");
        let _g2 = EnvGuard::unset("HYSTERSIS_AUTH_PATH");
        ...
        assert_eq!(
            first_kind(&built.methods),
            Some(AuthMethodKind::HystersisCom),
            "no cached token AND no api key: pager must show login (hystersis.com first)",
        );
    }
    ...
    fn pin_oidc_with_session_hides_api_key() {
        ...
        assert_eq!(
            method_ids(&built),
            vec![CACHED_TOKEN_AUTH_METHOD_ID, HYSTERSIS_COM_METHOD_ID]
        );
    ...
    fn pin_oidc_without_session_is_interactive_only() {
        ...
        assert_eq!(method_ids(&built), vec![HYSTERSIS_COM_METHOD_ID]);
        assert!(built.default_auth_method_id.is_none());
    }
}
