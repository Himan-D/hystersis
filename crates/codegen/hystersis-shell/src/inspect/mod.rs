...
use hystersis_tools::util::truncate::estimate_tokens;
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = hystersis_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = hystersis_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = hystersis_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        hystersis_version: hystersis_version::VERSION.to_string(),
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = hystersis_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        hystersis_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `hystersis_agent::prompt::agents_md::read_agents_config_with_paths`
...
async fn list_permissions(cwd: &Path, project_trusted: bool) -> PermissionsReport {
    use hystersis_workspace::permission::resolution;
...
fn list_hooks(
    ...
    discovered_plugins: &[hystersis_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = hystersis_tools::types::compat::CompatConfig::default();
    ...
    let config_layers = hystersis_config::hook_config_layers();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            use hystersis_hooks::config::HookOrigin as O;
            ...
            let source = match hystersis_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(hystersis_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(hystersis_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(hystersis_config::USER_CONFIG_FILENAME),
                },
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &hystersis_agent::plugins::PluginRegistry,
    skills_config: &hystersis_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = hystersis_agent::prompt::skills::list_skills_with_plugins(
...
fn slash_name_counts(
    skills: &[hystersis_agent::prompt::skills::SkillInfo],
    ...
    for skill in skills.iter().filter(|s| s.user_invocable && s.enabled) {
        ...
        *counts
            .entry(
                hystersis_tools::implementations::skills::skill::format_skill_name(skill)
                    .to_lowercase(),
            )
            .or_default() += 1;
    }
...
fn slash_collision(
    skill: &hystersis_agent::prompt::skills::SkillInfo,
    ...
    let qualified =
        hystersis_tools::implementations::skills::skill::format_skill_name(skill).to_lowercase();
...
fn skill_entry_source(s: &hystersis_agent::prompt::skills::SkillInfo) -> ConfigSource {
...
fn list_agents(
    ...
    let agents = hystersis_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[hystersis_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                hystersis_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                hystersis_agent::plugins::PluginScope::Project => Scope::Project,
                hystersis_agent::plugins::PluginScope::User => Scope::User,
                hystersis_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: hystersis_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    hystersis_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = hystersis_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
mod tests {
    use super::*;
    use hystersis_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use hystersis_tools::implementations::skills::types::SkillScope;
    ...
    async fn list_skills_honors_skills_config() {
        ...
        let registry = hystersis_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
