...
pub enum ClientMode {
    /// Headless mode (hystersis agent, hystersis agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (hystersis agent stdio, hystersis -p) - uses local IPC.
...
pub struct ClientCapabilities {
    ...
    /// Whether this client has advertised `hystersis.com/codeNavigation.enabled`.
    ...
    /// Whether this client will draw a status row (`hystersis.com/statusLine`). When
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `hystersis update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `hystersis update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `hystersis update` just installed; the leader uses
...
impl InternalMethod {
    pub(crate) const fn name(self) -> &'static str {
        match self {
            Self::AuthCleared => "hystersis.com/internal/auth_cleared",
            Self::EvictSessions => "hystersis.com/internal/evict_sessions",
            Self::ReloadAllMcpServers => "hystersis.com/internal/reload_all_mcp_servers",
            Self::ReloadModels => "hystersis.com/internal/reload_models",
            Self::ReloadModelsCache => "hystersis.com/internal/reload_models_cache",
            Self::ReloadProjectMcpServers => "hystersis.com/internal/reload_project_mcp_servers",
            Self::ReloadSkills => "hystersis.com/internal/reload_skills",
            Self::ReloadWorkflows => "hystersis.com/internal/reload_workflows",
        }
    }
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["hystersis-a".into(), "hystersis-b".into()],
            pid: 4242,
        };
