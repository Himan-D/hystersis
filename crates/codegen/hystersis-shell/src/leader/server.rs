...
use hystersis_computer_hub_sdk::{AuthCredential, AuthIdentity, AuthProvider};
use hystersis_workspace::WorkspaceHandle;
...
struct ClientState {
    ...
    /// The client type string from IPC registration (e.g., "hystersis-tui", "hystersis-code-extension").
...
impl AuthProvider for LeaderAuthProvider {
    ...
    /// read. Mirrors the in-process path (`mvp_agent`): prefer `HystersisAuth.team_id`
...
/// `params._meta["hystersis.com/leaderClientId"]`.
///
...
///
/// - `hystersis.com/sessions/changed` — roster delta; every open dashboard must stay
///   in sync.
/// - `hystersis.com/models/update` — the model catalog changed (config.toml
...
/// - `hystersis.com/mcp/servers_updated` — the MCP catalog resolved/changed (managed
...
/// - `hystersis.com/announcements/update` — the announcements list changed (startup
...
///   (`hystersis.com/settings/update` stays non-broadcast — it carries auth/gate state.)
///
/// Matched via [`method_of`], NOT the raw top-level `method`: agent ext
/// notifications arrive `_`-prefixed on the wire (`_hystersis.com/sessions/changed`),
/// so a raw compare would miss the production form.
fn is_machine_wide_broadcast_notification(json: &serde_json::Value) -> bool {
    matches!(
        method_of(json),
        Some(
            "hystersis.com/sessions/changed"
                | "hystersis.com/models/update"
                | "hystersis.com/mcp/servers_updated"
                | "hystersis.com/announcements/update"
        )
    )
}
/// Whether a payload is the `hystersis.com/scheduled_task_inject_prompt` notification.
///
...
///   - direct:  `{"method":"hystersis.com/foo", ...}`                                 -> `hystersis.com/foo`
///   - wrapped: `{"method":"_hystersis.com/foo","params":{"method":"hystersis.com/foo",...}}`  -> `hystersis.com/foo`
///
...
fn is_scheduled_task_inject_prompt(json: &serde_json::Value) -> bool {
    method_of(json) == Some("hystersis.com/scheduled_task_inject_prompt")
}
...
/// `hystersis.com/session_notification` whose `update.sessionUpdate ==
...
fn extract_interaction_resolved_tool_call_id(json: &serde_json::Value) -> Option<String> {
    if method_of(json) != Some("hystersis.com/session_notification") {
        return None;
    }
...
fn extract_session_id_from_prompt_complete(json: &serde_json::Value) -> Option<String> {
    ...
    if method != "hystersis.com/session/prompt_complete" {
        return None;
    }
    json.get("params")?
        .get("sessionId")
...
fn inject_session_request_context(
    ...
    if let Some(params) = json.get_mut("params").and_then(|p| p.as_object_mut()) {
        ...
        if let Some(meta_obj) = meta.as_object_mut() {
            ...
            if !meta_obj.contains_key("hystersis.com/leaderClientId") {
                meta_obj.insert(
                    "hystersis.com/leaderClientId".to_string(),
...
/// Extract yolo_mode change from hystersis.com/yolo_mode_changed notification.
///
...
fn extract_yolo_mode_change(json: &serde_json::Value) -> Option<bool> {
    let method = json.get("method")?.as_str()?;
    if method != "hystersis.com/yolo_mode_changed" {
        return None;
    }
    let params = json.get("params")?;
    params.get("yolo_mode").and_then(|v| v.as_bool())
}
/// Extract the auto-mode intent from an `hystersis.com/yolo_mode_changed` notification, so
...
fn inject_client_identity_into_yolo_notification(
    ...
    let is_yolo = json
        .get("method")
        .and_then(|m| m.as_str())
        .is_some_and(|m| m == "hystersis.com/yolo_mode_changed");
    if !is_yolo {
        return false;
    }
    let mut mutated = false;
...
async fn handle_workspace_start(
    ...
    let handle = hystersis_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
        Some(metadata),
...
fn make_version_mismatch_notification(
    ...
    Some(
        serde_json::json!({
            "jsonrpc": "2.0",
            "method": "hystersis.com/leader/version_mismatch",
            "params": {
                "clientVersion": client_version,
                "leaderVersion": leader_version,
                "message": format!(
                    "Client version {client_version} differs from leader version \
                     {leader_version}. Restart the hystersis binary to use the same version."
                )
            }
        })
        .to_string(),
...
///   hystersis.com WebSocket relay until this fires, so a leader serving only
