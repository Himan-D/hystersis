//! Shim — see `hystersis_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `hystersis_shell::instrumentation_timer!`). Keeping the macro here
...
pub use hystersis_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `hystersis_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `hystersis_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `hystersis_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `hystersis_telemetry::instrumentation`.
#[macro_export]
