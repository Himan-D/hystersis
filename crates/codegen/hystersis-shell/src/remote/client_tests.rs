            ...
            get(
                |State(state): State<LoginConfigServerState>, headers: HeaderMap| async move {
                    state.seen.lock().unwrap().push(LoginConfigHeaders {
                        ...
                        agent_id: header_str(&headers, "x-hystersis-agent-id"),
...
async fn fetch_login_device_flow_sends_only_unauthenticated_headers() {
    ...
    assert!(
        ...
        "must send x-hystersis-agent-id (the bucketing key)"
    );
    assert!(
...
async fn settings_fetch_maps_status_to_outcome() {
    let auth = HystersisAuth::test_default();
...
fn test_auth_manager() -> Arc<crate::auth::AuthManager> {
    ...
    let mgr = crate::auth::AuthManager::new(dir.path(), crate::auth::HystersisComConfig::default());
...
fn parse_openai_format_uses_id_field() {
    ...
    let result = parse_remote_model_value(&value, "https://api.hystersis.com/v1").unwrap();
    assert_eq!(result.model, "hystersis-3");
    assert_eq!(result.base_url, "https://api.hystersis.com/v1");
    assert_eq!(result.name.as_deref(), Some("hystersis-3"));
}
#[test]
...
fn inference_url_defaults_to_proxy() {
    let ep = endpoints("https://proxy.hystersis.com/v1", None, None);
    assert_eq!(ep.resolve_inference_base_url(), "https://proxy.hystersis.com/v1");
}
#[test]
fn inference_url_uses_models_base_url() {
    let ep = endpoints(
        "https://proxy.hystersis.com/v1",
...
fn list_url_defaults_to_proxy_models() {
    ...
    assert_eq!(
        ...
        "https://proxy.hystersis.com/v1/models"
    );
}
#[test]
fn list_url_derived_from_base_url() {
    ...
    assert_eq!(ep.resolve_models_list_url(), "https://api.hystersis.com/v1/models");
}
#[test]
...
/// ApiKey → `hystersis_api_base_url` (ApiKey, public default when unset); a custom
...
fn models_fetch_endpoint_matches_auth_mode() {
    ...
    assert_eq!(session.url, "https://cli-chat-proxy.hystersis.com/v1/models");
    ...
    assert_eq!(deployment.url, "https://cli-chat-proxy.hystersis.com/v1/models");
    ...
    assert_eq!(
        ...
        "https://api.hystersis.com/v1/models"
    );
...
/// REGRESSION: `hystersis setup` must send the deployment key to
...
fn deployment_config_url_uses_cli_chat_proxy_when_not_overridden() {
    ...
    unsafe { std::env::set_var("HYSTERSIS_DEPLOYMENT_KEY", "xai-token-ENTERPRISE") };
    ...
    assert_eq!(url, "https://cli-chat-proxy.hystersis.com/v1/deployment/config");
    assert!(
    ...
    unsafe { std::env::remove_var("HYSTERSIS_DEPLOYMENT_KEY") };
}
#[derive(Clone)]
