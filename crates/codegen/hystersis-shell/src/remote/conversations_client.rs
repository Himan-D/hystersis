...
/// Body for `PUT /rest/app-chat/conversations/{id}` (hystersis-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("HYSTERSIS_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    async fn require_hystersis_auth(&self) -> Result<HystersisAuth, ConvError> {
    ...
    pub async fn list_conversations(
        ...
        let auth = self.require_hystersis_auth().await?;
