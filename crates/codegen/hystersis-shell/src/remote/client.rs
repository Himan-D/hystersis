...
const HYSTERSIS_CODE_BACKEND_URL: &str = "https://code.hystersis.com";
const DEFAULT_TIMEOUT: Duration = Duration::from_secs(30);
const HYSTERSIS_CODE_WEB_URL: &str = "https://hystersis.com";
/// Build a share URL from a permission ID
pub fn share_url(permission_id: &str) -> String {
    let web_url =
        std::env::var("HYSTERSIS_CODE_WEB_URL").unwrap_or_else(|_| HYSTERSIS_CODE_WEB_URL.to_string());
...
fn add_cli_chat_proxy_headers_blocking(
    ...
    let mut builder = builder
        ...
        .header("X-XAI-Token-Auth", HystersisComConfig::default().token_header)
...
async fn add_bundle_fetch_headers(
    ...
    let mut credentials = crate::util::hystersis_auth_credentials::HystersisAuthCredentials::new(
...
async fn fetch_bundle_inner(
    ...
    let client: reqwest_middleware::ClientWithMiddleware = if let Some(am) = auth_manager {
        let provider: std::sync::Arc<dyn hystersis_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
...
impl BackendClient {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            reqwest_client,
            base_url: std::env::var("HYSTERSIS_CODE_BACKEND_URL")
                .unwrap_or_else(|_| HYSTERSIS_CODE_BACKEND_URL.to_string()),
            auth_manager: None,
        }
    }
    ...
    /// instead of requiring the caller to pass `&HystersisAuth`.
    pub(crate) fn with_auth_manager(
        mut self,
        manager: std::sync::Arc<crate::auth::AuthManager>,
    ) -> Self {
        let credentials: std::sync::Arc<dyn hystersis_auth::AuthCredentialProvider> =
            std::sync::Arc::new(
                crate::auth::credential_provider::ShellAuthCredentialProvider::new(
                    manager.clone(),
                    None,
    ...
    async fn resolve_auth(&self) -> Result<HystersisAuth, BackendError> {
        let manager = self
            .auth_manager
            .as_ref()
    ...
    /// See: crates/codegen/xai-hystersis-shell/src/agent/app.rs:run_headless
    async fn auth_header_map(&self) -> Result<reqwest::header::HeaderMap, BackendError> {
        ...
        headers.insert(
            "X-XAI-Token-Auth",
            required(&HystersisComConfig::default().token_header, "X-XAI-Token-Auth")?,
        );
        ...
        headers.insert(
            ...
            HeaderValue::from_static(hystersis_version::VERSION),
        );
        Ok(headers)
    }
    async fn send_with_auth(
        ...
        let builder = hystersis_file_utils::trace_context::inject_trace_context_into_request(
...
/// Fetch `hystersis_build_login_device_flow` from cli-chat-proxy `GET /v1/login-config`.
///
/// Unauthenticated (pre-login); `x-hystersis-agent-id` is the per-install bucketing key.
...
pub async fn fetch_login_device_flow(cli_chat_proxy_base_url: &str) -> Option<bool> {
    let agent_id = tokio::task::spawn_blocking(hystersis_telemetry::id::agent_id)
        .await
        .ok()?;
    ...
    let response = client
        ...
        .header("x-hystersis-agent-id", agent_id)
        .header("x-grok-client-version", hystersis_version::VERSION)
        .header(
            "x-grok-client-identifier",
            crate::http::process_client_identifier(),
...
impl ListModelsEndpoint {
    fn from_endpoints(
        ...
        if endpoints.has_custom_endpoint() {
            ...
            Self {
                url: format!("{}/models", endpoints.hystersis_api_base_url),
                auth: EndpointAuth::ApiKey,
            }
        } else {
            Self {
...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = crate::agent::auth_method::read_hystersis_api_key_env()
                .or_else(|_| {
...
pub(crate) fn parse_remote_model_value(
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        reasoning_efforts: obj
            ...
            .map(|arr| hystersis_sampling_types::parse_reasoning_effort_options(arr))
            .unwrap_or_default(),
        ...
        compactions_remaining: obj
            ...
            .or_else(|| {
                obj
                    ...
                    .and_then(|v| v.as_bool())
                    .map(hystersis_sampling_types::CompactionsRemaining::Dynamic)
            }),
...
fn parse_compaction_at_tokens(
    v: &serde_json::Value,
) -> Option<hystersis_sampling_types::CompactionAtTokens> {
...
fn parse_compactions_remaining(
    v: &serde_json::Value,
) -> Option<hystersis_sampling_types::CompactionsRemaining> {
