//! hystersis.com product Skills catalog — the same REST sources hystersis-web uses:
//! - `POST /rest/skills` — first-party bundled skills (docx, pdf, ffmpeg, …)
//! - `GET  /rest/user-skills` — enabled user-uploaded skills
//!
//! Transport only. Chat `hystersis.com/commands/list` / ACP `available_commands_update`
...
use hystersis_tools::implementations::skills::skill::extract_skill_body;
use hystersis_tools::implementations::skills::types::{SkillInfo, SkillScope};

use crate::auth::AuthManager;

const HYSTERSIS_WEB_URL: &str = "https://hystersis.com";
...
/// connect timeout; without this a hung hystersis.com stalls the session actor.
...
/// Combined product catalog matching hystersis-web's Skills menu composition.
...
impl ProductSkillsCatalog {
    ...
    ///
    /// Mirrors hystersis-web: enabled user skills first (and hide same-named
...
pub enum SkillsError {
    #[error("no hystersis.com credentials")]
    NoAuth,
...
fn primary_is_tenant_tagged(auth: &crate::auth::HystersisAuth) -> bool {
    auth.team_id.is_some() || auth.organization_id.is_some()
}

fn entry_is_untagged(auth: &crate::auth::HystersisAuth) -> bool {
...
fn entry_matches_primary_tenant(
    primary: &crate::auth::HystersisAuth,
    entry: &crate::auth::HystersisAuth,
) -> bool {
...
fn skills_auth_alt_candidates<'a>(
    primary: &crate::auth::HystersisAuth,
    entries: impl IntoIterator<Item = &'a crate::auth::HystersisAuth>,
...
/// Stateless transport for product Skills REST (hystersis-web `skillsApi`).
...
impl SkillsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("HYSTERSIS_SKILLS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("HYSTERSIS_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("HYSTERSIS_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| HYSTERSIS_WEB_URL.to_string());
        Self {
    ...
    fn apply_auth_headers(
        ...
        let mut builder = builder
            ...
            .header(
                "X-XAI-Token-Auth",
                self.auth.hystersis_com_config().token_header.clone(),
            )
            .header("x-userid", user_id)
            .header("x-grok-client-version", hystersis_version::VERSION)
            .header(
        ...
        hystersis_file_utils::trace_context::inject_trace_context_into_request(builder)
    }

    /// Grok.com product Skills require first-party session auth (same gate as
    /// managed MCP / sibling hystersis.com clients — not plain BYOK API keys).
    async fn require_skills_auth(&self) -> Result<crate::auth::HystersisAuth, SkillsError> {
    ...
    /// Credentials to try for hystersis.com product Skills REST.
    ///
    /// Primary first. When primary is OIDC on the default hystersis.com host, also
    ...
    fn skills_auth_candidates(&self, primary: &crate::auth::HystersisAuth) -> Vec<SkillsAuthCandidate> {
        ...
        if self.base_url != HYSTERSIS_WEB_URL {
            return out;
        }
    ...
    /// feature-flag allow-list (same as hystersis-web attach menu).
    ///
...
mod tests {
    ...
    fn test_auth_manager() -> Arc<AuthManager> {
        use crate::auth::{AuthMode, HystersisAuth, HystersisComConfig, XAI_OAUTH2_ISSUER};
    ...
    fn skills_auth_alts_prefer_tagged_then_untagged_recovery() {
        ...
        let team_plus_org = HystersisAuth {
