...
//! `StorageClient` constructed inside `hystersis_file_utils::gcs::*` helpers.
//!
...
//! to an `hystersis_file_utils::gcs::*` helper.
...
/// shell-side adapter that lets `hystersis_file_utils::gcs::*` helpers wire
...
/// `HYSTERSIS_TELEMETRY_GCS_BUCKET`; `None` disables trace uploads until a bucket
/// is configured.
pub(crate) const SESSION_TRACES_BUCKET: Option<&str> =
    option_env!("HYSTERSIS_SESSION_TRACES_BUCKET_DEFAULT");
...
pub(crate) async fn upload_to_auth_diagnostics(
    ...
    match hystersis_file_utils::gcs::upload_bytes(
