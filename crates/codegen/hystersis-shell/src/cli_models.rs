//! Data APIs for `hystersis models`. Clients own display.
...
/// Status for the `hystersis models` banner (display order ≠ sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `hystersis_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    /// [`crate::agent::auth_method::should_advertise_hystersis_api_key`] so
    ...
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if agent_config.create_auth_manager().current().is_some() {
            let origin = &agent_config.hystersis_com_config.hystersis_ws_origin;
            let host = origin
                .strip_prefix("https://")
        ...
        if crate::agent::auth_method::should_advertise_hystersis_api_key(
            agent_config.hystersis_com_config.api_key_auth_disabled(),
            models.values(),
...
/// Fetch model state via `hystersis.com/models/list` over an initialized channel.
pub async fn fetch_model_state(acp_tx: &AcpAgentTx) -> Result<acp::SessionModelState> {
    ...
    let resp: acp::ExtResponse = acp_send(
        acp::ExtRequest::new("hystersis.com/models/list", params.into()),
        acp_tx,
    )
    .await?;
...
/// Parse an `hystersis.com/models/list` payload; a handler error wins over a
/// missing result.
...
mod tests {
    ...
    ///
    /// Uses `HYSTERSIS_AUTH_PATH` (not `HYSTERSIS_HOME`) so a OnceLock-cached real home
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("HYSTERSIS_AUTH"),
            EnvGuard::set("HYSTERSIS_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("HYSTERSIS_DEPLOYMENT_KEY"),
            EnvGuard::unset("HYSTERSIS_WS_ORIGIN"),
            EnvGuard::unset("HYSTERSIS_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        let (_dir, _g) = isolate_auth_sources();
        let token = HystersisAuth {
            key: "session-token".into(),
            auth_mode: AuthMode::WebLogin,
            ..HystersisAuth::test_default()
        };
        let json = serde_json::to_string(&token).unwrap();
        let _auth = EnvGuard::set("HYSTERSIS_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("hystersis.com".to_owned())
        );
    }
    ...
    fn models_list_response_round_trips() {
        let state = acp::SessionModelState::new(
            acp::ModelId::new("hystersis-4"),
            vec![acp::ModelInfo::new(acp::ModelId::new("hystersis-4"), "Hystersis 4")],
        );
        let ok = crate::session::ExtMethodResult::success(state)
            .to_ext_response()
            .unwrap();
        let parsed = parse_models_list_response(ok.0.get()).unwrap();
        assert_eq!(parsed.current_model_id.0.as_ref(), "hystersis-4");
