//! Notification bridge: translates `xai-hystersis-tools` `ToolNotification` events
//! into `xai-hystersis-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
pub(crate) struct NotificationBridgeConfig {
    ...
    /// `hystersis.com/incrementalBashOutput` capability.
    ...
    pub task_completion_reservations:
        hystersis_tools::reminders::task_completion::TaskCompletionReservations,
    pub task_wake_suppressed: hystersis_tools::reminders::task_completion::TaskWakeSuppressed,
    ...
    /// next-turn exit reminder. Hystersis-build leaves this `false` — its
...
fn stamp_scheduler_meta(
    ...
    meta.insert("hystersis.com/schedulerGeneration".to_owned(), generation.into());
    meta.insert("hystersis.com/schedulerRevision".to_owned(), revision.into());
}
...
async fn handle_scheduled_task_removed(
    config: &NotificationBridgeConfig,
    removed: hystersis_tools::notification::ScheduledTaskRemoved,
        ...
        Ok(params) => {
            ...
            config
                ...
                .forward_fire_and_forget(acp::ExtNotification::new(
                    "hystersis.com/scheduled_task_deleted",
                    params.into(),
                ));
            Ok(())
        }
...
async fn emit_current_mode_update(
    config: &NotificationBridgeConfig,
    mode: hystersis_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::BashExecutionBackgrounded(bg) => {
            ...
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("hystersis.com/task_backgrounded", params.into());
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == hystersis_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.is_auto_wake_suppressed() {
                ...
                let body = if is_monitor {
                    hystersis_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    hystersis_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = hystersis_tools::reminders::wrap_reminder(&body);
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    ...
                    .tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, hystersis_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
        ...
        ToolNotification::ScheduledTaskFired(fired) => {
                ...
                serde_json::to_value(&fired_notif).and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "hystersis.com/scheduled_task_fired",
                        params.into(),
                    ));
            }
        }
        ...
        ToolNotification::ScheduledTaskCreated(created) => {
                ...
                .and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "hystersis.com/scheduled_task_created",
                        params.into(),
                    ));
            }
        }
