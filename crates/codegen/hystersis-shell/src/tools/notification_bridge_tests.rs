use super::*;
use hystersis_tools::computer::types::TaskKind;
...
fn make_test_config_full() -> (
    NotificationBridgeConfig,
    mpsc::UnboundedReceiver<hystersis_acp_lib::AcpClientMessage>,
...
fn make_test_config_full_raw() -> (
    ...
    let gateway = hystersis_acp_lib::AcpAgentGatewaySender::new(gateway_tx);
...
/// drain it). The pager's `hystersis.com/task_completed` notification still fires.
#[tokio::test]
async fn bash_task_completed_suppresses_auto_wake_during_goal_loop() {
    ...
    while let Ok(msg) = gateway_rx.try_recv() {
        if let hystersis_acp_lib::AcpClientMessage::ExtNotification(args) = msg
            && args.request.method.as_ref() == "hystersis.com/task_completed"
        {
            found_ext = true;
        }
    }
    assert!(
        found_ext,
        "hystersis.com/task_completed ExtNotification must still be sent for UI"
    );
}
...
fn task_completed_will_wake(
    gateway_rx: &mut mpsc::UnboundedReceiver<hystersis_acp_lib::AcpClientMessage>,
) -> Option<bool> {
    while let Ok(msg) = gateway_rx.try_recv() {
        if let hystersis_acp_lib::AcpClientMessage::ExtNotification(args) = msg
            && args.request.method.as_ref() == "hystersis.com/task_completed"
...
async fn task_completed_notification_stamps_will_wake() {
    ...
    *config
    ...
    assert!(
        persisted,
        "declined admission must still persist hystersis.com/task_completed"
    );
}
...
async fn monitor_event_skipped_after_task_completed_auto_wake() {
    ...
    handle_notification(
        &config,
        ToolNotification::MonitorEvent(hystersis_tools::notification::types::MonitorEvent {
...
async fn scheduled_task_created_is_persisted() {
    ...
    let notification = ToolNotification::ScheduledTaskCreated(
        hystersis_tools::notification::types::ScheduledTaskCreated {
            task_id: "loop-1".into(),
    ...
    match msg {
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
            ...
            assert_eq!(meta["hystersis.com/schedulerGeneration"], "generation-a");
            assert_eq!(meta["hystersis.com/schedulerRevision"], 1);
            assert!(
                notif
                    .meta
                    .as_ref()
...
async fn bash_output_chunk_forwards_live_without_persisting() {
    ...
    let notification =
        ToolNotification::BashOutputChunk(hystersis_tools::notification::types::BashOutputChunk {
            base: hystersis_tools::notification::types::BashNotificationBase {
    ...
    match gateway_rx.try_recv().expect("chunk must be broadcast") {
        hystersis_acp_lib::AcpClientMessage::SessionNotification(args) => {
            assert!(
                args.request
                    .meta
                    .as_ref()
...
async fn live_in_progress_event_id_is_not_a_prepare_replay_cursor() {
    ...
    let live_id = match gateway_rx.try_recv().unwrap() {
        hystersis_acp_lib::AcpClientMessage::SessionNotification(args) => args
            .request
            .meta
            .as_ref()
            .and_then(|m| m.get("eventId"))
...
async fn scheduled_task_removed_is_persisted() {
    ...
    let (config, _gateway_rx, mut persistence_rx, _cmd_rx) = make_test_config_full();
    let removed = hystersis_tools::notification::ScheduledTaskRemoved::new(
        "loop-1".into(),
        hystersis_tools::notification::ScheduledTaskRemovedReason::Expired,
        "generation-a".into(),
        2,
    );
        ...
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
            assert!(matches!(
                ...
                crate::extensions::notification::SessionUpdate::ScheduledTaskDeleted {
                    reason: hystersis_tools::notification::ScheduledTaskRemovedReason::Expired,
                    ..
                }
            ));
            assert!(
                hystersis_persisted_event_id(&notif).is_some(),
                "the persisted deletion line must be stamped"
            );
            let meta = notif.meta.as_ref().expect("scheduler metadata");
            assert_eq!(meta["hystersis.com/schedulerGeneration"], "generation-a");
            assert_eq!(meta["hystersis.com/schedulerRevision"], 2);
        }
...
async fn acknowledged_scheduler_removal_appends_before_ack_and_broadcast() {
    ...
    let removed = hystersis_tools::notification::ScheduledTaskRemoved::new(
        "loop-ack".into(),
        hystersis_tools::notification::ScheduledTaskRemovedReason::Deleted,
        "generation-a".into(),
        17,
    );
    ...
    let persistence = async {
        ...
        assert_eq!(notification.meta.unwrap()["hystersis.com/schedulerRevision"], 17);
    ...
    assert!(matches!(
        gateway_rx.try_recv(),
        Ok(hystersis_acp_lib::AcpClientMessage::ExtNotification(_))
    ));
}

fn hystersis_persisted_event_id(
...
async fn task_backgrounded_persisted_line_is_stamped() {
    ...
    let notification = ToolNotification::BashExecutionBackgrounded(
        hystersis_tools::notification::types::BashExecutionBackgrounded {
            base: hystersis_tools::notification::types::BashNotificationBase {
    ...
    match persistence_rx.try_recv().expect("must persist") {
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Xai(notif)) => {
            assert!(hystersis_persisted_event_id(&notif).is_some());
        }
...
async fn current_mode_update_persisted_line_is_stamped() {
    ...
    emit_current_mode_update(&config, hystersis_tools::types::SessionMode::Plan).await;
...
async fn scheduled_task_fired_is_not_persisted() {
    ...
    let notification = ToolNotification::ScheduledTaskFired(
        hystersis_tools::notification::types::ScheduledTaskFired {
            task_id: "loop-1".into(),
            prompt: "check deploy".into(),
            human_schedule: "every 5 minutes".into(),
            next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    let hystersis_acp_lib::AcpClientMessage::ExtNotification(fired) = fired else {
        panic!("expected scheduler fire notification");
    };
    let value: serde_json::Value = serde_json::from_str(fired.request.params.get()).unwrap();
    assert_eq!(value["_meta"]["hystersis.com/schedulerGeneration"], "generation-a");
    assert_eq!(value["_meta"]["hystersis.com/schedulerRevision"], 3);
}
...
async fn legacy_monitor_event_without_owner_is_injected() {
    // Legacy / non-hystersis backends record no owner; such events must
...
async fn block_waited_task_skips_auto_wake_prompt() {
    ...
    // The hystersis.com/task_completed ExtNotification for UI updates must still be sent.
    let mut found_ext = false;
    while let Ok(msg) = gateway_rx.try_recv() {
        if let hystersis_acp_lib::AcpClientMessage::ExtNotification(args) = msg
            && args.request.method.as_ref() == "hystersis.com/task_completed"
...
async fn plan_mode_exited_emits_current_mode_update_default() {
    ...
    *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
    ...
    let notification =
        ToolNotification::PlanModeExited(hystersis_tools::notification::types::PlanModeExited {
    ...
    while let Ok(msg) = gateway_rx.try_recv() {
        if let hystersis_acp_lib::AcpClientMessage::SessionNotification(args) = msg
...
/// Default (hystersis) polarity: the exit_plan_mode tool result is the model's
...
async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
    ...
    let notification =
        ToolNotification::PlanModeExited(hystersis_tools::notification::types::PlanModeExited {
            tool_call_id: "tc-exit-hystersis".into(),
            plan_content: Some("- step 1".into()),
            plan_file_path: "/tmp/test-session/plan.md".into(),
        });
...
async fn plan_mode_entered_emits_current_mode_update_plan() {
    ...
    let notification =
        ToolNotification::PlanModeEntered(hystersis_tools::notification::types::PlanModeEntered {
...
async fn task_completed_notification_is_frame_bounded() {
    ...
    let params = params.expect("expected an hystersis.com/task_completed notification");
    assert!(
