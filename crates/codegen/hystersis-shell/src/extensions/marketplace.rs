//! `hystersis.com/marketplace/*` extension handlers.
//!
//! Provides marketplace browsing and install endpoints for the pager modal.
//! Delegates to `xai-hystersis-plugin-marketplace` crate for scanning and install logic.

use agent_client_protocol as acp;
use hystersis_hooks_plugins_types::{
...
fn load_filtered_marketplace_sources() -> Vec<hystersis_plugin_marketplace::MarketplaceSource> {
    crate::plugin::load_filtered_marketplace_sources()
}

pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "hystersis.com/marketplace/list" => handle_list().await,
        "hystersis.com/marketplace/action" => handle_action(agent, args).await,
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                hystersis_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                hystersis_plugin_marketplace::SourceKind::Local { path } => {
...
async fn handle_action(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            {
                Ok(outcome) => outcome,
                Err(e) => hystersis_hooks_plugins_types::ActionOutcome {
                    status: hystersis_hooks_plugins_types::OutcomeStatus::InternalError,
...
fn refresh_sources(
    sources: &[hystersis_plugin_marketplace::MarketplaceSource],
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                ...
                hystersis_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let hystersis_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = hystersis_plugin_marketplace::git::default_cache_root();
            if let Err(e) = hystersis_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
    ...
    hystersis_hooks_plugins_types::ActionOutcome {
        status: hystersis_hooks_plugins_types::OutcomeStatus::Success,
        message: msg,
        requires_reload: false,
        requires_restart: false,
    }
...
async fn handle_update(
    ...
    use hystersis_plugin_marketplace::installer;
    use hystersis_hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &hystersis_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            hystersis_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            hystersis_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match hystersis_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        hystersis_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = hystersis_plugin_marketplace::git::default_cache_root();
            match hystersis_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                hystersis_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = hystersis_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = hystersis_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = hystersis_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, hystersis_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(hystersis_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            hystersis_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match hystersis_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    hystersis_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &hystersis_plugin_marketplace::MarketplaceSource,
    ...
    let scan = hystersis_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = hystersis_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match hystersis_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: hystersis_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.hystersis/config.toml`.
async fn handle_add_source(url: &str) -> hystersis_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &hystersis_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            hystersis_plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
                ...
                return ActionOutcome {
                    ...
                    message: format!(
                        "{e}. Not a reachable git repository — to add it anyway (e.g. a \
                         VPN-gated host), run: hystersis plugin marketplace add {url} --force"
                    ),
                    requires_reload: false,
                    requires_restart: false,
                };
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if hystersis_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        hystersis_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = hystersis_config::hystersis_home().join("config.toml");
    let hystersis_home = hystersis_config::hystersis_home();
    let _save_guard = crate::util::config::lock_config_writes().await;
    let write = {
        let name = name.clone();
        tokio::task::spawn_blocking(move || {
            let _flock = acquire_init_lock(&hystersis_home).ok();
...
/// Remove a marketplace source from `~/.hystersis/config.toml` and uninstall all
/// plugins that were installed from it.
async fn handle_remove_source(source_url_or_path: &str) -> hystersis_hooks_plugins_types::ActionOutcome {
...
fn remove_source_locked(source_url_or_path: &str) -> hystersis_hooks_plugins_types::ActionOutcome {
    use crate::plugin;
    use hystersis_hooks_plugins_types::{ActionOutcome, OutcomeStatus};
    ...
    let config_path = hystersis_home.join("config.toml");
    let is_official = hystersis_plugin_marketplace::is_official_source_url(source_url_or_path);
...
/// Acquire an advisory exclusive `flock` on `<hystersis_home>/.config-init.lock`,
...
fn acquire_init_lock(hystersis_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(hystersis_home);
    let lock_path = hystersis_home.join(".config-init.lock");
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a hystersis_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub(crate) fn purge_default_skills_installs(hystersis_home: &std::path::Path) {
    purge_default_skills_installs_impl(hystersis_home, || {
        hystersis_agent::plugins::install_registry::InstallRegistry::try_load_from(
            hystersis_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}

fn purge_default_skills_installs_impl(
    hystersis_home: &std::path::Path,
    load_registry: impl FnOnce() -> Result<
        hystersis_agent::plugins::install_registry::InstallRegistry,
        hystersis_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = hystersis_home.join("config.toml");
    ...
    let _lock = match acquire_init_lock(hystersis_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %hystersis_home.join(".config-init.lock").display(),
...
pub(crate) fn ensure_official_marketplace_source(hystersis_home: &std::path::Path) {
    let config_path = hystersis_home.join("config.toml");
    ...
    // (settings.json / known_marketplaces.json) under hystersis_home. Scoped to
    // hystersis_home only (not ~/.claude) to keep tests hermetic; a user with the URL
    // solely in ~/.claude gets one duplicate entry that the UI dedupes by URL.
    let toml_sources = hystersis_plugin_marketplace::load_sources(&parsed);
    let json_sources = hystersis_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&hystersis_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, hystersis_plugin_marketplace::SourceKind::Git { url, .. }
            if hystersis_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            hystersis_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<hystersis_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        hystersis_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            hystersis_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                hystersis_plugin_marketplace::SourceKind::Git { url, .. }
                    if hystersis_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            hystersis_plugin_marketplace::SourceKind::Git { url, .. }
                if url == hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
...
mod default_skills_purge_tests {
    use super::*;
    use hystersis_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = hystersis_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            ...
            components: Some(hystersis_hooks_plugins_types::PluginComponents {
                skills: vec![hystersis_hooks_plugins_types::ComponentItem::new(
                    "code-review",
