...
/// `hystersis_mcp::wire`), these methods are private to the agent↔client channel and
...
pub struct McpServerEntry {
    ...
    pub icons: Vec<hystersis_mcp::servers::McpIcon>,
...
pub async fn notify_servers_updated(
    gateway: &hystersis_acp_lib::AcpAgentGatewaySender,
...
pub(crate) async fn build_mcp_status(
    ...
    tool_bridge: &Arc<hystersis_tools::bridge::ToolBridge>,
    event_writer: Option<&hystersis_session_events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(hystersis_session_events::Event::McpHealthCheck {
...
pub(crate) async fn init_agent_mcp_pool(
    ...
    let noop = hystersis_session_events::EventWriter::noop();
    ...
    let clients: hystersis_mcp::owned_clients::OwnedClients = results
        .into_iter()
...
impl hystersis_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<hystersis_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(hystersis_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<hystersis_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(hystersis_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(hystersis_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(hystersis_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
async fn handle_setup(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let allowlist = &hystersis_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
mod tests {
    ...
    fn test_mcp_list_icons_serialization() {
        let entry = McpServerEntry {
            ...
            icons: vec![hystersis_mcp::servers::McpIcon {
                src: "https://example.com/icon.png".to_string(),
                mime_type: Some("image/png".to_string()),
                sizes: Some(vec!["48x48".to_string()]),
                theme: Some(hystersis_mcp::servers::McpIconTheme::Dark),
            }],
            source: McpServerSource::Local,
            source_label: None,
            setup: None,
