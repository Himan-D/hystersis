//! `hystersis.com/auth/*` and legacy `hystersis.com/{get,set}ApiKey` extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "hystersis.com/auth/getBearerToken" => handle_get_bearer_token(agent).await,
        "hystersis.com/getApiKey" => handle_get_api_key(),
        "hystersis.com/setApiKey" => handle_set_api_key(args),
        "hystersis.com/auth/submit_code" => handle_submit_code(agent, args),
        "hystersis.com/auth/get_url" => handle_get_url(agent).await,
        "hystersis.com/auth/cancel" => handle_cancel(agent, args),
        "hystersis.com/auth/logout" => handle_logout(agent, args).await,
        "hystersis.com/auth/info" => handle_info(agent),
        "hystersis.com/auth/check_subscription" => handle_check_subscription(agent).await,
...
fn handle_get_api_key() -> ExtResult {
    let key = crate::agent::auth_method::read_hystersis_api_key_env().ok();
...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&hystersis_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("XAI_API_KEY") };
        } else {
            crate::auth::store_api_key(&hystersis_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `hystersis-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a hystersis-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("hystersis-asset:///{key}")),
        None => None,
    };
    to_raw_response(&AuthInfoResponse {
        ...
        // matching `AuthManager::allows_data_collection` / HystersisAuth Default.
