...
pub const TITLE_IS_MANUAL_META_KEY: &str = "hystersis.com/titleIsManual";
...
impl PromptUsage {
    ...
    pub(crate) fn project_from_ledger(
        ledger: Option<&hystersis_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
...
impl From<&hystersis_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &hystersis_chat_state::UsageTotals) -> Self {
        ...
        let hystersis_chat_state::UsageTotals {
            input_tokens,
            output_tokens,
            cached_read_tokens,
...
impl From<&hystersis_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &hystersis_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<hystersis_hooks_plugins_types::HookInfo>,
    ...
    SessionStatus(Box<hystersis_status_line::StatusLineContext>),
    ...
    ///
    /// Emitted by the `hystersis.com/recap` ext method: on demand via the `/recap`
    ...
    /// outcome. Rides the persisted `_hystersis.com/session/update` rail (unlike the
    /// fire-and-forget `hystersis.com/session/prompt_complete` notification), so a
...
/// detailed migration guidance (`hystersis update` / `hystersis logout` / `hystersis login`)
...
pub struct CompactionSegmentFile {
    pub items: Vec<hystersis_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: hystersis_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    /// Note: this is a per-artifact ID, not the model API's `x_hystersis_req_id`
    ...
    /// or `"detailed"` (10-section structured prompt for hystersis and similar agents).
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<hystersis_chat_state::compaction_utils::CompactionAttempt>,
}
...
pub struct RecapRequestFile {
    ...
    /// API's `x_hystersis_req_id` (also recorded below for proxy correlation).
    pub request_id: String,
    ...
    pub x_hystersis_req_id: String,
    /// Sampling conversation id (`recap-{uuid}`).
    pub x_hystersis_conv_id: String,
...
mod tests {
    ...
    fn recap_request_file_roundtrips() {
        let artifact = RecapRequestFile {
            ...
            x_hystersis_req_id: "xai-recap-abc".into(),
            x_hystersis_conv_id: "recap-abc".into(),
        ...
        assert_eq!(parsed.x_hystersis_req_id, "xai-recap-abc");
        assert_eq!(
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use hystersis_chat_state::compaction_utils::CompactionAttempt;
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.hystersis/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.hystersis/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.hystersis/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.hystersis/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.hystersis/memory/MEMORY.md".into(),
    ...
    fn make_goal_updated_full() -> SessionUpdate {
        SessionUpdate::GoalUpdated {
            ...
            live_tokens_by_model: vec![("hystersis-4".into(), 6_000), ("hystersis-3".into(), 4_000)],
    ...
    fn goal_updated_serializes_snake_case_tag() {
        ...
        assert_eq!(json["live_tokens_by_model"][0][0], "hystersis-4");
    ...
    fn model_changed_serializes_snake_case_with_optional_effort() {
        ...
        assert_eq!(json["sessionUpdate"], "model_changed");
        assert_eq!(json["model_id"], "hystersis-3");
        assert!(
    ...
    fn model_changed_envelope_carries_session_id_at_top_level() {
        ...
        assert_eq!(json["update"]["model_id"], "hystersis-4");
    }
