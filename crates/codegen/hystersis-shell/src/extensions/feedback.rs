//! `hystersis.com/feedback`, `hystersis.com/feedback/dismiss`, `hystersis.com/btw`, and `hystersis.com/review/*`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "hystersis.com/feedback" | "hystersis.com/feedback/dismiss" => {
            tracing::info!("handling user feedback");
            handle_feedback(agent, args).await
        }
        "hystersis.com/feedback/upload-trace" => handle_upload_trace(agent, args).await,
        m if m.starts_with("hystersis.com/review") => {
...
/// Handle `hystersis.com/btw` -- a side question that doesn't interrupt the current turn.
...
async fn handle_feedback(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !agent.cfg.borrow().is_feedback_enabled() {
        return Err(acp::Error::internal_error().data(
            "Feedback is disabled. To enable, set HYSTERSIS_FEEDBACK_ENABLED=true or \
    ...
    match args.method.as_ref() {
        ...
        "hystersis.com/feedback/dismiss" => {
...
async fn handle_upload_trace(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    match tokio::time::timeout(
        ...
        hystersis_file_utils::gcs::upload_bytes(
...
///
/// Methods:
/// - `hystersis.com/review/comment`: record a new inline code comment to cloud storage
/// - `hystersis.com/review/comment/delete`: record a tombstone event for a deleted comment
async fn handle_review(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "hystersis.com/review/comment" => {
        ...
        "hystersis.com/review/comment/delete" => {
