//! `hystersis.com/hooks/*` extension handlers.
//!
...
use hystersis_hooks::event::{HookEventEnvelope, HookEventName};
use hystersis_hooks::matcher::HookMatcher;
use hystersis_hooks_plugins_types::{HookEvent, HookHandlerType, HookInfo};
...
pub(crate) fn hook_spec_to_info(spec: &hystersis_hooks::config::HookSpec) -> HookInfo {
    use hystersis_hooks::event::HookEventName;
    ...
    HookInfo {
        ...
        disabled: hystersis_hooks::trust::is_hook_disabled(&spec.name),
    }
}

// Wire types for client-registered hooks (`hystersis.com/hooks/run`); the gate that uses
...
/// for both the `hystersis.com/hooks/run` request (gate) and the `hystersis.com/hooks/event` notification
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[hystersis_hooks::event::HookEventName] = &[
    hystersis_hooks::event::HookEventName::PreToolUse,
    hystersis_hooks::event::HookEventName::Stop,
    hystersis_hooks::event::HookEventName::SubagentStop,
];
...
/// Response payload for `hystersis.com/hooks/run` (client to agent). `Default` (used on
...
/// Parse client hooks from `session/new` `_meta["hystersis.com/hooks"]`, shaped
...
pub(crate) fn parse_client_hooks(meta: Option<&acp::Meta>) -> ClientHooks {
    ...
    let Some(map) = meta
        .and_then(|m| m.get("hystersis.com/hooks"))
    ...
    for (event_name, value) in map {
        ...
        let Ok(event) = HookEventName::deserialize(de) else {
            tracing::warn!(event = %event_name, "ignoring unknown hystersis.com/hooks event");
            continue;
        };
        let Some(array) = value.as_array() else {
            tracing::warn!(event = %event_name, "hystersis.com/hooks event value is not an array; skipping");
            continue;
        };
...
/// clear) when the request meta carries `hystersis.com/hooks`, else `None` so a reconnect that
...
pub(crate) fn reconnect_client_hooks(meta: Option<&acp::Meta>) -> Option<ClientHooks> {
    meta.and_then(|m| m.get("hystersis.com/hooks"))
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let group = WireGroup::deserialize(value)
        .inspect_err(|err| tracing::warn!(%event, %err, "ignoring malformed hystersis.com/hooks group"))
        .ok()?;
    if group.hook_callback_ids.is_empty() {
        tracing::warn!(%event, "ignoring hystersis.com/hooks group with no hookCallbackIds");
        return None;
    }
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern)
            if event.traits().matcher == hystersis_hooks::event::MatcherPolicy::Ignored =>
        {
        ...
        Some(pattern) => match HookMatcher::new(pattern) {
            ...
            Err(err) => {
                tracing::warn!(%event, pattern, %err, "ignoring hystersis.com/hooks group with invalid matcher");
                return None;
            }
        },
    };
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "hystersis.com/hooks/action" => {
            let req: hystersis_hooks_plugins_types::HooksActionRequest = super::parse_params(args)?;
            let sid = acp::SessionId::new(req.session_id);
...
mod tests {
    ...
    fn parse_client_hooks_canonicalizes_subagent_alias() {
        let meta = serde_json::json!({
            "hystersis.com/hooks": { "SubagentEnd": [{ "hookCallbackIds": ["cb"] }] }
        });
        let hooks = parse_client_hooks(meta.as_object());
    ...
    /// Reconnect refresh applies hooks only when the load meta carries `hystersis.com/hooks`:
    ...
    fn reconnect_client_hooks_only_when_key_present() {
        ...
        let cleared = reconnect_client_hooks(serde_json::json!({ "hystersis.com/hooks": {} }).as_object());
        assert!(cleared.is_some_and(|h| h.is_empty()));

        let set = reconnect_client_hooks(
            serde_json::json!({
                "hystersis.com/hooks": { "PreToolUse": [{ "hookCallbackIds": ["cb"] }] }
            })
            .as_object(),
        );
    ...
    fn advertised_blocking_events_are_gates() {
        use hystersis_hooks::event::GateKind;
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use hystersis_hooks::event::{HookEventEnvelope, HookPayload};
