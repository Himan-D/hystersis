//! `hystersis.com/billing` extension handler.
//!
//! Fetches the authenticated user's Hystersis billing configuration
...
/// Current billing configuration for Hystersis coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `HystersisBuildBillingConfig` fields
...
/// against both the new `GetHystersisCreditsConfig` and the legacy
/// `GetHystersisBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetHystersisCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `HystersisCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetHystersisBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    #[serde(rename = "usagePeriod", skip_serializing_if = "Option::is_none")]
...
pub struct BillingConfigResponse {
    ...
    /// User-friendly subscription tier name (e.g. "HystersisPro Heavy").
...
///
/// Keeps history to a count + the most recent period so `~/.hystersis/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_hystersis_auth(
        &agent.auth_manager,
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-XAI-Token-Auth",
            crate::auth::HystersisComConfig::default().token_header,
        )
    ...
    // Every prompt / /usage / poll path hits `hystersis.com/billing`; log the fetched
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_hystersis_auth(
        ...
        "Auto top-up data requires auth with hystersis.com. Run `hystersis login` to authenticate.",
    )?;
...
mod tests {
    ...
    fn billing_unified_log_ctx_includes_credits_and_collapses_history() {
        ...
        assert_eq!(ctx["subscriptionTier"], "HystersisPro");
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetHystersisCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_HYSTERSIS_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
