//! `hystersis.com/session/usage` — cumulative session token/cost as [`PromptUsage`].
//!
//! Projects the in-memory [`hystersis_chat_state::UsageLedger`] (main-loop + folded
...
/// Wire response for `hystersis.com/session/usage`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "hystersis.com/session/usage" => handle_session_usage(agent, args).await,
...
mod tests {
    use super::*;
    use hystersis_chat_state::UsageLedger;
    ...
    fn response_serializes_ledger_as_prompt_usage_wire_shape() {
        ...
        ledger.record_main_loop_call("hystersis", &usage(100, 10), Some(50), Some(20_000_000));
        ...
        assert_eq!(v["usage"]["modelUsage"]["hystersis"]["inputTokens"], 100);
