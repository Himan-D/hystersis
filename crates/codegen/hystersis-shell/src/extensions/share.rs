//! `hystersis.com/share_session` extension handler.
//!
...
async fn handle_share_session(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let auth = require_hystersis_auth_for_share(&agent.auth_manager)?;
...
async fn upload_share_data_to_gcs(
    ...
    if let Err(e) = hystersis_file_utils::gcs::upload_bytes_signed(
...
fn require_hystersis_auth_for_share(
    auth_manager: &crate::auth::AuthManager,
) -> Result<crate::auth::HystersisAuth, acp::Error> {
    super::auth_gate::require_hystersis_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `hystersis login` to authenticate.",
    )
}
...
mod tests {
    ...
    fn make_auth_manager_with_token_expiring_in(
        ...
        // Only OIDC tokens against https://auth.hystersis.com (or the local-dev equivalent)
        // return true from is_hystersis_auth(). This is required for the share tests to
        // exercise the happy path through require_hystersis_auth_for_share.
    ...
    fn share_works_outside_the_5m_early_invalidation_window() {
        ...
        assert!(require_hystersis_auth_for_share(&mgr).is_ok());
    }
    ...
    fn share_succeeds_inside_the_5m_early_invalidation_window() {
        ...
        let res = require_hystersis_auth_for_share(&mgr);
        assert!(
            res.is_ok(),
            "require_hystersis_auth_for_share must succeed for a still-valid buffered xAI token"
        );
    }
    ...
    fn share_fails_with_no_auth_at_all() {
        ...
        assert!(require_hystersis_auth_for_share(&mgr).is_err());
    }

    #[test]
    fn share_rejects_non_hystersis_auth_with_actionable_hystersis_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            HystersisComConfig::default(),
        ...
        let err = require_hystersis_auth_for_share(&mgr)
