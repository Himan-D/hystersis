...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "HYSTERSIS_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$HYSTERSIS_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${HYSTERSIS_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$HYSTERSIS_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$HYSTERSIS_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the HYSTERSIS_MEMORY env var.
...
/// Run `f` with HYSTERSIS_MEMORY explicitly unset.
fn without_hystersis_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("HYSTERSIS_MEMORY", None, f)
}
/// Run `f` with HYSTERSIS_MEMORY set to a specific value.
fn with_hystersis_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("HYSTERSIS_MEMORY", Some(value), f)
}
#[test]
fn memory_config_default_disabled() {
    without_hystersis_memory(|| {
...
fn memory_config_env_var_zero_does_not_enable() {
    with_hystersis_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "HYSTERSIS_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_hystersis_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "HYSTERSIS_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_hystersis_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "HYSTERSIS_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_hystersis_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "HYSTERSIS_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_hystersis_memory(
        ...
        || {
            ...
            assert!(
                mem.enabled,
                "CLI --experimental-memory should override HYSTERSIS_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_hystersis_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(!mem.enabled, "--no-memory should override HYSTERSIS_MEMORY=1");
        },
    );
}
#[test]
...
"#;
        ...
        assert_eq!(mem.flush.flush_model.as_deref(), Some("hystersis-4"));
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$HYSTERSIS_TEST_USER $HYSTERSIS_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the HYSTERSIS_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with HYSTERSIS_SUBAGENTS explicitly unset.
fn without_hystersis_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("HYSTERSIS_SUBAGENTS", None, f)
}
/// Run `f` with HYSTERSIS_SUBAGENTS set to a specific value.
fn with_hystersis_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("HYSTERSIS_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_hystersis_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
...
fn subagent_limit_counts_resolve_env_over_toml_over_remote_over_default() {
    use hystersis_tools::implementations::hystersis_build::task::admission;
...
fn subagent_sampling_limit_applies_precedence_and_clamps() {
    ...
    use hystersis_tools::implementations::hystersis_build::task::admission::DEFAULT_MAX_CONCURRENT;
...
fn subagent_limit_behavior_resolves_env_over_toml_over_remote_over_queue() {
    use hystersis_tools::implementations::hystersis_build::task::admission::LimitBehavior;
    ...
    with_hystersis_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(!sa.enabled, "HYSTERSIS_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_hystersis_subagents(
        ...
        || {
            ...
            assert!(
                !sa.enabled,
                "HYSTERSIS_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_hystersis_subagents(
        ...
        || {
            ...
            assert!(
                sa.enabled,
                "--subagents CLI flag should override HYSTERSIS_SUBAGENTS=0"
            );
        },
    );
}
...
fn subagents_config_models_parsed() {
    without_hystersis_subagents(|| {
        ...
        assert_eq!(sa.models.get("explore").unwrap(), "hystersis-3-fast");
        assert_eq!(sa.models.get("plan").unwrap(), "hystersis-4.5");
    });
}
#[test]
    ...
    with_hystersis_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(sa.enabled, "HYSTERSIS_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "hystersis-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        "HYSTERSIS_MANAGED_MCPS_ENABLED",
        managed_mcps,
        || with_env_var_opt("HYSTERSIS_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "HYSTERSIS_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "HYSTERSIS_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "HYSTERSIS_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("HYSTERSIS_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_hystersis_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_hystersis_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "HYSTERSIS_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("HYSTERSIS_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_hystersis_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_hystersis_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_hystersis_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
...
fn tools_config_env_var_disables() {
    with_hystersis_respect_gitignore(
        "0",
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let tc = ToolsConfig::resolve(&config);
    ...
    with_hystersis_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "HYSTERSIS_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn media_gen_caps_resolve_env_over_toml_over_remote_over_default() {
    use hystersis_tools::media_gen_limits::{
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".hystersis/prompts/impl.md"
        "#;
    ...
    assert_eq!(researcher.model.as_deref(), Some("hystersis-3"));
    ...
    assert_eq!(
            ...
            Some(".hystersis/prompts/impl.md")
        );
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".hystersis/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".hystersis").join("roles");
...
fn personas_parse_from_toml() {
    let toml_str = r#"
            ...
            instructions_file = ".hystersis/personas/concise.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    ...
    assert_eq!(
            ...
            Some(".hystersis/personas/concise.md")
        );
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".hystersis").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".hystersis").join("bundled");
    std::fs::create_dir_all(workspace.join(".hystersis").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".hystersis").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".hystersis").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".hystersis").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".hystersis/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".hystersis/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".hystersis/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".hystersis/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".hystersis/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".hystersis/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".hystersis/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".hystersis/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.hystersis/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
/// a repo `.hystersis/config.toml` structurally cannot supply one.)
#[test]
...
fn enterprise_two_file_merge_routes_deployment_key_to_proxy() {
    for k in [
        ...
        "HYSTERSIS_TRACE_UPLOAD_ENDPOINT_URL",
    ] {
...
cli_chat_proxy_base_url = "https://cli-chat-proxy.hystersis.com/v1"
...
"#,
    ...
    assert_eq!(
            ...
            "https://cli-chat-proxy.hystersis.com/v1/deployment/config"
        );
    assert!(
            !cfg.endpoints
...
/// RCE guard: a project `.hystersis/config.toml` must never source
...
fn project_config_never_sources_feedback_user() {
    ...
    let hystersis = repo.path().join(".hystersis");
...
fn apply_requirements_value_overrides_user_settings() {
    ...
    assert_eq!(
            ...
            cfg.endpoints.hystersis_api_base_url
        );
    assert_eq!(
...
fn a_repeated_pin_is_reported_against_the_layer_that_decided() {
    ...
    let user = RequirementSource::Requirements {
        path: std::path::PathBuf::from("/home/dev/.hystersis/requirements.toml"),
    };
    let system = RequirementSource::Requirements {
        path: std::path::PathBuf::from("/etc/hystersis/requirements.toml"),
    };
...
/// HYSTERSIS_TITLE_REFRESH, which a config-tier value would lose to.
#[test]
#[serial_test::serial]
fn apply_requirements_pins_title_refresh_over_the_environment() {
    ...
    assert!(!resolved.value, "the pin lost to HYSTERSIS_TITLE_REFRESH");
    assert_eq!(
            resolved.source,
...
#[test]
fn validate_hooks_path_rejects_outside_hystersis_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
            msg.contains("must be under ~/.hystersis/"),
            "should mention ~/.hystersis/ restriction, got: {msg}"
        );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let hystersis_home = crate::util::hystersis_home::hystersis_home();
    let traversal = format!("{}/../evil", hystersis_home.display());
...
#[test]
fn validate_hooks_path_accepts_hystersis_hooks_subdir() {
    let hystersis_home = crate::util::hystersis_home::hystersis_home();
    let valid_path = hystersis_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.hystersis/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use crate::agent::config::Feature;
    use hystersis_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own hystersis yolo.
#[test]
...
fn simulate_release_build() -> hystersis_test_support::EnvGuard {
    hystersis_test_support::EnvGuard::set(hystersis_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".hystersis"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
#[test]
fn explicit_hystersis_root_is_the_only_user_source() {
    let tmp = tempfile::tempdir().unwrap();
    let ambient = tmp.path().join("ambient-home/.hystersis");
    let configured = tmp.path().join("configured-hystersis-home");
...
/// verdicts untouched). HYSTERSIS_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores HYSTERSIS_HOME even on panic). No user-global
/// `$HYSTERSIS_HOME/config.toml` is seeded: `hystersis_home()` is `OnceLock`-cached,
...
/// in `xai-hystersis-agent`. An ABSOLUTE plugin path is used so the merged
...
/// HYSTERSIS_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use hystersis_agent::plugins::{TrustStore, discover_plugins};
    use hystersis_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("HYSTERSIS_HOME", home.path());
    let _flag = EnvGuard::unset("HYSTERSIS_FOLDER_TRUST");
    ...
    std::fs::write(plugin_dir.join("plugin.json"), r#"{"name":"cfgpath-probe"}"#)
        .unwrap();
    let hystersis = cwd.join(".hystersis");
    std::fs::create_dir_all(&hystersis).unwrap();
    std::fs::write(
            hystersis.join("config.toml"),
            format!("[plugins]\npaths = ['{}']\n", plugin_dir.display()),
...
/// arm (store-only reconcile) could never lift. HYSTERSIS_HOME-isolated (empty
/// store); HYSTERSIS_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
fn kill_switched_cold_cwd_stays_allowed_through_plugins_config_read() {
    ...
    let hystersis = repo.path().join(".hystersis");
    std::fs::create_dir_all(&hystersis).unwrap();
    std::fs::write(hystersis.join("config.toml"), "[plugins]\npaths = [\"./proj-plugin\"]\n")
        .unwrap();
    let cwd = repo.path();
    let remote = crate::util::config::RemoteSettings {
...
/// Writeback requires hystersis.com auth: remote may advertise it, but a non-xai
/// credential is downgraded to `Local`.
#[test]
#[serial_test::serial]
fn from_remote_gated_requires_hystersis_auth_for_writeback() {
