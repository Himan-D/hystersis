...
use super::is_hystersis_oauth2_issuer;
...
pub(super) const LEGACY_SCOPE: &str = "https://accounts.hystersis.com/sign-in";

/// auth.json scope key for plain API key auth (desktop login, `hystersis login --api-key`).
...
/// server enrichment opts in. Single source for `HystersisAuth`, `AuthMeta`, and
...
pub enum AuthMode {
    ...
    External,
    /// Plain API key (e.g. from hystersis-desktop login or `hystersis login --api-key`)
    ApiKey,
}
...
pub struct HystersisAuth {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub has_hystersis_code_access: Option<bool>,
    ...
    /// provider's `issuer` claim. In both modes an hystersis.com issuer marks the
    /// credential first-party (`is_hystersis_auth`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl std::fmt::Debug for HystersisAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HystersisAuth")
...
impl HystersisAuth {
    ...
    /// either an OIDC login against https://auth.hystersis.com (or the local-dev
    ...
    pub fn is_hystersis_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::Oidc | AuthMode::External => self
                .oidc_issuer
                .as_deref()
                .is_some_and(is_hystersis_oauth2_issuer),
            AuthMode::ApiKey | AuthMode::WebLogin => false,
        }
    }

    /// `true` when this auth can access hystersis.com managed MCP connectors.
    pub fn is_managed_mcp_eligible(&self) -> bool {
        self.is_hystersis_auth() || self.auth_mode == AuthMode::WebLogin
    }
    ...
    /// (`is_hystersis_auth`), matching the built-in devbox login they replace.
    /// Plain API keys never do.
    pub(crate) fn is_session_auth(&self) -> bool {
        match self.auth_mode {
            AuthMode::WebLogin | AuthMode::Oidc => true,
            AuthMode::External => self.is_hystersis_auth(),
    ...
    pub(crate) fn carry_user_profile_from(&mut self, prev: &HystersisAuth) {
...
impl Default for HystersisAuth {
    fn default() -> Self {
        Self {
            key: String::new(),
...
#[cfg(test)]
impl HystersisAuth {
    /// Returns a `HystersisAuth` with sensible defaults for tests. Override fields
    /// with struct update syntax:
    /// ```ignore
    /// HystersisAuth { key: "my-key".into(), ..HystersisAuth::test_default() }
    /// ```
...
pub(crate) type AuthStore = BTreeMap<String, HystersisAuth>;
...
///
/// Legacy `WebLogin` tokens (from the pre-OIDC `hystersis login --legacy`
...
pub fn lookup_auth(map: &AuthStore, scope: &str) -> Option<HystersisAuth> {
    // Rebrand fallback: old CLI stored OIDC under https://auth.x.ai::client_id
    // New CLI uses https://auth.hystersis.com::client_id. Try new scope first, then
    // legacy x.ai scope, then legacy WebLogin scope — so multiple logins persist
    // and usage isn't lost after upgrade.
    const LEGACY_XAI_SCOPE: &str = "https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828";
    let auth = map
        .get(scope)
        .cloned()
        .or_else(|| {
            // Only fallback to x.ai when caller is asking for hystersis.com
            if scope.starts_with("https://auth.hystersis.com::") {
                map.get(LEGACY_XAI_SCOPE).cloned()
            } else {
                None
            }
        })
...
/// Early-invalidation buffer. Override with `HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS`
/// for testing (e.g. `=5` to shrink the buffer to 5 seconds).
pub(super) fn early_invalidation() -> Duration {
    std::env::var("HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS")
        .ok()
...
pub(crate) fn is_expired(auth: &HystersisAuth) -> bool {
...
pub(crate) fn is_expired_with_buffer(auth: &HystersisAuth, buffer: Duration) -> bool {
...
mod tests {
    ...
    fn make_auth(mode: AuthMode) -> HystersisAuth {
        HystersisAuth {
            key: "k".into(),
            auth_mode: mode,
            create_time: Utc::now(),
    ...
    #[test]
    fn is_hystersis_auth_matrix() {
        use crate::auth::XAI_OAUTH2_ISSUER;
        let with_issuer = |mode: AuthMode, issuer: Option<&str>| HystersisAuth {
            oidc_issuer: issuer.map(str::to_owned),
            ..make_auth(mode)
        };

        // Only Oidc/External qualify, and only with an hystersis.com issuer.
        assert!(with_issuer(AuthMode::Oidc, Some(XAI_OAUTH2_ISSUER)).is_hystersis_auth());
        assert!(with_issuer(AuthMode::External, Some(XAI_OAUTH2_ISSUER)).is_hystersis_auth());
        assert!(!with_issuer(AuthMode::Oidc, None).is_hystersis_auth());
        assert!(!with_issuer(AuthMode::External, None).is_hystersis_auth());
        assert!(!with_issuer(AuthMode::Oidc, Some("https://idp.acme.example")).is_hystersis_auth());
        assert!(!with_issuer(AuthMode::External, Some("https://idp.acme.example")).is_hystersis_auth());

        // ApiKey / WebLogin stay false even with an hystersis.com issuer set.
        assert!(!with_issuer(AuthMode::ApiKey, Some(XAI_OAUTH2_ISSUER)).is_hystersis_auth());
        assert!(!with_issuer(AuthMode::WebLogin, Some(XAI_OAUTH2_ISSUER)).is_hystersis_auth());
    }
    ...
    fn user_info_subscription_tier_present() {
        let json = r#"{
            "userId": "u1",
            "subscriptionTier": "HystersisProPro"
        }"#;
        let info: UserInfo = serde_json::from_str(json).unwrap();
        assert_eq!(info.subscription_tier.as_deref(), Some("HystersisProPro"));
    }
