...
//! attempt (or an explicit `hystersis.com/auth/cancel`) cancels the previous one, so
...
//! scopes explicit cancels so a delayed `hystersis.com/auth/cancel` cannot tear down a
//! successor login.
...
pub(crate) struct AttemptChannels {
    /// Forwards pasted codes from `hystersis.com/auth/submit_code` to the flow.
    code_tx: tokio::sync::mpsc::Sender<String>,
    /// Yields the auth URL to `hystersis.com/auth/get_url`. `Option` so
