...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, HystersisAuth};
...
pub(crate) enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `hystersis login` or set XAI_API_KEY instead."
    )]
    NotEnabled,
}
...
///
/// On success, persists credentials to `~/.hystersis/auth.json` and returns
/// the authenticated `HystersisAuth`.
///
...
pub(crate) async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `hystersis login --device-auth` again.");
        }
...
pub(crate) async fn run_device_code_login_channels(
    ...
    // `hystersis.com/auth/get_url` isn't blocked on a slow/hanging browser launch
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, HystersisComConfig};
    ...
    fn auth_manager_with_hystersis_home(
        hystersis_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(hystersis_home, HystersisComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let hystersis_home = temp_dir.path().join(".hystersis");
        std::fs::create_dir_all(&hystersis_home).unwrap();
        let auth_manager = auth_manager_with_hystersis_home(&hystersis_home, "http://127.0.0.1:9");
    ...
    /// persist nothing.
    fn assert_build_auth_rejected(cfg: HystersisComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let hystersis_home = temp_dir.path().join(".hystersis");
        std::fs::create_dir_all(&hystersis_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&hystersis_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !hystersis_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::HystersisAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_hystersis_home(temp_dir.path(), "http://127.0.0.1:9");
