...
use crate::auth::{AuthManager, HystersisAuth, HystersisComConfig, parse_output};
use crate::http::TransportFailureKind;
use crate::util::hystersis_home;
use hystersis_telemetry::events::{LoginFailed, LoginFailureKind};
...
/// session.
fn is_cached_credential_compatible(auth: &HystersisAuth, hystersis_com_config: &HystersisComConfig) -> bool {
    let expected_issuer = hystersis_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| hystersis_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    let issuer_compatible = match (auth.oidc_issuer.as_deref(), expected_issuer) {
        (Some(actual), Some(expected)) => actual == expected,
        (None, Some(expected)) => {
            // Legacy token without issuer: allow one verification attempt so
            // `enrich_auth_inline` / refresh can populate `oidc_issuer` after
            // interactive login. Strict rejection here caused TUI to re-prompt
            // login immediately after a successful mint, appearing as "not
            // verified". Log and treat as compatible; expiry/refresh will still
            // gate reuse.
            tracing::warn!(
                expected_issuer = expected,
                "auth: cached credential lacks oidc_issuer, treating as compatible for verification"
            );
            true
        }
        _ => true,
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(hystersis_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("HYSTERSIS_LOGIN_DEVICE_FLOW")
...
/// Whether `run_cli_login` should use the device flow. Previously only xAI
/// OAuth2 supported it (enterprise OIDC forced loopback). Now enterprise OIDC
/// may also try device flow when explicitly requested via --device-auth /
/// HYSTERSIS_LOGIN_DEVICE_FLOW / [auth] login_device_flow, with NotEnabled
/// fallback to loopback. Keeps SSH/remote ergonomics for your deployment.
async fn cli_should_use_device(
    _config: &HystersisComConfig,
    login_override: LoginTransportOverride,
) -> bool {
    should_use_device_flow(login_override).await
}

/// Whether interactive xAI OAuth2 login uses the RFC 8628 device flow (vs loopback).
///
/// Precedence: CLI (`--oauth`/`--device-auth`) > `HYSTERSIS_LOGIN_DEVICE_FLOW` env >
/// `[auth] login_device_flow` config > `hystersis_build_login_device_flow` remote feature flag > loopback.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("HYSTERSIS_LOGIN_DEVICE_FLOW");
...
/// How login presents itself; surfaced to the TUI via `hystersis.com/auth/get_url`.
...
impl AuthUrlMode {
    /// Wire string for the `hystersis.com/auth/get_url` ACP response.
...
/// Sets no `HYSTERSIS_AUTH_EXPIRED`: operator binaries, which live outside this
...
async fn run_external_auth_provider(
...
) -> anyhow::Result<(HystersisAuth, bool)> {
    ...
    let principal_policy =
        crate::auth::oidc::login_principal_policy(auth_manager.hystersis_com_config());
...
pub(crate) async fn run_auth_flow_with_stderr_bridge(
    auth_manager: &Arc<AuthManager>,
    hystersis_com_config: &HystersisComConfig,
...
async fn run_auth_flow_inner(
    ...
    {
        hystersis_telemetry::session_ctx::log_event(event);
    }
    result
}
...
async fn run_auth_flow_steps(
    ...
    tracing::info!(
        has_oidc = hystersis_com_config.oidc.is_some(),
        has_oauth2 = hystersis_com_config.oauth2.is_some(),
        has_external_auth = hystersis_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if reauth {
        auth_manager.clear()?;
        // Also remove the legacy accounts.hystersis.com scope so stale tokens
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, hystersis_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, hystersis_com_config)
        }) {
            hystersis_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = hystersis_com_config.auth_provider_command {
    ...
    // `hystersis login --devbox` uses run_devbox_login and is not gated here.
    if !hystersis_com_config.blocks_automatic_oidc()
    ...
    // Device flow is now attempted for both xAI OAuth2 and enterprise OIDC
    // when should_use_device_flow() is true ( --device-auth / env / config ).
    // If IdP returns 404 NotEnabled, we fall back to loopback below, so your
    // enterprise IdP works whether it supports RFC8628 or not. Loopback remains
    // the default for backwards compat.
    if crate::auth::oidc::is_configured(hystersis_com_config) {
        if should_use_device_flow(login_override).await {
            if let Some(ref oidc_cfg) = hystersis_com_config.oidc {
                ...
                .await
                {
                    ...
                    {
                        tracing::warn!(
                            "auth: enterprise OIDC device flow unavailable (404), falling back to loopback login"
                        );
                    }
                    other => return other,
                }
        ...
        return crate::auth::oidc::run_login_flow(hystersis_com_config, auth_manager, channels).await;
    }

    if let Some(ref oauth2_cfg) = hystersis_com_config.oauth2 {
        if should_use_device_flow(login_override).await {
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `hystersis login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
pub async fn try_ensure_fresh_auth(hystersis_com_config: &HystersisComConfig) -> Option<HystersisAuth> {
    try_ensure_fresh_auth_with(&build_startup_auth_manager(hystersis_com_config)).await
}

/// Builds and configures the startup `AuthManager`; the policy helpers below
/// take it injected so tests can substitute their own.
fn build_startup_auth_manager(hystersis_com_config: &HystersisComConfig) -> Arc<AuthManager> {
    let auth_manager = Arc::new(AuthManager::new(
        &hystersis_home::hystersis_home(),
        hystersis_com_config.clone(),
    ));
    // auth()'s OIDC/external refresh needs the refresher configured first.
    auth_manager.configure_refresher(hystersis_com_config.auth_provider_command.clone(), None);
    auth_manager
}

/// Policy: cached-valid creds, else silent refresh (no interactive login).
async fn try_ensure_fresh_auth_with(auth_manager: &Arc<AuthManager>) -> Option<HystersisAuth> {
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_noninteractive_auth_no_mint(
    hystersis_com_config: &HystersisComConfig,
) -> Option<HystersisAuth> {
    try_noninteractive_auth_no_mint_with(&build_startup_auth_manager(hystersis_com_config)).await
}

/// Policy behind [`try_noninteractive_auth_no_mint`], with the `AuthManager`
/// injected for tests.
async fn try_noninteractive_auth_no_mint_with(auth_manager: &Arc<AuthManager>) -> Option<HystersisAuth> {
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<HystersisAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_hystersis_auth() && a.refresh_token.is_some())
}
...
pub(crate) async fn mint_session_noninteractive(
    ...
    let hystersis_com_config = auth_manager.hystersis_com_config();
    ...
    if let Some(cmd) = hystersis_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: HystersisAuth) -> HystersisAuth {
...
fn report_signed_in(auth: &HystersisAuth) {
    eprint!("\r\x1b[K");
    match auth.email {
...
pub async fn ensure_authenticated_with_override(
    ...
    let hystersis_home = hystersis_home::hystersis_home();
    let auth_manager = Arc::new(AuthManager::new(&hystersis_home, hystersis_com_config.clone()));
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<HystersisAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(hystersis_com_config).await)
    } else {
        ensure_authenticated(hystersis_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}

/// Unified `hystersis login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device,
/// otherwise `HYSTERSIS_LOGIN_DEVICE_FLOW` env / `[auth] login_device_flow` config /
...
pub async fn run_cli_login(
    ...
    // client, and `hystersis login` never boots an agent, so without this every
    ...
    let auth_manager = Arc::new(AuthManager::new(
        &hystersis_home::hystersis_home(),
        config.hystersis_com_config.clone(),
    ));
...
async fn run_cli_login_steps(
    ...
    // supports the device flow. Without this guard, `hystersis login` on an
    // enterprise-OIDC deployment would wrongly enter the device branch (which
    // requires `oauth2`) and error.
    let authenticated = if cli_should_use_device(&config.hystersis_com_config, login_override).await {
        if config.hystersis_com_config.oauth2.is_none() {
        ...
        if device_auth && crate::auth::oidc::is_configured(&config.hystersis_com_config) {
            eprintln!(
...
/// config was actually applied; `hystersis setup` reports the no-config case.
async fn apply_post_login_config(authenticated: HystersisAuth) -> anyhow::Result<()> {
...
pub struct LogoutResult {
    ...
    /// `true` if `XAI_API_KEY` / `HYSTERSIS_CODE_XAI_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        hystersis_telemetry::external::set_identity(
            hystersis_telemetry::external::IdentityAttrs::default(),
        );
        hystersis_telemetry::external::flush();
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_hystersis_api_key_env(),
    })
}

/// `hystersis logout` CLI handler. Calls [`perform_logout`] and formats
/// the result to stderr.
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let hystersis_home = hystersis_home::hystersis_home();
    let auth_manager = AuthManager::new(&hystersis_home, config.hystersis_com_config.clone());
...
mod tests {
    ...
    /// `xai-hystersis-http`; what's local is which `LoginFailureKind` each maps to,
    ...
    /// unrelated auth tests, so classification is covered in `xai-hystersis-http`.
    #[test]
    ...
    /// Run `f` with `HYSTERSIS_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("HYSTERSIS_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("HYSTERSIS_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("HYSTERSIS_LOGIN_DEVICE_FLOW"),
        };
        f()
    }

    // A hystersis.com first-party (hystersis.com-issuer) OIDC session — `is_hystersis_auth()` true.
    fn oidc_session(key: &str, refresh: Option<&str>) -> HystersisAuth {
        HystersisAuth {
            key: key.into(),
    ...
    fn expired_refreshable_session_gate() {
        ...
        let mgr = AuthManager::new(dir.path(), HystersisComConfig::default());
        ...
        // Third-party external (no hystersis.com issuer) stays excluded.
        mgr.hot_swap(HystersisAuth {
            oidc_issuer: None,
            auth_mode: AuthMode::External,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn persist_or_use_minted_returns_token_when_save_fails() {
        ...
        // Read-only hystersis_home: reading a missing auth.json succeeds (empty), but
        // writing fails — exercising the save-failure path.
        let dir = tempfile::tempdir().unwrap();
        std::fs::set_permissions(dir.path(), std::fs::Permissions::from_mode(0o500)).unwrap();
        let mgr = Arc::new(AuthManager::new(dir.path(), HystersisComConfig::default()));
    ...
    async fn interactive_login_carries_no_expired_flag_even_over_a_stale_credential() {
        let echo_env = "printf '%s' \"e=${HYSTERSIS_AUTH_EXPIRED:-unset}\"";
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), HystersisComConfig::default())
    ...
    async fn a_provider_written_to_the_published_contract_can_sign_in_after_an_expiry() {
        let conforming =
            r#"if [ "$HYSTERSIS_AUTH_EXPIRED" = "1" ]; then exit 1; else printf '%s' sso-token; fi"#;
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), HystersisComConfig::default())
                .with_proxy_base_url(&dead_proxy_url()),
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        ...
        {
            let _guard = EnvVarGuard::set("HYSTERSIS_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("HYSTERSIS_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    async fn enterprise_oidc_never_uses_device_flow() {
        // oidc=Some, oauth2=None: `hystersis login` must use loopback, not device —
        ...
        let xai = HystersisComConfig::default();
    ...
    fn oidc_auth(issuer: &str) -> HystersisAuth {
        HystersisAuth {
    ...
    fn weblogin_cred_is_never_compatible() {
        let cfg = HystersisComConfig::default();
    ...
    fn pinned_cfg(team: &str) -> HystersisComConfig {
        HystersisComConfig {
    ...
    async fn run_auth_flow_defers_to_consumer_refresh_on_transient_failure() {
        ...
        let expired_with_rt = HystersisAuth {
            key: "expired-access-token".into(),
            auth_mode: AuthMode::Oidc,
            expires_at: Some(Utc::now() - chrono::Duration::hours(1)),
    ...
    async fn run_auth_flow_falls_through_when_no_refresh_token() {
        ...
        let mut cfg = HystersisComConfig::default();
        cfg.oauth2.as_mut().unwrap().issuer = "http://127.0.0.1:1".into();

        let writer = Arc::new(
            AuthManager::new(dir.path(), cfg.clone()).with_proxy_base_url("http://127.0.0.1:1"),
        );
        let expired_no_rt = HystersisAuth {
        ...
        // HYSTERSIS_LOGIN_DEVICE_FLOW / the real config file (the CLI override
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Hystersis: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `hystersis login` passes `on_stderr=None`; stderr must be inherited so
