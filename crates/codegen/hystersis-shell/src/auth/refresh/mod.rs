...
pub(crate) trait ExternalCommandRunner: Send + Sync {
    ...
    async fn run_external_command(&self, command: &str) -> Option<HystersisAuth>;
}
...
impl RefreshOutcome {
    ...
    pub(crate) fn success(auth: HystersisAuth) -> Self {
    ...
    /// flow). Any refresher holding the [`HystersisAuth`] it sent must use
    ...
    ///
    /// Prefer this wherever the attempted [`HystersisAuth`] is in hand: it captures
...
mod tests {
    use super::*;
    use crate::auth::{AuthMode, HystersisAuth, HystersisComConfig};
