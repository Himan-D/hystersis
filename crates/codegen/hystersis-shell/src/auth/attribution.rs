...
//!
//! 1. [`hystersis_telemetry::unified_log::warn`] for the local
...
//!
//! [`hystersis_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`hystersis_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`hystersis_sampler::SamplerConfig::attribution_callback`] at every
...
use hystersis_sampler::{Auth401AttributionCallback, SamplingConsumer};
use hystersis_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`hystersis_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `hystersis_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn hystersis_tools::Auth401AttributionCallback>` for the
...
mod tests {
    ...
    /// `ShellAttribution` implements `hystersis_tools::Auth401AttributionCallback`
