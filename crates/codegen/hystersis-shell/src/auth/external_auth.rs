...
/// Parse stdout into a session-credential `HystersisAuth`.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<HystersisAuth> {
...
pub(crate) async fn run_external_refresh(command: &str) -> Option<HystersisAuth> {
    ...
    cmd.env("HYSTERSIS_AUTH_EXPIRED", "1");
...
pub(crate) async fn refresh_with_command(command: &str, prev_auth: &HystersisAuth) -> Option<HystersisAuth> {
...
mod tests {
    ...
    #[test]
    fn parse_output_issuer_claim_enables_hystersis_auth() {
        ...
        // hystersis.com issuer claim → first-party session (relay-eligible).
        let auth = parse_output(&ok(
            r#"{"access_token":"t","expires_in":900,"issuer":"https://auth.hystersis.com"}"#,
        ))
        .unwrap();
        assert_eq!(auth.oidc_issuer.as_deref(), Some("https://auth.hystersis.com"));
        assert!(auth.is_hystersis_auth());

        // Non-hystersis.com issuer is stored but stays third-party.
        let auth = parse_output(&ok(
    ...
    #[tokio::test]
    async fn sets_hystersis_auth_expired_env_on_refresh() {
        let auth = run_external_refresh("echo $HYSTERSIS_AUTH_EXPIRED")
            .await
            .unwrap();
