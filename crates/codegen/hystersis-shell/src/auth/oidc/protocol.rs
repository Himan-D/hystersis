...
use super::super::config::{ForceLoginTeam, HystersisComConfig, OAuth2ProviderConfig, OidcAuthConfig};
use super::super::{AuthMode, HystersisAuth};
use base64::Engine;
...
pub(crate) fn is_configured(config: &HystersisComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &HystersisComConfig) -> Option<ForceLoginTeam> {
...
pub(super) fn build_hystersis_auth(
...
async fn refresh_tokens_once(
    ...
    if !resp.status().is_success() {
        ...
        tracing::warn!(
            ...
            rt_prefix = hystersis_auth::bearer_suffix(refresh_token),
            client_id = %client_id,
            principal_type = ?principal_type,
...
mod tests {
    ...
    fn authorize_url_includes_required_oidc_params() {
        let config = OidcAuthConfig {
            ...
            audience: Some("api://hystersis".into()),
        };
    ...
    fn authorize_url_includes_team_principal_params() {
        ...
        let discovery = Discovery {
            authorization_endpoint: "https://auth.hystersis.com/authorize".into(),
            token_endpoint: "https://auth.hystersis.com/token".into(),
            jwks_uri: None,
            id_token_signing_alg_values_supported: None,
        };
        let pkce = Pkce {
        ...
        assert!(url.contains("referrer=hystersis"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("hystersis-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.hystersis.com/authorize".into(),
            token_endpoint: "https://auth.hystersis.com/token".into(),
        ...
        assert!(url.contains("referrer=hystersis-desktop"));
        assert!(!url.contains("referrer=hystersis"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
