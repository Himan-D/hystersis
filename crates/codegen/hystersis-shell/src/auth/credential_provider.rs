...
use crate::util::hystersis_auth_credentials::HystersisAuthCredentials;
...
fn api_key_id_for(auth: Option<&crate::auth::HystersisAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| hystersis_telemetry::config::deployment_id_from_key(&a.key))
}
/// Sampler [`BearerResolver`](hystersis_sampler::BearerResolver) over a live
...
impl WireValidBearerResolver {
    ...
    pub(crate) fn shared(auth_manager: Arc<AuthManager>) -> hystersis_sampler::SharedBearerResolver {
...
impl hystersis_sampler::BearerResolver for WireValidBearerResolver {
...
pub(crate) struct ShellAuthCredentialProvider {
    ...
    static_credentials: HystersisAuthCredentials,
}
impl ShellAuthCredentialProvider {
    pub(crate) fn new(
        ...
        let mut static_credentials = HystersisAuthCredentials::new(None);
...
pub(crate) fn embedding_session_credentials(
...
) -> hystersis_memory::EndpointScopedCredentials {
    ...
    hystersis_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
        crate::util::is_hystersis_api_bearer_url,
        auth_credentials,
        api_key_provider,
    )
}
...
///
/// - `"hystersis-shell"`   — classic Hystersis CLI / TUI (xai-hystersis-shell)
/// - `"hystersis-pager"`   — new Hystersis Pager / TUI (xai-hystersis-pager)
/// - `"hystersis-desktop"` — Hystersis Desktop app
/// - `"hystersis-extension"` — VS Code / browser extension
///
...
pub fn build_storage_client_for_proxy(
...
) -> hystersis_file_utils::storage_client::StorageClient {
    ...
    if let Some(am) = auth_manager {
        ...
        let bridge: Arc<dyn hystersis_file_utils::storage_client::Auth401AttributionCallback> =
            Arc::new(StorageClientAttributionBridge::new(am, session_id));
        hystersis_file_utils::storage_client::StorageClient::with_provider(
            proxy_base_url,
            http_client,
            provider,
        )
        .with_client_identity(hystersis_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = HystersisAuthCredentials::new(user_token);
...
impl hystersis_file_utils::storage_client::Auth401AttributionCallback for StorageClientAttributionBridge {
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = HystersisAuthCredentials::new(None);
...
pub(crate) fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        hystersis_telemetry::external::set_identity(
            hystersis_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `xai-hystersis-pager` and `xai-hystersis-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> hystersis_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let hystersis_com_config = crate::auth::HystersisComConfig::default();
    let exporter = hystersis_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = hystersis_com_config.token_header.clone();
    let hystersis_home = crate::util::hystersis_home::hystersis_home();
    let bootstrap = Arc::new(AuthManager::new(&hystersis_home, hystersis_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    hystersis_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    use crate::auth::HystersisComConfig;
    use crate::auth::manager::AuthManager;
    use chrono::{Duration as ChronoDuration, Utc};
    use std::sync::Mutex;
    use hystersis_auth::AuthCredentialProvider;
    /// Serializes tests that pin `HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS`, since
    /// env vars are process-global and parallel tests would race.
    static EARLY_INVALIDATION_LOCK: Mutex<()> = Mutex::new(());
    /// RAII guard: pins `HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS` to the production
    ...
    impl EarlyInvalidationGuard {
        fn pin_to_default() -> Self {
            ...
            let previous = std::env::var("HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS").ok();
            unsafe { std::env::set_var("HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS", "300") };
            Self {
                _lock: lock,
                previous,
            }
        ...
        fn drop(&mut self) {
            unsafe {
                match self.previous.take() {
                    Some(prev) => std::env::set_var("HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS", prev),
                    None => std::env::remove_var("HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS"),
                }
            }
        }
    }
    fn make_auth(key: &str, expires_in: ChronoDuration) -> HystersisAuth {
        HystersisAuth {
            key: key.to_string(),
    ...
    fn make_manager(dir: &tempfile::TempDir, initial: Option<HystersisAuth>) -> Arc<AuthManager> {
    ...
    /// pinned in xai-hystersis-sampler's resolver tests): over a real
    ...
    fn wire_valid_resolver_tracks_manager_across_expiry_and_refresh() {
        use hystersis_sampler::BearerResolver;
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: hystersis_tools::types::SharedApiKeyProvider =
            Arc::new(crate::auth::manager::SharedAuthKeyProvider(mgr.clone()));
        for denied in ["https://byok.attacker.example/v1", "http://api.hystersis.com/v1"] {
            let resolved =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use hystersis_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
    ...
    fn otel_bootstrap_snapshot_picks_up_disk_writes() {
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
        let scope = crate::auth::HystersisComConfig::default().auth_scope();
    ...
    async fn otel_live_refresh_after_unauthorized_drives_recovery() {
        ...
        live_mgr.hot_swap(HystersisAuth {
            key: "stale".into(),
            auth_mode: crate::auth::AuthMode::Oidc,
            create_time: chrono::Utc::now() - ChronoDuration::hours(2),
            user_id: "u".into(),
    ...
    /// user token, matching the precedence in `HystersisAuthCredentials::apply`.
