...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    use hystersis_tools::types::ApiKeyProvider;
...
fn is_dark_wake_false_when_power_listener_not_started() {
    let _unset = hystersis_test_support::EnvGuard::unset("HYSTERSIS_AUTH_FORCE_DARK_WAKE");
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), HystersisComConfig::default());
    assert!(
        !mgr.is_dark_wake(),
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use hystersis_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use hystersis_telemetry::events::{AuthTokenKind, ManualAuthSurface};
