...
use crate::auth::model::{HystersisAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: HystersisAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.hystersis_com_config.token_header;
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut HystersisAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: HystersisAuth) {
    let started = std::time::Instant::now();
    ...
    if disk.key != auth.key || disk.refresh_token != auth.refresh_token {
        hystersis_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                ...
                "written_key_prefix": hystersis_auth::bearer_suffix(&auth.key),
                "disk_key_prefix": hystersis_auth::bearer_suffix(&disk.key),
            })),
        );
        return;
    }
...
pub(super) fn apply_user_info_enrichment(disk: &mut HystersisAuth, user_info: UserInfo) {
