...
use hystersis_auth::bearer_suffix;
...
use crate::auth::config::HystersisComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use hystersis_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, HystersisAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<HystersisAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<hystersis_system_power::SystemPowerListener>>,
...
impl AuthManager {
    pub fn new(hystersis_home: &Path, hystersis_com_config: HystersisComConfig) -> Self {
        let scope = hystersis_com_config.auth_scope();
        let proxy_base_url =
        ...
        hystersis_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "hystersis_home": hystersis_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "HYSTERSIS_HOME": std::env::var("HYSTERSIS_HOME").unwrap_or_else(|_| "(unset)".into()),
                "HYSTERSIS_AUTH_PATH": std::env::var("HYSTERSIS_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "HYSTERSIS_AUTH": std::env::var("HYSTERSIS_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // HYSTERSIS_AUTH_PATH: custom file path (overrides default $HYSTERSIS_HOME/auth.json).
        // Resolved before the HYSTERSIS_AUTH branch so inline-credential managers
        ...
        let path = std::env::var("HYSTERSIS_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| hystersis_home.join("auth.json"));

        // HYSTERSIS_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("HYSTERSIS_AUTH") {
            if let Ok(auth) = serde_json::from_str::<HystersisAuth>(&inline_json) {
            ...
            tracing::warn!("HYSTERSIS_AUTH set but failed to parse as JSON, falling back to file");
        }
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &HystersisAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.hystersis_com_config)?;
    ...
    fn vet_cached(&self, auth: HystersisAuth) -> Option<HystersisAuth> {
    ...
    pub(crate) fn current(&self) -> Option<HystersisAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<HystersisAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&HystersisAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<HystersisAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<HystersisAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    pub(crate) fn expired_auth(&self) -> Option<HystersisAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &HystersisAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &HystersisAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &HystersisAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.hystersis_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `HystersisAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(self: &Arc<Self>, auth: HystersisAuth) -> std::io::Result<HystersisAuth> {
        ...
        match &write_result {
            Ok(()) => hystersis_telemetry::unified_log::info(
            ...
            Err(e) => hystersis_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: HystersisAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut HystersisAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    /// Path to the `auth.json` this manager reads/writes (respects
    /// `HYSTERSIS_AUTH_PATH` / constructor home). Prefer this over
    /// `hystersis_home()/auth.json` so temp-home tests and custom stores stay isolated.
    pub(crate) fn auth_json_path(&self) -> &Path {
        &self.path
    }

    pub(crate) fn hystersis_com_config(&self) -> &HystersisComConfig {
        &self.hystersis_com_config
    }
    ...
    pub(crate) async fn run_external_refresh_command(&self, command: &str) -> Option<HystersisAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: HystersisAuth) {
    ...
    pub(crate) fn try_use_disk_token(
        &self,
        disk_auth: Option<&HystersisAuth>,
        reason: RefreshReason,
    ) -> Result<HystersisAuth, DiskTokenDecline> {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<HystersisAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> HystersisAuth {
        self.inner.read().clone().unwrap_or_else(|| HystersisAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: HystersisAuth) -> Option<HystersisAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<HystersisAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<HystersisAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<HystersisAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in HYSTERSIS_AUTH mode).
            DiskAuthState::Ok => {
                hystersis_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<HystersisAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<HystersisAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<HystersisAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.hystersis_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`hystersis login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.hystersis_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery(snapshot_key.as_deref()).await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(
        self: &Arc<Self>,
        unusable: Option<&str>,
    ) -> Result<HystersisAuth, AuthError> {
        if self.hystersis_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        hystersis_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    pub(crate) fn is_different_token(&self, candidate: &HystersisAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    fn is_external_provider_refresh_authority(&self) -> bool {
        self.hystersis_com_config.auth_provider_command.is_some()
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<hystersis_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[hystersis_com] auth_token_ttl` to drive a real schedule.
...
impl hystersis_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.hystersis_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.hystersis_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.hystersis_com_config.preferred_method,
    ...
    non_empty_key(crate::agent::auth_method::read_hystersis_api_key_env().ok())
...
) -> hystersis_tools::types::SharedApiKeyProvider {
