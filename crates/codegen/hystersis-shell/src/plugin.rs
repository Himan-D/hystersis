...
use hystersis_agent::plugins::git_install::{self, UpdateStatus};
use hystersis_agent::plugins::install_registry::{
    InstallError, InstallKind, InstallRegistry, InstalledRepo, MarketplaceProvenance,
};
use hystersis_plugin_marketplace::git::{self, SourceCacheLease};
use hystersis_plugin_marketplace::{
...
fn marketplace_root_for_provenance(
    provenance: &hystersis_agent::plugins::install_registry::MarketplaceProvenance,
...
pub(crate) fn marketplace_require_sha() -> bool {
    hystersis_config::ConfigLayers::load()
        .map(|layers| {
            hystersis_plugin_marketplace::load_require_sha(
                &layers.effective_config_base_without_overlay(),
            )
        })
        .unwrap_or_else(|_| hystersis_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &hystersis_workspace::permission::resolution::MarketplaceAllowlist,
...
mod tests {
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> hystersis_workspace::permission::resolution::MarketplaceAllowlist {
        hystersis_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = hystersis_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
