...
use hystersis_sampler::RetryPolicy;
use hystersis_sampling_types::doom_loop::{DoomLoopSignalKind, SAMPLE_CHECK_EVENT_DATA_CUMULATIVE};
...
use hystersis_test_support::{MockInferenceServer, MockModelEntry, ScriptedResponse};
...
async fn headless_config_enables_doom_loop_check_header() {
    ...
    let workdir = hystersis_test_support::git_workdir();
    let sandbox = hystersis_test_support::TestSandbox::builder()
        .mock_url(server.url())
        .build();
    ...
    let mut cmd = tokio::process::Command::new(hystersis_test_support::hystersis_binary());
    ...
    let result = hystersis_test_support::run_headless_in_sandbox(cmd, sandbox).await;
    hystersis_test_support::assert_headless_success(&result, "doom-loop header e2e", Some(&server));
