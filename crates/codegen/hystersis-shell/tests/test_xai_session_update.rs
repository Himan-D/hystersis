//! Integration test for `_hystersis.com/session/update` notifications.
//!
...
use hystersis_shell::session::info::Info as SessionInfo;
use hystersis_shell::session::persistence::default_model_id;
use hystersis_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that xAI session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_hystersis_session_notification_storage_roundtrip() {
    ...
    let hystersis_notification = SessionNotification {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Xai(Box::new(hystersis_notification.clone())),
        )
        .await
        .unwrap();
...
/// must survive `updates.jsonl` and reload as a replayable `_hystersis.com/session/update`.
#[tokio::test]
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Xai(Box::new(hystersis_notification)))
        .await
        .unwrap();
...
#[test]
fn test_hystersis_session_notification_serialization() {
