...
//! [`synth`](hystersis_shell::session::testkit::synth) generator (redundant
...
use hystersis_shell::session::info::Info;
use hystersis_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use hystersis_shell::session::testkit::e2e::load_session_via_agent;
use hystersis_shell::session::testkit::synth::{self, SessionSpec};
...
async fn phase_breakdown_real_functions() {
    ...
    use hystersis_workspace::session::file_state::FileStateTracker;
    let t = Instant::now();
...
async fn full_session_load_e2e() {
    ...
    let _ = tracing_subscriber::registry()
        .with(hystersis_shell::instrumentation::layer::<Registry>())
        .try_init();
    ...
    local
        .run_until(async move {
            ...
            let _ = hystersis_shell::instrumentation::finalize();
