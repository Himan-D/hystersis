...
use hystersis_shell::auth::{AuthManager, HystersisComConfig};
use hystersis_shell::session::info::Info;
use hystersis_shell::session::persistence::delete_session_history;
...
fn start_index() -> SearchIndexManager {
    ...
    match start_if_enabled(&hystersis_shell::agent::config::Config::default()) {
...
async fn deleting_a_session_clears_only_its_own_search_row() {
    ...
    let session_dir =
        hystersis_shell::util::hystersis_home::sessions_cwd_dir_in(root, "/ws-a").join("orphan");
...
fn loading_config_applies_requirement_pins() {
    ...
    let loaded = hystersis_shell::config::load_agent_config_disk_only();
    std::fs::remove_file(&pin).unwrap();
    let config = loaded.expect("config loads");

    let resolved = config.feature(hystersis_shell::agent::config::Feature::SessionSearch);
    assert!(
        !resolved.value,
    ...
    assert_eq!(
        resolved.source,
        hystersis_shell::agent::config::ConfigSource::Requirement
    );
}
