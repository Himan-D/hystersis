...
use hystersis_shell::session::storage::{JsonlStorageAdapter, StorageAdapter, prepare_replay_lines};
use hystersis_shell::session::testkit::synth::{self, SessionSpec};
use hystersis_test_support::env::env_parse;
...
mod rss {
    ...
    use hystersis_test_support::resources::{RssOutcome, RssSampler};
    ...
    async fn count_replayed_notifications(session_id: acp::SessionId, cwd: PathBuf) -> u64 {
        ...
        local
            .run_until(async move {
                ...
                let loaded = hystersis_shell::session::testkit::e2e::load_session_via_agent(
