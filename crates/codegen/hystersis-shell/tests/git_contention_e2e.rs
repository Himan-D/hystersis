...
//!
//! With the fs-watch machinery on (`hystersis.com/hunkTracker` + `hystersis.com/gitHeadChanged`
...
//!
//! Knobs: HYSTERSIS_PERF_GIT_FILES (default 300), HYSTERSIS_PERF_GIT_PICKS (default 6).
...
use hystersis_test_support::{MockInferenceServer, ScriptedResponse, SseEvent};

const DUPLEX_BUFFER_BYTES: usize = 8 * 1024 * 1024;

use hystersis_test_utils::env::env_usize;

/// Run a git command with deterministic author/committer; assert success.
fn git(dir: &Path, args: &[&str]) -> String {
    hystersis_test_utils::git::run_git(dir, args)
}

// ── scan counting ─────────────────────────────────────────────────────────

use hystersis_hunk_tracker::{REFRESH_SCAN_LOG_PREFIX, REFRESH_SKIP_LOG_PREFIX};
use hystersis_test_utils::tracing_capture::MessagePrefixCounter;
...
fn install_global_scan_counter() -> ScanCounter {
    // HYSTERSIS_E2E_LOG=<filter> tees shell logs to stderr for local debugging.
    let filter = std::env::var("HYSTERSIS_E2E_LOG").ok();
    ScanCounter(
        hystersis_test_utils::tracing_capture::install_prefix_counter_global(
...
fn build_repo(files: usize, picks: usize) -> (TempDir, String) {
    ...
    hystersis_test_utils::git::write_fanout_tree(wd, files, 100);
    git(wd, &["add", "."]);
    git(wd, &["commit", "-m", "populate tree"]);

    let base = hystersis_test_utils::git::make_feature_branch(wd, picks);
    (dir, base)
}
...
async fn run_storm(
    ...
    let files = env_usize("HYSTERSIS_PERF_GIT_FILES", 300);
    let picks = env_usize("HYSTERSIS_PERF_GIT_PICKS", 6);
...
fn git_rebase_refresh_storm_e2e() {
    ...
    let on = agent_rt.block_on(run_storm(
        &server,
        Some(json!({
            "hystersis.com/hunkTracker": { "mode": "agent_only" },
