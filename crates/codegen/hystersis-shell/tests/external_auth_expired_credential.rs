...
//!
//! One `#[test]`: the phases share one process-global `HYSTERSIS_HOME` and env, so
...
use hystersis_shell::agent::mvp_agent::MvpAgent;
use hystersis_test_support::{MockInferenceServer, MockModelEntry};
...
/// Records `hystersis.com/session/update` payloads so a phase can read the terminal
/// `retryState`.
...
fn seed_credential(hystersis_home: &Path, expires_at: chrono::DateTime<chrono::Utc>) {
    let auth = json!({
        "https://accounts.hystersis.com/sign-in": {
            "key": STALE_TOKEN,
            "auth_mode": "external",
    ...
    std::fs::create_dir_all(hystersis_home).expect("create hystersis home");
    std::fs::write(
...
fn write_interactive_only_provider(hystersis_home: &Path) -> String {
    use std::os::unix::fs::PermissionsExt;

    let script = hystersis_home.join("acme-auth.sh");
    std::fs::write(
        &script,
        "#!/bin/sh\n\
...
fn provider_runs(hystersis_home: &Path) -> usize {
    std::fs::read_to_string(hystersis_home.join("provider-runs"))
        .map(|s| s.lines().count())
        .unwrap_or(0)
}
...
fn expired_external_credential_routes_to_the_provider_login_flow() {
    ...
    let hystersis_home = TempDir::new().expect("hystersis home");
    let workdir = TempDir::new().expect("workdir");
    seed_credential(
        hystersis_home.path(),
        chrono::Utc::now() - chrono::Duration::hours(1),
    );
    let provider = write_interactive_only_provider(hystersis_home.path());
    ...
    unsafe {
        ...
        std::env::set_var("HYSTERSIS_MODELS_BASE_URL", server.url());
        std::env::set_var("HYSTERSIS_AUTH_PROVIDER_COMMAND", &provider);
        std::env::set_var("HYSTERSIS_AUTH_PROVIDER_LABEL", PROVIDER_LABEL);
    ...
    agent_rt.block_on(local.run_until(async move {
        ...
        assert_eq!(
            provider_runs(hystersis_home.path()),
            1,
        ...
        std::fs::remove_file(hystersis_home.path().join("auth.json")).expect("remove auth.json");
