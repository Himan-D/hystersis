...
use hystersis_shell::auth::{AuthManager, AuthMode, HystersisAuth, HystersisComConfig};
use hystersis_shell::heap_profile::{
...
use hystersis_test_support::{EnvGuard, MockInferenceServer};
use hystersis_version::TEST_VERSION_ENV;
...
fn assert_storage_auth(uploads: &[hystersis_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
