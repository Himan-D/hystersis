...
fn set_test_env(hystersis_home: &std::path::Path, server_url: &str) {
    ...
    unsafe {
        ...
        std::env::set_var("HYSTERSIS_CLI_CHAT_PROXY_BASE_URL", server_url);
        std::env::set_var("HYSTERSIS_XAI_API_BASE_URL", server_url);
        ...
        std::env::set_var("HYSTERSIS_TURN_SUMMARY", "false");
    }
}

/// Runs `body` against a mock inference server with `HYSTERSIS_HOME` isolated to a
...
where
    F: FnOnce(std::path::PathBuf, std::rc::Rc<hystersis_test_support::MockInferenceServer>) -> Fut,
    Fut: std::future::Future<Output = ()>,
{
    ...
    let server = std::rc::Rc::new(
        mock_rt
            .block_on(hystersis_test_support::MockInferenceServer::start())
            .expect("mock server"),
    );
    let hystersis_home = tempfile::TempDir::new().expect("hystersis home");
    let workdir = tempfile::TempDir::new().expect("workdir");
    set_test_env(hystersis_home.path(), &server.url());
