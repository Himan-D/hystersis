...
use std::path::Path;
use hystersis_agent::config::{AgentDefinition, IsolationMode};
use hystersis_agent::plugins::PluginRegistry;
use hystersis_agent::prompt::context::{PromptAudience, PromptContext};
...
use hystersis_tool_types::{SubagentCapabilityMode, SubagentIsolationMode};
...
pub fn discover_agent_definition(
    ...
    hystersis_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        context.cwd,
        context.plugins,
    )
...
pub fn available_agent_names(context: &DefinitionResolutionContext<'_>) -> Vec<String> {
    let mut available: Vec<String> = hystersis_agent::discovery::all_subagents_with_plugins(
        context.cwd,
        context.toggles,
        context.plugins,
    )
...
pub async fn render_subagent_initial_user_message(
    ...
    let agents_md_files = hystersis_agent::prompt::agents_md::read_agents_config_with_paths(
...
mod tests {
    ...
    fn general_purpose_definition_omits_workflow() {
        ...
        assert!(
            ...
            "general-purpose must keep the rest of the hystersis child tools"
        );
    }
    #[test]
    fn child_tool_policy_strips_workflow_and_keeps_other_tools() {
        ...
        definition
            .tool_config
            .tools
            .push((&hystersis_tools::implementations::hystersis_build::WorkflowTool).into());
    ...
    fn kindless_workflow_id_is_stripped_by_child_policy() {
        ...
        definition
            .tool_config
            .tools
            .push(ToolConfig::from_id("HystersisBuild:workflow"));
        apply_child_tool_policy(&mut definition, None, true);
        assert!(definition.tool_config.tools.iter().all(|tool| {
            tool.kind != Some(ToolKind::Workflow)
                && tool.id.rsplit(':').next()
                    != Some(hystersis_tools::implementations::hystersis_build::WORKFLOW_TOOL_NAME)
        }));
    }
    #[test]
