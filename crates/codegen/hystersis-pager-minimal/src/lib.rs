//! Minimal (scrollback-native) render mode — `hystersis --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the
//! terminal's *native* scrollback (via `hystersis_ratatui_inline::Terminal::insert_before`,
...
//! # Wiring
//!
//! `xai-hystersis-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`hystersis_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`xai-hystersis-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `xai-hystersis-pager`.
///
...
/// subsequent calls are ignored (see [`hystersis_pager::minimal_hook`]).
pub fn install() {
    hystersis_pager::minimal_hook::install(hystersis_pager::minimal_hook::MinimalHooks { draw });
}
