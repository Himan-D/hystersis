...
//! Backend environment presets for the Hystersis CLI crate family: endpoint URL
//! defaults, environment selection, and env-var test support.
//!
//! Public builds expose production endpoints. Values resolve as a `HYSTERSIS_*`
//! env-var override when set, else the compiled production default.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct HystersisBuildEndpoints {
...
const PRODUCTION_ENDPOINTS: HystersisBuildEndpoints = HystersisBuildEndpoints {
    cli_chat_proxy_base_url: "https://cli-chat-proxy.hystersis.com/v1",
    asset_server_url: "https://assets.hystersis.com",
    relay_ws_url: "wss://code.hystersis.com/ws/code-agent",
    gateway_ws_url: "wss://hystersis.com/ws/gw/",
    ws_origin: "https://hystersis.com",
};
...
pub enum HystersisBuildEnvironment {
    #[default]
    Production,
}
impl HystersisBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        HystersisBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            HystersisBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, HystersisBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            HystersisBuildEnvironment::Production => "HYSTERSIS_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (production by default).
    pub fn endpoints(&self) -> HystersisBuildEndpoints {
        match self {
            HystersisBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    ...
    /// The relay WebSocket URL (Web Frontend at `hystersis.com/code` driving a
    ...
    /// `HYSTERSIS_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for HystersisBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            HystersisBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            HystersisBuildEnvironment::Production.env_prefix(),
            "HYSTERSIS_PRODUCTION"
        );
    }
    #[test]
    fn env_var_guard_set_value_updates_then_restores_on_drop() {
        const KEY: &str = "XAI_HYSTERSIS_ENV_VAR_GUARD_SET_VALUE_PROBE";
    ...
    fn env_var_guard_chains_keys_under_one_lock_and_restores_all() {
        const A: &str = "XAI_HYSTERSIS_ENV_VAR_GUARD_CHAIN_A_PROBE";
        const B: &str = "XAI_HYSTERSIS_ENV_VAR_GUARD_CHAIN_B_PROBE";
        {
    ...
    fn env_var_guard_rejects_a_second_guard_on_the_same_thread() {
        const KEY: &str = "XAI_HYSTERSIS_ENV_VAR_GUARD_REENTRANCY_PROBE";
    ...
    /// loop mistakenly connecting to `wss://hystersis.com/ws/gw/`).
    #[test]
    fn relay_and_gateway_urls_are_distinct() {
        assert_ne!(
            HystersisBuildEnvironment::Production.relay_ws_url(),
            HystersisBuildEnvironment::Production.gateway_ws_url(),
        );
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            HystersisBuildEnvironment::from_flags(false, false),
            HystersisBuildEnvironment::Production
        );
    }
}
