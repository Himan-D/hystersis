//! Headless markdown analysis sharing Hystersis's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `xai-hystersis-markdown` so analysis matches what Hystersis
//! Build actually renders 1:1.
//!
//! After parsing, Hystersis applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Hystersis uses to render markdown.
///
...
/// Offset event stream from Hystersis's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Hystersis's options; count elements and flag structural issues.
