//! Local, ACP, and PTY terminal runners for the hystersis shell.
//!
//! `xai-hystersis-shell` re-exports this crate as `xai_hystersis_shell::terminal`.
...
/// Resolved absolute path to bash. On Unix uses the `xai_hystersis_config` shell
/// resolution cascade (`$HYSTERSIS_SHELL` > `$SHELL` > `which` > common dirs >
...
pub(crate) fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        xai_hystersis_config::shell::unix_shell_path(xai_hystersis_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `xai-hystersis-tools`.
pub use xai_hystersis_tools::util::pager_env;
...
/// tool output. Used when the client sets `hystersis.com/bashOutputNoColor: true` (e.g.
/// xai-hystersis-pager which renders its own UI and doesn't need raw ANSI codes).
///
