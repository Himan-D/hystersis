...
//!
//! ```rust,no_run
//! use hystersis_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
impl SandboxManager {
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        if requires_hook_write_deny(&self.profile, workspace) {
            hystersis_config::ensure_hystersis_hook_slots(paths::hystersis_home().as_path())
...
mod tests {
    ...
    fn bwrap_hook_plan_binds_ancestors_then_leaves() {
        ...
        let sources = [hystersis_config::GlobalHookSource {
            path: leaf.clone(),
            kind: hystersis_config::GlobalHookSourceKind::ConfiguredSource,
        }];
