...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = hystersis_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> hystersis_sandbox::ProfileName {
    hystersis_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_profile_and_bwrap_reexec(profile: &hystersis_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !hystersis_sandbox::is_inside_bwrap() {
            ...
            match hystersis_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = hystersis_sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_hystersis_home: &Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        match hystersis_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    ...
    let profile = hystersis_sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
