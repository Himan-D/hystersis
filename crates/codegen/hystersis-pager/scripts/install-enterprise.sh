#!/bin/bash
#
# Hystersis CLI installer (enterprise channel) — https://hystersis.com/cli/enterprise-install.sh
#
...
# Env: HYSTERSIS_BIN_DIR, HYSTERSIS_PROXY_URL
#
# Usage:
#   curl -fsSL https://hystersis.com/cli/enterprise-install.sh | bash            # latest enterprise
#   curl -fsSL https://hystersis.com/cli/enterprise-install.sh | bash -s 0.1.42  # specific version
#   HYSTERSIS_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://hystersis.com/cli/enterprise-install.sh)
#
...
if [ "$os" = "windows" ]; then
    ...
    chmod +x "$binary_path"
    ln -sf "$binary_path" "$BIN_DIR/hystersis"
