...
//! `hystersis.com/log` ACP notifications. Call [`init`] once at startup with
...
use hystersis_telemetry::unified_log::{
...
fn make_entry(
    ...
    ClientLogEntry {
        ...
        ver: Some(hystersis_version::VERSION.to_owned()),
        lvl,
...
fn send_entries(entries: Vec<ClientLogEntry>) {
    ...
    handle.spawn(async move {
        let _ = hystersis_acp_lib::acp_send(notification, &tx).await;
    });
}
