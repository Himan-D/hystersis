...
pub fn make_worktree_record(
    ...
    use hystersis_fast_worktree::{WorktreeKind, WorktreeRecord, WorktreeStatus};
    WorktreeRecord {
        id: id.to_owned(),
...
/// Shared HYSTERSIS_HOME boundary fixture for the resume-by-title startup and
/// pre-sandbox tests.
///
/// `hystersis_home()` is OnceLock-cached process-wide, so summaries land under the
/// *resolved* home (possibly the real `~/.hystersis` when another test pinned the
...
pub struct HystersisHomeFixture {
...
impl Default for HystersisHomeFixture {
    fn default() -> Self {
        Self::new()
    }
}
...
impl TempGitRepo {
    ...
    /// CoW-style standalone clone: `.git` is a directory plus `hystersis-worktree-source`.
    pub fn standalone_clone(&self, branch: &str) -> Self {
        ...
        std::fs::write(
            path.join(".git").join("hystersis-worktree-source"),
