...
use hystersis_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
pub fn build_voice_auth(auth_manager: Arc<hystersis_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        hystersis_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
