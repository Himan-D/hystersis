//! `hystersis du`: what the user hystersis home uses on disk. It creates no hystersis home,
...
use hystersis_fast_worktree::{
    ...
    managed_worktree_roots, path_under_worktree_roots, resolve_hystersis_home,
};
...
#[command(
    after_help = "Lists every top-level directory in the hystersis home, largest first, then every \
worktree under `worktrees/` and `worktree_pool/` with its size, age, and label. To reclaim space, preview a sweep with \
`hystersis worktree gc --max-age 7d --dry-run`: without `--max-age`, gc expires nothing, it \
...
pub fn run(args: DiskUsageArgs) -> Result<()> {
    // The registry's own resolution, unlike hystersis_config::hystersis_home().
    let hystersis_home = resolve_hystersis_home()?;
    let mut out = std::io::stdout().lock();
    let present = hystersis_home
        .try_exists()
        .with_context(|| format!("cannot stat {}", hystersis_home.display()))?;
    if !present {
        if args.json {
            return write_report(&empty_report(&hystersis_home), args.json, &mut out);
        }
        let written = display::print_missing_home(&hystersis_home.to_string_lossy(), &mut out);
        return Ok(crate::util::ignore_broken_pipe(written)?);
    }
    // Rows store canonical paths, so the home must match to strip-prefix.
    let hystersis_home = dunce::canonicalize(&hystersis_home).unwrap_or(hystersis_home);
    write_report(&collect_report(&hystersis_home)?, args.json, &mut out)
}
...
fn empty_report(hystersis_home: &Path) -> DiskUsageReport {
    DiskUsageReport {
        hystersis_home: hystersis_home.to_string_lossy().into_owned(),
...
fn collect_report(hystersis_home: &Path) -> Result<DiskUsageReport> {
    let mut top_level_dirs = Vec::new();
    let mut root_files_bytes = 0u64;
    let mut issues = WalkIssues::default();
    let mut unfollowed_dir_symlinks = 0u64;
    let volume = Volume::of(hystersis_home);
    let mut worktree_sizes: HashMap<PathBuf, Measure> = HashMap::new();
    let children = std::fs::read_dir(hystersis_home)
        .with_context(|| format!("cannot read {}", hystersis_home.display()))?;
    for child in children {
        let child = match child {
            Ok(child) => child,
            Err(e) => {
                tracing::debug!(path = %hystersis_home.display(), error = %e, "du: home entry could not be read");
    ...
    let (registry, records, registry_path) = load_registry(hystersis_home);
    let rows = collect_worktrees(hystersis_home, records, &worktree_sizes, volume);
    issues.merge(rows.issues);
    let (volume_capacity_bytes, volume_available_bytes) = volume_bytes(hystersis_home).unzip();
    Ok(DiskUsageReport {
        schema_version: SCHEMA_VERSION,
        hystersis_home: hystersis_home.to_string_lossy().into_owned(),
        total_bytes,
...
fn load_registry(hystersis_home: &Path) -> (RegistryState, Vec<WorktreeRecord>, PathBuf) {
    classify(WorktreeDb::open_read_only(hystersis_home))
}
...
fn collect_worktrees(
    ...
    for found in discover_worktrees(hystersis_home).found {
