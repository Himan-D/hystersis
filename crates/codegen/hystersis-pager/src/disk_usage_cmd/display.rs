//! Pure renderer over [`DiskUsageReport`]. `hystersis_config::hystersis_home()`,
...
use hystersis_fast_worktree::WorktreeStatus;
...
pub fn print_report(
    ...
    let home_label = home_prefix_label(&report.hystersis_home);
    ...
    match report.registry {
        ...
        RegistryState::Unopenable => {
            writeln!(
                ...
                abbreviate(&report.registry_path, &report.hystersis_home, &home_label)
            )?;
        }
        RegistryState::Corrupt => {
            writeln!(
                out,
                "  Worktree registry is damaged; rows show as untracked. Remove {} and run `hystersis worktree db rebuild` to recreate it.",
                abbreviate(&report.registry_path, &report.hystersis_home, &home_label)
            )?;
        }
    }
    if report.worktrees.is_empty() {
        ...
        for (wt, kind) in report.worktrees.iter().zip(&kind_cells) {
            ...
            writeln!(
                ...
                abbreviate(&wt.path, &report.hystersis_home, &home_label),
            )?;
        }
    }
...
pub fn print_missing_home(hystersis_home: &str, out: &mut impl Write) -> std::io::Result<()> {
    writeln!(
        out,
        "Nothing on disk yet at {}.",
        home_prefix_label(hystersis_home)
    )
}
...
fn home_prefix_label(hystersis_home: &str) -> String {
    crate::util::display_hystersis_home_prefix_for(Path::new(hystersis_home))
}
