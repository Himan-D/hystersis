...
pub enum Command {
    /// Run Hystersis without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Hystersis discovers for this directory
    Inspect {
        /// Emit machine-readable JSON output.
        #[arg(long)]
        json: bool,
    },
    /// Check terminal, clipboard, color, and input support without starting Hystersis
    Doctor(crate::doctor_cmd::DoctorArgs),
    /// Interactively configure AI providers, API keys, and models
    #[command(alias = "init")]
    Configure,
    /// Manage running leader processes
    Leader(LeaderMgmtArgs),
    /// Show system information and Hystersis configuration (Neofetch style)
    Info,
    /// Sign out and clear cached credentials
    Logout,
    /// Sign in to Hystersis
    Login {
        /// Ignored (kept for backwards compatibility). OAuth2 is now the only auth method.
        #[arg(long, hide = true)]
        legacy: bool,
        /// Use Hystersis OAuth via auth.hystersis.com.
...
Examples:
  hystersis wrap docker exec -it my-container bash
  hystersis wrap kubectl exec -it my-pod -- bash

See ~/.hystersis/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    Update {
        ...
        enterprise: bool,
        /// Internal: what spawned this `hystersis update` (`user_command`,
    ...
    /// Show what the hystersis home (~/.hystersis) uses on disk
    ...
    /// `HYSTERSIS_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// `~/.hystersis/config.toml` or when the `HYSTERSIS_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `hystersis leader` / `hystersis workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `hystersis leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
...
pub struct AgentArgs {
    ...
    pub hystersis_api_base_url: Option<String>,
    /// Agent runtime mode
    #[command(subcommand)]
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "hystersis: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("hystersis: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the Hystersis WebSocket relay
...
pub struct HeadlessArgs {
    #[arg(long = "hystersis-ws-origin")]
    pub hystersis_ws_origin: Option<String>,
    #[arg(long = "hystersis-ws-url")]
    pub hystersis_ws_url: Option<String>,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "HYSTERSIS_AGENT_SECRET")]
...
pub struct LeaderArgs {
    ...
    /// Defer the hystersis.com relay WebSocket until the first headless IPC client
...
#[command(
    name = "hystersis",
    version = hystersis_version::full_version(),
    about = "Hystersis TUI",
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.hystersis/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// (default) persists per-segment markdown to grep. Sets `HYSTERSIS_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            hystersis_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `hystersis -p` runs is to wait (up to `--background-wait-timeout`)
    ...
    #[arg(long, env = "HYSTERSIS_SANDBOX", value_name = "PROFILE")]
    ...
    /// Session-scoped only, does not write config. To default plain `hystersis` to
    /// minimal, set `[ui] screen_mode = "minimal"` in ~/.hystersis/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.hystersis/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "HYSTERSIS_LOG_SAMPLING", hide = true)]
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `hystersis "fix the bug"` or `hystersis --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    pub(crate) fn local_resume_selection(
        &self,
    ) -> hystersis_shell::session::persistence::RecentSessionSelection {
        use hystersis_shell::session::unified_list::HeadlessPolicy;
        let policy =
        ...
        hystersis_shell::session::persistence::RecentSessionSelection::from_headless_policy(policy)
    }
    ...
    /// explicit `--sandbox`/`HYSTERSIS_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                hystersis_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                hystersis_shell::session::persistence::resolve_recent_session_sandbox_profile(
                    cwd,
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<hystersis_sandbox::ProfileName>().ok()
                    != s.parse::<hystersis_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `hystersis "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_flags_parse_as_early_intent_without_exiting() {
        for flag in ["--version", "-v", "-V"] {
            let args = PagerArgs::try_parse_from(["hystersis", flag]).expect("version flag parses");
    ...
    fn ordinary_and_doctor_parsing_do_not_set_version_intent() {
        assert!(!PagerArgs::try_parse_from(["hystersis"]).unwrap().version);
        assert!(
            !PagerArgs::try_parse_from(["hystersis", "doctor"])
                .unwrap()
                .version
        );
        assert!(matches!(
            PagerArgs::try_parse_from(["hystersis", "version"])
                .unwrap()
                .command,
    ...
    fn doctor_accepts_report_and_explicit_fix_forms() {
        let bare = PagerArgs::try_parse_from(["hystersis", "doctor"]).expect("bare doctor parses");
        assert!(matches!(
            bare.command,
        ...
        let json =
            PagerArgs::try_parse_from(["hystersis", "doctor", "--json"]).expect("doctor --json parses");
        assert!(matches!(
            json.command,
            Some(Command::Doctor(crate::doctor_cmd::DoctorArgs {
                json: true,
        ...
        for id in [
            ...
            let fix = PagerArgs::try_parse_from(["hystersis", "doctor", "fix", id, "--yes"])
        ...
        let list = PagerArgs::try_parse_from(["hystersis", "doctor", "fix"])
        ...
        for unsupported in [
            vec!["hystersis", "doctor", "all"],
            vec!["hystersis", "doctor", "fix", "ssh-wrap", "extra"],
            vec!["hystersis", "doctor", "fix", "--yes"],
            vec!["hystersis", "doctor", "--json", "fix", "terminal.ssh-wrap"],
        ] {
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["hystersis", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["hystersis", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["hystersis", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
        assert_eq!(
            PagerArgs::try_parse_from(["hystersis"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["hystersis", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["hystersis"]).expect("bare hystersis parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["hystersis", "leader", "list", "--json"])
            .expect("hystersis leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["hystersis", "leader", "info", "--pid", "42"])
            .expect("hystersis leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill = PagerArgs::try_parse_from(["hystersis", "leader", "kill"]).expect("hystersis leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["hystersis", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["hystersis", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["hystersis", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["hystersis", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["hystersis"]).expect("bare hystersis parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["hystersis", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias =
            PagerArgs::try_parse_from(["hystersis", "--trust-folder"]).expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["hystersis", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias =
            PagerArgs::try_parse_from(["hystersis", "--effort", "high"]).expect("--effort alias parses");
        assert_eq!(alias.reasoning_effort.as_deref(), Some("high"));
    }
    #[test]
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["hystersis", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep =
            PagerArgs::try_parse_from(["hystersis", "--reasoning-effort", "deep"]).expect("deep parses");
        assert_eq!(deep.reasoning_effort.as_deref(), Some("deep"));
    }
    #[test]
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["hystersis", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["hystersis", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["hystersis", "agent", "--effort", "max", "stdio"])
