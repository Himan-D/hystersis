...
pub enum DeferredSessionStartup {
    ...
    ForeignResume {
        tool: hystersis_foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = hystersis_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = hystersis_shell::util::hystersis_home::hystersis_home().join("sessions");
    let encoded = hystersis_shell::util::hystersis_home::encode_cwd_dirname(&cwd_str);
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    hystersis_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub use hystersis_shell::session::persistence::RecentSessionSelection;
/// Resolve most-recent session id for cwd, or error.
async fn most_recent_session_id(
    cwd: &str,
    selection: RecentSessionSelection,
) -> anyhow::Result<(String, Option<String>)> {
    let summaries = hystersis_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries
        .iter()
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &hystersis_shell::agent::config::Config,
) -> std::sync::Arc<hystersis_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(hystersis_shell::auth::AuthManager::new(
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if hystersis_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = hystersis_shell::session::resolve_local_session(session_id, cwd) {
    ...
    if let Some(original_cwd) = hystersis_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
...
async fn restore_session_from_remote(
    ...
    if let Some((false, source)) =
        hystersis_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    let agent_config = hystersis_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use hystersis_shell::agent::session_registry_client::SessionRegistryClient;
    use hystersis_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    use hystersis_shell::session::restore::{RestoreSessionOpts, restore_session_with_storage};
    use hystersis_shell::util::hystersis_home::hystersis_home;
    ...
    let storage_client = hystersis_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: hystersis_shell::session::restore::ProgressCallback = Box::new(move |event| {
    ...
    let recovered_local_id = hystersis_shell::session::find_local_child_for_remote(session_id, cwd);
...
mod tests {
    ...
    fn deferred_startup_owner_take_is_atomic() {
        let mut actions = DeferredStartupActions {
            session: Some(DeferredSessionStartup::ForeignResume {
                tool: hystersis_foreign_sessions::ForeignSessionTool::Cursor,
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        let encoded = hystersis_shell::util::hystersis_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = hystersis_shell::util::hystersis_home::hystersis_home()
            .join("sessions")
            .join(&encoded);
    ...
    #[cfg(feature = "local-workspace")]
    fn advertised_tools_env() -> hystersis_test_support::EnvGuard {
        hystersis_test_support::EnvGuard::set(
    ...
    fn resolve_local_workspace_empty_cli_attach_falls_back_to_env() {
        let _env = advertised_tools_env();
        let _sid = hystersis_test_support::EnvGuard::set(
    ...
    fn resolve_local_workspace_cwd_only_is_not_a_request() {
        let tmp = tempfile::tempdir().unwrap();
        let _cwd = hystersis_test_support::EnvGuard::set(
            HYSTERSIS_CHAT_LOCAL_WORKSPACE_CWD_ENV,
            tmp.path().to_str().unwrap(),
        );
        let _enable = hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_LOCAL_WORKSPACE_ENV);
        let _mode = hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_own_env_defaults() {
        let _env = advertised_tools_env();
        let _enable = hystersis_test_support::EnvGuard::set(HYSTERSIS_CHAT_LOCAL_WORKSPACE_ENV, "1");
        let _mode = hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_LOCAL_WORKSPACE_MODE_ENV);
        let _sid = hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_LOCAL_WORKSPACE_SERVER_ID_ENV);
    ...
    fn resolve_local_workspace_defaults_cwd_and_denies_home() {
        let _tools = hystersis_test_support::EnvGuard::set(
            HYSTERSIS_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV,
            "workspace.fs_list",
        );
        let _allow =
            hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_LOCAL_WORKSPACE_ALLOW_HOME_ENV);
        let home = tempfile::tempdir().unwrap();
        let home_str = home.path().to_str().unwrap();
        let _home = hystersis_test_support::EnvGuard::set("HOME", home_str);
        let _userprofile = hystersis_test_support::EnvGuard::set("USERPROFILE", home_str);
        let err =
    ...
    fn resolve_local_workspace_refuses_uncheckable_toolset() {
        let _tools =
            hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_LOCAL_WORKSPACE_ADVERTISED_TOOLS_ENV);
        let tmp = tempfile::tempdir().unwrap();
        let err =
    ...
    fn local_workspace_non_tty_requires_ack() {
        let _ack = hystersis_test_support::EnvGuard::unset(HYSTERSIS_CHAT_LOCAL_WORKSPACE_ACK_ENV);
        let home = tempfile::tempdir().unwrap();
        let _home =
            hystersis_test_support::EnvGuard::set("HYSTERSIS_HOME", home.path().to_str().unwrap());
