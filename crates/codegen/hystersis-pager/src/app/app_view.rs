...
use hystersis_acp_lib::AcpAgentTx;
...
/// [`hystersis_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<hystersis_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: hystersis_shell::util::config::WorktreeHintMode) -> Self {
        use hystersis_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        hystersis_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            hystersis_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`hystersis_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `HYSTERSIS_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `HYSTERSIS_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("HYSTERSIS_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`hystersis_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => hystersis_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    pub pending_startup: Option<hystersis_telemetry::startup::PendingStartup>,
    ...
    /// stays sans-IO.
    pub current_ui: hystersis_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    ...
    /// Current appearance config (hot-reloadable from ~/.hystersis/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`HYSTERSIS_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `HYSTERSIS_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<hystersis_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<hystersis_announcements::RemoteAnnouncement>,
    ...
    /// Whether the plugin marketplace CTA is enabled. Env `HYSTERSIS_PLUGIN_CTA`
    ...
    /// No hystersis.com billing session exists; `/usage` and credit UI stay off.
    ...
    /// Dashboard entry points (`/dashboard`, `Ctrl+\`, `hystersis dashboard`, the
    ...
    /// `hystersis.com/sessions/list` polls and `hystersis.com/sessions/changed` broadcasts.
    ...
    /// poll outside leader mode, so we fetch the same `hystersis.com/session/list` the
    ...
    /// Reconciled from `hystersis.com/queue/changed` broadcasts so
    ...
    /// confirming `hystersis.com/queue/changed` broadcast has not yet arrived. Keyed by
    ...
    /// non-selectable headers. Gated by `HYSTERSIS_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `HYSTERSIS_CANCEL_REWIND` env /
    ...
    /// When false, the pager must not request recaps (zero `hystersis.com/recap` traffic).
    ...
    /// empty and inert outside `--minimal`; the `xai-hystersis-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<hystersis_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat: hystersis_foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `hystersis_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Stamps `_meta["hystersis.com/session"].kind = "chat"` and omits Build agent
    ...
    /// at startup and on settings toggles from `HYSTERSIS_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: hystersis_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<hystersis_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `hystersis wrap` tip: set after the first
    ...
    /// Login button label from `AuthMethod.name` (e.g., "hystersis.com", "Acme Corp").
    ...
    /// `hystersis_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, hystersis_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `hystersis_build_access_gate`. `Some` = blocked.
    pub gate: Option<hystersis_shell::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "HystersisPro", "Free").
    ...
    pub pending_gate_verification: Option<hystersis_shell::auth::GateInfo>,
    ...
    /// `~/.hystersis/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `HYSTERSIS_VOICE_MODE=0` — the STT pipeline is not started and
...
impl AppView {
    ...
    pub(crate) fn finish_startup(&mut self, outcome: hystersis_telemetry::startup::StartupOutcome) {
        hystersis_telemetry::startup::PendingStartup::finish_held(
    ...
    pub fn gate_from_settings(
        rs: &hystersis_shell::util::config::RemoteSettings,
    ) -> Option<hystersis_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(hystersis_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &hystersis_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            hystersis_telemetry::session_ctx::log_event(
                hystersis_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: hystersis_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: hystersis_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: hystersis_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && hystersis_voice::AUDIO_SUPPORTED
    }
    ...
    /// `hystersis.com/settings/update` handler when the subscription tier changes, so
    ...
    fn voice_send(&self, cmd: hystersis_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(hystersis_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(hystersis_voice::VoiceCommand::PttRelease);
    ...
    /// `hystersis.com/queue/changed` broadcast. The broadcast is
    ...
    /// `hystersis.com/queue/changed` broadcast replaces it (matched by `id`) rather than
    ...
    /// Rows the dev `HYSTERSIS_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<hystersis_shell::extensions::session_search::SearchSessionHit>>,
...
fn handle_welcome_input(ev: &Event, ctx: &mut WelcomeInputCtx<'_>) -> InputOutcome {
    ...
    if let Event::Key(key) = ev {
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    hystersis_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
        ...
        if *ctx.prompt_focused {
            // On Pending screen, 'l'/'q'/Enter/Ctrl+C/D are menu shortcuts, not text.
            // Don't consume them as prompt input so the Pending branch below can
            // dispatch Login/Quit. See welcome/mod.rs Pending menu handling.
            let is_pending_shortcut = matches!(ctx.auth_state, AuthState::Pending { .. })
                && (key!('l').matches(key)
                    || key!('q').matches(key)
                    || key!(Enter).matches(key)
                    || key!('c', CONTROL).matches(key)
                    || key!('d', CONTROL).matches(key));
            if !is_pending_shortcut {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = ctx.gate_url_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::OpenSuperhystersisUrl);
                }
                if let Some(rect) = ctx.upgrade_cta_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        hystersis_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
fn dispatch_access_gate_menu_action(index: usize) -> InputOutcome {
    match index {
        0 => InputOutcome::Action(Action::OpenSuperhystersisUrl),
...
impl AppView {
    ...
    pub fn draw(&mut self, terminal: &mut PagerTerminal) {
        ...
        hystersis_telemetry::startup::record_first_frame();
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            hystersis_telemetry::session_ctx::log_event(
                                hystersis_telemetry::events::HystersisProUpsellShown {
                                    source:
                                        hystersis_telemetry::events::HystersisProUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`hystersis_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use hystersis_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = hystersis_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                hystersis_telemetry::session_ctx::log_event(
                    hystersis_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: hystersis_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `hystersis wrap` tip trigger, run at the top of every `draw`
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::ContextualTip {
                tip: hystersis_telemetry::events::ContextualTipKind::ImageInput,
                action: hystersis_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
