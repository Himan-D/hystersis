...
use hystersis_shell::sampling::types::ReasoningEffort;
use hystersis_shell::session::unified_list::SessionKind;
...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: hystersis_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// Open hystersis.com in the browser for HystersisPro subscription upsell.
    OpenSuperhystersisUrl,
    /// Re-check subscription status via the shell's `hystersis.com/auth/check_subscription`.
    CheckSubscription,
    ...
    /// Open hystersis.com managed connectors, appending session teamId when set.
    ...
    /// `prompt_id`. Routed to the agent as `hystersis.com/queue/remove`;
    /// the resulting `hystersis.com/queue/changed` rebroadcast is the source of truth.
    QueueRemoveShared {
        id: String,
        expected_version: u64,
    },
    /// Reorder the server-authoritative (shared) queued prompts to match
    /// `ordered_ids`. Routed as `hystersis.com/queue/reorder`.
    QueueReorderShared {
        ordered_ids: Vec<String>,
    },
    /// Clear the caller's server-authoritative (shared) queued prompts.
    /// Routed as `hystersis.com/queue/clear`.
    QueueClearShared,
    /// Replace the text of a server-authoritative (shared) queued prompt.
    /// Routed to the agent as `hystersis.com/queue/edit`; the rebroadcast of
    /// `hystersis.com/queue/changed` is the source of truth. Last write wins via the
    ...
    /// merges its text into the in-flight turn. Routed as `hystersis.com/queue/interject`;
    /// the `hystersis.com/session/interjection` + `hystersis.com/queue/changed` rebroadcasts are
    ...
    RunEditedQueuedCommand {
        ...
        /// `hystersis.com/queue/remove` request is sent.
    ...
    OpenTranscriptPager,
    /// Minimal mode (`hystersis --minimal`): re-print the most-recently committed
    ...
    OpenExtensionsModal {
        ...
        trigger: hystersis_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(hystersis_hooks_plugins_types::HooksAction),
    /// Execute a plugins management action from the modal.
    ExecutePluginsAction(hystersis_hooks_plugins_types::PluginsAction),
    /// Execute a marketplace management action from the modal.
    ExecuteMarketplaceAction(hystersis_hooks_plugins_types::MarketplaceAction),
    /// Add or update an MCP server via hystersis.com/mcp/upsert.
    UpsertMcpServer {
        name: String,
        config: Box<hystersis_shell::util::config::McpServerConfig>,
    },
    /// Delete an MCP server via hystersis.com/mcp/delete.
    DeleteMcpServer {
        server_name: String,
    },
    ...
    /// Toggle a skill enable/disable via hystersis.com/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    DemoteToBackground,
    /// Request current bundle cache status via `hystersis.com/bundle/status`.
    RequestBundleStatus,
    ...
    AnnouncementsOpenCta(hystersis_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"hystersisnight"`, `"auto"`).
    SetTheme(String),
    ...
    SetTrinetraApiKey(String),
    SetOllamaApiBase(String),
    SetLmStudioApiBase(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `hystersis dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.hystersis/config.toml` and route ACP
/// `hystersis.com/yolo_mode_changed` notifications. The ACP notification is
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: hystersis_foreign_sessions::EnabledForeignSessionSources,
        hystersis_home: std::path::PathBuf,
    ...
    FetchSessionList {
        ...
        generation: u64,
        /// Text search pushed down to `hystersis.com/session/list` as `query` (chat
        ...
        /// When set, stamped as `_meta["hystersis.com/facetFilters"].kind` so the shell
        ...
        headless_policy: hystersis_shell::session::unified_list::HeadlessPolicy,
    },
    ...
    /// `hystersis.com/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `hystersis.com/session/list` — the non-leader fallback for the
    ...
    KillBgTask {
        session_id: acp::SessionId,
        task_id: String,
        source: hystersis_shell::extensions::task::TaskKillSource,
    },
    /// Cancel a subagent via `hystersis.com/subagent/cancel`.
    KillSubagent {
        session_id: acp::SessionId,
        subagent_id: String,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.hystersis/config.toml`.
    ...
    /// Persist a typed setting to `~/.hystersis/config.toml`. On failure,
    ...
    /// `hystersis.com/queue/remove`. The agent re-broadcasts the authoritative queue.
    QueueRemove {
        session_id: acp::SessionId,
        id: String,
        expected_version: u64,
    },
    /// Reorder server-owned queued prompts: fire-and-forget `hystersis.com/queue/reorder`.
    QueueReorder {
        session_id: acp::SessionId,
        ordered_ids: Vec<String>,
    },
    ...
    /// `hystersis.com/queue/edit`. The session actor's serialized mailbox makes this
    /// last-writer-wins for concurrent edits; the rebroadcast of
    /// `hystersis.com/queue/changed` is the truth signal.
    QueueEdit {
        session_id: acp::SessionId,
        id: String,
        new_text: String,
    ...
    /// edits it: fire-and-forget `hystersis.com/queue/hold_edit`.
    QueueHoldEdit {
        session_id: acp::SessionId,
        id: String,
    },
    /// Release a previous [`Self::QueueHoldEdit`]: `hystersis.com/queue/release_edit`.
    QueueReleaseEdit {
        session_id: acp::SessionId,
        id: String,
    },
    /// Interject a server-owned queued prompt into the running turn:
    /// fire-and-forget `hystersis.com/queue/interject`. The session actor atomically
    ...
    /// Resolve the running agent name for a session (`hystersis.com/session/info`).
    ...
    /// Fetch MCP server list from the shell (hystersis.com/mcp/list).
    FetchMcpsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
        cache: bool,
    },
    /// Trigger MCP OAuth for a server (hystersis.com/mcp/auth_trigger).
    McpAuthTrigger {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch hooks list from the shell (hystersis.com/hooks/list).
    FetchHooksList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch plugins list from the shell (hystersis.com/plugins/list).
    FetchPluginsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Fetch skills list from the shell (hystersis.com/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Toggle a skill via hystersis.com/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    MarketplaceAction {
        agent_id: AgentId,
        session_id: acp::SessionId,
        action: hystersis_hooks_plugins_types::MarketplaceAction,
    },
    /// Install a plugin from the inline CTA via `hystersis.com/marketplace/action`,
    ...
    /// Reload plugins after a CTA install via `hystersis.com/plugins/action`
    ...
    /// Read the MCP server list after a CTA install via `hystersis.com/mcp/list`,
    ...
    /// the same `hystersis.com/mcp/list` fetch as `FetchPluginCtaMcps`, reported back via
    ...
    /// Upsert an MCP server via hystersis.com/mcp/upsert.
    UpsertMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        name: String,
    ...
    /// Live-toggle an MCP server via hystersis.com/mcp/toggle (no restart needed).
    ToggleMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Toggle a single MCP tool via hystersis.com/mcp/toggle_tool.
    ToggleMcpTool {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch and display session info via hystersis.com/session/info.
    ...
    /// Fetch and display detailed context usage via hystersis.com/session/info.
    ShowContextInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
        /// Usage-modal fetch generation; echoed back on the task result.
    ...
    /// Fetch current bundle cache status via `hystersis.com/bundle/status`.
    FetchBundleStatus,
    /// Fetch a bundled entry's raw content via `hystersis.com/bundle/entry/get`.
    ...
    SendFeedback {
        ...
        images: Vec<hystersis_shell::session::FeedbackImage>,
    },
    ...
    /// Send raw note to hystersis.com/memory/rewrite for LLM-powered reformatting.
    ...
    /// Fire a /btw side question via hystersis.com/btw ext method.
    SendBtw {
        agent_id: AgentId,
        session_id: acp::SessionId,
        question: String,
    ...
    /// Request a session recap via the hystersis.com/recap ext method. Fire-and-forget:
    /// the recap arrives later as a `SessionRecap` notification.
    SendRecap {
        session_id: acp::SessionId,
        auto: bool,
    },
    /// Send a mid-turn interjection via hystersis.com/interject ext method.
    SendInterject {
        agent_id: AgentId,
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `hystersis.com/session/interjection`
    ...
    /// Log out via `hystersis.com/auth/logout` (shell clears auth.json + in-memory state).
    Logout,
    /// Cancel an in-flight interactive auth on the shell (`hystersis.com/auth/cancel`).
    ...
    /// Re-check subscription status via `hystersis.com/auth/check_subscription`.
    ...
    /// registry (`~/.hystersis/active_sessions.json`).
    ...
    /// Call `hystersis.com/session/fork` to create a peer session that resumes
    ...
    /// Fetch billing/credit usage from the agent's `hystersis.com/billing` extension.
    ...
    FetchAppBilling,
    /// Fetch per-session token/cost via `hystersis.com/session/usage` (auth-agnostic).
    FetchSessionUsage {
        agent_id: AgentId,
        session_id: acp::SessionId,
        /// Usage-modal fetch generation; echoed back on the task result.
    ...
    /// Send an ACP `hystersis.com/suggest` request to the shell. `agent_id` is echoed
    ...
    /// Send an ACP `hystersis.com/suggestPrompt` request to the shell — predict the
    /// user's likely next prompt after a completed turn (tab autocomplete
    /// ghost text).
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`hystersis-0.1` when the
...
/// Wire params for `hystersis.com/session/rename`. Shared with the effect executor
...
/// Outcome of an `hystersis.com/subagent/cancel` request, telling dispatch whether the
...
pub enum TaskResult {
    ...
    SessionCreated {
        ...
        /// `_meta["hystersis.com/schedulerBackgroundLoops"]`). `None` from a shell that
    ...
    WorktreeForked {
        ...
        restore_degree: Option<hystersis_workspace::session::git::RestoreDegree>,
    ...
    SessionLoaded {
        ...
        /// `_meta["hystersis.com/runningPromptId"]`), present only when the session was
    ...
    SessionListLoaded {
        ...
        /// Degraded conversations lane (`_meta["hystersis.com/partial"]`), surfaced
        /// as an actionable picker notice instead of a silent empty list.
        partial: Option<crate::app::effects::ConversationsPartial>,
        /// Directory scope `sessions` were drawn from (`hystersis.com/listScope`).
        scope: hystersis_shell::session::unified_list::ListScope,
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<hystersis_foreign_sessions::RecentForeignSession>,
    },
    ...
    /// Leader session roster loaded via `hystersis.com/sessions/list`.
    RosterLoaded {
    ...
    /// Response to `hystersis.com/subagent/cancel`; see [`SubagentKillOutcome`].
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<hystersis_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<hystersis_shell::util::changelog::ChangelogEntry>,
    },
    ...
    AuthUrlReady {
        ...
        external: bool,
        /// Presentation mode from `hystersis.com/auth/get_url`; `None` on older agents.
        mode: Option<String>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<hystersis_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<hystersis_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<hystersis_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<hystersis_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<hystersis_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        info: Box<hystersis_shell::session::SessionInfoResponse>,
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<hystersis_shell::extensions::notification::PromptUsage>,
        nonce: u64,
    },
    ...
    /// `hystersis.com/recap` request acknowledged (fire-and-forget). The recap itself
    ...
    LogoutComplete,
    /// Best-effort `hystersis.com/auth/cancel` finished (no UI update; state already left Authenticating).
    AuthCancelComplete,
    /// Shell responded to `hystersis.com/auth/check_subscription`. `verify` echoes
    ...
    DeepSearchResults {
        ...
        generation: u64,
        results: Vec<hystersis_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    /// `hystersis.com/session/fork` completed (no-worktree path). The pager adopts
    ...
    /// `hystersis.com/session/fork` failed. The placeholder agent stays in
    ...
    GateRefreshed {
        settings: Option<hystersis_shell::util::config::RemoteSettings>,
    },
    ...
    /// Shell suggestions loaded from ACP `hystersis.com/suggest`. `request_text` /
    ...
    /// Predicted next prompt loaded from ACP `hystersis.com/suggestPrompt`.
