...
pub use hystersis_prompt_queue as prompt_queue;
mod acp_handler;
mod connect_timeout;
mod csi_filter;
mod dispatch;
...
pub(crate) fn push_gboom_keyboard_flags() {
    ...
    hystersis_shell::util::with_locked_stderr(|stderr| {
...
/// (`[ui] mouse_reporting_toggle` / `HYSTERSIS_MOUSE_REPORTING_TOGGLE`). Seeded once
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use hystersis_shell::agent::config::{ConfigSource, Feature, FeatureSources};
...
///
/// Uses [`hystersis_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `xai-hystersis-pager-minimal`
...
impl ScreenMode {
    ...
    /// allowlist (`xai-hystersis-telemetry`'s `KNOWN_SCREEN_MODES`); renaming one
...
pub fn resolve_leader_mode<'p>(
    ...
    _remote_settings: Option<&hystersis_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<hystersis_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<hystersis_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
async fn bounded_connect(
    ...
    let context = || startup_failure::Context {
        target,
        attempt,
        version: hystersis_version::display_version_with_commit(
            hystersis_version::full_version(),
            hystersis_update::channel_label(),
        ),
        log_path: hystersis_telemetry::unified_log::path(),
    };
    tokio::select! {
        biased;
...
/// should print a message telling the user to relaunch `hystersis`.
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<hystersis_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    hystersis_tty_utils::redirect_native_stderr();
    ...
    let hystersis_com_config = match hystersis_shell::agent::config::Config::new_from_toml_cfg(
        &raw_config,
    ) {
        Ok(c) => c.hystersis_com_config,
        Err(e) => {
            tracing::warn!(error = %e, "failed to parse config for auth refresh, using defaults");
            hystersis_shell::auth::HystersisComConfig::default()
        }
    };
    let refreshed_auth = tokio::time::timeout(
        hystersis_shell::http::STARTUP_AUTH_REFRESH_TIMEOUT,
        hystersis_shell::auth::try_ensure_fresh_auth(&hystersis_com_config),
    )
    .await
    .unwrap_or(None);
    let early_prefetch = match refreshed_auth {
        Some(auth) => hystersis_shell::agent::models::start_early_prefetch_with_auth(Some(auth)),
        None => hystersis_shell::agent::models::start_early_prefetch(Some(hystersis_com_config.clone())),
    };
    ...
    if had_prefetch {
        hystersis_telemetry::startup::record_prefetch_wait(prefetch_wait_started.elapsed());
    }
    hystersis_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(hystersis_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = hystersis_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("HYSTERSIS_HUNK_TRACKER").ok();
    ...
    let launch_auto = hystersis_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
        hystersis_shell::util::config::default_interactive_permission_mode(),
    );
    ...
    hystersis_telemetry::external::init(
        hystersis_shell::agent::config::resolve_external_otel_config(
            hystersis_telemetry::external::config::ExternalClientInfo {
                service_version: hystersis_version::full_version().to_owned(),
                client_version: hystersis_version::VERSION.to_owned(),
                app_entrypoint: "tui".to_owned(),
            },
        ),
    );
    let pending_startup = hystersis_telemetry::startup::PendingStartup::new();
    let timer = hystersis_telemetry::startup::begin(crate::acp::Owner::Client);
    ...
    drop(agent_guard);
    hystersis_telemetry::session_ctx::drain_at_process_exit().await;
    hystersis_tty_utils::global_process_scope().kill_all();
...
fn print_exit_resume_hint(info: &ExitInfo, max_width: usize, w: &mut impl Write) {
    ...
    if info.minimal {
        let _ = writeln!(w, "  hystersis --minimal --resume {}", info.session_id);
    } else {
        let _ = writeln!(w, "  hystersis --resume {}", info.session_id);
    }
}
...
fn disable_mouse_paste_raw() {
    hystersis_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(hystersis_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    hystersis_crash_handler::enable_terminal_escape_restore();
    ...
    let (terminal, screen_mode) = (|| -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        hystersis_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(hystersis_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                hystersis_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = hystersis_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let term = hystersis_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
    ...
    .inspect_err(|_| {
        ...
        hystersis_crash_handler::disable_terminal_escape_restore();
    })?;
    Ok(TerminalInit {
        terminal,
        screen_mode,
...
/// BEL/ESC (titles can arrive from hystersis.com conversation metadata) would
...
mod tests {
    ...
    fn restore_runs_teardown_even_when_writer_failed() {
        ...
        let terminal = hystersis_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
    ...
    fn terminal_title_strips_control_characters() {
        assert_eq!(
            ...
            "evil]52;c;payloadtitle - hystersis"
        );
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "hystersis");
        assert_eq!(terminal_title_string(""), "hystersis");
        assert_eq!(terminal_title_string("My chat"), "My chat - hystersis");
    }
    #[test]
    ...
    fn cli_leader_and_no_leader_conflict() {
        let result = try_parse_pager(&["hystersis-pager", "--leader", "--no-leader"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_leader_flag_parses() {
        let args = try_parse_pager(&["hystersis-pager", "--leader"]).unwrap();
        assert!(args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    fn cli_no_leader_flag_parses() {
        let args = try_parse_pager(&["hystersis-pager", "--no-leader"]).unwrap();
        assert!(!args.leader);
        assert!(args.no_leader);
    }
    #[test]
    fn cli_hidden_memory_compat_flags_parse_and_collapse() {
        let enabled = try_parse_pager(&["hystersis-pager", "--experimental-memory"]).unwrap();
        assert_eq!(enabled.memory_enabled_override(), Some(true));
        assert_eq!(
            enabled.memory_override_flag(),
            Some("--experimental-memory")
        );
        let disabled = try_parse_pager(&["hystersis-pager", "--no-memory"]).unwrap();
        assert_eq!(disabled.memory_enabled_override(), Some(false));
        assert_eq!(disabled.memory_override_flag(), Some("--no-memory"));
        let deferred = try_parse_pager(&["hystersis-pager"]).unwrap();
    ...
    fn cli_hidden_memory_compat_flags_conflict() {
        assert!(try_parse_pager(&["hystersis-pager", "--experimental-memory", "--no-memory"]).is_err());
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["hystersis-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["hystersis-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["hystersis-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = hystersis_shell::util::config::RemoteSettings {
    ...
    fn cli_resume_parses_session_id() {
        let args = try_parse_pager(&["hystersis-pager", "--resume", "abc-123"]).unwrap();
        assert_eq!(args.session_to_resume(), Some("abc-123"));
    }
    #[test]
    fn cli_short_r_parses_session_id() {
        let args = try_parse_pager(&["hystersis-pager", "-r", "abc-123"]).unwrap();
        assert_eq!(args.session_to_resume(), Some("abc-123"));
    }
    #[test]
    fn cli_load_alias_parses_session_id() {
        let args = try_parse_pager(&["hystersis-pager", "--load", "abc-123"]).unwrap();
        assert_eq!(args.session_to_resume(), Some("abc-123"));
    }
    #[test]
    fn cli_resume_preferred_over_load() {
        let mut args = try_parse_pager(&["hystersis-pager", "--resume", "from-resume"]).unwrap();
        args.load_session = Some("from-load".into());
        assert_eq!(args.session_to_resume(), Some("from-resume"));
    }
    #[test]
    fn cli_continue_flag_parses() {
        let args = try_parse_pager(&["hystersis-pager", "--continue"]).unwrap();
        assert!(args.continue_last_session);
        assert_eq!(args.session_to_resume(), None);
    }
    #[test]
    fn cli_continue_short_c_parses() {
        let args = try_parse_pager(&["hystersis-pager", "-c"]).unwrap();
        assert!(args.continue_last_session);
    }
    #[test]
    fn cli_resume_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["hystersis-pager", "--resume"]).unwrap();
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["hystersis-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["hystersis-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["hystersis-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_chat_flag_rejected_without_feature() {
        assert!(try_parse_pager(&["hystersis-pager", "--chat"]).is_err());
    }
    #[cfg(feature = "local-workspace")]
    #[test]
    fn cli_local_workspace_attach_requires_chat() {
        assert!(
            try_parse_pager(&["hystersis-pager", "--local-workspace-attach=srv"]).is_err(),
            "attach without --chat must clap-error"
        );
        let args =
            try_parse_pager(&["hystersis-pager", "--chat", "--local-workspace-attach=srv"]).unwrap();
    ...
    fn cli_local_workspace_cwd_requires_chat() {
        assert!(try_parse_pager(&["hystersis-pager", "--local-workspace-cwd=/tmp/a"]).is_err());
    ...
    fn cli_local_workspace_flags_rejected_without_feature() {
        assert!(try_parse_pager(&["hystersis-pager", "--local-workspace-attach=srv"]).is_err());
        assert!(try_parse_pager(&["hystersis-pager", "--local-workspace"]).is_err());
        assert!(try_parse_pager(&["hystersis-pager", "--local-workspace-cwd=/tmp"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["hystersis-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["hystersis-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["hystersis-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["hystersis-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["hystersis-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["hystersis-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["hystersis-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["hystersis-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["hystersis-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["hystersis-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_resume_and_fork_ok() {
        let args =
            try_parse_pager(&["hystersis-pager", "-s", "a", "--resume", "b", "--fork-session"]).unwrap();
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["hystersis-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_hystersis() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "hystersis");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "Hystersis TUI",
                "",
                "Usage: hystersis [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["hystersis-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["hystersis-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
    ...
    fn print_exit_resume_hint_includes_minimal_flag() {
        ...
        assert_eq!(
            String::from_utf8(buf).unwrap(),
            "\nResume this session with:\n  hystersis --minimal --resume sess-abc\n"
        );
    }
    #[test]
    ...
    fn print_exit_resume_hint_truncates_summary_to_width() {
        ...
        assert!(out.contains(&format!("\n> {}…\n", "p".repeat(17))));
        assert!(out.contains(&format!("\n  {}…\n", "r".repeat(17))));
        assert!(out.contains("  hystersis --resume sess-abc\n"));
    }
    #[test]
