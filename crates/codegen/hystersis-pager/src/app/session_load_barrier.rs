...
//! `xai-hystersis-shell` leader), so another client's live event cannot land on this
...
mod tests {
    ...
    fn ext_session_update_raw(params: serde_json::Value) -> AcpClientMessage {
        ext_notification("hystersis.com/session/update", params)
    }
    ...
    fn request_permission(session: &str) -> AcpClientMessage {
        ...
        AcpClientMessage::RequestPermission(hystersis_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        })
    }
