...
//!
//! Env sandboxing follows this crate's `serial(HYSTERSIS_HOME)` idiom; note
//! `hystersis_home()` is process-cached (OnceLock), so disk assertions always go
//! through [`effective_hystersis_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary: the harness
//! mutates process-global env (proxy URLs, `XAI_API_KEY`,
//! `HYSTERSIS_LEADER_SOCKET`, `HYSTERSIS_HOME`) for a real agent's whole lifetime, and
//! in a several-thousand-test process that poisons concurrently-running tests
//! (and `hystersis_home()`'s OnceLock is usually already pinned). Run on demand:
//!
//! ```bash
//! cargo test -p xai-hystersis-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
use hystersis_acp_lib::{AcpClientRx, acp_send};
...
/// The hystersis home the agent actually persisted under: `hystersis_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_hystersis_home() -> PathBuf {
    hystersis_config::hystersis_home()
}
...
struct PagerLeaderCluster {
    ...
    /// agent on the same HYSTERSIS_HOME (two agents on one updates.jsonl is the
    ...
    _hystersis_home: TempDir,
}

impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(HYSTERSIS_HOME)]`
    ...
    async fn start() -> Self {
        ...
        let sock_path = hystersis_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("HYSTERSIS_HOME", hystersis_home.path()),
            crate::test_util::EnvVarGuard::set("HYSTERSIS_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("HYSTERSIS_XAI_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("XAI_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("HYSTERSIS_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("HYSTERSIS_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("HYSTERSIS_TRACE_UPLOAD", "false"),
        ...
        let mut cluster = Self {
            ...
            _flock: flock,
            _env: env,
            _hystersis_home: hystersis_home,
        };
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (hystersis_version), not
        ...
        generation_tasks.extend(hystersis_shell::leader::in_process::spawn_agent(
            acp_rx,
            response_tx,
        ));
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same HYSTERSIS_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `HYSTERSIS_LEADER_SOCKET`, flock held
    ...
    async fn client(&mut self, name: &str, reconnect: bool) -> ClusterClient {
        ...
        let (reconnector, status_rx) = if reconnect {
            ...
            let reconnector = LeaderReconnector::new(
                ...
                ClientMode::Stdio,
                LeaderEnvUrls {
                    hystersis_ws_url: String::new(),
                    hystersis_ws_origin: String::new(),
                },
                LeaderClientCapabilities {
                    client_version: Some("0.0.0-test".to_string()),
                    ..Default::default()
