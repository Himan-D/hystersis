#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.hystersis` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.hystersis` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.hystersis` reads).
    #[test]
