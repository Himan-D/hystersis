#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn settings_non_api_key_tier_clears_stale_api_key_flag() {
        ...
        assert!(app.voice_mode_enabled);
        assert!(handle_ext_notification(
            &tier_settings_update("HystersisPro"),
            &mut app
        ));
        assert!(!app.is_api_key_auth);
        assert!(app.voice_mode_enabled);
    ...
    /// Build an `hystersis.com/settings/update` carrying only the scheduler flag.
    ...
    fn loop_instruction(app: &mut AppView, args: &str) -> String {
        ...
        if let Some(agent) = app.agents.get_mut(&AgentId(0)) {
            agent
                ...
                .registry_mut()
                .set_available_tools(
                    [hystersis_tools::implementations::hystersis_build::SCHEDULER_CREATE_TOOL_NAME]
                        .into_iter()
                        .map(str::to_string)
                        .collect(),
                );
    ...
    /// gen-ordered `hystersis.com/announcements/update` for every settings writer, and a
