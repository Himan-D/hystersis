#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn retry_exhausted_rate_limited_empty_reason_uses_oauth_fallback() {
        use hystersis_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_OAUTH;
    ...
    fn retry_exhausted_api_key_rewrites_consumer_subscription_upsell() {
        use hystersis_shell::sampling::error::RATE_LIMITED_USER_MESSAGE_API_KEY;
        ...
        match last_session_event(&scrollback) {
            Some(SessionEvent::RetryFailed { error, .. }) => {
                ...
                assert!(!error.contains("hystersis.com/upgrade"));
            }
    ...
    fn apply_retry_state_auth_failure_pushes_reauth_prompt() {
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) from https://cli-chat-proxy.hystersis.com/v1/messages: \
                          no auth context"
                    .into(),
            },
            &mut session,
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `hystersis logout` /
    /// `hystersis login` guidance), not the generic re-auth prompt.
    #[test]
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) ... deprecated authentication method (WebLogin) ... \
                          run `hystersis logout` then `hystersis login`"
                    .into(),
            },
            &mut session,
            &mut scrollback, false);
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(hystersis_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
    ...
    fn tool_call_delta_chunk_ignored_during_replay() {
        ...
        let request = acp::ExtNotification::new("hystersis.com/session_notification", raw.into());
        let changed = handle(
    ...
    fn summary_generated_ext(
        ...
        let meta = if title_is_manual {
            Some(hystersis_shell::extensions::notification::title_is_manual_meta())
        } else {
            None
        };
    ...
    fn manual_meta_false_clears_display_name() {
        ...
        let n = SessionNotification {
            ...
            meta: Some(serde_json::json!({ "hystersis.com/titleIsManual": false })),
        };
        let raw = serde_json::value::to_raw_value(&n).unwrap();
        let notif = acp::ExtNotification::new("hystersis.com/session_notification", std::sync::Arc::from(raw));
    ...
    fn auto_title_notification_strips_controls_and_caps() {
        use hystersis_shell::session::persistence::MAX_TITLE_SCALARS;
        let mut app = make_app_with_agent("sess-1");
        let dirty = format!(
