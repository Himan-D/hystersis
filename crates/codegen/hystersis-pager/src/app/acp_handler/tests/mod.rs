...
use hystersis_shell::extensions::notification::RetryState;
use hystersis_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> hystersis_shell::extensions::notification::ImageCompressedEntry {
    hystersis_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn interjection_broadcast(
    ...
    acp::ExtNotification::new(
        "hystersis.com/session/interjection",
        std::sync::Arc::from(
...
pub(super) fn make_exit_plan_ext(
    ...
    hystersis_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<hystersis_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn make_exit_plan_ext_with_tool_call_id(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    let request = acp::ExtRequest::new("hystersis.com/exit_plan_mode", raw.into());
    let (tx, rx) = tokio::sync::oneshot::channel();
    (
        hystersis_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        },
        rx,
...
/// Build a `hystersis.com/queue/changed` notification carrying `runningPromptId`.
...
pub(super) fn send_tool_call_update(
    ...
    handle(
        AcpClientMessage::SessionNotification(hystersis_acp_lib::AcpArgs {
...
pub(super) fn make_inject_notif(payload: &serde_json::Value) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "hystersis.com/scheduled_task_inject_prompt",
...
pub(super) fn make_fired_notif(
    ...
    acp::ExtNotification::new("hystersis.com/scheduled_task_fired", std::sync::Arc::from(raw))
}
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[hystersis_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
        "hystersis.com/announcements/update",
        std::sync::Arc::from(
            serde_json::value::to_raw_value(
...
pub(super) fn make_created_ext_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("hystersis.com/scheduled_task_created", std::sync::Arc::from(raw))
}
pub(super) fn make_deleted_ext_notif(
    ...
    make_deleted_ext_notif_with_reason(
        session_id,
        task_id,
        hystersis_tools::notification::ScheduledTaskRemovedReason::Unknown,
        false,
    )
}
pub(super) fn make_deleted_ext_notif_with_reason(
    session_id: &str,
    task_id: &str,
    reason: hystersis_tools::notification::ScheduledTaskRemovedReason,
    is_replay: bool,
) -> acp::ExtNotification {
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("hystersis.com/scheduled_task_deleted", std::sync::Arc::from(raw))
}
...
pub(super) fn hystersis_model_switch_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
...
pub(super) fn hystersis_unhandled_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
...
/// Build an `hystersis.com/session/prompt_complete` ext-notification for `session_id`.
pub(super) fn prompt_complete_ext(session_id: &str) -> acp::ExtNotification {
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("hystersis.com/session/prompt_complete", std::sync::Arc::from(raw))
}
...
/// Build an `hystersis.com/session/prompt_complete` ext-notification with an explicit
...
/// Build an `hystersis.com/session/prompt_complete` ext-notification carrying a
...
/// Build a durable `TurnCompleted` update on the `hystersis.com/session/update` rail,
...
pub(super) fn hystersis_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    is_replay: bool,
...
pub(super) fn hystersis_turn_completed_notif_with_cancel_trigger(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    cancel_trigger: &str,
...
pub(super) fn hystersis_wake_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
...
/// `hystersis.com/session/update` rail, optionally stamped `isReplay`.
/// `prompt_id == None` models pre-attribution shells.
pub(super) fn hystersis_hook_execution_notif_for_prompt(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
) -> acp::ExtNotification {
    use hystersis_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
    hystersis_hook_execution_notif_with_runs(
        session_id,
        event_name,
        prompt_id,
        is_replay,
...
pub(super) fn hystersis_hook_execution_notif_with_runs(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
    runs: Vec<hystersis_shell::extensions::notification::HookRunEntryDto>,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn hystersis_hook_execution_notif(
    session_id: &str,
    event_name: &str,
    is_replay: bool,
) -> acp::ExtNotification {
    hystersis_hook_execution_notif_for_prompt(session_id, event_name, None, is_replay)
}
...
/// Build an `hystersis.com/session/interjection` ext-notification (no id).
pub(super) fn interjection_ext(session_id: &str, text: &str) -> acp::ExtNotification {
    interjection_ext_with_id(session_id, text, None)
}
/// Build an `hystersis.com/session/interjection` ext-notification with an optional
...
pub(super) fn interjection_ext_with_id(
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("hystersis.com/session/interjection", std::sync::Arc::from(raw))
}
...
/// Build an `ExtNotification` envelope for `hystersis.com/session_notification`.
...
pub(super) fn make_ext_session_notification_with_method(
    ...
    AcpClientMessage::ExtNotification(hystersis_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    })
}
...
/// Shared temp `HYSTERSIS_HOME` for disk-replay tests. `hystersis_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `HYSTERSIS_HOME`
...
/// Runs `f` with a thread-local hystersis home override so disk replay tests do not
/// depend on process-wide `hystersis_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_hystersis_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_hystersis_home_for_tests(None);
    out
}
pub(super) fn write_child_updates_jsonl(
    ...
    write_child_updates_jsonl_under_cwd(hystersis_home, "/tmp", child_sid, content);
}
pub(super) fn write_child_updates_jsonl_under_cwd(
    ...
    let sessions_dir = hystersis_home
        .join("sessions")
        .join(hystersis_config::encode_cwd_dirname(cwd))
        .join(child_sid);
...
pub(super) fn dispatch_goal_update(
    ...
    handle(
        AcpClientMessage::ExtNotification(hystersis_acp_lib::AcpArgs {
            request: acp::ExtNotification::new("hystersis.com/session_notification", raw.into()),
            response_tx: tx,
        }),
        app,
    )
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(hystersis_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
}
/// Build an `hystersis.com/session_notification` carrying
...
pub(super) fn interaction_resolved_ext(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("hystersis.com/session_notification", std::sync::Arc::from(raw))
}
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = hystersis_workspace::session::git::GitHeadChanged {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("hystersis.com/git_head_changed", std::sync::Arc::from(raw))
}
pub(super) fn make_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("hystersis.com/task_backgrounded", std::sync::Arc::from(raw))
}
...
pub(super) fn make_replayed_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("hystersis.com/session/update", std::sync::Arc::from(raw))
}
...
pub(super) fn send_late_bg_detection(app: &mut AppView, tc_id: &str) {
    use serde_json::json;
    use hystersis_tools::types::output::{BashOutput, ToolOutput};
    let agent = app.agents.get_mut(&AgentId(0)).unwrap();
    let meta = crate::acp::meta::NotificationMeta::default();
    let bash = BashOutput {
...
pub(super) fn task_completed_notif(
    ...
    use hystersis_tools::types::TaskSnapshot;
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("hystersis.com/task_completed", std::sync::Arc::from(raw))
}
pub(super) fn make_monitor_event_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("hystersis.com/monitor_event", std::sync::Arc::from(raw))
}
...
pub(super) fn make_models_update_notif(
    ...
    acp::ExtNotification::new("hystersis.com/models/update", std::sync::Arc::from(raw))
}
/// `hystersis.com/models/update` carrying a single reasoning-capable model whose
...
/// Helper: build an `hystersis.com/mcp/init_progress` notification.
pub(super) fn make_mcp_init_progress_notif(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("hystersis.com/mcp/init_progress", std::sync::Arc::from(raw))
}
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: hystersis_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use hystersis_shell::extensions::mcp::{
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("hystersis.com/mcp/server_status", std::sync::Arc::from(raw))
}
...
pub(super) fn make_servers_updated_notif() -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("hystersis.com/mcp/servers_updated", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = hystersis_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "hystersis_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("hystersis.com/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName": "hystersis_com_linear", "tools": [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("hystersis.com/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_mcp_initialized_notif(session_id: &str) -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("hystersis.com/mcp_initialized", std::sync::Arc::from(raw))
}
