#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn reconnect_reload_failure_restores_pre_outage_transcript() {
        ...
        let id = AgentId(0);
        {
            ...
            agent.last_applied_hystersis_event_seq = Some(5);
            agent.begin_session_reload(1);
        }
        ...
        let _ = handle_ext_notification(&hystersis_model_switch_notif("sess-rc", "sess-rc-30"), &mut app);
        assert_eq!(
        ...
        assert_eq!(
            app.agents[&id].last_applied_hystersis_event_seq,
            Some(30),
        ...
        assert_eq!(
            agent.last_applied_hystersis_event_seq,
            Some(5),
    ...
    #[test]
    fn hystersis_session_update_dedup_drops_already_applied_event() {
        let mut app = make_app_with_agent("sess-xdup");
        let id = AgentId(0);

        assert!(handle_ext_notification(
            &hystersis_model_switch_notif("sess-xdup", "sess-xdup-10"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 1);
        assert_eq!(app.agents[&id].last_applied_hystersis_event_seq, Some(10));
        ...
        assert!(handle_ext_notification(
            &hystersis_model_switch_notif("sess-xdup", "sess-xdup-11"),
            &mut app
        ));
        assert_eq!(app.agents[&id].scrollback.len(), 2);
        assert_eq!(app.agents[&id].last_applied_hystersis_event_seq, Some(11));

        // Lower-stale re-delivery (an already-applied lower id re-sent by
        // the cursor tail, e.g. goal mode) is dropped too — `<=`, not just
        // equality.
        assert!(!handle_ext_notification(
            &hystersis_model_switch_notif("sess-xdup", "sess-xdup-9"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].scrollback.len(),
    ...
    #[test]
    fn unhandled_hystersis_update_does_not_advance_cursor_or_highwater() {
        let mut app = make_app_with_agent("sess-ig");
        let id = AgentId(0);

        assert!(!handle_ext_notification(
            &hystersis_unhandled_notif("sess-ig", "sess-ig-7"),
            &mut app
        ));
        assert_eq!(
        ...
        assert_eq!(app.agents[&id].last_applied_hystersis_event_seq, Some(8));
    }
    ...
    #[test]
    fn direct_hystersis_event_does_not_shoot_down_delayed_acp_chunk() {
        let mut app = make_app_with_agent("sess-split");
        let id = AgentId(0);

        // Direct xAI emission stamped N+1 arrives first.
        assert!(handle_ext_notification(
            &hystersis_model_switch_notif("sess-split", "sess-split-21"),
            &mut app
        ));
        assert_eq!(app.agents[&id].last_applied_hystersis_event_seq, Some(21));
    ...
    #[test]
    fn hystersis_session_update_replay_gating_and_cursor() {
    ...
    fn ignored_model_changed_does_not_advance_cursor_applied_one_does() {
        ...
        {
            let agent = app.agents.get_mut(&id).unwrap();
            seed_models(agent, "hystersis-3", &["hystersis-3", "hystersis-4"]);
        }

        // Unknown model → ignored → both markers untouched.
        assert!(!handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "hystersis-99-unknown", "sess-1-7"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id, None,
        ...
        assert!(handle_ext_notification(
            &model_changed_ext_with_event("sess-1", "hystersis-4", "sess-1-8"),
            &mut app
        ));
        assert_eq!(
            app.agents[&id].last_seen_event_id.as_deref(),
