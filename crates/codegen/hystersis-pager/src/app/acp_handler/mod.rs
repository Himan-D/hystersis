...
//! and xAI session extension notifications (`hystersis.com/session_notification` and
//! replay-path `hystersis.com/session/update`).
...
use hystersis_acp_lib::AcpClientMessage;

use super::actions::Effect;
use hystersis_shell::extensions::notification::{
    SessionNotification, SessionUpdate as XaiSessionUpdate, is_reauthable_failure,
};
use hystersis_shell::tools::todo::todo_item_from_plan_entry;
use hystersis_tools::notification::ScheduledTaskRemovedReason;
use hystersis_workspace::permission::bash_command_splitting::BashCommandHighlights;
...
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    match method {
        "hystersis.com/follow_ups" => handle_follow_ups(notif, app),
        "hystersis.com/task_backgrounded" => handle_task_backgrounded(notif, app),
        "hystersis.com/task_completed" => handle_task_completed(notif, app),
        "hystersis.com/models/update" => handle_models_update(notif, app),
        "hystersis.com/settings/update" => handle_settings_update(notif, app),
        "hystersis.com/sessions/changed" => handle_sessions_changed(notif, app),
        "hystersis.com/queue/changed" => handle_queue_changed(notif, app),
        ...
        "hystersis.com/session/prompt_complete" => handle_prompt_complete(notif, app),
        "hystersis.com/session/interjection" => handle_interjection(notif, app),
        "hystersis.com/monitor_event" => handle_monitor_event(notif, app),
        "hystersis.com/scheduled_task_created" => handle_scheduled_task_created(notif, app),
        "hystersis.com/scheduled_task_fired" => handle_scheduled_task_fired(notif, app),
        "hystersis.com/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        "hystersis.com/scheduled_task_inject_prompt" => handle_scheduled_task_inject_prompt(notif, app),
        "hystersis.com/announcements/update" => handle_announcements_update(notif, app),
        "hystersis.com/git_head_changed" => handle_git_head_changed(notif, app),
        "hystersis.com/leader/version_mismatch" => handle_version_mismatch(notif, app),
        "hystersis.com/mcp/init_progress" => handle_mcp_init_progress(notif, app),
        "hystersis.com/mcp/tools_changed" | "hystersis.com/mcp_initialized" => handle_mcp_tools_changed(notif, app),
        "hystersis.com/mcp/server_status" if push_server_status_enabled() => {
            handle_mcp_server_status(notif, app)
        }
        "hystersis.com/mcp/elicit_complete" => handle_mcp_elicit_complete(notif, app),
        "hystersis.com/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}
...
fn handle_version_mismatch(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Some(banner) = crate::acp::version_mismatch_banner(notif.params.get()) else {
        tracing::warn!("ignoring hystersis.com/leader/version_mismatch without usable versions");
        return false;
    };
...
/// Handle `hystersis.com/session/interjection` — the leader broadcasts this
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse hystersis.com/session/interjection");
        return false;
    };
...
fn handle_ext_method(ext: hystersis_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
    match ext.request.method.as_ref() {
        "hystersis.com/ask_user_question" => handle_ask_user_question(ext, app),
        "hystersis.com/exit_plan_mode" => handle_exit_plan_mode(ext, app),
        "hystersis.com/mcp/elicit" => handle_mcp_elicit(ext, app),
        unknown => {
