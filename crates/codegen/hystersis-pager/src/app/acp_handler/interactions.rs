...
fn cancel_elicitation_request(
    response_tx: tokio::sync::oneshot::Sender<hystersis_acp_lib::AcpResult<acp::ExtResponse>>,
) {
    let cancelled = hystersis_tools::mcp_elicitation::McpElicitExtResponse::Cancel;
...
pub(crate) fn handle_mcp_elicit(
    ext: hystersis_acp_lib::AcpArgs<acp::ExtRequest>,
    app: &mut AppView,
) -> bool {
    use crate::views::elicitation_view::ElicitationViewState;
    use hystersis_tools::mcp_elicitation::McpElicitExtRequest;
...
/// Handle `hystersis.com/ask_user_question` ext-method.
///
            ...
            match kind {
                ...
                kind => {
                    ...
                    let cmd = match kind {
                        ...
                        LocalQuestionKind::FreeUsageUpsell { .. } => "HystersisPro upsell",
...
/// Handle an `hystersis.com/exit_plan_mode` ext_method request.
///
