use super::*;
use hystersis_shell::sampling::error::format_rate_limited_user_message;
...
/// Handle `hystersis.com/session_notification` and replay-path `hystersis.com/session/update`.
///
...
pub(super) fn handle_session_notification_with_origin(
    ...
    let matched = match find_session_match(app, &session_notif.session_id) {
        ...
        None => {
            tracing::debug!(
                ...
                "load-race: hystersis.com/session_notification DROPPED — no agent matches session_id"
            );
            return false;
        }
    };
    ...
    if !is_workflow_update
        ...
        && meta.event_seq.is_some_and(|seq| {
            agent
                .last_applied_hystersis_event_seq
        ...
        })
    {
        tracing::debug!(
            ...
            last_applied = agent.last_applied_hystersis_event_seq,
            "hystersis.com/session update DROPPED by dedup highwater (event_seq <= last_applied)"
        );
        return false;
    }
    ...
    let changed = match session_notif.update {
        ...
        XaiSessionUpdate::HookExecution {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        hystersis_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        hystersis_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        hystersis_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
                            blocked: true,
            ...
            let is_stop_hook =
                hystersis_hooks_plugins_types::HookEvent::from_wire(&event_name).is_turn_end();
            if is_tool_hook {
        ...
        XaiSessionUpdate::HooksChanged {
            ...
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(hystersis_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        XaiSessionUpdate::SessionSummaryGenerated { session_summary } => {
            let title_is_manual = session_notif.meta.as_ref().and_then(|v| {
                v.get(hystersis_shell::extensions::notification::TITLE_IS_MANUAL_META_KEY)
                    .and_then(|v| v.as_bool())
            });
            match title_is_manual {
                Some(true) => {
                    if let Some(clean) =
                        hystersis_shell::session::persistence::sanitize_and_cap_title(
                            &session_summary,
                        )
                    {
                ...
                other => {
                    ...
                    if let Some(clean) =
                        hystersis_shell::session::persistence::sanitize_and_cap_title(&decoded)
                    {
                        agent.generated_session_title = Some(clean);
        ...
        XaiSessionUpdate::ModelChanged {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if hystersis_shell::agent::chat_modes::process_chat_mode_enabled() {
    ...
    if let Some(agent) = app.agents.get_mut(&parent_id) {
        if let Some(seq) = meta.event_seq
            ...
            && !is_workflow_update
        {
            agent.last_applied_hystersis_event_seq = Some(
                agent
                    .last_applied_hystersis_event_seq
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(hystersis_token_estimation::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[hystersis_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &hystersis_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                hystersis_telemetry::session_ctx::log_event(
                    hystersis_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
            ...
            let is_free_usage = *rate_limited
                && hystersis_shell::sampling::error::is_free_usage_exhausted_error(reason);
            if is_credit_limit {
    ...
    if is_credit_limit {
        hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use hystersis_tools::types::SessionMode;
