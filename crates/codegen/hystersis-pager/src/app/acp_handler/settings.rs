...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(campaigns) = update.campaigns.clone() {
        ...
        hystersis_shell::util::config::set_remote_campaigns_from_settings(Some(&rs));
    }
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        // without a restart.
        hystersis_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = hystersis_shell::util::config::auto_permission_mode_enabled_from_disk();
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = hystersis_shell::config::load_effective_config()
    ...
    if let Some(v) = update.privacy_notice_rollout {
        app.privacy_notice_rollout =
            hystersis_config::env_bool("HYSTERSIS_PRIVACY_NOTICE_ROLLOUT").unwrap_or(v);
    }
    ...
    if let Some(v) = update.subscription_tier_display {
        ...
        if was_api_key
            ...
            && app
                .subscription_tier
                .as_deref()
                .is_some_and(hystersis_shell::tier::is_restricted_tier_name)
        {
            app.voice_reset();
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(hystersis_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        hystersis_shell::config::load_merged_requirements(),
        hystersis_shell::config::load_from_disk().ok(),
        hystersis_shell::config::load_managed_config().ok(),
    );
    ...
    // until restart.
    let remote = hystersis_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = hystersis_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = hystersis_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use hystersis_shell::util::config::resolve_tips;
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use hystersis_shell::util::config::resolve_slash_command_tags;
        let effective_config = hystersis_shell::config::load_effective_config().ok();
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = hystersis_shell::util::config::selected_permission_mode(effective_ui, remote)
        .unwrap_or_else(hystersis_shell::util::config::default_interactive_permission_mode);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        hystersis_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn notify_sessions_leave_auto(app: &AppView, session_ids: &[acp::SessionId]) {
    ...
    let args = hystersis_acp_lib::AcpArgs {
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<hystersis_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[hystersis_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = hystersis_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = hystersis_announcements::filter_expired(merged);
    ...
    if hystersis_announcements::prune_hidden_announcement_ids(
...
) -> Option<hystersis_announcements::RemoteAnnouncement> {
...
pub(super) struct PagerSettingsUpdate {
    ...
    #[serde(default)]
    campaigns: Option<Vec<hystersis_shell::util::config::CampaignOverride>>,
    #[serde(default)]
    ...
    #[serde(
        default,
        deserialize_with = "hystersis_shell::util::config::deserialize_tolerant"
    )]
    consent_gate: Option<hystersis_shell::util::config::ConsentGate>,
}
