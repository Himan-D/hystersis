...
fn expired_task_notice(info: &crate::app::agent::ScheduledTaskInfo) -> String {
    ...
    format!(
        ...
        info.human_schedule,
        hystersis_tools::implementations::hystersis_build::scheduler::types::RECURRING_TASK_TTL_DAYS,
    )
}
...
pub(super) fn handle_git_head_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(params) = serde_json::from_str::<hystersis_workspace::session::git::GitHeadChanged>(
        notif.params.get(),
    ) else {
        return false;
    };
