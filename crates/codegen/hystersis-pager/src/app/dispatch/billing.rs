...
use hystersis_telemetry::events::{HystersisProUpsell, HystersisProUpsellClicked};
...
/// Whether the user is at the highest subscription tier (HystersisPro Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("hystersispro_heavy") and CCP display names ("HystersisPro Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "hystersispro_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str = "https://hystersis.com/upgrade";
...
///
/// **`max_tier = true`** (positively identified as HystersisPro Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        hystersis_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        ...
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            hystersis_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(hystersis_telemetry::events::CreditLimitUpsellShown {
            surface: hystersis_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
...
/// Open the HystersisPro upsell for a tier-restricted slash command
...
/// Which situation opened the HystersisPro upsell modal. Controls the heading
...
fn open_hystersispro_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            HystersisProUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with HystersisPro.",
            HystersisProUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(hystersis_telemetry::events::HystersisProUpsellShown {
        source,
        auth_method,
    });

    // /hystersispro lists all plans; every upgrade option lands there.
    let options = vec![
        QuestionOption {
            label: "Upgrade to HystersisPro".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to HystersisPro Plus".into(),
            description: "Significantly higher usage and rate limits".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to HystersisPro Heavy".into(),
            description: "Get the most out of Hystersis. Highest usage limits.".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
    ];
...
/// `hystersis.com/auth/check_subscription` completed. Meta is authoritative
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<hystersis_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<hystersis_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_hystersispro_url(app: &mut AppView) -> Vec<Effect> {
    log_event(HystersisProUpsellClicked {
        source: HystersisProUpsell::WelcomeScreen,
        auth_method: app.login_method_id.as_ref().map(|id| id.0.to_string()),
    });
    let url = "https://hystersis.com/upgrade";
    // Funnel attribution: tag CLI-originated HystersisPro upsell clicks
    // with `referrer=hystersis`, matching the OAuth consent flow and
    // hystersis.com/cli marketing links. Applied even when the URL came from
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "hystersis");
