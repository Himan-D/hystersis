...
use super::billing::dispatch_open_hystersispro_url;
use super::ctx::{
    ...
    let effects = match action {
        Action::Quit | Action::QuitConfirmed => {
            if let Some(tx) = &app.voice_cmd_tx {
                let _ = tx.try_send(hystersis_voice::VoiceCommand::Shutdown);
            }
        ...
        Action::OpenBlockViewer => {
            ...
            let mut credit_card: Option<(String, hystersis_telemetry::events::CreditLimitChoice)> =
                None;
            with_scrollback(app, |s| {
            ...
            if let Some((url, choice)) = credit_card {
                log_event(hystersis_telemetry::events::CreditLimitUpsellClicked {
        ...
        Action::AnnouncementsOpenCta(surface) => {
            if let Some((promo, url)) = crate::views::announcements::promo_cta_target(
                ...
                log_event(hystersis_telemetry::events::AnnouncementCtaClicked {
                    id: promo_id,
                    source: surface,
                });
                open_url_or_show(app, &url);
        ...
        Action::SetCodingDataSharing { opted_in } => set_coding_data_sharing(
            app,
            opted_in,
            hystersis_telemetry::events::CodingDataConsentSource::Settings,
        ),
        ...
        Action::SetOpenAIApiKey(v) => vec![Effect::PersistSetting {
            key: "openai_api_key",
            value: crate::settings::SettingValue::String(v),
            rollback_value: crate::settings::SettingValue::String(String::new()),
        }],
        Action::SetAnthropicApiKey(v) => vec![Effect::PersistSetting {
            key: "anthropic_api_key",
            value: crate::settings::SettingValue::String(v),
            rollback_value: crate::settings::SettingValue::String(String::new()),
        }],

        Action::SetOllamaApiBase(v) => vec![Effect::PersistSetting {
            key: "ollama_api_base",
            value: crate::settings::SettingValue::String(v),
            rollback_value: crate::settings::SettingValue::String(String::new()),
        }],
        Action::SetLmStudioApiBase(v) => vec![Effect::PersistSetting {
            key: "lm_studio_api_base",
            value: crate::settings::SettingValue::String(v),
            rollback_value: crate::settings::SettingValue::String(String::new()),
        }],
        Action::SetOpenRouterApiKey(v) => vec![Effect::PersistSetting {
            key: "openrouter_api_key",
            value: crate::settings::SettingValue::String(v),
            rollback_value: crate::settings::SettingValue::String(String::new()),
        }],
        Action::SetTrinetraApiKey(v) => vec![Effect::PersistSetting {
        ...
        Action::OpenSuperhystersisUrl => dispatch_open_hystersispro_url(app),
        Action::OpenUrl(url) => {
...
pub(super) fn dispatch_action_result(
    ...
    use hystersis_hooks_plugins_types::OutcomeStatus;
                ...
                if let Some(ref mut modal) = agent.extensions_modal {
                    if !outcome.message.trim().is_empty() && modal.result_notice.is_none() {
                        let entry_index = match modal.last_plugins_action {
                            Some(hystersis_hooks_plugins_types::PluginsAction::Uninstall { .. }) => None,
            ...
            OutcomeStatus::ConfirmationRequired => {
                if let Some(ref mut modal) = agent.extensions_modal {
                    let confirmed_action = modal.last_plugins_action.as_ref().map(|a| {
                        ...
                        if let hystersis_hooks_plugins_types::PluginsAction::Uninstall {
                            ref mut confirmed,
                            ..
                        } = action
                        {
