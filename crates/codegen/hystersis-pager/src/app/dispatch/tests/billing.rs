...
use super::*;
use hystersis_shell::sampling::error::is_free_usage_exhausted_error;
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: hystersis_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
fn team_auth_disables_agent_billing_surface() {
    ...
    app.apply_auth_meta(&hystersis_shell::auth::AuthMeta {
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    ...
    app.screen_mode = crate::app::ScreenMode::Minimal;
    let before = agent_scrollback_len(&app);
    let usage = hystersis_shell::extensions::notification::PromptUsage {
        totals: hystersis_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        hystersis_shell::extensions::notification::PromptUsage {
            totals: hystersis_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: hystersis_telemetry::events::HystersisProUpsell::FreeUsagePaywall,
            }
        )
    ));
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use hystersis_shell::extensions::notification::{RetryState, SessionUpdate};
...
fn restricted_command_submit_opens_three_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: hystersis_telemetry::events::HystersisProUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let kind = LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            hystersis_telemetry::events::CreditLimitChoice::UpgradeTier,
            hystersis_telemetry::events::CreditLimitChoice::PurchaseCredits,
        ],
    };
