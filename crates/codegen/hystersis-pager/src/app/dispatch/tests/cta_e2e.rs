...
fn cta_outcome_reload(
    status: hystersis_hooks_plugins_types::OutcomeStatus,
    message: &str,
...
fn plugin_cta_catalog_keeps_official_not_installed_only() {
    ...
    assert_eq!(
        ...
        Some(hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL),
...
fn plugin_cta_marketplace_override_naming_official_selects_it() {
    ...
    app.plugin_cta_marketplace =
        Some(hystersis_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string());
    let id = AgentId(0);
    ...
    for phase in [
        ...
        let id = AgentId(0);
        {
            ...
            cta.source_url_or_path =
                Some(hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into());
            cta.candidates = vec![cta_entry("figma", "not_installed")];
            cta.debounce_generation = 1;
...
mod cta_e2e {
    ...
    fn figma_candidate() -> hystersis_hooks_plugins_types::MarketplacePluginEntry {
        let mut entry = cta_entry("figma", "not_installed");
        entry.keywords = vec!["figma".into()];
    ...
    fn happy_path_with_auth_handoff() {
        ...
        let servers = vec![
            cta_mcp_server("hystersis_com_managed", None, McpServerDisplayStatus::Ready),
    ...
    fn dismissed_plugin_suppressed_on_next_recompute() {
        ...
        let dismissed = hystersis_shell::config::dismissed_plugin_ctas_in_file(&config_path);
        let matched =
        ...
        hystersis_shell::config::add_dismissed_plugin_cta_to_file("figma", &config_path)
        ...
        for (enabled, source_present) in [(false, true), (true, false)] {
            ...
            app.plugin_cta_enabled = enabled;
            {
                ...
                cta.source_url_or_path = source_present
                    .then(|| hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string());
                cta.candidates = vec![figma_candidate()];
                cta.debounce_generation = 1;
                cta.phase = CtaPhase::Matched {
                    plugin_relative_path: "plugins/figma".into(),
