...
fn external_prompt_editor_refuses_elements_with_visible_message() {
    ...
    let image = crate::prompt_images::PastedImage {
        element_id: hystersis_ratatui_textarea::ElementId::from_raw(0),
        display_number: 0,
...
fn seed_foreign_resume_hint(
    ...
    app.apply_foreign_resume_detection(
        launch_token,
        &canonical_cwd,
        Some(hystersis_foreign_sessions::RecentForeignSession {
            tool,
...
fn critical_announcement(id: &str) -> hystersis_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> hystersis_announcements::RemoteAnnouncement {
    hystersis_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(hystersis_announcements::AnnouncementCta {
            label: Some("Go".into()),
...
fn conversation_entry_load_sets_chat_kind_bit() {
    ...
    assert_eq!(
        agent.rename_kind(),
        hystersis_shell::session::unified_list::SessionKind::Chat
    );
    let rename = dispatch(
    ...
    assert!(
        matches!(
            ...
            [Effect::RenameSession { kind, .. }]
                if *kind == hystersis_shell::session::unified_list::SessionKind::Chat
        ),
...
fn load_sticky_chat_history_bypass_rename_kind_is_build() {
    ...
    assert_eq!(
        agent.rename_kind(),
        hystersis_shell::session::unified_list::SessionKind::Build
    );
    let rename = dispatch(
        Action::RenameSession {
    ...
    assert!(
        matches!(
            ...
            [Effect::RenameSession { kind, title, .. }]
                if *kind == hystersis_shell::session::unified_list::SessionKind::Build
                    && title == "local title"
        ),
