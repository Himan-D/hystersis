...
fn test_app() -> AppView {
    ...
    AppView {
        ...
        auth_methods: vec![acp::AuthMethod::Agent(acp::AuthMethodAgent::new(
            acp::AuthMethodId::new("hystersis.com"),
            "Hystersis".to_string(),
        ))],
...
fn cta_entry(name: &str, status: &str) -> hystersis_hooks_plugins_types::MarketplacePluginEntry {
...
/// Plant a Build session under the process `hystersis_home()` (OnceLock-cached;
/// do not rely on setting `HYSTERSIS_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = hystersis_shell::util::hystersis_home::hystersis_home();
    let encoded = hystersis_shell::util::hystersis_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
/// `hystersis.com/ask_user_question` ext-method request. Returns the args
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionMode::Default,
...
fn enqueue_permission_with_enable_always_approve(
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    hystersis_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
    ...
    agent.permission_queue.push_back(PermissionViewState {
        request: hystersis_acp_lib::AcpArgs {
            request,
            response_tx,
        },
        id: 1,
...
const POLICY_WARNING: &str =
    hystersis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
...
fn push_synthetic_permission(
    ...
    let request = hystersis_acp_lib::AcpArgs {
