...
pub(crate) fn log_trace_consent_selected(reenables_sharing: bool, choice: FeedbackTraceChoice) {
    use hystersis_telemetry::events::{FeedbackTraceConsentChoice, FeedbackTraceConsentSelected};
    hystersis_telemetry::session_ctx::log_event(FeedbackTraceConsentSelected {
        choice: match choice {
        ...
        Some(FeedbackTraceChoice::AlwaysUpload) => {
            ...
            if app.coding_data_retention_opt_out {
                let (sharing, outcome) = super::status::set_coding_data_sharing_tracked(
                    app,
                    true,
                    hystersis_telemetry::events::CodingDataConsentSource::FeedbackTraceCard,
                );
...
fn encode_feedback_images(
    images: crate::views::prompt_widget::FeedbackImages,
) -> (Vec<hystersis_shell::session::FeedbackImage>, Option<String>) {
    use base64::Engine as _;
    use hystersis_shell::session::{
    ...
    for image in images.as_slice() {
        ...
        encoded.push(hystersis_shell::session::FeedbackImage {
