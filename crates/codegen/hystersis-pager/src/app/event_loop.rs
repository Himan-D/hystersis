...
use hystersis_acp_lib::{AcpClientMessage, acp_send};
...
/// [`decide`](hystersis_workspace::folder_trust::decide) precedence.
///
...
pub(crate) fn seed_consent_state_from_gate(
    app: &mut AppView,
    gate: Option<&hystersis_shell::util::config::ConsentGate>,
) {
    use crate::app::consent::{ConsentInputs, consent_verdict};
    let stored = hystersis_shell::config::load_from_disk()
        .ok()
        .map(|root| hystersis_shell::util::config::load_config_from_toml(&root).consent)
        .unwrap_or_default();
...
pub(crate) async fn run(
    ...
    pending_startup: hystersis_telemetry::startup::PendingStartup,
    ...
    hystersis_telemetry::startup::enter(hystersis_telemetry::startup::StartupPhase::AppInit);
    ...
    let launch_effective_config = hystersis_shell::config::load_effective_config().ok();
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(hystersis_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = hystersis_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match hystersis_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.privacy_notice_rollout = hystersis_config::env_bool("HYSTERSIS_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
    ...
    app.privacy_banner_acked = hystersis_shell::config::load_from_disk()
        .ok()
        .and_then(|root| {
            hystersis_shell::util::config::load_config_from_toml(&root)
                .privacy
                .privacy_banner_acked
        });
    app.plugin_cta_enabled = hystersis_config::env_bool("HYSTERSIS_PLUGIN_CTA")
    ...
    app.workspace_dashboard_enabled = hystersis_config::env_bool("HYSTERSIS_WORKSPACE_DASHBOARD")
        .or_else(|| {
            remote_settings
                .as_ref()
    ...
    app.session_picker_grouped = std::env::var("HYSTERSIS_SESSION_PICKER_GROUPED")
        ...
        .or_else(|| {
            hystersis_shell::config::load_effective_config()
                .ok()
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    hystersis_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<hystersis_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == hystersis_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = hystersis_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = hystersis_shell::config::load_merged_requirements();
    let user_config = hystersis_shell::config::load_from_disk().ok();
    let managed_config = hystersis_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match hystersis_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = hystersis_shell::agent::config::resolve_compat_sessions_from_raw(
        effective_config.as_ref().ok_or(()),
        remote_settings.as_ref(),
    );
    app.foreign_session_compat = hystersis_foreign_sessions::EnabledForeignSessionSources {
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            let endpoints_base =
                hystersis_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .hystersis_api_base_url;
            app.voice_config =
                hystersis_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    ...
    app.zdr_access_enabled = hystersis_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        hystersis_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        hystersis_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        hystersis_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    app.scheduler_background_loops_seed =
        hystersis_shell::util::config::resolve_scheduler_background_loops(
            remote_settings
                .as_ref()
...

    {
        ...
        app.active_announcements = hystersis_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let hystersis_home = hystersis_tools::util::hystersis_home::hystersis_home();
            app.tip = hystersis_shell::util::tips::pick_and_advance(&app.tips, &hystersis_home);
        }
    ...
    let hints = hystersis_shell::util::config::resolve_hints(
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        hystersis_shell::util::config::clamped_display_permission_mode(
            hystersis_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        hystersis_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = hystersis_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = hystersis_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<hystersis_voice::VoiceEvent>>;
    ...
    if let Some(action) = startup_action {
        ...
        app.finish_startup(hystersis_telemetry::startup::StartupOutcome::Ok);
    }
    ...
    loop {
        ...
        if let Err(e) = run_pending_suspends(
            ...
            app.finish_startup(hystersis_telemetry::startup::StartupOutcome::Error);
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(hystersis_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
...
pub(crate) fn load_initial_ui_config() -> hystersis_shell::agent::config::UiConfig {
    use hystersis_shell::agent::config::UiConfig;
    static INITIAL_UI: std::sync::OnceLock<UiConfig> = std::sync::OnceLock::new();

    INITIAL_UI
        .get_or_init(|| {
            let Ok(root) = hystersis_shell::config::load_effective_config() else {
...
fn emit_event_loop_stall(window: super::event_loop_stall::StallWindow) {
    hystersis_telemetry::session_ctx::log_event(super::event_loop_stall::event_loop_stall_event(
        window,
    ));
}
...
async fn drain_and_process(
    ...
    let mut handle_one = |routed: &RoutedInputEvent| -> bool {
        ...
        if let Event::Key(ke) = ev
            ...
            && hystersis_voice::AUDIO_SUPPORTED
            && is_voice_chord(ke)
...
mod tests {
    ...
    async fn pending_create_uses_auto_selected_before_effect_execution() {
        ...
        let request = match acp_rx.recv().await.expect("session/new request") {
            hystersis_acp_lib::AcpAgentMessage::NewSession(args) => args.request,
    ...
    async fn welcome_paste_preserves_create_and_forwarded_prompt() {
        ...
        assert!(matches!(
            ...
            hystersis_acp_lib::AcpAgentMessage::NewSession(_)
        ));
        assert_eq!(
    ...
    fn plugin_cta_marketplace_from_managed_layer() {
        let layers = hystersis_config::ConfigLayers {
    ...
    fn plugin_cta_marketplace_from_unset_or_empty_is_none() {
        let unset = hystersis_config::ConfigLayers::default();
        assert_eq!(
            plugin_cta_marketplace_from(&unset.effective_config_base()),
            None
        );
        let empty = hystersis_config::ConfigLayers {
        ...
        let blank = hystersis_config::ConfigLayers {
            user: toml::from_str("[marketplace]\nplugin_cta_marketplace = \"   \"\n").unwrap(),
            ..Default::default()
        };
        assert_eq!(
