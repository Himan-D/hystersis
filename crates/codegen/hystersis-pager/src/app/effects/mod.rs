...
use hystersis_telemetry::startup::{self, StartupPhase};
use actions::{
...
use hystersis_shell::sampling::error::http_status_from_error;
use hystersis_shell::session::{ExtMethodResult, SessionInfoResponse};
...
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = hystersis_active_sessions::register(hystersis_active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let tx = acp_tx.clone();
            let compat = hystersis_tools::types::compat::CompatConfig::default();
            let mcp_servers = hystersis_shell::util::config::load_mcp_servers(
                &session_cwd,
                &compat,
            );
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = hystersis_shell::util::config::worktree_type();
                        ...
                        let ext_req = acp::ExtRequest::new(
                            "hystersis.com/git/worktree/resume_session",
                    ...
                    let ext_req = acp::ExtRequest::new(
                        "hystersis.com/git/worktree/create_from_worktree_sync",
        ...
        Effect::LoadSession { agent_id, session_id, session_cwd, chat_kind } => {
            ...
            if let Some(rc) = session_flags.restore_code {
                meta.get_or_insert_with(acp::Meta::new)
                    .insert("hystersis.com/restore_code".into(), serde_json::Value::Bool(rc));
            }
        ...
        Effect::ScanForeignSessions { cwd, compat, hystersis_home, coordinator, seq } => {
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            hystersis_foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &hystersis_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || hystersis_foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
            ...
            tasks
                .spawn(async move {
                    ...
                    if let Some(kinds) = &kind_filter {
                        ...
                        "hystersis.com/facetFilters": { "kind": kinds },
                    });
                        tracing::info!(
                        target: "hystersis.pager.workspace_mode",
                    ...
                    match acp_send(request, &tx).await {
                        Ok(resp) => {
                            ...
                            match parsed {
                                ...
                                None => {
                                    tracing::warn!("failed to parse hystersis.com/sessions/list response");
        ...
        Effect::FetchDashboardSessions => {
            ...
            tasks
                .spawn(async move {
                    ...
                    "cwd": cwd.to_string_lossy(),
                    "limit": 30,
                    "headless": hystersis_shell::session::unified_list::HeadlessPolicy::Exclude
                        .as_wire_str(),
                });
                    let request = acp::ExtRequest::new(
                        "hystersis.com/session/list",
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            ...
            use hystersis_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = hystersis_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = hystersis_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = hystersis_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        hystersis_shell::session::restore::ProgressCallback,
                    > = {
                        use hystersis_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
                    ...
                    match restore_session_with_storage(
                            ...
                            &session_id,
                            &cwd_str,
                            hystersis_shell::session::restore::RestoreSessionOpts {
                                turn_override: None,
                                progress,
                                restore_code: true,
                            },
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            ...
                            let history_path = hystersis_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use hystersis_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
            ...
            tasks
                .spawn(async move {
                    ...
                    let notification = acp::ExtNotification::new(
                        "hystersis.com/toggle_plan_mode",
                        serde_json::value::to_raw_value(&params)
            ...
            tasks
                .spawn(async move {
                    ...
                    let req = acp::ExtRequest::new(
                        "hystersis.com/compact_conversation",
                        serde_json::value::to_raw_value(&params)
        ...
        Effect::KillBgTask { session_id, task_id, source } => {
            ...
            tasks
                .spawn(async move {
                    let params = hystersis_shell::extensions::task::KillTaskRequest {
                        session_id: sid.clone(),
                        task_id: task_id.clone(),
                        source,
                    };
                    let req = acp::ExtRequest::new(
                        "hystersis.com/task/kill",
                        serde_json::value::to_raw_value(&params)
        ...
        Effect::DeleteScheduledTask { session_id, task_id } => {
            ...
            tasks
                .spawn(async move {
                    ...
                    let req = acp::ExtRequest::new(
                        "hystersis.com/scheduler/delete",
                        serde_json::value::to_raw_value(&params)
        ...
        Effect::DemoteToBackground { session_id, tool_call_id } => {
            ...
            tasks
                .spawn(async move {
                    ...
                    let req = acp::ExtRequest::new(
                        "hystersis.com/terminal/background",
                        serde_json::value::to_raw_value(&params)
        ...
        Effect::SwitchModel {
            ...
            tasks
                .spawn(async move {
                    let meta = effort
                        .map(|eff| {
                            use hystersis_shell::sampling::types::{
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use hystersis_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            hystersis_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            hystersis_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    hystersis_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPrivacyBannerAcked { acked_at } => {
            tasks
                .spawn(async move {
                    if let Err(e) = hystersis_shell::util::config::set_privacy_banner_acked(
                            acked_at,
                        )
                        .await
                    {
        ...
        Effect::PersistConsentAnswer { account, notice_id, version, acked } => {
            tasks
                .spawn(async move {
                    match hystersis_shell::util::config::set_consent_answer(
                            account,
                            notice_id,
                            version,
                            acked,
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = hystersis_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
        ...
        Effect::SubmitAuthCode { request_seq, code } => {
            ...
            tasks
                .spawn(async move {
                    ...
                    let req = acp::ExtRequest::new(
                        "hystersis.com/auth/submit_code",
                        serde_json::value::to_raw_value(&params)
        ...
        Effect::McpAuthTrigger { agent_id, session_id, server_name } => {
            ...
            tasks
                .spawn(async move {
                    ...
                    let req = acp::ExtRequest::new(
                        "hystersis.com/mcp/auth_trigger",
                        serde_json::value::to_raw_value(&params)
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                hystersis_hooks_plugins_types::HooksListResponse,
                            >(inner.clone())
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                hystersis_hooks_plugins_types::PluginsListResponse,
                            >(inner.clone())
        ...
        Effect::HooksAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = hystersis_hooks_plugins_types::HooksActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                        "hystersis.com/hooks/action",
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                hystersis_hooks_plugins_types::ActionOutcome,
                            >(inner.clone())
        ...
        Effect::PluginsAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = hystersis_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
                        "hystersis.com/plugins/action",
                        serde_json::value::to_raw_value(&req_body)
        ...
        Effect::FetchMarketplaceList { agent_id, session_id } => {
            ...
            tasks
                .spawn(async move {
                    ...
                    let req = acp::ExtRequest::new(
                        "hystersis.com/marketplace/list",
                        serde_json::value::to_raw_value(&params)
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                hystersis_hooks_plugins_types::MarketplaceListResponse,
                            >(inner.clone())
            ...
            tasks
                .spawn(async move {
                    ...
                    for (name, old, new, source_url, rel_path) in outdated {
                        let action = hystersis_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path: source_url.clone(),
                            plugin_relative_path: rel_path.clone(),
                        };
                        let req_body = hystersis_hooks_plugins_types::MarketplaceActionRequest {
                            session_id: session_id.0.to_string(),
                            action,
                        };
                        let update_req = acp::ExtRequest::new(
                            "hystersis.com/marketplace/action",
                            serde_json::value::to_raw_value(&req_body)
                    ...
                    if !succeeded.is_empty() {
                        ...
                        let notify_req = acp::ExtRequest::new(
                            "hystersis.com/plugins/notify-updates",
            ...
            tasks
                .spawn(async move {
                    let plugin_name = plugin_relative_path
                        ...
                        .to_string();
                    let action = hystersis_hooks_plugins_types::MarketplaceAction::Install {
                        source_url_or_path,
                        plugin_relative_path,
                    };
                    let req_body = hystersis_hooks_plugins_types::MarketplaceActionRequest {
        ...
        Effect::ReloadPluginsForCta { agent_id, session_id, plugin_name } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = hystersis_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action: hystersis_hooks_plugins_types::PluginsAction::Reload,
                    };
                    let req = acp::ExtRequest::new(
                        "hystersis.com/plugins/action",
                        serde_json::value::to_raw_value(&req_body)
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: hystersis_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use hystersis_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
                        "hystersis.com/share_session",
                        serde_json::value::to_raw_value(
                                &ShareSessionRequest {
                                    session_id: session_id.0.to_string(),
                                },
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model, nonce } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = hystersis_shell::agent::auth_method::has_hystersis_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text, images } => {
            use hystersis_shell::session::ClientType;
            use hystersis_shell::session::acp_types::ClientFeedbackInput;
        ...
        Effect::UploadFeedbackTrace { agent_id, session_id } => {
            ...
            tasks
                .spawn(async move {
                    ...
                    let request = acp::ExtRequest::new(
                        "hystersis.com/feedback/upload-trace",
                        raw_params.into(),
                    );
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = hystersis_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    hystersis_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
        ...
        Effect::FetchCatalogEntry { kind, name } => {
            ...
            tasks
                .spawn(async move {
                    ...
                    let request = acp::ExtRequest::new(
                        "hystersis.com/bundle/entry/get",
                        serde_json::value::to_raw_value(&params)
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            hystersis_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionMetaFromDisk {
            ...
            tasks
                .spawn(async move {
                    ...
                    let path = hystersis_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    ...
                    let (title, last_turn_summary) = tokio::task::spawn_blocking(move || -> Option<
                        ...
                        > {
                            ...
                            let summary: hystersis_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent, nonce } => {
            ...
            tasks
                .spawn(async move {
                    use hystersis_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
                        "hystersis.com/billing",
                        serde_json::value::to_raw_value(&serde_json::json!({}))
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !hystersis_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let hystersis_home = hystersis_shell::util::hystersis_home::hystersis_home();
                            let store = hystersis_shell::auth::read_auth_json(
                                    &hystersis_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = hystersis_shell::auth::HystersisComConfig::default()
                                .auth_scope();
                            let auth = hystersis_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    hystersis_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            hystersis_shell::remote::fetch_settings_blocking(
                                    &proxy_base,
                                    &auth,
                                    None,
                                )
...
/// Fetch session info from ACP via `hystersis.com/session/info`.
async fn fetch_session_info(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<SessionInfoResponse, String> {
    let request = acp::ExtRequest::new(
        "hystersis.com/session/info",
        serde_json::value::to_raw_value(
                &serde_json::json!({
            "sessionId": session_id.0.to_string()
        }),
...
/// `hystersis.com/session/usage` → [`PromptUsage`] (bare response, no envelope).
async fn fetch_session_usage(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<hystersis_shell::extensions::notification::PromptUsage, String> {
    let request = acp::ExtRequest::new(
    ...
    let parsed: hystersis_shell::extensions::usage::SessionUsageResponse = serde_json::from_str(
            resp.0.get(),
        )
        .map_err(|e| {
...
/// Shared `hystersis.com/session/rename` RPC for rename and `/rename --auto`.
...
/// (`cwd` from the `hystersis.com/session/info` response), never the all-sessions list.
...
async fn lookup_session_title_in(
    ...
    use hystersis_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
    let info = hystersis_shell::session::info::Info {
        id: session_id.clone(),
        cwd: cwd.to_string(),
    };
...
fn session_info_fields(
    ...
    push(
        "Shell version",
        hystersis_version::display_version(hystersis_update::channel_label()),
        false,
    );
    ...
    let model_display = hystersis_shell::session::model_display_name(
    ...
    if hystersis_shell::session::should_show_model_fingerprint(
    ...
    if let Some(profile) = hystersis_sandbox::profile_name() {
...
fn format_auth_lines(is_api_key_auth: bool, api_key_env_set: bool) -> String {
    if is_api_key_auth {
        ...
        return format!(
            "{method}  Run `hystersis login` to use your HystersisPro subscription instead.\n"
        );
    }
...
/// Build the `hystersis.com/interject` params. The optional structured `content`
