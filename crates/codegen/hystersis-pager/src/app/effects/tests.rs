...
use super::*;
use hystersis_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn format_acp_error_rate_limit_surfaces_detail_or_fallback() {
    ...
    let rpm_body = "You are sending requests too quickly. Please slow down, or upgrade to a Hystersis subscription for higher limits: https://hystersis.com/upgrade";
    let rpm = acp::Error::new(RATE_LIMITED_ERROR_CODE, "Rate limited")
        .data(format!("API error (status 429 Too Many Requests): {rpm_body}"));
    assert!(format_acp_error(&rpm, false).contains("hystersis.com/upgrade"));
...
/// legacy `hystersis.com/interject` wire shape stays byte-identical.
#[test]
...
fn picker_keeps_conversation_with_empty_cwd_and_missing_updated_at() {
    let payload = serde_json::json!({
            "sessions": [{
                ...
                "source": "conversation",
                "_meta": { "hystersis.com/session": { "kind": "chat" } }
            }]
        });
...
/// Untitled hystersis.com chats must stay listed, rendered as "Untitled".
#[test]
...
fn session_list_partial_parses_reasons() {
    let payload = |reason: &str| {
        serde_json::json!({
                "sessions": [],
                "_meta": { "hystersis.com/partial": { "conversations": true, "reason": reason } }
            })
    };
    assert_eq!(
...
fn session_list_partial_absent_for_healthy_or_meta_less_responses() {
    let healthy = serde_json::json!({
            "sessions": [],
            "_meta": { "hystersis.com/partial": { "conversations": false } }
        });
...
fn parse_kill_outcome_reads_result_envelope() {
    use hystersis_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use hystersis_workspace::session::git::RestoreDegree;
...
/// Spawn a fake ACP agent that counts `hystersis.com/yolo_mode_changed`
/// notifications. Exits when the channel closes.
fn spawn_fake_acp_agent(
    mut rx: tokio::sync::mpsc::UnboundedReceiver<hystersis_acp_lib::AcpAgentMessage>,
) -> Arc<AtomicUsize> {
    let counter = Arc::new(AtomicUsize::new(0));
    let counter_clone = counter.clone();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let hystersis_acp_lib::AcpAgentMessage::ExtNotification(args) = msg {
                if args.request.method.as_ref() == "hystersis.com/yolo_mode_changed" {
...
/// Redirect `HYSTERSIS_HOME` to a tempdir for test isolation.
fn setup_hystersis_home_in_tempdir() -> tempfile::TempDir {
    let tmp = tempfile::tempdir().expect("tempdir creation");
    unsafe {
        std::env::set_var("HYSTERSIS_HOME", tmp.path());
    }
    tmp
}
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use hystersis_active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            hystersis_active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_hystersis_home_in_tempdir();
    ...
    assert_eq!(
            ...
            1,
            "ACP `hystersis.com/yolo_mode_changed` notification must fire exactly once \
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use hystersis_acp_lib::AcpAgentMessage;
    use hystersis_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
            ...
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "hystersis.com/marketplace/action" => {
                        ...
                        let req: hystersis_hooks_plugins_types::MarketplaceActionRequest = serde_json::from_str(
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: hystersis_foreign_sessions::EnabledForeignSessionSources::default(),
            hystersis_home: PathBuf::from("/path/that/must/not/be-read"),
...
/// browse opts into `allowRelax` and parses `hystersis.com/listScope`; all
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "hystersis.com/session/list");
                ...
                let body = if fail {
                    ...
                    serde_json::json!({
                            "result": {
                                "sessions": [],
                                "_meta": { "hystersis.com/listScope": "repo" },
                            }
                        })
                } else {
    ...
    match tasks.join_next().await.expect("task").expect("no panic") {
        TaskResult::SessionListLoaded { scope, seq, query, .. } => {
            ...
            assert!(
                    scope.is_relaxed(),
                    "_meta[\"hystersis.com/listScope\"] must parse into the task result"
                );
        }
        other => panic!("expected SessionListLoaded, got {other:?}"),
    }
...
async fn fetch_session_list_sends_kind_facet_filter() {
    ...
    assert_eq!(
            captured[0]["_meta"]["hystersis.com/facetFilters"]["kind"],
...
async fn fetch_workflows_list_sends_session_id() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "hystersis.com/workflows/list");
                let params: serde_json::Value = serde_json::from_str(
                        args.request.params.get(),
                    )
                    .expect("params JSON");
...
async fn deep_search_sessions_echoes_routing_and_policy() {
    ...
    execute(
        Effect::DeepSearchSessions {
            ...
            query: "abc".into(),
            seq: 4,
            headless_policy: hystersis_shell::session::unified_list::HeadlessPolicy::Only,
        },
        &mut tasks,
        &tx,
        Path::new("."),
    ...
    let captured = captured.lock().unwrap();
    assert_eq!(captured.len(), 1);
    assert_eq!(captured[0].0, "hystersis.com/session/search");
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use hystersis_agent::config::BuiltinAgentName;
...
/// Default flags produce no agent profile (uses hystersis default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (hystersis already has TaskTool).
#[test]
...
/// Neutralize `HYSTERSIS_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(HYSTERSIS_AGENT)]` (process-global env).
fn without_hystersis_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("HYSTERSIS_AGENT", "")
}
...
#[serial_test::serial(HYSTERSIS_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_hystersis_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "hystersis-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "hystersis-plan-no-subagents");
...
/// --ask-user alone selects the hystersis-ask-user profile.
#[serial_test::serial(HYSTERSIS_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "hystersis-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `hystersis` already has it).
#[test]
...
/// All three flags on at the runtime default produce hystersis-plan
...
fn to_meta_chat_mode_stamps_kind_and_omits_agent_profile() {
    ...
    assert_eq!(meta["hystersis.com/session"]["kind"], "chat");
    assert!(
...
fn assert_chat_meta_has_no_workspace_bind_keys(meta: &serde_json::Value) {
    ...
    assert!(
            meta.get("hystersis.com/cloud_existing_workspace").is_none(),
...
fn chat_load_meta_never_includes_workspace_bind_keys() {
    ...
    apply_chat_kind_meta(&mut meta);
    {
        ...
        obj.insert("hystersis.com/cloud_server_id".into(), serde_json::json!("srv-poison"));
        obj.insert(
...
fn scrub_chat_workspace_matrix_attach_exception() {
    ...
    let mut meta = Some(acp::Meta::new());
    {
        ...
        obj.insert("hystersis.com/cloud_server_id".into(), serde_json::json!("hub-x"));
        obj.insert(
            "hystersis.com/cloud_existing_workspace".into(),
    ...
    assert!(scrubbed.get("hystersis.com/cloud_server_id").is_none());
    assert!(scrubbed.get("hystersis.com/cloud_existing_workspace").is_none());
    let mut meta = Some(acp::Meta::new());
    ...
    );
    {
        ...
        obj.insert("hystersis.com/cloud_server_id".into(), serde_json::json!("hub-must-go"));
    }
    scrub_chat_workspace_bind_meta(&mut meta);
    let scrubbed = meta.as_ref().unwrap();
    assert!(scrubbed.get("envId").is_none(), "envId must stay scrubbed");
    assert!(
            scrubbed.get("hystersis.com/cloud_server_id").is_none(),
            "Direct hub must stay scrubbed"
        );
    assert_eq!(
            scrubbed["hystersis.com/cloud_existing_workspace"]["server_id"],
            "srv-dogfood"
        );
    assert_eq!(scrubbed["hystersis.com/local_workspace"]["mode"], "attach");
    assert_eq!(scrubbed["hystersis.com/local_workspace"]["server_id"], "srv-dogfood");
    assert_eq!(scrubbed["hystersis.com/local_workspace"]["cwd"], "/tmp/repo");
}
#[cfg(feature = "local-workspace")]
#[test]
fn to_meta_chat_attach_stamps_local_and_existing() {
    ...
    assert_eq!(meta["hystersis.com/session"]["kind"], "chat");
    assert_eq!(meta["hystersis.com/local_workspace"]["mode"], "attach");
    assert_eq!(meta["hystersis.com/cloud_existing_workspace"]["server_id"], "srv-1");
    assert!(meta.get("envId").is_none());
    assert!(meta.get("hystersis.com/cloud_server_id").is_none());
}
#[cfg(feature = "local-workspace")]
#[test]
fn to_meta_chat_own_stamps_intent_without_existing() {
    ...
    let meta = flags.to_meta().expect("meta");
    assert_eq!(meta["hystersis.com/local_workspace"]["mode"], "own");
    assert_eq!(meta["hystersis.com/local_workspace"]["cwd"], "/tmp/repo-own");
    assert!(meta["hystersis.com/local_workspace"].get("server_id").is_none());
    assert!(
            meta.get("hystersis.com/cloud_existing_workspace").is_none(),
...
fn mid_session_add_params_scrub_envid() {
    ...
    assert_eq!(params["meta"]["hystersis.com/local_workspace"]["mode"], "attach");
    assert_eq!(
            params["meta"]["hystersis.com/cloud_existing_workspace"]["server_id"],
            "srv-add"
        );
...
fn finalize_chat_session_meta_stamps_attach_on_worktree_path() {
    ...
    assert_eq!(meta["hystersis.com/session"]["kind"], "chat");
    assert_eq!(meta["hystersis.com/local_workspace"]["mode"], "attach");
    assert_eq!(meta["hystersis.com/cloud_existing_workspace"]["server_id"], "srv-wt");
    assert!(meta.get("envId").is_none());
}
#[test]
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> hystersis_shell::session::SessionInfoResponse {
    use hystersis_shell::session::acp_types::{ContextInfo, SessionInfoData};
    hystersis_shell::session::SessionInfoResponse {
...
fn format_session_info_session_auth_ignores_api_key_env() {
    ...
    assert!(!text.contains("hystersis.com"), "{text}");
    assert!(!text.contains("hystersis login"), "{text}");
}
#[test]
fn format_session_info_api_key_without_env() {
    ...
    assert!(!text.contains("Manage account and credits"), "{text}");
    assert!(
            text.contains("Run `hystersis login` to use your HystersisPro subscription instead."),
            "{text}"
        );
    assert!(!text.contains("hystersis.com"), "{text}");
}
#[test]
fn format_session_info_api_key_auth_suggests_hystersis_login() {
    let info = make_session_info("auto", None, 1000, 10000);
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("hystersis-4.5", Some("hystersis-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: hystersis-4.5 (hystersis-4.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("hystersis-4.5", Some("hystersis-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model: hystersis-4.5"));
    assert!(!text.contains("hystersis-4.3"));
}
...
async fn lookup_session_title_loads_single_summary_by_cwd() {
    ...
    let dir = root
        .path()
        .join("sessions")
        .join(hystersis_shell::util::hystersis_home::encode_cwd_dirname(cwd))
        .join("sess-1");
...
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("hystersis-4.5", Some("hystersis-4.5"), 1000, 10000);
    let text = format_session_info(&info, None, true, false, false);
    assert!(text.contains("Model: hystersis-4.5"));
    assert!(!text.contains("(hystersis-4.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("hystersis", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false, false, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    let entry = SessionPickerEntry {
        ...
        model_id: Some("hystersis-4".to_string()),
        num_messages: 3,
    ...
    assert_eq!(roster.model_id.as_deref(), Some("hystersis-4"));
