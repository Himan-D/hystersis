...
use hystersis_shell::extensions::notification::GoalClassifierVerdict;
...
pub enum AgentCommand {
    ...
    /// `hystersis.com/session/fork` request is in flight.
    ForkSession,
}
impl AgentCommand {
...
impl BgTaskState {
    ...
    pub fn tombstone_from_snapshot(
        snapshot: &hystersis_tools::types::TaskSnapshot,
        status: BgTaskStatus,
...
pub struct AgentSession {
    ...
    /// (`hystersis.com/prompt_history` scoped via `filter_session_id`). Most-recent-first.
    /// Fetched on session create/load; prompts sent in this session are
    /// additionally front-inserted locally on send.
    pub prompt_history: Vec<String>,
    /// True until the session's startup/load `hystersis.com/prompt_history` fetch completes.
...
pub struct ChipElement {
    ...
    pub kind: hystersis_ratatui_textarea::ElementKind,
...
impl AgentSession {
    ...
    /// [`hystersis_prompt_queue::combine_prefix_len`]. `editing_id` is held out of the
    ...
    pub fn dequeue_combined_prompt(&mut self, editing_id: Option<u64>) -> Option<QueuedPrompt> {
        use hystersis_prompt_queue::{CombineGate, TEXT_SEPARATOR, combine_prefix_len, join_texts};
