...
impl McpInitProgress {
    ...
    ///   `hystersis.com/mcp_initialized` clears the progress.
...
pub struct BannerSlotParams<'a> {
    ...
    pub(crate) announcements: &'a [hystersis_announcements::RemoteAnnouncement],
...
pub(crate) struct DeferredSubagentFinish {
    pub notification: hystersis_shell::extensions::notification::SessionNotification,
...
pub(crate) struct SessionReload {
    ...
    last_applied_hystersis_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    pub candidates: Vec<hystersis_hooks_plugins_types::MarketplacePluginEntry>,
...
/// (`hystersis.com/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    /// ([`Self::last_applied_hystersis_event_seq`]) because the two streams are not
    ...
    /// deduped.
    pub last_applied_hystersis_event_seq: Option<u64>,
    ...
    pub context_state: Option<hystersis_shell::session::ContextInfo>,
    pub status_context: Option<hystersis_status_line::StatusLineContext>,
    ...
    /// (optimistic echo). The shell broadcasts `hystersis.com/session/interjection` to
    ...
    pub(crate) pending_elicitation: Option<(
        hystersis_tools::mcp_elicitation::McpElicitExtRequest,
        tokio::sync::oneshot::Sender<hystersis_acp_lib::AcpResult<agent_client_protocol::ExtResponse>>,
    )>,
    ...
    pub(crate) deferred_session_mode: Option<hystersis_tools::types::SessionMode>,
    ...
    /// MCP servers, cleared when `hystersis.com/mcp_initialized` arrives.
    ...
    /// Running agent definition for this session (`hystersis.com/session/info` `agentName`).
    ...
    /// refreshed by `hystersis.com/settings/update`: the fire side is pinned for the
    ...
    /// Armed when an `hystersis.com/session/prompt_complete` broadcast arrives for the
    ...
    /// `hystersis.com/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `hystersis.com/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `hystersis.com/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`hystersis.com/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `hystersis.com/follow_ups` that arrived for a turn that is NOT yet the
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(hystersis_telemetry::events::CreditLimitChoice::PayAsYouGo);
            hystersis_telemetry::session_ctx::log_event(
                hystersis_telemetry::events::CreditLimitUpsellClicked {
                    surface: hystersis_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
                    choice,
                },
            );
        ...
        LocalQuestionKind::FreeUsageUpsell { source } => {
            ...
            hystersis_telemetry::session_ctx::log_event(
                hystersis_telemetry::events::HystersisProUpsellClicked {
                    source,
                    auth_method: None,
                },
            );
...
fn remember_mode_enabled() -> bool {
    let path = hystersis_tools::util::hystersis_home::hystersis_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &hystersis_tools::implementations::hystersis_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(crate) mod test_fixtures {
    ...
    -> crate::views::permission_view::PermissionViewState {
        ...
        let perm = hystersis_acp_lib::AcpArgs {
            request,
            response_tx,
        };
        crate::views::permission_view::PermissionViewState {
    ...
    /// FIX 4 (b): after adopting a NEW turn, a buffer-replayed `hystersis.com/follow_ups`
    ...
    /// FINDING B (stamped path): a LATE FIRST-TIME (never-seen) `hystersis.com/follow_ups`
    ...
    /// upstream by the `_meta["hystersis.com/replayed"]` gate); it is pinned here so the
    ...
    /// FIX (buffer-before-adoption): a stamped `hystersis.com/follow_ups` for a turn that
