...
impl AgentView {
    ...
    pub(crate) fn log_prompt_suggestion_shown_if_visible(&mut self) {
        ...
        hystersis_telemetry::session_ctx::log_event(hystersis_telemetry::events::PromptSuggestion {
            action: hystersis_telemetry::events::PromptSuggestionAction::Shown,
            chars,
            words,
        });
    }
    ...
    /// Apply an `hystersis.com/follow_ups` notification, keyed by `response_id`
    ...
    /// on each `hystersis.com/follow_ups` notification (the same `promptId` it stamps on
    ...
    /// - A buffer-replayed `hystersis.com/follow_ups` for a PRIOR turn's `response_id`
    ...
    /// Buffer a stamped `hystersis.com/follow_ups` for a turn that is not yet current,
    ...
    /// Flush a buffered `hystersis.com/follow_ups` for `prompt_id` (a turn that has just
    ...
    /// the running turn's `hystersis.com/follow_ups` arrive on the ext channel DURING
    ...
    pub(in crate::app) fn connect_matched_plugin(&mut self) {
        ...
        hystersis_telemetry::session_ctx::log_event(
            hystersis_telemetry::events::PluginCtaConnectClicked {
...
mod prompt_suggestion_gate_tests {
    ...
    fn an_open_question_card_closes_the_suggestion_gate() {
        ...
        use hystersis_tools::implementations::hystersis_build::ask_user_question::Question;
...
mod plugin_cta_notify_tests {
    ...
    fn cta_entry(name: &str) -> hystersis_hooks_plugins_types::MarketplacePluginEntry {
