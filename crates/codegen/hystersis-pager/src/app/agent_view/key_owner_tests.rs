...
use std::sync::Arc;
use hystersis_tools::implementations::hystersis_build::ask_user_question::{Question, QuestionOption};
...
fn open_elicitation(agent: &mut AgentView) {
    ...
    use hystersis_tools::mcp_elicitation::{McpElicitExtRequest, McpElicitModeFields};
...
fn elicitation_draft_stops_at_named_cap() {
    use hystersis_tools::mcp_elicitation::MAX_ELICIT_DRAFT_CHARS;
    let mut agent = make_agent();
    open_elicitation(&mut agent);
