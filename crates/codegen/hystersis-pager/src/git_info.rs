...
///   - [`update_from_notification`] — the `hystersis.com/git_head_changed` ACP
...
pub struct CwdGitInfo {
    ...
    /// (linked `git worktree`, hystersis standalone clone, or worktree DB hit).
...
fn compute_snapshot(cwd: &Path) -> GitSnapshot {
    ...
    // Standalone hystersis worktrees are CoW clones (`.git` is a directory), so
    // `path != commondir` is false; the source marker is the back-pointer.
    let mut marker_main_repo = None;
    for ancestor in cwd.ancestors() {
        let git = ancestor.join(".git");
        if let Ok(contents) = std::fs::read_to_string(git.join("hystersis-worktree-source"))
...
pub fn worktree_label_index() -> std::collections::HashMap<PathBuf, String> {
    ...
    let Ok(db) = hystersis_fast_worktree::db::WorktreeDb::open_default() else {
        return map;
    };
...
///
/// `HYSTERSIS_NERD_FONTS=1` forces Powerline; `HYSTERSIS_NERD_FONTS=0` forces the fallback.
pub(crate) fn branch_icon() -> &'static str {
    static ICON: OnceLock<&str> = OnceLock::new();
    ICON.get_or_init(|| {
        decide_branch_icon(
            std::env::var("HYSTERSIS_NERD_FONTS").ok().as_deref(),
            HostOs::current(),
...
///
/// An explicit `HYSTERSIS_NERD_FONTS` override (`0`/`false` → off, anything else →
...
mod tests {
    ...
    #[test]
    fn compute_cwd_git_info_standalone_hystersis_worktree() {
        ...
        assert!(
            clone
                .path
                .join(".git")
                .join("hystersis-worktree-source")
                .is_file(),
    ...
    fn compute_cwd_git_info_nested_repo_does_not_inherit_db_record() {
        ...
        // serial(HYSTERSIS_HOME) orders peers; EnvVarGuard restores on drop so
        // later `open_default()` callers do not see a deleted temp home.
        let _hystersis_home = crate::test_util::EnvVarGuard::set("HYSTERSIS_HOME", home.path());
        let _ = hystersis_fast_worktree::db::WorktreeDb::open(home.path());

        let wt = crate::test_util::TempGitRepo::init("wt-branch");
        let canon_wt = dunce::canonicalize(&wt.path).unwrap_or_else(|_| wt.path.clone());
        let db = hystersis_fast_worktree::db::WorktreeDb::open(home.path()).unwrap();
        db.register(&hystersis_fast_worktree::db::WorktreeRecord {
            id: "db-wt".into(),
            path: canon_wt,
            source_repo: PathBuf::from("/src/main-repo"),
            repo_name: "main-repo".into(),
            kind: hystersis_fast_worktree::db::WorktreeKind::Session,
            ...
            status: hystersis_fast_worktree::db::WorktreeStatus::Alive,
