...
use hystersis_acp_lib::{AcpAgentTx, AcpClientMessageBox, AcpClientRx, acp_send};
use hystersis_shell::agent::auth_method::AuthMethodKind;
use hystersis_shell::agent::config::Config as AgentConfig;
use hystersis_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use hystersis_shell::util::config as cli_config;
use hystersis_telemetry::startup::PendingStartup;
...
pub(crate) fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    hystersis_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn mcp_server_names(cwd: &Path) -> Vec<McpServer> {
    let servers =
        cli_config::load_mcp_servers(cwd, &hystersis_tools::types::compat::CompatConfig::default());
    servers
        .iter()
        .filter_map(|s| {
            let name = match s {
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !hystersis_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !hystersis_shell::util::clipboard::is_remote_session();
...
pub async fn run_single_turn(
    ...
    hystersis_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = hystersis_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = hystersis_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        hystersis_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    hystersis_telemetry::startup::enter(crate::acp::StartupPhase::AcpInitialize);
    ...
    hystersis_telemetry::startup::enter(crate::acp::StartupPhase::EagerAuth);
    ...
    hystersis_telemetry::startup::enter(crate::acp::StartupPhase::SessionCreate);
    ...
    if track_active {
        let _ = hystersis_active_sessions::register(hystersis_active_sessions::ActiveSession {
    ...
    let session_models = if needs_fresh_catalog {
        match hystersis_shell::cli_models::fetch_model_state(&acp_tx).await {
    ...
    if track_active {
        ...
        let _ = hystersis_active_sessions::try_unregister(&session_id);
    }
    ...
    let outcome: Result<()> = match prompt_result {
        Some(Ok(resp)) => {
            ...
            let is_max_turns = resp
                ...
                .and_then(|v| v.as_str())
                == Some(hystersis_shell::session::commands::MAX_TURNS_REACHED_CATEGORY);
            if is_max_turns {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = hystersis_shell::sampling::error::prompt_usage_from_error(&err) {
            ...
            let stop_reason_override =
                (hystersis_shell::sampling::error::stop_reason_for_turn_error(&err) == "MaxTokens")
    ...
    let (method, params) = match work {
        ...
        BackgroundWork::Task(id) => (
            ...
            serde_json::value::to_raw_value(&KillTaskRequest {
                session_id: session_id.0.to_string(),
                task_id: id.clone(),
                source: hystersis_shell::extensions::task::TaskKillSource::Teardown,
            })?,
        ),
    };
