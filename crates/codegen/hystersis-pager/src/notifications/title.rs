...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str("hystersis");
        }
    ...
    pub fn reset(&mut self) -> String {
        let esc = build_title_escape("hystersis");
        self.last_title.clear();
        self.last_title.push_str("hystersis");
...
/// strings (e.g. hystersis.com conversation titles), which must not terminate the
...
mod tests {
    ...
    #[test]
    fn hystersis_only_produces_just_hystersis() {
        let cfg = config_with_items(vec![TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        let state = idle_state();
        mgr.update(&state);
        assert_eq!(mgr.last_title, "hystersis");
    }

    #[test]
    fn session_name_and_hystersis_joined_with_separator() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "my project - hystersis");
    }
    ...
    fn spinner_only_shown_when_active() {
        let cfg = config_with_items(vec![TitleItem::Spinner, TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        ...
        mgr.update(&state);
        assert!(mgr.last_title.contains(" - hystersis"));
    ...
    fn activity_hidden_when_idle() {
        let cfg = config_with_items(vec![TitleItem::Activity, TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        mgr.update(&idle_state());
        assert_eq!(mgr.last_title, "hystersis");
    }
    ...
    fn activity_shows_waiting_when_busy_without_activity() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Waiting - hystersis");
    }
    ...
    fn activity_prefers_specific_activity_over_waiting() {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "Thinking - hystersis");
    }
    ...
    fn action_required_visible_on_first_tick() {
        let cfg = config_with_items(vec![TitleItem::ActionRequired, TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    #[test]
    fn empty_items_produces_hystersis_fallback() {
    ...
    fn model_item_shown_when_present() {
        let cfg = config_with_items(vec![TitleItem::Model, TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
            model: Some("hystersis-3"),
            ..idle_state()
        };
        mgr.update(&state);
        assert_eq!(mgr.last_title, "hystersis-3 - hystersis");
    }
    ...
    fn cwd_shows_last_component() {
        let cfg = config_with_items(vec![TitleItem::Cwd, TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
    ...
    fn turn_timer_shown_when_above_one_second() {
        let cfg = config_with_items(vec![TitleItem::TurnTimer, TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_eq!(mgr.last_title, "42s - hystersis");
    }
    ...
    #[test]
    fn reset_clears_state_and_emits_hystersis() {
        let cfg = config_with_items(vec![TitleItem::SessionName, TitleItem::Hystersis]);
        let mut mgr = TitleManager::new(&cfg);
        let activity = TurnActivity::Thinking;
        let state = TitleState {
        ...
        mgr.update(&state);
        assert_ne!(mgr.last_title, "hystersis");
    ...
    fn default_config_active_turn_with_permissions() {
        ...
        for t in [&t1, &t2] {
            assert!(t.contains("hystersis"), "title missing 'hystersis': {t}");
    ...
    fn all_items_present_in_order() {
        ...
        assert_eq!(
            mgr.last_title,
            "Thinking - proj - hystersis-3 - workspace - hystersis"
        );
    }
