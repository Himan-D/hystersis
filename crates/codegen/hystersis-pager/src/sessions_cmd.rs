...
use hystersis_shell::auth::{AuthManager, try_ensure_fresh_auth};
use hystersis_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // deployments that only configure a deployment_key + custom hystersis_api_base_url.
    // If the user has previously run the interactive `hystersis` TUI (which succeeds
    ...
    let auth = try_ensure_fresh_auth(&agent_config.hystersis_com_config).await;
    ...
    let client = hystersis_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = hystersis_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                hystersis_shell::session::merge::CwdScope::WithSiblings,
                None,
                limit,
                // The CLI listing is an inventory, not the resume picker.
                hystersis_shell::session::visibility::HeadlessPolicy::Include,
            )
            .await;
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use hystersis_shell::session::merge::REMOTE_TIMEOUT;
            use hystersis_shell::session::storage::search::{
                IndexDecision, SessionSearchRequest, execute_search,
            };

            // The only subcommand that reads the index, so the only one to start one.
            let search = hystersis_shell::session::storage::search::start_if_enabled(agent_config);
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = hystersis_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
