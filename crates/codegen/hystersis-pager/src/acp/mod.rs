...
pub(crate) fn is_session_update_ext_method(method: &str) -> bool {
    matches!(method, "hystersis.com/session_notification" | "hystersis.com/session/update")
}

use hystersis_telemetry::process_info::{
    Entrypoint, Interactivity, LeaderMode, ProcessIdentity, set_identity,
};
use hystersis_telemetry::startup;
pub use hystersis_telemetry::startup::{
...
use hystersis_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
use hystersis_shell::agent::auth_method::AuthMethodKind;
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a hystersis-shell instance.
    pub is_hystersis_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<hystersis_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `hystersis.com`).
    ...
    /// disabled feature produces zero `hystersis.com/recap` traffic. Defaults to `false`
    ...
    pub auth_manager: std::sync::Arc<hystersis_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    /// `hystersis.com/statusLine` so the agent can skip an unpainted payload.
    ...
    pub remote_settings: Option<hystersis_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_hystersis_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    startup::set_auth_mode(hystersis_shell::managed_config::classify_auth_mode());
    let mut agent_config = AgentConfig::new_from_toml_cfg(raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    hystersis_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = hystersis_shell::util::hystersis_home::hystersis_home().join("config.toml");
...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    let mut meta = serde_json::json!({
        "hystersis.com/incrementalBashOutput": true,
        "hystersis.com/hunkTracker": { "mode": hunk_mode },
        "hystersis.com/bashOutputNoColor": true,
        "hystersis.com/gitHeadChanged": true,
    });
    meta[hystersis_status_line::STATUS_LINE_CAPABILITY] = flags.status_line.into();
    meta
}
...
async fn initialize(
    ...
    // Check if this is a hystersis-shell agent
    let is_hystersis_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("hystersisShell"))
...
///
/// Matches TUI startup behavior: if the first method is `hystersis.com`, defer auth
...
/// for a `hystersis.com` or `oidc` method — these are the ones that can trigger
...
mod tests {
    ...
    fn is_session_update_ext_method_covers_both_carriers() {
        assert!(is_session_update_ext_method("hystersis.com/session_notification"));
        assert!(is_session_update_ext_method("hystersis.com/session/update"));
        assert!(!is_session_update_ext_method("hystersis.com/task_completed"));
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "hystersisShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "hystersisShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_hystersis_com_no_provider_needs_login_pending() {
        let methods = vec![make_auth_method("hystersis.com", "hystersis.com", None)];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
        assert_eq!(label.as_deref(), Some("hystersis.com"));
        assert_eq!(method_id.as_ref().unwrap().0.as_ref(), "hystersis.com");
        assert_eq!(mode, AuthStartMode::Pending);
    }

    #[test]
    fn startup_auth_hystersis_com_with_external_provider_command() {
        let meta = serde_json::json!({ "external_provider": true });
        let methods = vec![make_auth_method("hystersis.com", "Acme Corp", Some(meta))];
        let (needs, label, method_id, mode) = startup_auth_metadata(&methods);
        assert!(needs);
    ...
    #[test]
    fn startup_auth_non_hystersis_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.hystersis/config.toml` with a
    ...
    /// (`agent::auth_method::tests::enterprise_byok_first_method_is_hystersis_api_key`
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use hystersis_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};

        let built = build_auth_methods(AuthMethodsBuildInputs {
            // enterprise-style: model has `env_key` set and the env var resolves,
            // so the shell-side predicate returns true.
            has_external_api_key: true,
            // Realistic enterprise user: no cached session token, default `hystersis.com`
    ...
    /// `startup_auth_hystersis_com_no_provider_needs_login_pending` above and
    /// either passes or fails on a meaningful new code path.
    #[test]
    fn startup_auth_hystersis_api_key_not_first_still_requires_login() {
        use hystersis_shell::agent::auth_method::{HYSTERSIS_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};

        let methods = vec![
            make_auth_method(HYSTERSIS_COM_METHOD_ID, "Hystersis", None),
            make_auth_method(XAI_API_KEY_METHOD_ID, "xai.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with hystersis.com first, the pager must require login -- pinning \
    ...
    fn startup_auth_method_id_is_copied_not_synthesized() {
        let methods = vec![make_auth_method("hystersis.com", "My Login", None)];
    ...
    fn startup_auth_external_provider_false_is_pending() {
        ...
        let methods = vec![make_auth_method("hystersis.com", "hystersis.com", Some(meta))];
    ...
    fn client_capabilities_meta_defaults_absent_or_blank_mode_to_off() {
        ...
        assert_eq!(absent["hystersis.com/hunkTracker"]["mode"], "off");
        let blank = client_capabilities_meta(&ConnectFlags {
            hunk_tracker_mode: Some("   ".into()),
            ..Default::default()
        });
        assert_eq!(blank["hystersis.com/hunkTracker"]["mode"], "off");
    }
    ...
    fn client_capabilities_meta_advertises_the_status_line_the_config_asked_for() {
        let key = hystersis_status_line::STATUS_LINE_CAPABILITY;
    ...
    fn client_capabilities_meta_canonicalizes_off_and_mixed_case() {
        ...
        for raw in ["off", "OFF", "Disabled"] {
            ...
            assert_eq!(meta["hystersis.com/hunkTracker"]["mode"], "off", "raw={raw}");
        }
    }
}
