...
//!
//! Simplified to only support HystersisShell (in-process) mode.
...
use hystersis_shell::{
    ...
    auth::AuthManager,
    util::hystersis_home::hystersis_home,
};
...
///
/// Hold one from every site that calls [`spawn_hystersis_shell`] (headless, the TUI,
...
/// Spawn a HystersisShell agent in a background thread.
pub async fn spawn_hystersis_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<hystersis_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    hystersis_shell::agent::app::apply_otel_config(&auth_manager, &agent_config.hystersis_com_config);
    ...
    hystersis_shell::managed_config::ensure_managed_policy_present(&auth_manager).await;
    ...
    let (agent_config, models_manager) =
        hystersis_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
...
async fn spawn_agent_thread_direct(
    ...
    let rt = tokio::task::spawn_blocking(|| {
        ...
        hystersis_tty_utils::runtime::build_with_blocking_pool(builder.enable_all())
    })
    .await
            ...
            local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        hystersis_agent::prompt::workspace_user::optional_workspace_user_dir();
                    hystersis_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    hystersis_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    hystersis_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
