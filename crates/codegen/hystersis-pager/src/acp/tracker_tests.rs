...
fn test_search_tool_call_flow() {
    use hystersis_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
    let mut tracker = AcpUpdateTracker::new();
...
fn activity_writing_tool_call_copy_covers_taxonomy_map() {
    for (name, _) in hystersis_tools::tool_taxonomy::WRITING_TOOL_WIRE_NAMES {
        let mut tracker = AcpUpdateTracker::new();
        tracker.note_tool_call_arguments_delta(Some(name), 0);
        let Some(TurnActivity::WritingToolCall(writing)) = tracker.activity() else {
            panic!("expected WritingToolCall activity for {name:?}");
...
fn video_tool_variants_use_typed_path_not_generic_scrape() {
    ...
    let cases: &[(&str, ToolOutput)] = &[
        (
            "ImageToVideo",
            ToolOutput::ImageToVideo(hystersis_tools::types::output::MediaGenOutput::new(
                video_path.clone(),
            )),
        ),
        (
            "ReferenceToVideo",
            ToolOutput::ReferenceToVideo(hystersis_tools::types::output::MediaGenOutput::new(
                video_path.clone(),
            )),
        ),
    ];
...
fn media_gen_ref_skips_uploaded_only_video() {
    let output = ToolOutput::ImageToVideo(hystersis_tools::types::output::MediaGenOutput::uploaded(
...
fn tier_restricted_media_shows_upsell_text_not_error() {
    ...
    let output = ToolOutput::Text(hystersis_tools::types::output::TextOutput::from(upsell));
    let tc = acp::ToolCall::new(
