//! Decoding of the shell's `hystersis.com/*` extension notifications into the headless
...
use hystersis_acp_lib::{AcpArgsBox, AcpResult};
...
pub(crate) fn reply_headless_ext_method(args: AcpArgsBox<acp::ExtRequest>) {
    use hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionExtResponse;
    ...
    let response = match method {
        ...
        "hystersis.com/ask_user_question" => ext_response_from(&AskUserQuestionExtResponse::Cancelled),
        "hystersis.com/mcp/elicit" => {
            use hystersis_tools::mcp_elicitation::McpElicitExtResponse;
            ext_response_from(&McpElicitExtResponse::Cancel)
        }
        // Model sees "Your plan has been approved. You can now start coding.".
        "hystersis.com/exit_plan_mode" => ext_response_from(&ExitPlanModeExtResponse {
...
pub(crate) fn handle_ext_notification(
    notif: &hystersis_acp_lib::AcpArgsBox<acp::ExtNotification>,
) -> ExtEvent {
    ...
    match method {
        "hystersis.com/task_backgrounded" => decode_task_backgrounded(method, params),
        "hystersis.com/task_completed" => decode_task_completed(method, params),
        "hystersis.com/monitor_event" => ExtEvent::MonitorEvent,
        "hystersis.com/leader/version_mismatch" => {
            match crate::acp::version_mismatch_banner(params) {
                Some(banner) => tracing::warn!(%banner, "hystersis.com/leader/version_mismatch"),
                None => {
    ...
    match serde_json::from_str::<TaskBgEnvelope>(params) {
        Ok(env) => match env.update {
            ...
            TaskBgUpdate::Other => {
                tracing::error!(
                    method,
                    payload = params,
                    "headless: hystersis.com/task_backgrounded with mismatched sessionUpdate \
        ...
        Err(e) => {
            tracing::error!(
                ...
                error = %e,
                payload = params,
                "headless: undecodable hystersis.com/task_backgrounded notification; \
    ...
    match serde_json::from_str::<TaskDoneEnvelope>(params) {
        Ok(env) => match env.update {
            ...
            TaskDoneUpdate::Other => {
                tracing::error!(
                    method,
                    payload = params,
                    "headless: hystersis.com/task_completed with mismatched sessionUpdate \
        ...
        Err(e) => {
            tracing::error!(
                ...
                error = %e,
                payload = params,
                "headless: undecodable hystersis.com/task_completed notification; \
...
fn decode_session_notification(method: &str, params: &str) -> ExtEvent {
    ...
    enum XaiUpdate {
        ...
        ResponseCompleted {
            ...
            #[serde(default)]
            usage: Option<hystersis_shell::extensions::notification::ResponseUsage>,
            #[serde(default)]
            signature: Option<String>,
            #[serde(default)]
    ...
    let hystersis_notif = match serde_json::from_str::<XaiNotif>(params) {
        Ok(n) => n,
        Err(e) => {
            tracing::warn!(
                method,
                error = %e,
                "headless: malformed hystersis.com session notification; ignoring"
            );
            return ExtEvent::None;
        }
    };

    match hystersis_notif.update {
                ...
                && matches!(tag.as_str(), "task_backgrounded" | "task_completed")
            {
                tracing::error!(
                    ...
                    "headless: background-task lifecycle tag on a session notification \
                     (expected the dedicated hystersis.com/task_backgrounded|task_completed method); \
