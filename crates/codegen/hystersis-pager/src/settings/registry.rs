...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: hystersis_voice::STT_LANGUAGE_DEFAULT.to_string(),
...
///
/// Delegates to [`hystersis_voice::canonicalize_stt_language`] so the pager and
...
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    hystersis_voice::canonicalize_stt_language(value)
}
    ...
    match key {
        ...
        "remember_tool_approvals" => Some(SettingValue::Bool(
            ui.remember_tool_approvals
                .unwrap_or(hystersis_shell::util::config::DEFAULT_REMEMBER_TOOL_APPROVALS),
        )),
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = hystersis_shell::models::default_model();
...
mod tests {
        ...
        for meta in reg.all() {
            ...
            match (meta.key, &meta.kind) {
                ...
                ("remember_tool_approvals", SettingKind::Bool { default }) => {
                    assert_eq!(
                        *default,
                        hystersis_shell::util::config::DEFAULT_REMEMBER_TOOL_APPROVALS,
                ...
                ("fork_secondary_model", SettingKind::DynamicEnum { default, .. }) => {
                    ...
                    assert_eq!(
                        ...
                        hystersis_shell::models::default_model(),
    ...
    /// Spot-check the delegation to `hystersis_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = hystersis_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = hystersis_voice::STT_LANGUAGES
            .iter()
            .map(|l| l.code)
            .collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match hystersis_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: hystersis_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
