...
pub use hystersis_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
...
pub use hystersis_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "hystersis_shell=info,hystersis_pager=trace,hystersis_tools=info,hystersis_session_search=info,hystersis_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = hystersis_telemetry::otel_layer::build_otel_layer(
        hystersis_telemetry::otel_layer::OtelClientInfo {
            client_name: "hystersis-pager",
            client_version: hystersis_version::VERSION,
            service_version: hystersis_version::full_version(),
            app_entrypoint: "tui",
        },
        hystersis_shell::auth::credential_provider::build_default_otel_layer_config(),
    );
    let instrumentation_layer = hystersis_telemetry::instrumentation::layer();
    let sampling_log_layer = hystersis_telemetry::sampling_log::layer();
    let hooks_log_layer = hystersis_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    hystersis_telemetry::debug_log::install_firehose(registry, "tui");
