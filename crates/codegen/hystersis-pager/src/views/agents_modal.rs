...
use hystersis_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use hystersis_shell::agent::config::AgentSelectionConfig;
use hystersis_tools::implementations::skills::discovery::extract_first_paragraph;
...
impl AgentsModalState {
    ...
    #[cfg(test)]
    fn search_viewport(&self, width: usize) -> hystersis_ratatui_textarea::SingleLineViewport {
...
pub fn build_agent_list(
    cwd: &Path,
    toggle: &HashMap<String, bool>,
    plugins: Option<&hystersis_agent::plugins::PluginRegistry>,
) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = hystersis_agent::discovery::discover(cwd);
    ...
    if let Some(registry) = plugins {
        for agent in hystersis_agent::discovery::plugin_agents(registry) {
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match hystersis_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => hystersis_config::hystersis_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".hystersis").join("personas"),
    }
}
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    hystersis_shell::config::load_effective_config()
        .ok()
        .and_then(|root| hystersis_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    hystersis_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        hystersis_agent::config::PromptMode::Extend => "extend",
        hystersis_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use hystersis_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn plugin_registry_with_reviewer(
        plugin_root: &Path,
    ) -> hystersis_agent::plugins::PluginRegistry {
        use hystersis_agent::plugins::discovery::PluginId;
        use hystersis_agent::plugins::{
