...
use hystersis_tools::implementations::skills::types::SkillInfo;
...
pub fn fuzzy_matches_hook(hook: &hystersis_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
fn hook_row_label(hook: &hystersis_hooks_plugins_types::HookInfo) -> String {
    let matcher = hook
        .matcher
        .as_deref()
        .map(|m| format!(" /{m}"))
...
fn is_official_marketplace_source(source: &hystersis_hooks_plugins_types::MarketplaceScanResult) -> bool {
    source.source_name == hystersis_plugin_marketplace::OFFICIAL_SOURCE_NAME
        || hystersis_plugin_marketplace::is_official_source_url(&source.source_url_or_path)
}
...
pub(crate) fn ordered_marketplace_view(
    sources: &[hystersis_hooks_plugins_types::MarketplaceScanResult],
...
fn skill_group(skill: &SkillInfo) -> SkillGroup {
    use hystersis_tools::implementations::skills::types::SkillScope;
    use hystersis_tools::types::config_source::ConfigSource;
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<hystersis_hooks_plugins_types::PluginOrigin>,
) -> hystersis_hooks_plugins_types::PluginInfo {
    hystersis_hooks_plugins_types::PluginInfo {
        name: name.to_string(),
        id: format!("user/abcd1234/{name}"),
        root: format!("/tmp/{name}"),
        scope: hystersis_hooks_plugins_types::PluginScope::User,
        trusted: true,
        enabled: true,
        version: None,
        description: None,
        ...
        agent_count: 0,
        agent_names: vec![],
        hook_status: hystersis_hooks_plugins_types::HookStatus::None,
        hook_count: 0,
        mcp_server_count: 0,
        mcp_status: hystersis_hooks_plugins_types::McpStatus::None,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    PluginGroupSortKey,
    Vec<(usize, &'a hystersis_hooks_plugins_types::PluginInfo)>,
>;
...
/// sensible groups.
pub fn plugin_group(plugin: &hystersis_hooks_plugins_types::PluginInfo) -> PluginGroup {
    use hystersis_hooks_plugins_types::{PluginOrigin, PluginScope};

    match &plugin.origin {
        Some(PluginOrigin::ProjectHystersis) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserHystersis) => PluginGroup::new(2, "origin:user", "User"),
        ...
        Some(PluginOrigin::MarketplaceInstall {
            ...
            rank: 5,
            key: format!("hystersis-mp:{source}"),
            label: source.clone(),
        },
        Some(PluginOrigin::MarketplaceInstall {
            source_name: None, ..
...
fn build_hook_groups<'a>(
    hooks: &'a [hystersis_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<HookGroupView<'a>> {
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> hystersis_telemetry::events::ExtensionsModalTab {
        use hystersis_telemetry::events::ExtensionsModalTab;
        match self {
...
pub enum ButtonAction {
    ...
    HooksAction(hystersis_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(hystersis_hooks_plugins_types::PluginsAction),
    ...
    AddMcpServer {
        name: String,
        config: Box<hystersis_shell::util::config::McpServerConfig>,
    },
    ...
    RefreshMcpList,
    /// Open hystersis.com connectors page (MCP tab: press `o`).
    ...
    MarketplaceAction(hystersis_hooks_plugins_types::MarketplaceAction),
...
impl ModalInputField {
    ...
    pub(crate) fn viewport(&self, width: usize) -> hystersis_ratatui_textarea::SingleLineViewport {
...
pub enum ConfirmationAction {
    ...
    Hooks(hystersis_hooks_plugins_types::HooksAction),
    ...
    Plugins(hystersis_hooks_plugins_types::PluginsAction),
    /// Replay a marketplace action (uninstall plugin or remove source).
    Marketplace(hystersis_hooks_plugins_types::MarketplaceAction),
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use hystersis_hooks_plugins_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use hystersis_hooks_plugins_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            hystersis_hooks_plugins_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use hystersis_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub struct ExtensionsModalState {
    ...
    pub hooks_data: TabDataState<hystersis_hooks_plugins_types::HooksListResponse>,
    /// Plugins list data (fetched from shell).
    pub plugins_data: TabDataState<hystersis_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<hystersis_hooks_plugins_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<hystersis_hooks_plugins_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[hystersis_hooks_plugins_types::PluginInfo]) {
...
fn classify_hook_source(source_dir: &str) -> HookSourceMeta {
    let hystersis = hystersis_config::hystersis_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user hystersis home (HYSTERSIS_HOME-aware)
    // or a project-scoped `{cwd}/.hystersis/<subdir>/`. Returns the first path
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User hystersis home (HYSTERSIS_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(hystersis.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.hystersis/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".hystersis" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    ...
    // Global hooks under $HYSTERSIS_HOME/hooks
    let global_hooks = hystersis.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.hystersis/hooks") || source_dir.contains("/.hystersis/hooks/") {
        return HookSourceMeta {
    ...
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&hystersis) {
            let prefix = crate::util::display_hystersis_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            hystersis_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_hystersis_home(path) {
                    crate::util::display_user_hystersis_path("skills")
            ...
            hystersis_tools::types::config_source::ConfigSource::Project { path } => {
                let s = path.display().to_string();
                if s.contains("/.hystersis/") {
                    ".hystersis/skills".into()
            ...
            hystersis_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &hystersis_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use hystersis_hooks_plugins_types::McpStatus;
...
fn component_categories(
    components: &hystersis_hooks_plugins_types::PluginComponents,
) -> [(&'static str, &[hystersis_hooks_plugins_types::ComponentItem]); 6] {
    use hystersis_hooks_plugins_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &hystersis_hooks_plugins_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.hystersis/plugins/<name>/` must label as
        // a (non-removable) plugin, not a removable "Custom" source. The user
        // hystersis-home branch is HYSTERSIS_HOME-aware; this covers the project fallback.
        let (label, is_custom) = derive_source_label("/repo/work/.hystersis/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.hystersis/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        let servers = vec![
            make_mcp_server_for_rows("hystersis_com_linear", McpWireSource::Managed, vec![]),
        ...
        assert!(
            rows.labels
                .iter()
                .any(|l| l.starts_with("Managed by hystersis.com")),
            "managed section header must appear"
        );
        assert!(
            !rows.labels.iter().any(|l| l == "hystersis_com_linear"),
    ...
    fn make_skill(
        name: &str,
        desc: &str,
    ) -> hystersis_tools::implementations::skills::types::SkillInfo {
        hystersis_tools::implementations::skills::types::SkillInfo {
            name: name.to_string(),
            display_name: None,
            ...
            metadata: None,
            path: "test".to_string(),
            scope: hystersis_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = hystersis_tools::implementations::skills::types::SkillScope::Plugin;
        skill.config_source = Some(hystersis_tools::types::config_source::ConfigSource::Plugin {
    ...
    fn make_plugin(name: &str) -> hystersis_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: hystersis_hooks_plugins_types::PluginOrigin,
    ) -> hystersis_hooks_plugins_types::PluginInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> hystersis_hooks_plugins_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(hystersis_hooks_plugins_types::PluginsListResponse {
            plugins: vec![
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    hystersis_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    hystersis_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                hystersis_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn make_hook(
        ...
        source_dir: &str,
        disabled: bool,
    ) -> hystersis_hooks_plugins_types::HookInfo {
        hystersis_hooks_plugins_types::HookInfo {
            name: name.to_string(),
            event: hystersis_hooks_plugins_types::HookEvent::PreToolUse,
            handler_type: hystersis_hooks_plugins_types::HookHandlerType::Command,
            matcher: None,
    ...
    fn superpowers_source() -> hystersis_hooks_plugins_types::MarketplaceScanResult {
        hystersis_hooks_plugins_types::MarketplaceScanResult {
    ...
    struct TestPlugin {
        ...
        components: Option<hystersis_hooks_plugins_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> hystersis_hooks_plugins_types::MarketplacePluginEntry {
            ...
            hystersis_hooks_plugins_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                hystersis_hooks_plugins_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                hystersis_hooks_plugins_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(hystersis_hooks_plugins_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_renders_header_with_error_badge() {
        let error_source = hystersis_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> hystersis_hooks_plugins_types::ComponentItem {
        hystersis_hooks_plugins_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> hystersis_hooks_plugins_types::PluginComponents {
        hystersis_hooks_plugins_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(hystersis_hooks_plugins_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = hystersis_hooks_plugins_types::PluginComponents {
            skills: (0..12)
    ...
    fn marketplace_modal_state(
        source: hystersis_hooks_plugins_types::MarketplaceScanResult,
    ...
    fn plugins_modal_state(
        plugins: Vec<hystersis_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(hystersis_hooks_plugins_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_maps_each_origin_variant() {
        use hystersis_hooks_plugins_types::PluginOrigin;
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectHystersis, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserHystersis, 2, "origin:user", "User"),
            (
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = hystersis_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = hystersis_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = hystersis_hooks_plugins_types::PluginScope::Config;
        ...
        assert_eq!(group.key, "hystersis-mp:xAI Official");
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            hystersis_hooks_plugins_types::PluginOrigin::Unknown,
        );
        assert_eq!(plugin_group(&unknown).key, "origin:user");

        unknown.marketplace_source = Some("xAI Official".into());
        assert_eq!(plugin_group(&unknown).key, "hystersis-mp:xAI Official");
    }
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserHystersis),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use hystersis_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserHystersis),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use hystersis_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserHystersis);
    ...
    fn marketplace_placeholders_render_only_when_expanded() {
        ...
        source.plugins[0].components = Some(hystersis_hooks_plugins_types::PluginComponents::default());
    ...
    fn plugins_sort_az_by_name_within_group() {
        use hystersis_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("Zebra", PluginOrigin::UserHystersis),
            make_plugin_with_origin("alpha", PluginOrigin::UserHystersis),
            make_plugin_with_origin("MID", PluginOrigin::UserHystersis),
        ]);
    ...
    fn ordered_marketplace_view_pins_official_then_az() {
        ...
        let sources = vec![
            ...
            mp(
                "xAI Official",
                hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
                None,
                &["zeta", "alpha"],
            ),
    ...
    fn skills_groups_then_az_by_label() {
        ...
        project_z.scope = hystersis_tools::implementations::skills::types::SkillScope::Local;
        project_z.config_source = Some(
            hystersis_tools::types::config_source::ConfigSource::Project {
                path: std::path::PathBuf::from("/repo/.hystersis/skills/zzz"),
            },
        );
        project_z.display_name = Some("zeta-proj".into());

        let mut project_a = make_skill("aaa-proj", "other project");
        project_a.scope = hystersis_tools::implementations::skills::types::SkillScope::Repo;
        project_a.config_source = Some(
            hystersis_tools::types::config_source::ConfigSource::Project {
                path: std::path::PathBuf::from("/repo/.hystersis/skills/aaa"),
            },
        );
    ...
    fn hooks_group_and_row_order() {
        ...
        h_stop.event = hystersis_hooks_plugins_types::HookEvent::Stop;
        let mut h_pre = make_hook("pre-hook", "/tmp/hooks-src", false);
        h_pre.event = hystersis_hooks_plugins_types::HookEvent::PreToolUse;
        h_pre.matcher = Some("Bash".into());
        let mut h_notify = make_hook("notify-hook", "/tmp/hooks-src", false);
        h_notify.event = hystersis_hooks_plugins_types::HookEvent::Notification;

        let hooks = vec![
            make_hook("c", "/zzz/custom", false),
            h_stop,
            make_hook("a", "/repo/.hystersis/hooks", false),
            h_pre,
            h_notify,
