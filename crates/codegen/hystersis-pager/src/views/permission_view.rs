...
use hystersis_workspace::permission::bash_command_splitting::{
...
pub struct PermissionViewState {
    ...
    pub request: hystersis_acp_lib::AcpArgs<acp::RequestPermissionRequest>,
    ...
    /// permissions). Imported from `xai-hystersis-shell`, NOT duplicated locally.
...
impl PermissionViewState {
    ...
    pub fn has_adjustable_scope(&self) -> bool {
        ...
        let bash_adjustable = self.bash_highlights.as_ref().is_some_and(|h| {
            ...
            let allow_adjustable = has_row(ALLOW_ALWAYS_COMMAND_OPTION_ID)
                && (1..=len)
                    .filter(|&n| hystersis_workspace::permission::always_allow_scope_persists(h, n))
                    .nth(1)
                    .is_some();
    ...
    pub fn step_persisting_allow_scope(&self, right: bool) -> usize {
        use hystersis_workspace::permission::always_allow_scope_persists;
...
pub(crate) fn preview_command_text(state: &PermissionViewState) -> String {
    match state.bash_highlights.as_ref() {
        Some(h) => hystersis_workspace::permission::bash_command_splitting::unwrap_command_wrappers(
            &h.highlighted_words,
        )
        .join(" "),
...
fn render_pattern_preview_line(
    ...
    match edit.trimmed() {
        ...
        Some(pattern) if hystersis_workspace::permission::bash_glob_is_catchall(pattern) => {
            spans.push(Span::styled(
        ...
        Some(pattern) => {
            if hystersis_workspace::permission::bash_pattern_matches_command(pattern, command) {
                spans.push(Span::styled(
            ...
            if hystersis_workspace::permission::bash_pattern_is_broad(pattern) {
...
/// ```text
///  3 [ ] Tell Hystersis what to do differently
...
mod tests {
    ...
    fn render_shows_planned_mcp_args() {
        ...
        state.description = vec![
            ...
            "  \"body\": \"hello from hystersis\"".to_string(),
            "}".to_string(),
        ];
        ...
        assert!(
            text.contains("\"body\": \"hello from hystersis\""),
            "args JSON not rendered:\n{text}"
        );
    ...
    ///
    /// ```text
    /// PERMISSION_UI_RENDER_REVIEW=1 cargo test -p xai-hystersis-pager --lib \
    ...
    fn historic_bash_fixture_path() -> std::path::PathBuf {
        // CARGO_MANIFEST_DIR = crates/codegen/xai-hystersis-pager
