...
fn compact_cwd_strips_home() {
    let p = Path::new("/Users/alice/projects/hystersis");
    assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/hystersis");
}
...
fn rename_at_cap_drops_extra_char() {
    assert_eq!(
        MAX_RENAME_SCALARS,
        hystersis_shell::session::persistence::MAX_TITLE_SCALARS
    );
    assert_eq!(hystersis_shell::session::persistence::MAX_TITLE_SCALARS, 100);
...
/// per-key serial lock. The `HYSTERSIS_AGENT_DASHBOARD` key means this
/// test runs serially with any other test that decorates itself
/// with `#[serial_test::serial(HYSTERSIS_AGENT_DASHBOARD)]` — see the
...
fn env_var_force_disables() {
    ...
    // same `HYSTERSIS_AGENT_DASHBOARD` key reads it concurrently.
    unsafe { std::env::set_var("HYSTERSIS_AGENT_DASHBOARD", "0") };
    assert!(!super::super::dashboard_enabled());
    unsafe { std::env::remove_var("HYSTERSIS_AGENT_DASHBOARD") };
}
