...
//! the generic [`CodeBlockSpan`](hystersis_markdown::CodeBlockSpan) API) and
...
use hystersis_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](hystersis_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](hystersis_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a hystersis_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
/// dark. The render worker maps this to `hystersis_mermaid::MermaidTheme`; it
