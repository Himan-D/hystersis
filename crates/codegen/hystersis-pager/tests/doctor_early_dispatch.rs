...
fn pager_binary() -> Result<std::path::PathBuf, String> {
    for key in ["PAGER_BINARY", "CARGO_BIN_EXE_xai-hystersis-pager"] {
    ...
    Err("PAGER_BINARY/CARGO_BIN_EXE_xai-hystersis-pager not set".to_owned())
}
...
fn doctor_json_bypasses_unrelated_startup_state() {
    ...
    let hystersis_home = temp.path().join("hystersis-home");
    std::fs::create_dir_all(&home).expect("create HOME");
    std::fs::create_dir_all(&hystersis_home).expect("create HYSTERSIS_HOME");
    ...
    let before = directory_entries(&hystersis_home);
    ...
    assert!(!String::from_utf8_lossy(&output.stdout).contains("Hystersis Doctor"));

    let after = directory_entries(&hystersis_home);
    ...
    for absent in ["docs", "crash", "memtrace", "active_sessions.json"] {
        assert!(
            !hystersis_home.join(absent).exists(),
...
fn doctor_fix_without_id_lists_tmux_fixes_from_current_probe_evidence() {
    ...
    let home = temp.path().join("home");
    let hystersis_home = temp.path().join("qhome");
...
fn doctor_hostile_home_and_byobu_create_no_config_files() {
    ...
    for (key, value) in [("HOME", "."), ("BYOBU_CONFIG_DIR", "relative")] {
        let mut command = base_pager_command(&binary, &home, &hystersis_home, "/bin/bash");
        command
...
fn doctor_fix_without_id_lists_only_applicable_automatic_fixes() {
    ...
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(
        stdout.contains("On your local computer, run: hystersis doctor fix ssh-wrap"),
        "{stdout}"
    );
    assert!(!home.join(".bashrc").exists());
...
fn doctor_tmux_fix_yes_writes_only_actual_home_tmux_config() {
    ...
    assert!(!hystersis_home.join(".tmux.conf").exists());
}
...
fn doctor_fix_yes_writes_only_actual_home_shell_rc() {
    ...
    assert_eq!(
        ...
        "# >>> hystersis doctor >>>\n# >>> terminal.ssh-wrap >>>\nalias ssh='hystersis wrap ssh'\n# <<< terminal.ssh-wrap <<<\n# <<< hystersis doctor <<<"
    );
    assert!(!hystersis_home.join(".bashrc").exists());
}
...
fn doctor_fix_safety_boundaries_are_process_isolated() {
    ...
    assert!(
        stderr.contains("Hystersis found an existing SSH alias or function")
...
fn run_pager(
    ...
    let mut command = base_pager_command(binary, home, hystersis_home, shell);
...
fn base_pager_command(
    ...
    command
        .env_clear()
        .env("HOME", home)
        .env("HYSTERSIS_HOME", hystersis_home)
        .env("SHELL", shell)
