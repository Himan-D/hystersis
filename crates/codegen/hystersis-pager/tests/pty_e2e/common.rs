...
pub(crate) use hystersis_pager_pty_harness::{
...
/// `hystersis_telemetry::unified_log::path()` resolves the calling process's
...
pub(crate) fn unified_log_path(content: &ContentController) -> PathBuf {
    content
        .sandbox()
        .hystersis_home()
        .join(hystersis_telemetry::unified_log::LOG_DIR)
        .join("unified.jsonl")
}
...
// `hystersis_pager_pty_harness::flows` (re-exported above).
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".hystersis")
        .join(hystersis_workspace::trust::TRUST_FILE_NAME);
    let store = hystersis_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&hystersis_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `hystersis_pager_pty_harness::flows`
// (re-exported above).
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = hystersis_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
pub(crate) use hystersis_pager_pty_harness::host_clipboard::{
...
pub(crate) use hystersis_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
