...
Agent mode runs Hystersis as a long-lived server that clients talk to over [ACP](https://agentclientprotocol.com) (JSON-RPC). Use it from IDEs, SDKs, eval harnesses, and custom apps. For a one-shot prompt that prints and exits, use `hystersis -p` instead ([headless mode](14-headless-mode.md)).
...
The [Agent Client Protocol (ACP)](https://agentclientprotocol.com) defines how clients talk to coding agents over JSON-RPC. With Hystersis it covers:
...
```bash
hystersis agent --always-approve --model hystersis stdio
hystersis agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```

| Flag | Description |
| ---- | ----------- |
| `-m, --model <MODEL>` | Model ID (for example `hystersis`). |
...
Clients connect over WebSocket and authenticate with the secret token. If you omit `--secret`, the agent prints a generated token at startup, or set `HYSTERSIS_AGENT_SECRET`. The process keeps state across client reconnects. Permissions match other entry points; see [Permissions and safety](22-permissions-and-safety.md).

This is a server you run yourself — Hystersis's hosted cloud sandboxes do not run `hystersis agent serve`.
...
```bash
hystersis agent --always-approve headless --hystersis-ws-url wss://your-relay.example.com/ws
```
...
Beyond the base ACP protocol, Hystersis defines extension methods under the `hystersis.com/` prefix for SpaceXAI-specific functionality. These cover:

| Category                   | Prefix               | Examples                                         |
| -------------------------- | -------------------- | ------------------------------------------------ |
| **Filesystem**             | `hystersis.com/fs/*`          | `list`, `exists`, `read_file`, `write_file`      |
| **Git**                    | `hystersis.com/git/*`         | `status`, `stage`, `commit`, `diffs`, `discard`  |
| **Git Worktree**           | `hystersis.com/git/worktree/*`| `create`, `remove`, `apply`, `list`, `gc`        |
| **Search**                 | `hystersis.com/search/*`      | `fuzzy/open`, `fuzzy/change`, `content`          |
| **Terminal**               | `hystersis.com/terminal/*`    | `create`, `kill`, `output`, `wait_for_exit`      |
| **Session Management**     | `hystersis.com/session/*`     | `fork`, `resolve_local_for_worktree_resume`      |
| **Conversation & History** | `hystersis.com/*`             | `prompt_history`, `rewind/*`, `compact_conversation` |
| **Authentication**         | `hystersis.com/auth/*`        | `get_url`, `submit_code`                         |
| **Feedback & Telemetry**   | `hystersis.com/*`             | `feedback`, `telemetry/*`                        |

The tables here show representative methods in each category. The `hystersis.com/*` set is SpaceXAI-specific and may expand across releases, so treat it as non-exhaustive and discover the available methods from the agent's `initialize` response.
...
| -------------------------- | ------------------------------------ |
| `hystersis.com/search/fuzzy/status` | Fuzzy search results update          |
| `hystersis.com/git/worktree/status` | Worktree creation progress           |
| `hystersis.com/fs_notify`           | Filesystem change notification       |
| `hystersis.com/fs/index`            | Full file index update               |
| `hystersis.com/fs/index/delta`      | Incremental file index update        |
| `hystersis.com/session_notification`| Session-specific updates (diff review, retry state, auto-compact) |
| `hystersis.com/session/update`      | Session update (tool calls, content) |
...
class HystersisACPChat {
  ...
  async init() {
    this.proc = spawn("hystersis", ["agent", "--always-approve", "stdio"]);
