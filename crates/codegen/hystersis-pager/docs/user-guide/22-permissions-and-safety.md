...
Control what Hystersis can access and do: permission modes, allow/ask/deny rules, hooks, and the optional OS-level sandbox.

- **Modes** set how often Hystersis asks for approval (always-approve, auto, ask, and related).
...
When Hystersis edits a file, runs a command, or calls an external tool, it may pause for approval. Permission modes control how often that happens.
...
```bash
hystersis -p "Run the tests" --always-approve
hystersis agent --always-approve stdio
hystersis agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
hystersis --always-approve -p "Run the test suite"
hystersis --permission-mode auto
hystersis agent --always-approve serve --bind 127.0.0.1:2419 --secret <token>
```
...
```bash
hystersis -p "Deploy the service" --always-approve --deny 'Bash(rm -rf *)'
```
...
Organizations can prevent always-approve from being enabled via CLI, TUI, or `/always-approve`. Set this in `requirements.toml` (user-level under `~/.hystersis/`, or system-wide under `/etc/hystersis/` for enforcement users cannot remove):
...
Hystersis can still load Claude-style permission **rules** from managed settings; always-approve is locked with `requirements.toml` as shown above.
...
Hystersis reads permission rules from three compatible sources. Rules from all sources are merged into one set; a rule's effect depends on its action (`deny` > `ask` > `allow`), not on which file it came from.
...
|-------|------|-----------------------|
| Global (all projects) | `~/.hystersis/config.toml` | No |
| Project (committed) | `<project>/.hystersis/config.toml` | Yes (commit it) |
| Project (personal) | `<project>/.claude/settings.local.json` | No (gitignore it) |
| Interactive grants | Stored internally by Hystersis, per project | No |

Notes on scoping:

- Hystersis discovers a `.hystersis/config.toml` at every directory level from the repository root down to your working directory, so a subdirectory can add rules on top of the repo root's.
...
- Hystersis has no native `config.local.toml`. For personal, uncommitted rules in a project, use `.claude/settings.local.json`; Hystersis reads it directly (see [Claude Code Compatibility](#3-claude-code-compatibility-claudesettingsjson)).
...
To stop prompts for a specific command in one project, add a narrow allow rule to that project's `.hystersis/config.toml` (or `.claude/settings.json`):
...
```bash
hystersis -p "Review the API changes" \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)' \
  --allow 'Read' \
  --allow 'Grep' \
...
### 2. Native Configuration (`~/.hystersis/config.toml` and `.hystersis/config.toml`)
...
Rules from the global `~/.hystersis/config.toml` and every project `.hystersis/config.toml` (from the repo root down to your working directory) are merged into one rule set, alongside any `.claude/settings.json` rules.

Managed configuration deployed by your organization also contributes `[permission]` rules: the system `/etc/hystersis/managed_config.toml`, and a user-level copy that Hystersis maintains automatically at `~/.hystersis/managed_config.toml`. Managed rules merge like rules from any other source, with two properties specific to managed `allow` rules: your own `deny` and `ask` rules win over a managed `allow` (severity ordering), and a catch-all managed `allow` is ignored when always-approve is locked off. For rules that users cannot edit away, use the root-owned system `/etc/hystersis/requirements.toml`.
...
Hystersis reads `~/.claude/settings.json` and `~/.claude/settings.local.json`, plus the project-level `<project>/.claude/settings.json` and `settings.local.json` (walking up to the repo root). The native `.hystersis` source for permission rules is `config.toml`, described in the section above.
...
Supported `defaultMode` values include `default`, `auto`, `acceptEdits`, `bypassPermissions`, `dontAsk`, and `plan`. Hystersis reads `defaultMode` from its canonical location under `permissions`; a top-level `defaultMode` is also accepted when the nested key is absent.
...
**Chained commands.** Hystersis parses each command like a shell and splits it on `&&`, `||`, `;`, `|`, and newlines. The rule actions treat segments differently:
...
`MCPTool(...)` patterns match the full Hystersis tool name in `server__tool` form, with glob support: `MCPTool(linear__*)` matches every tool from the `linear` server. Hystersis tool names carry no `mcp__` prefix.
...
Interactive grants are stored in Hystersis's own state directory under your home directory, scoped to the git repository you launched Hystersis in (its repository root), so a grant accepted at the repo root also applies in sessions started from a subdirectory of the same repository. Outside a git repository, grants are scoped to the launch directory, and each git worktree keeps its own grants. A grant made in one project never applies in another, grants are not written into the repository, and they are not meant to be hand-edited.

To inspect or reset a project's grants, open the `sessions` subdirectory of your Hystersis home (the `.hystersis` directory under your home directory, or `$HYSTERSIS_HOME`): each project directory there (URL-encoded scope root) holds a `permission.toml` (plus per-client `permission_<client>.toml` variants) listing the remembered command prefixes, globs, MCP tools/servers, web-fetch domains, and "never allow" entries. Deleting the file resets that project's grants; the next matching tool call prompts again. Treat it as read-only state — to *add* rules, use the declarative `[permission]` configuration instead.

Interactive grants are personal, per-machine state. For an allowlist you can review in code review and share with teammates, use declarative rules in the project's `.hystersis/config.toml` instead.
...
**`~/.hystersis/hooks/git-gh-only.json`**
...
**`~/.hystersis/hooks/git-gh-only.sh`**
...
```bash
chmod +x ~/.hystersis/hooks/git-gh-only.sh
```
...
```bash
hystersis -p "Implement the feature using only git and GitHub CLI" \
  --allow 'Read' \
  --allow 'Grep' \
  --allow 'Bash(git *)' \
  --allow 'Bash(gh *)'
...
3. **Review project configuration from unfamiliar sources.** Project permission rules in `.hystersis/config.toml` and `.claude/settings.json` are gated on folder trust: an untrusted checkout's project rules (including `allow` rules and `defaultMode`) are skipped, and their presence triggers the folder-trust question. Trusting the folder applies them, so review them — and any project hooks — before granting trust to an unfamiliar checkout (see the security notes in [10-hooks.md](10-hooks.md)).
