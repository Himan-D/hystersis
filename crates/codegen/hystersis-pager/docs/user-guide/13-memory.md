...
Memory lets Hystersis recall facts, decisions, and patterns from earlier sessions. Hystersis indexes the information you save and searches it automatically, so a new session can reuse relevant context.

---

## What Is Memory?

Without memory, each Hystersis session starts fresh: the model knows nothing about previous sessions. When you enable memory, Hystersis can:
...
2. `HYSTERSIS_MEMORY` env var: `1`/`true` enables, `0`/`false` disables
...
|----------|-------|-------------|
| `~/.hystersis/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.hystersis/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.hystersis/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |

Hystersis suffixes each workspace directory with a short hash of the repository's identity. The identity is the `origin` remote in `org/repo` form when the directory is a Git repository with an `origin` remote, or the directory path otherwise. Because clones and worktrees of the same repository share an `origin` remote, they also share one memory directory.
...
When a session ends, Hystersis saves a structured metadata summary to that session's daily log. The summary contains:
...
Hystersis builds the summary from conversation metadata without an LLM call, without added latency. Hystersis skips the save for trivial sessions -- those with fewer than three substantive prompts, or fewer than 50 bytes of user text.
...
Ask Hystersis to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.hystersis/memory/MEMORY.md` for cross-project preferences:

```
> remember to always open PR links after pushing
```

Hystersis records entries as durable statements under organized headings, such as `## Preferences`, `## Project Context`, or `## Debugging`. The file watcher reindexes the change on the next memory search, so the new entry is searchable within the current session.
...
Run `/remember` with no text to enter remember mode, where the next line you type becomes the note. Either way, Hystersis opens a review panel showing the note (with an optional rewritten version you can toggle with `Tab`); the note is written only after you confirm. On save, Hystersis shows `Memory saved to ~/.hystersis/memory/MEMORY.md`.

### Forget

Ask Hystersis to forget something, and it finds and removes the matching entry:

```
> forget the snake_case convention
```

Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.hystersis/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.

### Recall

Ask what Hystersis remembers:

```
> what do you remember?
```

Hystersis searches across all memory files and summarizes what it knows, grouped by source: global preferences, project-specific knowledge, and session history. Use `/memory` to browse the raw files.

### Direct Editing

You can edit memory files directly under `~/.hystersis/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
When you save a note with `/remember`, Hystersis confirms in the scrollback:

```
Memory saved to ~/.hystersis/memory/MEMORY.md
```

Background saves — flush, dream, and session-end — run silently and do not post a scrollback message. Use `/memory` at any time to browse what Hystersis has stored.
...
Dream also runs automatically. By default, Hystersis checks the consolidation gates when a session ends and runs Dream once enough time has passed and enough sessions have accumulated:
...
On the first turn of each session, Hystersis automatically searches memory for content relevant to the current project and injects it as context. This means Hystersis starts with knowledge from previous sessions without a reminder.
...
Hystersis searches memory automatically, but you can also trigger searches manually in the chat:
...
The `hystersis memory` command manages memory from the shell. It has one subcommand, `clear`:

```bash
# Clear workspace memory (MEMORY.md, sessions/, and index.sqlite). This is the default scope.
hystersis memory clear

# The same scope, stated explicitly
hystersis memory clear --workspace

# Clear the global MEMORY.md
hystersis memory clear --global

# Clear both workspace and global memory
hystersis memory clear --all

# Skip the confirmation prompt (-y is the short form)
hystersis memory clear --yes
```

To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.hystersis/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.hystersis/memory/` for external edits and reindex |
...
| `flush_model` | unset | Model for the flush turn. When unset or `""`, Hystersis uses the session's primary model. |
...
When a session memory is old, Hystersis attaches a staleness note to it in search results. Older results get a stronger reminder to verify the current state before you rely on them. These notes help you spot stored facts that might no longer be accurate. Global and workspace memories never receive staleness notes, because they hold curated long-term knowledge.

---

## File Watcher

By default, Hystersis watches `~/.hystersis/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
1. Verify memory is enabled: check `hystersis inspect` output.
2. Check `HYSTERSIS_MEMORY` or `[memory] enabled` in effective TOML.
3. Check for `HYSTERSIS_MEMORY=0` or a deprecated compatibility flag overriding config.
...
```bash
ls ~/.hystersis/memory/
cat ~/.hystersis/memory/MEMORY.md
$EDITOR ~/.hystersis/memory/MEMORY.md
```
