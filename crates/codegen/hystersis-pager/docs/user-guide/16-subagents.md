...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.hystersis/agents/` or `~/.hystersis/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.hystersis/personas/`; applied during subagent resolution |
...
| **Examples** | `hystersis`, `explore`, `plan` | `researcher`, `concise` |
...
Hystersis discovers file-based personas from these locations, in priority order:

- `.hystersis/personas/*.toml` (project)
- `~/.hystersis/personas/*.toml` (user)
...
> **Note:** Hystersis applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Hystersis resolves the effective model and reasoning effort in this order, highest priority first:
...
Hystersis manages worktrees through the `hystersis.com/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "hystersis"               # route explore to a specific model
```
...
# instructions_file = ".hystersis/personas/concise.md"  # or load from a file
```

Hystersis also discovers roles from `.hystersis/roles/*.toml` and personas from `.hystersis/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Hystersis shows running and finished work in side panes on the agent screen:
...
- `Subagent running: "do the thing" (Implementer · hystersis-3) · Thinking`
