...
Hooks let you run a script or send an HTTP request at key moments in a Hystersis session. Use them to automate tasks, enforce safety checks, log activity, send notifications, and integrate your own tools.

---

## What Are Hooks?

A hook is a shell command or HTTP endpoint that Hystersis calls when a specific lifecycle event occurs. Hooks can:
...
1. Create the hooks directory:
   ...
   ```sh
   mkdir -p ~/.hystersis/hooks
   ```

2. Create a hook file, e.g. `~/.hystersis/hooks/session-start.json`:
   ...
   ```json
   {
     "hooks": {
       "SessionStart": [
         {
           "hooks": [
             { "type": "command", "command": "echo 'Hystersis session started in '$(pwd)" }
           ]
         }
       ]
     }
...
3. Start (or restart) a Hystersis session. The hook runs automatically on `SessionStart`.
...
|-------|------|----------|-------|
| Global | `~/.hystersis/hooks/*.json` | Always | Personal hooks |
...
| Project | `<project>/.hystersis/hooks/*.json` | Requires trust | Per-repo automation |
...
| Config | `~/.hystersis/config.toml` | Always | Your hooks alongside the rest of your config |
| Config | `managed_config.toml` (`$HYSTERSIS_HOME` and `/etc/hystersis`) | Always | Organization-distributed hooks (server-synced and on-device) |
...
Config-file hooks live in the same TOML your organization already controls; see [Hooks in Config Files](#hooks-in-config-files) for the format. The compatible vendor hook sources are scanned by default. To disable scanning for a specific vendor, set `[compat.<vendor>] hooks = false` in `~/.hystersis/config.toml` or the corresponding environment variable. See [Configuration](05-configuration.md#harness-compatibility) for details.

**Trusting a project**: The first time you open a project with hooks, you must trust it before its project hooks will run; until then they are silently skipped. Grant trust by running `/hooks-trust` (or launching with `--trust`); the decision is recorded in the unified folder-trust store (`~/.hystersis/trusted_folders.toml`), the same gate that governs repo-local MCP/LSP servers. Global hooks in `~/.hystersis/hooks/` are always trusted and need no entry. This prevents untrusted repos from running arbitrary code.

Because hooks are unified under folder-trust, a `--trust` / `/hooks-trust` grant trusts the whole folder for **MCP, LSP, and hooks** together, and covers subdirectories of the same repository. A nested git checkout under that folder is a separate workspace and is not covered. Conversely, disabling folder-trust (`HYSTERSIS_FOLDER_TRUST=0` or `[folder_trust] enabled = false`) ungates project hooks along with MCP/LSP.
...
Hystersis accepts Cursor's camelCase hook event names, so `~/.cursor/hooks.json` loads unchanged:
...
- **Event name** (top-level key): any event listed in [Hook Events](#hook-events). Hystersis skips unrecognized event names so a shared Claude or Cursor settings file still loads.
...
In a `matcher`, Hystersis maps Claude-style tool names to its own so hooks migrated from Claude fire correctly. Common aliases include:
...
When an event fires, Hystersis resolves it in four steps:
...
Hooks can also live directly in your Hystersis config, so a team can distribute them with the rest of their configuration instead of shipping separate JSON files. The same `hooks` object is read from three TOML files:

| File | Tier | Who sets it |
|------|------|-------------|
| `~/.hystersis/config.toml` | User | You |
| `managed_config.toml` (`$HYSTERSIS_HOME`, `/etc/hystersis`) | Managed / system | Your organization |
...
A script that tracks busy and idle should key on `promptId`, following the rules below. hystersis mints one per turn, but a client that supplies its own in `_meta` owns its uniqueness, so treat the id as opaque and scope it to the session.
...
- **Settle unconditionally when there is no `promptId`.** That is hystersis reporting on the session
...
- **camelCase input**: hystersis's stdin envelope uses camelCase keys throughout where Claude uses snake_case. A script reading `.stop_hook_active`, `.hook_event_name`, or `.background_tasks[].agent_type` must switch to `.stopHookActive`, `.hookEventName`, and `.backgroundTasks[].agentType` (the event value is `"stop"`). Hooks registered through the hystersis-agent-sdk convert both the top-level keys and the `backgroundTasks`/`sessionCrons` entry keys to snake_case, so the wire's `.backgroundTasks[].agentType` reads as `.background_tasks[].agent_type` in the SDK.
...
- **StopFailure classes**: hystersis emits six (`rate_limit`, `authentication_failed`, `invalid_request`, `server_error`, `max_output_tokens`, `unknown`). Capacity errors (503/529) classify as `rate_limit`. A matcher on an error class hystersis does not emit never fires.
- **Default timeout**: hystersis defaults observe hooks to 5 seconds, which is shorter than most. Set `timeout` explicitly on an imported hook that does real work.
- **`UserPromptSubmit` is observe-only**: hystersis ignores its exit code and its stdout, so an imported prompt-validation hook silently stops blocking. Use `PreToolUse` to enforce.
- **`StopCancelled` is hystersis-specific**: a config that uses it is not portable to a runtime with no interrupt hook.
- **`idle_prompt` fires on any turn end**: hystersis fires it after an interrupted or errored turn too, not only a completed one, because it reports a state rather than an outcome. Its `message` is display text and can change between releases, so match on `notificationType` instead.
- **Subagent identity is `subagentType`, not `agent_type`**: hystersis puts it in the payload of the events that can fire inside a subagent, matching its own `SubagentStart`/`SubagentStop`, rather than in the common fields.
- **permission_mode values**: hystersis emits `default`, `auto`, `plan`, or `bypassPermissions`. Claude's `acceptEdits`/`dontAsk` have no hystersis equivalent (hystersis's `auto` is the nearest), so a check like `permission_mode === "acceptEdits"` never matches.
...
- **`/goal`**: hystersis's goal loop is a separate feature that runs before the stop gate; it is not a prompt-type Stop hook.
...
Hystersis sets several environment variables on every hook process. These are useful when writing context-aware or plugin-aware hook scripts.
...
|-----------------------|-------------|
| `HYSTERSIS_HOOK_EVENT`     | The name of the event that triggered the hook (e.g. `pre_tool_use`, `session_start`, `post_tool_use`, `session_end`, `stop`, `notification`). |
| `HYSTERSIS_HOOK_NAME`      | The configured name of this specific hook (includes the plugin prefix for plugin-provided hooks). |
| `HYSTERSIS_SESSION_ID`     | The unique identifier of the current Hystersis session. |
| `HYSTERSIS_WORKSPACE_ROOT` | Absolute path to the root of the current workspace. |
| `CLAUDE_PROJECT_DIR`  | Absolute path to the workspace root. A Claude Code-compatible alias for `HYSTERSIS_WORKSPACE_ROOT`, set for every hook. |
...
When a hook originates from a plugin, Hystersis additionally injects the following variables:

| Variable             | Description |
|----------------------|-------------|
| `HYSTERSIS_PLUGIN_ROOT`   | Absolute path to the plugin's installed directory. |
| `HYSTERSIS_PLUGIN_DATA`   | Absolute path to the plugin's writable data directory (for storing plugin state, caches, etc.). |

These values are provided by the plugin system. For the four plugin-related keys (`HYSTERSIS_PLUGIN_ROOT`, `HYSTERSIS_PLUGIN_DATA`, and their Claude aliases), the plugin adapter ensures the official plugin values always win over any user-declared values in the hook's `env` map.
...
```json
{ "type": "http", "url": "https://hooks.example.com/hystersis-event", "timeout": 15 }
```
...
Press `r` in the Hooks tab to reload all hooks from disk. Hystersis re-reads every hook source, so this picks up changes you made to hook files during the session.
...
- Global hooks (`~/.hystersis/hooks/`) run with your user permissions; treat them like shell scripts.
...
5. **Version control project hooks**: commit `.hystersis/hooks/` (but never secrets).
...
- **See errors?** Capture logs by launching with `RUST_LOG=debug HYSTERSIS_LOG_FILE=/tmp/hystersis.log hystersis`, then check `/tmp/hystersis.log`.
