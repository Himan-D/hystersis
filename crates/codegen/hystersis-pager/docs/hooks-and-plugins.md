...
Hystersis supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.hystersis/hooks/`
- **Project hooks** — from `.hystersis/hooks/` in your repo
...
Plugin hooks automatically receive `HYSTERSIS_PLUGIN_ROOT` and `HYSTERSIS_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.hystersis/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `hystersis plugin marketplace add <source>`)
...
Sources land in `~/.hystersis/config.toml`:
...
Or in `~/.hystersis/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../xai-hystersis-hooks/examples/README.md) — ready-to-use sample hooks
