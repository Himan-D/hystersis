//! TLS policy for the hystersis CLI: OS roots, Mozilla roots, and opt-in extra
//! roots from `HYSTERSIS_EXTRA_CA_BUNDLE` (fallback: `SSL_CERT_FILE`). A bad bundle
...
pub const ENV_HYSTERSIS_EXTRA_CA_BUNDLE: &str = "HYSTERSIS_EXTRA_CA_BUNDLE";
...
/// Builds a reqwest client with the hystersis TLS policy: the shared roots (OS store,
...
fn select_bundle(
    ...
    match bundle {
        Some(p) if !p.is_empty() => Some((ENV_HYSTERSIS_EXTRA_CA_BUNDLE, p.into())),
        Some(_) => None,
        None => match ssl {
