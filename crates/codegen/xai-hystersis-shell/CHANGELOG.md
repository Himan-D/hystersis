...
- **hystersis clone** now reuses matching local checkouts as linked worktrees for faster session creation.
...
- **hystersis clone** now fetches only the selected branch tip by default, with --full-history for legacy full clones.
- **hystersis clone** can now reuse a matching local checkout as a linked worktree instead of always fetching from the network.
...
- **hystersis mcp add** now correctly treats bare http(s):// URLs as HTTP servers instead of stdio commands.
...
- **Interactive hystersis sessions** now start in ask mode by default again instead of auto.
...
- Users hitting startup timeouts can now raise the connect budget with the `HYSTERSIS_CONNECT_UI_TIMEOUT_SECS` environment variable.
...
- **hystersis clone** can now fetch a repo into a content store and mount a projected working tree.
...
- **HYSTERSIS_CONFIG** and **HYSTERSIS_CONFIG_PATH** environment variables now let launchers override selected config settings without editing config.toml.
- **Worktrees** under ~/.hystersis/worktrees are now automatically reclaimed when safe, with strong safeguards that never delete a user's last copy.
...
- **HYSTERSIS_FORCE_LOGIN_TEAM_ID** environment variable now lets launchers restrict interactive login to one or more teams.
...
- **hystersis inspect** no longer crashes when its output is piped into a command that closes the pipe early.
...
- **Session search index** can now be disabled via HYSTERSIS_SESSION_SEARCH or [features] session_search for hosts sharing $HYSTERSIS_HOME.
...
- Tool commands and MCP servers now receive a HYSTERSIS_SESSION_ID environment variable matching the current session.
...
- **Keystrokes typed while Hystersis is starting** are now preserved in the composer instead of being lost.
...
- **`hystersis du`** and worktree commands now work on Windows when only USERPROFILE is set.
...
- **Subagent spawning** is dramatically faster when you have many sessions in ~/.hystersis.
...
- New `hystersis du` command shows disk usage of ~/.hystersis including worktrees and sessions.
...
- **hystersis trace** exports now bundle memory trace files for easier debugging.
...
- **Failed alpha/enterprise updates** now suggest the matching HYSTERSIS_CHANNEL reinstall command.
- **On Apple Silicon**, hystersis now installs the native arm64 build even from a Rosetta shell or x86_64 updater.
...
- **Non-interactive sessions** (`hystersis -p`) no longer fail when the agent asks for user input or plan approval.
...
- Hystersis skips the project-directory prompt when launched from home or other non-project directories
...
- Sandboxed Hystersis starts on large directories with many deny-glob matches
...
- Hystersis retries more server errors during outages
...
- **hystersis doctor** now warns when tmux is reducing colors and can fix the config.
...
- **HYSTERSIS_EXTRA_CA_BUNDLE** env var allows adding custom TLS root certificates.
...
- **Hystersis** no longer crashes on startup when the host machine has no free threads.
...
- **MCP servers** can now be enabled or disabled directly from the CLI with `hystersis mcp enable <name>` and `hystersis mcp disable <name>`.
- **Full plan markdown** can now be copied to the clipboard with `y` during plan approval or preview.
- **Added support for the new HystersisPro Plus subscription tier** in authentication and feature gating.
...
- **Invalid MCP server entries** in config.toml no longer prevent Hystersis from starting; problems are shown in `hystersis inspect`.
...
- **Fixed credential loss** across multiple hystersis processes sharing the same auth file.
...
- **New /tutorial slash command** opens an opt-in nine-topic onboarding tour of Hystersis.
...
- **`hystersis doctor fix`** can now repair common tmux clipboard and passthrough problems.
- **Per-provider auth helpers** now work on Windows and can run from a configurable working directory.
- **/resume** now shows only native Hystersis sessions by default and shows a hint when external sessions are hidden.
- **`hystersis --resume`** can now resume a session by its title as well as by ID.
...
- **Web search** now defaults to hystersis-4.5.
...
- You can now run `hystersis doctor fix` commands directly from inside the TUI instead of only from the CLI.
...
- **npm package** now installs the native binary under `$HYSTERSIS_HOME/bin` (honoring the same override as the Rust CLI).
...
- Hystersis now automatically stops a turn that keeps repeating the exact same tool call many times in a row.
...
- When the auto-permission classifier times out or fails, Hystersis now shows a normal permission prompt instead of silently denying.
...
- **hystersis doctor fix ssh-wrap** can set up `hystersis wrap ssh` automatically for Bash, zsh, and fish.
...
- **hystersis doctor** checks terminal, tmux, clipboard, and keyboard setup without opening the TUI.

## Bug Fixes

- **Image paste** over hystersis wrap now works on headless remotes.
...
- **Added HYSTERSIS_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `hystersis --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Default model** is now Hystersis 4.5 with high/medium/low reasoning effort and improved compaction settings.
...
- **Global rules** from ~/.hystersis/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `hystersis wrap ssh <host>` for clipboard and terminal restore.
...
- **hystersis** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **hystersis wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `hystersis login`.
...
- **hystersis login** now requests Hystersis Projects scopes so workspace listing works after consent.
...
- **`hystersis plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using hystersis -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **hystersis inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct hystersis --minimal --resume command.
...
- **Hystersis no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **hystersis setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask Hystersis above the footer shortcuts.
...
- **hystersis.com/pr/status** now reports whether an open PR is in the merge queue.
...
- **`hystersis worktree ls`** now works as a short alias for `hystersis worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.hystersis/config.toml`.
...
- **Resumed hystersis.com chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.hystersis/worktrees/.
...
- **Terminal command cards** on hystersis.com now correctly settle after foreground bash tasks.
...
- **hystersis inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain hystersis launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive hystersis agent stdio without silent parse drops on session/* calls.
...
- **hystersis --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by hystersis.com" link in the Extensions modal is now clickable and underlined.
...
- **hystersis --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **hystersis update** on Windows no longer fails when a previous .old executable is still running.
...
- **hystersis models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `hystersis plugin marketplace add`.
...
- **Plan files** now default to .hystersis/plan.md to match Hystersis conventions.
...
- **--minimal** flag now shows in `hystersis --help`.
...
- **Tool result previews** no longer paint opaque panels in `hystersis --minimal`.
- **hystersis wrap** now correctly handles quoted strings and shell aliases.
...
- **Chat sessions show the hystersis.com model catalog** in the picker.
...
- **`hystersis update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `hystersis wrap`.
...
- **hystersis agent stdio** now checks for updates in the background like other modes.
...
- **hystersis wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `hystersis agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `hystersis wrap`** to run any command with local clipboard support.
...
- **Fixed `hystersis agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or hystersis-desktop.
...
- You can now pass --json-schema to hystersis -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `HYSTERSIS_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **hystersis -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in hystersis -p and hystersis agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `hystersis` after running `hystersis update`.
- Headless (`hystersis -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`hystersis plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **hystersis sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **hystersis quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, hystersis sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, HYSTERSIS_HUNK_TRACKER, or config.
...
- **Hystersis** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`hystersis dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`hystersis mcp add`** now accepts positional arguments (e.g. `hystersis mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`hystersis update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.hystersis.
...
- **MCP servers menu** and slash commands now work when starting hystersis outside a project directory.
...
- **hystersis --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `hystersis --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `hystersis update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`hystersis --debug`** now produces per-session log files under ~/.hystersis/debug/ even with a leader process.
...
- **`hystersis update`** now restarts any older running leader so all clients get the new binary.
...
- **`hystersis login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `hystersis agent --leader` without forced interactive login or timeouts.
...
- **Image and video generation** now use api.hystersis.com directly for all users.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the HYSTERSIS_IMAGE_GEN_HARNESS env var.
...
- **hystersis inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via HYSTERSIS_CURSOR_MCPS_ENABLED / HYSTERSIS_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside Hystersis and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the hystersis model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality hystersis-imagine-image-quality model.
...
- Memory system: /remember command, note modal with raw/enhanced preview, hystersis.com/memory/rewrite ACP extension, Ctrl+F fullscreen toggle for /memory modal.
