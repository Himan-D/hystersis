...
use crate::auth::{HystersisAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.hystersis/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.hystersis/config.toml`,
    ...
    /// (`<cwd>/.hystersis/config.toml`, `<cwd>/.mcp.json`, or
    ...
    /// hystersis process sharing the home dir). The agent should consult the cache
    /// thrash unrelated sessions.
    ProjectMcpServersChanged {
        /// The project root whose `.hystersis/`, `.mcp.json`, or
    ...
    Skills(xai_hystersis_agent::prompt::skills::SkillsConfig),
    /// Updated `[compat]` vendor-compatibility config. Applied on the
    /// next agent (re)build, which re-resolves `compat_resolved`.
    Compat(Box<xai_hystersis_tools::types::compat::CompatConfigToml>),
    ...
    ModelsChanged,
    /// `~/.hystersis/models_cache.json` was rewritten on disk (possibly by another
    ...
    ModelsCacheChanged,
    /// Updated UI settings — agent broadcasts `hystersis.com/config_changed` to IPC clients.
    Ui {
        theme: Option<String>,
        yolo: bool,
...
impl ConfigReloader {
    ...
    pub(crate) async fn run(
        ...
        loop {
            ...
            if has_auth {
                ...
                match result {
                    Ok(Err(e)) => {
                        ...
                        let path = self.hystersis_home.join("auth.json");
            ...
            // same tick (e.g. `~/.hystersis/config.toml` AND
    ...
    pub(crate) fn reload_auth(&mut self) -> anyhow::Result<()> {
        let auth_path = self.hystersis_home.join("auth.json");
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.hystersis/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.hystersis/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.hystersis/config.toml` | `<cwd>`           |
...
/// for `.hystersis/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.hystersis/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
/// `ConfigUpdate::Skills`) and `hystersis inspect` (via the `crate::config`
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> xai_hystersis_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> xai_hystersis_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    fn make_auth(key: &str) -> HystersisAuth {
        HystersisAuth {
            key: key.to_string(),
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<HystersisAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.hystersis/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".hystersis").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .hystersis/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.hystersis/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.hystersis/skills".to_string()]);
...
theme = "dark"
yolo = true
fork_secondary_model = "hystersis-4.5"
"#,
        )
        .unwrap();
        let (theme, yolo, fork) = extract_ui_fields(&config);
        ...
        assert!(yolo);
        assert_eq!(fork.as_deref(), Some("hystersis-4.5"));
    }
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.hystersis/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.hystersis/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.hystersis/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.hystersis/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
