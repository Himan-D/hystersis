...
fn subagent_limit_counts_resolve_env_over_toml_over_remote_over_default() {
    use xai_hystersis_tools::implementations::hystersis_build::task::admission;
...
fn subagent_sampling_limit_applies_precedence_and_clamps() {
    ...
    use xai_hystersis_tools::implementations::hystersis_build::task::admission::DEFAULT_MAX_CONCURRENT;
...
fn subagent_limit_behavior_resolves_env_over_toml_over_remote_over_queue() {
    use xai_hystersis_tools::implementations::hystersis_build::task::admission::LimitBehavior;
...
fn media_gen_caps_resolve_env_over_toml_over_remote_over_default() {
    use xai_hystersis_tools::media_gen_limits::{
...
fn managed_settings_disables_features_and_requirements_overrides() {
    use crate::agent::config::Feature;
    use xai_hystersis_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
fn simulate_release_build() -> xai_hystersis_test_support::EnvGuard {
    xai_hystersis_test_support::EnvGuard::set(xai_hystersis_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use xai_hystersis_agent::plugins::{TrustStore, discover_plugins};
    use xai_hystersis_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("HYSTERSIS_HOME", home.path());
    let _flag = EnvGuard::unset("HYSTERSIS_FOLDER_TRUST");
