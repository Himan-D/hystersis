...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.hystersis/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** hystersis process
    /// sharing the same `~/.hystersis` (the writer may also be this process;
...
/// Watches `~/.hystersis/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.hystersis/config.toml`, `.mcp.json`,
...
/// processes (e.g. `hystersis login` in another terminal).
///
/// Adds two **non-recursive** watches per `cwd` argument:
/// `<cwd>/` (catches `.mcp.json` and `.claude.json` at the project root) and
/// `<cwd>/.hystersis/` (catches `<cwd>/.hystersis/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.hystersis/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let hystersis_home_buf = hystersis_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(hystersis_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(hystersis_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(hystersis_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(hystersis_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %hystersis_home.display(),
                    error = %e,
                    "failed to watch hystersis home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.hystersis/config.toml` so the loop above
        // watches `<cwd>/.hystersis/`), the call below installs a
        ...
        tracing::info!(
            hystersis_home = %hystersis_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.hystersis/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.hystersis/config.toml` trigger a
    ...
    /// `<cwd>/.hystersis/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.hystersis/` does not yet
/// exist at session-open time, the `.hystersis/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.hystersis/` followed by a
/// write to `<cwd>/.hystersis/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let hystersis_dir = cwd.join(".hystersis");
    if let Err(e) = debouncer
        .watcher()
        .watch(&hystersis_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .hystersis directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let hystersis_dir = cwd.join(".hystersis");
    if let Err(e) = debouncer.watcher().unwatch(&hystersis_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .hystersis directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.hystersis/` hasn't
...
const VENDOR_CONFIG_ROOT_NAMES: &[&str] = &[".hystersis", ".agents", ".claude", ".cursor"];

/// Vendor roots (by name or `hystersis_home`) must use scoped watches — they can
/// contain large non-skill trees (`worktrees/`, etc.).
fn is_vendor_config_root(dir: &Path, hystersis_home: &Path) -> bool {
    if paths_equal(dir, hystersis_home) {
        return true;
    }
    dir.file_name()
        .and_then(|n| n.to_str())
...
fn project_hystersis_refresh_dirs(project_root: &Path) -> Vec<(PathBuf, RecursiveMode)> {
    let project_hystersis = project_root.join(".hystersis");
    let mut dirs = vec![(project_hystersis.clone(), RecursiveMode::NonRecursive)];
    dirs.extend(vendor_skill_refresh_dirs(&project_hystersis));
    dirs
}
...
fn plan_skills_watch_targets(
    ...
    for dir in dirs_to_watch {
        if is_vendor_config_root(dir, hystersis_home) {
...
/// Watches project `.hystersis` skills/commands/workflows for mid-session discovery.
///
...
impl ProjectDiscoveryWatcher {
    pub(crate) fn start(cwd: &Path) -> Option<(Self, mpsc::UnboundedReceiver<DiscoveryChange>)> {
        ...
        let project_hystersis = project_root.join(".hystersis");
        let (tx, rx) = mpsc::unbounded_channel();
        let project_hystersis_for_events = project_hystersis.clone();
        let mut debouncer =
            new_filtered_debouncer(SKILLS_DEBOUNCE, move |res: DebounceEventResult| {
                ...
                for event in events
                    .iter()
                    .filter(|event| event.path.starts_with(&project_hystersis_for_events))
                {
        ...
        let initial = if project_hystersis.is_dir() {
            project_hystersis.clone()
        } else {
            project_root.clone()
        };
        ...
        let refresh_dirs = project_hystersis_refresh_dirs(&project_root);
...
impl SkillsFileWatcher {
    ...
    /// [`collect_skill_config_dirs`](xai_hystersis_agent::prompt::skills::collect_skill_config_dirs).
    ///
    ...
    pub fn start(
        ...
        let dirs_to_watch = xai_hystersis_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &hystersis_home,
            config_paths,
        ...
        Self::start_with_dirs(&dirs_to_watch, &hystersis_home, project_root.as_deref())
    }
    ...
    pub fn start_with_dirs(
        ...
        let plan = plan_skills_watch_targets(dirs_to_watch, hystersis_home, project_root);
...
mod tests {
    ...
    fn is_vendor_config_root_matches_known_names_at_any_tier() {
        ...
        let home = home.path();
        let hystersis_home = home.join(".hystersis");

        assert!(is_vendor_config_root(&hystersis_home, &hystersis_home));
        assert!(is_vendor_config_root(&home.join(".claude"), &hystersis_home));
        assert!(is_vendor_config_root(&home.join(".cursor"), &hystersis_home));
        assert!(is_vendor_config_root(&home.join(".agents"), &hystersis_home));
        assert!(is_vendor_config_root(
            &home.join("repo").join(".hystersis"),
            &hystersis_home
        ));
        assert!(is_vendor_config_root(
        ...
        assert!(!is_vendor_config_root(&home.join("my-skills"), &hystersis_home));
        assert!(!is_vendor_config_root(&home.join(".config"), &hystersis_home));
        assert!(!is_vendor_config_root(
            &home.join("repo").join("my-skills"),
            &hystersis_home
        ));

        let custom_home = home.join("custom-hystersis-home");
    ...
    #[test]
    fn project_hystersis_refresh_dirs_matches_vendor_layout() {
        let project = Path::new("/tmp/repo");
        let hystersis = project.join(".hystersis");
        let dirs = project_hystersis_refresh_dirs(project);

        assert_eq!(dirs.len(), 4);
        assert_eq!(dirs[0], (hystersis.clone(), RecursiveMode::NonRecursive));
        assert_eq!(
            &dirs[1..],
            [
                (hystersis.join("skills"), RecursiveMode::Recursive),
                (hystersis.join("commands"), RecursiveMode::NonRecursive),
                (hystersis.join("workflows"), RecursiveMode::NonRecursive),
            ]
        );
        assert_eq!(dirs[1..], vendor_skill_refresh_dirs(&hystersis));
    }
    ...
    fn start_with_dirs_keeps_parent_only_watch() {
        ...
        let hystersis_home = project.join("home-hystersis");
        fs::create_dir_all(&hystersis_home).unwrap();

        let plan = plan_skills_watch_targets(&[], &hystersis_home, Some(project));
        ...
        let (watcher, _rx) = SkillsFileWatcher::start_with_dirs(&[], &hystersis_home, Some(project))
    ...
    fn plan_skills_watch_targets_scopes_vendors_and_seeds_refresh() {
        ...
        let project_hystersis = project.join(".hystersis");
        let custom = project.join("my-skills");
        fs::create_dir_all(&project_claude).unwrap();
        fs::create_dir_all(&project_hystersis).unwrap();
        fs::create_dir_all(&custom).unwrap();

        let dirs = vec![project_claude.clone(), project_hystersis.clone(), custom.clone()];
        let plan = plan_skills_watch_targets(&dirs, &hystersis_home, Some(project));

        assert_eq!(
            plan.vendor_roots,
            vec![project_claude.clone(), project_hystersis.clone()]
        );
        ...
        let mut expected_refresh: Vec<(PathBuf, RecursiveMode)> =
            vendor_skill_refresh_dirs(&project_claude)
                .into_iter()
                .chain(vendor_skill_refresh_dirs(&project_hystersis))
                .collect();
    ...
    fn plan_skills_watch_targets_multi_vendor_refresh_fanout() {
        let hystersis_home = PathBuf::from("/home/u/.hystersis");
        let a = PathBuf::from("/repo/.claude");
        let b = PathBuf::from("/repo/.agents");
        let plan = plan_skills_watch_targets(&[a.clone(), b.clone()], &hystersis_home, None);
    ...
    fn plan_skills_watch_targets_seeds_all_missing_project_vendor_roots() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let hystersis_home = project.join("elsewhere").join(".hystersis");
        let plan = plan_skills_watch_targets(&[], &hystersis_home, Some(project));
    ...
    fn plan_skills_watch_targets_does_not_double_seed_present_vendor_roots() {
        ...
        let plan = plan_skills_watch_targets(&present, &hystersis_home, Some(project));
    ...
    fn plan_skills_watch_targets_partial_vendors_seed_only_missing() {
        ...
        for name in [".hystersis", ".agents", ".cursor"] {
            let root = project.join(name);
            expected.push((root.clone(), RecursiveMode::NonRecursive));
            expected.extend(vendor_skill_refresh_dirs(&root));
        }
    ...
    #[test]
    fn plan_skills_watch_targets_parent_watches_project_when_hystersis_present_siblings_missing() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let hystersis_home = project.join("home-hystersis");
        let project_hystersis = project.join(".hystersis");
        ...
        let plan = plan_skills_watch_targets(
            std::slice::from_ref(&project_hystersis),
            &hystersis_home,
            Some(project),
        );

        assert_eq!(plan.vendor_roots, vec![project_hystersis.clone()]);
        assert_eq!(plan.project_parent_watch.as_deref(), Some(project));
        assert!(
            !plan
                .refresh_dirs
                .contains(&(project_hystersis.clone(), RecursiveMode::NonRecursive))
        );
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        let hystersis = Path::new("/tmp/project/.hystersis");
        assert_eq!(
            discovery_change_for_path(hystersis),
        ...
        assert_eq!(
            discovery_change_for_path(&hystersis.join("skills")),
            Some(DiscoveryChange::Skills)
        );
        assert_eq!(
            discovery_change_for_path(&hystersis.join("commands")),
            Some(DiscoveryChange::Skills)
        );
        assert_eq!(
            discovery_change_for_path(&hystersis.join("workflows")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&hystersis.join("workflows/review.rhai")),
            Some(DiscoveryChange::Workflows)
        );
        assert_eq!(
            discovery_change_for_path(&hystersis.join("skills/review/SKILL.md")),
            Some(DiscoveryChange::Skills)
        );
    }
    ...
    /// A write to `<hystersis_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another hystersis process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.hystersis/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        let hystersis_home = TempDir::new().unwrap();
        let cwd = TempDir::new().unwrap();
        let project_hystersis = cwd.path().join(".hystersis");
        fs::create_dir_all(&project_hystersis).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_hystersis.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_hystersis.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.hystersis/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.hystersis/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_hystersis = new_cwd.path().join(".hystersis");
        fs::create_dir_all(&project_hystersis).unwrap();
        fs::write(project_hystersis.join("config.toml"), "").unwrap();
