...
fn api_key_id_for(auth: Option<&crate::auth::HystersisAuth>) -> Option<String> {
    auth.filter(|a| matches!(a.auth_mode, crate::auth::AuthMode::ApiKey))
        .map(|a| xai_hystersis_telemetry::config::deployment_id_from_key(&a.key))
}
/// Sampler [`BearerResolver`](xai_hystersis_sampler::BearerResolver) over a live
...
impl WireValidBearerResolver {
    ...
    pub(crate) fn shared(auth_manager: Arc<AuthManager>) -> xai_hystersis_sampler::SharedBearerResolver {
...
impl xai_hystersis_sampler::BearerResolver for WireValidBearerResolver {
...
pub(crate) fn embedding_session_credentials(
...
) -> xai_hystersis_memory::EndpointScopedCredentials {
    ...
    xai_hystersis_memory::EndpointScopedCredentials::for_endpoint(
        embed_base_url,
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(xai_hystersis_version::VERSION, client_identifier)
...
pub(crate) fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        xai_hystersis_telemetry::external::set_identity(
            xai_hystersis_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
pub fn build_default_otel_layer_config() -> xai_hystersis_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let hystersis_com_config = crate::auth::HystersisComConfig::default();
    let exporter = xai_hystersis_telemetry::otel_layer::OtelExporterConfig {
    ...
    xai_hystersis_telemetry::otel_layer::OtelLayerConfig {
...
mod tests {
    ...
    fn wire_valid_resolver_tracks_manager_across_expiry_and_refresh() {
        use xai_hystersis_sampler::BearerResolver;
    ...
    fn embedding_session_credentials_scopes_to_first_party() {
        ...
        let api_key_provider: xai_hystersis_tools::types::SharedApiKeyProvider =
    ...
    fn snapshot_populates_tenant_id_per_auth_mode() {
        use xai_hystersis_telemetry::config::deployment_id_from_key;
        let _guard = EarlyInvalidationGuard::pin_to_default();
        let dir = tempfile::tempdir().unwrap();
