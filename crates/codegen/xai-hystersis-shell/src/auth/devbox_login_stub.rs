...
//! points that can still be reached directly (`hystersis login --devbox`) return a
//! descriptive error.

use super::manager::AuthManager;
use super::model::HystersisAuth;
...
pub(crate) async fn mint_devbox_auth(_auth_manager: &AuthManager) -> anyhow::Result<HystersisAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// Unreachable in practice (guarded by [`is_devbox_environment`]); errors
/// defensively if called.
pub(super) async fn mint_devbox_auth_raw() -> anyhow::Result<HystersisAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// `hystersis login --devbox` entry point: always errors in this build.
