...
//!
//! 1. [`xai_hystersis_telemetry::unified_log::warn`] for the local
//!    `~/.hystersis/logs/unified.jsonl` file (best-effort; ships to GCS
...
//!
//! [`xai_hystersis_sampler`] is intentionally decoupled from this crate. It
//! invokes the trait [`xai_hystersis_sampler::Auth401AttributionCallback`] at
//! its six 401 arms; this module provides [`ShellAttribution`], the
//! concrete impl that the shell wires into
//! [`xai_hystersis_sampler::SamplerConfig::attribution_callback`] at every
...
use xai_hystersis_sampler::{Auth401AttributionCallback, SamplingConsumer};
use xai_hystersis_tools::{Auth401AttributionCallback as ToolAuth401AttributionCallback, ToolConsumer};
...
impl ShellAttribution {
    ...
    /// [`xai_hystersis_sampler::SamplerConfig::attribution_callback`].
    ///
    /// (Returns `Arc<dyn Trait>` rather than `Self` because the
    /// `xai_hystersis_sampler::SamplerConfig` field expects exactly that;
    ...
    /// `Arc<dyn xai_hystersis_tools::Auth401AttributionCallback>` for the
...
/// in `xai-hystersis-tools` emits a 401 attribution event through this
...
/// All 401 emit sites in `xai-hystersis-shell` go through this helper -- the
...
pub(crate) fn record_auth_401(
    ...
    // Sink 1 -- local file (~/.hystersis/logs/unified.jsonl) + scrubbed
...
fn compute_attribution_payload(
    ...
    // `hystersis_com_config.auth_token_ttl` when `expires_at` is `None`
...
mod tests {
    ...
    /// nothing from a developer's actual `~/.hystersis/auth.json` leaks in.
    ...
    fn fresh_auth(key: &str) -> HystersisAuth {
        HystersisAuth {
            key: key.to_string(),
            create_time: Utc::now(),
    ...
    fn hard_expired_auth(key: &str) -> HystersisAuth {
        HystersisAuth {
            key: key.to_string(),
    ...
    /// `ShellAttribution` implements `xai_hystersis_tools::Auth401AttributionCallback`
