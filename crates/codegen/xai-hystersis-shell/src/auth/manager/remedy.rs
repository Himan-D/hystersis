...
pub(crate) enum BoundedRefresh {
    ...
    /// [`SilentRefresh::Renewed`]: `HystersisAuth` is large and the other variant
    /// is unit-sized.
    Resolved(Box<Result<HystersisAuth, AuthError>>),
    ...
    pub(crate) fn auth_remedy(&self) -> AuthRemedy {
        ...
        match (user_must_act, provider_mints_sessions) {
            ...
            (true, true) => AuthRemedy::ProviderLogin {
                label: self.hystersis_com_config().auth_provider_label.clone(),
            },
...
mod tests {
    ...
    use crate::auth::{HystersisComConfig, error::RefreshTokenFailedReason};
    ...
    fn external_credential(expires_at: chrono::DateTime<Utc>) -> HystersisAuth {
        HystersisAuth {
            key: "external".into(),
    ...
    fn provider_manager(dir: &std::path::Path, credential: HystersisAuth) -> Arc<AuthManager> {
    ...
    fn expired_credential_without_a_provider_command_needs_a_manual_login() {
        let dir = tempfile::tempdir().unwrap();
        let manager = Arc::new(AuthManager::new(dir.path(), HystersisComConfig::default()));
    ...
    async fn renewed_carries_the_wire_valid_bearer_the_buffer_hides() {
        ...
        manager.hot_swap(HystersisAuth {
            key: "wire-valid".into(),
            auth_mode: AuthMode::Oidc,
            refresh_token: Some("rt-live".into()),
