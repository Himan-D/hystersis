...
mod tests {
    ...
    struct FakeRunner {
        results: Mutex<Vec<Option<HystersisAuth>>>,
        calls: AtomicU32,
    }
    impl FakeRunner {
        fn new(results: Vec<Option<HystersisAuth>>) -> Self {
            Self {
    ...
    impl ExternalCommandRunner for FakeRunner {
        async fn run_external_command(&self, _command: &str) -> Option<HystersisAuth> {
