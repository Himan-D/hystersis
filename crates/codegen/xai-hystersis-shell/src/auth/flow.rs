...
use xai_hystersis_telemetry::events::{LoginFailed, LoginFailureKind};
...
async fn run_auth_flow_inner(
    ...
    {
        xai_hystersis_telemetry::session_ctx::log_event(event);
    }
    result
}
...
async fn run_auth_flow_steps(
    ...
    // --device-auth / HYSTERSIS_LOGIN_DEVICE_FLOW / [auth] login_device_flow.
...
pub fn perform_logout(
    ...
    if was_logged_in {
        ...
        xai_hystersis_telemetry::external::set_identity(
            xai_hystersis_telemetry::external::IdentityAttrs::default(),
        );
        xai_hystersis_telemetry::external::flush();
...
mod tests {
    ...
    // A hystersis.com first-party (hystersis.com-issuer) OIDC session — `is_xai_auth()` true.
