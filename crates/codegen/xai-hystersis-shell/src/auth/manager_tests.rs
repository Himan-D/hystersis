...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> HystersisAuth {
    HystersisAuth {
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older hystersis version).
...
fn new_scope_takes_precedence_over_legacy() {
    ...
    let legacy_auth = HystersisAuth {
        key: "legacy-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    let new_auth = HystersisAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
async fn team_login_then_personal_evicts_team_token() {
    ...
    let personal_auth = HystersisAuth {
...
/// legacy `https://accounts.hystersis.com/sign-in` entry from auth.json.
#[test]
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(HystersisAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
fn try_use_disk_token_accepts_different_key_on_server_rejected() {
    ...
    let mem_auth = HystersisAuth {
        key: "old-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(mem_auth);

    let disk_auth = HystersisAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
fn try_use_disk_token_skips_disk_token_older_than_memory_mint() {
    ...
    let fresh_mint = HystersisAuth {
        key: "fresh-mint".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(fresh_mint);

    let lagging_disk = HystersisAuth {
        key: "stale-disk".into(),
        ..make_auth(
...
fn try_use_disk_token_lagging_guard_holds_for_buffered_in_memory_token() {
    ...
    let buffered_mint = HystersisAuth {
...
async fn refresh_chain_pre_lock_adopt_ignores_expired_disk_token() {
    ...
    let buffered_disk = HystersisAuth {
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `hystersis login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn start_proactive_refresh_is_idempotent() {
    ...
    let stale_api_key = HystersisAuth {
        key: "stale-api-key".into(),
        auth_mode: AuthMode::ApiKey,
        create_time: Utc::now() - Duration::days(60),
        expires_at: None,
...
async fn refresh_chain_demotes_when_attributed_tried_rt_differs_from_disk() {
    ...
    struct AttributedRejection(HystersisAuth);
    #[async_trait::async_trait]
    ...
    assert!(
        ...
        "demotion must not record a sticky verdict that locks out every \
         sibling process until the user re-runs `hystersis login`",
    );
}
...
async fn permanent_other_retains_credentials() {
    ...
    let mut store = AuthStore::new();
    store.insert(HystersisComConfig::default().auth_scope(), session);
    write_auth_json(&dir.path().join("auth.json"), &store).unwrap();
...
async fn sticky_permanent_allows_refresh_when_attempted_key_differs() {
    ...
    impl crate::auth::refresh::TokenRefresher for CountingOk {
        async fn refresh(
            ...
            self.0.fetch_add(1, Ordering::SeqCst);
            crate::auth::refresh::RefreshOutcome::Success(Box::new(HystersisAuth {
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `hystersis logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `hystersis login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn oidc_refresh_not_blocked_by_model_api_key() {
    ...
    let expired_oidc = HystersisAuth {
...
/// `has_hystersis_code_access`. The `&mut HystersisAuth` signature already
...
/// contributor adding a token-like field to both `HystersisAuth` and
/// `UserInfo` is forced to look here.
#[test]
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = HystersisAuth {
        key: "ROT_KEY".into(),
        ...
        auth_mode: AuthMode::Oidc,
        create_time: Utc::now() - Duration::minutes(10),
        has_hystersis_code_access: Some(true),
    ...
    assert_eq!(disk.has_hystersis_code_access, snapshot.has_hystersis_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    use xai_hystersis_tools::types::ApiKeyProvider;

    let _xai = EnvGuard::unset("XAI_API_KEY");
    let _legacy = EnvGuard::unset("HYSTERSIS_CODE_XAI_API_KEY");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), HystersisComConfig::default()));
    mgr.hot_swap(HystersisAuth {
...
async fn refresh_chain_server_rejected_bypasses_valid_token_double_check() {
    ...
    let valid_but_rejected = HystersisAuth {
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), HystersisComConfig::default()).with_proxy_base_url(&base);

    let mut auth = HystersisAuth {
        key: "tok".into(),
        ..HystersisAuth::test_default()
    };
...
fn oidc_session_for_team(principal_id: &str) -> HystersisAuth {
    HystersisAuth {
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || HystersisAuth {
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), HystersisComConfig::default()));
...
#[test]
#[serial_test::serial(force_dark_wake_env)] // reads HYSTERSIS_AUTH_FORCE_DARK_WAKE
fn is_dark_wake_false_when_power_listener_not_started() {
    let _unset = xai_hystersis_test_support::EnvGuard::unset("HYSTERSIS_AUTH_FORCE_DARK_WAKE");
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), HystersisComConfig::default());
    assert!(
        !mgr.is_dark_wake(),
...
/// `HYSTERSIS_AUTH_FORCE_DARK_WAKE` forces the dark-wake answer for manual and
...
fn is_dark_wake_env_override_forces_both_states() {
    ...
    {
        let _g = EnvGuard::set("HYSTERSIS_AUTH_FORCE_DARK_WAKE", "1");
        assert!(
            mgr.is_dark_wake(),
    ...
    {
        let _g = EnvGuard::set("HYSTERSIS_AUTH_FORCE_DARK_WAKE", "0");
    ...
    }
    {
        ...
        let _g = EnvGuard::set("HYSTERSIS_AUTH_FORCE_DARK_WAKE", "yes");
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use xai_hystersis_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use xai_hystersis_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, HystersisComConfig::default()));
...
fn external_provider_config() -> HystersisComConfig {
    HystersisComConfig {
