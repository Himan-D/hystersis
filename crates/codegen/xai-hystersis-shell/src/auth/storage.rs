...
use super::model::{API_KEY_SCOPE, AuthMode, AuthStore, HystersisAuth};
...
pub fn read_api_key(hystersis_home: &Path) -> Option<String> {
    let path = hystersis_home.join("auth.json");
...
pub fn store_api_key(hystersis_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = hystersis_home.join("auth.json");
...
pub fn clear_api_key(hystersis_home: &Path) -> std::io::Result<()> {
    let path = hystersis_home.join("auth.json");
