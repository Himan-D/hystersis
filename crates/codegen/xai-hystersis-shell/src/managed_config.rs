...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    xai_hystersis_config::MANAGED_CONFIG_FILENAME,
    xai_hystersis_config::REQUIREMENTS_FILENAME,
    xai_hystersis_config::signed_policy::SIGNATURE_SIDECAR_FILE,
    xai_hystersis_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE,
];
...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            xai_hystersis_config::MANAGED_CONFIG_CACHE_FILE,
    ...
    let atomic_write_tmp_prefixes = [
        format!("{}.", xai_hystersis_config::MANAGED_CONFIG_CACHE_FILE),
        format!(
            "{}.",
            xai_hystersis_config::signed_policy::SIGNATURE_SIDECAR_FILE
        ),
...
/// would just 401).
fn eligible_team_principal(auth: HystersisAuth) -> Option<HystersisAuth> {
...
/// config is a hystersis.com feature with one hystersis.com auth.
fn read_active_team_auth() -> Option<HystersisAuth> {
    let home = crate::util::hystersis_home::hystersis_home();
...
pub fn clear_orphan() {
    ...
    if xai_hystersis_config::fail_closed_policy_armed_at(&home) {
        tracing::info!(
...
/// files (TUI tick vs `hystersis login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `hystersis setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `HYSTERSIS_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("HYSTERSIS_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
    ...
    match failure.kind {
        TransportFailureKind::CertificateUntrusted => {
            ManagedConfigError::CertificateUntrusted(certificate_detail(
                failure.detail,
                xai_hystersis_extra_ca::configured_bundle_env(),
                xai_hystersis_extra_ca::extra_root_ders().len(),
            ))
        }
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `xai-hystersis-http` classifier.
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = xai_hystersis_config::signed_policy::stored_envelope_nonce(
        &crate::util::hystersis_home::hystersis_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            xai_hystersis_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
/// Override with `HYSTERSIS_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("HYSTERSIS_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
/// snapshots, and OTel: the **server** HystersisBuildDeployment UUID (the id
...
/// Resolve deployment key from `HYSTERSIS_DEPLOYMENT_KEY` env var, then config files.
...
/// Callers doing auto-fetch should check this; explicit user actions (hystersis setup) skip it.
///
/// Overlay-free: reads the raw config layers via
/// [`crate::config::ConfigLayers::effective_config_base_without_overlay`] rather
/// than the overlay-inclusive effective config, so a `HYSTERSIS_CONFIG` overlay cannot
...
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("HYSTERSIS_MANAGED_CONFIG") {
        return v;
    }
...
/// Fetch managed config + requirements and write to `~/.hystersis/`, trying the
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<HystersisAuth>,
...
/// read-only `hystersis setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if xai_hystersis_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(xai_hystersis_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        xai_hystersis_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        ...
        {
            clear_squatting_dir(
                &home.join(xai_hystersis_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            xai_hystersis_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    ...
    clear_squatting_dir(&home.join(xai_hystersis_config::MANAGED_CONFIG_CACHE_FILE));
...
fn served_principal_of(payload: &xai_hystersis_config::signed_policy::SignedPayload) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
fn verified_claim_sidecar(
...
) -> Option<xai_hystersis_config::signed_policy::SignatureEnvelope> {
    use xai_hystersis_config::signed_policy::now_unix;
    ...
    let claim = match xai_hystersis_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
fn claim_binds_to(
    claim: &xai_hystersis_config::signed_policy::ManagedIdentityClaim,
    served_principal: Option<&str>,
) -> bool {
...
/// Post-login hook for `hystersis login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<HystersisAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `hystersis setup` could install config for.
...
pub fn classify_auth_mode() -> xai_hystersis_telemetry::startup::AuthMode {
    auth_mode(
...
) -> xai_hystersis_telemetry::startup::AuthMode {
    use xai_hystersis_telemetry::startup::AuthMode;
...
pub async fn ensure_managed_policy_present(
    ...
    xai_hystersis_telemetry::startup::enter(xai_hystersis_telemetry::startup::StartupPhase::ManagedPolicy);
    ...
    xai_hystersis_telemetry::startup::set_auth_mode(auth_mode(has_deployment_key, &signed_in_team));
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(HystersisAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// are the non-forgeable layers. Recovery: reconnect / `hystersis setup`; ceasing to serve
...
fn bump_managed_rollback_floor() {
    ...
    if !xai_hystersis_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::hystersis_home::hystersis_home();
    match try_lock_managed_config(&home) {
        Some(_lock) => {
            xai_hystersis_config::bump_rollback_floor(&home);
        }
...
/// Outcome of the `hystersis setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.hystersis`.
    Installed,
...
/// Result of `hystersis setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `hystersis setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `hystersis setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `hystersis setup --json` without writing anything:
...
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    ...
    // Match the installer's trust decision: a payload `hystersis setup` would refuse
...
/// Run the `hystersis setup` sync for the current principal. The caller must check
