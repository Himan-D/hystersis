...
//!
//! The page
//! `crates/codegen/xai-hystersis-pager/docs/user-guide/26-config-reference.md` is the
...
//! `~/.hystersis/docs/user-guide/` on launch.

use std::collections::BTreeMap;
use std::path::PathBuf;

use xai_hystersis_config_types::{FEATURES, KNOWN_MCP_SERVER_FIELDS};
...
/// Nested HystersisComConfig / OAuth2 / OIDC leaves enterprise writes today.
/// Keep in sync with `src/auth/config.rs`.
const HYSTERSIS_COM_CONFIG_LEAVES: &[&str] = &[
    "hystersis_com_config.hystersis_ws_origin",
    "hystersis_com_config.hystersis_ws_url",
    "hystersis_com_config.token_header",
    "hystersis_com_config.auth_provider_label",
    "hystersis_com_config.auth_token_ttl",
    "hystersis_com_config.auth_provider_command",
    "hystersis_com_config.preferred_method",
    "hystersis_com_config.disable_api_key_auth",
    "hystersis_com_config.force_login_team_uuid",
    "hystersis_com_config.oauth2.issuer",
    "hystersis_com_config.oauth2.client_id",
    "hystersis_com_config.oauth2.scopes",
    "hystersis_com_config.oauth2.principal_type",
    "hystersis_com_config.oauth2.principal_id",
    "hystersis_com_config.oauth2.referrer",
    "hystersis_com_config.oidc.issuer",
    "hystersis_com_config.oidc.client_id",
    "hystersis_com_config.oidc.scopes",
    "hystersis_com_config.oidc.audience",
];
...
fn committed_markdown_path() -> PathBuf {
    if let Some(path) = std::env::var_os("HYSTERSIS_CONFIG_REFERENCE_MD") {
        return PathBuf::from(path);
    }
    let root = find_monorepo_root().unwrap_or_else(|| {
        panic!(
            "committed config-reference user-guide not found; set HYSTERSIS_CONFIG_REFERENCE_MD or run from the monorepo (CARGO_MANIFEST_DIR={})",
            env!("CARGO_MANIFEST_DIR")
        )
    });
    root.join(format!(
        "crates/codegen/xai-hystersis-pager/docs/user-guide/{USER_GUIDE_FILENAME}"
    ))
}
...
fn agents_md_path() -> PathBuf {
    if let Some(path) = std::env::var_os("HYSTERSIS_CONFIG_DOCS_AGENTS_MD") {
        return PathBuf::from(path);
    }
    ...
    let root = find_monorepo_root().unwrap_or_else(|| {
        panic!(
            "xai-hystersis-shell AGENTS.md not found; set HYSTERSIS_CONFIG_DOCS_AGENTS_MD or run from the monorepo (CARGO_MANIFEST_DIR={})",
            env!("CARGO_MANIFEST_DIR")
        )
    });
    root.join("crates/codegen/xai-hystersis-shell/AGENTS.md")
}
...
mod tests {
    ...
    #[test]
    fn hystersis_com_config_nested_fields_and_auth_aliases() {
        let (config, _, _) = page();
        let map = by_key(&config);
        for leaf in HYSTERSIS_COM_CONFIG_LEAVES {
            let row = map.get(*leaf).unwrap_or_else(|| panic!("missing {leaf}"));
            let alias = leaf.replacen("hystersis_com_config.", "auth.", 1);
            let alias_row = map
                .get(alias.as_str())
        ...
        assert_eq!(
            map["hystersis_com_config.disable_api_key_auth"].requirements,
            "pin"
        );
        assert_eq!(
            map["hystersis_com_config.force_login_team_uuid"].requirements,
            "pin"
        );
    }
    ...
    #[test]
    fn overlay_free_gates_do_not_claim_hystersis_config() {
        let (config, req, _) = page();
        for row in config.iter().chain(req.iter()) {
            if matches!(
                ...
                assert!(
                    !row.details.contains("HYSTERSIS_CONFIG"),
                    "{} must not claim HYSTERSIS_CONFIG",
                    row.key
                );
            }
        }
    ...
    fn page_is_the_user_facing_field_list() {
        ...
        assert!(!md.contains("Generated from `xai-hystersis-shell`"));
        for leak in [
            "FEATURES",
