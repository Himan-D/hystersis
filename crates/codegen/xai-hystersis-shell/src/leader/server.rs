...
use xai_hystersis_workspace::WorkspaceHandle;
...
async fn handle_workspace_start(
    ...
    let status_config = xai_hystersis_workspace::StatusConfig::from_env();
    ...
    let handle = xai_hystersis_workspace::connect_local_workspace(
        cwd_path.clone(),
        url,
        auth,
        Some(metadata),
