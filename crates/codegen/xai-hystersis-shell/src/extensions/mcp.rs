...
//!
//! - `hystersis.com/mcp/list` — list available MCP servers (agent-scoped or session-annotated)
//! - `hystersis.com/mcp/call` — invoke an MCP tool directly, outside the LLM loop
//! - `hystersis.com/mcp/servers_updated` — local/plugin catalog after launch-dir discovery
//!   or a folder-trust grant (not gateway connectors)
//! - `hystersis.com/mcp/server_status` — per-server delta pushed by the
...
// rmcp is quarantined in xai-hystersis-mcp; see that crate's docs.
use xai_hystersis_mcp::rmcp;
// `wire::MCP_CALL` is the one cross-SDK contract literal; the agent-only siblings
// live in `mcp_methods` below.
use xai_hystersis_mcp::wire;

use super::{ExtResult, parse_params, to_ext_response};

/// Agent-only `hystersis.com/mcp/*` ACP method/notification names.
///
/// Unlike [`wire::MCP_CALL`] (the cross-SDK contract, which stays in
/// `xai_hystersis_mcp::wire`), these methods are private to the agent↔client channel and
...
pub mod mcp_methods {
    ...
    pub const PREFIX: &str = "hystersis.com/mcp/";

    pub const LIST: &str = "hystersis.com/mcp/list";
    pub const READ_RESOURCE: &str = "hystersis.com/mcp/read_resource";
    pub const AUTH_STATUS: &str = "hystersis.com/mcp/auth_status";
    pub const AUTH_TRIGGER: &str = "hystersis.com/mcp/auth_trigger";
    pub const SETUP: &str = "hystersis.com/mcp/setup";
    pub const TOGGLE: &str = "hystersis.com/mcp/toggle";
    pub const TOGGLE_TOOL: &str = "hystersis.com/mcp/toggle_tool";
    pub const UPSERT: &str = "hystersis.com/mcp/upsert";
    pub const DELETE: &str = "hystersis.com/mcp/delete";

    pub const SERVERS_UPDATED: &str = "hystersis.com/mcp/servers_updated";
    pub const TOOLS_CHANGED: &str = "hystersis.com/mcp/tools_changed";
    pub const INIT_PROGRESS: &str = "hystersis.com/mcp/init_progress";
}
...
pub struct McpServerEntry {
    ...
    pub icons: Vec<xai_hystersis_mcp::servers::McpIcon>,
...
// Re-export the `hystersis.com/mcp/server_status` schema +
// method constant from the dispatcher module so external callers
// have a single import point alongside the other `hystersis.com/mcp/*`
// types.
//
...
pub async fn notify_servers_updated(
    ...
    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
        ...
        tracing::info!("Sent hystersis.com/mcp/servers_updated notification to client");
    }
}

// ── Dispatch ────────────────────────────────────────────────────────

/// Inbound `hystersis.com/mcp/*` methods this agent services, resolved from the wire string.
///
...
pub(crate) async fn build_mcp_status(
    ...
    tool_bridge: &Arc<xai_hystersis_tools::bridge::ToolBridge>,
    event_writer: Option<&xai_hystersis_session_events::EventWriter>,
) -> McpStatusSnapshot {
    ...
    for client in &clients {
        ...
        if let Some(ew) = event_writer {
            ew.emit(xai_hystersis_session_events::Event::McpHealthCheck {
...
pub(crate) async fn init_agent_mcp_pool(
    ...
    let noop = xai_hystersis_session_events::EventWriter::noop();
    ...
    let clients: xai_hystersis_mcp::owned_clients::OwnedClients = results
        .into_iter()
...
//
// Implements the `McpResourceProvider` trait from xai-hystersis-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `xai-hystersis-mcp` directly.
...
impl xai_hystersis_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<xai_hystersis_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(xai_hystersis_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<xai_hystersis_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(xai_hystersis_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(xai_hystersis_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(xai_hystersis_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
async fn handle_setup(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let allowlist = &xai_hystersis_workspace::permission::resolution::managed_settings().mcp_allowlist;
...
mod tests {
    ...
    /// The emit-only reverse method (`hystersis.com/mcp/sdk_call`) shares the `hystersis.com/mcp/`
    ...
    fn inbound_sdk_call_has_no_forward_route() {
        ...
        assert_eq!(
            ...
            None,
            "inbound hystersis.com/mcp/sdk_call must not resolve to a forward handler"
        );
    ...
    fn test_mcp_list_response_serialization() {
            ...
            servers: vec![
                McpServerEntry {
                    ...
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("Hystersis CLI".to_string()),
                    },
                    source_label: None,
                    setup: None,
                    setup_values: None,
        ...
        assert_eq!(json["servers"][0]["scopeName"], "Hystersis CLI");
    ...
    fn test_mcp_list_icons_serialization() {
        let entry = McpServerEntry {
            ...
            icons: vec![xai_hystersis_mcp::servers::McpIcon {
                src: "https://example.com/icon.png".to_string(),
                mime_type: Some("image/png".to_string()),
                sizes: Some(vec!["48x48".to_string()]),
                theme: Some(xai_hystersis_mcp::servers::McpIconTheme::Dark),
            }],
            source: McpServerSource::Local,
            source_label: None,
            setup: None,
    ...
    #[test]
    fn hystersis_com_local_name_is_not_managed_in_catalog() {
        let local = acp::McpServer::Http(
            acp::McpServerHttp::new("hystersis_com_slack", "https://mcp.example.test/sse")
                .headers(vec![]),
        );
        let servers = build_mcp_catalog_with_gateway_tools(&[local], None, &Default::default());
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "hystersis_com_slack");
        assert_eq!(servers[0].source, McpServerSource::Local);
        assert!(matches!(servers[0].config, McpServerConfig::Http { .. }));

        let placeholder = disabled_server_placeholder_entry("hystersis_com_slack");
