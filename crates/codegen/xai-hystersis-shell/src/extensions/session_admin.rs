...
//! lifecycle:
//!
//! - `hystersis.com/session/rename`                  rename a session locally + remote
//! - `hystersis.com/session/delete`                  delete a session locally + remote
//! - `hystersis.com/session/update_mcp_servers`      mid-session MCP server swap
//! - `hystersis.com/session/add_local_workspace`     mid-session local workspace add-only (chat)
//! - `hystersis.com/session/fork`                    fork a session into a new one
//! - `hystersis.com/internal/reload_all_mcp_servers` config hot-reload, all sessions
//! - `hystersis.com/internal/reload_project_mcp_servers` config hot-reload, cwd-scoped
//! - `hystersis.com/internal/reload_skills`          skills file watcher fan-out
//! - `hystersis.com/internal/reload_models`          model list hot-reload from config.toml
//! - `hystersis.com/internal/reload_models_cache`    model catalog hot-reload from disk cache
//! - `hystersis.com/internal/auth_cleared`           auth hot-clear cleanup
//! - `hystersis.com/plugins/reload`                  rebuild shared plugin registry
//! - `hystersis.com/commands/list`                   list slash commands
...
use xai_hystersis_telemetry::id::agent_id;
...
pub(crate) async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    match args.method.as_ref() {
        "hystersis.com/session/rename" => handle_session_rename(agent, args).await,
        "hystersis.com/session/delete" => handle_session_delete(agent, args).await,
        "hystersis.com/session/update_mcp_servers" => handle_update_mcp_servers(agent, args).await,
        #[cfg(feature = "local-workspace")]
        "hystersis.com/session/add_local_workspace" => handle_add_local_workspace(agent, args).await,
        "hystersis.com/session/fork" => handle_session_fork(agent, args).await,
        "hystersis.com/plugins/reload" => handle_plugins_reload(agent).await,
        "hystersis.com/commands/list" => handle_commands_list(agent, args).await,
...
/// `_meta.hystersis.com/titleIsManual: false` so followers drop `display_name`
...
/// `_meta.hystersis.com/titleIsManual` so followers can set `display_name`.
...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `hystersis sessions delete <id>` CLI path.
...
async fn handle_add_local_workspace(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    if !agent.is_chat_kind_session(&params.session_id) {
        return Err(acp::Error::invalid_params().data(serde_json::json!({
            ...
            "message": "hystersis.com/session/add_local_workspace is only available on chat-kind sessions",
        })));
    }
...
/// hot-reload watcher when `<cwd>/.hystersis/config.toml`,
...
/// Hot-reload the model catalog from `~/.hystersis/models_cache.json` after an
...
async fn handle_plugins_reload(agent: &MvpAgent) -> ExtResult {
    ...
    // Explicit desktop `hystersis.com/plugins/reload`: force a full local-install re-copy.
    agent
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `hystersis.com/commands/list` (the pull used by hystersis-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .hystersis/plugins under it, plus
