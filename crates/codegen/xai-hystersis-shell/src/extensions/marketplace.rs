...
fn load_filtered_marketplace_sources() -> Vec<xai_hystersis_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                xai_hystersis_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                xai_hystersis_plugin_marketplace::SourceKind::Local { path } => {
...
fn refresh_sources(
    sources: &[xai_hystersis_plugin_marketplace::MarketplaceSource],
    ...
    for source in sources {
        if let Some(filter) = source_url_or_path {
            let identity = match &source.kind {
                ...
                xai_hystersis_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
            };
            if identity != filter {
                continue;
            }
        }
        if let xai_hystersis_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = xai_hystersis_plugin_marketplace::git::default_cache_root();
            if let Err(e) = xai_hystersis_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
...
async fn handle_update(
    ...
    use xai_hystersis_plugin_marketplace::installer;
    use xai_hooks_plugins_types::{ActionOutcome, OutcomeStatus};

    let sources = load_filtered_marketplace_sources();

    let source_identity = |s: &xai_hystersis_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            xai_hystersis_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            xai_hystersis_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match xai_hystersis_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        xai_hystersis_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = xai_hystersis_plugin_marketplace::git::default_cache_root();
            match xai_hystersis_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                xai_hystersis_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = xai_hystersis_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = xai_hystersis_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    let mut registry = xai_hystersis_agent::plugins::install_registry::InstallRegistry::load();
    ...
    match update_result {
        ...
        Err(xai_hystersis_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
        ...
        let marketplace_root = match &source.kind {
            ...
            xai_hystersis_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match xai_hystersis_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    xai_hystersis_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
...
fn scan_source(
    source: &xai_hystersis_plugin_marketplace::MarketplaceSource,
    ...
    let scan = xai_hystersis_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = xai_hystersis_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match xai_hystersis_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: xai_hystersis_plugin_marketplace::MarketplaceEntry,
...
async fn handle_add_source(url: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &xai_hystersis_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let existing = xai_hystersis_plugin_marketplace::load_sources(&config);
    let already_configured = match &input {
        MarketplaceAddInput::GitUrl(git_url) => {
            let normalized = git_url.trim_end_matches(".git");
            existing.iter().any(|s| {
                matches!(&s.kind, xai_hystersis_plugin_marketplace::SourceKind::Git { url: u, .. }
                    if u.trim_end_matches(".git") == normalized)
            })
        }
        MarketplaceAddInput::LocalPath(path) => existing.iter().any(|s| {
            matches!(&s.kind, xai_hystersis_plugin_marketplace::SourceKind::Local { path: p }
                if p == path)
        }),
    };
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            xai_hystersis_plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
    ...
    let is_official = matches!(&input, MarketplaceAddInput::GitUrl(u)
        if xai_hystersis_plugin_marketplace::is_official_source_url(u));
    let name = if is_official {
        xai_hystersis_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string()
    } else {
        match &input {
    ...
    let config_path = xai_hystersis_config::hystersis_home().join("config.toml");
    let hystersis_home = xai_hystersis_config::hystersis_home();
...
fn remove_source_locked(source_url_or_path: &str) -> xai_hooks_plugins_types::ActionOutcome {
    ...
    let is_official = xai_hystersis_plugin_marketplace::is_official_source_url(source_url_or_path);
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a xai_hystersis_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub(crate) fn purge_default_skills_installs(hystersis_home: &std::path::Path) {
    purge_default_skills_installs_impl(hystersis_home, || {
        xai_hystersis_agent::plugins::install_registry::InstallRegistry::try_load_from(
            xai_hystersis_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}
...
fn purge_default_skills_installs_impl(
    ...
    load_registry: impl FnOnce() -> Result<
        xai_hystersis_agent::plugins::install_registry::InstallRegistry,
        xai_hystersis_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = hystersis_home.join("config.toml");
...
pub(crate) fn ensure_official_marketplace_source(hystersis_home: &std::path::Path) {
    ...
    let toml_sources = xai_hystersis_plugin_marketplace::load_sources(&parsed);
    let json_sources = xai_hystersis_plugin_marketplace::load_extra_sources_from_settings_in(
        &toml_sources,
        std::slice::from_ref(&hystersis_home.to_path_buf()),
    );
    let already_present = toml_sources.iter().chain(json_sources.iter()).any(|s| {
        matches!(&s.kind, xai_hystersis_plugin_marketplace::SourceKind::Git { url, .. }
            if xai_hystersis_plugin_marketplace::is_official_source_url(url))
    });
    ...
    let write_result = if already_present {
        ...
        add_marketplace_source(
            &config_path,
            xai_hystersis_plugin_marketplace::OFFICIAL_SOURCE_NAME,
            &crate::plugin::MarketplaceAddInput::GitUrl(
                xai_hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string(),
            ),
            true,
        )
    };
    ...
    match write_result {
        Ok(()) if !already_present => {
            tracing::info!(
                url = xai_hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<xai_hystersis_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        xai_hystersis_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            xai_hystersis_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn removing_last_nonofficial_source_preserves_flag_and_blocks_readd() {
        ...
        assert!(
            !sources.iter().any(|s| matches!(&s.kind,
                xai_hystersis_plugin_marketplace::SourceKind::Git { url, .. }
                    if xai_hystersis_plugin_marketplace::is_official_source_url(url))),
    ...
    fn first_run_creates_source_and_sets_flag() {
        ...
        assert!(matches!(
            &sources[0].kind,
            xai_hystersis_plugin_marketplace::SourceKind::Git { url, .. }
                if url == xai_hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL
        ));
    ...
    fn pre_existing_official_source_just_sets_flag() {
        ...
        std::fs::write(
            ...
            format!(
                "[[marketplace.sources]]\nname = \"{}\"\ngit = \"{}\"\n",
                xai_hystersis_plugin_marketplace::OFFICIAL_SOURCE_NAME,
                xai_hystersis_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use xai_hystersis_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = xai_hystersis_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
