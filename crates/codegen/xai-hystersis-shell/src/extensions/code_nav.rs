...
//! | Method | Description |
//! |--------|-------------|
//! | `hystersis.com/code/goto-definition` | Definition location(s) for symbol at position |
//! | `hystersis.com/code/goto-references` | Reference location(s) for symbol at position |
//! | `hystersis.com/code/find-definitions` | All definitions of a symbol by name |
//! | `hystersis.com/code/find-references` | All references to a symbol by name |
//! | `hystersis.com/code/status` | Indexing status |
...
/// Reason string for the `hystersis.com/code/status` response.
///
...
pub async fn handle(
    ...
    use xai_hystersis_workspace::workspace_ops::*;

    match args.method.as_ref() {
        "hystersis.com/code/goto-definition" => {
        ...
        "hystersis.com/code/goto-references" => {
            let req: GotoRequest = serde_json::from_str(args.params.get())
                .map_err(|e| acp::Error::invalid_params().data(format!("invalid params: {e}")))?;
            let cwd = resolve_cwd(agent, req.cwd.clone(), req.session_id.as_ref())?;
            let was_newly_started =
        ...
        "hystersis.com/code/find-definitions" => {
        ...
        "hystersis.com/code/find-references" => {
            let req: FindSymbolRequest = serde_json::from_str(args.params.get())
                .map_err(|e| acp::Error::invalid_params().data(format!("invalid params: {e}")))?;
            let cwd = resolve_cwd(agent, req.cwd.clone(), req.session_id.as_ref())?;
            let was_newly_started =
...
fn to_code_nav_ext_response(resp: xai_hystersis_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for hystersis-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise hystersis.com/codeNavigation.enabled to use code navigation"
        }
