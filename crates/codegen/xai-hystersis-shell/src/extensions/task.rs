...
use xai_hystersis_tools::types::{KillOutcome, KillSource, TaskSnapshot};
...
/// Wire DTO for the `hystersis.com/task/kill` ext request.
///
/// `pub` (with both serde directions) so ACP clients (xai-hystersis-pager) build
...
/// Client-facing kill reason on `hystersis.com/task/kill`. Older clients omit it.
...
/// Wire DTO for the `hystersis.com/task/kill` ext response payload (nested under
...
/// Wire DTO for the `hystersis.com/subagent/cancel` ext request.
///
/// `pub` (with both serde directions) so ACP clients (xai-hystersis-pager) build
...
/// Wire DTO for the `hystersis.com/subagent/cancel` response payload (under `result` in
...
/// Handle `hystersis.com/scheduler/*` extension methods.
...
/// Handle `hystersis.com/subagent/*` extension methods.
pub(crate) async fn handle_subagent(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "hystersis.com/subagent/list_running" => {
...
mod tests {
    ...
    // ── hystersis.com/subagent/cancel outcome wire DTO ──────────────────────────
