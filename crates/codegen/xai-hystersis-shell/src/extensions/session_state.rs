//! `hystersis.com/session/state` reads a session's metadata columns; `hystersis.com/session/import`
...
/// `hystersis.com/session/state`: return metadata columns keyed by logical name. Errors when
/// the session isn't found on this host, since it reads a single record whose absence
/// is not an empty result (unlike the collection returned by `hystersis.com/session/updates`).
...
/// `hystersis.com/session/import`: recreate a session on this host from mirrored columns and
...
pub(crate) async fn handle_import(args: &acp::ExtRequest) -> ExtResult {
    ...
    if !has_local_session {
        ...
        crate::util::hystersis_home::ensure_sessions_cwd_dir(&request.cwd)
...
fn sanitize_summary_for_host(summary: &mut serde_json::Map<String, Value>, id: &str, cwd: &str) {
    ...
    set_or_remove(
        summary,
        "hystersis_home",
        crate::session::persistence::hystersis_home_string(),
    );
    set_or_remove(
        summary,
        "sandbox_profile",
        xai_hystersis_sandbox::configured_profile_name().map(String::from),
    );
}
