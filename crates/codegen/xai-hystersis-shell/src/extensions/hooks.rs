...
use xai_hystersis_hooks::event::{HookEventEnvelope, HookEventName};
use xai_hystersis_hooks::matcher::HookMatcher;
...
pub(crate) fn hook_spec_to_info(spec: &xai_hystersis_hooks::config::HookSpec) -> HookInfo {
    use xai_hystersis_hooks::event::HookEventName;
    ...
    HookInfo {
        ...
        disabled: xai_hystersis_hooks::trust::is_hook_disabled(&spec.name),
    }
}
...
pub(crate) const ADVERTISED_BLOCKING_EVENTS: &[xai_hystersis_hooks::event::HookEventName] = &[
    xai_hystersis_hooks::event::HookEventName::PreToolUse,
    xai_hystersis_hooks::event::HookEventName::Stop,
    xai_hystersis_hooks::event::HookEventName::SubagentStop,
];
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern)
            if event.traits().matcher == xai_hystersis_hooks::event::MatcherPolicy::Ignored =>
        {
...
mod tests {
    ...
    fn advertised_blocking_events_are_gates() {
        use xai_hystersis_hooks::event::GateKind;
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use xai_hystersis_hooks::event::{HookEventEnvelope, HookPayload};
