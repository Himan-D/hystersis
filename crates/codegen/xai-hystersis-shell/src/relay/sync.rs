...
/// Format: https://hystersis.com/build/{sessionId}
pub(crate) fn build_share_url(session_id: &str) -> String {
    let base_url =
        std::env::var("HYSTERSIS_CODE_WEB_URL").unwrap_or_else(|_| "https://hystersis.com".to_string());
...
impl RelaySyncState {
    ...
    pub fn save(&self, session_dir: &std::path::Path) -> std::io::Result<()> {
        crate::util::hystersis_home::create_dir_all_owner_only(session_dir)?;
...
fn handle_relay_message(
    ...
    match method {
        Some("initialize") => {
            ...
            if let Some(request_id) = id {
                ...
                *initialized = true;
                ...
                let upsert = json!({
                    "jsonrpc": "2.0",
                    "method": "_hystersis.com/session/upsert",
                    "params": {
                        "sessionId": session_id,
                        "cwd": cwd,
                    }
        ...
        Some("_hystersis.com/relay/initialized") => {
...
mod tests {
    ...
    fn test_build_share_url_default() {
        ...
        assert_eq!(url, "https://hystersis.com/build/test-session-123");
    }
    ...
    fn test_build_share_url_with_uuid() {
        ...
        assert_eq!(
            url,
            "https://hystersis.com/build/01937d8a-1234-7abc-9def-0123456789ab"
        );
    }
