...
use xai_hystersis_agent::plugins::discovery::PluginScope;
use xai_hystersis_agent::plugins::git_install::{self, UpdateStatus};
...
use xai_hystersis_plugin_marketplace::git::{self, SourceCacheLease};
use xai_hystersis_plugin_marketplace::{
...
/// When `keep_data` is true, `~/.hystersis/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &xai_hystersis_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `hystersis plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `hystersis plugin install ./{name}`, or add a \
                     source with `hystersis plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `hystersis plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `hystersis plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
/// both tighten-only: neither a remote campaign nor the `HYSTERSIS_CONFIG` /
/// `HYSTERSIS_CONFIG_PATH` overlay must be able to relax a local security policy, and
...
pub(crate) fn marketplace_require_sha() -> bool {
    xai_hystersis_config::ConfigLayers::load()
        .map(|layers| {
            xai_hystersis_plugin_marketplace::load_require_sha(
                &layers.effective_config_base_without_overlay(),
            )
        })
        .unwrap_or_else(|_| xai_hystersis_plugin_marketplace::env_require_sha())
}
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &xai_hystersis_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.hystersis/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user hystersis via user_hystersis_home() (None when no home resolves) and
    // home separately, so removal still runs from $HYSTERSIS_HOME when no home dir
    // exists, and never touches a cwd-relative .hystersis.
    let home = dirs::home_dir();
    let hystersis = xai_hystersis_config::user_hystersis_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref hystersis) = hystersis {
        settings_candidates.push(hystersis.join("settings.local.json"));
        settings_candidates.push(hystersis.join("settings.json"));
    }
    ...
    if let Some(ref hystersis) = hystersis {
        known_candidates.push(hystersis.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("hystersis plugin install ./sentry"), "{msg}");
        assert!(msg.contains("hystersis plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("hystersis plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> xai_hystersis_workspace::permission::resolution::MarketplaceAllowlist {
        xai_hystersis_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = xai_hystersis_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
