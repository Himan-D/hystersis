//! `hystersis inspect` — configuration introspection.
//!
//! Shows everything Hystersis discovers in the current directory: project
...
use xai_hystersis_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[hystersis_com_config]`
...
pub(crate) struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `HYSTERSIS_DISABLE_API_KEY_AUTH`).
...
pub(crate) struct ConfigSources {
    ...
    /// .hystersis/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `hystersis inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
/// Write the report. A closed stdout (`hystersis inspect | head`) is a clean stop.
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = xai_hystersis_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = xai_hystersis_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = xai_hystersis_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        hystersis_version: xai_hystersis_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == hystersis_home.join("rules"))
        || has_rules_directory(file_path, ".hystersis")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = xai_hystersis_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        xai_hystersis_agent::prompt::skills::CompatConfig::default(),
    )
    .await;
    ...
    // `xai_hystersis_agent::prompt::agents_md::read_agents_config_with_paths`
    ...
    configs
        ...
        .map(|c| {
            let file_type =
                instruction_file_type(&c.file_path, &hystersis_home, imported, &extra_rule_prefixes);
            let scope = instruction_scope(&c.file_path, &hystersis_home, &vendor_homes, &workspace_root);
...
/// which handles both Hystersis TOML and vendor settings fallback in one codepath.
async fn list_permissions(cwd: &Path, project_trusted: bool) -> PermissionsReport {
    use xai_hystersis_workspace::permission::resolution;
...
/// (`[hystersis_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let hystersis_com_config = config
        .map(|c| c.hystersis_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: hystersis_com_config.api_key_auth_disabled(),
        disable_api_key_auth: hystersis_com_config.disable_api_key_auth,
        force_login_team_uuid: hystersis_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[xai_hystersis_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = xai_hystersis_tools::types::compat::CompatConfig::default();
    ...
    let config_layers = xai_hystersis_config::hook_config_layers();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            use xai_hystersis_hooks::config::HookOrigin as O;
            ...
            let source = match xai_hystersis_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(xai_hystersis_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(xai_hystersis_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(xai_hystersis_config::USER_CONFIG_FILENAME),
                },
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &xai_hystersis_agent::plugins::PluginRegistry,
    skills_config: &xai_hystersis_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = xai_hystersis_agent::prompt::skills::list_skills_with_plugins(
...
fn slash_name_counts(
    skills: &[xai_hystersis_agent::prompt::skills::SkillInfo],
    ...
    for skill in skills.iter().filter(|s| s.user_invocable && s.enabled) {
        ...
        *counts
            .entry(
                xai_hystersis_tools::implementations::skills::skill::format_skill_name(skill)
                    .to_lowercase(),
            )
            .or_default() += 1;
    }
...
fn slash_collision(
    skill: &xai_hystersis_agent::prompt::skills::SkillInfo,
    ...
    let qualified =
        xai_hystersis_tools::implementations::skills::skill::format_skill_name(skill).to_lowercase();
...
fn skill_entry_source(s: &xai_hystersis_agent::prompt::skills::SkillInfo) -> ConfigSource {
    use xai_hystersis_tools::implementations::skills::types::SkillScope;
...
fn list_agents(
    ...
    let agents = xai_hystersis_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
...
fn list_plugins(discovered: &[xai_hystersis_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                xai_hystersis_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                xai_hystersis_agent::plugins::PluginScope::Project => Scope::Project,
                xai_hystersis_agent::plugins::PluginScope::User => Scope::User,
                xai_hystersis_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: xai_hystersis_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    xai_hystersis_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = xai_hystersis_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `hystersis mcp doctor` (no loaded RemoteSettings in a standalone command).
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// `.hystersis/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_hystersis_home() {
    ...
    let inline_env = crate::config::HYSTERSIS_CONFIG_ENV;
    let path_env = crate::config::HYSTERSIS_CONFIG_PATH_ENV;
...
fn print_human(r: &InspectReport, out: &mut impl Write) -> std::io::Result<()> {
    ...
    writeln!(out, "  {TREE} Version: {} [{}]", r.hystersis_version, r.channel)?;
    ...
    if r.mcp_servers.is_empty() {
        ...
        writeln!(out, "  {TREE} (none) \u{2014} see `hystersis mcp add --help`")?;
    } else {
        print_columns(
            out,
            "MCP Servers",
...
mod tests {
    use super::*;
    use xai_hystersis_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use xai_hystersis_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for (vendor, path) in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.hystersis"), false, &[]);
        ...
        for path in ["/repo/.hystersis/rules/team.md", r"C:\repo\.hystersis\rules\team.md"] {
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.hystersis"), false, &[]),
                "rules"
            );
        }
        for path in [
            ...
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.hystersis"), true, &[]),
                "rules"
            );
        }
        for path in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.hystersis"), true, &[]);
    ...
    #[test]
    fn hystersis_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let hystersis_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, hystersis_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.hystersis/rules/project.md",
    ...
    fn vendor_home_nested_in_workspace_keeps_direct_surfaces_global() {
        ...
        for path in ["/repo/.claude/rules/global.md", "/repo/.claude/CLAUDE.md"] {
            assert!(matches!(
                instruction_scope(path, Path::new("/other/hystersis"), &vendor_homes, workspace),
                Scope::Global
            ));
        }
        for path in [
    ...
    #[test]
    fn workspace_scope_wins_inside_hystersis_home() {
        let hystersis_home = Path::new("/custom/hystersis");
        let workspace = Path::new("/custom/hystersis/worktrees/repo");
        for path in [
            "/custom/hystersis/worktrees/repo/.cursor/rules/project.md",
            "/custom/hystersis/worktrees/repo/src/AGENTS.md",
        ] {
            assert!(matches!(
                instruction_scope(path, hystersis_home, &[], workspace),
                Scope::Project
        ...
        assert!(matches!(
            instruction_scope("/custom/hystersis/rules/global.md", hystersis_home, &[], workspace,),
            Scope::Global
        ));
    }

    #[test]
    fn custom_hystersis_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.hystersis/requirements.toml";
    ...
    fn config_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("hystersis-4.5"));

        let human = render_config_warnings(&warnings);
        assert!(human.contains("Config Warnings"), "{human}");
        assert!(
            human.contains("[model.\"hystersis-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"hystersis-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["key"], "hystersis-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let s = skill_fixture("a", "/repo/.hystersis/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.hystersis/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.hystersis/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(skill_entry_source(&s), ConfigSource::User { .. }));

        let s = skill_fixture(
            "d",
            "/home/u/.hystersis/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s),
        ...
        let s = skill_fixture("e", "/home/u/.hystersis/bundled/e/SKILL.md", SkillScope::Bundled);
        assert!(matches!(
            skill_entry_source(&s),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.hystersis dirs.
        ...
        let registry = xai_hystersis_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
