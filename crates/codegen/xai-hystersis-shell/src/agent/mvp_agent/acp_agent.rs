...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        xai_hystersis_telemetry::unified_log::info("agent initialized", None, None);
        xai_hystersis_telemetry::startup::mark_agent_serving();
        ...
        let hystersis_home = xai_fast_worktree::resolve_hystersis_home();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    xai_hystersis_workspace::permission::cleanup_stale_permission_state(
        ...
        xai_hystersis_workspace::trust::migrate_legacy_hook_trust();
        if let Some(auth) = self.auth_manager.current() {
            ...
            xai_hystersis_telemetry::unified_log::info(
                ...
                Some(
                    ...
                    "key_prefix": xai_hystersis_auth::bearer_suffix(&auth.key),
                    "rt_prefix": auth.refresh_token.as_deref().map(xai_hystersis_auth::bearer_suffix),
                }),
                ),
            );
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let pre = self
            ...
            .current()
            .map(|a| (
                xai_hystersis_auth::bearer_suffix(&a.key).to_owned(),
                a
                    .refresh_token
                    .as_deref()
                    .map(|t| xai_hystersis_auth::bearer_suffix(t).to_owned()),
            ));
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    ...
                    (xai_hystersis_mcp::wire::MCP_SDK): true,
                    ...
                    "agentVersion": xai_hystersis_version::VERSION,
                    "agentId": agent_id(),
    ...
    async fn authenticate(
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::XAI_API_KEY_METHOD_ID => {
                ...
                log_event(xai_hystersis_telemetry::events::Login {
    ...
    async fn prompt(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match xai_hystersis_sampling_types::ToolOverridesUpdate::parse(value) {
            ...
            Ok(turn_ok) => {
                ...
                if let Some(ctx) = trace_context {
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = xai_hystersis_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = xai_hystersis_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "hystersis.com/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "hystersis.com/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| xai_hystersis_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "hystersis.com/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "hystersis.com/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| xai_hystersis_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == xai_hystersis_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                xai_hystersis_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            xai_hystersis_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
