...
impl MvpAgent {
    ...
    pub(crate) fn remove_session(&self, id: &acp::SessionId) {
        let _ = self
            .subagent_event_tx
            .send(xai_hystersis_tools::implementations::hystersis_build::task::types::SubagentEvent::TeardownSession {
        ...
        self.log_resource_usage(xai_hystersis_telemetry::events::ResourceReportTrigger::SessionClose);
    }
    ...
    /// Counts for `hystersis.com/debug/agent`, including maps outside the registry.
...
/// Field names are the wire contract of `hystersis.com/debug/agent`'s `registries`
