...
//! (`hystersis.com/folder_trust/request`) that asks a GUI client (hystersis-desktop) to decide
...
//! `hystersis.com/folderTrust.interactive` AND the folder-trust feature flag is on AND the
//! verdict is [`xai_hystersis_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.hystersis/lsp.json` servers start on the NEXT session open (the durable grant
...
/// ACP `hystersis.com/folder_trust/request` payload (agent → GUI client). Serialized as
...
/// ACP `hystersis.com/folder_trust/request` response (GUI client → agent).
...
impl MvpAgent {
    /// Parse the `hystersis.com/folderTrust.interactive` capability from an initialize
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("hystersis.com/folderTrust"))
    ...
    /// `hystersis.com/folderTrust.interactive` AND [`folder_trust::prompt_warranted`]
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = xai_hystersis_workspace::trust::workspace_key(cwd);
        ...
        self.session_registry.for_each_resident(|_, h| {
            if xai_hystersis_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("hystersis.com/folder_trust/request", raw_params.into());
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a xai_hystersis_agent::plugins::SharedPluginRegistryHandle,
    compat: &'a xai_hystersis_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.hystersis/hooks`, `.cursor/hooks.json`),
...
mod tests {
    ...
    fn parse_interactive_trust_capability_present_and_true() {
        ...
        meta.insert(
            "hystersis.com/folderTrust".to_string(),
