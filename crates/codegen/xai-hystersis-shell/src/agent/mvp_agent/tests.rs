...
fn harness_pair(id: &str) -> Vec<xai_hystersis_sampling_types::conversation::ConversationItem> {
    use xai_hystersis_sampling_types::ToolCall;
    use xai_hystersis_sampling_types::conversation::ConversationItem;
    vec![
...
async fn upload_harness_trace_turns_numbers_siblings_and_persists_counter() {
    ...
    let queue = crate::upload::trace::spawn_upload_queue(
        queue_home.path(),
        &queue_cfg,
        Some(xai_hystersis_version::VERSION),
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = xai_hystersis_agent::AgentDefinition::from_json(&serde_json::json!(
...
fn inherited_harness_template_skips_nonstrict_model() {
    use xai_hystersis_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    assert_eq!(
        def.model,
        xai_hystersis_agent::config::ModelOverride::Override("test-model-123".to_string()),
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use xai_hystersis_tools::computer::local::{LocalFs, LocalTerminalBackend};
    use xai_hystersis_tools::notification::ToolNotificationHandle;
    use xai_hystersis_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = xai_hystersis_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: xai_hystersis_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: xai_hystersis_tools::implementations::hystersis_build::image_gen::ImageGenConfig::default(),
        video_gen_config: xai_hystersis_tools::implementations::hystersis_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: xai_hystersis_tools::implementations::hystersis_build::app_builder::AppBuilderDeployerConfig::default(),
        api_key_provider: None,
        auth_provider: None,
        attribution_callback: None,
        system_reminder_tag: xai_hystersis_tools::reminders::DEFAULT_REMINDER_TAG,
    };
    let toolset = builder
...
fn make_test_handle(
    ...
    crate::session::SessionHandle {
        ...
        tool_context: crate::tools::ToolContext::new_local_context(
            xai_hystersis_paths::AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap(),
            std::sync::Arc::new(xai_hystersis_workspace::file_system::LocalFs::new(
                std::path::PathBuf::from("/tmp"),
            )),
        ...
        permission_handle: xai_hystersis_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        ...
        hook_registry: None,
        workspace_ops: xai_hystersis_workspace::WorkspaceOps::for_test(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use xai_hystersis_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn apply_supported_effort_assigns_only_when_supported() {
    ...
    use xai_hystersis_sampling_types::ReasoningEffort;
    let agent = build_minimal_agent_for_tests();
...
async fn restore_effort_via_load(
    initial: Option<xai_hystersis_sampling_types::ReasoningEffort>,
    persisted: Option<xai_hystersis_sampling_types::ReasoningEffort>,
) -> Option<xai_hystersis_sampling_types::ReasoningEffort> {
    use crate::agent::config::{EndpointsConfig, ModelEntry};
    let agent = build_minimal_agent_for_tests();
    let mut entry = ModelEntry::fallback("effort-model", &EndpointsConfig::default());
    entry.info.supports_reasoning_effort = true;
...
fn personal_xai_oauth_auth() -> crate::auth::HystersisAuth {
    crate::auth::HystersisAuth {
...
async fn feedback_trace_offer_asks_personal_oauth_accounts() {
    use xai_hystersis_test_support::EnvGuard;
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use xai_hystersis_tools::implementations::hystersis_build::video_gen::{
...
async fn prepare_video_gen_config_respects_feature_flag() {
    use xai_hystersis_tools::implementations::hystersis_build::video_gen::VideoGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
    assert!(matches!(
        agent.prepare_video_gen_config(),
...
async fn prepare_image_gen_config_fails_open_without_auth() {
    use xai_hystersis_tools::implementations::hystersis_build::image_gen::ImageGenConfig;
    let agent = build_minimal_agent_for_tests();
    agent.sampling_config.borrow_mut().api_key = Some("test-key".to_string());
...
fn search_index_env() -> (tempfile::TempDir, [xai_hystersis_test_support::EnvGuard; 2]) {
    use xai_hystersis_test_support::EnvGuard;
...
async fn search_index_honors_the_session_search_feature() {
    ...
    {
        let _off = xai_hystersis_test_support::EnvGuard::set("HYSTERSIS_SESSION_SEARCH", "0");
        let agent = build_agent_with_auth(crate::auth::HystersisAuth::test_default());
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = xai_hystersis_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = xai_hystersis_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(xai_hystersis_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<xai_hystersis_workspace::permission::PermissionEvent>();
    agent
        .session_registry
...
impl Drop for RestoreOtelGate {
    fn drop(&mut self) {
        xai_hystersis_telemetry::external::mark_external_otel_settings_resolved();
    }
}
...
async fn post_auth_settings_xai_upgrades_writeback_emits_and_opens_gate() {
    ...
    let server = xai_hystersis_test_support::MockInferenceServer::start()
        .await
        .unwrap();
    ...
    xai_hystersis_telemetry::external::suppress_external_otel_until_settings();
    assert!(!xai_hystersis_telemetry::external::is_settings_gate_open());
    agent.maybe_fetch_post_auth_settings().await;
    assert_eq!(
        agent.storage_mode(),
    ...
    assert!(
        xai_hystersis_telemetry::external::is_settings_gate_open(),
...
async fn settings_self_heal_refetches_after_token_rotation() {
    ...
    use crate::auth::{HystersisAuth, XAI_OAUTH2_ISSUER};
    let _restore = RestoreOtelGate;
    let server = xai_hystersis_test_support::MockInferenceServer::start_with_required_auth(
        vec![xai_hystersis_test_support::MockModelEntry::new("hystersis")],
        "rotated-key",
    )
    .await
    .unwrap();
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.insert_resident(&sid, handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                xai_hystersis_subagent_resolution::config::SubagentRole {
...
fn project_roles_personas_gated_via_resolve_and_record_chain() {
    ...
    let _sim = EnvGuard::set(xai_hystersis_version::TEST_VERSION_ENV, "0.0-sim");
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use xai_hystersis_test_support::EnvGuard;
    use xai_hystersis_workspace::trust::{TrustStore, workspace_key};
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("HYSTERSIS_HOME", home.path());
    let _sim = EnvGuard::set(xai_hystersis_version::TEST_VERSION_ENV, "0.0-sim");
    let _flag = EnvGuard::unset("HYSTERSIS_FOLDER_TRUST");
...
fn ann(id: &str) -> xai_hystersis_announcements::RemoteAnnouncement {
    xai_hystersis_announcements::RemoteAnnouncement {
...
/// fields as needed).
fn settings_with(
    announcements: Option<Vec<xai_hystersis_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = xai_hystersis_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = xai_hystersis_announcements::RemoteAnnouncement {
...
fn status_line_meta(enabled: bool) -> acp::Meta {
    let mut meta = acp::Meta::new();
    meta.insert(
        xai_hystersis_status_line::CLIENT_STATUS_LINE_META.to_string(),
...
fn init_advertising_status_line(enabled: bool) -> acp::InitializeRequest {
    let mut meta = serde_json::Map::new();
    meta.insert(
        xai_hystersis_status_line::STATUS_LINE_CAPABILITY.to_string(),
        serde_json::json!(enabled),
    );
    acp::InitializeRequest::new(acp::ProtocolVersion::V1).client_capabilities(
        acp::ClientCapabilities::new()
