...
use xai_hystersis_telemetry::id::{agent_id, agent_instance_id};
use xai_hystersis_telemetry::session_ctx::log_event;
use xai_hystersis_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use xai_hystersis_workspace::permission::{ClientType, PermissionEvent};
...
use xai_hystersis_sampler::SamplerConfig as SamplingConfig;
...
use xai_hystersis_paths::AbsPathBuf;
use xai_hystersis_workspace::session::git::GitDiscoveryResult;
...
pub(crate) struct SessionSpawnOptions<'a> {
    ...
    pub envrc: Option<xai_hystersis_workspace::envrc::EnvrcLoad>,
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a xai_hystersis_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[xai_hystersis_announcements::RemoteAnnouncement]>,
    last_emitted: &[xai_hystersis_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<xai_hystersis_announcements::RemoteAnnouncement>> {
    let current = xai_hystersis_announcements::filter_expired_at(
...
pub struct MvpAgent {
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        xai_hystersis_tools::implementations::hystersis_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: xai_hystersis_tools::implementations::hystersis_build::monitor::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: xai_hystersis_agent::plugins::SharedPluginRegistryHandle,
    ...
    workspace_ops: RefCell<Option<xai_hystersis_workspace::WorkspaceOps>>,
    ...
    last_emitted_announcements: RefCell<Vec<xai_hystersis_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &xai_hystersis_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<xai_hystersis_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !xai_hystersis_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = xai_hystersis_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use xai_hystersis_agent::config::is_strict_harness_agent_type;
    match (
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = xai_hystersis_tools::types::TaskSnapshot {
                ...
                completed: true,
                kind: xai_hystersis_tools::computer::types::TaskKind::Bash,
                block_waited: false,
