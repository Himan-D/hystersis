...
/// Router for hystersis.com/session/* and hystersis.com/session_summaries/* methods.
pub(crate) async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "hystersis.com/session/info" => handle_session_info(agent, args).await,
        "hystersis.com/session/close" => handle_session_close(agent, args).await,
        "hystersis.com/session/list" => handle_session_list(agent, args).await,
        "hystersis.com/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("hystersis.com/session_summaries/") => {
...
/// `hystersis.com/sessions/list` — the FleetView roster. Returns every
...
/// `hystersis.com/sessions/changed` broadcast.
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "hystersis.com/session_summaries/session_list" => {
        ...
        "hystersis.com/session_summaries/workspace_list" => {
        ...
        "hystersis.com/session_summaries/workspace_list_recent" => {
