...
fn auto_mode_config_parses_from_toml_and_json_equivalently() {
    use xai_hystersis_workspace::permission::ClassifierPromptType;
    let toml_src = r#"
enabled = true
...
fn session_resolver_is_not_stamped_onto_third_party_samplers() {
    ...
    impl xai_hystersis_sampler::BearerResolver for SessionResolver {
...
fn shell_environment_policy_known_keys_track_the_policy_struct() {
    let xai_hystersis_tools::util::ShellEnvironmentPolicy {
        inherit: _,
        ignore_default_excludes: _,
        exclude: _,
        set: _,
        include_only: _,
    } = xai_hystersis_tools::util::ShellEnvironmentPolicy::default();
...
fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
    ...
    assert_eq!(config.api_backend, ApiBackend::Messages);
    let client = xai_hystersis_sampler::SamplingClient::new(config).expect("client should build");
...
fn user_override_parses_compaction_at_tokens_from_toml() {
    use xai_hystersis_sampling_types::CompactionAtTokens;
    let dm = crate::models::default_model();
    let raw_config: toml::Value = toml::from_str(&format!(
        r#"
            [model."{dm}"]
...
fn user_override_parses_compactions_remaining_from_toml() {
    use xai_hystersis_sampling_types::CompactionsRemaining;
    let dm = crate::models::default_model();
    let raw_config: toml::Value = toml::from_str(&format!(
        r#"
            [model."{dm}"]
...
fn ext_client() -> xai_hystersis_telemetry::external::config::ExternalClientInfo {
    xai_hystersis_telemetry::external::config::ExternalClientInfo::default()
}
#[test]
...
"#,
    ...
    xai_hystersis_config::apply_version_overrides(&mut value, &v).unwrap();
...
fn remote_settings_disarm_managed_config_signatures() {
    ...
    assert!(xai_hystersis_config::signed_policy::verification_active());
...
fn remote_settings_disarm_requires_prod_proxy_when_keys_embedded() {
    ...
    apply_remote_settings_side_effects(Some(&settings));
    assert!(
        !xai_hystersis_config::signed_policy::verification_active(),
