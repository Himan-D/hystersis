...
//! [`xai_hystersis_telemetry::external`]:
//!
...
pub(crate) fn suppress() {
    xai_hystersis_telemetry::external::set_settings_gate_max_wait(SETTINGS_GATE_MAX_WAIT);
...
/// Returns whether a session-less startup is about to mint a hystersis.com session
pub(crate) fn is_session_pending(
    has_session: bool,
    hystersis_com_config: &crate::auth::HystersisComConfig,
) -> bool {
    !has_session
        && (hystersis_com_config.auth_provider_command.is_some()
