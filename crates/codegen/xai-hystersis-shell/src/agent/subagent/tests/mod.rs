...
use xai_hystersis_subagent_resolution::resolve_effective_overrides;
use xai_hystersis_tools::implementations::hystersis_build::task::coordinator::{
...
async fn subagent_inherits_session_cli_overrides() {
    use xai_hystersis_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    use xai_hystersis_agent::config::PermissionMode;
    const PIN: &str = xai_hystersis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let reservations = xai_hystersis_tools::reminders::task_completion::TaskCompletionReservations::default();
...
fn partial_override_fills_from_role() {
    ...
    let role = xai_hystersis_subagent_resolution::config::SubagentRole {
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            xai_hystersis_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = xai_hystersis_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = xai_hystersis_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
            ...
            ConversationItem::Reasoning(xai_hystersis_sampling_types::synthesized_reasoning_item(
                "deliberating",
            )),
...
fn verbatim_fork_keeps_items_byte_for_byte_when_small() {
    use xai_hystersis_sampling_types::conversation::{
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use xai_hystersis_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
            ConversationItem::system("parent system"),
...
fn test_sampling_config(model_slug: &str) -> xai_hystersis_sampling_types::SamplingConfig {
