...
use xai_hystersis_tools::implementations::hystersis_build::task::backend::ChannelBackend;
#[test]
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = xai_hystersis_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn normalize_forked_context_consecutive_users() {
    ...
    let (conv, prefix_len) = xai_hystersis_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = xai_hystersis_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = xai_hystersis_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .find_map(|item| {
            if let ConversationItem::User(u) = item {
                let text: String = u
                    ...
                    .iter()
                    .filter_map(|p| match p {
                        xai_hystersis_sampling_types::conversation::ContentPart::Text {
                            text,
                        } => Some(text.as_ref()),
                        _ => None,
                    })
            ...
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                    p,
                    xai_hystersis_sampling_types::conversation::ContentPart::Text { text } if text.contains("<background_context>")
                )
                    })
            } else {
                false
...
fn subagent_tool_filter_removes_ask_user_question() {
    let mut tools = vec![
            xai_hystersis_sampling_types::ToolSpec {
...
fn token_estimation_accounts_for_images() {
    use xai_hystersis_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
            xai_hystersis_subagent_resolution::validate_resume_identity(
                "general-purpose",
                None,
                &source,
            )
...
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    ...
    ctx.parent_chat_state
        .as_ref()
        .unwrap()
        .update_sampling_config(xai_hystersis_sampling_types::SamplingConfig {
...
async fn runtime_override_wins_over_subagents_models_pin_in_precedence_path() {
    use xai_hystersis_agent::config::ModelOverride;
    let build_ctx = || {
        let mut models = indexmap::IndexMap::new();
...
fn non_cursor_persona_injected_as_system_reminder() {
    use xai_hystersis_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_hystersis_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_hystersis_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_hystersis_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_hystersis_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_hystersis_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_hystersis_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_hystersis_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(&result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &xai_hystersis_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn resolve_inherited_pool_named_filters() {
    ...
    let result = super::resolve_inherited_mcp_pool(
            Some(pool),
            &xai_hystersis_agent::config::McpInheritance::Named(vec!["atlassian".into()]),
        )
...
fn make_test_skill(
    ...
    xai_hystersis_tools::implementations::skills::types::SkillInfo {
        ...
        scope: xai_hystersis_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
