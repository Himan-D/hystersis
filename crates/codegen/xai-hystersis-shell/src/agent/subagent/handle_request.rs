...
use xai_hystersis_tools::implementations::{hystersis_build, opencode};
...
pub(crate) async fn run_shell_child(
    ...
    let spawn_timer = xai_hystersis_telemetry::subagent_spawn::SubagentSpawnTimer::new_shared();
    ...
    let mut effective_runtime = xai_hystersis_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = xai_hystersis_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    effective_runtime.capability_mode = xai_hystersis_subagent_resolution::intersect_capability_modes(
    ...
    xai_hystersis_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    {
        let reminder = xai_hystersis_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let child_cwd_abs = xai_hystersis_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        xai_hystersis_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let attribution_callback: Option<xai_hystersis_sampler::SharedAttributionCallback> =
    ...
    let yolo_policy_block = xai_hystersis_workspace::permission::resolution::yolo_disabled_by_policy();
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<xai_hystersis_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                xai_hystersis_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
        ...
        if is_plugin_agent {
            ...
            if !specs.is_empty() {
                let specs: Vec<_> = specs
                    .into_iter()
                    .map(|mut s| {
                        if s.event == xai_hystersis_hooks::event::HookEventName::Stop {
                            s.event = xai_hystersis_hooks::event::HookEventName::SubagentStop;
                        }
                        s
                    })
                    .collect();
    ...
    let agent_mcp_servers: Vec<_> = if !agent_owned_mcp_servers_allowed(is_plugin_agent) {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    xai_hystersis_agent::config::McpServerRef::Named(name) => {
                    ...
                    xai_hystersis_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            xai_hystersis_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    let _active = xai_hystersis_telemetry::activity::SUBAGENTS_ACTIVE.enter();
    debug_assert!(
        xai_hystersis_telemetry::activity::SUBAGENTS_ACTIVE.get() >= 1,
        "SubagentLaunched must stamp a self-inclusive count"
    );
    xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        xai_hystersis_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
        pins.mode,
        ...
        std::sync::Arc::new(parking_lot::Mutex::new(
            xai_hystersis_workspace::file_system::CodebaseIndexManager::new(),
        )),
        false,
        subagent_fs_watch,
        ...
        if inherit_skills {
            ...
            xai_hystersis_agent::prompt::skills::SkillsConfig::default()
        },
        if inherit_skills {
        ...
        ctx.yolo_mode
            || matches!(
                ...
                xai_hystersis_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        xai_hystersis_agent::prompt::context::PromptAudience::Subagent,
    ...
    let outcome = if result.success {
        xai_hystersis_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        xai_hystersis_telemetry::events::Outcome::Cancelled
    } else {
        xai_hystersis_telemetry::events::Outcome::Error
    };
    let mut completed = xai_hystersis_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        owner: telemetry_owner_kind(&request),
        workflow_run_id: request.owner.workflow_run_id().map(str::to_string),
    ...
    xai_hystersis_telemetry::session_ctx::log_event(completed);
    match (
