...
//! two ways:
//!
//!   - request/response `hystersis.com/sessions/list` → `{ "sessions": [RosterEntry, …] }`
//!   - broadcast notification `hystersis.com/sessions/changed` →
...
/// Response payload for `hystersis.com/sessions/list`.
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
pub struct RosterListResponse {
    pub sessions: Vec<RosterEntry>,
}

/// Params payload for the `hystersis.com/sessions/changed` broadcast notification.
#[derive(Serialize, Deserialize, Clone, Debug, Default)]
...
pub const SESSIONS_LIST_METHOD: &str = "hystersis.com/sessions/list";
pub const SESSIONS_CHANGED_METHOD: &str = "hystersis.com/sessions/changed";
...
mod merge_roster_tests {
    ...
    fn headless_summaries_exclude_dormant_and_resident_rows() {
        let mut headless = summary("oneshot", Some("hystersis -p run"), 5_000);
