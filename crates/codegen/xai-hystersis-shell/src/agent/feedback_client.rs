...
impl FeedbackClient {
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(xai_hystersis_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::hystersis_auth_credentials::HystersisAuthCredentials,
    ) -> Arc<dyn xai_hystersis_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(xai_hystersis_auth::StaticAuthCredentialProvider::new(
    ...
    fn record_401_attribution_if_needed(
        ...
        stamp: Option<&xai_hystersis_auth::StampedBearerSuffix>,
        op: &str,
    ) {
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, xai_hystersis_version::VERSION)
            .header(
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let (response, stamp) = xai_hystersis_auth::execute_with_stamp(&self.client, req)
            .await
            .context(context)?;
