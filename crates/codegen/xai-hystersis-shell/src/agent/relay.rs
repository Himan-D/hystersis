...
impl RelayConfig {
    ...
    /// (`is_xai_auth`: hystersis.com-issuer OIDC or external credential) with a
    ...
    match recovered {
        Ok(new_auth) if new_auth.key == config.auth.key => {
            ...
            xai_hystersis_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "key_prefix": xai_hystersis_auth::bearer_suffix(&new_auth.key),
                })),
            );
            false
        }
        Ok(new_auth) => {
            ...
            xai_hystersis_telemetry::unified_log::info(
                ...
                Some(serde_json::json!({
                    "context": context,
                    "new_key_prefix": xai_hystersis_auth::bearer_suffix(&new_auth.key),
                })),
            );
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(xai_hystersis_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    tokio::select! {
        ...
        result = tokio::time::timeout(connect_timeout, async {
            if let Some(proxy_url) = proxy_url {
                ...
                let connector =
                    tokio_tungstenite::Connector::Rustls(xai_hystersis_extra_ca::rustls_client_config());
