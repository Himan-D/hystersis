...
pub(crate) struct ModelsCache {
    ...
    pub(crate) hystersis_version: Option<String>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl ModelsCacheManager {
    pub(crate) fn new() -> Self {
        Self {
            path: crate::util::hystersis_home::hystersis_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    pub(crate) fn load_fresh(
        ...
        if cache.hystersis_version.as_deref() != Some(xai_hystersis_version::VERSION) {
    ...
    /// Per-writer temp path: `~/.hystersis` is shared across concurrent CLI
