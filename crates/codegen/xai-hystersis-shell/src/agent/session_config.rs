...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct HystersisSessionDetail {
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("hystersis", "Hystersis"),
            model("hystersis-4.5", "Hystersis 4.5"),
        ];
        let current = acp::ModelId::from("hystersis");
        ...
        assert_eq!(selected_models[0].id, "hystersis");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("hystersis", "Hystersis")];
        let current = acp::ModelId::from("hystersis");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("hystersis", "")];
        let current = acp::ModelId::from("hystersis");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "hystersis");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "hystersis".to_string(),
            category: "model".to_string(),
            label: "Hystersis".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "hystersis");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "Hystersis");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn hystersis_session_detail_serializes_camel_case() {
        let detail = HystersisSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["currentModelId"], "hystersis");
