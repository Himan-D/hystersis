...
pub fn bootstrap(
    ...
    xai_hystersis_telemetry::startup::enter(xai_hystersis_telemetry::startup::StartupPhase::Bootstrap);
    ...
    xai_hystersis_telemetry::startup::enter(xai_hystersis_telemetry::startup::StartupPhase::ModelCatalog);
    let models_manager = {
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        xai_hystersis_telemetry::session_ctx::log_event(limits.into_event());
    });
}
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    ...
    let hystersis_auth = auth_manager.current().filter(|a| a.is_xai_auth());
