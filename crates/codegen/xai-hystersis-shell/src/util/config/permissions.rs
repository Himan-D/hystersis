...
/// `xai-hystersis-telemetry`; re-exported here so existing call sites continue
/// to work.
pub use xai_hystersis_telemetry::enums::PermissionMode;
...
pub(crate) const ENV_DEFAULT_PERMISSION_MODE: &str = "HYSTERSIS_DEFAULT_PERMISSION_MODE";
...
/// `xai-hystersis-workspace`.
use xai_hystersis_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    fn campaign_cannot_arm_permission_mode_gate() {
        ...
        layers.campaigns.managed = vec![xai_hystersis_config::CampaignEntry {
            id: "c1".into(),
    ...
    fn effective_auto_for_launch_cli_auto_not_yolo() {
        ...
        unsafe { std::env::set_var("HYSTERSIS_AUTO_PERMISSION_MODE", "1") };
        ...
        unsafe { std::env::remove_var("HYSTERSIS_AUTO_PERMISSION_MODE") };
    }
    ...
    /// With the gate forced OFF (`HYSTERSIS_AUTO_PERMISSION_MODE=0`), explicit
    ...
    fn effective_auto_for_launch_inert_when_gate_off() {
        let _g = crate::util::config::resolve::AUTO_PERMISSION_MODE_ENV_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        unsafe { std::env::set_var("HYSTERSIS_AUTO_PERMISSION_MODE", "0") };
        assert!(
    ...
    fn interactive_default_slot() {
        ...
        for (raw, expected) in [
            ...
            assert_eq!(
                default_interactive_permission_mode(),
                expected,
                "HYSTERSIS_DEFAULT_PERMISSION_MODE={raw}"
            );
        }
    ...
    // definition in `xai_hystersis_workspace::permission::claude_compat`.

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = xai_hystersis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
