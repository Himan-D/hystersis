...
use xai_hystersis_tools::types::compat::{CompatConfig, CompatConfigToml};

pub use xai_hystersis_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `xai-hystersis-config-types` (config
...
pub use xai_hystersis_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use xai_hystersis_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.hystersis/config.toml` at each level.
/// Project-scoped `.hystersis/config.toml` entries override global `~/.hystersis/config.toml`
...
/// `.hystersis/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.hystersis/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.hystersis/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.hystersis/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .hystersis/config.toml"
                );
...
pub(crate) fn mcp_preferences_path() -> PathBuf {
    xai_hystersis_config::hystersis_home().join("mcp_preferences.json")
}
...
/// Used to surface setup-required rows and drive `hystersis.com/mcp/setup`.
///
...
pub(crate) fn collect_mcp_setup_configs(
    cwd: &std::path::Path,
    plugin_registry: Option<&xai_hystersis_agent::plugins::PluginRegistry>,
    compat: &CompatConfig,
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    xai_hystersis_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
/// Upsert an MCP server entry in `~/.hystersis/config.toml`.
///
...
/// config file, e.g. a project-scoped `.hystersis/config.toml`.
...
/// Delete an MCP server entry from `~/.hystersis/config.toml`.
///
...
/// config file, e.g. a project-scoped `.hystersis/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = xai_hystersis_mcp::credentials::McpCredentialStore::load_default() {
...
/// transport-less case is steered to `disabled_mcp_servers`, Hystersis's real
/// disable mechanism.
    ...
    let message = if !has_command && !has_url {
        format!(
            "`mcp_servers.{name}` has no transport. To run it, set `command = \"...\"` or \
             ...
             See ~/.hystersis/docs/user-guide/07-mcp-servers.md"
        )
    } else {
        format!(
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` has an unrecognized field `{field}`; it is \
                             ignored. See ~/.hystersis/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                }
                if config.enabled
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` is enabled but its `{field}` is blank. \
                             ...
                             off. See ~/.hystersis/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                    continue;
                }
...
// `.mcp.json` discovery moved to `xai-hystersis-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use xai_hystersis_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.hystersis/config.toml` files from `cwd` up to the
...
/// [`load_mcp_server_configs_with_project`], for `hystersis inspect`.
...
/// Names `hystersis mcp enable`/`disable` may target: user/project TOML (including
...
/// `hystersis_com_*` is known only when a TOML / disabled / compat / plugin
...
/// registry snapshot.
pub fn load_cli_plugin_registry(cwd: &std::path::Path) -> xai_hystersis_agent::plugins::PluginRegistry {
    ...
    let discovered = xai_hystersis_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    xai_hystersis_agent::plugins::PluginRegistry::from_discovered(
        discovered,
...
fn config_path() -> PathBuf {
    crate::util::hystersis_home::hystersis_home().join("config.toml")
}

/// Path to the user-level config file (`~/.hystersis/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.hystersis/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".hystersis").join("config.toml")
}
...
/// Overrides `[cli] session_registry`; usable before `~/.hystersis/config.toml` exists.
pub const SESSION_REGISTRY_ENV_VAR: &str = "HYSTERSIS_SESSION_REGISTRY";

pub(crate) fn session_registry_from_env_opt() -> Option<bool> {
    xai_hystersis_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = xai_hystersis_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = xai_hystersis_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = xai_hystersis_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
    ...
    fn load_cli_plugin_registry_includes_project_config_path_plugins() {
        ...
        let _env = xai_hystersis_test_support::EnvGuard::set("HYSTERSIS_HOME", home.path());
        ...
        let agents = xai_hystersis_agent::discovery::plugin_agents(&registry);
        assert!(
            agents
                .iter()
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.hystersis/skills", "~/.hystersis/skills/special/SKILL.md"]
ignore = ["~/.hystersis/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.hystersis/skills", "~/.hystersis/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.hystersis/skills/noisy/SKILL.md"]);
    }
...
"#;
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${HYSTERSIS_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
    ...
    fn mcp_json_all_toml_names_includes_disabled() {
        let tmp = tempfile::tempdir().unwrap();
        let hystersis_dir = tmp.path().join(".hystersis");
        std::fs::create_dir_all(&hystersis_dir).unwrap();
        std::fs::write(
            hystersis_dir.join("config.toml"),
            r#"
...
"#,
    ...
    fn apply_mcp_server_enabled_managed_name_only_updates_array() {
        ...
        apply_mcp_server_enabled(table, "hystersis_com_slack", false);
        let disabled = table
            .get("disabled_mcp_servers")
            .and_then(|v| v.as_array())
            .expect("disabled_mcp_servers array");
        assert_eq!(disabled.len(), 1);
        assert_eq!(disabled[0].as_str(), Some("hystersis_com_slack"));
        assert!(table.get("mcp_servers").is_none());

        apply_mcp_server_enabled(table, "hystersis_com_slack", true);
        assert!(table.get("disabled_mcp_servers").is_none());
        assert!(table.get("mcp_servers").is_none());
    }
...
"#,
    ...
    async fn enable_unstick_only_touches_nearest_project_definition() {
        ...
        std::fs::create_dir_all(nested.join(".hystersis")).unwrap();
...
enabled = false
"#;
        let ancestor = tmp.path().join(".hystersis").join("config.toml");
        let nearer = nested.join(".hystersis").join("config.toml");
    ...
    async fn restore_mcp_server_enabled_after_enable_scopes_tiers() {
        ...
        // save_mcp_server_enabled_in (that reads ambient config_path / hystersis_home).
