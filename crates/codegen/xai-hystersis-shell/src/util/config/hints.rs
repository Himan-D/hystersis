...
/// env master `HYSTERSIS_CONTEXTUAL_HINTS` (all-on/off) > user config
...
pub fn resolve_contextual_hints(
    ...
    let resolve_tip = |user: Option<bool>, feature_flag: Option<bool>| -> bool {
        BoolFlag::env("HYSTERSIS_CONTEXTUAL_HINTS")
            .config(user)
...
mod tests {
    ...
    const ENV_CONTEXTUAL_HINTS: &str = "HYSTERSIS_CONTEXTUAL_HINTS";

    // `HYSTERSIS_CONTEXTUAL_HINTS` is process-global; serialize the tests reading it
