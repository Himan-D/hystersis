...
use xai_hystersis_workspace::permission::resolution::McpServerAllowlist;
...
pub(crate) fn reenableable_disabled_stubs(
    ...
    let settings = xai_hystersis_workspace::permission::resolution::managed_settings();
...
mod tests {
    ...
    use xai_hystersis_workspace::permission::resolution::AllowedMcpServer;
    ...
    /// Git repo with project `.hystersis/config.toml` so discovery is bounded to cwd.
    fn project_repo(toml: &str) -> tempfile::TempDir {
        let tmp = tempfile::tempdir().unwrap();
        git2::Repository::init(tmp.path()).unwrap();
        // Pin trust so ambient HYSTERSIS_CLI_VERSION cannot drop project MCP names.
        crate::agent::folder_trust::record_for_test(tmp.path(), true);
        std::fs::create_dir_all(tmp.path().join(".hystersis")).unwrap();
        std::fs::write(tmp.path().join(".hystersis").join("config.toml"), toml).unwrap();
        tmp
    }

    /// Isolate HOME/HYSTERSIS_HOME so ambient user MCP config cannot pad discovery.
    fn isolated_home() -> (
        tempfile::TempDir,
        xai_hystersis_test_support::EnvGuard,
        xai_hystersis_test_support::EnvGuard,
    ) {
        let home = tempfile::tempdir().unwrap();
        let hystersis_home = home.path().join(".hystersis");
        std::fs::create_dir_all(&hystersis_home).unwrap();
        std::fs::write(hystersis_home.join("config.toml"), "").unwrap();
        let home_guard = xai_hystersis_test_support::EnvGuard::set("HOME", home.path());
        let hystersis_guard = xai_hystersis_test_support::EnvGuard::set("HYSTERSIS_HOME", &hystersis_home);
        (home, home_guard, hystersis_guard)
    }
    ...
    #[test]
    #[serial_test::serial]
    fn orphan_hystersis_com_name_is_not_a_definition() {
        ...
        assert!(!index.contains("hystersis_com_slack"));
        let disabled = HashSet::from(["hystersis_com_slack".into(), "totally_orphan_xyz".into()]);
        let stubs = index.reenableable_for_list(&disabled, &HashSet::new(), &unrestricted());
        assert!(!stubs.contains("hystersis_com_slack"));
