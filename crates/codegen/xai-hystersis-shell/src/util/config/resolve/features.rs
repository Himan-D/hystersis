...
pub fn resolve_zdr_access_enabled(
    ...
    BoolFlag::env("HYSTERSIS_ZDR_ACCESS_ENABLED")
...
/// (true). Overlay-free: the `HYSTERSIS_CONFIG` / `HYSTERSIS_CONFIG_PATH` overlay is
...
fn remote_fetch_enabled_from_layers(layers: &crate::config::ConfigLayers) -> bool {
    ...
    // `env_overlay` is deliberately NOT in the walk: the `HYSTERSIS_CONFIG` overlay
