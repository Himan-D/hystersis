...
/// [`xai_hystersis_config::env_bool`] parser (`HYSTERSIS_TOOLS_FIND_BFS` /
/// `HYSTERSIS_TOOLS_GREP_UGREP`, plus the `HYSTERSIS_FIND_BFS` / `HYSTERSIS_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.hystersis/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`xai_hystersis_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub(crate) fn resolve_search_tools_enabled(
    ...
    let disable = xai_hystersis_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = xai_hystersis_config::env_bool(primary).or_else(|| xai_hystersis_config::env_bool(alias));
    ...
    (
        resolve("HYSTERSIS_TOOLS_FIND_BFS", "HYSTERSIS_FIND_BFS", "find_bfs"),
        resolve("HYSTERSIS_TOOLS_GREP_UGREP", "HYSTERSIS_GREP_UGREP", "grep_ugrep"),
    )
}
...
) -> Option<xai_hystersis_tools::util::ShellEnvironmentPolicy> {
    let value = effective_cfg?.get("shell_environment_policy")?.clone();
    match value.try_into::<xai_hystersis_tools::util::ShellEnvironmentPolicy>() {
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "HYSTERSIS_LOGIN_ENV";
...
/// [`crate::config::load_merged_requirements`]) > `HYSTERSIS_LOGIN_ENV` env >
/// `HYSTERSIS_CONFIG` overlay > user `config.toml` > managed layers > remote >
...
/// user, below requirements), letting a `HYSTERSIS_CONFIG` toggle reach it while
...
mod login_shell_capture_tests {
    ...
    fn env_beats_overlay() {
        let _g = guard();
        // `HYSTERSIS_LOGIN_ENV` outranks the overlay (env > overlay).
        unsafe { std::env::set_var(ENV_LOGIN_SHELL_CAPTURE, "1") };
...
const ENV_SCHEDULER_BACKGROUND_LOOPS: &str = "HYSTERSIS_SCHEDULER_BACKGROUND_LOOPS";
...
///
/// Precedence: requirements > env (`HYSTERSIS_SCHEDULER_BACKGROUND_LOOPS`) > user
...
/// the shared [`xai_hystersis_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "HYSTERSIS_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`HYSTERSIS_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            xai_hystersis_tools::implementations::hystersis_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`HYSTERSIS_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        xai_hystersis_tools::implementations::hystersis_build::ask_user_question::response_timeout_env_secs(),
...
) -> xai_hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
fn cap_web_search_domains(list: Option<Vec<String>>, field: &str) -> Option<Vec<String>> {
    const MAX: usize = xai_hystersis_sampling_types::MAX_WEB_SEARCH_DOMAINS;
...
///
/// [`WebSearchOptions`]: xai_hystersis_sampling_types::WebSearchOptions
pub(crate) fn resolve_web_search_domains_from_disk()
-> Option<xai_hystersis_sampling_types::WebSearchOptions> {
...
fn web_search_options_from_section(
    ...
    let opts = xai_hystersis_sampling_types::WebSearchOptions {
    ...
    match opts.validate() {
        ...
        Err(e) => {
            ...
            Some(xai_hystersis_sampling_types::WebSearchOptions {
...
mod web_search_domains_tests {
    ...
    // (see `xai_hystersis_config::loader` normalization tests). These cover only the
    ...
    fn allowlist_parsed() {
        let got = web_search_options_from_section(&section(
            r#"allowed_domains = ["docs.hystersis.com", "arxiv.org"]"#,
        ))
        .unwrap();
        assert_eq!(
            got.allowed_domains,
            Some(vec!["docs.hystersis.com".to_string(), "arxiv.org".to_string()])
        );
    ...
    fn both_set_degrades_to_allowlist() {
        ...
        // defensive path.
        let got = web_search_options_from_section(&section(
            "allowed_domains = [\"docs.hystersis.com\"]\nexcluded_domains = [\"reddit.com\"]",
        ))
        .unwrap();
        assert_eq!(got.allowed_domains, Some(vec!["docs.hystersis.com".to_string()]));
        assert!(got.excluded_domains.is_none());
    }
}
...
mod ask_user_question_timeout_tests {
    ...
    use xai_hystersis_tools::implementations::hystersis_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = xai_hystersis_tools::implementations::hystersis_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes HYSTERSIS_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
...
mod shell_env_policy_tests {
    use super::*;
    use xai_hystersis_tools::util::{EnvironmentVariablePattern, ShellEnvironmentPolicyInherit};
