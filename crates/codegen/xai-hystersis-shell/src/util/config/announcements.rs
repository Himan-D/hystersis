...
/// Re-exported from `xai-hystersis-announcements` for backward compatibility.
pub use xai_hystersis_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `HYSTERSIS_ANNOUNCEMENTS_OVERRIDE` (a JSON
...
pub(crate) fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("HYSTERSIS_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid HYSTERSIS_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// `HYSTERSIS_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev-only escape hatch).
