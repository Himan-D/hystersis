...
pub mod hystersis_auth_credentials;
pub mod hooks;
pub mod limits;
...
// The foundation utilities live in `xai-hystersis-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `xai_hystersis_shell::util::…` users compile
// unchanged.
pub use xai_hystersis_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let hystersis_rules = hystersis_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == hystersis_home || parent == hystersis_rules)
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn hystersis_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_hystersis_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
            Path::new("/custom/hystersis/worktrees/repo/src/AGENTS.md"),
