...
use std::sync::Arc;
/// Credentials for authenticating with hystersis backend services.
///
...
#[derive(Clone)]
pub struct HystersisAuthCredentials {
...
impl std::fmt::Debug for HystersisAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HystersisAuthCredentials")
            .field(
                "user_token",
...
impl HystersisAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your HYSTERSIS_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `hystersis login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> HystersisAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> HystersisAuthCredentials {
...
impl xai_hystersis_auth::HttpAuth for HystersisAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        HystersisAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = HystersisAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = HystersisAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = HystersisAuthCredentials::new(Some("static-token".into()));
