// Re-export all types from the standalone xai-hystersis-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use xai_hystersis_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `xai-hystersis-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `xai-hystersis-sampler` crate can reference them without a circular
// dep on `xai-hystersis-shell`.

// Tests for the types now live in xai-hystersis-sampling-types crate.

use xai_hystersis_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `xai-hystersis-sampling-types` or
/// `xai-hystersis-tools`) so neither of those crates needs to depend on
