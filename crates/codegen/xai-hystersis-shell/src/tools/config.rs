...
use xai_hystersis_sampler::SamplerConfig;
use xai_hystersis_tools::implementations::hystersis_build;
use xai_hystersis_tools::registry::types::ToolConfig;

/// Production hystersis foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub(crate) fn to_bash_params_json(
        ...
        // production hystersis opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `HYSTERSIS_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `HYSTERSIS_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub(crate) fn resolve_params(
    ...
    ) -> xai_hystersis_tools::implementations::hystersis_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("HYSTERSIS_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| xai_hystersis_config::env_bool("HYSTERSIS_WEB_FETCH_ALLOW_LOCAL"));
...
/// from `xai_hystersis_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `HystersisBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `HystersisBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "HystersisBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "HystersisBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"HystersisBuild:read_file"));
        assert!(ids.contains(&"HystersisBuild:search_replace"));
        assert!(ids.contains(&"HystersisBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"HystersisBuildHashline:hashline_read"));
        assert!(ids.contains(&"HystersisBuildHashline:hashline_edit"));
        assert!(ids.contains(&"HystersisBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            xai_hystersis_agent::config::AgentDefinition::plan(),
            xai_hystersis_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "HystersisBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"HystersisBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"HystersisBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"HystersisBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production hystersis must set the 10h foreground ceiling"
        );
    }
