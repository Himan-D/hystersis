...
use xai_hystersis_tools::notification::types::{ToolNotification, ToolNotificationHandle};
...
pub(crate) struct NotificationBridgeConfig {
    ...
    pub task_completion_reservations:
        xai_hystersis_tools::reminders::task_completion::TaskCompletionReservations,
    pub task_wake_suppressed: xai_hystersis_tools::reminders::task_completion::TaskWakeSuppressed,
...
async fn handle_scheduled_task_removed(
    config: &NotificationBridgeConfig,
    removed: xai_hystersis_tools::notification::ScheduledTaskRemoved,
...
async fn emit_current_mode_update(
    config: &NotificationBridgeConfig,
    mode: xai_hystersis_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == xai_hystersis_tools::computer::types::TaskKind::Monitor;
            ...
            if task_snapshot.is_auto_wake_suppressed() {
                ...
                let body = if is_monitor {
                    xai_hystersis_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    xai_hystersis_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = xai_hystersis_tools::reminders::wrap_reminder(&body);
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    ...
                    .tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, xai_hystersis_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
