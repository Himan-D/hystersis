...
//!
//! Used by both the ACP `hystersis.com/session/list` handler and the `hystersis sessions`
...
pub(crate) async fn fetch_lanes(
    ...
    let repo_urls_fut = async {
        ...
        cwd.map(|c| {
            xai_hystersis_workspace::session::git::resolve_normalized_remote_urls(std::path::Path::new(
                c,
            ))
        })
        .unwrap_or_default()
    ...
    // `hystersis --resume <uuid>` resolves across every cwd. `/resume` search was
