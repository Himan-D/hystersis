...
//!
//! Receives [`xai_hystersis_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`xai_hystersis_mcp::liveness`]),
//! - the [`xai_hystersis_mcp::servers::HystersisClientHandler`] (server-pushed
...
pub enum McpServerStatus {
    /// Client is in [`xai_hystersis_mcp::servers::ClientStateKind::Ready`]
...
/// [`xai_hystersis_mcp::servers::McpClientEventKind`]). Used as the
...
fn flush_elicitation_completes(
    ...
    for (server, elicitation_id) in completes {
        let payload = xai_hystersis_tools::mcp_elicitation::McpElicitCompletePayload {
            session_id: session_id.to_string(),
            elicitation_id,
        ...
        match serde_json::value::to_raw_value(&payload) {
            Ok(raw) => {
                gateway.forward_fire_and_forget(acp::ExtNotification::new(
                    xai_hystersis_mcp::wire::MCP_ELICIT_COMPLETE,
                    raw.into(),
                ));
            }
            Err(e) => {
...
mod tests {
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        ...
        use xai_hystersis_mcp::servers::{McpClient, McpState};
