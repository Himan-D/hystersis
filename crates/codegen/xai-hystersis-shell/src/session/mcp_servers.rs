...
pub use xai_hystersis_mcp::servers::{
...
use xai_hystersis_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
use xai_hystersis_mcp::servers as inner;
...
pub(crate) fn build_config_resolved_event(
    ...
    cwd: &Path,
) -> xai_hystersis_session_events::Event {
    let disabled: Vec<String> = crate::util::config::disabled_mcp_server_names(cwd)
        .into_iter()
        .collect();
    let servers = configs
        .iter()
        .map(|c| xai_hystersis_session_events::McpConfigServer {
        ...
        .collect();
    xai_hystersis_session_events::Event::McpConfigResolved { servers, disabled }
}
