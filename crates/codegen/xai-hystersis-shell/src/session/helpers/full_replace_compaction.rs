...
//! (`xai_hystersis_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](xai_hystersis_compaction::sample_full_replace_summary);
...
//! [`FullReplaceError`](xai_hystersis_compaction::FullReplaceError).
...
use xai_hystersis_sampling_types::{ConversationItem, HostedTool, ToolSpec};
use xai_hystersis_telemetry::events::{CompactionRetryDegraded, CompactionTrigger};
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    xai_hystersis_telemetry::session_ctx::log_event(CompactionRetryDegraded {
