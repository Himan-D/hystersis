...
use xai_hystersis_workspace::session::file_state::RewindPoint;
...
pub use xai_hystersis_shared::session::session_dir;
...
impl Summary {
    pub(crate) fn new(info: &Info, model_id: acp::ModelId) -> std::io::Result<Self> {
        let git_metadata =
            xai_hystersis_workspace::session::git::resolve_persisted_session_git_metadata_sync(
...
fn collect_mcp_stderr_logs(files: &mut Vec<CopiedSessionFile>) {
    let mcp_log_dir = xai_hystersis_config::hystersis_home().join("logs").join("mcp");
...
pub async fn delete_session_history(
    ...
    needs_remote: bool,
    auth_manager: Arc<crate::auth::AuthManager>,
    search_index: Option<&xai_hystersis_session_search::SearchIndexManager>,
...
pub(crate) fn cleanup_stale_sessions(skip_session_dir: Option<&Path>) {
    CLEANUP_SESSIONS_ONCE.call_once(|| {
        ...
        tracing::info!(
            target: "xai_hystersis_shell::session::persistence",
