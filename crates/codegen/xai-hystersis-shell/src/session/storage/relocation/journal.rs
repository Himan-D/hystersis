...
impl RelocationJournal {
    ...
    pub(super) fn validate(&self, hystersis_home: &Path) -> Result<()> {
        ...
        if session_dir_at(hystersis_home, &self.source_cwd, &self.session_id)
            == session_dir_at(hystersis_home, &self.target_cwd, &self.session_id)
        {
            return Err(RelocationError::Inconsistent(
...
pub(super) fn acquire(hystersis_home: &Path, session_id: &str) -> Result<RelocationLease> {
    validate_component("session id", session_id)?;
    let dir = relocation_dir(hystersis_home);
...
pub(super) fn read(hystersis_home: &Path, session_id: &str) -> Result<RelocationJournal> {
    validate_component("session id", session_id)?;
    let path = journal_path(hystersis_home, session_id);
    ...
    journal.validate(hystersis_home)?;
    Ok(journal)
}
...
pub(super) fn write(
    ...
    journal
        .validate(hystersis_home)
        .map_err(WriteFailure::NotCommitted)?;
    let path = journal_path(hystersis_home, &journal.session_id);
...
pub(super) fn relocation_dir(hystersis_home: &Path) -> PathBuf {
    hystersis_home.join(RELOCATION_DIR)
}

pub(super) fn journal_path(hystersis_home: &Path, session_id: &str) -> PathBuf {
    relocation_dir(hystersis_home).join(format!("{session_id}.json"))
}

pub(super) fn session_dir_at(hystersis_home: &Path, cwd: &str, session_id: &str) -> PathBuf {
    hystersis_home
        .join("sessions")
...
pub(super) fn sync_namespace(hystersis_home: &Path) -> Result<()> {
    let dir = relocation_dir(hystersis_home);
