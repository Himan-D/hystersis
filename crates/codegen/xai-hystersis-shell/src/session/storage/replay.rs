...
/// `~/.hystersis/sessions` RelocationView scan.
...
/// hystersis home. Typed, materialize-all replay reader: collects every update into
...
pub fn load_updates_for_replay_at(
    ...
    let Some(updates_path) =
        resolve_replay_updates_path(session_id, hystersis_home, ReplayPathHint::default())?
    else {
        return Ok(None);
    };
    Ok(Some(collect_replay_updates(&updates_path)?))
...
fn try_fast_replay_updates_path(
    ...
    for cwd in [hint.child_cwd, hint.parent_cwd].into_iter().flatten() {
        let encoded = xai_hystersis_config::encode_cwd_dirname(&cwd.to_string_lossy());
        let candidate = hystersis_home.join("sessions").join(encoded).join(session_id);
...
/// Resolve `updates.jsonl` for `session_id` under `hystersis_home`, or `None` when
...
pub(crate) fn resolve_replay_updates_path(
    ...
    match try_fast_replay_updates_path(session_id, hystersis_home, hint) {
        Some(path) if !super::relocation::has_relocation_journal(hystersis_home, session_id) => {
            return Ok(Some(path));
        }
        Some(_journaled) => {
...
/// Invoke `f` once per client-replay ACP update for a session under `hystersis_home`,
...
pub fn stream_replay_updates_at<F: FnMut(acp::SessionUpdate)>(
    ...
    stream_replay_updates_at_hinted(session_id, hystersis_home, ReplayPathHint::default(), |update| {
...
pub fn replay_would_emit(
    ...
    let Some(path) = resolve_replay_updates_path(session_id, hystersis_home, hint)? else {
        return Ok(false);
    };
...
pub fn stream_replay_updates_at_hinted<F: FnMut(ReplayedUpdate)>(
    ...
    let Some(updates_path) = resolve_replay_updates_path(session_id, hystersis_home, hint)? else {
