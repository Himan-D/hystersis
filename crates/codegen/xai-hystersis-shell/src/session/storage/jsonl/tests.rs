...
async fn workflow_run_manifest_round_trips_and_clear_tombstone_wins() {
    ...
    assert_eq!(loaded_with_effort.workflow_runs[0].effort,
            Some(xai_hystersis_sampling_types::ReasoningEffort::High));
...
async fn merge_rewind_points_from_round_trips_file_snapshots() {
    use xai_hystersis_paths::RelPathBuf;
    use xai_hystersis_workspace::session::file_state::{FileSnapshot, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn reads_never_modify_rewind_or_updates_files() {
    use xai_hystersis_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    xai_hystersis_sandbox::set_configured_profile("workspace");
    let expected = xai_hystersis_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
/// path end-to-end (loader + serde + xai_hystersis_sampling_types::
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let xai_hystersis_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    ...
    let xai_hystersis_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
