//! Binds the `xai-hystersis-session-search` index to this crate's JSONL session
...
use xai_hystersis_session_search::{IndexableSession, SessionSource};

pub use xai_hystersis_session_search::{
...
/// feature cannot be bypassed. One live manager per hystersis home, at most.
...
pub fn notify_session_updated(index: Option<&SearchIndexManager>, session_id: &str, cwd: &str) {
    ...
    let root = crate::util::hystersis_home::hystersis_home();
...
pub(crate) async fn evict_session(root_dir: &Path, session_id: &str) {
    xai_hystersis_session_search::evict_session(root_dir, session_id).await;
}
...
pub async fn execute_search(
    ...
    xai_hystersis_session_search::execute_search(index, root_dir, req).await
}
