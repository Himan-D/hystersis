...
fn sampler_surfaces_429() -> xai_hystersis_sampler::RetryPolicy {
    xai_hystersis_sampler::RetryPolicy {
        max_retries: 5,
        rate_limit_retry_threshold: xai_hystersis_sampler::RATE_LIMIT_RETRY_DISABLED,
        ..Default::default()
    }
}

fn sampler_retries_429() -> xai_hystersis_sampler::RetryPolicy {
    xai_hystersis_sampler::RetryPolicy {
        max_retries: 5,
        rate_limit_retry_threshold: xai_hystersis_sampler::RATE_LIMIT_RETRY_THRESHOLD,
        ..Default::default()
    }
}
...
async fn actor_under_test(
    ...
    session: SessionKind,
    retry_policy: xai_hystersis_sampler::RetryPolicy,
) -> (Arc<SessionActor>, CapturedRetries) {
    let sampler_max_retries = retry_policy.max_retries;
    let sampling_cfg = xai_hystersis_sampler::SamplerConfig {
        base_url: server.url(),
    ...
    let sampler_handle =
        xai_hystersis_sampler::SamplerActor::spawn(sampling_cfg, retry_policy, sampler_event_tx);
