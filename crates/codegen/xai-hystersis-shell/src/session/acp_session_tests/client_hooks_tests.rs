...
fn install_client_hook(
    actor: &SessionActor,
    event: xai_hystersis_hooks::event::HookEventName,
    callback_ids: &[&str],
) {
    ...
    local
        .run_until(async {
            ...
            actor.fire_hook(
                xai_hystersis_hooks::event::HookEventName::Stop,
                None,
                xai_hystersis_hooks::event::HookPayload::Stop {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                xai_hystersis_tools::registry::types::ToolConfig::for_tool::<
                    xai_hystersis_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        xai_hystersis_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
pub(super) fn file_registry_with_spec(
    event: xai_hystersis_hooks::event::HookEventName,
    script: &str,
) -> xai_hystersis_hooks::discovery::HookRegistry {
...
fn file_registry(
    ...
    let (mut registry, _) = xai_hystersis_hooks::discovery::load_hooks(None, None);
    registry.append_specs(vec![xai_hystersis_hooks::config::HookSpec {
...
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                xai_hystersis_hooks::event::HookEventName::SubagentEnd,
                None,
                xai_hystersis_hooks::event::HookPayload::SubagentStop {
                    phase: xai_hystersis_hooks::event::SubagentStopPhase::Observe,
...
async fn only_a_completed_stop_hook_is_the_turns_report() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(
                &actor,
                xai_hystersis_hooks::event::HookEventName::StopFailure,
                &["cb_fail"],
            );
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry(
                ...
                let reported = actor.claim_and_queue(
                    ...
                    super::turn_end_hooks::TurnEnd::Failed {
                        error: xai_hystersis_hooks::event::StopFailureKind::Unknown,
                        error_details: None,
                        last_assistant_message: None,
                    },
                );
