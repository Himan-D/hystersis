...
//! `xai-hystersis-session-events`. The orphan-rule items (`From<&permission::Decision>`
//! and the doom-loop categorizer) stay here since they need shell-local
//! types.

pub(crate) use xai_hystersis_session_events::tracker::EventTracker;
pub(crate) use xai_hystersis_session_events::types::{
...
) -> Option<xai_hystersis_sampling_types::PriorTurnInterrupt> {
    use xai_hystersis_sampling_types::PriorTurnInterrupt;
    match category {
...
/// committing it would silently run hystersis flavor. Distinct from
