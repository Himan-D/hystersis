...
pub(super) async fn fire_session_end_hooks(session: &SessionActor, reason: &str) {
    let envelope = session.fire_hook(
        xai_hystersis_hooks::event::HookEventName::SessionEnd,
        None,
        xai_hystersis_hooks::event::HookPayload::SessionEnd {
    ...
    if let Some(registry) = session.hook_registry.borrow().clone() {
        ...
        let results = xai_hystersis_hooks::dispatcher::dispatch_non_blocking(
            &registry,
            xai_hystersis_hooks::event::HookEventName::SessionEnd,
            &envelope,
            &ctx,
...
async fn log_session_ended(session: &SessionActor) {
    ...
    if let Some(signals) = session.signals_handle().snapshot().await {
        xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::SessionEnded {
...
pub(super) async fn run_session(
    ...
    let _elicitation_coordinator = if !session.startup_hints.is_subagent {
        let elicit_inbox = xai_hystersis_mcp::elicitation::ElicitationInbox::new();
        {
    ...
    if !session.startup_hints.is_subagent {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<xai_hystersis_mcp::servers::McpClientEvent>();
        {
            let mut mcp_state = session.mcp_state.lock().await;
        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: xai_hystersis_telemetry::memory_log::TARGET,
                        ...
                        tracing::debug!(target: xai_hystersis_telemetry::memory_log::TARGET,
                    ...
                    match cmd {
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        xai_hystersis_tools::implementations::hystersis_build::monitor::types::MonitorEventNotification {
                        ...
                        SessionCommand::RefreshSkillBaseline => {
                            ...
                            tokio::task::spawn_local(async move {
                                ...
                                let new_skills = xai_hystersis_agent::prompt::skills::list_skills_with_plugins(
                                    Some(&cwd),
                                    &skills_config,
                                    pr.as_deref(),
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            {
                                let _ = tx.send(
                                    xai_hystersis_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::ToggleMcpServer { server_name, enabled, server_config, respond_to } => {
                            session.events.emit(xai_hystersis_session_events::Event::McpServerToggled {
                        ...
                        SessionCommand::DispatchSessionStartHook { source } => {
                            let envelope = session.fire_hook(
                                xai_hystersis_hooks::event::HookEventName::SessionStart,
                                None,
                                xai_hystersis_hooks::event::HookPayload::SessionStart {
                                    source,
                                    model_id: None,
                                    agent_type: None,
                                },
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[xai_hystersis_sampling_types::ConversationItem],
    turn_number: usize,
