...
impl SessionActor {
    ...
    /// sees "Hystersis is thinking", streaming token chunks, or any other
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let items = vec![
            ConversationItem::System(xai_hystersis_sampling_types::SystemItem {
        ...
        let request = ConversationRequest {
            ...
            // Don't pass `reasoning_effort` — `hystersis-4.5` (and
            // other tool-flavoured Hystersis variants) reject the field at
            ...
            x_hystersis_conv_id: Some(format!("trace-classifier-{}", uuid::Uuid::new_v4())),
            x_hystersis_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_hystersis_session_id: Some(session_id_str),
