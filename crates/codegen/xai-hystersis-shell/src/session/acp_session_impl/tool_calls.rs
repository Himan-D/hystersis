...
pub(super) fn is_file_backed_exit_plan_kind(
    kind: Option<xai_hystersis_tools::types::tool::ToolKind>,
) -> bool {
    matches!(kind, Some(xai_hystersis_tools::types::tool::ToolKind::ExitPlan))
}
...
fn split_exit_plan_tail(
    ...
    kind_of: impl Fn(&str) -> Option<xai_hystersis_tools::types::tool::ToolKind>,
) -> (
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &xai_hystersis_tools::implementations::hystersis_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        xai_hystersis_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) fn media_gen_over_cap(
        &self,
        calls: &[xai_hystersis_sampling_types::ToolCall],
    ) -> Vec<xai_hystersis_tools::media_gen_limits::MediaGenOverCap> {
        let kind_of = |name: &str| self.agent.borrow().tool_bridge().tool_kind(name);
        xai_hystersis_tools::media_gen_limits::over_cap_by_name(
    ...
    async fn reject_excess_media_gen_calls(
        ...
        let kind_of = |name: &str| self.agent.borrow().tool_bridge().tool_kind(name);
        let (allowed, rejected) = xai_hystersis_tools::media_gen_limits::partition_media_gen_batch(
            tool_calls,
        ...
        for (name, rejected_n) in &rejected_by_name {
            let max = self
                ...
                .tool_bridge()
                .tool_kind(name)
                .and_then(|k| xai_hystersis_tools::media_gen_limits::max_calls_per_batch(k, limits))
                .unwrap_or(0);
            ...
            match self.prepare_tool_call(call, deferred_followups).await? {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(xai_hystersis_session_events::Event::McpToolCallCompleted {
        ...
        while let Some((idx, result, duration_ms)) = dispatch_rx.recv().await {
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .may_have_hooks_for(xai_hystersis_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self.may_have_hooks_for(
                        xai_hystersis_hooks::event::HookEventName::PostToolUseFailure,
                    ) {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            xai_hystersis_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            xai_hystersis_hooks::event::HookEventName::PostToolUseFailure,
                            xai_hystersis_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    xai_hystersis_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    xai_hystersis_hooks::event::HookEventName::PostToolUse,
                    xai_hystersis_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
            ...
            self.observability_bridge
                ...
                .await;
            let (ext_file_path, ext_parameters) = if xai_hystersis_telemetry::external::is_active() {
            ...
            xai_hystersis_telemetry::session_ctx::log_event(
                xai_hystersis_telemetry::events::ToolCallCompleted {
                    tool_name: prepared.tool_name.clone(),
                    outcome: tool_outcome,
                    duration_ms,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        );
        {
            ...
            let subagent_background =
                xai_hystersis_tools::is_task_tool_id(&call.function.name).then(|| {
                    early_raw_input
                        .as_ref()
        ...
        if self.may_have_hooks_for(xai_hystersis_hooks::event::HookEventName::PreToolUse) {
            let mut envelope =
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    xai_hystersis_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let xai_hystersis_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                xai_hystersis_workspace::permission::AccessKind::Read(p) => (
                    xai_hystersis_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                xai_hystersis_workspace::permission::AccessKind::Edit(p) => {
                    (xai_hystersis_telemetry::events::AccessKind::Edit, p.clone())
                }
                xai_hystersis_workspace::permission::AccessKind::Bash(cmd) => {
                    (xai_hystersis_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                xai_hystersis_workspace::permission::AccessKind::Grep { path, glob } => (
                    xai_hystersis_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                xai_hystersis_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (xai_hystersis_telemetry::events::AccessKind::Mcp, name.clone())
                }
                xai_hystersis_workspace::permission::AccessKind::WebFetch(u) => {
                    (xai_hystersis_telemetry::events::AccessKind::Web, u.clone())
                }
                xai_hystersis_workspace::permission::AccessKind::WebSearch(q) => {
                    (xai_hystersis_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            xai_hystersis_telemetry::session_ctx::log_event(
                xai_hystersis_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            let path_context = Some(xai_hystersis_workspace::permission::types::RequestPathContext {
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        xai_hystersis_session_events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        xai_hystersis_session_events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        xai_hystersis_session_events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        xai_hystersis_session_events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    self.handle_tool_not_executed(&call.id, &tool_call_id, message)
                        .await?;
                    let (tool_input_value, tool_input_truncated) =
                        xai_hystersis_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        xai_hystersis_hooks::event::HookEventName::PermissionDenied,
                        xai_hystersis_hooks::event::HookPayload::PermissionDenied {
    ...
    pub(super) async fn request_plan_approval(
        ...
        xai_hystersis_tools::implementations::hystersis_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use xai_hystersis_tools::implementations::hystersis_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                xai_hystersis_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
    ...
    async fn send_tool_call_start(
        ...
        let (title, kind, locations, content) = match tool_call_input {
                ...
                (
                    ...
                    vec![
                        acp::ToolCallLocation::new(read_file.path)
                            ...
                            .line(
                                xai_hystersis_tools::normalization::norm_offset_i64(read_file.offset)
                                    .map(|l| l as u32),
                            ),
                    ],
                    Vec::new(),
            ...
            ToolInput::Skill(skill) => {
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::SkillDispatched {
                        skill_name: skill.skill.clone(),
                        plugin_source: None,
                        trigger: xai_hystersis_telemetry::events::SkillTrigger::SkillTool,
                    },
                );
                tracing::info_span!(
                    "skill.activated",
    ...
    async fn skill_for_read_path(
        &self,
        path: &str,
    ) -> Option<xai_hystersis_tools::implementations::skills::types::SkillInfo> {
        if self.is_chat_kind {
            return None;
        }
        let read_path = xai_hystersis_tools::types::resources::resolve_model_path(
            self.tool_context.cwd.as_path(),
    ...
    fn emit_skill_md_read(&self, skill: xai_hystersis_tools::implementations::skills::types::SkillInfo) {
        ...
        .in_scope(|| {});
        xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::SkillDispatched {
            skill_name: skill.name,
            plugin_source: skill.plugin_name,
            trigger: xai_hystersis_telemetry::events::SkillTrigger::SkillMdRead,
        });
    }
    ...
    ) -> xai_hystersis_hooks::event::HookEventEnvelope {
        let (tool_input, tool_input_truncated) =
            xai_hystersis_hooks::event::truncate_payload(input.clone());
        self.make_hook_envelope(
            xai_hystersis_hooks::event::HookEventName::PreToolUse,
            None,
            xai_hystersis_hooks::event::HookPayload::PreToolUse {
    ...
    /// `xai_hystersis_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use xai_hystersis_telemetry::enums::PrCreationSource;
        use xai_hystersis_tools::util::git_detect;
        match &result.output {
            xai_hystersis_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    xai_hystersis_telemetry::session_ctx::log_event(
                        xai_hystersis_telemetry::events::PrMerged {},
                    );
                }
            }
            xai_hystersis_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: xai_hystersis_tools::util::git_detect::PrRef,
        source: xai_hystersis_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            xai_hystersis_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                xai_hystersis_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let xai_hystersis_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                xai_hystersis_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | xai_hystersis_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            xai_hystersis_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            xai_hystersis_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`xai_hystersis_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: xai_hystersis_sampler::SamplingEvent,
    ) {
        use xai_hystersis_sampler::{SamplingChannel, SamplingEvent};
        match event {
            ...
            SamplingEvent::Retrying {
                ...
                if kind == xai_hystersis_sampler::SamplingErrorKind::DoomLoopDetected {
            ...
            SamplingEvent::Failed { request_id, error } => {
                ...
                xai_hystersis_telemetry::unified_log::error(
...
fn execute_tool_call_parts(
    ...
    let display = xai_hystersis_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod exit_plan_tail_predicate_tests {
    ...
    use xai_hystersis_tools::types::ToolInput;
    use xai_hystersis_tools::types::tool::ToolKind;
    ...
    fn exit_plan_kind_is_file_backed_exit() {
        ...
        assert!(is_file_backed_exit_plan_input(&ToolInput::ExitPlanMode(
            xai_hystersis_tools::implementations::hystersis_build::exit_plan_mode::ExitPlanModeInput {}
        )));
    }
...
mod plan_mode_edit_gate_tests {
    ...
    use xai_hystersis_tools::types::ToolInput;
    use xai_hystersis_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use xai_hystersis_tools::implementations::hystersis_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use xai_hystersis_tools::implementations::opencode::write::WriteInput;
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use xai_hystersis_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
    ...
    fn non_edit_tools_not_gated() {
        use xai_hystersis_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use xai_hystersis_tools::implementations::hystersis_build::exit_plan_mode::ExitPlanModeExtResponse;
...
mod wait_interrupt_tests {
    ...
    use xai_hystersis_tools::types::output::ToolOutput;
