...
impl SessionActor {
    ...
    pub(super) async fn list_running_subagents(
        &self,
    ) -> Vec<xai_hystersis_tools::implementations::hystersis_build::task::types::SubagentInspection> {
        self.reconcile_live_orphaned_subagents().await;
        let Some(event_tx) = self.tool_context.subagent_event_tx.clone() else {
            return Vec::new();
        };
        xai_hystersis_tools::implementations::hystersis_build::task::backend::ChannelBackend::new(event_tx)
    ...
    pub(super) fn sweep_monitor_buffer_into_pending(
        ...
        for event in xai_hystersis_tools::implementations::hystersis_build::monitor::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        ) {
            Self::push_pending_notification(
    ...
    fn notification_blocks(
        ...
        use xai_hystersis_tools::implementations::hystersis_build::monitor::types::MonitorEventNotification;
        ...
        if let (Some(index), Some(batch)) = (
            monitor_section_idx,
            xai_hystersis_tools::reminders::task_completion::format_monitor_events(
                &monitor_events,
