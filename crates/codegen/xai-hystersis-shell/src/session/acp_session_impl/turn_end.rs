...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch, commit) = tokio::join!(
            xai_hystersis_workspace::session::git::get_worktree_info(cwd),
            xai_hystersis_workspace::session::git::get_branch(cwd),
            xai_hystersis_workspace::session::git::get_current_commit(cwd),
        );
        ...
        let params = xai_hystersis_workspace::session::git::GitHeadChanged {
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<xai_hystersis_tools::implementations::hystersis_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn classify_turn_error(err: &acp::Error) -> String {
        use xai_hystersis_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn stop_failure_error_type(
        err: &acp::Error,
    ) -> xai_hystersis_hooks::event::StopFailureKind {
        use xai_hystersis_hooks::event::StopFailureKind as K;
    ...
    pub(super) fn classify_install_error(
        err: &xai_hystersis_agent::plugins::install_registry::InstallError,
    ) -> String {
