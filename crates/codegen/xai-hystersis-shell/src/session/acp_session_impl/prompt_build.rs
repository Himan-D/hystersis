...
fn partition_rules_by_scope(
    files: Vec<xai_hystersis_agent::prompt::agents_md::AgentConfigFile>,
    hystersis_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_roots: &[&std::path::Path],
) -> (
    Vec<xai_hystersis_agent::prompt::user_message::RuleEntry>,
    Vec<xai_hystersis_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = xai_hystersis_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use xai_hystersis_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[xai_hystersis_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
        entries.iter().map(|entry| entry.content.as_str()).collect()
    }
    #[test]
    fn partitions_custom_hystersis_and_vendor_home_rules_as_user_scope() {
        let files = vec![
    ...
    #[test]
    fn hystersis_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.hystersis/rules/project.md"),
    ...
    #[test]
    fn nested_hystersis_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
            file("/custom/hystersis/rules/global.md"),
            file("/custom/hystersis/worktrees/repo/.cursor/rules/project.md"),
            file("/custom/hystersis/worktrees/repo/src/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/custom/hystersis"),
            &[],
            &[Path::new("/custom/hystersis/worktrees/repo")],
        );
        assert_eq!(paths(&user), vec!["/custom/hystersis/rules/global.md"]);
        assert_eq!(
            paths(&workspace),
            vec![
    ...
    fn partitioned_snapshot_bodies_match_rules_and_old_reminder() {
        let files = vec![
            ...
            AgentConfigFile {
                file_name: "AGENTS.md".into(),
                file_path: "/home/user/.hystersis/AGENTS.md".into(),
                content: "home-hystersis-body".into(),
            },
            AgentConfigFile {
                file_name: "CLAUDE.md".into(),
            ...
            AgentConfigFile {
                file_name: "x.md".into(),
                file_path: "/repo/.hystersis/rules/x.md".into(),
                content: "repo-hystersis-rules-x".into(),
            },
        ];
        let vendor_homes = vec![(Path::new("/home/user/.claude").to_path_buf(), true)];
        let (workspace, user) = partition_rules_by_scope(
            files.clone(),
            Path::new("/home/user/.hystersis"),
            &vendor_homes,
            &[Path::new("/repo")],
        );
        let rules =
            xai_hystersis_agent::prompt::user_message::format_rules_section(&workspace, &user).unwrap();
        let reminder = xai_hystersis_agent::prompt::agents_md::format_agents_md_section(&files).unwrap();
        for body in [
            "repo-agents-body",
            "repo-claude-body",
            "home-hystersis-body",
            "home-claude-body",
            "repo-hystersis-rules-x",
        ] {
        ...
        assert!(rules.contains("name=\"/repo/.hystersis/rules/x.md\""));
    ...
    fn fork_ondisk_and_display_prefixes_both_count_as_workspace() {
        let files = vec![
            file("/home/user/.hystersis/worktrees/repo/AGENTS.md"),
            file("/home/user/repo/crates/foo/AGENTS.md"),
            file("/home/user/.hystersis/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/home/user/.hystersis"),
            &[],
            &[
                Path::new("/home/user/.hystersis/worktrees/repo"),
        ...
        assert_eq!(
            paths(&workspace),
            vec![
                "/home/user/.hystersis/worktrees/repo/AGENTS.md",
                "/home/user/repo/crates/foo/AGENTS.md",
            ]
        );
        assert_eq!(paths(&user), vec!["/home/user/.hystersis/AGENTS.md"]);
    }
}
...
/// end for hystersis ordering), so a failed offload never
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == Some(xai_hystersis_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    pub(super) async fn build_user_message_prefix(&self) -> String {
        ...
        if matches!(template, UserMessageTemplate::Default) && include_verification {
            ...
            user_rules.splice(
                0..0,
                xai_hystersis_agent::prompt::browser_verification::synthetic_user_rules(),
            );
            xai_hystersis_agent::prompt::user_message::append_rules_section(
                &mut out,
                &workspace_rules,
                &user_rules,
            );
    ...
    fn gather_partitioned_rules(
        ...
        let workspace_roots: Vec<&std::path::Path> = display_root
            ...
            .collect();
        partition_rules_by_scope(files, &hystersis_home, &vendor_homes, &workspace_roots)
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: xai_hystersis_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use xai_hystersis_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use xai_hystersis_workspace::file_system::{git_status_short, jj_status};
        use xai_hystersis_workspace::session::git::VcsKind;
    ...
    ) -> Vec<xai_hystersis_agent::prompt::user_message::McpServerEntry> {
        use xai_hystersis_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = xai_hystersis_sampler::SamplingClient::new(sampler_config).map_err(|e| {
