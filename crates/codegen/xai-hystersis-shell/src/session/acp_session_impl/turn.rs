...
use xai_hystersis_tools::implementations::hystersis_build::LoopFireMode;
...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &xai_hystersis_tools::implementations::hystersis_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(super) async fn handle_turn_input(
        ...
        let _active = xai_hystersis_telemetry::activity::TURNS_ACTIVE.enter();
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let prompt_blocks = match resolved {
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = xai_hystersis_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        xai_hystersis_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    xai_hystersis_telemetry::session_ctx::log_event(
                        xai_hystersis_telemetry::events::SlashCommandUsed {
                    ...
                    xai_hystersis_telemetry::session_ctx::log_event(
                        xai_hystersis_telemetry::events::SkillDispatched {
                            ...
                            trigger: xai_hystersis_telemetry::events::SkillTrigger::SlashCommand,
                        },
                    );
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        xai_hystersis_telemetry::session_ctx::log_event(
                            xai_hystersis_telemetry::events::PluginUsed {
        ...
        xai_hystersis_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
            session_id: self.session_info.id.0.to_string(),
            turn_number: turn_idx,
        });
        let current_prompt_index = self.chat_state_handle.get_prompt_index().await;
        xai_hystersis_telemetry::session_ctx::begin_prompt_id();
        let mut chunk_meta = serde_json::Map::new();
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = xai_hystersis_tools::util::base64_images::extract_base64_images(query);
        ...
        let model_id = self
            ...
            .map(|c| c.model)
            .unwrap_or_default();
        if self.telemetry_enabled || xai_hystersis_telemetry::external::is_active() {
            let effective_client_identifier =
                prompt_client_identifier.or_else(|| self.client_identifier.clone());
            let ev = xai_hystersis_telemetry::events::PromptSubmitted {
            ...
            xai_hystersis_telemetry::session_ctx::log_event_dual(self.telemetry_enabled, ev);
        }
        ...
        self.tool_bridge_handle()
            .update_resource(xai_hystersis_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        self.dispatch_hook(
            xai_hystersis_hooks::event::HookEventName::UserPromptSubmit,
            xai_hystersis_hooks::event::HookPayload::UserPromptSubmit {
        ...
        match &result {
            Ok(TurnOutcome::Completed { refusal, .. }) => {
                ...
                if let Some(explanation) = refusal {
                    self.report_turn_end(
                        prompt_id,
                        TurnEnd::Failed {
                            error: xai_hystersis_hooks::event::StopFailureKind::InvalidRequest,
                            error_details: None,
                ...
                .await;
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::TurnCompleted {
                        outcome: xai_hystersis_telemetry::events::Outcome::Completed,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::TurnCompleted {
                        outcome: xai_hystersis_telemetry::events::Outcome::Cancelled,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                xai_hystersis_telemetry::session_ctx::log_session_event(
                    xai_hystersis_telemetry::events::ApiError {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::TurnCompleted {
                        outcome: xai_hystersis_telemetry::events::Outcome::Error,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
                        cancellation_category: None,
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = xai_hystersis_tools::implementations::hystersis_build::monitor::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = xai_hystersis_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = xai_hystersis_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        if !self.memory.initial_injection_config.enabled {
            ...
            crate::session::memory_observation::log_memory_injection(
                ...
                xai_hystersis_telemetry::memory_telemetry::MemoryInjectionOutcome::Skipped,
                Default::default(),
            );
            return None;
        }
        ...
        use xai_hystersis_tools::types::memory_backend::MemoryBackend as _;
        ...
        let (outcome, mut inject_results) = match search_result {
            Ok(results) if results.is_empty() => (
                xai_hystersis_telemetry::memory_telemetry::MemoryInjectionOutcome::Empty,
                results,
            ),
            Ok(results) => (
                xai_hystersis_telemetry::memory_telemetry::MemoryInjectionOutcome::Results,
                results,
            ),
            Err(error) => {
                tracing::warn!(
                ...
                (
                    xai_hystersis_telemetry::memory_telemetry::MemoryInjectionOutcome::Error,
                    Vec::new(),
                )
            }
        };
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<xai_hystersis_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn is_run_true_step(
        &self,
        tool_calls: &[xai_hystersis_sampling_types::conversation::ToolCall],
    ) -> bool {
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            if identical_tool_calls.run_len >= identical_tool_calls.hard_stop_threshold() {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::ActionStationarityStop {
                        true_noop,
            ...
            if identical_tool_calls.take_nudge() {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::ActionStationarityNudge {
                        problematically_repeating,
                        run_len,
                        tool_name: tool_name.clone(),
                    },
            ...
            request.x_hystersis_agent_id = Some(xai_hystersis_telemetry::id::agent_id());
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            if xai_hystersis_tools::media_gen_limits::should_resample_egregious(
                &over_cap,
                media_gen_resamples,
                ...
                let reminder = xai_hystersis_tools::media_gen_limits::resample_reminder(&egregious);
                tracing::warn!(
                    session_id = %self.session_info.id,
            ...
            let turn_refused =
                stop_reason == Some(xai_hystersis_sampling_types::StopReason::ContentFilter);
            ...
            if is_true_noop {
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::ShellTrueNoop {
...
fn step_signature(tool_calls: &[xai_hystersis_sampling_types::conversation::ToolCall]) -> String {
...
mod identical_tool_call_run_tests {
    ...
    fn step_signature_ignores_key_order_and_call_order() {
        let call = |name: &str, args: &str| xai_hystersis_sampling_types::conversation::ToolCall {
            id: "id".into(),
            name: name.to_string(),
            arguments: args.into(),
        };
