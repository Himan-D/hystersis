...
impl SessionActor {
                ...
                if let ConversationItem::System(sys) = item {
                    if use_concise {
                        sys.content = std::sync::Arc::<str>::from(
                            xai_hystersis_agent::prompt::template::COMPACT_SYSTEM_PROMPT,
                        );
                    } else {
                        sys.content =
    ...
    ///
    /// Builds a fresh [`xai_hystersis_agent::Agent`] from the cached
    /// [`crate::session::agent_rebuild::AgentRebuildSpec`] + the supplied
    /// [`xai_hystersis_agent::AgentDefinition`], replaces `self.agent`,
