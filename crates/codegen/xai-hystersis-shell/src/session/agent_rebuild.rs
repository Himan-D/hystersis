...
//! [`xai_hystersis_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls
//! [`xai_hystersis_agent::AgentBuilder::new`]. Both initial session spawn
...
//! ## Why this exists
//!
//! [`xai_hystersis_agent::Agent`] owns an [`xai_hystersis_tools::bridge::ToolBridge`]
...
//!
//! ## WHEN ADDING A NEW [`xai_hystersis_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use xai_hystersis_agent::config::AgentDefinition;
use xai_hystersis_agent::error::AgentBuildError;
use xai_hystersis_agent::prompt::context::PromptAudience;
use xai_hystersis_agent::prompt::skills::SkillsConfig;
use xai_hystersis_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
...
use xai_hystersis_tools::implementations::hystersis_build::ask_user_question::types::UserQuestionRequest;
...
use xai_hystersis_tools::implementations::hystersis_build::task::types::{SubagentEvent, TaskModelValidator};
...
use xai_hystersis_tools::implementations::lsp::LspBackend;
...
use xai_hystersis_tools::types::SharedApiKeyProvider;
use xai_hystersis_tools::types::compat::CompatConfig;
use xai_hystersis_tools::types::memory_backend::MemoryBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub web_search_domains: Option<xai_hystersis_sampling_types::WebSearchOptions>,
    ...
    pub attribution_callback: Option<xai_hystersis_tools::SharedAttributionCallback>,
    ...
    pub managed_gateway_tool_client:
        Option<xai_hystersis_tools::types::resources::ManagedGatewayToolClient>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        let session_env = {
            ...
            env.insert("HYSTERSIS_SESSION_ID".to_string(), session_id_str.clone());
            Arc::new(env)
        };
        ...
        agent
            ...
            .update_resources_with(|resources| {
                ...
                if let Some(event_tx) = subagent_event_tx.clone() {
                    use xai_hystersis_tools::implementations::hystersis_build::task::backend::{
                ...
                resources
                    .insert(
                        xai_hystersis_tools::types::resources::RespectGitignore(
                            *respect_gitignore,
                        ),
                    );
                resources
                    .insert(
                        xai_hystersis_tools::types::resources::SchedulerBackgroundLoops(
                            *scheduler_background_loops,
                        ),
                    );
                resources
                    .insert(
                        xai_hystersis_tools::types::resources::PathNotFoundHints(
                            *path_not_found_hints,
                        ),
                    );
                ...
                {
                    use xai_hystersis_tools::implementations::hystersis_build::ask_user_question::UserQuestionSender;
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            xai_hystersis_tools::computer::local::LocalTerminalBackend::new_local(
                xai_hystersis_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
        ...
        subagent_depth: 0,
        subagents_max_depth: xai_hystersis_tools::implementations::hystersis_build::task::MAX_SUBAGENT_DEPTH,
        ...
        system_prompt_label: xai_hystersis_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(xai_hystersis_tools::types::tool::ToolKind::Task)
            .expect("HystersisBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
            .find(|definition| definition.function.name == task_name)
            .and_then(|definition| definition.function.description)
            .expect("HystersisBuild Task description should be present")
    }
    /// The `[toolset.web_search]` policy is authoritative on the backend-hosted
    /// path: agent frontmatter is model-writable (`.hystersis/agents/*.md`), so a
    ...
    async fn config_web_search_domains_beat_agent_frontmatter() {
        use xai_hystersis_sampling_types::{HostedTool, ToolOverrides, WebSearchOptions};
        ...
        let definition = || {
            let mut d = AgentDefinition::default_hystersis_build();
    ...
    async fn rebuild_projects_fresh_public_model_keys_into_task_description() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let first = spec
                    .build_agent(AgentDefinition::default_hystersis_build())
                    .await
