...
//!
//! Hooks registered at `session/new` (`_meta["hystersis.com/hooks"]`) come in two flavors,
//! both matched by the agent ([`xai_hystersis_hooks::matcher::HookMatcher`], shared with
//! file hooks):
//! - **Gates** (awaited reverse *requests* `hystersis.com/hooks/run`):
...
//! - **All other events**: fire-and-forget *notifications* `hystersis.com/hooks/event`,
...
use xai_hystersis_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use xai_hystersis_telemetry::events::ClientHookGateOutcome;
...
const HOOK_EVENT_METHOD: &str = "hystersis.com/hooks/event";
const HOOK_RUN_METHOD: &str = "hystersis.com/hooks/run";
...
/// Outcome of the `hystersis.com/hooks/run` reverse request, before interpreting it as a
    ...
    match outcome {
        ReverseOutcome::Responded(raw) => {
            match serde_json::from_str::<ClientHookResponse>(raw.get()) {
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "hystersis.com/hooks/run returned an unknown decision value; failing open"
                            );
                ...
                Err(err) => {
                    tracing::warn!(%err, "malformed hystersis.com/hooks/run response; failing open");
                    (
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "hystersis.com/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("hystersis.com/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
fn matching_callback_ids<'a>(
    ...
    groups
        .iter()
        .filter(|group| {
            xai_hystersis_hooks::matcher::matcher_allows(group.matcher.as_ref(), match_value)
        })
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::HookBlocked {
    ...
    /// Fan one `hystersis.com/hooks/run` gate dispatch out to every matching callback,
    ...
    > {
        let default_timeout =
            if envelope.hook_event_name.traits().gate == xai_hystersis_hooks::event::GateKind::Stop {
        ...
        groups
            .iter()
            .filter(move |group| {
                xai_hystersis_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    xai_hystersis_telemetry::session_ctx::log_event(
                        xai_hystersis_telemetry::events::ClientHookGate {
    ...
    /// `hystersis.com/hooks/run` once per matching callback with the shared `envelope` (the
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> xai_hystersis_hooks::dispatcher::StopDispatchResult {
        use xai_hystersis_hooks::result::HookRunResult;

        let mut out = xai_hystersis_hooks::dispatcher::StopDispatchResult::default();
        ...
        for (callback_id, (response, elapsed, gate_outcome)) in ordered {
            ...
            let detail = xai_hystersis_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                xai_hystersis_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
    ...
    /// Issue one `hystersis.com/hooks/run` reverse request, bounded by a per-callback `timeout`.
    ...
    /// `hystersis.com/hooks/event` notification to each matching registered callback.
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    xai_hystersis_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.may_have_hooks_for(HookEventName::Stop));
