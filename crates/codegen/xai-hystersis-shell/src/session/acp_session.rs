...
use xai_hystersis_agent::prompt::agents_md::LEGACY_AGENTS_MD_REMINDER_PREFIX;
use xai_hystersis_agent::prompt::skills::SkillsConfig;
use xai_hystersis_sampler::SamplerConfig as SamplingConfig;
use xai_hystersis_sampling_types::truncate_bytes;
...
use xai_hystersis_tools::types::compat::CompatConfig;
use xai_hystersis_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use xai_hystersis_workspace::file_system::CodebaseIndexManager;
use xai_hystersis_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use xai_hystersis_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<xai_hystersis_sampling_types::ToolOverridesUpdate>,
...
impl xai_hystersis_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        xai_hystersis_tools::types::resources::ManagedGatewayToolCallResponse,
...
pub(crate) struct SessionActor {
    ...
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<xai_hystersis_sampler::SharedAttributionCallback>,
    ...
    pub(crate) tool_overrides: std::cell::RefCell<Option<xai_hystersis_sampling_types::ToolOverrides>>,
    ...
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<xai_hystersis_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<xai_hystersis_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<xai_hystersis_sampling_types::CompactionAtTokens>>,
    ...
    pub(crate) doom_loop_recovery: Option<xai_hystersis_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) agent: std::cell::RefCell<xai_hystersis_agent::Agent>,
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<xai_hystersis_tools::implementations::hystersis_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<xai_hystersis_hooks::discovery::HookRegistry>>>,
    ...
    pub(crate) vcs_kind: xai_hystersis_workspace::session::git::VcsKind,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<xai_hystersis_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<xai_hystersis_agent::plugins::SharedPluginRegistryHandle>,
    ...
    pub(crate) pending_image_strip:
        parking_lot::Mutex<Option<(xai_hystersis_sampler::RequestId, Vec<std::sync::Arc<str>>)>>,
    ...
    pub(crate) sampler_handle: xai_hystersis_sampler::SamplerHandle,
    ...
    /// Cached recipe for constructing this session's [`xai_hystersis_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`xai_hystersis_tools::bridge::ToolBridge`], tool
    ...
    pub(crate) workspace_ops: xai_hystersis_workspace::WorkspaceOps,
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == xai_hystersis_tools::implementations::hystersis_build::SCHEDULER_CREATE_TOOL_NAME
            }),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == xai_hystersis_tools::implementations::hystersis_build::workflow::WORKFLOW_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<xai_hystersis_tools::bridge::ToolBridge> {
...
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &xai_hystersis_agent::PromptContext) {
...
) -> Option<xai_hystersis_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use xai_hystersis_tools::types::output::{MCPOutput, ToolOutput};
    use xai_hystersis_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl xai_hystersis_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    ...
    use super::*;
    use tokio::sync::mpsc;
    use xai_hystersis_tools::registry::types::ToolConfig;
    use xai_hystersis_tools::tool_taxonomy::TOOL_META_KEY;
    use xai_hystersis_workspace::permission::PermissionCommand;
                        ...
                        } = cmd
                        {
                            *captured_in_task.lock().await = Some(tool_call_update);
                            let _ = respond_to.send(
                                xai_hystersis_workspace::permission::PermissionResolution {
...
mod managed_gateway_tool_tests {
    ...
    use xai_hystersis_tools::types::output::{MCPOutput, ToolOutput};
    use xai_hystersis_tools::types::tool::{ToolKind, ToolNamespace};
    use xai_hystersis_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<xai_hystersis_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
