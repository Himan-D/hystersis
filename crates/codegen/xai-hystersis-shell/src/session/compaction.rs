...
use xai_hystersis_sampling_types::{ApiBackend, ConversationItem};
...
impl SessionActor {
    ...
    async fn maybe_pre_compaction_flush(
        ...
        if xai_hystersis_memory::flush::should_flush(
            total_tokens,
            context_window,
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                xai_hystersis_telemetry::events::CompactionTrigger::Manual,
                false,
            )
            .await
        {
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            xai_hystersis_telemetry::session_ctx::log_event(
                xai_hystersis_telemetry::events::AutoCompactSuppressed {
    ...
    pub(crate) async fn surface_compact_auth_failure(&self, err: acp::Error) -> acp::Error {
        ...
        acp::Error::auth_required().data(crate::sampling::error::terminal_error_data(
            message,
            Some(401),
            xai_hystersis_sampler::SamplingErrorKind::Auth,
        ))
    }
    ...
    async fn run_compact_inner(
        ...
        trigger: xai_hystersis_telemetry::events::CompactionTrigger,
        lossy_input: bool,
    ) -> Result<(), acp::Error> {
        let (cancel, _cancel_scope) = self.compaction.cancel.enter();
        ...
        let trigger_str = match trigger {
            xai_hystersis_telemetry::events::CompactionTrigger::Manual => "manual",
            xai_hystersis_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = xai_hystersis_telemetry::events::CompactionScope::begin(
            xai_hystersis_telemetry::events::CompactionBeginParams {
                trigger,
                ...
                compaction_mode: match self.compaction.compaction_mode {
                    xai_chat_state::CompactionMode::Summary => {
                        xai_hystersis_telemetry::events::CompactionModeLabel::Summary
                    }
                    xai_chat_state::CompactionMode::Transcript => {
                        xai_hystersis_telemetry::events::CompactionModeLabel::Transcript
                    }
                    xai_chat_state::CompactionMode::Segments(_) => {
                        xai_hystersis_telemetry::events::CompactionModeLabel::Segments
                    }
                },
        ...
        self.dispatch_hook(
            xai_hystersis_hooks::event::HookEventName::PreCompact,
            xai_hystersis_hooks::event::HookPayload::PreCompact {
        ...
        let effective_tool_defs: Vec<xai_hystersis_sampling_types::ToolDefinition> = self
        ...
        let compaction_tools: Vec<xai_hystersis_sampling_types::ToolSpec> = effective_tool_defs
            .into_iter()
            .map(xai_hystersis_sampling_types::ToolSpec::from)
            .collect();
        let compaction_hosted_tools: Vec<xai_hystersis_sampling_types::HostedTool> =
        ...
        let auto_trigger = matches!(trigger, xai_hystersis_telemetry::events::CompactionTrigger::Auto);
        let wall_clock_budget_secs = self
            .agent
            .borrow()
            .compaction_policy()
        ...
        let fr_config = xai_hystersis_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match xai_hystersis_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(xai_hystersis_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(xai_hystersis_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(xai_hystersis_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    ...
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            xai_hystersis_telemetry::session_ctx::log_event(
                                xai_hystersis_telemetry::events::CompactionRetryDegraded {
                                    trigger,
            ...
            if use_short_prompt {
                ...
                let ctx = {
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                xai_hystersis_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                xai_hystersis_tools::computer::types::TaskKind::Bash => {
                    ...
                    let todos = {
                        ...
                        use xai_hystersis_tools::types::resources::State;
        ...
        let memory_ref: Option<&dyn xai_hystersis_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn xai_hystersis_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        xai_hystersis_sampling_types::ContentPart::Text { text } => {
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: xai_hystersis_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        self.dispatch_hook(
            xai_hystersis_hooks::event::HookEventName::PostCompact,
            xai_hystersis_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
        ...
        compaction.complete(
            xai_hystersis_telemetry::events::CompactionCompleteStats {
                tokens_after,
                two_pass_used,
                segments_queued,
            ...
            xai_hystersis_telemetry::events::CompactionTiming {
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &xai_hystersis_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn run_compact_only(
        ...
        xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                xai_hystersis_telemetry::events::CompactionTrigger::Auto,
                lossy_input,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<xai_hystersis_sampling_types::ToolSpec>,
        user_context: Option<&str>,
