...
use xai_hystersis_workspace::file_system::MockFs;
use xai_hystersis_workspace::permission::PermissionHandle;
#[derive(Debug)]
...
fn switch_target_config(model: &str, base_url: String) -> xai_hystersis_sampler::SamplerConfig {
    xai_hystersis_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.replace_conversation(vec![
                ...
                ConversationItem::Reasoning(xai_hystersis_sampling_types::rs::ReasoningItem {
                ...
                ConversationItem::assistant_tool_calls(vec![xai_hystersis_sampling_types::ToolCall {
                    id: std::sync::Arc::<str>::from("call_xai_minted_id"),
                    name: "run_terminal_command".to_string(),
                    arguments: std::sync::Arc::<str>::from(r#"{"command":"ls"}"#),
                }]),
                ConversationItem::ToolResult(xai_hystersis_sampling_types::ToolResultItem {
...
fn api_error_with_context_window(context_window: u64) -> xai_hystersis_sampler::SamplingErrorInfo {
...
async fn test_compact_on_error_noop_without_model_metadata() {
    ...
    local
        .run_until(async {
            ...
            let err = xai_hystersis_sampler::SamplingErrorInfo {
                kind: xai_hystersis_sampler::SamplingErrorKind::Api,
                status_code: Some(400),
                message: "prompt is too long".to_string(),
                is_retryable: false,
