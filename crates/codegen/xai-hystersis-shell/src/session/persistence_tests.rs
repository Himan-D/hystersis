...
fn test_actor_inner(
    ...
    let sampling_client = OaiCompatClient::new(xai_hystersis_sampler::SamplerConfig::default()).unwrap();
    let mut summary =
...
fn save_session_titles(
    server: &xai_hystersis_test_support::MockInferenceServer,
    session_id: &str,
) -> Vec<SaveTitle> {
...
fn request_path_summary(server: &xai_hystersis_test_support::MockInferenceServer) -> Vec<String> {
    server
        .requests()
        .iter()
