...
use xai_hystersis_tools::implementations::skills::skill::format_skill_name;
...
/// Unconditional reservations for `hystersis inspect`. Live advertising still
...
pub(crate) struct ListCommandsRequest {
    ...
    /// Product lane: `"chat"` filters to Hystersis Chat / Hystersis Computer first-party
...
/// list, and per-turn resolve do not stampede hystersis.com.
...
fn product_skills_negative_stamp(auth: &crate::auth::HystersisAuth) -> ProductSkillsIdentityStamp {
...
fn clear_degraded_cache_for_auth(auth: &crate::auth::HystersisAuth) {
...
fn clear_negative_cache_for_auth(auth: &crate::auth::HystersisAuth) {
...
/// Product (hystersis.com) Skills catalog as SkillInfo rows for slash advertising
...
pub(crate) async fn product_skill_infos(
    ...
    let hystersis_auth = match auth.auth().await {
        Ok(a) => a,
        Err(err) => {
    ...
    if let Some(skills) = product_skills_cache_lookup(&hystersis_auth) {
        return skills;
    }
    ...
    match client.try_list_catalog(PRODUCT_SKILLS_LOCALE).await {
        Ok((catalog, used_untagged_recovery)) if catalog.user_list_failed => {
            let skills = catalog.to_skill_infos();
            {
                ...
                if let Some(entry) = guard.as_ref()
                    && product_skills_cache_matches(entry, &hystersis_auth)
                {
                    tracing::warn!(
            ...
            *PRODUCT_SKILLS_DEGRADED_CACHE.lock() = Some(product_skills_cache_entry_after_fetch(
            ...
            clear_negative_cache_for_auth(&hystersis_auth);
            tracing::warn!(
        ...
        Ok((catalog, used_untagged_recovery)) => {
            ...
            *PRODUCT_SKILLS_CACHE.lock() = Some(product_skills_cache_entry_after_fetch(
            ...
            clear_degraded_cache_for_auth(&hystersis_auth);
            clear_negative_cache_for_auth(&hystersis_auth);
            Some(skills)
        }
        Err(err) => {
            ...
            *PRODUCT_SKILLS_NEGATIVE_CACHE.lock() = Some(product_skills_negative_stamp(&hystersis_auth));
            tracing::warn!(
                error = %err,
...
fn product_skills_cache_lookup(
    hystersis_auth: &crate::auth::HystersisAuth,
    ...
    }
    {
        ...
        if let Some(entry) = guard.as_ref()
            && product_skills_negative_matches(entry, hystersis_auth)
...
///   catalog** (same as hystersis-web) + builtins — not Build disk skills. Without
...
///   matching today's Hystersis flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use xai_hystersis_tools::implementations::skills::skill::{
...
/// `xai-hystersis-tools` so it stays identical to the pager's `LoopCommand` and the
