...
fn chat_completion_bodies(server: &xai_hystersis_test_support::MockInferenceServer) -> Vec<String> {
    server
        .requests()
        .into_iter()
        .filter(|r| r.path == "/v1/chat/completions")
        .filter(|r| {
            !r.header("x-hystersis-req-id")
...
fn session_chat_jsonl() -> std::path::PathBuf {
    let sessions = std::path::PathBuf::from(std::env::var("HYSTERSIS_HOME").expect("HYSTERSIS_HOME set"))
        .join("sessions");
