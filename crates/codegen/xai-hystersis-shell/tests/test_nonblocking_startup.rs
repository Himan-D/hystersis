...
//!
//! ```bash
//! cargo test -p xai-hystersis-shell --test test_nonblocking_startup -- --ignored
//! ```
...
use xai_hystersis_test_support::leader::LeaderFixture;
...
/// The background catalog refresh must re-fetch and push `hystersis.com/models/update`
        ...
        .run_until(async {
            ...
            common::leader::run_with_cleanup(&fixture, &mut clients, |fixture, clients| {
                Box::pin(async move {
                    ...
                    // (5s-base backoff) re-fetches and pushes `hystersis.com/models/update`.
                    ...
                    assert!(
                        healed,
                        "no hystersis.com/models/update after the endpoint recovered\nstderr:\n{}",
                        clients[0].stderr_text(),
                    );
                })
            })
