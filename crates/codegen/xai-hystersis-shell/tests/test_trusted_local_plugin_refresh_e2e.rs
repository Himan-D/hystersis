...
//! 4. Smoke-validate session JSON under `$HYSTERSIS_HOME/sessions/` after exit.
//!
//! Requires a built `hystersis` binary (`HYSTERSIS_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p xai-hystersis-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p xai-hystersis-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
use xai_hystersis_agent::plugins::SharedPluginRegistryHandle;
use xai_hystersis_agent::plugins::discovery::DiscoveryConfig;
use xai_hystersis_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _hystersis_guard = EnvVarGuard::set("HYSTERSIS_HOME", &hystersis_home);
    ...
    let mut registry = InstallRegistry::empty(hystersis_home.join("installed-plugins"));
    ...
    // hystersis_home() caches the first HYSTERSIS_HOME per process; a separate test
    ...
    assert_eq!(
        plugin.scope,
        xai_hystersis_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built hystersis binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    let home = dunce::canonicalize(home_tmp.path()).unwrap();
    let hystersis_home = home.join(".hystersis");
    std::fs::create_dir_all(&hystersis_home).unwrap();
    ...
    // The spawned binary gets HOME/HYSTERSIS_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via hystersis_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _hystersis_guard = EnvVarGuard::set("HYSTERSIS_HOME", &hystersis_home);
    ...
    sandbox
        ...
        .set_env("HYSTERSIS_HOME", &hystersis_home);
    ...
    // Smoke: session storage under HYSTERSIS_HOME/sessions has JSON artifacts.
    let sessions_root = hystersis_home.join("sessions");
    assert!(
