...
//!
//! ```bash
//! LEADER_SOAK_SECS=1200 cargo test -p xai-hystersis-shell --features test-support \
...
use xai_hystersis_test_support::resources::ResourceSnapshot;
...
/// Per-registry entry counts. `hystersis.com/debug/agent` answers under the extension
...
async fn registry_counts(client: &mut LeaderClient, id: u64) -> serde_json::Value {
    let resp = rpc(
        client,
        format!(r#"{{"jsonrpc":"2.0","id":{id},"method":"_hystersis.com/debug/agent","params":{{}}}}"#),
        id,
        "hystersis.com/debug/agent",
    )
    .await;
    let counts = resp["result"]["result"]["registries"].clone();
    assert!(
        counts.is_object(),
        "hystersis.com/debug/agent returned no registries: {resp}"
    );
    counts
}
...
async fn leader_soak_churning_clients_no_leaks_no_zombies() {
    ...
    let sock_path = hystersis_home.path().join("leader-soak.sock");
    ...
    local
        .run_until(async {
            ...
            xai_hystersis_shell::leader::in_process::spawn_agent(acp_rx, response_tx.clone());
                ...
                for (i, client) in clients.iter_mut().enumerate() {
                    for s in 0..2u64 {
                        ...
                        rpc(
                            client,
                            format!(
                                r#"{{"jsonrpc":"2.0","id":{close_id},"method":"_hystersis.com/session/close","params":{{"sessionId":"{sid}"}}}}"#
                            ),
                            close_id,
