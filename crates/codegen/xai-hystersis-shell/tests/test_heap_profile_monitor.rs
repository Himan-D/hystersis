...
use xai_hystersis_shell::auth::{AuthManager, AuthMode, HystersisAuth, HystersisComConfig};
use xai_hystersis_shell::heap_profile::{
...
use xai_hystersis_test_support::{EnvGuard, MockInferenceServer};
use xai_hystersis_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = HystersisComConfig::default().auth_scope();
...
fn assert_storage_auth(uploads: &[xai_hystersis_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), HystersisComConfig::default()));
