...
//!   cargo test --release -p xai-hystersis-shell --test subagent_latency_sweep \
...
//!
//! Knobs: HYSTERSIS_SWEEP_NS, HYSTERSIS_SWEEP_ISOLATION ("none"|"worktree"),
//! HYSTERSIS_SWEEP_DEADLINE_S, HYSTERSIS_SWEEP_REPO_FILES,
//! HYSTERSIS_SWEEP_ASSERT_NO_FAILURES, HYSTERSIS_SWEEP_LOG.
...
fn subagent_burst_latency_sweep() {
    ...
    let ns: Vec<usize> = std::env::var("HYSTERSIS_SWEEP_NS")
        .unwrap_or_else(|_| "1,5,10,25,50".to_string())
        .split(',')
        .filter_map(|s| s.trim().parse::<usize>().ok())
        .filter(|&v| v > 0)
        .collect();
    let isolation = std::env::var("HYSTERSIS_SWEEP_ISOLATION").unwrap_or_else(|_| "none".to_string());
