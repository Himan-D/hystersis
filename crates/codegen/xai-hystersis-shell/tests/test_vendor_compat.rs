...
//! `.hystersis`, `.cursor`, and `.claude` vendor dirs, spawns a real `hystersis agent
//! stdio` process against the mock inference server (toggling the
//! `HYSTERSIS_<VENDOR>_<SURFACE>_ENABLED` env vars via `cmd.env`), sends one prompt,
//! and asserts on the full inference request bodies:
//!
//! - the Hystersis-native skill is always present regardless of toggles
...
//! ```bash
//! cargo test -p xai-hystersis-shell --test test_vendor_compat -- --ignored
//! ```
...
const MARKER_HYSTERSIS_SKILL: &str = "ZZ_HYSTERSIS_SKILL_MARKER";
...
fn seed_fixtures(home: &Path, cwd: &Path) {
    ...
    write_skill(home, ".hystersis", "hystersis-skill", MARKER_HYSTERSIS_SKILL);
...
async fn run_scenario(env: &[(&str, &str)]) -> String {
    ...
    let client = HystersisStdioClient::spawn_with_sandbox(&server, workdir.workspace(), sandbox).await;
...
/// Defaults (all vendors on): hystersis + cursor-vendor + claude-vendor skills present; the
...
async fn vendor_compat_defaults_include_vendor_skills_but_drop_denylisted() {
    with_local_set(|| async {
        ...
        assert!(
            body.contains(MARKER_HYSTERSIS_SKILL),
            "hystersis-skill must always be present"
        );
        assert!(
...
/// `HYSTERSIS_CURSOR_SKILLS_ENABLED=false` drops the cursor-vendor skill; hystersis stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("HYSTERSIS_CURSOR_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_HYSTERSIS_SKILL),
            "hystersis-skill always present"
        );
        assert!(
            !body.contains(MARKER_CURSOR_SKILL),
...
/// `HYSTERSIS_CLAUDE_SKILLS_ENABLED=false` drops the claude-vendor skill; hystersis stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_skills_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("HYSTERSIS_CLAUDE_SKILLS_ENABLED", "false")]).await;
        assert!(
            body.contains(MARKER_HYSTERSIS_SKILL),
            "hystersis-skill always present"
        );
...
/// `HYSTERSIS_CURSOR_RULES_ENABLED=false` drops cursor-vendor rules; claude-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("HYSTERSIS_CURSOR_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_RULE),
...
/// `HYSTERSIS_CLAUDE_RULES_ENABLED=false` drops claude-vendor rules; cursor-vendor rules stay.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_rules_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("HYSTERSIS_CLAUDE_RULES_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_RULE),
...
/// `HYSTERSIS_CURSOR_AGENTS_ENABLED=false` drops cursor-vendor AGENTS.md; claude-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_cursor_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("HYSTERSIS_CURSOR_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CURSOR_AGENTS),
...
/// `HYSTERSIS_CLAUDE_AGENTS_ENABLED=false` drops claude-vendor AGENTS.md; cursor-vendor stays.
#[tokio::test]
#[ignore] // requires pre-built binary
async fn vendor_compat_claude_agents_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[("HYSTERSIS_CLAUDE_AGENTS_ENABLED", "false")]).await;
        assert!(
            !body.contains(MARKER_CLAUDE_AGENTS),
...
async fn vendor_compat_all_cursor_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("HYSTERSIS_CURSOR_SKILLS_ENABLED", "false"),
            ("HYSTERSIS_CURSOR_RULES_ENABLED", "false"),
            ("HYSTERSIS_CURSOR_AGENTS_ENABLED", "false"),
        ])
        .await;
        ...
        assert!(body.contains(MARKER_HYSTERSIS_SKILL), "hystersis always present");
...
async fn vendor_compat_all_claude_disabled() {
    with_local_set(|| async {
        let body = run_scenario(&[
            ("HYSTERSIS_CLAUDE_SKILLS_ENABLED", "false"),
            ("HYSTERSIS_CLAUDE_RULES_ENABLED", "false"),
            ("HYSTERSIS_CLAUDE_AGENTS_ENABLED", "false"),
        ])
        .await;
...
/// All vendor compat OFF: only hystersis-native skill survives.
#[tokio::test]
#[ignore] // requires pre-built binary
