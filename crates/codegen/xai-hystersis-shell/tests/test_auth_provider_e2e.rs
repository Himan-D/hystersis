...
//!
//! Runs the built hystersis binary headless against the mock inference server with
...
//! the release artifact via `HYSTERSIS_BINARY`; run locally (auto-builds the pager):
//! ```bash
//! cargo test -p xai-hystersis-shell --test test_auth_provider_e2e -- --ignored
//! ```
//!
...
async fn provider_backed_model_sends_minted_token_on_the_wire() {
    ...
    sandbox.remove_env("HYSTERSIS_LEADER_SOCKET");

    let hystersis_home = sandbox.hystersis_home().to_path_buf();
    std::fs::create_dir_all(&hystersis_home).expect("create .hystersis home");

    let counter = hystersis_home.join("mint-count");
    let helper = hystersis_home.join("mock-auth.sh");
    std::fs::write(
        &helper,
        format!(
...
async fn provider_with_args_and_json_output_sends_minted_token() {
    ...
    let seen_args = hystersis_home.join("seen-args");
    let helper = hystersis_home.join("mock-auth-json.sh");
    std::fs::write(
        &helper,
        format!(
            "#!/bin/sh\nprintf '%s' \"$*\" > {}\n\
