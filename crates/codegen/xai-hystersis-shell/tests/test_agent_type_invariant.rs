...
//! - Session resume
//!
//! Each test spawns a real `hystersis agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p xai-hystersis-shell --test test_agent_type_invariant -- --ignored
//! ```
...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".hystersis").join("models_cache.json");
    if cache.exists() {
        std::fs::remove_file(&cache).expect("failed to delete models_cache.json");
    }
}
/// Hystersis-build system template marker. Absence means the alternate harness.
fn is_hystersis_build_system_prompt(sys_prompt: &str) -> bool {
    sys_prompt.contains("<work_policy>")
}
/// Alternate-agent compiled prompts: hystersis's `<work_policy>` is absent
...
fn assert_alternate_harness(server: &MockInferenceServer, workspace: &std::path::Path) {
    ...
    assert!(
        !is_hystersis_build_system_prompt(&sys_prompt),
        "alternate template must not use hystersis `<work_policy>`\n\
...
/// - `default-model`: no agent_type (→ defaults to "hystersis")
...
/// - `model-a`: no agent_type (→ "hystersis")
/// - `model-b`: no agent_type (→ "hystersis")
...
/// `hystersis` harness. The system prompt sent to the LLM should contain
/// the hystersis identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_hystersis_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            is_hystersis_build_system_prompt(&sys_prompt),
            "default model should use hystersis harness\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
    })
    .await;
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        drop(writer);
        let reader =
            HystersisStdioClient::spawn_with_sandbox(&server, workdir.workspace(), shared_sandbox).await;
        ...
        let original_is_hystersis = is_hystersis_build_system_prompt(&original_sys_prompt);
        let resumed_is_hystersis = is_hystersis_build_system_prompt(&resumed_sys_prompt);
        assert_eq!(
            original_is_hystersis,
            resumed_is_hystersis,
            "resumed session should use the same harness as the original\n\
             original hystersis template: {original_is_hystersis}\n\
             resumed hystersis template: {resumed_is_hystersis}\n\
...
/// default to `"hystersis"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_hystersis_build() {
    with_local_set(|| async {
            ...
            assert!(
            is_hystersis_build_system_prompt(&sys_prompt),
            "model without agent_type should default to hystersis harness\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
}
/// The `HYSTERSIS_AGENT` escape hatch should override the model's agent_type.
/// Setting `HYSTERSIS_AGENT=hystersis` with an alternate-agent model should use
/// hystersis harness.
#[tokio::test]
#[ignore]
async fn test_hystersis_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            assert!(
            result.is_ok(),
            "prompt with HYSTERSIS_AGENT override failed: {:?}\nstderr:\n{}",
            result.err(),
            client.stderr()
        );
            let sys_prompt = server
            ...
            assert!(
            is_hystersis_build_system_prompt(&sys_prompt),
            "HYSTERSIS_AGENT=hystersis should override catalog model agent_type\nsystem prompt preview: {}",
            &sys_prompt[..sys_prompt.len().min(500)]
        );
        })
        .await;
