//! One binary, one home: `hystersis_home()` memoizes the first read for the process, so tests that
//! need a temp home have to share one, and `#[serial]` keeps their env writes apart.

use std::sync::{Arc, OnceLock};

use agent_client_protocol as acp;
use xai_hystersis_shell::auth::{AuthManager, HystersisComConfig};
use xai_hystersis_shell::session::info::Info;
use xai_hystersis_shell::session::persistence::delete_session_history;
use xai_hystersis_shell::session::storage::search::{
...
fn home() -> &'static std::path::Path {
    ...
    HOME.get_or_init(|| {
        ...
        let guard = EnvGuard::set("HYSTERSIS_HOME", dir.path());
        (dir, guard)
    })
    .0
    .path()
...
fn start_index() -> SearchIndexManager {
    let _default_on = EnvGuard::unset("HYSTERSIS_SESSION_SEARCH");
    match start_if_enabled(&xai_hystersis_shell::agent::config::Config::default()) {
...
async fn deleting_a_session_clears_only_its_own_search_row() {
    ...
    let auth = Arc::new(AuthManager::new(root, HystersisComConfig::default()));

    let session_dir =
        xai_hystersis_shell::util::hystersis_home::sessions_cwd_dir_in(root, "/ws-a").join("orphan");
...
fn loading_config_applies_requirement_pins() {
    ...
    let loaded = xai_hystersis_shell::config::load_agent_config_disk_only();
    std::fs::remove_file(&pin).unwrap();
    let config = loaded.expect("config loads");

    let resolved = config.feature(xai_hystersis_shell::agent::config::Feature::SessionSearch);
    assert!(
        !resolved.value,
    ...
    assert_eq!(
        resolved.source,
        xai_hystersis_shell::agent::config::ConfigSource::Requirement
    );
}
