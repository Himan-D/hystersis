...
//! asserts via `hystersis.com/debug/agent` that every registry count returns
...
async fn read_counts(conn: &acp::ClientSideConnection) -> Counts {
    let resp = ext_method(conn, "hystersis.com/debug/agent", json!({})).await;
    serde_json::from_value(resp["result"]["registries"].clone())
        .unwrap_or_else(|e| panic!("hystersis.com/debug/agent: bad registries payload: {e}\n{resp}"))
}
...
async fn close_session(conn: &acp::ClientSideConnection, session_id: &acp::SessionId) {
    ...
    assert_eq!(
        resp["result"]["success"],
        json!(true),
        "hystersis.com/session/close on {} failed: {resp}",
        session_id.0
    );
}
