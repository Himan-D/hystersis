...
//!
//! `#[ignore]`d by default: it needs the hystersis binary (`HYSTERSIS_BINARY` or a local debug build).
//! `cargo test -p xai-hystersis-shell --test test_stop_cancelled_hook_e2e -- --ignored`
...
async fn stop_cancelled_hook_fires_with_the_matched_reason() {
    ...
    let hooks = sandbox.hystersis_home().join("hooks");
