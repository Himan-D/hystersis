...
- **Using Hystersis**
  ...
  - [SSH Passthrough](#ssh-passthrough-hystersis-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`hystersis inspect`)](#introspection)
...
- [Building with Hystersis](#building-with-hystersis) — headless API, ACP SDK integration
...
curl -fsSL https://hystersis.com/cli/install.sh | bash -s 0.1.42
```
...
On first launch, Hystersis opens your browser to authenticate with hystersis.com:

```bash
hystersis
```

Credentials are stored in `~/.hystersis/auth.json` and persist across sessions. Tokens expire after 7 days; Hystersis will prompt you to re-authenticate when needed.
...
For CI/CD, automation, or environments without browser access, use an API key from [hystersis.com](https://hystersis.com):
...
Authenticate developers via your own Identity Provider (Okta, Azure AD, Auth0) instead of `accounts.hystersis.com`.
...
```toml
# ~/.hystersis/config.toml
[hystersis_com_config.oidc]
...
export HYSTERSIS_OIDC_ISSUER="https://acme.okta.com"
export HYSTERSIS_OIDC_CLIENT_ID="0oa1b2c3d4e5f6g7h8i9"
```

Customers typically also override the API endpoint to point at their own proxy:
```bash
export HYSTERSIS_CLI_CHAT_PROXY_BASE_URL="https://hystersis-proxy.acme.com/v1"
```

**3. Run `hystersis`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.hystersis/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Hystersis is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.

#### How It Works

```
┌──────────────┐     sh -c     ┌────────────────────────┐
│     Hystersis     │──────────────▶│  your auth binary      │
│              │               │                        │
...
1. Hystersis runs your command via `sh -c "<command>"`
...
4. **stdout** → captured by Hystersis and saved to `~/.hystersis/auth.json` as the access token
5. exit 0 → success; exit non-zero → Hystersis falls through to interactive login
...
|--------|---------------|-------------|
| **stdout** | The token — nothing else | Hystersis (parsed and stored in `auth.json`) |
| **stderr** | Login URLs, status messages, errors, progress | The user (displayed in their terminal) |

**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Hystersis reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Hystersis to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Hystersis when to proactively refresh. Without it, Hystersis assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Hystersis captures this)
...
```bash
# Or via environment variables
export HYSTERSIS_AUTH_PROVIDER_COMMAND="/usr/local/bin/my-auth-provider"
export HYSTERSIS_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export HYSTERSIS_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Hystersis cannot detect expiry proactively and will only refresh after a 401.
...
> **Windows:** write the path as a TOML *literal* string (single quotes) so backslashes survive: `auth_provider_command = 'C:\corp\hystersis-auth.exe'`. Inside a double-quoted TOML string `\t`, `\n`, `\r`, `\b` and `\f` are escape sequences, so `"C:\temp\auth.exe"` parses into a path containing a tab character and the provider fails to start — after which Hystersis falls back to browser login as if the setting were ignored.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with hystersis.com". In headless mode (`hystersis -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
Hystersis runs your binary on two different contracts, and `HYSTERSIS_AUTH_EXPIRED` is how it tells them apart:

| | `HYSTERSIS_AUTH_EXPIRED=1` | unset |
|---|---|---|
| **What it is** | A headless refresh over a credential Hystersis already holds — near-expiry rotation, or a token the server rejected | A sign-in: `hystersis login`, the sign-in screen, or the escalation after a headless run couldn't mint |
...
| **Budget** | A few seconds (7s), then Hystersis kills the process | 300s — enough for a browser round trip or a device code |
| **So your binary should** | Mint silently, or **exit non-zero**. Never block | Do the full SSO flow, and always mint fresh |

```bash
#!/bin/sh
if [ "$HYSTERSIS_AUTH_EXPIRED" = "1" ]; then
    # Headless: silent refresh only. If that can't work — the SSO session
    # lapsed, say — exit non-zero rather than block. Hystersis then shows the
...
Exiting promptly on `HYSTERSIS_AUTH_EXPIRED=1` is what makes the handover to the sign-in screen fast: a binary that blocks instead pays the whole refresh timeout on every start with an expired token.

One case stays ambiguous, and only in **leader mode** (`--leader`, or `[cli] use_leader = true`; off by default): with no credential at all, the leader makes one extra attempt in the background just after startup, and that run has the variable unset, like a sign-in. A binary that mints without help (service account, keytab, mounted token) succeeds there and the session heals itself. One that must prompt just sits, up to the 300s sign-in ceiling — nothing waits on it, the sign-in screen is already up, and its stderr goes to `~/.hystersis/leader.log` rather than to the user.

`HYSTERSIS_AUTH_EXPIRED` is optional — if your binary ignores it, Hystersis still works. It just runs the same flow for both login and refresh, and a flow that prompts will be killed on the headless run before it can finish.

### Automatic Credential Refresh

Hystersis supports automatic credential refresh for external auth providers and OIDC. When Hystersis detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Hystersis handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Hystersis re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Hystersis re-runs the binary and retries the request once.
- **When the refresh run can't mint:** refreshes are headless (no stdin, short timeout), so a binary that needs you to complete an SSO flow cannot succeed there. Hystersis then stops treating the stored credential as usable and runs your binary in its interactive mode instead — at startup that is the same sign-in flow a machine with no credentials gets; mid-session the turn fails with a re-auth prompt and `/login` re-runs the binary.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Hystersis silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Hystersis refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `hystersis login` before starting — Hystersis runs your binary automatically on first launch. You _can_ run `hystersis login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Hystersis tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
hystersis --debug-file /tmp/hystersis-auth.log -p "hello"
tail -f /tmp/hystersis-auth.log
```
...
|-------------|---------------|
| `auth: running external auth provider (headless refresh)` | Your binary is being called with `HYSTERSIS_AUTH_EXPIRED=1` and a few seconds to work |
| `auth: running external auth provider (interactive login)` | Your binary is being called on the sign-in contract: no `HYSTERSIS_AUTH_EXPIRED`, stderr shown, 300s |
...
| `auth: external auth provider timed out (likely needs interactive auth), killing` | Binary didn't exit before the 7s headless-refresh timeout and was killed. Exiting non-zero on `HYSTERSIS_AUTH_EXPIRED=1` avoids this wait entirely |
| `auth: failed to start external auth provider` | The command couldn't be spawned (e.g. binary not found) |

### Per-Model Auth Providers

`auth_provider_command` above replaces Hystersis's *session* auth: it mints the token sent to xAI's backend. If you instead want xAI models on normal xAI login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- `HYSTERSIS_AUTH_EXPIRED=1` is set whenever Hystersis re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `HYSTERSIS_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `HYSTERSIS_AUTH_PROVIDER_REFRESH_TOKEN` / `HYSTERSIS_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and Hystersis re-runs the command on restart to re-mint.
- Hystersis runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.hystersis/config.toml`, managed config, requirements). A project's `.hystersis/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits Hystersis's environment (so it sees `PATH`, `HOME`, and any other secrets there), but Hystersis's own first-party credentials (`XAI_API_KEY`, `HYSTERSIS_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `HYSTERSIS_AUTH_PROVIDER_*` handback for the prior credential.

### Using auth.json for API Access

If you've authenticated with `hystersis login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the hystersis CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.hystersis.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.hystersis.com/sign-in".key' ~/.hystersis/auth.json)" \
  -H "X-XAI-Token-Auth: xai-grok-cli" \
  -H "x-hystersis-model-override: hystersis" \
  -d '{
    "model": "hystersis",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.hystersis/auth.json` (set by `hystersis login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-grok-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-hystersis-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `hystersis` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `hystersis`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `hystersis login` to refresh.
...
hystersis --cwd ~/projects/my-app

# Start with an initial task
hystersis --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
hystersis --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
hystersis --always-approve --prompt "Format all files"
```
...
Run Hystersis non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `hystersis`)               |
...
hystersis -p "Explain this codebase" --tools "read_file,grep,list_dir"

# Remove web access and file editing
hystersis -p "Review this code" --disallowed-tools "web_search,web_fetch,search_replace"

# Remove shell access
hystersis -p "Review this code" --disallowed-tools "run_terminal_cmd"
```
...
hystersis -p "Fix this bug" --disallowed-tools "Agent"

# Block only the explore subagent
hystersis -p "Refactor this module" --disallowed-tools "Agent(explore)"
```
...
hystersis -p "Clean up this project" --deny "Bash(rm*)"

# Allow npm commands, deny everything else dangerous
hystersis -p "Set up the project" --allow "Bash(npm*)" --deny "Bash(sudo*)"

# Deny edits outside src/
hystersis -p "Refactor the code" --deny "Edit(/etc/**)"

# Allow all bash commands (auto-approve without prompting)
hystersis -p "Build the project" --allow "Bash"

# Combine: allow fetching docs sites, deny other URLs
hystersis --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
hystersis -p "What does this project do?"

# Use a specific model
hystersis -p "Optimize this function" -m hystersis

# Get JSON output for parsing
hystersis -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
hystersis -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
hystersis -p "Remember: the secret number is 42" --output-format json
hystersis -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
hystersis -p "Continue where we left off" -c

# Run in a different directory
hystersis -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
hystersis -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
hystersis -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
hystersis -p "Generate a README" > README.md

# Parse JSON output with jq
hystersis -p "List files" --output-format json | jq -r '.text'

# CI/CD: automated code review
hystersis -p "Review changes for bugs and security issues." \
  --output-format json --always-approve | jq -r '.text' > review.md

# Pipeline: chain with other tools
git diff --staged | hystersis -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  hystersis -p "Migrate $file from CommonJS to ES modules." --always-approve
done

# Pre-commit hook
hystersis -p "Review staged changes for obvious bugs. Reply OK if fine, or list issues." \
...
Run Hystersis as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `hystersis`)                           |
...
```bash
hystersis agent headless --hystersis-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`hystersis ssh`)

Use `hystersis ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
hystersis ssh user@host

# With SSH flags
hystersis ssh -t user@host
hystersis ssh -L 8080:localhost:8080 user@host

# With remote command
hystersis ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `hystersis ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.

This runs entirely locally.

---

## Building with Hystersis

Hystersis can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `hystersis -p` and parses JSON output.
...
class HystersisChat:
    ...
    def _build_cmd(self, prompt, model, stream):
        return ["hystersis", "-p", prompt, "-m", model, "--cwd", self.cwd,
                "--output-format", "streaming-json" if stream else "json", "--always-approve"]

    async def create(self, messages, model="hystersis", stream=False):
...
class HystersisChat {
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("hystersis", this.buildArgs(prompt, model, true));
...
// Usage
const client = new HystersisChat(".");
...
# Usage
async def main():
    client = await HystersisACPChat(cwd=".").init()
...
class HystersisACPChat {
  ...
  async init() {
    this.proc = spawn("hystersis", ["agent", "stdio"]);
...
// Usage
const client = await new HystersisACPChat(".").init();
...
Hystersis implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Hystersis reads configuration from `~/.hystersis/config.toml`. If the file doesn't exist, Hystersis uses sensible defaults. You only need to specify values you want to override.
...
[models]
default = "hystersis-4.6"                   # model used for new sessions
web_search = "hystersis-4.6"                # model used by the web_search tool
...
telemetry = false                      # anonymous usage telemetry (env: HYSTERSIS_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: HYSTERSIS_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: HYSTERSIS_RESPECT_GITIGNORE)
...
allowed_domains = ["docs.rs", "hystersis.com"]           # override the built-in ~84-domain allowlist
```
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: HYSTERSIS_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: HYSTERSIS_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: HYSTERSIS_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: HYSTERSIS_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: HYSTERSIS_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `HYSTERSIS_TELEMETRY_BUILD_EVENTS_URL`, `HYSTERSIS_TELEMETRY_BUILD_EVENTS_API_KEY`, and `HYSTERSIS_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Hystersis can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Hystersis language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Hystersis can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Hystersis can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Hystersis looks for server definitions in:

- project config: `<repo>/.hystersis/lsp.json`
- user config: `~/.hystersis/lsp.json`
...
- LSP tools are enabled (`HYSTERSIS_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
HYSTERSIS_LSP_TOOLS=1 hystersis
```
...
If LSP tools are enabled but no usable server config is found, Hystersis emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Hystersis does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `HYSTERSIS_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[models]
default = "company-hystersis"

[model.company-hystersis]
model = "hystersis"
base_url = "https://hystersis-proxy.acme.com/"
name = "Hystersis Latest (Proxy)"
...
With this config, `hystersis` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Hystersis reads these files and appends their contents to the system prompt.

Hystersis scans for agent rules in this order:

1. `~/.hystersis/` (global rules)
...
Within each directory, Hystersis checks for these filenames:
...
Skills are reusable prompt packages that extend Hystersis with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Hystersis discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.hystersis/skills/`           | Local | Highest  |
| `<repo_root>/.hystersis/skills/` | Repo  | Medium   |
| `~/.hystersis/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.hystersis/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Hystersis decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Hystersis invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Hystersis automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Hystersis discovers agent definitions from `.hystersis/agents/` (project), `~/.hystersis/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `HYSTERSIS_AGENT` env var
4. Default `hystersis` agent
...
```bash
hystersis --agent-profile ./my-agent.md
# or
export HYSTERSIS_AGENT="my-custom-agent"
```
...
```bash
export HYSTERSIS_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "hystersis"              # route explore to a lighter model
```
...
model = "hystersis"
prompt_file = ".hystersis/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".hystersis/personas/concise.md"  # or load from file
```

Both are also discovered from `.hystersis/roles/*.toml` and `.hystersis/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Hystersis with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.hystersis/plugins/`            | Project |
| `~/.hystersis/plugins/`          | User    |
...
Hystersis discovers hooks from `.hystersis/hooks/` in the project directory. Manage them with:
...
The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Hystersis falls back to built-in defaults for known models.
...
[model.hystersis-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, Hystersis starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `HYSTERSIS_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `hystersis models` output). You also need a `[model.*]` entry so Hystersis knows
...
>
> [model.my-custom-model]              # 2. tell Hystersis how to reach it
...
Point Hystersis at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `HYSTERSIS_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `HYSTERSIS_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export HYSTERSIS_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
hystersis
```

Hystersis fetches the model list from `{HYSTERSIS_MODELS_BASE_URL}/models` on startup and sends inference requests to `HYSTERSIS_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `HYSTERSIS_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.hystersis/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Hystersis uses API key auth (`Authorization: Bearer`) instead of session auth. `hystersis login` is not required — only the API key.

---

## MCP Servers

Extend Hystersis's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.hystersis/config.toml`:
...
MCP servers can also be configured per-project in `.hystersis/config.toml`. Hystersis walks from the current directory up to the git repo root, loading `.hystersis/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.hystersis/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.hystersis/config.toml` | This repository   | ↑        |
| `<cwd>/.hystersis/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.hystersis/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.hystersis/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.hystersis/config.toml`. Other config sections (models, etc.) are only read from `~/.hystersis/config.toml`.
...
> **Experimental:** enable with `HYSTERSIS_MEMORY=1`, `[memory] enabled = true`, or managed remote settings.

Cross-session memory lets Hystersis remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.hystersis/memory/`:
- **Global** (`~/.hystersis/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.hystersis/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.hystersis/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
At the end of each session, Hystersis saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.hystersis/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Hystersis searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
hystersis memory edit

# Open global MEMORY.md
hystersis memory edit --global

# Show memory statistics: file count, chunk count, and index size
hystersis memory stats
```

### Configuration reference

Key options under `[memory]` in `~/.hystersis/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.hystersis/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Hystersis emits the `hystersis-shell-memory_injection`
...
Hystersis can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.hystersis/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.hystersis/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.hystersis/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.hystersis/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.hystersis/sandbox.toml` (global) or `.hystersis/sandbox.toml` (per-project):
...
```bash
hystersis --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire hystersis process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Hystersis logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.hystersis/sandbox-events.jsonl`
...
Use `hystersis inspect` to see everything Hystersis discovers in the current directory:

```bash
hystersis inspect          # human-readable output
hystersis inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.hystersis/skills/`, `~/.hystersis/skills/`, plugins, and config paths
...
Hystersis automatically discovers configuration from Claude Code directories alongside native `.hystersis/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Hystersis uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.hystersis/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Hystersis at runtime.

---

## Built-in Tools

Hystersis includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `HYSTERSIS_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (SpaceXAI, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `hystersis.com/company`).

---

## Session Persistence

Hystersis automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.hystersis/sessions/`, organized by URL-encoded working directory:

```
~/.hystersis/sessions/<encoded-cwd>/<session-id>/
...
hystersis -p "Remember: X=42" -s my-session
hystersis -p "What is X?" -s my-session

# Resume existing session (errors if not found)
hystersis -p "Continue" -r my-session

# Continue most recent session in current directory
hystersis -p "What were we doing?" -c
```

Session ID is returned in JSON output:

```bash
hystersis -p "Hello" --output-format json | jq -r '.sessionId'
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.hystersis/config.toml` | Configuration file                                  |
| `~/.hystersis/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.hystersis/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.hystersis/memory/`     | Cross-session memory files and index                |
| `~/.hystersis/skills/`     | User-scoped skill definitions                       |
| `~/.hystersis/plugins/`    | User-scoped plugins                                 |
| `~/.hystersis/agents/`     | User-scoped agent definitions                       |
| `.hystersis/config.toml`   | Project-scoped config (MCP servers)                 |
| `.hystersis/skills/`       | Project-scoped skill definitions                    |
| `.hystersis/plugins/`      | Project-scoped plugins                              |
| `.hystersis/agents/`       | Project-scoped agent definitions                    |
| `.hystersis/hooks/`        | Project-scoped hooks                                |
| `.hystersis/lsp.json`      | LSP server configuration                            |
...
| -------------------------------- | -------------------------------------------------------------------------------------------------------- |
| `XAI_API_KEY`         | API key from [hystersis.com](https://hystersis.com). Used for custom endpoint auth and API key login      |
| `HYSTERSIS_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.hystersis.com/v1`)                          |
| `HYSTERSIS_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `HYSTERSIS_MODELS_LIST_URL`          | Override the model list URL if it differs from `{HYSTERSIS_MODELS_BASE_URL}/models`                                              |
| `HYSTERSIS_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `HYSTERSIS_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `HYSTERSIS_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `HYSTERSIS_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `HYSTERSIS_HOME`                     | Override config directory (default: `~/.hystersis`)                                                           |
| `HYSTERSIS_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `HYSTERSIS_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `HYSTERSIS_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `HYSTERSIS_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `HYSTERSIS_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `HYSTERSIS_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `HYSTERSIS_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `HYSTERSIS_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `HYSTERSIS_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `HYSTERSIS_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.hystersis/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `HYSTERSIS_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `hystersis` commands and flags.
...
hystersis completions bash > ~/.local/share/bash-completion/completions/hystersis
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Hystersis-managed location):

```bash
mkdir -p ~/.hystersis/completions/bash
hystersis completions bash > ~/.hystersis/completions/bash/hystersis.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.hystersis/completions/bash/hystersis.bash" ]] && source "$HOME/.hystersis/completions/bash/hystersis.bash"
```
...
hystersis completions zsh > ~/.zsh/completions/_hystersis
```
...
```bash
mkdir -p ~/.hystersis/completions/zsh
hystersis completions zsh > ~/.hystersis/completions/zsh/_hystersis
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.hystersis/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `hystersis` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `hystersis --debug` or `HYSTERSIS_LOG_FILE` instead:

```bash
# Per-session debug log (~/.hystersis/debug/<sessionId>.txt)
hystersis --debug

# Log to a custom path
HYSTERSIS_LOG_FILE=/tmp/hystersis-debug.log hystersis

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.hystersis/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `HYSTERSIS_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
HYSTERSIS_LOG_FILE=/tmp/hystersis-debug.log RUST_LOG="info,xai_hystersis_shell::auth=debug" hystersis
```
...
hystersis --debug-file /tmp/hystersis-auth.log -p "hello"
grep "auth:" /tmp/hystersis-auth.log
```
...
ls ~/.hystersis/sessions/

# Read session metadata
cat ~/.hystersis/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.hystersis/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.hystersis/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
