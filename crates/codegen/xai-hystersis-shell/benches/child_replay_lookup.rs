...
//! order as a fat local `~/.hystersis/sessions`). Override with env:
//!
//! ```text
//! cargo bench -p xai-hystersis-shell --bench child_replay_lookup
//! CHILD_REPLAY_LOOKUP_CWDS=3000 CHILD_REPLAY_LOOKUP_PER_CWD=3 cargo bench ...
//! # read-only against a real store (no writes):
//! CHILD_REPLAY_LOOKUP_HOME=$HOME/.hystersis cargo bench -p xai-hystersis-shell --bench child_replay_lookup
//! ```
...
use xai_hystersis_config::encode_cwd_dirname;
