...
//! live in `hystersis_tools::implementations::skills::discovery`.

use std::collections::{HashMap, HashSet};
use std::path::{Path, PathBuf};

use crate::plugins::discovery::PluginScope;
use hystersis_tools::implementations::skills::types::skill_name_from_path;
pub use hystersis_tools::implementations::skills::types::{SkillInfo, SkillScope};
/// Re-export so agent-side discovery (and the shell) can name the resolved
/// vendor-compat config without reaching into `hystersis_tools` directly.
pub use hystersis_tools::types::compat::CompatConfig;

use hystersis_tools::implementations::skills::discovery::{
...
pub async fn list_skills_with_plugins(
    ...
    let mut skills = list_skills_with_options(
        ...
        &hystersis_tools::util::hystersis_home::hystersis_home(),
        compat,
    )
    .await;
...
fn collect_config_skills(config_paths: &[String], git_root: Option<&Path>) -> Vec<SkillInfo> {
    ...
    for skill in &mut skills {
        skill.config_source = Some(
            hystersis_tools::types::config_source::ConfigSource::ConfigToml {
...
pub(crate) fn format_skill_for_injection(skill: &SkillInfo) -> Option<String> {
    skill.body.as_ref().filter(|b| !b.is_empty()).map(|body| {
        hystersis_tools::implementations::skills::skill::build_skill_message(skill, body)
    })
}
    ...
    for name in names {
        ...
        let skill = discovered.iter().find(|s| {
            s.name.eq_ignore_ascii_case(name)
                || hystersis_tools::implementations::skills::skill::format_skill_name(s)
        ...
        match hystersis_tools::implementations::skills::skill::load_skill_with_body(skill).await {
...
mod tests {
    ...
    fn collect_config_skills_stamps_config_toml_source() {
        ...
        match &skills[0].config_source {
            Some(hystersis_tools::types::config_source::ConfigSource::ConfigToml { path }) => {
    ...
    async fn list_skills_auto_and_config_overlap_keeps_config_toml_source() {
        ...
        assert!(
            matches!(
                ...
                Some(hystersis_tools::types::config_source::ConfigSource::ConfigToml { .. })
            ),
