...
use std::sync::Arc;
use hystersis_tools::bridge::ToolBridge;
use hystersis_tools::computer::types::{AsyncFileSystem, TerminalBackend};
use hystersis_tools::notification::ToolNotificationHandle;
use hystersis_tools::registry::types::SessionContext;
use hystersis_tools::types::tool::ToolKind;
/// The Hystersis [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-Hystersis
/// tool registry in `xai-hystersis-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    hystersis_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.hystersis/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<hystersis_tools::implementations::hystersis_build::scheduler::types::SchedulerHandle>,
    ...
    memory_backend: Option<Arc<dyn hystersis_tools::types::memory_backend::MemoryBackend>>,
    web_search_config: hystersis_tools::implementations::web_search::WebSearchConfig,
    ...
    backend_search: bool,
    web_fetch_config: hystersis_tools::implementations::hystersis_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn hystersis_tools::implementations::lsp::LspBackend>>,
    image_gen_config: hystersis_tools::implementations::hystersis_build::image_gen::ImageGenConfig,
    video_gen_config: hystersis_tools::implementations::hystersis_build::video_gen::VideoGenConfig,
    ...
    compat: hystersis_tools::types::compat::CompatConfig,
    ...
    api_key_provider: Option<hystersis_tools::types::SharedApiKeyProvider>,
    attribution_callback: Option<hystersis_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in xai-hystersis-shell).
    ...
    preloaded_skills: Option<Vec<hystersis_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut hystersis_tools::registry::types::ToolServerConfig) {
    use hystersis_tools::implementations::hystersis_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("HystersisBuild:enter_plan_mode");
    let missing_exit = !existing.contains("HystersisBuild:exit_plan_mode");
    let missing_ask = !existing.contains("HystersisBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&hystersis_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&hystersis_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&hystersis_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut hystersis_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(hystersis_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<hystersis_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn hystersis_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: hystersis_tools::implementations::hystersis_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: hystersis_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `HYSTERSIS_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: hystersis_tools::implementations::hystersis_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn hystersis_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: hystersis_tools::implementations::hystersis_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: hystersis_tools::implementations::hystersis_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: hystersis_tools::implementations::hystersis_build::app_builder::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: hystersis_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the HystersisBuild Task description.
    ...
    /// `HystersisBuild:ask_user_question` is stripped from the agent's tool config
    ...
    /// Without this, only auto-discovered skills (cwd/.hystersis/skills, ~/.hystersis/skills)
    /// are included — custom paths added via `hystersis.com/skills/add` would be ignored.
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use hystersis_tools::implementations::hystersis_build;
                tool_config.tools.push((&hystersis_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use hystersis_tools::implementations::hystersis_build;
                tool_config.tools.push((&hystersis_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&hystersis_tools::implementations::hystersis_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&hystersis_tools::implementations::hystersis_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&hystersis_tools::implementations::hystersis_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&hystersis_tools::implementations::hystersis_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&hystersis_tools::implementations::hystersis_build::ReferenceToVideoTool).into(),
                );
            }
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&hystersis_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let hystersis_build_ns = hystersis_tools::types::tool::ToolNamespace::HystersisBuild.to_string();
            let mem_search_id = format!(
                "{hystersis_build_ns}:{}",
                hystersis_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{hystersis_build_ns}:{}",
                hystersis_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if self.prompt_audience == crate::prompt::context::PromptAudience::Subagent {
            tool_config
                .tools
                .retain(|tool| tool.kind != Some(hystersis_tools::types::tool::ToolKind::AskUser));
        } else if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                hystersis_tools::types::tool::ToolNamespace::HystersisBuild,
            );
        ...
        if task_stripped {
            use hystersis_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::HystersisBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::HystersisBuildConcise, "run_terminal_cmd", true)
        ...
        if let hystersis_tools::implementations::hystersis_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["HystersisBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "HystersisBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        if let Some(ref ask_params) = self.ask_user_question_params_json {
            merge_tool_params(
                &mut tool_config,
                &["HystersisBuild:ask_user_question"],
                ask_params,
            );
        }
        if self.is_non_interactive {
            ...
            merge_tool_params(&mut tool_config, &["HystersisBuild:ask_user_question"], &ni);
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    "tools allowlist had unmappable entries; keeping full hystersis toolset"
                );
            }
        }
        tool_config
        ...
        if self.prompt_audience == crate::prompt::context::PromptAudience::Subagent {
            tool_config.tools.retain(|tool| {
                !hystersis_tools::implementations::hystersis_build::is_workflow_tool(tool.kind, &tool.id)
            });
        }
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                hystersis_tools::types::resources::TruncationCfg(
                    hystersis_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(hystersis_sampling_types::HostedTool::WebSearch { options: None });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(hystersis_sampling_types::HostedTool::XSearch { options: None });
            }
            hystersis_sampling_types::apply_tool_overrides(
                &mut hosted_tools,
...
/// CLI naming for the shared [`hystersis_tool_types::build_task_description`] builder.
const TASK_TOOL_NAMING: hystersis_tool_types::TaskToolNaming<'static> = hystersis_tool_types::TaskToolNaming {
...
/// CLI [`hystersis_tool_types::SubagentToolNaming`]: each kind maps to its
...
const SUBAGENT_TOOL_NAMING: hystersis_tool_types::SubagentToolNaming<'static> =
    hystersis_tool_types::SubagentToolNaming {
...
/// shared [`hystersis_tool_types`] catalog and rendered with [`SUBAGENT_TOOL_NAMING`]
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        BuiltinAgentName::GeneralPurpose => hystersis_tool_types::GENERAL_PURPOSE_SUBAGENT,
        BuiltinAgentName::Explore => hystersis_tool_types::EXPLORE_SUBAGENT,
        BuiltinAgentName::Plan => hystersis_tool_types::PLAN_SUBAGENT,
        _ => return String::new(),
    };
...
/// [`hystersis_tool_types::SubagentDescriptor`] and defers to
/// [`hystersis_tool_types::build_task_description`] so the CLI and the prod chat
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<hystersis_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            hystersis_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = hystersis_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
fn resolve_shell_for_prompt() -> String {
    ...
    #[cfg(not(unix))]
    {
        hystersis_config::shell::detect_windows_shell()
            .name()
            .to_string()
    }
}
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: hystersis_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    fn build_task_description_builtin_includes_tools() {
        ...
        assert!(
            desc.contains(hystersis_tool_types::GENERAL_PURPOSE_SUBAGENT.tools_template),
            "should include general-purpose tool names"
        );
        assert!(
            desc.contains(hystersis_tool_types::EXPLORE_SUBAGENT.tools_template),
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: hystersis_tools::types::config_source::ConfigSource::Project {
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use hystersis_tools::types::template_renderer::TemplateRenderer;
        use hystersis_tools::types::tool::ToolKind;
    ...
    async fn discovery_snapshot_records_gated_and_preloaded_skills() {
        use hystersis_tools::computer::local::LocalTerminalBackend;
        use hystersis_tools::notification::ToolNotificationHandle;
        let tmp = tempfile::tempdir().unwrap();
        let write_skill = |dir: &str, content: &str| {
            let d = tmp.path().join(".hystersis/skills").join(dir);
        ...
        let mut definition = crate::config::AgentDefinition::default_hystersis_build();
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "hystersis / subagents+ask_user",
                profile: AgentDefinition::default_hystersis_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "hystersis / subagents / no-ask-user",
                profile: AgentDefinition::default_hystersis_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "hystersis / no-subagents / ask_user",
                profile: AgentDefinition::default_hystersis_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "hystersis / no-subagents / no-ask-user",
                profile: AgentDefinition::default_hystersis_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "hystersis-ask-user / subagents",
                profile: AgentDefinition::hystersis_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "hystersis-ask-user / no-subagents",
                profile: AgentDefinition::hystersis_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "hystersis-plan",
                profile: AgentDefinition::hystersis_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "hystersis-plan / no-ask-user",
                profile: AgentDefinition::hystersis_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "hystersis-plan-no-subagents",
                profile: AgentDefinition::hystersis_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "hystersis-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::hystersis_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn subagent_audience_never_receives_ask_user_question() {
        ...
        .from_definition(crate::config::AgentDefinition::hystersis_build_ask_user())
    ...
    async fn top_level_session_still_receives_workflow() {
        let names = workflow_tool_names(
            ...
            crate::config::AgentDefinition::default_hystersis_build(),
        )
        .await;
        assert!(
    ...
    async fn custom_child_toolset_cannot_reintroduce_workflow() {
        use hystersis_tools::implementations::hystersis_build::{ReadFileTool, WorkflowTool};
        ...
        definition.tool_config.tools = vec![
            (&ReadFileTool).into(),
            (&WorkflowTool).into(),
            hystersis_tools::registry::types::ToolConfig::from_id("HystersisBuild:workflow"),
        ];
        let names =
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use hystersis_tools::computer::local::LocalTerminalBackend;
        use hystersis_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_hystersis_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use hystersis_tools::computer::local::LocalTerminalBackend;
        use hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionParams;
        use hystersis_tools::notification::ToolNotificationHandle;
        use hystersis_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_hystersis_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "HystersisBuild:ask_user_question"),
    ...
    async fn non_interactive_build_stamps_ask_user_question_params() {
        ...
        .from_definition(crate::config::AgentDefinition::default_hystersis_build())
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use hystersis_tools::computer::local::LocalTerminalBackend;
        use hystersis_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_hystersis_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("HystersisBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use hystersis_tools::computer::local::LocalTerminalBackend;
        use hystersis_tools::implementations::hystersis_build::bash::BashParams;
        use hystersis_tools::notification::ToolNotificationHandle;
        use hystersis_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_hystersis_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Hystersis
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_hystersis_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their hystersis
    ...
    /// treat `mcp__*` as always-on, so hystersis keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        ...
        use hystersis_tools::implementations::web_search::WebSearchConfig;
        use hystersis_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_hystersis_build();
        ...
        .with_web_search_config(WebSearchConfig::Enabled {
            ...
            base_url: "https://api.hystersis.com/v1".into(),
    ...
    /// hystersis toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to hystersis's `search_tool` (MCP
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<hystersis_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use hystersis_tools::computer::local::LocalTerminalBackend;
        use hystersis_tools::implementations::web_search::WebSearchConfig;
        use hystersis_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, hystersis_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, hystersis_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = hystersis_sampling_types::XSearchOptions {
            date_bound: Some(
                hystersis_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(hystersis_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&hystersis_sampling_types::HostedTool::XSearch {
