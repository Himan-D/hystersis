//! Agent definition types — parsed from `.hystersis/agents/*.md` files.
...
use hystersis_tools::implementations::codex;
use hystersis_tools::implementations::hystersis_build;
use hystersis_tools::implementations::hystersis_build_concise;
use hystersis_tools::implementations::memory;
use hystersis_tools::implementations::opencode;
use hystersis_tools::implementations::search_tool;
use hystersis_tools::implementations::use_tool;
use hystersis_tools::registry::types::{ToolConfig, ToolServerConfig};
...
/// Orchestrator-specific prompt body appended to the standard HystersisBuild
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&hystersis_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&hystersis_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&hystersis_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` → `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&hystersis_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` → `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&hystersis_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_hystersis_build_toolset()` with tools that are dynamically
...
/// zero local dispatch.
pub fn workspace_hystersis_build_toolset() -> ToolServerConfig {
    let mut tools = default_hystersis_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&hystersis_build::EnterPlanModeTool).into());
    tools.push((&hystersis_build::ExitPlanModeTool).into());
    tools.push((&hystersis_build::AskUserQuestionTool).into());
    tools.push((&hystersis_build::WebSearchTool).into());
    tools.push((&hystersis_build::ImageGenTool).into());
    tools.push((&hystersis_build::ImageToVideoTool).into());
    tools.push((&hystersis_build::ReferenceToVideoTool).into());
    tools.push((&hystersis_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&hystersis_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `hystersis-computer` (workspace/sandbox) preset.
fn hystersis_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&hystersis_build::ReadFileTool).into(),
        (&hystersis_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&hystersis_build::ListDirTool).into(),
        (&hystersis_build::GrepTool).into(),
        (&hystersis_build::KillTerminalCommandTool).into(),
        (&hystersis_build::GetTerminalCommandOutputTool).into(),
        (&hystersis_build::SchedulerCreateTool).into(),
        (&hystersis_build::SchedulerDeleteTool).into(),
        (&hystersis_build::SchedulerListTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("hystersis", workspace_hystersis_build_toolset()),
        ("hystersis-concise", hystersis_build_concise_toolset()),
        ("hystersis-plan", hystersis_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("hystersis-computer", hystersis_computer_toolset()),
    ]
}
...
fn default_hystersis_build_toolset() -> ToolServerConfig {
    hystersis_build_core_toolset(true)
}
/// Same as the parent hystersis list, without `workflow`. The usual
/// `general-purpose` spawn path must not add that tool and then strip it.
fn general_purpose_toolset() -> ToolServerConfig {
    hystersis_build_core_toolset(false)
}
fn hystersis_build_core_toolset(include_workflow: bool) -> ToolServerConfig {
    let mut tools = vec![
        bash_tool_config(),
        (&hystersis_build::ReadFileTool).into(),
        (&hystersis_build::SearchReplaceTool).into(),
        ...
        (&hystersis_build::TodoWriteTool).into(),
        ...
        (&hystersis_build::SchedulerDeleteTool).into(),
        (&hystersis_build::SchedulerListTool).into(),
        (&hystersis_build::MonitorTool).into(),
        (&search_tool::SearchTool).into(),
        (&use_tool::UseTool).into(),
        (&hystersis_build::UpdateGoalTool).into(),
    ];
    if include_workflow {
        tools.push((&hystersis_build::WorkflowTool).into());
    }
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn hystersis_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&hystersis_build_concise::BashConciseTool).into(),
            (&hystersis_build_concise::ReadFileConciseTool).into(),
            (&hystersis_build_concise::SearchReplaceConciseTool).into(),
            (&hystersis_build::ListDirTool).into(),
            (&hystersis_build::GrepTool).into(),
            kill_task_tool_config(),
            (&hystersis_build::TodoWriteTool).into(),
            ...
            (&hystersis_build::MonitorTool).into(),
            (&hystersis_build::UpdateGoalTool).into(),
            (&hystersis_build::WorkflowTool).into(),
        ],
        behavior_preset: None,
    }
}
...
pub fn hystersis_build_hashline_toolset(
    hashline_tools: Vec<hystersis_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<hystersis_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&hystersis_build::WebSearchTool).into(),
        (&hystersis_build::SchedulerCreateTool).into(),
        (&hystersis_build::SchedulerDeleteTool).into(),
        (&hystersis_build::SchedulerListTool).into(),
        (&hystersis_build::MonitorTool).into(),
...
fn plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            (&hystersis_build::ListDirTool).into(),
            (&hystersis_build::GrepTool).into(),
            // (&hystersis_build::SkillTool).into(),
            (&hystersis_build::TodoWriteTool).into(),
...
/// Hystersis + plan mode toolset.
///
/// Extends the default `hystersis` toolset with plan mode tools:
...
fn hystersis_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard hystersis tools
            bash_tool_config(),
            (&hystersis_build::ReadFileTool).into(),
            (&hystersis_build::SearchReplaceTool).into(),
            (&hystersis_build::ListDirTool).into(),
            ...
            (&hystersis_build::WorkflowTool).into(),
            // Plan mode tools
            (&hystersis_build::EnterPlanModeTool).into(),
            (&hystersis_build::ExitPlanModeTool).into(),
            (&hystersis_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            // Web tools
            (&hystersis_build::WebSearchTool).into(),
            (&hystersis_build::WebFetchTool).into(),
            // Imagine
            (&hystersis_build::ImageGenTool).into(),
            (&hystersis_build::ImageToVideoTool).into(),
            (&hystersis_build::ReferenceToVideoTool).into(),
            // Memory
...
/// Hystersis + plan mode toolset WITHOUT subagent tools.
///
/// Same as `hystersis_build_plan_toolset` but excludes `TaskTool`,
/// `TaskOutputTool`, and `KillTaskTool`. Use this when the shell
/// does not have subagent infrastructure wired up.
fn hystersis_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard hystersis tools (minus TaskTool only — KillTaskTool and
...
/// Default Hystersis toolset + `ask_user_question`.
///
/// Same as `default_hystersis_build_toolset` with the `AskUserQuestionTool` added,
/// allowing the agent to ask structured questions without full plan mode.
fn hystersis_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&hystersis_build::ReadFileTool).into(),
...
/// - `model: hystersis-3-fast` → `Override("hystersis-3-fast")`
...
pub enum BuiltinAgentName {
    ...
    HystersisBuildPlan,
    HystersisBuildPlanNoSubagents,
    HystersisBuildAskUser,
    Codex,
    Opencode,
    GeneralPurpose,
    Explore,
    Plan,
    BrowserUse,
    #[strum(serialize = "hystersis-orchestrator")]
    HystersisBuildOrchestrator,
}
...
impl BuiltinAgentName {
    ...
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::HystersisBuild => AgentDefinition::default_hystersis_build(),
            Self::HystersisBuildConcise => AgentDefinition::hystersis_build_concise(),
            Self::HystersisBuildPlan => AgentDefinition::hystersis_build_plan(),
            Self::HystersisBuildPlanNoSubagents => AgentDefinition::hystersis_build_plan_no_subagents(),
            Self::HystersisBuildAskUser => AgentDefinition::hystersis_build_ask_user(),
            ...
            Self::HystersisBuildOrchestrator => AgentDefinition::hystersis_build_orchestrator(),
        }
    }
...
/// Portable agent identity — parsed from .hystersis/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_hystersis_build_toolset")]
    ...
    pub capability_mode: Option<hystersis_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<hystersis_sampling_types::ToolOverrides>,
...
pub enum AgentScope {
    /// .hystersis/agents/ (project-level, highest priority)
    Project,
    /// ~/.hystersis/agents/ (user-level)
    User,
    /// ~/.hystersis/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_hystersis_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.hystersis/agent-memory/<name>/`
    User,
    /// `<project>/.hystersis/agent-memory/<name>/`
    Project,
    /// `<project>/.hystersis/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: hystersis_config::hystersis_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".hystersis/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".hystersis/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// these into `hystersis_tools::registry::types::ToolsetConfig.bash`
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        let hystersis = hystersis_config::user_hystersis_home();
        let home = dirs::home_dir();
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), hystersis.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".hystersis/agents/") || path_str.contains(".hystersis\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".hystersis/bundled/agents/")
            || path_str.contains(".hystersis\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    pub fn include_browser_verification(&self) -> bool {
        matches!(
            self.builtin_name,
            Some(BuiltinAgentName::HystersisBuildPlan | BuiltinAgentName::HystersisBuildPlanNoSubagents)
        )
    }
    ...
    /// (`!inject_default_tools`). Stock `hystersis*` agents leave all
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<hystersis_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["HystersisBuild:read_file", "HystersisBuildHashline:hashline_read"],
            [
                "HystersisBuild:search_replace",
                "HystersisBuildHashline:hashline_edit",
            ],
            ["HystersisBuild:grep", "HystersisBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_hystersis_build_toolset(),
    ...
    pub fn default_hystersis_build() -> Self {
        Self::base(
            BuiltinAgentName::HystersisBuild,
            "Hystersis agent for software engineering tasks.",
        )
    }
    /// Hystersis Concise agent definition — concise output format for SFT/RL.
    pub fn hystersis_build_concise() -> Self {
        Self {
            tool_config: hystersis_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::HystersisBuildConcise,
                "Hystersis agent with concise output format.",
            )
        }
    }
    /// Hystersis agent with plan mode tools.
    pub fn hystersis_build_plan() -> Self {
        Self {
            tool_config: hystersis_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::HystersisBuildPlan,
                "Hystersis agent with plan mode support.",
            )
        }
    }
    /// Hystersis + plan mode WITHOUT subagent tools.
    pub fn hystersis_build_plan_no_subagents() -> Self {
        Self {
            tool_config: hystersis_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::HystersisBuildPlanNoSubagents,
                "Hystersis agent with plan mode (no subagents).",
            )
        }
    }
    /// Default Hystersis agent with the `ask_user_question` tool.
    pub fn hystersis_build_ask_user() -> Self {
        Self {
            tool_config: hystersis_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::HystersisBuildAskUser,
                "Hystersis agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: hystersis_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: hystersis_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: hystersis_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// Hystersis Orchestrator — GBL model with full HystersisBuild tools
    ...
    pub fn hystersis_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::HystersisBuildOrchestrator,
                "HystersisBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_hystersis_build_toolset();
        }
...
mod tests {
    ...
    fn task_tool_rename_matches_task_tool_id_predicate() {
        ...
        assert!(hystersis_tools::is_task_tool_id(&name));
    }
    ...
    fn bash_tool_rename_matches_writing_tool_kind() {
        ...
        assert_eq!(
            hystersis_tools::tool_taxonomy::writing_tool_kind(&name),
            Some(hystersis_tools::types::tool::ToolKind::Execute)
        );
    }
    /// Native presets only.
    #[test]
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("hystersis").unwrap();
    ...
    fn hystersis_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&hystersis_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&hystersis_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn hystersis_computer_preset_is_curated_hystersis_build_subset() {
        let gc = toolset_for_preset("hystersis-computer").unwrap();
        let gb = toolset_for_preset("hystersis").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = hystersis_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "hystersis-computer tool `{}` must also ship in the hystersis preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "hystersis-computer should be a curated subset of hystersis"
        );
    }
    #[test]
    fn hystersis_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("hystersis-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&hystersis_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&hystersis_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&hystersis_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&hystersis_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&hystersis_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&hystersis_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&hystersis_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The hystersis-computer preset must ship a full-file write tool (legacy
    /// `write_file` parity) — the same OpenCode `write` tool the hystersis
    ...
    #[test]
    fn hystersis_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("hystersis-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "hystersis-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn hystersis_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("hystersis-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&hystersis_build::LspTool).id,
            ToolConfig::from(&hystersis_build::EnterPlanModeTool).id,
            ToolConfig::from(&hystersis_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "hystersis-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_hystersis_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_hystersis_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::HystersisBuildOrchestrator => true,
            BuiltinAgentName::HystersisBuild
            | BuiltinAgentName::HystersisBuildConcise
            | BuiltinAgentName::HystersisBuildPlan
            | BuiltinAgentName::HystersisBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "hystersis-orchestrator"] {
            assert!(
...
"#;
    ...
    fn test_model_override_display_shows_id() {
        ...
        assert_eq!(
            ModelOverride::Override("hystersis-3-fast".to_string()).to_string(),
            "hystersis-3-fast"
        );
    }
    #[test]
...
"#;
        ...
        assert_eq!(def.model, ModelOverride::Override("hystersis-3".to_string()));
    }
    #[test]
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.hystersis/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.hystersis/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_hystersis_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have hystersis_build tools"
        );
    }
    #[test]
    ...
    fn same_name_acp_definition_does_not_enable_browser_verification() {
        ...
        assert!(AgentDefinition::hystersis_build_plan().include_browser_verification());
        assert!(AgentDefinition::hystersis_build_plan_no_subagents().include_browser_verification());
    }
    #[test]
    fn test_from_json_has_default_toolset_with_task_tool() {
        ...
        let def = AgentDefinition::from_json(&json).unwrap();
        let task_tool_id = "HystersisBuild:task";
        assert!(
    ...
    fn test_model_override_serde_explicit_model_id() {
        let yaml = "\"hystersis-3-fast\"";
        let m: ModelOverride = serde_yaml::from_str(yaml).unwrap();
        assert_eq!(m, ModelOverride::Override("hystersis-3-fast".to_string()));
    }
    #[test]
    ...
    fn test_model_override_serialize_override() {
        let m = ModelOverride::Override("hystersis-3-fast".to_string());
        let s = serde_json::to_string(&m).unwrap();
        assert_eq!(s, "\"hystersis-3-fast\"");
    }
    #[test]
    fn test_model_override_in_frontmatter() {
        let content = "---\nname: test\ndescription: Test\nmodel: hystersis-3-fast\n---\n";
        let def = AgentDefinition::parse(content).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("hystersis-3-fast".to_string())
        );
    }
    #[test]
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!({
            "name": "test",
            "description": "Test",
            "model": "hystersis-code-fast-1"
        });
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("hystersis-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("hystersis", BuiltinAgentName::HystersisBuild),
            ("hystersis-concise", BuiltinAgentName::HystersisBuildConcise),
            ("hystersis-ask-user", BuiltinAgentName::HystersisBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_hystersis_build();
