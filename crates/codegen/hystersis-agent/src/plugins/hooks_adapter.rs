...
use hystersis_hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    for spec in &mut specs {
        ...
        spec.layer = hystersis_hooks::config::HookProvenance::Plugin;
        spec.name = format!(
            "{}{}/{}",
            hystersis_hooks::config::PLUGIN_HOOK_PREFIX,
            plugin_name,
            spec.name
        );
        ...
        if let Some(cmd) = &spec.command {
            ...
            let expanded = hystersis_hooks::config::expand_env_skipping_runner_vars(&substituted);
            if expanded != cmd_str {
