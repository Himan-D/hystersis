...
use xai_hystersis_config_types::DisplayRefreshSettings;

use xai_hystersis_status_line::StatusLineConfig;
...
pub struct UiConfig {
    ...
    /// connect time (CLI `--hunk-tracker-mode` / `HYSTERSIS_HUNK_TRACKER` override
    ...
    /// Speech-to-text language preference for voice dictation. A Hystersis STT
...
pub struct ContextualHints {
    ...
    /// SSH wrap session-load tip (recommend `hystersis wrap ssh` when the session
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: xai_hystersis_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
