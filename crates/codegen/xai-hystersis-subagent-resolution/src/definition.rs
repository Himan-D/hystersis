...
use std::path::Path;
use xai_hystersis_agent::config::{AgentDefinition, IsolationMode};
use xai_hystersis_agent::plugins::PluginRegistry;
use xai_hystersis_agent::prompt::context::{PromptAudience, PromptContext};
...
pub fn discover_agent_definition(
    ...
    xai_hystersis_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        context.cwd,
        context.plugins,
    )
...
pub fn available_agent_names(context: &DefinitionResolutionContext<'_>) -> Vec<String> {
    let mut available: Vec<String> = xai_hystersis_agent::discovery::all_subagents_with_plugins(
        context.cwd,
        context.toggles,
        context.plugins,
    )
...
pub async fn render_subagent_initial_user_message(
    ...
    let agents_md_files = xai_hystersis_agent::prompt::agents_md::read_agents_config_with_paths(
...
mod tests {
    ...
    fn child_tool_policy_strips_workflow_and_keeps_other_tools() {
        ...
        definition
            .tool_config
            .tools
            .push((&xai_hystersis_tools::implementations::hystersis_build::WorkflowTool).into());
    ...
    fn kindless_workflow_id_is_stripped_by_child_policy() {
        ...
        assert!(definition.tool_config.tools.iter().all(|tool| {
            tool.kind != Some(ToolKind::Workflow)
                && tool.id.rsplit(':').next()
                    != Some(xai_hystersis_tools::implementations::hystersis_build::WORKFLOW_TOOL_NAME)
        }));
    }
    #[test]
