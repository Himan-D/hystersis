...
//!
//! Extracted from `xai-hystersis-shell/src/agent/subagent/` `normalize_forked_context()`.
...
/// `xai-hystersis-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use xai_hystersis_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(xai_hystersis_sampling_types::synthesized_reasoning_item(text))
    }
