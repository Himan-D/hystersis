...
//! `xai_hystersis_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `xai-hystersis-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(role.model.as_deref(), Some("hystersis-3"));
        assert_eq!(role.reasoning_effort.as_deref(), Some("high"));
        assert_eq!(
            role.prompt_file.as_deref(),
            Some(".hystersis/prompts/researcher.md")
        );
...
"#;
        ...
        assert_eq!(persona.model.as_deref(), Some("hystersis-3-fast"));
