//! Shared subprocess lifecycle ownership for hystersis test harnesses.
//!
...
pub struct TestProcessTree {
    pid: u32,
    label: String,
    group: Option<hystersis_tty_utils::ProcessGroup>,
...
impl TestProcessTree {
    ...
    pub fn try_attach(pid: u32, label: impl Into<String>) -> io::Result<Self> {
        ...
        let mut group = hystersis_tty_utils::ProcessGroup::new()?;
    ...
    pub fn attach(pid: u32, label: impl Into<String>) -> Self {
        let label = label.into();
        let (group, attachment_error) = match hystersis_tty_utils::ProcessGroup::new() {
    ...
    fn from_group(pid: u32, label: String, group: hystersis_tty_utils::ProcessGroup) -> Self {
        Self {
            pid,
            label,
            group: Some(group),
