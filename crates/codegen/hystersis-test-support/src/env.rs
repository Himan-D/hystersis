...
fn local_hystersis_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("xai-hystersis-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_hystersis_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = cmd
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build xai-hystersis-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build xai-hystersis-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
    ...
    assert!(
        binary.exists(),
        "xai-hystersis-pager build completed but binary missing at {}",
        binary.display()
    );
}

/// Resolve hystersis binary: `HYSTERSIS_BINARY` env (CI) or a locally built `xai-hystersis-pager` binary.
pub fn hystersis_binary() -> PathBuf {
    if let Ok(path) = std::env::var("HYSTERSIS_BINARY") {
        let p = PathBuf::from(path);
        assert!(p.exists(), "HYSTERSIS_BINARY does not exist: {}", p.display());
        // Bazel's HYSTERSIS_BINARY is runfiles-relative; the harness spawns the child
        // with a different cwd, so absolutize against the (runfiles-root) cwd now.
        return std::path::absolute(&p).unwrap_or(p);
    }

    if let Ok(path) = std::env::var("CARGO_BIN_EXE_xai-hystersis-pager") {
        let p = PathBuf::from(path);
        if p.exists() {
            return p;
        }
    ...
    let binary = local_hystersis_binary_path();
    ensure_local_hystersis_binary(&binary);
    binary
}
