//! Leader-mode (`hystersis agent --leader stdio`) test harness.
//!
...
use hystersis_acp_lib::LineBufferedRead;

use crate::env::hystersis_binary;
...
/// (version-skew) test. Falls back to [`hystersis_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "HYSTERSIS_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`hystersis_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "HYSTERSIS_BINARY_CLIENT";
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        match &*args.method {
            "hystersis.com/leader_reconnected" => {
                self.capture
                    .reconnected_count
                    .fetch_add(1, Ordering::SeqCst);
            }
...
/// A `hystersis agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderFixture {
    ...
    pub async fn start(
        ...
        Self::start_with_binary(&hystersis_binary(), server, cwd, sandbox).await
    }
    ...
    async fn start_with_binary_base_url_timeout(
        ...
        let socket = sandbox.hystersis_home().join("leader.sock");
        let lock = sandbox.hystersis_home().join("leader.lock");
        ...
        cmd.envs(hystersis_tty_utils::pager_env())
            .env("HYSTERSIS_CLI_CHAT_PROXY_BASE_URL", base_url)
            .env("HYSTERSIS_XAI_API_BASE_URL", base_url)
            .env("HYSTERSIS_MODELS_BASE_URL", base_url)
            .env("HYSTERSIS_FEEDBACK_BASE_URL", base_url)
            .env("HYSTERSIS_TRACE_UPLOAD_URL", base_url)
            .env("XAI_API_KEY", "test-key-for-ci")
            .env("HYSTERSIS_LEADER_SOCKET", &socket)
            .env("RUST_LOG", "hystersis_shell=debug");
        let log_path = sandbox.hystersis_home().join("leader.log");
        ...
        let pid = child.id();
        let tree = match TestProcessTree::try_attach(pid, "persistent hystersis test leader") {
            Ok(tree) => tree,
            Err(error) => {
                let _ = child.kill();
...
impl LeaderStdioClient {
    async fn spawn_with_binary_and_socket(
        ...
        let mut process = TestProcess::spawn(
            ...
            TestProcessConfig::new()
                .label("hystersis leader stdio client")
                ...
                .env("HYSTERSIS_TRACE_UPLOAD_URL", base_url)
                .env("XAI_API_KEY", "test-key-for-ci")
                .env("HYSTERSIS_LEADER_SOCKET", leader_socket)
                .env("RUST_LOG", "hystersis_shell=debug"),
        )
        .map_err(|error| {
            io::Error::new(
    ...
    /// Count of `hystersis.com/models/update` notifications received (catalog self-heal).
    pub fn models_update_count(&self) -> u32 {
        self.capture.models_update_count.load(Ordering::SeqCst)
    }

    /// Count of `hystersis.com/settings/update` notifications received (settings self-heal).
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".hystersis").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".hystersis").join("leader.log")).unwrap_or_default()
}
...
mod tests {
    ...
    fn fake_leader(script: &str) -> PersistentLeader {
        ...
        cmd.args(["-c", script])
            ...
            .envs(hystersis_tty_utils::pager_env());
        hystersis_tty_utils::detach_std_command(&mut cmd);
