...
//! Shared test utilities for hystersis crates: mock inference server, SSE
...
//! - [`HystersisStdioClient`] — ACP client that drives `hystersis agent stdio` as a subprocess
//! - [`RawStdioClient`] — raw-wire ACP driver for bytes the typed client can't
//!   produce (Foundation `\/` methods, string UUID ids)
//! - [`leader::LeaderStdioClient`] — ACP client that drives `hystersis agent --leader stdio` (unix)
...
//! - [`run_headless`] — Run `hystersis -p` against the mock server and capture output
//! - [`git_workdir`] — Create a git-initialized [`TestSandbox`]
//! - [`hystersis_binary`] — Resolve the hystersis binary path (HYSTERSIS_BINARY env or cargo_bin)
...
/// Multiply a harness timeout by `HYSTERSIS_TEST_TIMEOUT_SCALE` (positive integer,
/// default 1). CI lanes on shared runner pools raise it so pool load slows
/// tests instead of failing them (see the Hystersis merge CI workflow).
pub fn scaled(base: std::time::Duration) -> std::time::Duration {
    let scale = std::env::var("HYSTERSIS_TEST_TIMEOUT_SCALE")
        .ok()
...
#[cfg(unix)]
pub mod uds_proxy;
pub use acp_client::{HystersisStdioClient, RawStdioClient};
pub use counting_server::spawn_counting_server;
pub use env::{EnvGuard, git_workdir, hystersis_binary};
pub use headless::{
