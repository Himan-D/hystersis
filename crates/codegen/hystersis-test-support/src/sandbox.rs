//! Hermetic filesystem and child-environment owner for hystersis integration tests.
...
/// hystersis network kill switches, and explicit overrides.
pub struct TestSandbox {
    root: TempDir,
    home: PathBuf,
    hystersis_home: PathBuf,
    workspace: PathBuf,
    temp: PathBuf,
...
impl TestSandbox {
    ...
    /// Explicit hystersis state root.
    pub fn hystersis_home(&self) -> &Path {
        &self.hystersis_home
    }
    ...
    pub fn git_command(&self) -> Command {
        ...
        cmd.stdin(Stdio::null()).envs(hystersis_tty_utils::pager_env());
        for &(key, value) in &hystersis_tty_utils::GIT_AUTH_SUPPRESSION_ENVS {
            cmd.env(key, value);
        }
    ...
    pub fn diagnostic_summary(&self) -> String {
        let mut summary = format!(
            "root={} home={} hystersis_home={} workspace={} temp={}",
            self.root().display(),
            self.home.display(),
            self.hystersis_home.display(),
...
impl TestSandboxBuilder {
    /// Wire hystersis API, models, feedback, trace, conversation, and web traffic to
    ...
    pub fn build(self) -> TestSandbox {
        ...
        for path in [&home, &hystersis_home, &workspace, &temp] {
        ...
        let mut env = baseline_env(&home, &hystersis_home, &temp, &parent_cwd);
...
impl TestSandbox {
    fn init_git_workspace(&self) {
        ...
        run_git(self, &["config", "user.name", "Hystersis Test"]);
...
fn baseline_env(
    ...
    baseline_env_from_parent(home, hystersis_home, temp, parent_cwd, &parent_env)
}
...
fn baseline_env_from_parent(
    ...
    for (key, value) in [
        ("HYSTERSIS_TELEMETRY_ENABLED", "false"),
        ("HYSTERSIS_TELEMETRY_TRACE_UPLOAD", "false"),
        ("HYSTERSIS_FEEDBACK_ENABLED", "false"),
        ("HYSTERSIS_TRACE_UPLOAD", "false"),
        ("HYSTERSIS_INSTRUMENTATION", "disabled"),
        ("OTEL_SDK_DISABLED", "true"),
        ("DISABLE_TELEMETRY", "1"),
        ("DISABLE_FEEDBACK_COMMAND", "1"),
        ("HYSTERSIS_DISABLE_AUTOUPDATER", "1"),
        ("HYSTERSIS_PROMPT_SUGGESTIONS", "false"),
        // Pin so a developer-exported override cannot flake empty-home launch tests.
        ("HYSTERSIS_DEFAULT_PERMISSION_MODE", "ask"),
        // Post-turn summary side-calls would add unscripted requests to the
        // mock server and break exact wire-traffic assertions.
        ("HYSTERSIS_TURN_SUMMARY", "0"),
    ...
    env.insert(
        ...
        hystersis_home.join("gitconfig").into_os_string(),
    );
    env
}
...
fn diagnostic_value_is_sensitive(key: &OsStr) -> bool {
    ...
    is_secret_key(&key)
        ...
        || matches!(
            ...
            "HOME" | "USERPROFILE" | "HYSTERSIS_HOME" | "TMPDIR" | "TMP" | "TEMP" | "GIT_CONFIG_GLOBAL"
        )
}
...
mod tests {
    ...
    fn owns_distinct_isolated_paths() {
        ...
        assert_eq!(sandbox.hystersis_home(), sandbox.home().join(".hystersis"));
    }
    ...
    fn git_command_uses_sandbox_state_without_process_global_mutation() {
        ...
        let sandbox = TestSandbox {
            ...
            hystersis_home: root.path().join("home/.hystersis"),
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "HYSTERSIS_HOME"),
            Some(sandbox.hystersis_home().into())
        );
        assert_eq!(
        ...
        assert_eq!(
            env_value(&sandbox, "HYSTERSIS_DISABLE_AUTOUPDATER").as_deref(),
            Some(OsStr::new("1"))
        );
        assert_eq!(
            env_value(&sandbox, "HYSTERSIS_TELEMETRY_TRACE_UPLOAD").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "HYSTERSIS_LEADER_SOCKET"), None);
        assert_eq!(env_value(&sandbox, "HYSTERSIS_DISABLE_WEB_FETCH"), None);
        assert_eq!(env_value(&sandbox, "HYSTERSIS_WEB_FETCH"), None);
    }
    ...
    fn command_application_clears_ambient_env_and_command_override_wins() {
        ...
        cmd.env("AMBIENT_SECRET", "must-disappear")
            .env("HYSTERSIS_PROMPT_SUGGESTIONS", "ambient");
        sandbox.apply_to_std_command(&mut cmd);
        cmd.env("HYSTERSIS_PROMPT_SUGGESTIONS", "command");
        ...
        assert_eq!(
            env.get(OsStr::new("HYSTERSIS_PROMPT_SUGGESTIONS"))
                .map(OsString::as_os_str),
    ...
    fn explicit_overrides_win_and_can_remove_baseline_entries() {
        ...
        sandbox
            ...
            .set_env("HYSTERSIS_PROMPT_SUGGESTIONS", "true")
            .set_env("NO_PROXY", "override.invalid")
            .remove_env("HYSTERSIS_DISABLE_AUTOUPDATER");
        assert_eq!(
            env_value(&sandbox, "TERM_PROGRAM").as_deref(),
            Some(OsStr::new("vscode"))
        );
        assert_eq!(
            env_value(&sandbox, "HYSTERSIS_PROMPT_SUGGESTIONS").as_deref(),
            Some(OsStr::new("true"))
        );
        assert_eq!(
            env_value(&sandbox, "NO_PROXY").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "HYSTERSIS_DISABLE_AUTOUPDATER"), None);
    }
    ...
    fn diagnostics_fail_closed_for_credential_keys() {
        ...
        for (key, value) in [
            ...
            ("HYSTERSIS_DEPLOYMENT_KEY", "deployment-key-do-not-print"),
            ("HYSTERSIS_EXTRA_AUTH_KEY", "alpha-test-key-do-not-print"),
