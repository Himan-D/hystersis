...
# are the single source of truth for the `hystersis.com/announcements/update` payload.
# Pipeline:
...
DESKTOP_DIR="$REPO_ROOT/frontend/apps/hystersis-desktop"
...
for f in "${bindings[@]}"; do
  {
    ...
    echo "// Regenerate via generate.sh in the xai-hystersis-announcements package."
    cat "$f"
  } >"$DST/$(basename "$f")"
done
