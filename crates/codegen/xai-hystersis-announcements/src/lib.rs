//! Shared announcement types, persistence, and formatting for Hystersis CLI apps.
//!
//! This crate provides the common logic used by `xai-hystersis-shell` and
//! `xai-hystersis-pager` for handling announcements (banner notifications).
...
/// Payload for `hystersis.com/announcements/update` ACP notification.
...
/// Read hidden announcement ids from `~/.hystersis/announcements.json`.
...
/// Write hidden announcement ids to `~/.hystersis/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    xai_hystersis_tools::util::hystersis_home::hystersis_home().join("announcements.json")
}
...
///
/// Precedence: `HYSTERSIS_ANNOUNCEMENTS_OVERRIDE` env var (JSON override) → remote announcements.
...
pub fn resolve_startup(
    ...
    if let Ok(raw) = std::env::var("HYSTERSIS_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid HYSTERSIS_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("HYSTERSIS_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("HYSTERSIS_ANNOUNCEMENTS_OVERRIDE");
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Get HystersisPro","url":"https://hystersis.com/hystersis","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Get HystersisPro"));
        assert_eq!(cta.url.as_deref(), Some("https://hystersis.com/hystersis"));
