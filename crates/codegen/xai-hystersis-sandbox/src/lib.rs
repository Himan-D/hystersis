...
//! OS-level sandboxing for Hystersis via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use xai_hystersis_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
const BWRAP_ENV_VAR: &str = "__HYSTERSIS_INSIDE_BWRAP";
...
impl SandboxManager {
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        if requires_hook_write_deny(&self.profile, workspace) {
            xai_hystersis_config::ensure_hystersis_hook_slots(paths::hystersis_home().as_path())
...
/// Zero-permission placeholder (file or dir) under `hystersis_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent hystersis
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::hystersis_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    fn bwrap_hook_plan_binds_ancestors_then_leaves() {
        ...
        let sources = [xai_hystersis_config::GlobalHookSource {
            path: leaf.clone(),
            kind: xai_hystersis_config::GlobalHookSourceKind::ConfiguredSource,
        }];
    ...
    /// Create a temp workspace whose `.hystersis/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("hystersis-{tag}-{}-{nanos}", std::process::id()));
        let hystersis = ws.join(".hystersis");
        std::fs::create_dir_all(&hystersis).unwrap();
        std::fs::write(hystersis.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("hystersis-deny-dir-{}", std::process::id()));
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::hystersis_home()
