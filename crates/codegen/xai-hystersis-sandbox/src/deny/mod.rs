...
pub(crate) fn ancestors_within_writable_roots(
    ...
    let mut out = Vec::new();
    for anc in xai_hystersis_config::existing_ancestor_chain(path) {
...
pub(crate) fn apply_write_deny_paths_to_capability_set(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        tracing::info!(
            count = entries.len(),
            "Applied Seatbelt write-deny for Hystersis-owned direct hook sources"
        );
    }
    #[cfg(target_os = "linux")]
    {
...
mod tests {
    ...
    fn ancestors_pin_under_writable_root_not_home() {
        ...
        let hystersis = tmp.join("hystersis");
        let sessions = hystersis.join("sessions");
        let leaf = sessions.join("extra-hooks");
        std::fs::create_dir_all(&leaf).unwrap();
        let ws = tmp.join("ws");
        std::fs::create_dir_all(&ws).unwrap();

        let roots = [hystersis.clone(), ws.clone()];
        let pin = ancestors_within_writable_roots(&leaf, &roots);
        assert!(
            pin.iter().any(|p| p == &sessions),
            "must pin sessions under HYSTERSIS_HOME: {pin:?}"
        );
        assert!(
            pin.iter().any(|p| p == &hystersis),
            "must pin HYSTERSIS_HOME grant root: {pin:?}"
        );
        assert!(
