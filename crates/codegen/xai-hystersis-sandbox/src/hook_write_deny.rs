//! Hystersis-owned hook write-deny: plan, identity revalidation, and post-reexec checks.
...
use xai_hystersis_config::{GlobalHookSource, missing_configured_sources, resolve_global_hook_sources};

#[cfg(unix)]
use xai_hystersis_config::validated_hook_json_files_for_sources;
#[cfg(target_os = "linux")]
use xai_hystersis_config::{ensure_hystersis_hook_slots, unique_ancestors_rootward};
...
impl From<xai_hystersis_config::GlobalHookSourceError> for HookWriteDenyError {
    fn from(e: xai_hystersis_config::GlobalHookSourceError) -> Self {
...
fn reject_hardlinked_files(sources: &[GlobalHookSource]) -> Result<(), HookWriteDenyError> {
    use std::os::unix::fs::MetadataExt;
    use xai_hystersis_config::GlobalHookSourceKind;
    for s in sources {
...
pub fn resolve_hook_write_deny_snapshot() -> Result<Vec<GlobalHookSource>, HookWriteDenyError> {
    ...
    let resolved =
        resolve_global_hook_sources(Some(hystersis.as_path()), /* reject_symlinks */ true)?;
...
pub fn prepare_hook_write_deny(
    ...
    let hystersis = hystersis_home();
    ensure_hystersis_hook_slots(hystersis.as_path())?;
...
fn capture_dir_json_snapshot(dir: &Path) -> Result<DirJsonSnapshot, HookWriteDenyError> {
    use xai_hystersis_config::{list_direct_hook_json_files, validate_direct_hook_json_file};
