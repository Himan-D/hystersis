//! E2E path-deny and Hystersis hook write-deny (subprocess; arm64-tagged).
...
const HYSTERSIS_HOME_ENV: &str = "SANDBOX_E2E_HYSTERSIS_HOME";
...
fn apply_fixture_env(cmd: &mut Command, home: &Path, hystersis_home: &Path, workspace: &Path) {
    cmd.env(WORKSPACE_ENV, workspace.as_os_str())
        .env(HOME_ENV, home.as_os_str())
        .env(HYSTERSIS_HOME_ENV, hystersis_home.as_os_str())
        .env("HOME", home.as_os_str())
        .env("HYSTERSIS_HOME", hystersis_home.as_os_str());
}
...
fn run_scenario(
    ...
    apply_fixture_env(&mut cmd, home, hystersis_home, workspace);
    let output = cmd
...
fn skip_if_enforcement_unavailable() -> bool {
    ...
    let support = xai_hystersis_sandbox::SandboxManager::support_info();
...
fn profile_from_env() -> xai_hystersis_sandbox::ProfileName {
    xai_hystersis_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    // Isolate HOME/HYSTERSIS_HOME before any config OnceLock init.
    let home = PathBuf::from(std::env::var(HOME_ENV).expect(HOME_ENV));
    let hystersis_home = PathBuf::from(std::env::var(HYSTERSIS_HOME_ENV).expect(HYSTERSIS_HOME_ENV));
    ...
    match scenario.as_str() {
        ...
        "hook_write_deny_marker_spoof" => subprocess_hook_write_deny_marker_spoof(&hystersis_home),
        other => {
...
fn subprocess_profile_and_bwrap_reexec(profile: &xai_hystersis_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !xai_hystersis_sandbox::is_inside_bwrap() {
            ...
            match xai_hystersis_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = xai_hystersis_sandbox::SandboxManager::new(profile, workspace);
...
fn subprocess_hook_write_deny_marker_spoof(_hystersis_home: &Path) {
    #[cfg(not(target_os = "linux"))]
    {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        unsafe {
            std::env::set_var("__HYSTERSIS_INSIDE_BWRAP", "1");
        }
        match xai_hystersis_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
/// Workspace-profile Hystersis-owned hook write-deny probes (existing sources + first-run).
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    let home = PathBuf::from(std::env::var(HYSTERSIS_HOME_ENV).expect(HYSTERSIS_HOME_ENV));

    let profile = xai_hystersis_sandbox::ProfileName::Workspace;
    subprocess_profile_and_bwrap_reexec(&profile, workspace);
    ...
    if first_run {
        ...
        eprintln!("OK: first-run Hystersis hook slots denied");
    } else {
    ...
    // Parent grants remain creatable: Hystersis runtime sibling, workspace, temp.
    assert_write_ok(
        "hystersis runtime sibling",
...
/// Create isolated HOME + HYSTERSIS_HOME fixture dirs for a scenario.
fn fixture_homes(
    tag: &str,
) -> (
    PathBuf,
    ...
    let hystersis = unique_temp_dir(&format!("{tag}-hystersis"));
    let workspace = unique_temp_dir(&format!("{tag}-ws"));
    // Empty global sandbox config under fixture HYSTERSIS_HOME so generic tests do
    // not inherit the developer/runner's ~/.hystersis/sandbox.toml.
    fs::write(hystersis.join("sandbox.toml"), "").expect("empty global sandbox.toml");
    (
        home.clone(),
        hystersis.clone(),
        workspace.clone(),
        TempDirGuard(home),
        TempDirGuard(hystersis),
...
fn run_deny_case(
    ...
    let (home, hystersis, tmp, _ch, _cg, _cw) = fixture_homes(tag);
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".hystersis")).expect("mkdir .hystersis");
    fs::write(
        tmp.join(".hystersis").join("sandbox.toml"),
        format!("[profiles.{profile}]\nextends = \"workspace\"\ndeny = [{deny_list}]\n"),
    )
    .expect("write sandbox.toml");

    // Ensure Hystersis fixed slots exist so workspace-based custom profiles can
    // resolve hook write-deny without depending on the real user tree.
    fs::create_dir_all(hystersis.join("hooks")).expect("mkdir fixture hooks");
    fs::write(hystersis.join("hooks-paths"), b"").expect("write fixture hooks-paths");
    ...
    let (status, stderr) = run_scenario(&home, &hystersis, &tmp, profile, targets, controls, postlaunch);
    assert!(
        status.success(),
...
fn hardlinked_hooks_paths_refuses_startup() {
    ...
    let (home, hystersis, workspace, _ch, _cg, _cw) = fixture_homes("hook-hl");
    fs::create_dir_all(hystersis.join("hooks")).unwrap();
    let reg = hystersis.join("hooks-paths");
    let alias = hystersis.join("hooks-paths-alias");
    fs::write(&reg, b"").unwrap();
    fs::hard_link(&reg, &alias).unwrap();

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &hystersis, &workspace, "hook_write_deny");
    assert!(
        !status.success(),
...
/// Workspace profile: Hystersis-owned direct hook sources are write-denied but readable;
/// create / overwrite / unlink / rename / mkdir fail; absolute hooks-paths
/// targets are denied; parent rename is blocked; Hystersis/CWD/temp siblings stay writable.
#[test]
fn workspace_protects_direct_hook_sources() {
    ...
    let (home, hystersis, workspace, _ch, _cg, _cw) = fixture_homes("hook");

    fs::create_dir_all(hystersis.join("hooks")).expect("mkdir hooks");
    fs::write(hystersis.join("hooks").join("keep.json"), r#"{"keep-me":true}"#)
        .expect("write keep.json");
    let dynamic = hystersis.join("sessions").join("extra-hooks");
    ...
    for needle in [
        ...
        "OK: hystersis runtime sibling writable",
...
fn hardlinked_hooks_json_refuses_startup() {
    ...
    let (home, hystersis, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-hl");
    fs::create_dir_all(hystersis.join("hooks")).unwrap();
    fs::write(hystersis.join("hooks-paths"), b"").unwrap();
    let active = hystersis.join("hooks").join("active.json");
    let alias = hystersis.join("hooks").join("active-alias.json");
...
fn symlinked_hooks_json_refuses_startup() {
    ...
    let (home, hystersis, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-sym");
    fs::create_dir_all(hystersis.join("hooks")).unwrap();
    fs::write(hystersis.join("hooks-paths"), b"").unwrap();
    let real = hystersis.join("real-active.json");
    let active = hystersis.join("hooks").join("active.json");
...
/// First-run: missing fixed slots are created as real Hystersis state before apply,
...
fn workspace_protects_direct_hook_sources_first_run() {
    ...
    let (home, hystersis, workspace, _ch, _cg, _cw) = fixture_homes("hook-fr");
    // Intentionally leave hooks/ and hooks-paths absent (first-run ensure path).

    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &hystersis, &workspace, "hook_write_deny_first_run");
    assert!(
        status.success(),
    ...
    for needle in [
        "OK: first-run Hystersis hook slots denied",
    ...
    // Post-exit host: Hystersis slots exist and are valid; no vendor artifacts.
    assert!(
        hystersis.join("hooks").is_dir(),
        "post-exit: hooks dir must exist as a real directory"
    );
    assert!(
        hystersis.join("hooks-paths").is_file(),
        "post-exit: hooks-paths must exist as a real file"
    );
    assert_eq!(
        fs::read(hystersis.join("hooks-paths")).expect("read hooks-paths"),
        b"",
...
fn hook_write_deny_refuses_marker_spoof() {
    ...
    #[cfg(target_os = "linux")]
    {
        let (home, hystersis, workspace, _ch, _cg, _cw) = fixture_homes("hook-spoof");
        fs::create_dir_all(hystersis.join("hooks")).unwrap();
        fs::write(hystersis.join("hooks").join("x.json"), b"{}").unwrap();
        fs::write(hystersis.join("hooks-paths"), b"").unwrap();
        let (status, stderr) =
            run_hook_write_deny_scenario(&home, &hystersis, &workspace, "hook_write_deny_marker_spoof");
        assert!(
            status.success(),
