...
//!
//! ```text
//! cargo test -p xai-hystersis-sandbox --test read_write_trailing_glob_e2e -- --nocapture
//! ```
...
fn trailing_glob_read_write_grants_parent_directory() {
    ...
    let hystersis_home = root.join("hystersis-home");
    ...
    for dir in [&workspace, &hystersis_home, &cache.join("registry-a1b2")] {
        fs::create_dir_all(dir).expect("create fixture dir");
    }

    let cache_abs = dunce::canonicalize(&cache).expect("canonicalize cache");
    fs::write(
        hystersis_home.join("sandbox.toml"),
        format!(
            "[profiles.cargo]\nextends = \"workspace\"\nread_write = [{:?}]\n",
...
fn subprocess_entry() {
    ...
    let profile: xai_hystersis_sandbox::ProfileName = "cargo".parse().expect("profile name");
    let mut mgr = xai_hystersis_sandbox::SandboxManager::new(profile, &workspace);
