...
const ENV_THREADS: &str = "HYSTERSIS_GIX_STATUS_THREADS";
const ENV_THREADS_LEGACY: &str = "XAI_GIX_STATUS_THREADS";
const ENV_THREADS_LEGACY2: &str = "XAI_HYSTERSIS_GIX_STATUS_THREADS";
...
/// Production budget (`n >= 1`). Honours `HYSTERSIS_GIX_STATUS_THREADS=N` for `N >= 1`
/// (forced dial; bypasses nproc). Else cores + soft nproc + thread usage.
/// Falls back to `XAI_GIX_STATUS_THREADS` for one release (isolation).
pub fn compute_gix_status_thread_limit() -> usize {
    if let Ok(raw) = std::env::var(ENV_THREADS)
        .or_else(|_| std::env::var(ENV_THREADS_LEGACY))
        .or_else(|_| std::env::var(ENV_THREADS_LEGACY2))
...
mod tests {
    ...
    use hystersis_test_utils::git::run_git;
...
mod nproc_tests {
    ...
    const CHILD_ENV: &str = "HYSTERSIS_GIX_STATUS_NPROC_CHILD";
    const CHILD_ENV_LEGACY: &str = "XAI_GIX_STATUS_NPROC_CHILD";
    const CHILD_ENV_LEGACY2: &str = "XAI_HYSTERSIS_GIX_STATUS_NPROC_CHILD";
    const REPO_ENV: &str = "HYSTERSIS_GIX_STATUS_NPROC_REPO";
    const REPO_ENV_LEGACY: &str = "XAI_GIX_STATUS_NPROC_REPO";
    const REPO_ENV_LEGACY2: &str = "XAI_HYSTERSIS_GIX_STATUS_NPROC_REPO";
    ...
    fn spawn_child_or_skip(mode: &str) -> Option<std::process::Output> {
        if std::env::var_os(CHILD_ENV)
            .or_else(|| std::env::var_os(CHILD_ENV_LEGACY))
            .or_else(|| std::env::var_os(CHILD_ENV_LEGACY2))
            .is_some()
        {
    ...
    #[test]
    fn child_entry() {
        let Some(mode) = std::env::var_os(CHILD_ENV)
            .or_else(|| std::env::var_os(CHILD_ENV_LEGACY))
            .or_else(|| std::env::var_os(CHILD_ENV_LEGACY2))
        else {
            return;
        ...
        let repo = std::env::var_os(REPO_ENV)
            .or_else(|| std::env::var_os(REPO_ENV_LEGACY))
            .or_else(|| std::env::var_os(REPO_ENV_LEGACY2))
            .expect("repo env in child");
