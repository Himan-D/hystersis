...
//! for the shell's `hystersis.com/fs/*` ACP extension methods.
//!
//! These mirror the pure functions that previously lived only in the
//! shell (`xai-hystersis-shell/src/session/file_system.rs`) so that, in proxy
//! mode, a `hystersis.com/fs/*` request executes on the *remote* workspace server
...
use xai_hystersis_workspace_types::rpc::fs::FsReadEncoding;
