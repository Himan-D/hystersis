...
async fn dispatch_workspace_info_reports_server_version() {
    use xai_hystersis_workspace_types::rpc::workspace::{WorkspaceInfo, WorkspaceInfoReq};
    let handler = WorkspaceRpcHandler::new(make_handle());
    let value = handler
        .dispatch(
    ...
    assert_eq!(Some(xai_hystersis_version::VERSION.to_owned()), info.version);
}
#[tokio::test]
...
async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
    ...
    use xai_hystersis_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
    let handle = make_handle();
    ...
    let read_only = ToolServerConfig {
        tools: vec![tc(
            ...
            Some(xai_hystersis_tools::types::tool::ToolKind::Read),
        )],
...
async fn delete_scheduled_task_rpc_reports_honestly() {
    use xai_hystersis_workspace_types::rpc::workspace::DeleteScheduledTaskResponse;
    let handle = make_handle();
    let cfg = background_capable_cfg();
    let session = handle
        .create_session_with_config(
...
async fn kill_task_rpc_terminates_outstanding_background_task() {
    use xai_hystersis_workspace_types::rpc::workspace::{KillTaskOutcome, KillTaskResponse};
    let handle = make_handle();
    let cfg = background_capable_cfg();
    let session = handle
        .create_session_with_config(
...
async fn dispatch_knows_every_typed_method() {
    ...
    let methods = [
        ...
        <xai_hystersis_workspace_types::rpc::presence::PresenceNoteReq as WorkspaceRpc>::METHOD,
...
async fn dispatch_stamps_client_rpc_activity_for_mutations_only() {
    ...
    use xai_hystersis_workspace_types::rpc::workspace::DropSessionReq;
