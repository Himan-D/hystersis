...
use std::sync::Arc;
use xai_hystersis_tools::registry::types::ToolServerConfig;
use xai_hystersis_tools::types::tool::ToolKind;
use xai_hystersis_workspace_types::WorkspaceEvent;
...
impl xai_hystersis_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
    fn kind(&self) -> ToolKind {
        ToolKind::Execute
    }
    fn tool_namespace(&self) -> xai_hystersis_tools::types::tool::ToolNamespace {
        xai_hystersis_tools::types::tool::ToolNamespace::MCP
    }
...
impl xai_tool_runtime::Tool for BashCcoStub {
    ...
    type Output = xai_hystersis_tools::types::output::ToolOutput;
    ...
    ) -> Result<xai_hystersis_tools::types::output::ToolOutput, xai_tool_runtime::ToolError> {
        let output = BASH_CCO_STUB_STDOUT.as_bytes();
        Ok(xai_hystersis_tools::types::output::ToolOutput::Bash(
            xai_hystersis_tools::types::output::BashOutput {
                output: output.to_vec(),
                output_for_prompt:
                    xai_hystersis_tools::types::output::BashOutput::make_output_for_prompt(
...
async fn rebind_explicit_to_explicit_with_in_flight_call_defers_then_corrects() {
    use xai_hystersis_session_events::ToolOutcome;
...
async fn toolset_terminal(
    toolset: &Arc<xai_hystersis_tools::registry::types::FinalizedToolset>,
) -> Arc<dyn xai_hystersis_tools::computer::types::TerminalBackend> {
    let res = toolset.resources.lock().await;
    res.get::<xai_hystersis_tools::types::resources::Terminal>()
        .map(|t| t.0.clone())
...
///
/// [`TerminalRunRequest`]: xai_hystersis_tools::computer::types::TerminalRunRequest
pub(crate) fn terminal_run_request(
    command: &str,
    out_dir: &std::path::Path,
    tool_call_id: &str,
) -> xai_hystersis_tools::computer::types::TerminalRunRequest {
    xai_hystersis_tools::computer::types::TerminalRunRequest {
        ...
        notification_handle: xai_hystersis_tools::notification::ToolNotificationHandle::noop(),
...
pub(crate) async fn start_background_sleep(
    ...
    out_dir: &std::path::Path,
    tool_call_id: &str,
) -> xai_hystersis_tools::computer::types::BackgroundHandle {
    session
        .terminal_backend()
...
impl crate::config::SessionContextFactory for PersistentShellFactory {
    fn build_session_context(
        ...
        backend: Arc<dyn xai_hystersis_tools::computer::types::TerminalBackend>,
    ) -> xai_hystersis_tools::registry::types::SessionContext {
        self.inner
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            xai_hystersis_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
        )
    }
    fn registry_builder(&self) -> xai_hystersis_tools::registry::types::ToolRegistryBuilder {
...
async fn restarted_workspace_recreates_session_and_reports_lost_task() {
    ...
    let xai_hystersis_tools::types::output::ToolOutput::TaskOutput(
...
async fn events_jsonl_captures_turn_tool_toggle_and_mcp_variants() {
    ...
    handle.shared().session_event_writer(sid).emit(
        xai_hystersis_session_events::Event::McpToolCallStarted {
...
fn turn_outcome_label_maps_every_variant() {
    use xai_hystersis_session_events::TurnOutcomeLabel;
...
async fn hook_registry_snapshot_is_disconnected() {
    ...
    {
        let spec = xai_hystersis_hooks::config::HookSpec {
            name: "injected".into(),
            event: xai_hystersis_hooks::event::HookEventName::SessionStart,
            handler_type: xai_hystersis_hooks::config::HandlerType::Command,
            ...
            layer: xai_hystersis_hooks::config::HookProvenance::File,
        };
...
async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
    ...
    assert!(
        reason.contains(xai_hystersis_version::VERSION),
...
async fn rebind_session_root_rewrites_existing_cwd() {
    ...
    let cwd_res = {
        let toolset = session.toolset();
        let res = toolset.resources.lock().await;
        res.get::<xai_hystersis_tools::types::resources::Cwd>()
            .expect("toolset Cwd")
            .clone()
    };
    assert_eq!(
...
async fn local_harness_virtualizes_inbound_and_outbound() {
    ...
    impl xai_hystersis_tools::types::tool_metadata::ToolMetadata for LocalPathEcho {
        fn kind(&self) -> ToolKind {
            ToolKind::Other
        }
        fn tool_namespace(&self) -> xai_hystersis_tools::types::tool::ToolNamespace {
