//! Permission resolution engine: merges native `.hystersis/config.toml`,
...
/// (fail-safe) with a warn + skip record for `hystersis inspect`.
...
/// user `~/.hystersis` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    xai_hystersis_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Load `[permission]` rules from native Hystersis TOML config files:
///
///   * `~/.hystersis/config.toml` (lowest priority)
///   * Each `.hystersis/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.hystersis/config.toml` first (lowest priority within this layer).
    // Gated on user_hystersis_home() so a project's .hystersis/config.toml is never read as
    // global permissions when neither HYSTERSIS_HOME nor a home dir resolves.
    if let Some(global_path) = xai_hystersis_config::user_hystersis_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match xai_hystersis_config::load_config_file(&global_path) {
    ...
    // `.hystersis/config.toml` (same gate as project `.claude/settings.json`).
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match xai_hystersis_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[xai_hystersis_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Hystersis and Claude sources.
...
/// `.hystersis/config.toml` permission rules (mirrors [`load_claude_env_with_project`]).
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `hystersis inspect`.
...
struct ResolveInputs<'a> {
    ...
    /// `.claude/settings.json` / `.hystersis/config.toml` permission rules are dropped
    ...
    fn live(project_trusted: bool) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &xai_hystersis_config::managed_config_layers(),
            ),
            project_trusted,
        }
    }
...
/// bypass is pinned off via hystersis `requirements.toml`
...
///
/// `project_trusted` gates project-tier Claude settings and `.hystersis/config.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `hystersis inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in claude_settings_paths_for_trust(cwd, project_trusted) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `hystersis inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `hystersis inspect` via the outer resolver.
...
/// [`xai_hystersis_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = xai_hystersis_config::claude_managed_settings_path() else {
...
/// not consulted: hystersis must not inherit a host-wide always-approve lockdown from
/// that file. hystersis still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in hystersis use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = xai_hystersis_config::requirements_layers();
...
/// without `~/.hystersis`); `path` only names the layer in a non-bool warning.
...
/// Namespace prefix for legacy injected MCP server names (`hystersis_com_*`).
/// Policy matching still uses this spelling.
pub const MANAGED_MCP_PREFIX: &str = "hystersis_com_";

/// Max `char` length of a managed runtime name (`hystersis_com_` + normalized display
...
/// reduce to one key (strip `hystersis_com_`, [`normalize_managed_name`]) and
...
fn mcp_name_matches(pattern: &str, name: &str) -> bool {
    ...
    // Mirror the legacy injected-name truncation: `hystersis_com_*` runtime
