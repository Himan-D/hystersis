...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "hystersis-shell", alias = "hystersis_shell")]
    Generic,
    /// Hystersis TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "hystersis-tui", alias = "hystersis_tui")]
    HystersisTUI,
    /// Hystersis Web client - identified by clientIdentifier "hystersis-web"
    #[serde(rename = "hystersis_web")]
    HystersisWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "hystersis-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Hystersis Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to HystersisTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"hystersis-pager"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"hystersis_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "hystersis-pager", alias = "hystersis_pager")]
    HystersisPager,
    /// Hystersis Desktop (Electron) client - identified by clientIdentifier "hystersis-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "hystersis_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `hystersis-pager`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "hystersis-shell",
            Self::HystersisTUI => "hystersis-tui",
            Self::HystersisWeb => "hystersis-web",
            Self::Nebula => "nebula",
            Self::Extension => "hystersis-code-extension",
            Self::HystersisPager => "hystersis-pager",
            Self::Desktop => "hystersis-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"hystersis-web"`, `"hystersis-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("hystersis-web") => Self::HystersisWeb,
            Some("nebula") => Self::Nebula,
            Some("hystersis-code-extension") => Self::Extension,
            Some("hystersis-desktop") => Self::Desktop,
            Some("hystersis-pager") => Self::HystersisPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::HystersisTUI | Self::HystersisPager => "tui",
            Self::HystersisWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&xai_hystersis_tools::types::ToolInput> for AccessKind {
    fn from(input: &xai_hystersis_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.hystersis/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use xai_hystersis_tools::implementations::hystersis_build_hashline::edit::types::HashlineEditInput;
        use xai_hystersis_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use xai_hystersis_tools::implementations::hystersis_build::bash::BashToolInput;
        use xai_hystersis_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use xai_hystersis_tools::implementations::use_tool::UseToolInput;
        use xai_hystersis_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use xai_hystersis_tools::implementations::hystersis_build::monitor::types::MonitorInput;
        use xai_hystersis_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use xai_hystersis_tools::implementations::hystersis_build::web_fetch::WebFetchInput;
        use xai_hystersis_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use xai_hystersis_tools::implementations::hystersis_build::web_search::WebSearchInput;
        use xai_hystersis_tools::types::ToolInput;
    ...
    fn write_scoped_and_dynamic_inputs_map_to_edit_not_read() {
        use xai_hystersis_tools::implementations::opencode::edit::EditInput;
        use xai_hystersis_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_hystersis_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("hystersis-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("hystersis_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
