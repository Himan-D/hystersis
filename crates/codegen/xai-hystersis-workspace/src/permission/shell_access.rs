...
impl ProtectedEditReason {
    pub fn kind(self) -> &'static str {
        match self {
            ...
            Self::Etc => "etc",
            Self::HystersisConfig => "hystersis_config",
            Self::HystersisSandbox => "hystersis_sandbox",
    ...
    pub fn description(self) -> Option<&'static str> {
        match self {
            ...
            Self::HystersisConfig => Some(
                "Note: This edit contains changes to Hystersis config, which can alter permissions, tools, and other behavior in later sessions.",
            ),
            Self::HystersisSandbox => Some(
                "Note: This edit contains changes to the Hystersis sandbox config, which can loosen filesystem and network restrictions on commands.",
            ),
...
pub(crate) fn edit_target_protection(path: &Path) -> Option<ProtectedEditReason> {
    ...
    let lexical = xai_hystersis_paths::normalize_lexically(path);
...
fn protected_edit_reason(path: &Path) -> Option<ProtectedEditReason> {
    ...
    if protected_hystersis_hook_root(path, &string_components) {
    ...
    if let Some(reason) = protected_hystersis_config_file(path, &string_components) {
        return Some(reason);
    }
...
/// Hystersis config files that alter permissions (`config.toml`, the
...
/// inside any `.hystersis` dir (user-global default and workspace overlays) and
/// directly under a custom `$HYSTERSIS_HOME`, which the component match cannot see.
fn protected_hystersis_config_file(path: &Path, components: &[&str]) -> Option<ProtectedEditReason> {
    protected_hystersis_config_file_with_home(
        path,
        components,
        xai_hystersis_config::user_hystersis_home().as_deref(),
    )
}

fn protected_hystersis_config_file_with_home(
    path: &Path,
    components: &[&str],
    user_hystersis_home: Option<&Path>,
) -> Option<ProtectedEditReason> {
    let reason = match components.last().copied() {
        Some(
            xai_hystersis_config::USER_CONFIG_FILENAME
            | xai_hystersis_config::MANAGED_CONFIG_FILENAME
            | xai_hystersis_config::REQUIREMENTS_FILENAME,
        ) => ProtectedEditReason::HystersisConfig,
        Some("sandbox.toml") => ProtectedEditReason::HystersisSandbox,
        _ => return None,
    };
    let in_dot_hystersis = components.len() >= 2 && components[components.len() - 2] == ".hystersis";
    let in_hystersis_home = || hystersis_home_matches(user_hystersis_home, |home| path.parent() == Some(home));
    (in_dot_hystersis || in_hystersis_home()).then_some(reason)
}

/// True when `pred` holds for the user hystersis home in either its lexical or
...
fn hystersis_home_matches(home: Option<&Path>, pred: impl Fn(&Path) -> bool) -> bool {
    home.is_some_and(|home| {
        let lexical = xai_hystersis_paths::normalize_lexically(home);
        pred(&lexical)
            || resolve_following_symlinks(&lexical, 0).is_some_and(|resolved| pred(&resolved))
    })
}

fn path_is_under_user_hystersis_hook_root(path: &Path, hystersis_home: &Path) -> bool {
    path.starts_with(hystersis_home.join("hooks")) || path == hystersis_home.join("hooks-paths")
}

fn protected_hystersis_hook_root(path: &Path, components: &[&str]) -> bool {
    components.windows(2).any(|pair| pair == [".hystersis", "hooks"])
        || components.ends_with(&[".hystersis", "hooks-paths"])
        || hystersis_home_matches(xai_hystersis_config::user_hystersis_home().as_deref(), |home| {
            path_is_under_user_hystersis_hook_root(path, home)
        })
}
...
mod tests {
    ...
    fn sensitive_edit_targets_and_lexical_aliases_prompt() {
        for path in [
            ...
            "/home/user/.hystersis/sandbox.toml",
            "/work/project/.hystersis/sandbox.toml",
        ] {
            assert!(
        ...
        for path in [
            "/work/src/main.rs",
            "/work/project/.hystersis/config.toml/backup",
    ...
    fn edit_target_protection_classifies_reasons() {
        let cases = [
            (
                "/home/user/.hystersis/hooks/evil.json",
            ...
            (
                "/home/user/.hystersis/config.toml",
                ProtectedEditReason::HystersisConfig,
            ),
            (
                "/home/user/.hystersis/sandbox.toml",
                ProtectedEditReason::HystersisSandbox,
            ),
            (
                "/work/project/.hystersis/sandbox.toml",
                ProtectedEditReason::HystersisSandbox,
            ...
            (
                "/home/user/.hystersis/managed_config.toml",
                ProtectedEditReason::HystersisConfig,
            ),
            (
                "/home/user/.hystersis/requirements.toml",
                ProtectedEditReason::HystersisConfig,
            ),
            (
    ...
    fn sensitive_edit_targets_include_hook_roots() {
        for path in [
            "/home/user/.hystersis/hooks/evil.json",
            "/home/user/.hystersis/hooks/nested/deep.json",
            "/home/user/.hystersis/hooks-paths",
            "/home/user/.claude/settings.json",
            "/home/user/.claude/settings.local.json",
            "/home/user/.cursor/hooks.json",
            "/work/project/.hystersis/hooks/local.json",
            "/work/project/.hystersis/hooks-paths",
        ] {
            assert!(
                edit_target_protection(Path::new(path)).is_some(),
        ...
        for path in [
            "/home/user/.hystersis/hooks-disabled/note.json",
            "/home/user/.hystersis/hooks-evil/note.json",
    ...
    #[test]
    fn path_is_under_user_hystersis_hook_root_matches_relocated_home() {
        let home = Path::new("/custom/hystersis-home");
        for path in [
            "/custom/hystersis-home/hooks/x.json",
            "/custom/hystersis-home/hooks/nested/deep.json",
            "/custom/hystersis-home/hooks",
            "/custom/hystersis-home/hooks-paths",
        ] {
            assert!(
                path_is_under_user_hystersis_hook_root(Path::new(path), home),
                "must match under custom hystersis home: {path}"
            );
        }
        for path in [
            "/custom/hystersis-home/hooks-disabled/note.json",
            "/custom/hystersis-home/hooks-evil/note.json",
            "/custom/hystersis-home/config.toml",
            "/custom/other/hooks/x.json",
            "/custom/hystersis-home-extra/hooks/x.json",
        ] {
            assert!(
                !path_is_under_user_hystersis_hook_root(Path::new(path), home),
    ...
    fn sensitive_edit_targets_follow_symlinks() {
        ...
        .unwrap();
        let hystersis_hook = outside.path().join(".hystersis/hooks/evil.json");
        std::fs::create_dir_all(hystersis_hook.parent().unwrap()).unwrap();
        std::fs::write(&hystersis_hook, b"{}").unwrap();
        symlink(&hystersis_hook, ws.path().join("hystersis-hook-link")).unwrap();

        for path in [
            ws.path().join("file-link"),
            ws.path().join("hooks-link/new-hook"),
            ws.path().join("module-hooks-link/new-hook"),
            ws.path().join("hystersis-hook-link"),
        ] {
            assert!(
    ...
    /// A custom `$HYSTERSIS_HOME` has no `.hystersis` path component, so the live
    /// `config.toml` / `sandbox.toml` must be caught by the home-prefix branch.
    #[test]
    fn hystersis_config_files_under_custom_hystersis_home_are_protected() {
        let home = tempfile::tempdir().unwrap();
        let home_path = home.path();
        for (file, reason) in [
            ("config.toml", ProtectedEditReason::HystersisConfig),
            ("managed_config.toml", ProtectedEditReason::HystersisConfig),
            ("requirements.toml", ProtectedEditReason::HystersisConfig),
            ("sandbox.toml", ProtectedEditReason::HystersisSandbox),
        ] {
            let path = home_path.join(file);
            let components = [file];
            assert_eq!(
                protected_hystersis_config_file_with_home(&path, &components, Some(home_path)),
                Some(reason),
                "{file} directly under $HYSTERSIS_HOME must be protected"
            );
        }
    ...
    /// The resolved-symlink arm of the hystersis-home match must decide: `$HYSTERSIS_HOME`
    /// points at a symlink while the edit targets the physical home directory,
    /// so the lexical parent-equality arm cannot fire.
    #[test]
    #[cfg(unix)]
    fn hystersis_config_under_symlinked_hystersis_home_is_protected() {
        use std::os::unix::fs::symlink;
        ...
        assert_eq!(
            ...
            Some(ProtectedEditReason::HystersisSandbox)
        );
    }
    ...
    fn resolved_root_alias_matches_physical_destination() {
        ...
        assert!(resolved_path_is_within_root(
            &resolved_root.join("hystersis-test"),
            Path::new("/etc")
        ));
        assert!(!resolved_path_is_within_root(
            Path::new("/tmp/hystersis-test"),
            Path::new("/etc")
        ));
    }
