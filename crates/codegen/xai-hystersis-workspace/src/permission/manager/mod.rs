...
use xai_hystersis_mcp::servers::parse_mcp_qualified_name;
use xai_hystersis_paths::AbsPathBuf;
use xai_hystersis_tools::implementations::hystersis_build::web_fetch::{
    DomainMatcher, config::DEFAULT_ALLOWED_DOMAINS, domain::normalize_domain,
};
use xai_hystersis_tools::types::resources::resolve_model_path;
                ...
                PermissionCommand::Request {
                    ...
                    if matches!(&access, AccessKind::Bash(_))
                        && sandbox_may_auto_allow_bash(
                            bash_evaluation.as_ref(),
                            xai_hystersis_sandbox::should_auto_allow_bash(),
                        )
                        && !policy_forced_prompt
                        && !auto_forced_prompt
                    {
