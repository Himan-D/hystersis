...
// Shared so `HYSTERSIS_HOME`/`HOME` mutations here also serialize against the
...
fn load_claude_env_merges_with_precedence() {
    // HYSTERSIS_HOME-isolate so the claude-import marker reads clean (an imported
    ...
    let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let home = tempfile::tempdir().unwrap();
    let _home_guard = EnvVarGuard::set("HYSTERSIS_HOME", home.path());
    let _marker_guard = EnvVarGuard::unset("_HYSTERSIS_CLAUDE_MARKER_OVERRIDE");
    let tmp = tempfile::tempdir().unwrap();
    let claude_dir = tmp.path().join(".claude");
    std::fs::create_dir_all(&claude_dir).unwrap();
...
fn load_claude_env_empty_when_no_settings() {
    // Isolate HYSTERSIS_HOME (claude-import marker) AND HOME (global `~/.claude`)
...
fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
    ...
    // drop it. Isolate HYSTERSIS_HOME so the claude-import marker reads clean (an
...
fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
    ...
    assert!(mcp_name_matches("foo", "hystersis_com_foo"));
    assert!(mcp_name_matches("hystersis_com_foo", "foo"));
    assert!(mcp_name_matches("hystersis_com_foo", "hystersis_com_foo"));
    assert!(!mcp_name_matches("foo", "foobar"));
    assert!(!mcp_name_matches("foo", "hystersis_com_foobar"));
...
fn mcp_name_matches_is_case_and_space_insensitive() {
    ...
    assert!(mcp_name_matches("Slack", "hystersis_com_slack"));
    assert!(mcp_name_matches("My Server", "hystersis_com_my_server"));
    assert!(mcp_name_matches("hystersis_com_my_server", "My Server"));
    ...
    assert!(!mcp_name_matches("hystersis_com_", "hystersis_com_anything"));
}
...
/// Truncation applies only to `hystersis_com_*` names — long plain names
...
/// A long `hystersis_com_*` POLICY entry must not become a prefix grant over
...
fn long_managed_allow_entry_does_not_grant_plain_prefix_names() {
    ...
    // managed name AT the cap. An attacker-chosen `hystersis_com_*` decoy that is
...
fn denied_by_server_name_matches_bare_and_managed_prefix() {
    ...
    let managed = http_named("hystersis_com_foo", "https://foo.example.com/mcp");
    assert!(al.is_server_denied(&managed));
    assert!(!al.is_server_allowed(&managed));

    // Name match is transport-agnostic.
    let stdio = stdio_named("hystersis_com_foo", "npx");
    ...
    for unrelated in ["foobar", "hystersis_com_foobar", "barfoo", "bar"] {
...
fn allowed_by_server_name_restricts_across_transports() {
    ...
    assert!(al.is_server_allowed(&http_named("hystersis_com_foo", "https://evil.example.com/x")));
    assert!(al.is_server_allowed(&stdio_named("hystersis_com_foo", "/usr/bin/whatever")));
...
fn server_name_deny_beats_allow() {
    ...
    for s in [
        ...
        http_named("hystersis_com_foo", "https://foo.example.com/x"),
    ] {
...
fn server_name_prefix_edge_cases_vice_versa() {
    ...
    let al = allowlist_from(serde_json::json!({
        "deniedMcpServers": [ { "serverName": "hystersis_com_foo" } ]
    }));

    assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
    assert!(al.is_server_denied(&http_named("hystersis_com_foo", "https://x.example.com/mcp")));
    assert!(!al.is_server_denied(&http_named("foobar", "https://x.example.com/mcp")));
    assert!(!al.is_server_denied(&http_named("hystersis_com_foobar", "https://x.example.com/mcp")));
}
...
fn untrusted_project_claude_permissions_are_not_honored() {
    ...
    let home = tempfile::tempdir().unwrap();
    let _home_guard = EnvVarGuard::set("HOME", home.path());
    let _hystersis_guard = EnvVarGuard::set("HYSTERSIS_HOME", home.path());
    let _marker_guard = EnvVarGuard::unset("_HYSTERSIS_CLAUDE_MARKER_OVERRIDE");
...
/// Untrusted clone must not contribute project `.hystersis/config.toml` [permission].
///
/// Sync + `block_on` so `ENV_LOCK` is not held across `.await` (clippy
/// `await_holding_lock`). Does not assert exact global rule counts:
/// `xai_hystersis_config::hystersis_home()` is a process-wide `OnceLock`, so under
/// single-process `cargo test` an earlier test may have already pinned
/// `HYSTERSIS_HOME`. Project-rule filtering is independent of that; global
/// survival is checked only when our temp home is the live `user_hystersis_home()`.
#[test]
fn untrusted_project_config_toml_permissions_are_not_honored() {
    ...
    // Global allow (survives untrusted project when HYSTERSIS_HOME resolves here).
    std::fs::write(
...
"#,
    ...
    let rt = tokio::runtime::Builder::new_current_thread()
        ...
        .build()
        .expect("test runtime");
    // Untrusted may be None when no global rules load (HYSTERSIS_HOME OnceLock
    ...
    let global_live = xai_hystersis_config::user_hystersis_home()
        .is_some_and(|g| g == home.path() || g.starts_with(home.path()));
    if global_live {
        let untrusted = untrusted.expect("global rules present when HYSTERSIS_HOME is live");
        assert!(
            untrusted
                .config
                .rules
...
fn non_bool_lock_key_warns_and_does_not_lock() {
    ...
    let p = Path::new("/etc/hystersis/requirements.toml");
    let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
    ...
    assert!(
        out.contains("/etc/hystersis/requirements.toml"),
...
fn admin_source_trusts_only_root_owned_tiers() {
    ...
    // the user-writable `~/.hystersis/requirements.toml` is not, despite its path.
    ...
    assert!(is_admin_source(&RequirementSource::SystemRequirements {
        path: "/etc/hystersis/requirements.toml".into(),
    }));
    assert!(!is_admin_source(&RequirementSource::Requirements {
        path: "/home/u/.hystersis/requirements.toml".into(),
    }));
    assert!(!is_admin_source(&RequirementSource::ManagedConfig {
        path: "/etc/hystersis/managed_config.toml".into(),
    }));
...
fn drop_untrusted_catchall_allows_is_source_aware() {
    ...
    let rules = vec![
        ...
        // Managed config: defaults tier, untrusted even from /etc/hystersis.
        sourced(
            allow_any(Some("*")),
...
fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
    ...
    assert!(
        ...
        "typo should be recorded for hystersis inspect"
    );
}
...
async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
    ...
    let layers = xai_hystersis_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
    ...
    .unwrap();
    assert_eq!(
        xai_hystersis_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
        1
    );
