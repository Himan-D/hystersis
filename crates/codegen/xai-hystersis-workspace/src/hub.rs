...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(xai_hystersis_version::VERSION)
            .image_capabilities(
...
struct CallCompletedGuard {
    ...
    outcome: xai_hystersis_session_events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: xai_hystersis_session_events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: xai_hystersis_session_events::ToolOutcome) {
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    ToolStreamItem::Terminal(Ok(run_result)) => {
                        ...
                        _guard.set_outcome(xai_hystersis_session_events::ToolOutcome::Success);
                    ...
                    ToolStreamItem::Terminal(Err(e)) => {
                        ...
                        _guard.set_outcome(xai_hystersis_session_events::ToolOutcome::Error);
                        let e = match &virt {
...
mod tests {
    ...
    use xai_hystersis_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use xai_hystersis_tools::notification::types::{
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use xai_hystersis_tools::computer::types::{TaskKind, TaskSnapshot};
        ...
        ) -> Result<xai_hystersis_tools::types::output::ToolOutput, xai_tool_runtime::ToolError>
        {
            ...
            Ok(xai_hystersis_tools::types::output::ToolOutput::Bash(
                xai_hystersis_tools::types::output::BashOutput {
                    ...
                    output_for_prompt:
                        xai_hystersis_tools::types::output::BashOutput::make_output_for_prompt(stdout),
                    exit_code: 0,
