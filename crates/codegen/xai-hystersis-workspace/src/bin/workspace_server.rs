...
use url::Url;
use xai_hystersis_diag_server::{self as diag_server, DiagHandle, ErrorClass};
use xai_hystersis_workspace::config::WorkspaceServerMetadata;
use xai_hystersis_workspace::error::WorkspaceError;
use xai_hystersis_workspace_daemon::daemonize;
use xai_hystersis_workspace_daemon::preview_supervisor::{
...
struct TrackerSink(std::sync::Arc<xai_hystersis_workspace::activity::ActivityTracker>);
...
async fn run(
    ...
    {
        use xai_hystersis_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("HYSTERSIS_SANDBOX_PROFILE").ok() {
            ...
            None if xai_hystersis_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
        ...
        if profile == ProfileName::Off {
            ...
            sandbox.install();
            let active = xai_hystersis_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = xai_hystersis_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let mut status_config = xai_hystersis_workspace::StatusConfig::from_env();
    status_config.preview_control_port = args.preview.preview_control_port;
    let auth_provider = xai_hystersis_workspace::hub_auth::provider(
        &url,
    ...
    {
        let caps = xai_hystersis_workspace::image_capabilities::image_capabilities();
    ...
    xai_hystersis_workspace::init_metrics();
    let ws_handle = match xai_hystersis_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    let grace_budget = xai_hystersis_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            xai_hystersis_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    /// tracker's in `xai_hystersis_workspace::activity`; this covers the seam.
    #[test]
    fn tracker_sink_forwards_every_signal_to_the_activity_tracker() {
        use xai_hystersis_workspace::activity::ActivityTracker;
