...
/// `worktrees.db` (`xai_fast_worktree::resolve_hystersis_home`), so checkout dirs and
/// the metadata DB always live under the same `.hystersis` tree. That resolver
/// canonicalizes its `$HOME` fallback to match `xai_hystersis_config::hystersis_home()`,
/// so worktree paths also agree with trust/hooks and other hystersis-home paths.
fn hystersis_home() -> std::path::PathBuf {
    xai_fast_worktree::resolve_hystersis_home().unwrap_or_else(|_| {
        dirs::home_dir()
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &xai_hystersis_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use xai_hystersis_config_types::WorktreeKindMaxAge;
    ...
    // resolve paths.
    let xai_hystersis_config_types::WorktreeAutoGcSettings {
        enabled,
        max_age_secs,
        min_interval_secs,
        dry_run,
...
fn worktree_auto_gc_settings_from_toml(
    root: &toml::Value,
) -> Option<xai_hystersis_config_types::WorktreeAutoGcSettings> {
    root.get("worktree")
        .and_then(|w| w.get("auto_gc"))
        // toml::Value only deserializes by value (no &Value Deserializer).
        .and_then(|v| xai_hystersis_config_types::WorktreeAutoGcSettings::deserialize(v.clone()).ok())
}
...
fn local_auto_gc_policy(
    local: Option<&xai_hystersis_config_types::WorktreeAutoGcSettings>,
...
mod tests {
    ...
    fn local_auto_gc_policy_opts_in_only_when_enabled() {
        use xai_hystersis_config_types::WorktreeAutoGcSettings;
