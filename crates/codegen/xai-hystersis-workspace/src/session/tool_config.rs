...
use std::sync::Arc;
use xai_hystersis_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    notification_handle: Option<xai_hystersis_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<xai_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<xai_hystersis_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn xai_hystersis_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            xai_hystersis_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// [`LocalTerminalBackend`]: xai_hystersis_tools::computer::local::LocalTerminalBackend
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(xai_hystersis_tools::computer::local::LocalFs)
            as Arc<dyn xai_hystersis_tools::computer::types::AsyncFileSystem>;
        let notification_handle = xai_hystersis_tools::notification::ToolNotificationHandle::noop();
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            xai_hystersis_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    let version = xai_hystersis_version::VERSION;
    headers.insert(
...
fn build_web_fetch_config() -> xai_hystersis_tools::implementations::hystersis_build::web_fetch::WebFetchConfig
{
    use xai_hystersis_tools::implementations::hystersis_build::web_fetch::{WebFetchConfig, WebFetchParams};
    ...
    if xai_hystersis_config::env_bool("HYSTERSIS_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
...
mod tests {
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use xai_hystersis_tools::types::resources::{State, WebCitationCounter};
