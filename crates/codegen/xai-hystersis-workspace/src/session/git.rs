...
///
/// ```
/// use xai_hystersis_workspace::session::git::normalize_repo_url;
///
/// assert_eq!(
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = xai_hystersis_tools::util::hystersis_home::hystersis_home().join("worktrees");
...
pub fn git_rewind_enabled() -> bool {
    xai_hystersis_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
...
async fn seed_default_gitignore(git_root: &Path) -> Result<()> {
    ...
    tokio::fs::write(&path, xai_hystersis_workspace_types::binding::DEFAULT_GITIGNORE).await?;
