...
//!
//! The store is rooted at [`xai_hystersis_config::user_hystersis_home`] — the **Option**
...
impl TrustStore {
    ...
    ///
    /// Resolves via [`xai_hystersis_config::user_hystersis_home`], never
    /// [`xai_hystersis_config::hystersis_home`], so it never falls back to a cwd-relative
    ...
    pub fn default_path() -> Option<PathBuf> {
        Self::default_path_in(xai_hystersis_config::user_hystersis_home())
    }
...
pub fn migrate_legacy_hook_trust() {
    ...
    MIGRATED.call_once(|| {
        let Some(legacy_file) = xai_hystersis_hooks::trust::legacy_trust_file_path() else {
            return;
        };
...
fn migrate_legacy_hook_trust_in(legacy_file: &Path, store: &mut TrustStore) -> usize {
    ...
    let projects = match xai_hystersis_hooks::trust::list_trusted_projects_with_file(legacy_file) {
        Ok(p) => p,
        Err(e) => {
            tracing::warn!(
...
mod tests {
    ...
    fn default_path_sources_from_user_hystersis_home() {
        ...
        assert_eq!(
            ...
            xai_hystersis_config::user_hystersis_home().map(|h| h.join(TRUST_FILE_NAME))
        );
    }
