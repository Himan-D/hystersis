...
//!
//! `xai-hystersis-shell` re-exports this crate as `hystersis_shell::terminal`.
...
/// Resolved absolute path to bash. On Unix uses the `hystersis_config` shell
...
pub(crate) fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        hystersis_config::shell::unix_shell_path(hystersis_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
pub use hystersis_tools::util::pager_env;
