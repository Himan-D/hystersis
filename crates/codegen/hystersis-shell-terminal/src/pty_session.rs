...
use hystersis_workspace::file_system::TargetClientId;
...
/// Mirrors `hystersis_shell::extensions::routing::send_routed_notification` so
...
const NOTIFICATION_METHOD: &str = "hystersis.com/terminal/pty/notification";
...
enum Shell {
    Running {
        ...
        group: Option<Arc<hystersis_tty_utils::ProcessGroup>>,
    },
...
impl Shell {
    ...
    fn attach_group(&mut self, enrolled: Arc<hystersis_tty_utils::ProcessGroup>) {
    ...
    fn reap_now(&mut self) {
        if self.hangup() && self.wait_exit(hystersis_tty_utils::HANGUP_GRACE) {
            return;
        }
        self.kill();
...
pub async fn create_pty(
    ...
    if let Some(pid) = shell.pid() {
        ...
        let enrolled = tokio::task::spawn_blocking(move || {
            hystersis_tty_utils::global_process_scope().enroll_terminal_pid(pid)
        })
        .await
    ...
    if hystersis_tty_utils::global_process_scope().is_closed() {
...
fn reap(entry: &Arc<Mutex<PtySession>>) {
    ...
    if hung_up && wait_for_exit(entry, hystersis_tty_utils::HANGUP_GRACE) {
        return;
    }
...
mod tests {
    ...
    use hystersis_acp_lib::acp_gateway;
    ...
    async fn scope_teardown_kills_a_background_grandchild() {
        tokio::task::LocalSet::new()
            .run_until(async {
                ...
                let scope = hystersis_tty_utils::ProcessScope::new();
