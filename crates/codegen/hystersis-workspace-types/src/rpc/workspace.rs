...
/// over `hystersis_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `hystersis_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// Outcome of `workspace.kill_task`, mirroring `hystersis_tools::KillOutcome`
...
/// One TODO list item (slim DTO over `hystersis_tools`'s `TodoState`). `status`
...
pub struct WorkspaceInfo {
    ...
    pub shell: String,
    pub cwd: String,
    /// Server binary version (`hystersis_version::VERSION`); `None` on
