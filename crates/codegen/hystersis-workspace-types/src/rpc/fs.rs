...
//! the shell's `hystersis.com/fs/*` ACP methods.
...
// workspace server (`xai-hystersis-workspace`) and the hystersis.com backend
...
// `workspace.fs_*` ops above already serve the shell's `hystersis.com/fs/*`
...
/// Filesystem node kind. Wire values match the shell's `hystersis.com/fs/list`
/// node `type` strings.
...
/// `hystersis.com/fs/list` plus `offset` pagination).
