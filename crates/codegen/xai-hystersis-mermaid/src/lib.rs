...
//!
//! ```
//! use xai_hystersis_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `HystersisDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `HystersisNight`, `TokyoNight`).
    Dark,
}
