...
use xai_hystersis_tools::util::ProcessGroup;
...
pub async fn run_command_hook(
    ...
    let debug_payloads = std::env::var("HYSTERSIS_HOOK_DEBUG").is_ok_and(|v| v == "1");
    if debug_payloads {
        tracing::trace!(
            hook_name = %spec.name,
    ...
    let mut cmd = if is_shell_command {
        ...
        #[cfg(not(unix))]
        {
            ...
            let inv = xai_hystersis_config::shell::shell_command_argv(command_str.as_ref());
    ...
    // plugin) can spoof `HYSTERSIS_HOOK_EVENT`, `HYSTERSIS_HOOK_NAME`, `HYSTERSIS_SESSION_ID`,
    // `HYSTERSIS_WORKSPACE_ROOT`, or `CLAUDE_PROJECT_DIR`, which are the
    ...
    #[cfg(not(unix))]
    let env_root = {
        use xai_hystersis_config::shell::{WindowsShell, detect_windows_shell};
    ...
    let mut child = match cmd
        ...
        .env("HYSTERSIS_HOOK_EVENT", envelope.hook_event_name.to_string())
        .env("HYSTERSIS_HOOK_NAME", &spec.name)
        .env("HYSTERSIS_SESSION_ID", ctx.session_id)
        .env("HYSTERSIS_WORKSPACE_ROOT", env_root.as_ref())
        // Compatibility alias for external hooks that read this env name.
        // Same value as `HYSTERSIS_WORKSPACE_ROOT`; native `.hystersis` hooks should use
        // `HYSTERSIS_WORKSPACE_ROOT`.
...
///   hooks),
/// * the Hystersis process's own environment (which is inherited by the child),
...
mod tests {
    ...
    fn resolve_command_path_variants() {
        ...
        assert_eq!(
            resolve_command_path(&spec(
                HandlerType::Command,
                Some("bin/check.sh"),
                "/project/.hystersis/hooks"
            )),
            Some(std::path::PathBuf::from(
                "/project/.hystersis/hooks/bin/check.sh"
            ))
        );
        assert_eq!(
    ...
    fn make_scoped_ctx(scope: xai_hystersis_tools::util::ProcessScope) -> RunContext<'static> {
        RunContext {
    ...
    fn powershell_rewrite_cases() {
        ...
        let cases = [
            ...
            (
                "$CLAUDE_PROJECT_DIR/$HYSTERSIS_HOOK_NAME.ps1",
                r#"& "$env:CLAUDE_PROJECT_DIR/$env:HYSTERSIS_HOOK_NAME.ps1""#,
            ),
            (
    ...
    fn find_unresolved_skips_resolvable_vars() {
        ...
        let v = find_unresolved_env_vars(
            "${HYSTERSIS_HOOK_EVENT}/${CLAUDE_PROJECT_DIR}/${HYSTERSIS_SESSION_ID}/${CLAUDE_PLUGIN_ROOT}/foo",
            &env,
        );
        assert!(
            v.is_empty(),
    ...
    async fn test_tilde_expansion_runs_via_shell() {
        let tmp = tempfile::tempdir().unwrap();
        // Create the script at <tmp>/.hystersis-test-hooks-gb856/tilde-test.sh
        let hook_dir = tmp.path().join(".hystersis-test-hooks-gb856");
        ...
        // Inject HOME via extra_env so `sh -c "~/.hystersis-test-hooks-gb856/..."`
        ...
        let spec = HookSpec {
            ...
            enabled: true,
            command: Some(std::path::PathBuf::from(
                "~/.hystersis-test-hooks-gb856/tilde-test.sh",
            )),
            command_raw: Some("~/.hystersis-test-hooks-gb856/tilde-test.sh".to_string()),
            url: None,
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::env::temp_dir(),
    ...
    async fn command_hook_session_close_reaps_whole_group() {
        ...
        let envelope = make_envelope();
        let scope = xai_hystersis_tools::util::ProcessScope::new();
