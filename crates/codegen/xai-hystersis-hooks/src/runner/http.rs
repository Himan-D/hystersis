...
pub async fn run_http_hook(
    ...
    for (k, v) in [
        ("HYSTERSIS_HOOK_EVENT", envelope.hook_event_name.to_string()),
        ("HYSTERSIS_HOOK_NAME", spec.name.clone()),
        ("HYSTERSIS_SESSION_ID", ctx.session_id.to_string()),
        ("HYSTERSIS_WORKSPACE_ROOT", ctx.workspace_root.to_string()),
    ...
    // reach `~/.hystersis/logs`; threaded into the reqwest error format below so
...
mod tests {
    ...
    async fn url_unresolved_var_fails_validation() {
        let key = "HYSTERSIS_HOOKS_HTTP_TEST_UNRESOLVED";
