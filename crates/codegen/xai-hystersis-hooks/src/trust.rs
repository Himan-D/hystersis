...
// (`~/.hystersis/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_hystersis_home>/trusted-hook-projects`), or `None` when no user hystersis home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(xai_hystersis_config::user_hystersis_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user hystersis home (set $HYSTERSIS_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$HYSTERSIS_HOME/disabled-hooks`, or `None` when no user hystersis
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(xai_hystersis_config::user_hystersis_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let hystersis_dir = dir.join(".hystersis");
        std::fs::create_dir_all(&hystersis_dir).unwrap();
        hystersis_dir.join("trusted-hook-projects")
    }
