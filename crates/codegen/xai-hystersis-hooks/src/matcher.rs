use regex::Regex;
use xai_hystersis_tools::types::{claude_names_for, hystersis_names_for};
...
///   **exact** match against each name (after external→Hystersis alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the Hystersis tool `run_terminal_command`).
///
...
/// plus any Hystersis tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to Hystersis registry in
/// `xai-hystersis-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for hystersis_name in hystersis_names_for(term) {
            push(hystersis_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_hystersis_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // Hystersis name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_hystersis_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // Hystersis equivalent
        assert!(m.is_match("hashline_edit")); // second Hystersis alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn regex_against_claude_alias_matches_hystersis_tool() {
        // A regex written against an external alias still matches the Hystersis tool
