//! Integration tests for xai-hystersis-hooks.
//!
...
use xai_hystersis_hooks::discovery::load_hooks;
use xai_hystersis_hooks::dispatcher;
use xai_hystersis_hooks::event::*;
use xai_hystersis_hooks::result::HookDecision;
use xai_hystersis_hooks::runner::RunContext;
...
async fn non_blocking_dispatch() {
    ...
    assert!(matches!(
        &results[0],
        xai_hystersis_hooks::result::HookRunResult::Success { .. }
    ));
}
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: xai_hystersis_hooks::event::StopFailureKind::RateLimit,
        ...
        Case {
            ...
            payload: HookPayload::StopCancelled {
                reason: xai_hystersis_hooks::event::StopCancelledReason::UserInterrupt,
                cancelled_by: xai_hystersis_hooks::event::CancelledBy::User,
...
/// runner-reserved keys (`HYSTERSIS_HOOK_EVENT`, `HYSTERSIS_HOOK_NAME`,
/// `HYSTERSIS_SESSION_ID`, `HYSTERSIS_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$HYSTERSIS_HOOK_EVENT" > {f}; echo "NAME=$HYSTERSIS_HOOK_NAME" >> {f}; echo "SESSION=$HYSTERSIS_SESSION_ID" >> {f}; echo "ROOT=$HYSTERSIS_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "HYSTERSIS_HOOK_EVENT": "spoofed_event",
                                "HYSTERSIS_HOOK_NAME": "spoofed_name",
                                "HYSTERSIS_SESSION_ID": "spoofed_session",
                                "HYSTERSIS_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "HYSTERSIS_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed HYSTERSIS_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "HYSTERSIS_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed HYSTERSIS_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "HYSTERSIS_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed HYSTERSIS_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
