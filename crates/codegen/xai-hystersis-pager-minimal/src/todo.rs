...
use xai_hystersis_shell::tools::TodoStatus;
    ...
    let mut lines: Vec<Line<'static>> = todos
        ...
        .map(|t| {
            let (glyph, style) = match t.status {
                ...
                TodoStatus::Completed => (xai_hystersis_pager::glyphs::check_mark(), theme.muted()),
                TodoStatus::Cancelled => (
                    xai_hystersis_pager::glyphs::ballot_x(),
...
mod tests {
    ...
    use xai_hystersis_shell::tools::{TodoItem, TodoPriority};
