...
//! seam ([`xai_hystersis_pager::minimal_hook`]) of function pointers, and the
...
/// subsequent calls are ignored (see [`xai_hystersis_pager::minimal_hook`]).
pub fn install() {
    xai_hystersis_pager::minimal_hook::install(xai_hystersis_pager::minimal_hook::MinimalHooks { draw });
}
