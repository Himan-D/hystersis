//! Installed hystersis CLI version, lockstepped with shipping binaries.

use std::sync::OnceLock;

use semver::Version;

pub const TEST_VERSION_ENV: &str = "HYSTERSIS_TEST_VERSION";

pub const VERSION: &str = match option_env!("HYSTERSIS_VERSION") {
    Some(v) => v,
    None => env!("CARGO_PKG_VERSION"),
};

/// The release pipeline always injects `HYSTERSIS_VERSION`; without it the build is from source.
pub const IS_DEV_BUILD: bool = option_env!("HYSTERSIS_VERSION").is_none();
