...
const CHILD_WORKTREE: &str = "HYSTERSIS_FAST_WORKTREE_SAFETY_CHILD_WORKTREE";
const CHILD_WORKTREE_LEGACY: &str = "XAI_FAST_WORKTREE_SAFETY_CHILD_WORKTREE";
const CHILD_WORKTREE_LEGACY2: &str = "XAI_HYSTERSIS_FAST_WORKTREE_SAFETY_CHILD_WORKTREE";
const CHILD_SOURCE: &str = "HYSTERSIS_FAST_WORKTREE_SAFETY_CHILD_SOURCE";
const CHILD_SOURCE_LEGACY: &str = "XAI_FAST_WORKTREE_SAFETY_CHILD_SOURCE";
const CHILD_SOURCE_LEGACY2: &str = "XAI_HYSTERSIS_FAST_WORKTREE_SAFETY_CHILD_SOURCE";
...
fn no_combination_of_working_tree_shapes_loses_uncarried_work() {
    ...
    for subset in 0..(1u8 << shapes.len()) {
        ...
        let ref_name = format!("refs/hystersis/subagents/{name}");
...
fn reclaiming_a_worktree_runs_none_of_its_hooks() {
    ...
    snapshot_into(&worktree, &fixture.source, "refs/hystersis/subagents/has-hooks");
...
fn verdict_under_a_foreign_git_dir() {
    let worktree = std::env::var_os(CHILD_WORKTREE)
        .or_else(|| std::env::var_os(CHILD_WORKTREE_LEGACY))
        .or_else(|| std::env::var_os(CHILD_WORKTREE_LEGACY2))
...
fn snapshot_under_a_foreign_clean_filter() {
    ...
    let source = PathBuf::from(
        std::env::var_os(CHILD_SOURCE)
            .or_else(|| std::env::var_os(CHILD_SOURCE_LEGACY))
            .or_else(|| std::env::var_os(CHILD_SOURCE_LEGACY2))
            .expect("the parent sets CHILD_SOURCE"),
    );
    assert!(
        std::env::var_os("GIT_CONFIG_GLOBAL").is_some(),
        "the parent must set the variable this test is about"
    );
    let ref_name = "refs/hystersis/subagents/inherits-a-filter";
