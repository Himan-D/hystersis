...
//! (id → `refs/hystersis/worktrees/<id>` only).
...
pub const BACKING_MARKER_FILE: &str = "hystersis-nfs-worktree.json";
...
pub fn candidate_data_dirs() -> Vec<PathBuf> {
    ...
    if let Some(hystersis_home) = hystersis_home::resolve_hystersis_home() {
        push(hystersis_home.join("grove"));
    }
    dirs
}
...
pub fn gc_orphan_pins(
    ...
    for idn in identities.values() {
        ...
        let pin = format!("refs/hystersis/worktrees/{}", idn.worktree_id);
    ...
    for (id, ent) in &state.orphans {
        ...
        let pin = format!("refs/hystersis/worktrees/{id}");
        candidates
            .entry(id.clone())
...
mod tests {
    ...
    fn identities_prefer_metadata_grove_over_legacy_nfs() {
        ...
        rec.metadata = Some(serde_json::json!({
            "grove": {
                ...
                "source_pin": "refs/hystersis/worktrees/wt-grove"
            },
            "nfs": {
                "backing": "/legacy/should-not-win",
                "source_pin": "refs/hystersis/worktrees/legacy"
            }
        }));
        ...
        assert_eq!(
            ...
            Some("refs/hystersis/worktrees/wt-grove")
        );
    }
    #[test]
    ...
    fn db_loss_then_source_gc_keeps_pin_via_union_liveness() {
        ...
        hystersis_tty_utils::detach_std_command(&mut reset);
        assert!(
            reset
                .current_dir(&repo)
        ...
        let pin = "refs/hystersis/worktrees/wt-live";
        let mut uref = std::process::Command::new("git");
        hystersis_tty_utils::detach_std_command(&mut uref);
        assert!(
        ...
        hystersis_tty_utils::detach_std_command(&mut gc);
        assert!(
            gc.current_dir(&repo)
                .args(["gc", "--prune=now"])
                .status()
        ...
        hystersis_tty_utils::detach_std_command(&mut cat);
        let cat_out = cat
            .current_dir(&repo)
    ...
    fn aborted_partial_removal_prunes_after_grace() {
        ...
        let pin = "refs/hystersis/worktrees/wt-orphan";
        let mut uref = std::process::Command::new("git");
        hystersis_tty_utils::detach_std_command(&mut uref);
        assert!(
            uref.current_dir(&repo)
    ...
    fn in_flight_create_pin_survives_gc() {
        ...
        git_commit_all(&repo, "c");
        let oid = git_rev_parse(&repo, "HEAD");
        let pin = "refs/hystersis/worktrees/wt-fly";
        let mut uref = std::process::Command::new("git");
        hystersis_tty_utils::detach_std_command(&mut uref);
        assert!(
            uref.current_dir(&repo)
    ...
    fn aborted_journal_does_not_mask_marker_or_mounts_toml() {
        ...
        let pin = "refs/hystersis/worktrees/wt-mask";
        let mut uref = std::process::Command::new("git");
        hystersis_tty_utils::detach_std_command(&mut uref);
        assert!(
            uref.current_dir(&repo)
    ...
    fn planted_orphan_state_does_not_delete_heads_main() {
        ...
        hystersis_tty_utils::detach_std_command(&mut check);
        assert!(
            check
                .current_dir(&repo)
