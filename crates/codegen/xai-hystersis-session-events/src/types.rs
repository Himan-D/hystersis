...
pub enum Event {
    ...
    /// `TODO_GATE_*` discriminator constant in `xai-hystersis-shell::session::events`.
    TodoGateFired {
        fires: u32,
        pending: usize,
        in_progress: usize,
    ...
    /// constants in `xai-hystersis-shell::session::events`.
    ...
    /// in `xai-hystersis-shell::session::events`.
    ...
    /// discriminator constants in `xai-hystersis-shell::session::events`.
    ...
    /// `xai-hystersis-shell::session::events`.
    ...
    /// `xai-hystersis-shell::session::events`. Fail-open never pauses the goal.
    ...
    /// enumerated by
    /// `xai-hystersis-shell::session::goal_stop_detector::PATTERN_LABELS`;
...
pub enum InterjectionSource {
    /// Direct `hystersis.com/interject` while a turn was running (Ctrl+Enter).
    Direct,
...
pub enum RedirectKind {
    /// Mid-turn interjection — Ctrl+O / `hystersis.com/interject`, or "Send now" on a
...
/// Telemetry mirror of `xai-hystersis-shell`'s `GoalClassifierVerdict`. Two
/// crates due to the orphan rule; the conversion lives in
/// `xai-hystersis-shell/src/session/events.rs`.
#[derive(Debug, Clone, Copy, Serialize)]
#[serde(rename_all = "snake_case")]
...
/// Telemetry mirror of `xai-hystersis-shell`'s `GoalPauseReason`. The two types
...
mod tests {
    ...
    fn goal_strategist_fired_serializes_cadence_field() {
        ...
        let ev = Event::GoalStrategistFired {
            ...
            every: 3,
            model_id: "hystersis-4".to_string(),
        };
        let v = serde_json::to_value(&ev).unwrap();
        ...
        assert_eq!(v["every"], 3);
        assert_eq!(v["model_id"], "hystersis-4");
    }
