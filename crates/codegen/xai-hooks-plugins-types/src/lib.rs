...
//!
//! This crate defines the wire format for `hystersis.com/hooks/*` and `hystersis.com/plugins/*`
//! ACP extension methods. It is dependency-free (only `serde`) so both
//! `xai-hystersis-shell` and `xai-hystersis-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `xai-hystersis-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `xai-hystersis-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// Hystersis's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `xai-hystersis-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.hystersis/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}

/// Response for `hystersis.com/hooks/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `hystersis.com/plugins/list`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response for `hystersis.com/mcp/list` as consumed by the pager.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `hystersis.com/hooks/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Request wrapper for `hystersis.com/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Shared action response for both `hystersis.com/hooks/action` and `hystersis.com/plugins/action`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.hystersis/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.hystersis/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.hystersis/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserHystersis),
            conflict: None,
        };
...
// ---------------------------------------------------------------------------
// Marketplace types (wire format for hystersis.com/marketplace/* ACP endpoints)
// ---------------------------------------------------------------------------

/// Response for `hystersis.com/marketplace/list`.
...
/// Request wrapper for `hystersis.com/marketplace/action`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
