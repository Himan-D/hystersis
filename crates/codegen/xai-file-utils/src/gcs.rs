...
use xai_hystersis_auth::{AuthCredentialProvider, StaticAuthCredentialProvider};

use crate::storage_client::{Auth401AttributionCallback, StaticHystersisAuth, StorageClient};
...
/// `StaticHystersisAuth` carrying the inline user / deployment keys from
...
fn build_proxy_client_with_fallback(
    ...
    let provider = credentials.unwrap_or_else(|| {
        let mut creds = StaticHystersisAuth::new(Some(user_token.to_owned()));
