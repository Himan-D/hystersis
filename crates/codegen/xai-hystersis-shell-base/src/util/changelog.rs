...
//! are published per-version to the CDN at `hystersis.com/cli/changelogs/`.
//!
...
const CHANGELOG_BASE: &str = "https://hystersis.com/cli/changelogs";
...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$HYSTERSIS_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `hystersis_home()` OnceLock). A seeded `$HYSTERSIS_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("HYSTERSIS_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::hystersis_home::hystersis_home);
        Self {
    ...
    ///
    /// When `HYSTERSIS_CHANGELOG_OFFLINE` is set (PTY / integration tests), skip
    ...
    /// `$HYSTERSIS_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`HYSTERSIS_HOME` /
    /// `HYSTERSIS_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$HYSTERSIS_HOME` even when offline mode was not
...
fn changelog_offline() -> bool {
    std::env::var_os("HYSTERSIS_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
fn fetch_blocking(url: &str) -> anyhow::Result<String> {
    let client =
        xai_hystersis_extra_ca::build_blocking_reqwest_client(|builder| builder.timeout(FETCH_TIMEOUT))?;
...
mod tests {
    ...
    /// `$HYSTERSIS_HOME` env so tests never race the parallel harness.
    ...
    fn cdn_miss_falls_back_to_env_home_disk_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("hystersis-home-fallback");
        std::fs::create_dir_all(&home).unwrap();
