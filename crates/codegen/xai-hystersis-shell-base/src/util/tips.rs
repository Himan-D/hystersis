...
//! persisted to `~/.hystersis/tip_cursor.json`.
...
fn cursor_path(hystersis_home: &Path) -> PathBuf {
    hystersis_home.join(CURSOR_FILE)
}

/// Load the cursor from `~/.hystersis/tip_cursor.json`. Returns 0 on any error.
fn load_cursor(hystersis_home: &Path) -> u64 {
    let text = match std::fs::read_to_string(cursor_path(hystersis_home)) {
        Ok(t) => t,
        Err(_) => return 0,
    };
...
/// Save the cursor to `~/.hystersis/tip_cursor.json`. Silently ignores write errors.
fn save_cursor(hystersis_home: &Path, cursor: u64) {
    if let Ok(text) = serde_json::to_string(&TipState { cursor }) {
        let _ = std::fs::write(cursor_path(hystersis_home), text);
    }
}
...
/// cursor in `~/.hystersis/tip_cursor.json`, so every session sees the next tip
...
pub fn pick_and_advance(tips: &[String], hystersis_home: &Path) -> Option<String> {
    if tips.is_empty() {
        return None;
    }
    let cursor = load_cursor(hystersis_home);
    let tip = tips[cursor as usize % tips.len()].clone();
    save_cursor(hystersis_home, cursor + 1);
    Some(tip)
}
