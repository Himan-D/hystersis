...
use xai_hystersis_tools::types::{
    output::{MCPOutput, MCPOutputDetails, ToolOutput},
    tool::{ToolKind, ToolNamespace},
    tool_metadata::ToolMetadata,
};
use xai_hystersis_tools::util::{ProcessGroup, ProcessScope};

/// MCP tool name delimiter: server names are qualified as `"server__tool"`.
/// Canonical definition lives in `xai_hystersis_workspace_types`; re-exported here
/// for callers that historically imported it from this module.
pub use xai_hystersis_workspace_types::MCP_TOOL_NAME_DELIMITER;

/// Reqwest 0.13 twin of the 0.12 adapters in `xai_hystersis_extra_ca`.
fn with_extra_root_certificates(mut builder: reqwest::ClientBuilder) -> reqwest::ClientBuilder {
    ...
    for der in xai_hystersis_extra_ca::extra_root_ders() {
...
pub struct McpState {
    ...
    event_writer: xai_hystersis_session_events::EventWriter,
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: xai_hystersis_session_events::EventWriter::noop(),
    ...
    pub fn set_event_writer(&mut self, writer: xai_hystersis_session_events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &xai_hystersis_session_events::EventWriter {
        &self.event_writer
    }
...
pub use xai_hystersis_telemetry::enums::McpInitStrategy;
...
impl McpError {
    ...
    pub fn error_category(&self) -> xai_hystersis_session_events::McpErrorCategory {
        use xai_hystersis_session_events::McpErrorCategory;
        match self {
...
impl xai_tool_runtime::Tool for McpErasedTool {
    ...
    async fn run(
        ...
        event_writer.emit(xai_hystersis_session_events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(xai_hystersis_session_events::Event::McpAuthRetry {
                    server_name: server.clone(),
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(xai_hystersis_session_events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(xai_hystersis_session_events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &xai_hystersis_session_events::EventWriter,
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(xai_hystersis_session_events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(xai_hystersis_session_events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
...
async fn resolve_http_oauth_prep(
    ...
    let outcome = {
        let _auth_discovery_timer =
            xai_hystersis_telemetry::instrumentation::timer("mcp_http_auth_discovery");
        ...
        .await
        {
            ...
            Err(_) => {
                ...
                ctx.event_writer
                    .emit(xai_hystersis_session_events::Event::McpOAuthDiscoveryTimeout {
    ...
    match outcome {
        ...
        OauthProbeOutcome::Inconclusive => {
            ...
            ctx.event_writer
                .emit(xai_hystersis_session_events::Event::McpOAuthProbeResolved {
                    server_name: server_name.to_string(),
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(xai_hystersis_session_events::Event::McpTransportDecodeError {
...
enum ClientState {
    ...
    Ready {
        service: McpService,
        _connected: xai_hystersis_telemetry::activity::ActivityGaugeGuard,
    },
}
...
impl McpClient {
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: xai_hystersis_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
            ...
            match result {
                Ok(service) => {
                    ...
                    *guard = ClientState::Ready {
                        ...
                        _connected: xai_hystersis_telemetry::activity::MCP_SERVERS_CONNECTED.enter(),
                    };
                    tracing::info!(
                        server = %self.server_name,
    ...
    fn make_client_info(server_name: &str, advertise_elicitation: bool) -> ClientInfo {
        ...
        ClientInfo::new(
            ...
            Implementation::new(
                ...
                xai_hystersis_version::VERSION.to_string(),
            ),
        )
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            xai_hystersis_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = xai_hystersis_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = xai_hystersis_config::hystersis_home().join("logs").join("mcp");
...
static DEFAULT_USER_AGENT: LazyLock<reqwest::header::HeaderValue> = LazyLock::new(|| {
    format!("grok-cli/{}", xai_hystersis_version::VERSION)
        .parse()
...
pub struct McpSpawnCtx<'a> {
    ...
    pub(crate) event_writer: &'a xai_hystersis_session_events::EventWriter,
...
impl<'a> McpSpawnCtx<'a> {
    pub fn for_session(
        session_id: &'a str,
        event_writer: &'a xai_hystersis_session_events::EventWriter,
        mode: OauthInteractivity,
    ...
    pub fn session_less(event_writer: &'a xai_hystersis_session_events::EventWriter) -> Self {
        Self {
            session_id: None,
            event_writer,
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = xai_hystersis_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = xai_hystersis_telemetry::instrumentation::timer("mcp_stdio_spawn");
            ...
            .map_err(|e| {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::McpServerFailed {
                        server_name: name.clone(),
                        error_type: xai_hystersis_telemetry::events::McpErrorType::SpawnFailed,
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = xai_hystersis_telemetry::instrumentation::timer("mcp_start_servers");
...
impl ClientHandler for HystersisClientHandler {
    ...
    async fn on_url_elicitation_notification_complete(
        ...
        if !xai_hystersis_tools::mcp_elicitation::chars_within(
            &params.elicitation_id,
            xai_hystersis_tools::mcp_elicitation::MAX_ELICIT_ID_CHARS,
        ) {
            tracing::warn!(
                server = %self.server_name,
