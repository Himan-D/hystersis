...
//!
//! Credentials are stored in `$HYSTERSIS_HOME/mcp_credentials.json`, keyed by a
...
///
/// Local helper (not shell-base): `xai-hystersis-mcp` sits below `config-types` in the
...
/// File name for the credential store inside `$HYSTERSIS_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$HYSTERSIS_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$HYSTERSIS_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user hystersis home (set $HYSTERSIS_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$HYSTERSIS_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(xai_hystersis_config::user_hystersis_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$HYSTERSIS_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("hystersis-mcp-credentials-test");
