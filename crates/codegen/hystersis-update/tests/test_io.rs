...
use hystersis_update::write_version_cache;
...
async fn get_installed_version_uses_env_var_override() {
    ...
    let v = hystersis_update::version::get_installed_hystersis_version();
    assert_eq!(v, "9.9.9");
    unsafe {
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        hystersis_update::version::get_installed_hystersis_version()
    };

    unsafe {
        std::env::set_var("HYSTERSIS_TEST_VERSION", "0.0.0-test");
    }
    let overridden = hystersis_update::version::get_installed_hystersis_version();
