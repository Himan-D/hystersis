...
use hystersis_shell::util::config;
use hystersis_shell::util::hystersis_home::{hystersis_application, hystersis_home};
pub use hystersis_telemetry::events::CliUpdateTrigger;
use hystersis_telemetry::events::{
...
async fn run_update_subcommand(
    ...
    // delay an update.
    if let Some(mode) = hystersis_telemetry::client::current_mode() {
    ...
    match run_mode {
        ...
        UpdateRunMode::NonBlocking => {
            ...
            hystersis_tools::util::detach_command(&mut cmd);
...
pub async fn run_install_script(
    ...
    hystersis_telemetry::session_ctx::log_event(CliUpdate {
        outcome,
        trigger,
        from_version,
        to_version,
...
fn download_client() -> reqwest::Result<reqwest::Client> {
    hystersis_extra_ca::build_reqwest_client(|builder| builder.timeout(DOWNLOAD_REQUEST_TIMEOUT))
}
...
async fn gh_release_download(tag: &str, pattern: &str, dest: &std::path::Path) -> Result<()> {
    ...
    .stderr(Stdio::piped());
    hystersis_tools::util::detach_command(&mut cmd);
    cmd.envs(hystersis_tools::util::pager_env());
...
fn warn_if_other_hystersis_processes_running() {
    ...
    cmd.args(["-f", "hystersis"])
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::null());
    hystersis_tools::util::detach_std_command(&mut cmd);
...
async fn refresh_deployment_config() {
    if !hystersis_shell::managed_config::has_principal() {
        return;
    }
    if !hystersis_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    hystersis_shell::managed_config::clear_orphan();
    if !hystersis_shell::config::is_managed_config_stale_for(
        &hystersis_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
    match hystersis_shell::managed_config::sync().await {
