...
use hystersis_shell::env::HystersisBuildEnvironment;
use hystersis_shell::util::hystersis_home::hystersis_home;

const TTL_SECONDS_BEFORE_AUTO_UPDATE: Duration = Duration::from_secs(60 * 30);
const NPM_PACKAGE: &str = "@xai-official/hystersis";
pub const GH_RELEASE_REPO: &str = "xai-org-shared/hystersis";

/// Primary CLI base URL: Cloudflare-fronted hystersis.com endpoint with edge caching
/// for binaries and origin-respecting no-cache for channel pointers.
pub(crate) const CLI_BASE_URL_PRIMARY: &str = "https://hystersis.com/cli";
...
pub(crate) const CLI_BASE_URL_FALLBACK: &str =
    "https://storage.googleapis.com/hystersis-public-artifacts/cli";
...
/// [`CLI_BASE_URLS`] with a test seam: `HYSTERSIS_CLI_BASE_URL` points fetches
/// and downloads at one base (env-seam family of `HYSTERSIS_INSTALLER`).
...
pub(crate) fn cli_base_urls() -> Vec<String> {
    if let Ok(base) = std::env::var("HYSTERSIS_CLI_BASE_URL") {
        let base = base.trim();
        if is_loopback_base(base) {
            return vec![base.to_owned()];
        }
        if !base.is_empty() {
            tracing::warn!("HYSTERSIS_CLI_BASE_URL ignored: only loopback bases are honored");
        }
    }
...
///
/// Constructed once from `HystersisBuildEnvironment` at startup and threaded through the
/// update call chain so that `auto_update` and `version` never need to know
/// about the `HystersisBuildEnvironment` enum directly.
#[derive(Debug, Clone)]
pub struct UpdateConfig {
    /// Chat API proxy base URL (versioned `https://cli-chat-proxy.hystersis.com/v1` endpoint).
    pub proxy_base_url: String,
    /// Auth scope key for `~/.hystersis/auth.json`.
    pub auth_scope: String,
    /// Enterprise deployment key (HYSTERSIS_DEPLOYMENT_KEY).
...
impl UpdateConfig {
    pub fn from_environment(env: &HystersisBuildEnvironment) -> Self {
        Self {
            proxy_base_url: env.cli_chat_proxy_base_url(),
            auth_scope: hystersis_shell::auth::HystersisComConfig::default().auth_scope(),
            deployment_key: None,
            alpha_test_key: None,
...
async fn fetch_gcs_channel_pointer(channel: &str, base_url: &str) -> Result<String> {
    ...
    let client = hystersis_extra_ca::build_reqwest_client(|builder| {
...
pub async fn write_version_cache(version: &str, stable_version: Option<&str>) {
    let version_path = hystersis_home().join("version.json");
    let now = time::OffsetDateTime::now_utc();
    let json = HystersisVersion::new(
        version.to_string(),
...
pub async fn is_version_cache_fresh() -> bool {
    ...
    if let Ok(version_str) = fs::read_to_string(&version_path).await
        && let Ok(version) = serde_json::from_str::<HystersisVersion>(&version_str)
...
pub use hystersis_version::installed as get_installed_hystersis_version;

/// Version of the managed hystersis binary currently on disk, read from the
/// `~/.hystersis/bin/hystersis` symlink target (`../downloads/hystersis-<version>-<platform>`)
/// without exec'ing anything.
///
/// Concurrent updaters (TUI background download, leader hourly checker,
/// explicit `hystersis update`) decide staleness from this instead of their own
...
/// link whose target binary was deleted (e.g. manual `~/.hystersis/downloads`
...
pub fn installed_on_disk_version() -> Option<String> {
    #[cfg(unix)]
    {
        let app = hystersis_shell::util::hystersis_home::hystersis_application();
        ...
        version_from_versioned_binary_name(target.file_name()?.to_str()?, "hystersis")
    }
    #[cfg(not(unix))]
    {
        None
...
///
/// Handles the internal layout (`hystersis-0.1.150-macos-aarch64`, including
/// pre-releases: `hystersis-0.1.150-alpha.1-linux-x86_64` → `0.1.150-alpha.1`)
/// and the npm layout without a platform suffix (`hystersis-0.1.150`,
/// `hystersis-0.1.150-alpha.1`): everything between the `{bin_prefix}-` prefix
/// and the first platform-OS component is the version, validated as semver
/// so unknown layouts (`hystersis-latest`, `hystersis-pager-*` when `bin_prefix` is
/// `hystersis`) return `None` instead of garbage.
///
...
/// Read the cached stable version from `~/.hystersis/version.json` (sync, for display).
///
...
pub fn cached_stable_version() -> Option<String> {
    ...
    let gv: HystersisVersion = serde_json::from_str(&content).ok()?;
    gv.stable_version
}
...
pub fn channel_name() -> Option<&'static str> {
    ...
    *NAME.get_or_init(|| {
        ...
        derive_channel(hystersis_version::VERSION, &stable)
    })
}
...
/// `~/.hystersis/version.json` (written by the auto-updater):
...
pub fn channel_label() -> &'static str {
    ...
    LABEL.get_or_init(|| {
        ...
        match derive_channel(hystersis_version::VERSION, &stable) {
            Some("alpha") => " [alpha]",
            Some(_) => " [stable]",
            None => "",
        }
...
mod tests {
    ...
    fn loopback_base_rejects_userinfo_and_non_loopback() {
        ...
        assert!(!is_loopback_base("https://hystersis.com/cli"));
    ...
    fn test_is_fresh_rejects_future_timestamp() {
        ...
        let v = HystersisVersion::new("0.1.200".to_string(), None, future);
        assert!(
    ...
    /// symlink-target file name (`hystersis-<version>-<platform>`).
    #[test]
    fn test_version_from_versioned_binary_name() {
        let cases: &[(&str, Option<&str>)] = &[
            ("hystersis-0.2.46-darwin-arm64", Some("0.2.46")),
            ("hystersis-0.1.220-linux-x86_64", Some("0.1.220")),
            ("hystersis-0.2.5-windows-x86_64.exe", Some("0.2.5")),
            ...
            ("hystersis-0.1.220-alpha.4-linux-x86_64", Some("0.1.220-alpha.4")),
            ("hystersis-0.1.220-alpha.4", Some("0.1.220-alpha.4")), // npm layout
            ("hystersis-pager-0.1.5-darwin-arm64", None),           // "pager" is not a version
            ("hystersis-garbage-darwin-arm64", None),               // unparseable version
            ("hystersis-0.2.46", Some("0.2.46")),                   // no platform suffix
            ("other-0.2.46-darwin-arm64", None),               // wrong prefix
            ("hystersis-latest", None),                             // symlink alias, not a version
            ("hystersis", None),                                    // bare name
            ("", None),
        ];
        for (name, expected) in cases {
            assert_eq!(
                version_from_versioned_binary_name(name, "hystersis").as_deref(),
                *expected,
        ...
        // prefix but not under "hystersis".
        assert_eq!(
            version_from_versioned_binary_name("hystersis-pager-0.1.5-darwin-arm64", "hystersis-pager")
                .as_deref(),
            Some("0.1.5")
        );
    }
    ...
    // ──────────────────────────────────────────────────────────────────────
    // HystersisVersion JSON shape — backward compatibility invariants
    // ──────────────────────────────────────────────────────────────────────
    ...
    fn test_version_json_backward_compat() {
        ...
        let v: HystersisVersion = serde_json::from_str(old).unwrap();
        assert_eq!(v.version, "0.1.180");
        assert!(v.stable_version.is_none());

        // New format with stable_version round-trips correctly.
        let now = time::OffsetDateTime::now_utc();
        let new = HystersisVersion::new("0.2.5".to_string(), Some("0.2.3".to_string()), now);
        let json = serde_json::to_string(&new).unwrap();
        let parsed: HystersisVersion = serde_json::from_str(&json).unwrap();
        ...
        assert!(serde_json::from_str::<HystersisVersion>(future).is_ok());

        // Missing required field (checked_at) is rejected.
        let missing = r#"{"version":"0.1.180"}"#;
        assert!(serde_json::from_str::<HystersisVersion>(missing).is_err());
    }
    ...
    fn test_is_fresh_ttl_boundaries() {
        let now = time::OffsetDateTime::now_utc();
        let v = HystersisVersion::new("0.1.200".to_string(), None, now);
    ...
    fn test_update_config_default_channel_is_stable() {
        use hystersis_shell::env::HystersisBuildEnvironment;
        let cfg = UpdateConfig::from_environment(&HystersisBuildEnvironment::Production);
