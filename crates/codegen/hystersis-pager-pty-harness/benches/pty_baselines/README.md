...
```bash
cargo run -p xai-hystersis-pager --release --bin pty-bench -- \
  --all \
  --write-baseline crates/codegen/xai-hystersis-pager-pty-harness/benches/pty_baselines/<platform>.json
```
