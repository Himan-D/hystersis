...
pub struct ExpectedProfile {
    ...
    pub invert: bool,
    /// Speed multiplier (NOT the 1-100 setting): `HYSTERSIS_SCROLL_SPEED=100`
...
pub struct MatrixCell {
    ...
    /// appends `HYSTERSIS_SCROLL_LOG`; the harness's env strips guarantee the
...
const SPEED100: (&str, &str) = ("HYSTERSIS_SCROLL_SPEED", "100");
const MODE_WHEEL: (&str, &str) = ("HYSTERSIS_SCROLL_MODE", "wheel");
const MODE_TRACKPAD: (&str, &str) = ("HYSTERSIS_SCROLL_MODE", "trackpad");
const LINES1: (&str, &str) = ("HYSTERSIS_SCROLL_LINES", "1");
const INVERT: (&str, &str) = ("HYSTERSIS_INVERT_SCROLL", "1");
...
mod tests {
    ...
    /// pager would (`from_terminal_context` + the `HYSTERSIS_SCROLL_*` env
    ...
    fn derive_expected(env: &'static [(&'static str, &'static str)]) -> ExpectedProfile {
        ...
        if let Some(lines) = get("HYSTERSIS_SCROLL_LINES") {
        ...
        let speed = match get("HYSTERSIS_SCROLL_SPEED") {
            None | Some("50") => 1.0,
            Some("100") => 6.0,
            Some(other) => panic!("unmapped HYSTERSIS_SCROLL_SPEED {other:?}"),
        };
        ExpectedProfile {
            mode: get("HYSTERSIS_SCROLL_MODE").unwrap_or("auto"),
            ept,
            wheel_lpt,
            trackpad_lpt,
            invert: get("HYSTERSIS_INVERT_SCROLL") == Some("1"),
            speed,
        }
    }
