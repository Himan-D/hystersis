...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build xai-hystersis-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build xai-hystersis-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_xai-hystersis-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p xai-hystersis-pager-bin` (the composition-
///    root package that owns the `xai-hystersis-pager` binary)
