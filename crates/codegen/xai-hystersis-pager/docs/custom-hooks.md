...
Hooks let you run custom scripts or HTTP requests at key moments during a Hystersis session — for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.hystersis/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.hystersis/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.hystersis/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match Hystersis's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
Hystersis injects the following variables into every hook process:

- `HYSTERSIS_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `HYSTERSIS_HOOK_NAME` — the full configured name of this hook
- `HYSTERSIS_SESSION_ID` — the current session identifier
- `HYSTERSIS_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `HYSTERSIS_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `HYSTERSIS_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `HYSTERSIS_PLUGIN_ROOT` and `HYSTERSIS_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`HYSTERSIS_PLUGIN_ROOT` and `HYSTERSIS_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/hystersis-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env Hystersis itself sees).
...
`HYSTERSIS_WORKSPACE_ROOT`, `HYSTERSIS_HOOK_EVENT`, `HYSTERSIS_HOOK_NAME`,
`HYSTERSIS_SESSION_ID`) are not taken from the Hystersis process environment at
...
`${HYSTERSIS_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.hystersis/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.hystersis/...`) run with your user permissions — treat them like shell scripts.
...
- **`The argument '/.claude/hooks/….ps1' to the -File parameter does not exist`?** → PowerShell treated `$CLAUDE_PROJECT_DIR` as empty. Hystersis rewrites it to `$env:CLAUDE_PROJECT_DIR` unless `HYSTERSIS_SHELL=cmd`.
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.hystersis/logs`).

## More Examples

See the built-in examples in the `xai-hystersis-hooks` crate:

- [Safe Shell Guard](../../../xai-hystersis-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../../xai-hystersis-hooks/examples/hooks/no-recursive-grep.json) — hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../../xai-hystersis-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../../xai-hystersis-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.hystersis/hooks/` and customize.
