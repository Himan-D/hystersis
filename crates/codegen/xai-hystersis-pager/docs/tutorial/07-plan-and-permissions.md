...
Hystersis asks before doing anything risky — and can plan before it codes.

## Permissions

When Hystersis wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: Hystersis explores the
...
`s` requests changes — Hystersis iterates until you're happy, then implements.
...
background — Hystersis keeps working and you're notified when it finishes
