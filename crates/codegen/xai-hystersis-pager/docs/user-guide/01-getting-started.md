...
Hystersis is a terminal-based AI coding assistant from SpaceXAI. It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
```powershell
irm https://hystersis.com/cli/install.ps1 | iex
```

Install a specific version:

```powershell
$env:HYSTERSIS_VERSION="0.1.42"; irm https://hystersis.com/cli/install.ps1 | iex
```

The PowerShell installer automatically adds `%USERPROFILE%\.hystersis\bin` to your User PATH. Alternatively, install via [Git for Windows](https://gitforwindows.org/) (Git Bash) or MSYS2 using the bash script above. WSL users get the Linux binary automatically.
...
```bash
hystersis clone <url> [dir]
```

The default is a depth-1 checkout of the selected branch. Pass `--full-history`
for a complete clone. See [hystersis clone](27-hystersis-clone.md).

---

## First Launch

Start Hystersis by running:

```bash
hystersis
```

On first launch, Hystersis opens your browser to authenticate with hystersis.com. After you sign in, Hystersis stores your credentials in `~/.hystersis/auth.json`, where they persist across sessions. Hystersis refreshes your credentials automatically and prompts you to sign in again when they can no longer be renewed.
...
Once authenticated, Hystersis presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, Hystersis's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. Hystersis reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, Hystersis asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `hystersis --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.hystersis/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `hystersis -c`
...
- **Agent messages** -- Hystersis's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- Hystersis's reasoning process (collapsible)
...
Hystersis has built-in tools for:
...
hystersis "fix the failing auth test and run it"

# Initial prompt in a new git worktree. Use --worktree=<name> (with `=`) so the
# prompt isn't swallowed as the worktree name — `hystersis -w "refactor module X"`
# would treat "refactor module X" as the worktree label, not the prompt.
hystersis --worktree=feat "refactor module X"

# Base the worktree on a specific branch (e.g. main) instead of the current HEAD:
hystersis -w --ref main "implement feature from main"
...
hystersis -m hystersis

# Resume a previous session
hystersis --resume <session-id>

# Continue the most recent session
hystersis -c

# Experimental scrollback-native render mode. Sticky: plain `hystersis` reopens in
...
Run Hystersis non-interactively for scripting, CI/CD, and automation:
...
```bash
hystersis -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. Hystersis reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.hystersis/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. Hystersis also reads `CLAUDE.md` files for compatibility.
