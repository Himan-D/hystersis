...
Hystersis runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in Hystersis to check the current session and see available fixes. If
Hystersis cannot start, run `hystersis doctor` in your shell. Use `hystersis doctor --json`
...
`hystersis doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `hystersis doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. Hystersis preserves the file's
...
Hystersis deliberately does **not** run `tmux source-file` or change the live tmux
...
Hystersis detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach Hystersis.
...
Inside tmux there are two separate questions: what color Hystersis emits, and what
...
Hystersis writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when Hystersis runs inside tmux.
...
terminal focused. Older compositors may require Hystersis to remain focused until
the copy message appears. Hystersis shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`HYSTERSIS_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

Hystersis emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `HYSTERSIS_CLIPBOARD_NO_OSC52=1` before starting Hystersis to
...
A remote Hystersis process normally cannot read the local terminal's selection. Use
...
When Hystersis cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.hystersis/last-copy.txt` by
default; override with `HYSTERSIS_COPY_FILE`); the toast names that path when delivery
...
through `hystersis wrap`, for example `hystersis wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `hystersis wrap`, Hystersis shows the one-time tip
...
`[ui.contextual_hints]` in `$HYSTERSIS_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `hystersis doctor fix ssh-wrap`. It also shows
...
> **Warning**: `hystersis wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. Hystersis normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.hystersis/pager.toml`, or run
`hystersis --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with Hystersis

Zellij can intercept Ctrl/Alt keys before they reach Hystersis. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach Hystersis, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in Hystersis.
...
keys. Hystersis therefore does not negotiate the protocol there, and Shift+Enter can
...
If Hystersis stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, Hystersis stops capture and shows
...
to the terminal hosting Hystersis. Open **System Settings → Privacy & Security →
...
Run `hystersis doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone Hystersis would use. If no input device is available, Doctor
...
problem, set `HYSTERSIS_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
...
terminals, Terminal.app, Konsole, mlterm, and others). Hystersis therefore
**does not** reorder RTL by default.

If Arabic or Persian in **scrollback** (or list content) reads backwards,
enable app-side reordering in `~/.hystersis/pager.toml` (or project config):
