...
> **Status: alpha.** The schema below is versioned (`hystersis_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

Hystersis CLI can export usage **metrics** and **events** to your organization's
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `HYSTERSIS_TELEMETRY_ENABLED` |
| Coding data, retention, and training | Settings — `/privacy` opens the row |
| Trace upload | `[telemetry] trace_upload` / `HYSTERSIS_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `HYSTERSIS_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
```bash
export HYSTERSIS_EXTERNAL_OTEL=1                  # master switch
...
`HYSTERSIS_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `HYSTERSIS_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `HYSTERSIS_TELEMETRY_ENABLED`, which controls SpaceXAI-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `hystersis_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `HYSTERSIS_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`HYSTERSIS_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `hystersis setup` managed config /
...
(`hystersis --debug`) for `external otel:` lines — they record whether the stream
...
| `hystersis_code.schema.version` | `v1` |
...
## Metrics (meter scope `ai.xai.hystersis_code`)

| Metric | Unit | Attributes |
|---|---|---|
| `hystersis_code.session.count` | `{session}` | base attrs only |
| `hystersis_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `hystersis_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `hystersis_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `hystersis_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `hystersis_code.error.count` | `{error}` | `error_category`, `model` |
| `hystersis_code.startup.total` | `ms` | `outcome` = `ok` \| `timeout` \| `error`; `auth_mode` |
| `hystersis_code.startup.phase_duration` | `ms` | `phase`, `outcome`, `auth_mode` |
| `hystersis_code.startup.timeout` | `{timeout}` | `stuck_in`, `auth_mode` |
...
message Hystersis prints names the longest step instead, so the two can name
...
There is no `cost.usage` metric: join `hystersis_code.token.usage` with your own
...
|---|---|
| `hystersis_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `hystersis_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `hystersis_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `hystersis_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `hystersis_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `hystersis_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `hystersis_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `hystersis_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `hystersis_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `hystersis_code.permission_mode_changed` | `to_mode`, `trigger` |
| `hystersis_code.skill_activated` | `skill_source`, `trigger` = `slash_command` \| `skill_md_read` \| `skill_tool`; `skill.name` (**details**) |
| `hystersis_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `hystersis_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `hystersis_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `hystersis_code.auth` | `auth_method` |
| `hystersis_code.internal_error` | `error_type` (class only — no message, no location) |
| `hystersis_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(hystersis_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(hystersis_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(hystersis_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(hystersis_code_tool_decision_total[1h]))
```
