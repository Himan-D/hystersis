...
Hystersis reads settings from config files, environment variables, and CLI flags. This page covers the common options. The field list for `config.toml`, `managed_config.toml`, and `requirements.toml` is [26-config-reference.md](26-config-reference.md) (extracted to `~/.hystersis/docs/user-guide/` on launch).
...
2. **Environment variables** (e.g. `XAI_API_KEY`, `HYSTERSIS_MEMORY`)
3. **`requirements.toml` / MDM** (org-enforced; clamps every config layer below, including the overlay)
4. **`HYSTERSIS_CONFIG` / `HYSTERSIS_CONFIG_PATH` overlay** (above `config.toml` and managed, below `requirements.toml` / MDM)
5. **config.toml** (`~/.hystersis/config.toml`)
6. **`managed_config.toml`** (org-deployed defaults; below `config.toml`)
7. **Built-in defaults**

Within the config-file tier, the layers merge lowest-to-highest: `managed_config.toml` → `config.toml` → `HYSTERSIS_CONFIG` overlay → `requirements.toml` / MDM. So `requirements.toml` and MDM clamp **both** your `config.toml` and the overlay.

`HYSTERSIS_CONFIG` / `HYSTERSIS_CONFIG_PATH` (tier 4) are config **overlays**: a merged config layer, not direct-setting environment variables like `XAI_API_KEY` (tier 2). They set config keys (subject to the allowlist below), so read them as part of the config-file tier rather than the env-var tier.

### Injecting config with `HYSTERSIS_CONFIG`

A harness or ACP client that launches `hystersis agent stdio` can inject settings without writing a `config.toml` or relocating `$HYSTERSIS_HOME`:

- **`HYSTERSIS_CONFIG`**: an inline JSON object overlay.
- **`HYSTERSIS_CONFIG_PATH`**: an *additional* file overlay (not a replacement for `config.toml`), a JSON or TOML file read by its extension (`.json` → JSON, else TOML). `HYSTERSIS_CONFIG` wins if both are set. An empty `HYSTERSIS_CONFIG` is treated as unset, and a malformed one logs a warning and falls through to `HYSTERSIS_CONFIG_PATH`.

The overlay is **deep-merged** on top of your `config.toml` (it overrides only the keys it sets), placed above the user/managed layers but **below** `requirements.toml` / MDM so an enterprise pin still wins. A malformed blob is ignored with a warning. This mirrors `CODEX_CONFIG` from the `codex-acp` adapter (a JSON object merged into the session config); Hystersis is ACP-native, so the overlay lives in the agent itself. It only affects settings read from the merged config, and it is **not** a permission-escalation path. The overlay is confined, fail-closed, to an **allowlist** of soft settings (`models`, `features`, a narrowed `toolset`, and a `shell_environment_policy` limited to its filter fields, which select among env names the launcher already controls and cannot inject an env value into tool subprocesses); every other table is dropped at the choke point, so the overlay cannot spawn commands, set auth policy, redirect network traffic, elevate trust, or add a discovery source. Even on the allowlisted settings, a specific set of security gates read the raw disk layers rather than the overlay. The `ConfigLayers::env_overlay` rustdoc is the canonical list of what the overlay can and cannot reach and which gates read it overlay-free; see also the [internal environment-variables reference](../internal/22-environment-variables.md). Use `HYSTERSIS_DEFAULT_SELECTED_PERMISSION` for headless permission control. For example, to set the default reasoning effort:

```bash
HYSTERSIS_CONFIG='{"models": {"default_reasoning_effort": "high"}}' hystersis agent stdio
```
...
Location: `~/.hystersis/config.toml`. If the file is missing, Hystersis uses its built-in defaults, so you only need to set the values you want to override.
...
[models]
default = "hystersis-4.5"                   # model used for new sessions
web_search = "hystersis-4.5"                # model used by the web_search tool
...
# Env vars HYSTERSIS_MAX_PARALLEL_IMAGE_GEN_CALLS / HYSTERSIS_MAX_PARALLEL_VIDEO_GEN_CALLS
...
You can also flip it from the settings pane (`/settings` → **Disable vim input mode**); Hystersis writes your choice to `[ui] simple_mode`. `simple_mode` and `vim_mode` are independent — one governs the prompt editor, the other governs scrollback navigation. See [Keyboard Shortcuts](03-keyboard-shortcuts.md) for the full binding reference.
...
You can also override this with `HYSTERSIS_DEFAULT_SELECTED_PERMISSION`, which is handy for headless or agent test runs that shouldn't mutate `config.toml`. Precedence: env var → `config.toml` → `always_allow_all_sessions`.
...
Each setting also has an environment-variable override, applied on first load only (again, handy for headless / test runs): `HYSTERSIS_SCROLL_SPEED`, `HYSTERSIS_SCROLL_MODE`, `HYSTERSIS_INVERT_SCROLL` (`1`/`true`/`0`/`false`), and `HYSTERSIS_SCROLL_LINES`. Precedence: env var → `config.toml` → default. Unrecognized values fall back to the default, and out-of-range numbers clamp.
...
allowed_domains = ["docs.rs", "hystersis.com"]          # override the built-in allowlist
...
allowed_domains = ["docs.hystersis.com", "arxiv.org"]
# ...or block these domains instead (leave allowed_domains unset):
# excluded_domains = ["reddit.com", "pinterest.com"]
```

`allow_local` is off by default (SSRF fail-closed). Turn it on (or set `HYSTERSIS_WEB_FETCH_ALLOW_LOCAL=1`) and `web_fetch` may reach **explicit** loopback hosts only — private, link-local, and cloud-metadata ranges stay blocked. Resolution: TOML > env > default off.
...
MCP servers can also be set per-project in `.hystersis/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.hystersis/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.hystersis/config.toml` (current dir) > `<repo-root>/.hystersis/config.toml` > `~/.hystersis/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions. Enable memory with `HYSTERSIS_MEMORY=1`, `[memory] enabled = true`, or managed remote settings.
...
[subagents]
enabled = true
sampling_limit = 12                   # concurrent in-flight subagent sampling calls per process; defaults to max_concurrent (32) when unset (HYSTERSIS_SUBAGENT_SAMPLING_LIMIT)
...
[subagents.models]
explore = "hystersis"               # route to different models
```
...
Background workflows — the `workflow` tool, named `.hystersis/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default**. Disable with config, env, or remote settings.

```toml
[workflows]
enabled = false                       # disable background workflows (or HYSTERSIS_WORKFLOWS=0)
```

Project workflows are discovered from `<repo-root>/.hystersis/workflows/`; user workflows from `~/.hystersis/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`hystersis inspect` reports cells that still need session-start resolution as `?` until a value is available; cells with an explicit env or TOML value use that value. Affected discovery entries report `compatibilityStatus: "unresolved"` in JSON and `[compat unresolved]` in human output.
...
`[hints]` holds small persisted UI preferences: remembered answers and modal layout. Hystersis writes these for you as you use the TUI, but you can edit or delete them by hand; removing a key restores the default.

`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** these to your user `~/.hystersis/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "hystersis"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `hystersis`. |
...
| Hystersis Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

With `method = "auto"`, Hystersis detects the terminal brand and picks the best protocol. Set `method` explicitly to override that.

#### Notification hooks

Run your own commands when events fire. Hooks receive `$HYSTERSIS_EVENT`, `$HYSTERSIS_MESSAGE`, and `$HYSTERSIS_SESSION_ID` in the environment.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Hystersis' -message '$HYSTERSIS_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$HYSTERSIS_MESSAGE' ntfy.sh/my-hystersis-alerts"
...
- **`[features] telemetry`** / `HYSTERSIS_TELEMETRY_ENABLED` — the product-analytics master switch. `/privacy` doesn't change it.
- **Coding data, retention, and training** — the Settings row `/privacy` opens; coding-data sharing, separate from telemetry.
- **`[telemetry] trace_upload`** / `HYSTERSIS_TELEMETRY_TRACE_UPLOAD` — session traces; follows telemetry when unset.
- **`[telemetry] otel_*`** / `HYSTERSIS_EXTERNAL_OTEL` — external OTEL to your own collector (below).
...
Set these only to point telemetry at your own infrastructure or to switch parts off. The built-in endpoint and credentials are managed by Hystersis — leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= HYSTERSIS_EXTERNAL_OTEL)
...
- `minimum_version` (`HYSTERSIS_MINIMUM_VERSION`) is a soft anti-downgrade floor. The
  updater skips a target below it and keeps the current version. It never blocks
  startup.
- `maximum_version` (`HYSTERSIS_MAXIMUM_VERSION`) is a soft ceiling. The updater caps
  its target at it and never installs above it.
- `required_minimum_version` (`HYSTERSIS_REQUIRED_MINIMUM_VERSION`) and
  `required_maximum_version` (`HYSTERSIS_REQUIRED_MAXIMUM_VERSION`) are hard bounds. If
  the running version is outside the range, the CLI exits at startup and instructs
  the user to install an approved version. `hystersis update` and `hystersis --version` keep
...
- An explicit `hystersis update --version X` is allowed above the ceiling, to recover
...
Location: `~/.hystersis/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
|----------|-------------|
| `XAI_API_KEY` | API key from hystersis.com |
| `HYSTERSIS_AUTH_PROVIDER_COMMAND` | External auth binary path |
| `HYSTERSIS_AUTH_PROVIDER_LABEL` | Display name on TUI login screen |
| `HYSTERSIS_AUTH_TOKEN_TTL` | Token lifetime in seconds |
| `HYSTERSIS_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `HYSTERSIS_OIDC_ISSUER` | OIDC issuer URL |
| `HYSTERSIS_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `HYSTERSIS_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `HYSTERSIS_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `HYSTERSIS_WORKFLOWS` | Enable (`1`) or disable (`0`) background workflows and select the `/goal` driver (default on: host-owned workflow driver; off: legacy `update_goal`) |
| `HYSTERSIS_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `HYSTERSIS_WEB_FETCH_ALLOW_LOCAL` | Allow `web_fetch` to explicit loopback hosts only (`localhost` / `127.0.0.0/8` / `::1`). Same as `[toolset.web_fetch] allow_local`. Default off; private/metadata stay blocked. |
| `HYSTERSIS_AGENT` | Custom agent definition path or name |
| `HYSTERSIS_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |
| `HYSTERSIS_EXIT_TIMEOUT_SECS` | Seconds after a quit is requested before the process is force-exited if teardown hangs (default: 20, `0` disables; a hard exit follows 5s later) |

### Logging

| Variable | Description |
|----------|-------------|
| `HYSTERSIS_LOG_FILE` | Write logs to this file path (used verbatim as the path) |
| `RUST_LOG` | Log level filter (e.g. `debug`); controls the `HYSTERSIS_LOG_FILE` log and headless stderr output |
...
| `HYSTERSIS_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `HYSTERSIS_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `HYSTERSIS_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload |
| `HYSTERSIS_TELEMETRY_MIXPANEL_ENABLED` | Enable/disable Mixpanel specifically |
| `HYSTERSIS_EXTERNAL_OTEL` | External OTEL to your collector (see [24-monitoring-usage.md](24-monitoring-usage.md)) |
| `HYSTERSIS_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `HYSTERSIS_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.hystersis/config.toml` | Main configuration file |
| `~/.hystersis/pager.toml` | TUI appearance configuration |
...
| `~/.hystersis/lsp.json` | LSP server configuration (user-scoped) |
| `~/.hystersis/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
| `.hystersis/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some settings can be set per-project by placing files in `.hystersis/` inside your repository:

| File | What it configures |
|------|--------------------|
| `.hystersis/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.hystersis/config.toml`) |
| `.hystersis/skills/` | Project-specific skills |
| `.hystersis/hooks/` | Project-specific lifecycle hooks |
| `.hystersis/agents/` | Project-specific agent definitions |
| `.hystersis/lsp.json` | LSP server configuration |
| `.hystersis/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.hystersis/lsp.json` | All projects |
| Project | `.hystersis/lsp.json` | Current repository |
...
1. **Project** — `.hystersis/lsp.json`
2. **User** — `~/.hystersis/lsp.json`
