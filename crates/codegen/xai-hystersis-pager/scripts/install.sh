#!/bin/bash
#
# Hystersis CLI installer — https://hystersis.com/cli/install.sh
#
# Auth: HYSTERSIS_DEPLOYMENT_KEY (takes precedence) or ~/.hystersis/auth.json from `hystersis login`.
# Env: HYSTERSIS_CHANNEL (stable|alpha|enterprise, default: stable), HYSTERSIS_BIN_DIR, HYSTERSIS_PROXY_URL
#
# Usage:
#   curl -fsSL https://hystersis.com/cli/install.sh | bash            # latest stable
#   curl -fsSL https://hystersis.com/cli/install.sh | bash -s 0.1.42  # specific version
#   HYSTERSIS_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://hystersis.com/cli/install.sh)
#
...
# Read a token from ~/.hystersis/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_hystersis_token() {
    local auth_file="$HOME/.hystersis/auth.json"
    local scope="$1"
...
# Resolve auth: HYSTERSIS_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.hystersis.com::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.hystersis.com/sign-in"
AUTH_SOURCE=""

if [ -n "$HYSTERSIS_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_hystersis_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_hystersis_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.hystersis/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.hystersis/auth.json." >&2
    fi
fi
...
BASE_URL_PRIMARY="https://hystersis.com/cli"
BASE_URL_FALLBACK="https://storage.googleapis.com/hystersis-build-public-artifacts/cli"
DOWNLOAD_DIR="$HOME/.hystersis/downloads"
BIN_DIR="${HYSTERSIS_BIN_DIR:-$HOME/.hystersis/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${HYSTERSIS_CHANNEL:-stable}"

# Pick a working BASE_URL: try Cloudflare-fronted hystersis.com first, fall back to
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Hystersis $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Hystersis $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/hystersis-$platform"
artifact_base="${BASE_URL}/hystersis-${version}-${platform}"
...
echo "  Downloading hystersis ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Hystersis is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in hystersis.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/hystersis.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded hystersis failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.hystersis/bin and ~/.hystersis/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/hystersis"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/hystersis and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.hystersis/completions/bash" "$HOME/.hystersis/completions/zsh"
"$BIN_DIR/hystersis" completions bash > "$HOME/.hystersis/completions/bash/hystersis.bash" 2>/dev/null || true
"$BIN_DIR/hystersis" completions zsh  > "$HOME/.hystersis/completions/zsh/_hystersis"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/hystersis" completions fish > "$HOME/.config/fish/completions/hystersis.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.hystersis/config.toml"
...
if [ -n "$HYSTERSIS_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${HYSTERSIS_PROXY_URL:-https://cli-chat-proxy.hystersis.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$HYSTERSIS_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.hystersis/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.hystersis/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.hystersis/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.hystersis/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Hystersis $version installed to $BIN_DIR/hystersis.exe" >&2
else
    echo "Hystersis $version installed to $BIN_DIR/hystersis" >&2
fi

# --- Ensure hystersis is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so hystersis works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/hystersis" "$candidate/hystersis"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/hystersis -> $BIN_DIR/hystersis" >&2
...
# Also update shell config so ~/.hystersis/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> hystersis installer >>>
fish_add_path $HOME/.hystersis/bin
# <<< hystersis installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> hystersis installer >>>
export PATH="$HOME/.hystersis/bin:$PATH"
fpath=(~/.hystersis/completions/zsh $fpath)
...
# <<< hystersis installer <<<'
    ...
    if grep -qs "hystersis installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> hystersis installer >>>/ { skip=1; next }
            /# <<< hystersis installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'hystersis' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'hystersis' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'hystersis' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.hystersis/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use hystersis from cmd.exe or PowerShell, add %USERPROFILE%\\.hystersis\\bin to your PATH." >&2
fi
