...
fn hystersis_home_override_path_helpers() {
    ...
    assert_eq!(
        xai_hystersis_pager::util::pager_toml_path(),
        hystersis_home.join("pager.toml")
    );
    assert_eq!(
        xai_hystersis_pager::util::display_hystersis_home_prefix(),
        "$HYSTERSIS_HOME"
    );
    assert_eq!(
        xai_hystersis_pager::util::display_user_hystersis_path("config.toml"),
        "$HYSTERSIS_HOME/config.toml"
    );

    let memory_path = hystersis_home.join("memory/MEMORY.md");
    assert_eq!(
        xai_hystersis_pager::util::abbreviate_path(&memory_path.display().to_string()),
    ...
    assert_eq!(
        xai_hystersis_pager::clipboard::display_copy_path(&hystersis_home.join("last-copy.txt")),
        "$HYSTERSIS_HOME/last-copy.txt"
    );

    assert!(xai_hystersis_pager::util::is_under_user_hystersis_home(&memory_path));
    assert!(!xai_hystersis_pager::util::is_under_user_hystersis_home(
...
fn disk_usage_run_creates_no_hystersis_home() {
    ...
    for json in [false, true] {
        xai_hystersis_pager::disk_usage_cmd::run(xai_hystersis_pager::disk_usage_cmd::DiskUsageArgs { json })
