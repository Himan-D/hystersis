...
/// argv shape is the part `hystersis wrap` owns. The quoting round-trip against a
...
fn wrap_not_found_alias_routes_via_shell_contract() {
    ...
    let (code, raw) = run_wrap(
        &["hystersis-wrap-e2e-alias-xx", "with space"],
        &[("SHELL", &shell)],
    );
    ...
    assert!(
        raw.contains("ARG:hystersis-wrap-e2e-alias-xx 'with space'"),
