...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("hystersis-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.hystersis/config.toml` with a per-model reasoning-effort menu.
    let hystersis_home = content.home().join(".hystersis");
    std::fs::create_dir_all(&hystersis_home).expect("create .hystersis");
    // Quote the dotted model id: bare `[model.hystersis-4.5]` is TOML key-path syntax (model.hystersis-4.5), not the id "hystersis-4.5".
    std::fs::write(
