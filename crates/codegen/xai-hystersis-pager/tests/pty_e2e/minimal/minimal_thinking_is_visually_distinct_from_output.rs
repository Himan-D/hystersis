...
async fn run_reasoning_turn(collapse_thinking: bool) -> Turn {
    ...
    std::fs::create_dir_all(content.home().join(".hystersis")).expect("mk .hystersis");
    std::fs::write(
        content.home().join(".hystersis/config.toml"),
        "[ui]\nshow_thinking_blocks = true\n",
    )
    .expect("write config");
    if collapse_thinking {
        let hystersis_home = content.sandbox().hystersis_home().to_path_buf();
        std::fs::create_dir_all(&hystersis_home).expect("mk hystersis home");
        std::fs::write(
