...
/// backend, often with the hystersis.com proxy blocked. A TCP listener that accepts
...
async fn embedded_mode_boots_without_hanging_on_blocked_backend() {
    ...
    let env = [
        ...
        ("HYSTERSIS_HOME", hystersis_home.to_str().unwrap()),
        ("XAI_API_KEY", "test-key-for-ci"),
        ("HYSTERSIS_CLI_CHAT_PROXY_BASE_URL", base.as_str()),
        ("HYSTERSIS_XAI_API_BASE_URL", base.as_str()),
