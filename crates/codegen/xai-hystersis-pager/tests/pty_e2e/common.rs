...
pub(crate) use xai_hystersis_pager_pty_harness::{
...
/// `xai_hystersis_telemetry::unified_log::path()` resolves the calling process's
/// own hystersis home and the file-name const is private, so this composes the
/// sandbox hystersis home with the exported `LOG_DIR`.
pub(crate) fn unified_log_path(content: &ContentController) -> PathBuf {
    content
        .sandbox()
        .hystersis_home()
        .join(xai_hystersis_telemetry::unified_log::LOG_DIR)
        .join("unified.jsonl")
}
...
pub(crate) const CONTEXTUAL_HINTS_ENV: &[(&str, &str)] = &[("HYSTERSIS_CONTEXTUAL_HINTS", "1")];
...
// `xai_hystersis_pager_pty_harness::flows` (re-exported above).
...
/// would never fetch `/v1/settings`. Spawns WITHOUT `HYSTERSIS_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    overrides.push(EnvOp::set("HYSTERSIS_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// Explicit overrides for a folder-trust run. A self-built hystersis auto-trusts,
/// so `HYSTERSIS_TEST_VERSION` simulates a release; the gate is pinned both ways.
pub(crate) fn trust_env(feature_on: bool) -> [(&'static str, &'static str); 2] {
    [
        ("HYSTERSIS_TEST_VERSION", "0.0.0-sim"),
        ("HYSTERSIS_FOLDER_TRUST", if feature_on { "1" } else { "0" }),
    ]
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".hystersis")
        .join(xai_hystersis_workspace::trust::TRUST_FILE_NAME);
    let store = xai_hystersis_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&xai_hystersis_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `xai_hystersis_pager_pty_harness::flows`
// (re-exported above).
...
/// Budget for session creation plus the `hystersis.com/mcp/list` round-trip.
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    let hystersis_home = content.home().join(".hystersis");
    std::fs::create_dir_all(&hystersis_home).expect("create fake HYSTERSIS_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(hystersis_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in xai-hystersis-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.hystersis/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{HYSTERSIS_HOME|HOME}/.hystersis/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let hystersis_home = content.home().join(".hystersis");
    std::fs::create_dir_all(&hystersis_home).expect("create .hystersis");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let hystersis_home = content.home().join(".hystersis");
    std::fs::create_dir_all(&hystersis_home).expect("create .hystersis");
    // Minimal opt-in only — matches load_config's `{HYSTERSIS_HOME|HOME}/.hystersis/config.toml`.
...
pub(crate) fn seed_keep_text_selection_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&hystersis_home).expect("create .hystersis");
    std::fs::write(
        hystersis_home.join("config.toml"),
...
pub(crate) fn spawn_mouse_toggle_pager(content: &ContentController) -> PtyHarness {
    ...
    PtyHarness::spawn_with_content_env_ops(
        ...
        content,
        &[],
        &[EnvOp::set("HYSTERSIS_MOUSE_REPORTING_TOGGLE", "true")],
    )
    .expect("spawn pager")
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "HYSTERSIS_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = xai_hystersis_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
/// Locate `<hystersis_home>/sessions/<encoded cwd>/<session id>/`, where the shell
...
pub(crate) fn session_dir(content: &ContentController, harness: &mut PtyHarness) -> PathBuf {
    let sessions = content.home().join(".hystersis").join("sessions");
    for _ in 0..100 {
...
/// `hystersis wrap` run budget. Same contention math as the requirements-version
...
/// Run `hystersis wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `HYSTERSIS_HOME`, returning the exit code (`None` only while still running at
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("HYSTERSIS_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness =
        PtyHarness::new_inherited_env(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env, None)
            .expect("spawn hystersis wrap");
    ...
    let code = match wait_for_exit_status(&mut harness, WRAP_TIMEOUT) {
        ...
        Ok(PtyExitPoll::Running) => {
            harness.quit().expect("kill hystersis wrap after timeout");
            None
        }
        Ok(PtyExitPoll::PendingStatus) => {
            panic!("hystersis wrap exited but portable status remained unavailable for {WRAP_TIMEOUT:?}")
        }
        Err(error) => panic!("poll hystersis wrap exit: {error:#}"),
    };
...
/// `hystersis wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$HYSTERSIS_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("HYSTERSIS_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
/// `$HYSTERSIS_PTY_CAST_DIR/<file_stem>.{txt,html}` when the env var is set.
...
pub(crate) use xai_hystersis_pager_pty_harness::host_clipboard::{
...
pub(crate) use xai_hystersis_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
