...
/// turn and the turn-end `hystersis.com/suggestPrompt` call (the mock's fixed mode
...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`hystersis-0.1`, the shell's built-in default)
    ...
    let overrides = [("HYSTERSIS_PROMPT_SUGGESTIONS".to_owned(), "true".to_owned())];
