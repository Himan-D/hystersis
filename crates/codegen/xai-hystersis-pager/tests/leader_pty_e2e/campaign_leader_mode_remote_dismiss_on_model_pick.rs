...
#[ignore = "PTY e2e; run with cargo test -p xai-hystersis-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared HYSTERSIS_HOME so every spawn elects/attaches to the
    ...
    let state_path = hystersis_home.join("campaigns_state.json");
    ...
    let config = std::fs::read_to_string(hystersis_home.join("config.toml")).expect("read config.toml");
    assert!(
