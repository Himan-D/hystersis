...
//!
//! These tests exercise the real `xai-hystersis-pager` binary through a PTY using
...
/// enterprise deploy report: with `HYSTERSIS_GOAL=1`, `/goal` must
...
/// explicitly off (`HYSTERSIS_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `HYSTERSIS_FOLDER_TRUST=1` and a git repo that
...
/// also pins `HYSTERSIS_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
