...
//! the generic [`CodeBlockSpan`](xai_hystersis_markdown::CodeBlockSpan) API) and
...
use xai_hystersis_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](xai_hystersis_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](xai_hystersis_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a xai_hystersis_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `HystersisDay` is the only light theme; every other concrete theme (and the
/// `HystersisNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `xai_hystersis_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::HystersisDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_hystersisday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::HystersisDay),
            "HystersisDay is the light theme"
        );
        for dark in [
            ThemeKind::HystersisNight,
