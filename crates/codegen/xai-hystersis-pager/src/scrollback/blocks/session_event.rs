    ...
    pub fn message(&self) -> String {
        match self {
            ...
            SessionEvent::DiskFull => {
                xai_hystersis_shell::extensions::notification::DISK_FULL_USER_MESSAGE.to_string()
            }
...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "hystersis-4.5".into(),
            new_model_id: "hystersis".into(),
            reason: "Model \"hystersis-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"hystersis-4.5\" is no longer available. Switched to \"hystersis\"."
        );
    }
