...
/// firehose (`HYSTERSIS_DEBUG_LOG` / `HYSTERSIS_LOG_FILE`).
pub use xai_hystersis_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `xai-hystersis-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use xai_hystersis_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "xai_hystersis_shell=info,xai_hystersis_pager=trace,xai_hystersis_tools=info,xai_hystersis_session_search=info,xai_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = xai_hystersis_telemetry::otel_layer::build_otel_layer(
        xai_hystersis_telemetry::otel_layer::OtelClientInfo {
            client_name: "hystersis-pager",
            client_version: xai_hystersis_version::VERSION,
            service_version: xai_hystersis_version::full_version(),
            app_entrypoint: "tui",
        },
        xai_hystersis_shell::auth::credential_provider::build_default_otel_layer_config(),
    );
    let instrumentation_layer = xai_hystersis_telemetry::instrumentation::layer();
    let sampling_log_layer = xai_hystersis_telemetry::sampling_log::layer();
    let hooks_log_layer = xai_hystersis_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        ...
        .with(otel_layer);
    xai_hystersis_telemetry::debug_log::install_firehose(registry, "tui");
