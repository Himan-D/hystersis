...
pub struct EnumChoice {
    /// Canonical persisted value (e.g. `"hystersisnight"`).
    pub canonical: &'static str,
    /// Display label shown in the chooser (e.g. `"Hystersis Night"`).
...
impl Default for PagerLocalSnapshot {
    fn default() -> Self {
        Self {
            ...
            voice_stt_language: xai_hystersis_voice::STT_LANGUAGE_DEFAULT.to_string(),
...
///
/// Delegates to [`xai_hystersis_voice::canonicalize_stt_language`] so the pager and
/// the STT client share one catalog (official Hystersis STT languages + client-only
/// `auto`). Unknown/blank/`None` → `en`.
pub fn canonical_voice_stt_language(value: Option<&str>) -> &'static str {
    xai_hystersis_voice::canonicalize_stt_language(value)
}
...
pub fn current_value_for(
    ...
    match key {
        ...
        // Live cache; `HYSTERSIS_PROMPT_SUGGESTIONS` env overrides at the gate.
        ...
        "remember_tool_approvals" => Some(SettingValue::Bool(
            ui.remember_tool_approvals
                .unwrap_or(xai_hystersis_shell::util::config::DEFAULT_REMEMBER_TOOL_APPROVALS),
        )),
        ...
        "fork_secondary_model" => Some(SettingValue::String({
            let baseline = xai_hystersis_shell::models::default_model();
...
mod tests {
        ...
        for meta in reg.all() {
            ...
            match (meta.key, &meta.kind) {
                ...
                ("remember_tool_approvals", SettingKind::Bool { default }) => {
                    assert_eq!(
                        *default,
                        xai_hystersis_shell::util::config::DEFAULT_REMEMBER_TOOL_APPROVALS,
                        "remember_tool_approvals default drifts from the shared \
                         resolver const in xai-hystersis-shell"
                    );
                }
                    ...
                    assert_eq!(
                        *default,
                        ...
                        "toolset.ask_user_question.timeout_enabled default drifts from the \
                         shared resolver const in xai-hystersis-tools"
                    );
                }
                ...
                ("fork_secondary_model", SettingKind::DynamicEnum { default, .. }) => {
                    ...
                    assert_eq!(
                        ...
                        xai_hystersis_shell::models::default_model(),
    ...
    /// Spot-check the delegation to `xai_hystersis_voice::canonicalize_stt_language`
    ...
    fn voice_stt_language_settings_match_voice_crate_catalog() {
        ...
        for c in choices.iter() {
            ...
            let lang = xai_hystersis_voice::stt_language_by_code(c.canonical)
        ...
        let crate_codes: HashSet<&str> = xai_hystersis_voice::STT_LANGUAGES
            .iter()
            .map(|l| l.code)
            .collect();
        assert_eq!(
            setting_codes, crate_codes,
            "settings concrete languages must match xai_hystersis_voice::STT_LANGUAGES exactly"
        );
    }
    ...
    fn current_value_for_auto_dark_theme_filters_auto_value() {
        ...
        assert_eq!(
            value,
            SettingValue::Enum("hystersisnight"),
    ...
    fn current_value_for_auto_light_theme_filters_auto_value() {
        ...
        assert_eq!(
            value,
            SettingValue::Enum("hystersisday"),
    ...
    fn contextual_hints_group_and_children_registered() {
        ...
        let ui = UiConfig {
            contextual_hints: xai_hystersis_shell::agent::config::ContextualHints {
                undo: Some(false),
                ..Default::default()
            },
            ..UiConfig::default()
