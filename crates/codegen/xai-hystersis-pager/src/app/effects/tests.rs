...
use super::*;
use xai_hystersis_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use xai_hystersis_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use xai_hystersis_workspace::session::git::RestoreDegree;
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use xai_hystersis_active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            xai_hystersis_active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: xai_hystersis_foreign_sessions::EnabledForeignSessionSources::default(),
...
async fn deep_search_sessions_echoes_routing_and_policy() {
    ...
    execute(
        Effect::DeepSearchSessions {
            ...
            query: "abc".into(),
            seq: 4,
            headless_policy: xai_hystersis_shell::session::unified_list::HeadlessPolicy::Only,
        },
        &mut tasks,
        &tx,
        Path::new("."),
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use xai_hystersis_agent::config::BuiltinAgentName;
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> xai_hystersis_shell::session::SessionInfoResponse {
    use xai_hystersis_shell::session::acp_types::{ContextInfo, SessionInfoData};
    xai_hystersis_shell::session::SessionInfoResponse {
...
async fn lookup_session_title_loads_single_summary_by_cwd() {
    ...
    let dir = root
        .path()
        .join("sessions")
        .join(xai_hystersis_shell::util::hystersis_home::encode_cwd_dirname(cwd))
        .join("sess-1");
