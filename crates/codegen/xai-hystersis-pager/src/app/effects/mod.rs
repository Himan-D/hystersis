...
use xai_hystersis_telemetry::startup::{self, StartupPhase};
use actions::{
...
use xai_hystersis_shell::sampling::error::http_status_from_error;
use xai_hystersis_shell::session::{ExtMethodResult, SessionInfoResponse};
...
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = xai_hystersis_active_sessions::register(xai_hystersis_active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::CreateSession {
            ...
            let tx = acp_tx.clone();
            let compat = xai_hystersis_tools::types::compat::CompatConfig::default();
            let mcp_servers = xai_hystersis_shell::util::config::load_mcp_servers(
                &session_cwd,
                &compat,
            );
            ...
            tasks
                .spawn(async move {
                    if let Some(sid) = load_session_id {
                        ...
                        let wt_type = xai_hystersis_shell::util::config::worktree_type();
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            xai_hystersis_foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &hystersis_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || xai_hystersis_foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
        ...
        Effect::FetchDashboardSessions => {
            ...
            tasks
                .spawn(async move {
                    ...
                    "cwd": cwd.to_string_lossy(),
                    "limit": 30,
                    "headless": xai_hystersis_shell::session::unified_list::HeadlessPolicy::Exclude
                        .as_wire_str(),
                });
                    let request = acp::ExtRequest::new(
                        "hystersis.com/session/list",
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            ...
            use xai_hystersis_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = xai_hystersis_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = xai_hystersis_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = xai_hystersis_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        xai_hystersis_shell::session::restore::ProgressCallback,
                    > = {
                        use xai_hystersis_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
                    ...
                    match restore_session_with_storage(
                            ...
                            &session_id,
                            &cwd_str,
                            xai_hystersis_shell::session::restore::RestoreSessionOpts {
                                turn_override: None,
                                progress,
                                restore_code: true,
                            },
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            ...
                            let history_path = xai_hystersis_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use xai_hystersis_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id, source } => {
            ...
            tasks
                .spawn(async move {
                    let params = xai_hystersis_shell::extensions::task::KillTaskRequest {
        ...
        Effect::SwitchModel {
            ...
            tasks
                .spawn(async move {
                    let meta = effort
                        .map(|eff| {
                            use xai_hystersis_shell::sampling::types::{
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use xai_hystersis_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            xai_hystersis_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            xai_hystersis_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistAnnouncementsHidden { hidden_ids } => {
            tasks
                .spawn(async move {
                    xai_hystersis_announcements::write_hidden_announcement_ids(&hidden_ids)
                        .await;
        ...
        Effect::PersistPrivacyBannerAcked { acked_at } => {
            tasks
                .spawn(async move {
                    if let Err(e) = xai_hystersis_shell::util::config::set_privacy_banner_acked(
                            acked_at,
                        )
                        .await
                    {
        ...
        Effect::PersistConsentAnswer { account, notice_id, version, acked } => {
            tasks
                .spawn(async move {
                    match xai_hystersis_shell::util::config::set_consent_answer(
                            account,
                            notice_id,
                            version,
                            acked,
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = xai_hystersis_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            serde_json::from_value::<
                                Vec<
                                    xai_hystersis_tools::implementations::skills::types::SkillInfo,
                                >,
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: xai_hystersis_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use xai_hystersis_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model, nonce } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = xai_hystersis_shell::agent::auth_method::has_xai_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text, images } => {
            use xai_hystersis_shell::session::ClientType;
            use xai_hystersis_shell::session::acp_types::ClientFeedbackInput;
        ...
        Effect::SaveMemoryNote { agent_id, text, cwd } => {
            tasks
                .spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                            let storage = xai_hystersis_shell::session::memory::MemoryStorage::new(
                                &cwd,
                                None,
                            );
                            storage
                                .append_to_memory(
                                    xai_hystersis_shell::session::memory::MemoryScope::Global,
                                    &text,
                                )
                        })
                        .await
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            xai_hystersis_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionMetaFromDisk {
            ...
            tasks
                .spawn(async move {
                    ...
                    let path = xai_hystersis_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    ...
                    let (title, last_turn_summary) = tokio::task::spawn_blocking(move || -> Option<
                        ...
                        > {
                            ...
                            let summary: xai_hystersis_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
        ...
        Effect::FetchBilling { agent_id, silent, nonce } => {
            ...
            tasks
                .spawn(async move {
                    use xai_hystersis_shell::extensions::billing::BillingConfigResponse;
                    let req = acp::ExtRequest::new(
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !xai_hystersis_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let hystersis_home = xai_hystersis_shell::util::hystersis_home::hystersis_home();
                            let store = xai_hystersis_shell::auth::read_auth_json(
                                    &hystersis_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = xai_hystersis_shell::auth::HystersisComConfig::default()
                                .auth_scope();
                            let auth = xai_hystersis_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    xai_hystersis_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            xai_hystersis_shell::remote::fetch_settings_blocking(
                                    &proxy_base,
                                    &auth,
                                    None,
                                )
...
async fn fetch_session_usage(
    session_id: &acp::SessionId,
    tx: &AcpAgentTx,
) -> Result<xai_hystersis_shell::extensions::notification::PromptUsage, String> {
    let request = acp::ExtRequest::new(
    ...
    let parsed: xai_hystersis_shell::extensions::usage::SessionUsageResponse = serde_json::from_str(
            resp.0.get(),
        )
        .map_err(|e| {
...
async fn lookup_session_title_in(
    ...
    use xai_hystersis_shell::session::storage::{JsonlStorageAdapter, StorageAdapter};
    let info = xai_hystersis_shell::session::info::Info {
        id: session_id.clone(),
        cwd: cwd.to_string(),
    };
...
fn session_info_fields(
    ...
    push(
        "Shell version",
        xai_hystersis_version::display_version(xai_hystersis_update::channel_label()),
        false,
    );
    ...
    let model_display = xai_hystersis_shell::session::model_display_name(
    ...
    if xai_hystersis_shell::session::should_show_model_fingerprint(
    ...
    if let Some(profile) = xai_hystersis_sandbox::profile_name() {
