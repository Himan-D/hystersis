...
pub(crate) fn push_gboom_keyboard_flags() {
    ...
    xai_hystersis_shell::util::with_locked_stderr(|stderr| {
...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use xai_hystersis_shell::agent::config::{ConfigSource, Feature, FeatureSources};
...
pub fn resolve_leader_mode<'p>(
    ...
    _remote_settings: Option<&xai_hystersis_shell::util::config::RemoteSettings>,
    eligible: bool,
...
pub fn join_early_prefetch(
    handle: Option<xai_hystersis_shell::agent::models::EarlyPrefetchHandle>,
) -> Option<xai_hystersis_shell::util::config::RemoteSettings> {
    let handle = handle?;
...
async fn bounded_connect(
    ...
    let context = || startup_failure::Context {
        target,
        attempt,
        version: xai_hystersis_version::display_version_with_commit(
            xai_hystersis_version::full_version(),
            xai_hystersis_update::channel_label(),
        ),
        log_path: xai_hystersis_telemetry::unified_log::path(),
    };
    tokio::select! {
        biased;
...
pub async fn run(
    args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<xai_hystersis_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    ...
    let hystersis_com_config = match xai_hystersis_shell::agent::config::Config::new_from_toml_cfg(
        &raw_config,
    ) {
        Ok(c) => c.hystersis_com_config,
        Err(e) => {
            tracing::warn!(error = %e, "failed to parse config for auth refresh, using defaults");
            xai_hystersis_shell::auth::HystersisComConfig::default()
        }
    };
    let refreshed_auth = tokio::time::timeout(
        xai_hystersis_shell::http::STARTUP_AUTH_REFRESH_TIMEOUT,
        xai_hystersis_shell::auth::try_ensure_fresh_auth(&hystersis_com_config),
    )
    .await
    .unwrap_or(None);
    let early_prefetch = match refreshed_auth {
        Some(auth) => xai_hystersis_shell::agent::models::start_early_prefetch_with_auth(Some(auth)),
        None => xai_hystersis_shell::agent::models::start_early_prefetch(Some(hystersis_com_config.clone())),
    };
    ...
    if had_prefetch {
        xai_hystersis_telemetry::startup::record_prefetch_wait(prefetch_wait_started.elapsed());
    }
    xai_hystersis_shell::util::config::cache_remote_auto_mode(
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(xai_hystersis_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = xai_hystersis_shell::session::persistence::list_summaries(None).await?;
    ...
    let launch_auto = xai_hystersis_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
        xai_hystersis_shell::util::config::default_interactive_permission_mode(),
    );
    ...
    xai_hystersis_telemetry::external::init(
        xai_hystersis_shell::agent::config::resolve_external_otel_config(
            xai_hystersis_telemetry::external::config::ExternalClientInfo {
                service_version: xai_hystersis_version::full_version().to_owned(),
                client_version: xai_hystersis_version::VERSION.to_owned(),
                app_entrypoint: "tui".to_owned(),
            },
        ),
    );
    let pending_startup = xai_hystersis_telemetry::startup::PendingStartup::new();
    let timer = xai_hystersis_telemetry::startup::begin(crate::acp::Owner::Client);
    ...
    drop(agent_guard);
    xai_hystersis_telemetry::session_ctx::drain_at_process_exit().await;
...
mod tests {
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = xai_hystersis_shell::util::config::RemoteSettings {
