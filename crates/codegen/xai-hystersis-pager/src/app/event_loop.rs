...
struct AgentLoadOutcome {
    ...
    success: bool,
    /// `hystersis.com/runningPromptId` from the reload response: the turn another
    /// client is driving mid-reconnect, adopted at finalize (mirrors the
    /// `SessionLoaded` adoption in `dispatch.rs`).
    running_prompt_id: Option<String>,
    /// `hystersis.com/schedulerBackgroundLoops` from the reload response. A reconnect
...
/// [`decide`](xai_hystersis_workspace::folder_trust::decide) precedence.
///
...
pub(crate) fn seed_consent_state_from_gate(
    app: &mut AppView,
    gate: Option<&xai_hystersis_shell::util::config::ConsentGate>,
) {
    use crate::app::consent::{ConsentInputs, consent_verdict};
    let stored = xai_hystersis_shell::config::load_from_disk()
        .ok()
        .map(|root| xai_hystersis_shell::util::config::load_config_from_toml(&root).consent)
        .unwrap_or_default();
...
pub(crate) async fn run(
    ...
    pending_startup: xai_hystersis_telemetry::startup::PendingStartup,
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("HYSTERSIS_LOG_SAMPLING", "1") };
    }
    ...
    xai_hystersis_telemetry::startup::enter(xai_hystersis_telemetry::startup::StartupPhase::AppInit);
    ...
    let launch_effective_config = xai_hystersis_shell::config::load_effective_config().ok();
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(xai_hystersis_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = xai_hystersis_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match xai_hystersis_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.privacy_notice_rollout = xai_hystersis_config::env_bool("HYSTERSIS_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
        ...
        .unwrap_or(false);
    app.privacy_banner_reshow_days = std::env::var("HYSTERSIS_PRIVACY_BANNER_RESHOW_DAYS")
        .ok()
    ...
    app.privacy_banner_acked = xai_hystersis_shell::config::load_from_disk()
        .ok()
        .and_then(|root| {
            xai_hystersis_shell::util::config::load_config_from_toml(&root)
                .privacy
                .privacy_banner_acked
        });
    app.plugin_cta_enabled = xai_hystersis_config::env_bool("HYSTERSIS_PLUGIN_CTA")
    ...
    app.workspace_dashboard_enabled = xai_hystersis_config::env_bool("HYSTERSIS_WORKSPACE_DASHBOARD")
        .or_else(|| {
            remote_settings
                .as_ref()
    ...
    app.session_picker_grouped = std::env::var("HYSTERSIS_SESSION_PICKER_GROUPED")
        .ok()
        ...
        .or_else(|| {
            xai_hystersis_shell::config::load_effective_config()
                .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the hystersis.com method from the advertised list
            let hystersis_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "hystersis.com");
            if let Some(method) = hystersis_com {
                ...
                // No hystersis.com method available, use the first method as fallback
        ...
        // fail-closed: do not invent hystersis.com / auto-start OIDC.
        tracing::info!(
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    xai_hystersis_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<xai_hystersis_shell::auth::AuthMeta>(meta.clone()) {
        ...
        app.is_api_key_auth = app.auth_methods.iter().any(|m| {
            m.id().0.as_ref() == xai_hystersis_shell::agent::auth_method::XAI_API_KEY_METHOD_ID
        });
    ...
    app.hidden_announcement_ids = xai_hystersis_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = xai_hystersis_shell::config::load_merged_requirements();
    let user_config = xai_hystersis_shell::config::load_from_disk().ok();
    let managed_config = xai_hystersis_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match xai_hystersis_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = xai_hystersis_shell::agent::config::resolve_compat_sessions_from_raw(
        effective_config.as_ref().ok_or(()),
        remote_settings.as_ref(),
    );
    app.foreign_session_compat = xai_hystersis_foreign_sessions::EnabledForeignSessionSources {
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            // (config > HYSTERSIS_XAI_API_BASE_URL env > default).
            let endpoints_base =
                xai_hystersis_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .xai_api_base_url;
            app.voice_config =
                xai_hystersis_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    ...
    app.zdr_access_enabled = xai_hystersis_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        xai_hystersis_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        xai_hystersis_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        xai_hystersis_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    app.scheduler_background_loops_seed =
        xai_hystersis_shell::util::config::resolve_scheduler_background_loops(
            remote_settings
                .as_ref()
...

    {
        ...
        app.active_announcements = xai_hystersis_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            let hystersis_home = xai_hystersis_tools::util::hystersis_home::hystersis_home();
            app.tip = xai_hystersis_shell::util::tips::pick_and_advance(&app.tips, &hystersis_home);
        }
    ...
    let hints = xai_hystersis_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.hystersis/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        xai_hystersis_shell::util::config::clamped_display_permission_mode(
            xai_hystersis_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        xai_hystersis_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = xai_hystersis_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = xai_hystersis_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // HYSTERSIS_SCROLL_SPEED/_MODE/_LINES + HYSTERSIS_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<xai_hystersis_voice::VoiceEvent>>;
    ...
    if let Some(action) = startup_action {
        ...
        app.finish_startup(xai_hystersis_telemetry::startup::StartupOutcome::Ok);
    }

    // Initial prompt from the CLI positional (`hystersis "fix the bug"`). When
    ...
    // `hystersis dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `HYSTERSIS_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.

    if std::env::var("HYSTERSIS_OPEN_SETTINGS_AT_STARTUP").as_deref() == Ok("1") {
        unsafe { std::env::remove_var("HYSTERSIS_OPEN_SETTINGS_AT_STARTUP") };
        if app.session_startup_allowed() {
            let effs = dispatch::dispatch(Action::OpenSettings, &mut app);
            if process_effects(effs, &mut tasks, &mut app, &progress_tx) {
                return Ok(finish_run(&mut app));
            }
            presenter.request_presentation(&mut app, terminal, false);
    ...
    if std::env::var("HYSTERSIS_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
        // SAFETY: we are pre-multithreaded init for this app loop.
        unsafe { std::env::remove_var("HYSTERSIS_OPEN_DASHBOARD_AT_STARTUP") };
        if app.session_startup_allowed() {
    ...
    loop {
        ...
        if let Err(e) = run_pending_suspends(
            ...
            app.finish_startup(xai_hystersis_telemetry::startup::StartupOutcome::Error);
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(xai_hystersis_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice could not start. Restart Hystersis.");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.hystersis/config.toml [ui]`
                // rather than `~/.hystersis/pager.toml`. The watcher only knows
...
pub(crate) fn load_initial_ui_config() -> xai_hystersis_shell::agent::config::UiConfig {
    use xai_hystersis_shell::agent::config::UiConfig;
    static INITIAL_UI: std::sync::OnceLock<UiConfig> = std::sync::OnceLock::new();

    INITIAL_UI
        .get_or_init(|| {
            let Ok(root) = xai_hystersis_shell::config::load_effective_config() else {
...
fn emit_event_loop_stall(window: super::event_loop_stall::StallWindow) {
    xai_hystersis_telemetry::session_ctx::log_event(super::event_loop_stall::event_loop_stall_event(
        window,
    ));
}
...
async fn drain_and_process(
    ...
    let mut handle_one = |routed: &RoutedInputEvent| -> bool {
        ...
        if let Event::Key(ke) = ev
            ...
            && xai_hystersis_voice::AUDIO_SUPPORTED
            && is_voice_chord(ke)
...
mod tests {
    ...
    fn plugin_cta_marketplace_from_managed_layer() {
        let layers = xai_hystersis_config::ConfigLayers {
    ...
    fn plugin_cta_marketplace_from_unset_or_empty_is_none() {
        let unset = xai_hystersis_config::ConfigLayers::default();
        assert_eq!(
            plugin_cta_marketplace_from(&unset.effective_config_base()),
            None
        );
        let empty = xai_hystersis_config::ConfigLayers {
        ...
        let blank = xai_hystersis_config::ConfigLayers {
            user: toml::from_str("[marketplace]\nplugin_cta_marketplace = \"   \"\n").unwrap(),
            ..Default::default()
        };
        assert_eq!(
