...
//!
//! - Request/response `hystersis.com/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `hystersis.com/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `hystersis.com/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `hystersis.com/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `hystersis.com/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `xai-hystersis-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `hystersis.com/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `hystersis.com/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> xai_hystersis_shell::agent::roster::RosterEntry {
        use xai_hystersis_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `hystersis.com/sessions/list`.
        ...
        assert_eq!(e.model_id.as_deref(), Some("hystersis-4"));
        assert!(e.yolo);
    ...
    /// `hystersis.com/sessions/changed` round-trip: serialize the agent's `RosterChanged`
