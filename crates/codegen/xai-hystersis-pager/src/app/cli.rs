...
#[command(
    name = "hystersis",
    version = xai_hystersis_version::full_version(),
    about = "Hystersis TUI",
...
pub struct PagerArgs {
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            xai_hystersis_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
...
impl PagerArgs {
    ...
    pub(crate) fn local_resume_selection(
        &self,
    ) -> xai_hystersis_shell::session::persistence::RecentSessionSelection {
        use xai_hystersis_shell::session::unified_list::HeadlessPolicy;
        let policy =
        ...
        xai_hystersis_shell::session::persistence::RecentSessionSelection::from_headless_policy(policy)
    }
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                xai_hystersis_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                xai_hystersis_shell::session::persistence::resolve_recent_session_sandbox_profile(
                    cwd,
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<xai_hystersis_sandbox::ProfileName>().ok()
                    != s.parse::<xai_hystersis_sandbox::ProfileName>().ok() =>
            {
