...
fn log_coding_data_consent_selected(
    source: xai_hystersis_telemetry::events::CodingDataConsentSource,
    opted_in: bool,
    previous_opted_in: bool,
) {
    use xai_hystersis_telemetry::events::{CodingDataConsentChoice, CodingDataConsentSelected};
    xai_hystersis_telemetry::session_ctx::log_event(CodingDataConsentSelected {
        source,
...
/// (persists via ACP ext-request, NOT `~/.hystersis/config.toml`).
...
pub(super) fn append_consumer_billing_surface(app: &mut AppView, agent_id: AgentId) -> Vec<Effect> {
    ...
    // Remote-settings kill switch (`hystersis_build_usage_redirect_url`): link out
...
pub(super) fn dispatch_manage_billing(app: &mut AppView) -> Vec<Effect> {
    ...
    super::router::dispatch(
        crate::app::actions::Action::OpenUrl("https://hystersis.com/?_s=usage".to_string()),
        app,
    )
}
...
pub(in crate::app::dispatch) fn dispatch_privacy_banner_opt_in(app: &mut AppView) -> Vec<Effect> {
    ...
    set_coding_data_sharing(
        app,
        true,
        xai_hystersis_telemetry::events::CodingDataConsentSource::PrivacyBanner,
    )
}
