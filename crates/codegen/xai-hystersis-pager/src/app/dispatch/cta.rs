...
pub(super) fn plugin_cta_candidates(
    ...
    let winner = match cta_marketplace {
        ...
        None => sources
            .iter()
            .position(|s| {
                xai_hystersis_plugin_marketplace::is_official_source_url(&s.source_url_or_path)
            })
            .or_else(|| {
                sources.iter().position(|s| {
                    s.source_name == xai_hystersis_plugin_marketplace::OFFICIAL_SOURCE_NAME
                })
            }),
    };
...
pub(super) fn plugin_cta_phase_for(
    ...
    use xai_hystersis_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(xai_hystersis_telemetry::events::PluginCtaInstalled {
    ...
    match result {
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(xai_hystersis_telemetry::events::ExtensionsModalOpened {
                    trigger: xai_hystersis_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = xai_hystersis_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(xai_hystersis_telemetry::events::PluginCtaImpression {
