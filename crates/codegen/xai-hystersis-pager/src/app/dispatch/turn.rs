...
use xai_hystersis_telemetry::events::CancellationScope;
...
fn effective_cancel_subagents_preference(
    ...
    ui: &xai_hystersis_shell::agent::config::UiConfig,
) -> Option<bool> {
...
fn cancel_agent_turn(
    ...
    // FRONT queued prompt as the next turn — its `hystersis.com/queue/changed`
...
/// Grace window between a driver-side `hystersis.com/session/prompt_complete`
...
/// Finish turns whose end was announced by `hystersis.com/session/prompt_complete`
...
pub(super) fn dispatch_kill_bg_task(app: &mut AppView, task_id: String) -> Vec<Effect> {
    ...
    vec![Effect::KillBgTask {
        session_id,
        task_id,
        source: xai_hystersis_shell::extensions::task::TaskKillSource::ClientUi,
    }]
}
