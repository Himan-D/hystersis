...
use xai_hystersis_telemetry::events::{HystersisProUpsell, HystersisProUpsellClicked};
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        xai_hystersis_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        ...
        CreditLimitUpsellMode::LegacyPayg { enabled: true } => (
            ...
            xai_hystersis_telemetry::events::CreditLimitChoice::PayAsYouGo,
            true,
        ),
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(xai_hystersis_telemetry::events::CreditLimitUpsellShown {
            surface: xai_hystersis_telemetry::events::CreditLimitUpsellSurface::InlineCard,
            max_tier: true,
    ...
    log_event(xai_hystersis_telemetry::events::CreditLimitUpsellShown {
        surface: xai_hystersis_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
        max_tier: false,
        pay_as_you_go: payg_telemetry,
        unified_billing,
    });
...
fn open_hystersispro_upsell(
    ...
    log_event(xai_hystersis_telemetry::events::HystersisProUpsellShown {
        source,
        auth_method,
    });
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<xai_hystersis_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<xai_hystersis_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
