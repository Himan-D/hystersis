...
fn auth_copy_dispatch_preserves_all_delivery_states() {
    for delivery in [
        ...
        let effects = crate::app::dispatch::router::dispatch_copy_auth_url(&mut app, |url| {
            assert_eq!(url, "https://hystersis.com/auth");
            delivery
        });
...
fn seed_foreign_resume_hint(
    ...
    app.apply_foreign_resume_detection(
        launch_token,
        &canonical_cwd,
        Some(xai_hystersis_foreign_sessions::RecentForeignSession {
            tool,
...
fn critical_announcement(id: &str) -> xai_hystersis_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> xai_hystersis_announcements::RemoteAnnouncement {
    xai_hystersis_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(xai_hystersis_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: Some(format!("https://hystersis.com/{id}")),
            caption: None,
        }),
        ..Default::default()
    }
...
/// the promo url (observed via the `HYSTERSIS_TEST_OPEN_URL_FILE` seam, from every
...
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    let url_file = std::env::temp_dir().join(format!("hystersis-cta-open-{}.txt", std::process::id()));
    ...
    for surface in [
        ...
        assert!(
            opened().lines().any(|l| l == "https://hystersis.com/promo-open"),
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!({
        "scope": "user",
        "path": "/home/user/.hystersis/skills/pick-best/SKILL.md",
    });
...
fn conversation_entry_load_sets_chat_kind_bit() {
    ...
    assert_eq!(
        agent.rename_kind(),
        xai_hystersis_shell::session::unified_list::SessionKind::Chat
    );
    let rename = dispatch(
    ...
    assert!(
        matches!(
            ...
            [Effect::RenameSession { kind, .. }]
                if *kind == xai_hystersis_shell::session::unified_list::SessionKind::Chat
        ),
...
fn load_sticky_chat_history_bypass_rename_kind_is_build() {
    ...
    assert_eq!(
        agent.rename_kind(),
        xai_hystersis_shell::session::unified_list::SessionKind::Build
    );
    let rename = dispatch(
        Action::RenameSession {
    ...
    assert!(
        matches!(
            ...
            [Effect::RenameSession { kind, title, .. }]
                if *kind == xai_hystersis_shell::session::unified_list::SessionKind::Build
                    && title == "local title"
        ),
...
fn welcome_expand_skips_conversation_and_routes_build_card_detail() {
    ...
    assert_eq!(
        welcome_card_detail(&app),
        None,
        "Headless must not resurrect a cleared Hystersis row from card detail"
    );
}
