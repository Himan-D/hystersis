...
/// Resume into a cwd with `.git/hystersis-worktree-source` sets `session.is_worktree`.
#[test]
fn load_session_marks_standalone_worktree_cwd() {
    ...
    assert!(
        ...
        "resume into a standalone hystersis worktree must set session.is_worktree"
    );
...
/// A failed `hystersis.com/prompt_history` fetch arrives as `PromptHistoryLoaded` with an empty list.
#[test]
...
/// fast foreign lane landed (hidden by the Hystersis default → CTA) and the native
