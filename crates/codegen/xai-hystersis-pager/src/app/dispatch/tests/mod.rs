...
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = xai_hystersis_shell::util::hystersis_home::hystersis_home();
    let encoded = xai_hystersis_shell::util::hystersis_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: xai_hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionMode::Default,
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    xai_hystersis_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    xai_hystersis_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
