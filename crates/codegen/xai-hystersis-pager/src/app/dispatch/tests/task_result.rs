...
) -> xai_hystersis_foreign_sessions::RecentForeignSession {
    xai_hystersis_foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    launch.foreign_session_compat = xai_hystersis_foreign_sessions::EnabledForeignSessionSources {
        cursor: true,
        ..Default::default()
    };
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                xai_hystersis_foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(xai_hystersis_foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                xai_hystersis_foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = xai_hystersis_shell::agent::config::ModelSwitchIncompatibleAgentError {
...
fn session_success_arms_finish_startup_obligation() {
    xai_hystersis_telemetry::unified_log::redirect_to_temp_for_tests();
    let id = AgentId(0);
    let results = [
    ...
    for result in results {
        let mut app = test_app_with_agent();
        app.agents.get_mut(&id).unwrap().session.session_id = None;
        app.pending_startup = Some(xai_hystersis_telemetry::startup::PendingStartup::new());
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = xai_hystersis_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    app.gate = Some(xai_hystersis_shell::auth::GateInfo {
    ...
    let settings = xai_hystersis_shell::util::config::RemoteSettings::default();
...
fn gate_refreshed_no_effect_when_still_gated() {
    ...
    let settings = xai_hystersis_shell::util::config::RemoteSettings {
...
fn test_gate() -> xai_hystersis_shell::auth::GateInfo {
    xai_hystersis_shell::auth::GateInfo {
        message: "Subscribe".into(),
        url: None,
        label: None,
    }
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(xai_hystersis_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(xai_hystersis_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
