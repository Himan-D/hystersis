...
use super::*;
use xai_hystersis_shell::sampling::error::is_free_usage_exhausted_error;
...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("hystersispro_heavy")));
    assert!(is_max_tier(Some("HystersisPro Heavy")));
    assert!(is_max_tier(Some("SUPERHYSTERSIS_HEAVY")));
}

#[test]
fn is_max_tier_non_max_and_unknown() {
    assert!(!is_max_tier(Some("hystersispro")));
...
fn is_max_tier_handles_mixed_case_and_whitespace() {
    assert!(is_max_tier(Some("HystersisPro_Heavy")));
    assert!(is_max_tier(Some("hystersispro heavy")));
    assert!(is_max_tier(Some("SUPERHYSTERSIS HEAVY")));
}

#[test]
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("hystersispro_heav")));
    assert!(!is_max_tier(Some("hystersispro_heavy_plus")));
...
#[test]
fn upsell_non_max_upgrade_url_is_hystersispro() {
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
    ...
    let url = agent_qv(&app).questions[0].options[0]
        ...
        .as_deref()
        .unwrap();
    assert!(url.contains("hystersispro"), "got: {url}");
    assert!(url.contains("referrer=hystersis"), "got: {url}");
}
...
fn upsell_non_max_unified_shows_buy_credits() {
    ...
    assert_eq!(
        ...
        "Purchase credits to keep using Hystersis"
    );
}
...
fn is_credit_limit_error_matches_legacy_403_and_pool_402() {
    ...
    assert!(is_credit_limit_error(
        None,
        "API error (status 402 Payment Required): Hystersis usage balance exhausted"
    ));
    assert!(is_credit_limit_error(
        None,
        "status 403: run out of credits"
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: xai_hystersis_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
fn team_auth_disables_agent_billing_surface() {
    ...
    app.apply_auth_meta(&xai_hystersis_shell::auth::AuthMeta {
...
#[serial_test::serial(HYSTERSIS_TEST_OPEN_URL_FILE)]
#[test]
fn manage_billing_gates_on_consumer_billing_surface() {
    let out = std::env::temp_dir().join(format!("hystersis-manage-billing-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&out);
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("HYSTERSIS_TEST_OPEN_URL_FILE", &out) };
    let mut app = test_app_with_agent();
    dispatch(Action::ManageBilling, &mut app);
    let opened = std::fs::read_to_string(&out).unwrap_or_default();
    assert!(opened.contains("hystersis.com/?_s=usage"), "got: {opened}");
    let _ = std::fs::remove_file(&out);
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    ...
    app.screen_mode = crate::app::ScreenMode::Minimal;
    let before = agent_scrollback_len(&app);
    let usage = xai_hystersis_shell::extensions::notification::PromptUsage {
        totals: xai_hystersis_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        xai_hystersis_shell::extensions::notification::PromptUsage {
            totals: xai_hystersis_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("hystersispro_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("hystersispro_heavy"));
}
...
fn free_usage_upsell_shows_three_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: xai_hystersis_telemetry::events::HystersisProUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        (
            "Upgrade to HystersisPro",
            "For everyday coding and productivity tasks",
            Some(UPSELL_URL_UPGRADE),
        ),
        (
            "Upgrade to HystersisPro Plus",
            "Significantly higher usage and rate limits",
            Some(UPSELL_URL_UPGRADE),
        ),
        (
            "Upgrade to HystersisPro Heavy",
            "Get the most out of Hystersis. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use xai_hystersis_shell::extensions::notification::{RetryState, SessionUpdate};
...
/// Submitting a tier-restricted command opens the three-option HystersisPro
...
fn restricted_command_submit_opens_three_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: xai_hystersis_telemetry::events::HystersisProUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
    assert_eq!(q.question, "Unlock all features with HystersisPro.");
    assert_eq!(q.options.len(), 3);
    assert_eq!(q.options[0].label, "Upgrade to HystersisPro");
    assert_eq!(q.options[0].id.as_deref(), Some(UPSELL_URL_UPGRADE));
    assert_eq!(q.options[1].label, "Upgrade to HystersisPro Plus");
    assert_eq!(q.options[1].id.as_deref(), Some(UPSELL_URL_UPGRADE));
    assert_eq!(q.options[2].label, "Upgrade to HystersisPro Heavy");
...
/// `HYSTERSIS_TEST_OPEN_URL_FILE` seam), `Action::OpenUrl` for a billing CTA
...
fn open_url_shows_manual_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "hystersis-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("HYSTERSIS_TEST_OPEN_URL_FILE", &bad) };
    ...
    unsafe { std::env::remove_var("HYSTERSIS_TEST_OPEN_URL_FILE") };
}
...
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("hystersis-open-url-ok-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("HYSTERSIS_TEST_OPEN_URL_FILE", &url_file) };
...
fn open_url_welcome_toasts_single_line_url_when_browser_unavailable() {
    let bad = std::env::temp_dir().join(format!(
        "hystersis-open-url-welcome-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("HYSTERSIS_TEST_OPEN_URL_FILE", &bad) };
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "hystersis-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("HYSTERSIS_TEST_OPEN_URL_FILE", &bad) };
    ...
    let kind = LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            xai_hystersis_telemetry::events::CreditLimitChoice::UpgradeTier,
            xai_hystersis_telemetry::events::CreditLimitChoice::PurchaseCredits,
        ],
    };
...
fn billing_fetched_clears_usage_modal_loading() {
    ...
    assert_eq!(state.ctx.subscription_tier.as_deref(), Some("HystersisPro"));
