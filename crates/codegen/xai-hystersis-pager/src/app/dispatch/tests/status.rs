...
fn dispatch_confirm_reset_setting_reset_dispatches_typed_setter_for_shared_enum() {
    ...
    with_theme_test_env(|| {
        ...
        // Reset → SetTheme("hystersisnight") (the registered default).
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting { key, value, .. } => {
                assert_eq!(*key, "theme");
        ...
        assert_eq!(app.current_ui.theme.as_deref(), Some("hystersisnight"));
    });
}
...
fn context_info_response() -> xai_hystersis_shell::session::SessionInfoResponse {
    ...
    xai_hystersis_shell::session::SessionInfoResponse {
        ...
        data: SessionInfoData {
            agent_name: None,
            model: Some("hystersis".to_string()),
