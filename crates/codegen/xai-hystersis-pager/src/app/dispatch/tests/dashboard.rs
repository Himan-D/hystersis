...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(xai_hystersis_shell::sampling::types::ReasoningEffort::High),
...
fn dashboard_slash_usage_hidden_for_external_auth() {
    ...
    app.apply_auth_meta(&xai_hystersis_shell::auth::AuthMeta::default());
    open_dashboard(&mut app);
    let before = app.agents.len();
        ...
        effects.iter().any(|e| matches!(
            ...
            Effect::RenameSession { agent_id, title, kind, .. }
                if *agent_id == id
                    ...
                    && *kind == xai_hystersis_shell::session::unified_list::SessionKind::Build
        )),
    ...
    assert!(
        effects.iter().any(|e| matches!(
            ...
            Effect::RenameSession { agent_id, title, kind, .. }
                if *agent_id == id
                    && title == "Chat title"
                    && *kind == xai_hystersis_shell::session::unified_list::SessionKind::Chat
        )),
