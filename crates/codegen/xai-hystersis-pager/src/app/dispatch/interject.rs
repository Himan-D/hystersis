//! Mid-turn interjection dispatch: optimistic local echo, the `hystersis.com/interject` effect, and prompt-history recording.
...
/// prompt, and fires the `hystersis.com/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `hystersis.com/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject_on(
    ...
    // record its id so the broadcast echo (`hystersis.com/session/interjection`) is
