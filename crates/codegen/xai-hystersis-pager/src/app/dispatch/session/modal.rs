...
/// Rename the current session via hystersis.com/session/rename.
///
...
pub(in crate::app::dispatch) fn dispatch_rename_session(
    ...
    let title = xai_hystersis_shell::session::persistence::sanitize_rename_title(&title).into_owned();
    if title.is_empty() {
...
/// Unpin the current session title via `hystersis.com/session/rename` + `resetToAuto`.
///
