...
use xai_hystersis_shell::extensions::notification::RetryState;
use xai_hystersis_shell::extensions::notification::SessionUpdate as XaiSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> xai_hystersis_shell::extensions::notification::ImageCompressedEntry {
    xai_hystersis_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[xai_hystersis_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
...
pub(super) fn make_deleted_ext_notif(
    ...
    make_deleted_ext_notif_with_reason(
        session_id,
        task_id,
        xai_hystersis_tools::notification::ScheduledTaskRemovedReason::Unknown,
        false,
    )
}
pub(super) fn make_deleted_ext_notif_with_reason(
    session_id: &str,
    task_id: &str,
    reason: xai_hystersis_tools::notification::ScheduledTaskRemovedReason,
    is_replay: bool,
) -> acp::ExtNotification {
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn xai_hook_execution_notif_for_prompt(
    ...
    use xai_hystersis_shell::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
...
pub(super) fn xai_hook_execution_notif_with_runs(
    ...
    prompt_id: Option<&str>,
    is_replay: bool,
    runs: Vec<xai_hystersis_shell::extensions::notification::HookRunEntryDto>,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn write_child_updates_jsonl_under_cwd(
    ...
    let sessions_dir = hystersis_home
        .join("sessions")
        .join(xai_hystersis_config::encode_cwd_dirname(cwd))
        .join(child_sid);
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = xai_hystersis_workspace::session::git::GitHeadChanged {
...
pub(super) fn send_late_bg_detection(app: &mut AppView, tc_id: &str) {
    use serde_json::json;
    use xai_hystersis_tools::types::output::{BashOutput, ToolOutput};
    let agent = app.agents.get_mut(&AgentId(0)).unwrap();
    let meta = crate::acp::meta::NotificationMeta::default();
    let bash = BashOutput {
...
pub(super) fn task_completed_notif(
    ...
    use xai_hystersis_tools::types::TaskSnapshot;
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: xai_hystersis_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use xai_hystersis_shell::extensions::mcp::{
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = xai_hystersis_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
