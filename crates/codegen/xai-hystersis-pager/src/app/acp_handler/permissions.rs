...
fn enqueue_permission(
    ...
    let bash_selection_count = bash_highlights
        .as_ref()
        .map(|h| xai_hystersis_workspace::permission::default_always_allow_scope(&h.highlighted_words))
        .unwrap_or(0);
    let bash_deny_selection_count = bash_highlights
        .as_ref()
        .map(|h| xai_hystersis_workspace::permission::default_always_deny_scope(&h.highlighted_words))
        .unwrap_or(0);
    ...
    if let Some(h) = bash_highlights.as_ref() {
        ...
        if offers_allow_row
            && !xai_hystersis_workspace::permission::always_allow_scope_persists(h, bash_selection_count)
        {
            tracing::warn!(
    ...
    let mcp_scope = perm
        ...
        .and_then(|m| {
            serde_json::from_value::<xai_hystersis_workspace::permission::McpToolPermission>(
...
fn build_permission_display(
    ...
    let bash_input = req.tool_call.fields.raw_input.as_ref().and_then(|v| {
        serde_json::from_value::<xai_hystersis_tools::implementations::BashToolInput>(v.clone()).ok()
    });
    ...
    let title = if is_execute {
        ...
        if let Some(path) = file_path {
            ...
            format!(
                "Allow {}?",
                xai_hystersis_workspace::permission::mcp_pretty_name_if_qualified(t)
            )
        } else {
...
fn protected_edit_description(req: &acp::RequestPermissionRequest) -> Option<String> {
    ...
    let protected: xai_hystersis_workspace::permission::ProtectedEditPermission =
