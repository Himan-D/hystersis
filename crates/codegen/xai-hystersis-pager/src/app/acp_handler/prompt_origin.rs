...
pub(crate) fn is_server_initiated_prompt(prompt_id: &str) -> bool {
    xai_hystersis_shell::session::PromptOrigin::from_prompt_id(prompt_id).is_synthetic()
}
...
/// `hystersis.com/session/prompt_complete` turn-end signal. A viewer can thus safely
...
pub(crate) fn is_scheduler_fired_prompt(prompt_id: &str) -> bool {
    matches!(
        xai_hystersis_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        xai_hystersis_shell::session::PromptOrigin::SchedulerFired
    )
}
...
pub(crate) fn is_wake_prompt(prompt_id: &str) -> bool {
    matches!(
        xai_hystersis_shell::session::PromptOrigin::from_prompt_id(prompt_id),
        xai_hystersis_shell::session::PromptOrigin::TaskCompleted { .. }
            | xai_hystersis_shell::session::PromptOrigin::SubagentCompleted { .. }
            | xai_hystersis_shell::session::PromptOrigin::WorkflowCompleted { .. }
            | xai_hystersis_shell::session::PromptOrigin::NotificationDrain
    )
}
...
/// adoptable iff the turn emits a terminal `hystersis.com/session/prompt_complete`, the
