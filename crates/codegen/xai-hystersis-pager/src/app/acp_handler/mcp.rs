...
/// `HYSTERSIS_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        xai_hystersis_shell::util::config::resolve_mcp_push_server_status(
...
/// Handle `hystersis.com/mcp/tools_changed` and `hystersis.com/mcp_initialized`.
///
/// Routing rules (verified against the four shell emit sites in
/// `xai-hystersis-shell/src/session/acp_session.rs` — toggle-tool ~L6661,
...
///    [`xai_hystersis_shell::extensions::mcp::McpToolsChanged`] struct), and
...
pub(super) fn handle_mcp_tools_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    // Both `hystersis.com/mcp_initialized` and (newer shell)
    // `hystersis.com/mcp/tools_changed` carry `sessionId`. Route by it so a
    ...
    if method == "hystersis.com/mcp_initialized"
...
/// Handle `hystersis.com/mcp/server_status`.
///
...
/// ([`xai_hystersis_shell::extensions::mcp::McpServerStatusPayload`] +
/// [`xai_hystersis_shell::extensions::mcp::McpServerStatus`]) instead of
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use xai_hystersis_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};

    let Ok(payload) = serde_json::from_str::<McpServerStatusPayload>(notif.params.get()) else {
        tracing::warn!(
            "Failed to parse hystersis.com/mcp/server_status: {}",
            &notif.params.get()
        ...
        match serde_json::from_value::<Vec<McpToolEntry>>(raw) {
            ...
            Err(e) => {
                tracing::warn!(
                    server = %payload.name,
                    error = %e,
                    "hystersis.com/mcp/server_status: tools field present but not Vec<McpToolEntry>; status still applied"
                );
                None
            }
        }
...
/// Handle `hystersis.com/mcp/elicit_complete`: dismiss the matched agent's URL-mode
...
pub(super) fn handle_mcp_elicit_complete(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(payload) = serde_json::from_str::<
        xai_hystersis_tools::mcp_elicitation::McpElicitCompletePayload,
...
/// Handle `hystersis.com/mcp/servers_updated`.
///
/// Emitted by the shell from `MvpAgent` on managed-config resolve and
/// on config reload (`crates/codegen/xai-hystersis-shell/src/agent/mvp_agent.rs`
