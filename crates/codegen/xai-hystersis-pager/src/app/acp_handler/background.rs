...
/// Handle `hystersis.com/task_backgrounded` — a bash command transitioned to background.
///
...
pub(super) fn handle_task_backgrounded(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse hystersis.com/task_backgrounded");
        return false;
    };
...
/// Handle `hystersis.com/monitor_event` — background task or monitor emitted new output.
...
fn expired_task_notice(info: &crate::app::agent::ScheduledTaskInfo) -> String {
    ...
    format!(
        ...
        info.human_schedule,
        xai_hystersis_tools::implementations::hystersis_build::scheduler::types::RECURRING_TASK_TTL_DAYS,
    )
}
...
pub(super) fn handle_scheduled_task_inject_prompt(
    ...
    let payload: serde_json::Value = match serde_json::from_str(notif.params.get()) {
        Ok(v) => v,
        Err(e) => {
            tracing::warn!(error = %e, "Failed to parse hystersis.com/scheduled_task_inject_prompt");
            return false;
        }
    };
    let Some(session_id) = payload["sessionId"].as_str() else {
        tracing::warn!("hystersis.com/scheduled_task_inject_prompt: missing or non-string sessionId");
        return false;
    };
    let Some(prompt) = payload["prompt"].as_str().filter(|s| !s.is_empty()) else {
        tracing::warn!("hystersis.com/scheduled_task_inject_prompt: missing or empty prompt");
        return false;
    };
    ...
    // `hystersis.com/scheduled_task_inject_prompt` notification is routed by the leader
...
pub(super) fn handle_git_head_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(params) = serde_json::from_str::<xai_hystersis_workspace::session::git::GitHeadChanged>(
        notif.params.get(),
    ) else {
        return false;
    };
...
pub(super) fn handle_task_completed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    let Ok(session_notif) = serde_json::from_str::<SessionNotification>(notif.params.get()) else {
        tracing::warn!("Failed to parse hystersis.com/task_completed");
        return false;
    };
