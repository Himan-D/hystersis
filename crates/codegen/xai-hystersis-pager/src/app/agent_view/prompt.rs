...
impl AgentView {
    ...
    ///
    /// The browse reads that list alone, and the `hystersis.com/prompt_history` fetch delivers an empty list when it
    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    xai_hystersis_telemetry::session_ctx::log_event(
                        xai_hystersis_telemetry::events::PromptSuggestion {
                            action: xai_hystersis_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::PromptSuggestion {
                        action: xai_hystersis_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
            ...
            match self.prompt.handle_key(key) {
                PromptEvent::Edited => {
                    if undo_tip_accepted {
                        xai_hystersis_telemetry::session_ctx::log_event(
                            xai_hystersis_telemetry::events::ContextualTip {
    ...
    /// The pty-only `HYSTERSIS_ESC_DOUBLE_PRESS_MS` override can exceed this; no
...
mod prompt_suggestion_key_tests {
    ...
    /// right after a turn ends with `hystersis.com/suggestPrompt` resolved. Pins the
