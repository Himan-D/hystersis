...
impl AgentView {
    ...
    pub(in crate::app) fn force_interject_queue_row(&mut self, id: u64) -> InputOutcome {
        ...
        if is_server {
            // Server row: the agent promotes it to run next (`hystersis.com/queue/interject`); any kind may send now.
                ...
                && let Some(server_id) = row.server_id.clone()
            {
                ...
                // Park the intent; the confirming `hystersis.com/queue/changed` broadcast fires it with the row's authoritative version.
    ...
    /// `hystersis.com/queue/changed` broadcast (pre-merge entries — the mirrored
    ...
    /// QUEUED — the caller fires `hystersis.com/queue/interject` with that
    ...
    pub(in crate::app) fn handle_queue_key(
        ...
        if let Some(event) = self.queue.handle_key(key, registry) {
            // Server rows route to the agent as `hystersis.com/queue/*` commands (the rebroadcast is the source of truth); local rows mutate in place.
    ...
    /// Reorder payload for `hystersis.com/queue/reorder`. Omit only running; include
