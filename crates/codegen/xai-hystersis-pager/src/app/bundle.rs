...
//!
//! Pager-side cache of what `xai-hystersis-shell` reports from
//! `hystersis.com/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `hystersis.com/bundle/status` ACP responses.
...
/// Deserialized response from `hystersis.com/bundle/status`.
...
/// Deserialized response from `hystersis.com/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
