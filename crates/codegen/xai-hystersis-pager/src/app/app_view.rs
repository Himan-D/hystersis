...
/// [`xai_hystersis_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<xai_hystersis_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: xai_hystersis_shell::util::config::WorktreeHintMode) -> Self {
        use xai_hystersis_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        xai_hystersis_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            xai_hystersis_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`xai_hystersis_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// (toolset) gate via [`xai_hystersis_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => xai_hystersis_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    pub pending_startup: Option<xai_hystersis_telemetry::startup::PendingStartup>,
    ...
    /// stays sans-IO.
    pub current_ui: xai_hystersis_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    ...
    pub active_announcements: Vec<xai_hystersis_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<xai_hystersis_announcements::RemoteAnnouncement>,
    ...
    pub session_picker_content_results:
        Option<Vec<xai_hystersis_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat: xai_hystersis_foreign_sessions::EnabledForeignSessionSources,
    ...
    /// `xai_hystersis_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    pub contextual_hints: xai_hystersis_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<xai_hystersis_shell::util::config::ContextualHintsRemote>,
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, xai_hystersis_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `hystersis_build_access_gate`. `Some` = blocked.
    pub gate: Option<xai_hystersis_shell::auth::GateInfo>,
    ...
    pub pending_gate_verification: Option<xai_hystersis_shell::auth::GateInfo>,
    ...
    pub voice_config: xai_hystersis_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `XAI_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<xai_hystersis_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture — toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<xai_hystersis_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub(crate) fn finish_startup(&mut self, outcome: xai_hystersis_telemetry::startup::StartupOutcome) {
        xai_hystersis_telemetry::startup::PendingStartup::finish_held(
    ...
    pub fn gate_from_settings(
        rs: &xai_hystersis_shell::util::config::RemoteSettings,
    ) -> Option<xai_hystersis_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(xai_hystersis_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &xai_hystersis_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            xai_hystersis_telemetry::session_ctx::log_event(
                xai_hystersis_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: xai_hystersis_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: xai_hystersis_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: xai_hystersis_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && xai_hystersis_voice::AUDIO_SUPPORTED
    }
    ...
    fn voice_send(&self, cmd: xai_hystersis_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(xai_hystersis_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(xai_hystersis_voice::VoiceCommand::PttRelease);
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<xai_hystersis_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    xai_hystersis_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        xai_hystersis_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    pub fn draw(&mut self, terminal: &mut PagerTerminal) {
        ...
        xai_hystersis_telemetry::startup::record_first_frame();
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            xai_hystersis_telemetry::session_ctx::log_event(
                                xai_hystersis_telemetry::events::HystersisProUpsellShown {
                                    source:
                                        xai_hystersis_telemetry::events::HystersisProUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`xai_hystersis_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use xai_hystersis_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = xai_hystersis_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                xai_hystersis_telemetry::session_ctx::log_event(
                    xai_hystersis_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: xai_hystersis_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            xai_hystersis_telemetry::session_ctx::log_event(xai_hystersis_telemetry::events::ContextualTip {
                tip: xai_hystersis_telemetry::events::ContextualTipKind::ImageInput,
                action: xai_hystersis_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
