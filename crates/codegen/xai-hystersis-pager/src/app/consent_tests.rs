use super::*;
use xai_hystersis_shell::util::config::{ConsentAnswer, ConsentGate};

const ACCOUNT: &str = "user@example.com";
const NOTICE_ID: &str = "tos-2026-08";
const TOS_URL: &str = "https://hystersis.com/legal/terms-of-service";
...
fn two_links_are_indexed_in_order() {
    let mut gate = gate();
    gate.body = Some("Read [Terms](https://hystersis.com/a) and [Policy](https://hystersis.com/b).".to_owned());
    ...
    assert_eq!(notice.links, vec!["https://hystersis.com/a", "https://hystersis.com/b"]);
}
...
fn a_link_with_nothing_to_paint_is_dropped() {
    let mut gate = gate();
    gate.body = Some("Read [ ](https://hystersis.com/a) and [Terms](https://hystersis.com/b).".to_owned());

    let notice = ConsentNotice::try_from_remote(&gate).expect("valid");

    assert_eq!(notice.links, vec!["https://hystersis.com/b"]);
...
fn unpairable_markup_refuses() {
    for body in [
        ...
        "Read our [Terms](https://hystersis.com/legal/te",
        // A url holds no space, so this `)` closes the second link, not the first.
        "Read [Terms](https://hystersis.com/a [Policy](https://hystersis.com/b) now.",
        ...
        "[a] b](https://hystersis.com/legal/tos) applies.",
    ] {
        let mut gate = gate();
...
fn a_url_outside_a_link_refuses() {
    let mut gate = gate();
    gate.body = Some("Review the terms at https://hystersis.com/legal/tos before continuing.".to_owned());
...
fn no_body_shape_leaks_a_url_or_raw_markup() {
    let bodies = [
        "[](https://hystersis.com/legal/tos) applies.",
        "[note] see [AUP](https://hystersis.com/legal/aup).",
        "Read [Terms](javascript:alert(1)) now.",
        "Read [Terms](https://hystersis.com/a&calc) now.",
        // A `)` inside the url ends it early and leaves the rest to be painted as prose.
        "Read [Terms](https://hystersis.com/a(b)https://evil.example) now.",
    ];
