...
fn collect_report_joins_registry_and_flags_untracked() {
    ...
    let home = base.join("hystersis-home");
...
fn worktrees_dominate_at_half_of_total() {
    ...
    for case in cases {
        let report = DiskUsageReport {
            hystersis_home: "/home/user/.hystersis".into(),
...
fn json_shape_is_frozen() {
    let report = DiskUsageReport {
        ...
        registry: RegistryState::Read,
        registry_path: "/home/user/.hystersis/worktrees.db".into(),
        worktrees: vec![
            WorktreeUsage {
                last_modified_at: Some(1_700_005_000),
                path: "/home/user/.hystersis/worktrees/xai/wt-1".into(),
                ..tracked_row(
                    90,
                    TrackedRow {
            ...
            WorktreeUsage {
                ...
                path: "/home/user/.hystersis/worktree_pool/inst/wt-2".into(),
                ..untracked_row(10)
            },
        ],
    };
    assert_eq!(
        ...
        serde_json::json!({
            "schema_version": 1,
            "hystersis_home": "/home/user/.hystersis",
            "total_bytes": 100,
            ...
            "worktrees_outside_managed_roots": 0,
            "registry": "read",
            "registry_path": "/home/user/.hystersis/worktrees.db",
            "worktrees": [
                {
                    "bytes": 90,
                    "kind": "session",
                    ...
                    "repo_name": "xai",
                    "git_ref": "brian/fix",
                    "path": "/home/user/.hystersis/worktrees/xai/wt-1",
                },
                {
                    "bytes": 10,
                    "kind": "pool",
                    ...
                    "repo_name": null,
                    "git_ref": null,
                    "path": "/home/user/.hystersis/worktree_pool/inst/wt-2",
                },
            ],
        })
    );
...
fn missing_home_json_is_valid_and_empty() {
    let mut out = Vec::new();
    write_report(
        &empty_report(Path::new("/nonexistent/.hystersis")),
        /*json*/ true,
        &mut out,
    )
    .unwrap();
...
fn print_report_renders_registry_notices() {
    ...
    let cases = [
        ...
        Case {
            ...
            absent: &["db rebuild", "damaged", "Remove $HYSTERSIS_HOME/worktrees.db"],
        },
        Case {
...
fn reclaim_hint_names_a_sequence_that_frees_space() {
    const AGE: &str = "run `hystersis worktree gc --max-age 7d --dry-run`";
    const RM: &str = "Remove one with `hystersis worktree rm --dry-run <path>`";
...
fn symlinked_default_home_keeps_home_label() {
    ...
    let real_hystersis = tmp.path().join("hystersis-on-disk");
    std::fs::create_dir_all(&fake_home).unwrap();
    std::fs::create_dir_all(&real_hystersis).unwrap();
    std::os::unix::fs::symlink(&real_hystersis, fake_home.join(".hystersis")).unwrap();
    let _home = crate::test_util::EnvVarGuard::set("HOME", &fake_home);

    let resolved = dunce::canonicalize(&fake_home).unwrap().join(".hystersis");
    let canonical = dunce::canonicalize(&resolved).unwrap();
    assert_ne!(canonical, resolved, "the symlink must actually resolve");
    assert_eq!(
        crate::util::display_hystersis_home_prefix_for(&canonical),
        "~/.hystersis"
    );
}
