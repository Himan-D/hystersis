...
//! render mode.
//!
//! `xai-hystersis-pager` cannot depend on `xai-hystersis-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`xai-hystersis-pager-bin`) wires it up once at
//! startup by calling `xai_hystersis_pager_minimal::install()`. When nothing has
...
/// (`xai_hystersis_pager_minimal::draw`).
