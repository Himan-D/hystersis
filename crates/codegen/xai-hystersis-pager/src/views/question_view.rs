...
use xai_hystersis_markdown::StreamingMarkdownRenderer;
pub use xai_hystersis_tools::implementations::hystersis_build::ask_user_question::{
...
/// driven by an ACP `hystersis.com/ask_user_question` request).
///
...
pub enum LocalQuestionKind {
    ...
    CreditLimitUpsell {
        choices: Vec<xai_hystersis_telemetry::events::CreditLimitChoice>,
    },
    /// HystersisPro upsell modal: the free-usage paywall (429 +
    ...
    /// `id`.
    FreeUsageUpsell {
        /// Telemetry source for `HystersisProUpsellClicked` — distinguishes
        /// the paywall from the restricted-command upsell.
        source: xai_hystersis_telemetry::events::HystersisProUpsell,
    },
    ...
    AgentTypeMismatch {
        ...
        effort: Option<xai_hystersis_shell::sampling::types::ReasoningEffort>,
    },
    DoctorFix {
...
pub const FEEDBACK_QUESTION_LABEL: &str = "How can we improve Hystersis?";
...
///
/// Created when an `hystersis.com/ask_user_question` ext-method request arrives;
...
pub struct QuestionViewState {
    ...
    /// instead of by an ACP `hystersis.com/ask_user_question` request. `None` for
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `hystersis.com/ask_user_question`
...
impl QuestionViewState {
    ...
    /// questions (e.g. the HystersisPro upsell) have no freeform row, so
...
impl QuestionViewState {
    ...
    pub fn build_accepted_response(
        &self,
    ) -> xai_hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionExtResponse
    {
    ...
    pub fn send_ext_response(
        &mut self,
        response: xai_hystersis_tools::implementations::hystersis_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
...
mod tests {
    ...
    /// `no_freeform` questions (e.g. the HystersisPro upsell) have no "Other"
    ...
    fn chrome_height_option_less_question_drops_the_label_gap() {
        ...
        let q = make_question("How can we improve Hystersis?", &[], false);
        assert_eq!(
            chrome_height(
                &q,
                80,
