...
//! `hystersis.com/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `hystersis.com/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `hystersis.com/suggest` response.
...
// (e.g. xai-hystersis-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Parsed response from an ACP `hystersis.com/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `hystersis.com/suggest` response.
...
/// Wire `limit` for `hystersis.com/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `HYSTERSIS_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `HYSTERSIS_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `hystersis.com/suggest` request.
    /// Resolved at construction from `HYSTERSIS_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: xai_hystersis_config::env_bool("HYSTERSIS_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: xai_hystersis_config::env_bool("HYSTERSIS_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("HYSTERSIS_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `hystersis.com/suggest` response arrives, with the text and
