...
pub(crate) fn pending_delete_from_selection(
    ...
    content_results: Option<&[xai_hystersis_shell::extensions::session_search::SearchSessionHit]>,
...
/// matters because the fast foreign scan can land rows the default Hystersis view
...
pub(crate) fn loading_spinner_active(
    ...
    // External is the foreign cache: an Exclude refetch cannot change it.
    if source_filter.is_content_search_disabled() {
        return false;
    }
    // Native refetch in flight with only foreign (or Headless) rows matching:
    // the visible cache is not the policy being fetched.
    loading
...
///
/// Default is [`Self::Hystersis`]: native Hystersis sessions only (local / remote /
/// conversation), so `/resume` does not mix Claude/Codex/Cursor foreign
/// sessions or `hystersis -p` one-shots into the list. `f` cycles Hystersis → Headless →
/// External → All → Local → Remote. Entering or leaving `Headless` refetches:
/// the shell excludes headless rows from every other page's fetch, so a
/// client-side refilter of the cached page could never populate it.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SourceFilter {
    /// Native Hystersis sessions only — excludes Claude/Codex/Cursor foreign rows.
    #[default]
    Hystersis,
    /// `hystersis -p` one-shots only (`session_kind == "headless"`).
    Headless,
    Local,
    Remote,
    External,
...
impl SourceFilter {
    ...
    pub fn next(self) -> Self {
        match self {
            Self::Hystersis => Self::Headless,
            Self::Headless => Self::External,
            Self::External => Self::All,
            Self::All => Self::Local,
            Self::Local => Self::Remote,
            Self::Remote => Self::Hystersis,
        }
    }
    ...
    /// stores are not FTS-indexed. The Headless page searches like every
    /// native page; the server filters hits by the page's headless policy.
    pub fn is_content_search_disabled(self) -> bool {
        self == Self::External
    }

    /// The server-side headless policy a fetch or content search for this
    /// page must carry: `Only` on the Headless page, `Exclude` everywhere
    /// else (foreign rows are never headless, so External keeps the default).
    pub fn headless_policy(self) -> xai_hystersis_shell::session::unified_list::HeadlessPolicy {
    ...
    /// Returns `true` if a session with the given `source` string and
    /// `session_kind` passes the filter.
    ///
    /// hystersis.com conversations carry `source == "conversation"` and live remotely,
    /// so they pass the `Remote` filter (and `Hystersis` / `All`) but not `Local`.
    /// Foreign sources (`claude` / `codex` / `cursor`) only pass `External` and
    /// `All`. Headless rows pass only `Headless`; every other page excludes
    /// them, mirroring the server-side fetch policy.
    pub fn matches(self, source: &str, session_kind: Option<&str>) -> bool {
        let is_headless = session_kind == Some("headless");
        match self {
            Self::Hystersis => !crate::app::is_foreign_picker_source(source) && !is_headless,
...
pub(crate) fn build_content_entry_data(
    hits: &[xai_hystersis_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
mod tests {
    ...
    fn source_filter_matches() {
        // Default Hystersis filter: native only (not Claude/Codex/Cursor).
        assert!(SourceFilter::Hystersis.matches("local", None));
        assert!(SourceFilter::Hystersis.matches("remote", None));
        assert!(SourceFilter::Hystersis.matches("both", None));
        assert!(SourceFilter::Hystersis.matches("conversation", None));
        assert!(!SourceFilter::Hystersis.matches("claude", None));
        assert!(!SourceFilter::Hystersis.matches("codex", None));
        assert!(!SourceFilter::Hystersis.matches("cursor", None));
        ...
        // hystersis.com conversations are remote: visible under Hystersis + All + Remote, not Local.
    ...
    fn source_filter_cycles() {
        assert_eq!(SourceFilter::Hystersis.next(), SourceFilter::Headless);
        ...
        assert_eq!(SourceFilter::Remote.next(), SourceFilter::Hystersis);
        assert_eq!(SourceFilter::Hystersis.label(), "Hystersis");
        assert_eq!(SourceFilter::Headless.label(), "Headless");
        assert_eq!(SourceFilter::External.label(), "External");
        assert_eq!(SourceFilter::default(), SourceFilter::Hystersis);
    }
    ...
    fn source_filter_filters_entries() {
        ...
        let hystersis = filter_session_entries(Some(&entries), "", SourceFilter::Hystersis);
        assert_eq!(hystersis, vec![0, 1, 2]);
    ...
    fn source_filter_empty_and_unknown_source() {
        ...
        // foreign, so it passes Hystersis + All but never Local, Remote, or External.
        assert!(SourceFilter::Hystersis.matches("", None));
        ...
        assert!(SourceFilter::Hystersis.matches("unknown", None));
    ...
    fn source_filter_is_active() {
        assert!(!SourceFilter::Hystersis.is_active());
    ...
    fn hidden_external_hint_visibility() {
        ...
        assert!(hidden_external_hint(Some(&entries), SourceFilter::Hystersis).is_none());
