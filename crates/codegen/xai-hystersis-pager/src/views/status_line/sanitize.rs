...
//! `xai-hystersis-pager-render`'s `vte::Perform`, which discards OSC and quantizes
...
fn safe_link_target(uri: &str) -> Option<Arc<str>> {
    ...
    // The shared gate stops at the scheme, enough for Hystersis's own text but not a
