...
use xai_hystersis_shell::session::repo_changes::UploadMethod;
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            hystersis_version: xai_hystersis_version::full_version().to_owned(),
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    xai_hystersis_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
...
async fn run_upload(
    ...
    if bucket_url.is_none()
        && matches!(
            upload_method,
            xai_hystersis_shell::session::repo_changes::UploadMethod::Direct { .. }
        )
    {
        anyhow::bail!(
    ...
    let upload_config = xai_hystersis_shell::session::repo_changes::TraceExportConfig {
...
impl UploadAttempt<'_> {
    ...
    fn write_debug_log(&self, error: &anyhow::Error, output_dir: &Path) -> PathBuf {
        ...
        let _ = writeln!(log, "Hystersis version: {}", xai_hystersis_version::full_version());
        let _ = writeln!(
            log,
            "OS:           {} {}",
            std::env::consts::OS,
...
async fn upload_with_retries(
    config: &xai_hystersis_shell::session::repo_changes::TraceExportConfig,
    object_path: &str,
    archive: &[u8],
...
pub async fn resolve_upload_method(agent_config: &AgentConfig) -> Option<UploadMethod> {
    ...
    let auth_token = xai_hystersis_shell::auth::ensure_authenticated_or_noninteractive(
