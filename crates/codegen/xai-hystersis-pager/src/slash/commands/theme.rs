...
mod tests {
    ...
    fn with_test_env(f: impl FnOnce()) {
        ...
        theme_cache::set(ThemeKind::HystersisNight);
        f();
        system_appearance::clear_mock();
        theme_cache::reset_for_test();
    }
    ...
    fn suggest_args_explicit_active_when_not_auto() {
        with_test_env(|| {
            ...
            let items = cmd.suggest_args(&ctx, "").expect("should return items");
            let hystersisnight = items
                .iter()
                .find(|i| i.insert_text == "hystersisnight")
                .expect("hystersisnight should be in list");
            assert!(
                hystersisnight.description.contains("(active)"),
                "explicit theme should show (active), got: {}",
                hystersisnight.description
            );
        });
    }
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "hystersisnight");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "hystersisnight");
                }
                other => panic!("expected Action::SetTheme(\"hystersisnight\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] = HystersisNight; next is available[1].
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for HystersisNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "hystersisnight", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"hystersisnight\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // Default auto config maps Light -> HystersisDay.
            assert_eq!(Theme::current_kind(), ThemeKind::HystersisDay);
        });
    }
    ...
    fn cancel_preview_restores_previous_kind() {
        with_test_env(|| {
            ...
            cmd.cancel_preview("hystersisnight");
            assert_eq!(
                Theme::current_kind(),
                ThemeKind::HystersisNight,
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::HystersisDay);
            let cmd = ThemeCommand;
