...
use xai_hystersis_shell::sampling::types::supports_reasoning_effort_meta;
...
impl SlashCommand for ModelCommand {
    ...
    fn run(&self, ctx: &mut CommandExecCtx, args: &str) -> CommandResult {
        ...
        // often contain spaces ("Hystersis 4.5"); if we split on the last token
        // first, a shorter catalog entry ("Hystersis") would steal the prefix and
        ...
        // level (e.g. `none` on hystersis-4.5) surfaces the effort error with the
...
mod tests {
    ...
    fn empty_query_returns_one_row_per_logical_model() {
        ...
        let (pid, pinfo) = plain_model("hystersis-4.5", "Hystersis 4.5");
        ...
        let plain = items.iter().find(|i| i.match_text == "Hystersis 4.5").unwrap();
        assert_eq!(plain.insert_text, "Hystersis 4.5");
    }
    ...
    fn run_prefers_full_multi_word_model_name_over_prefix_plus_effort() {
        // Catalog has both "Hystersis" (reasoning) and "Hystersis 4.5". `/model Hystersis 4.5`
        // must select the full name, not treat "4.5" as an effort on "Hystersis".
        let mut state = ModelState::default();
        let (short_id, short_info) = model_with_reasoning("hystersis", "Hystersis");
        let (long_id, long_info) = model_with_reasoning("hystersis-4.5", "Hystersis 4.5");
        state.available.insert(short_id, short_info);
        state.available.insert(long_id.clone(), long_info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "Hystersis 4.5");
        match result {
            CommandResult::Action(Action::SetDefaultModel(resolved_id)) => {
                assert_eq!(resolved_id, long_id);
            }
            other => panic!("expected SetDefaultModel(Hystersis 4.5), got {other:?}"),
        }
    }
    ...
    fn run_rejects_effort_for_non_reasoning_model() {
        let mut state = ModelState::default();
        let (id, info) = plain_model("hystersis-4.5", "Hystersis 4.5");
        state.available.insert(id, info);
        let mut ctx = dummy_exec_ctx(&state);
        let result = ModelCommand.run(&mut ctx, "Hystersis 4.5 high");
    ...
    /// Case-insensitive matching against the catalog: `/model hystersis 4.5`
    /// resolves to the same `ModelId` as `/model Hystersis 4.5`.
    #[test]
