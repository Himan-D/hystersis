...
/// plus `/help`). Must stay covered by [`xai_hystersis_shell::session::PAGER_COMMAND_KEYS`]
...
impl CommandRegistry {
    ...
    /// but disappear from `get()` — invoking one shows the HystersisPro upsell
    ...
    fn rebuild_triggers(&mut self) {
        ...
        for (idx, command) in self.commands.iter().enumerate() {
            ...
            // invoking one shows the HystersisPro upsell instead.
