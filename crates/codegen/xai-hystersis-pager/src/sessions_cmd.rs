...
use xai_hystersis_shell::auth::{AuthManager, try_ensure_fresh_auth};
use xai_hystersis_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    let client = xai_hystersis_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = xai_hystersis_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                xai_hystersis_shell::session::merge::CwdScope::WithSiblings,
                None,
                limit,
                // The CLI listing is an inventory, not the resume picker.
                xai_hystersis_shell::session::visibility::HeadlessPolicy::Include,
            )
            .await;
        ...
        SessionsCommand::Search { query, limit } => {
            ...
            use xai_hystersis_shell::session::merge::REMOTE_TIMEOUT;
            ...
            let search = xai_hystersis_shell::session::storage::search::start_if_enabled(agent_config);
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = xai_hystersis_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
