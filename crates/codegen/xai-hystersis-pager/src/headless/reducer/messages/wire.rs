...
/// Map a Hystersis permission mode to the Messages `permissionMode` enum, clamping Hystersis-only values to `default`.
...
/// The `usage.server_tool_use` sub-object (Hystersis counts backend `web_search` only).
#[derive(Clone, Default, Serialize, Deserialize)]
