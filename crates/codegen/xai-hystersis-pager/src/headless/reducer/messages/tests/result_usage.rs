...
fn messages_model_usage_maps_and_zero_fills() {
    let rows = json!({
        "hystersis-4": {"inputTokens": 90, "outputTokens": 7, "cacheReadInputTokens": 10, "cacheCreationInputTokens": 25, "costUSD": 0.02},
    });
    let out = messages_model_usage(Some(&rows), Some("hystersis-4"), 0, Some(131_072));
    let mu = &out["hystersis-4"];
...
fn messages_model_usage_attributes_web_search_to_current_model() {
    let rows = json!({
        "hystersis-4": {"inputTokens": 90, "outputTokens": 7, "costUSD": 0.02},
        "hystersis-mini": {"inputTokens": 5, "outputTokens": 1},
    });
    let out = messages_model_usage(Some(&rows), Some("hystersis-4"), 3, Some(131_072));
    assert_eq!(out["hystersis-4"]["webSearchRequests"], 3);
    assert_eq!(out["hystersis-mini"]["webSearchRequests"], 0);
    assert_eq!(out["hystersis-4"]["contextWindow"], 131_072);
    assert!(out["hystersis-mini"]["contextWindow"].is_null());
}
