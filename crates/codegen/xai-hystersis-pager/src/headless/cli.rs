...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<xai_hystersis_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_cli_agents(
    json: &str,
) -> anyhow::Result<Vec<xai_hystersis_shell::agent::config::AgentDefinition>> {
    ...
    for (name, mut value) in map {
        ...
        let mut def = xai_hystersis_shell::agent::config::AgentDefinition::from_json(&value)
...
pub(crate) fn apply_agent_flag(
    agent: &Option<String>,
    config: &mut xai_hystersis_shell::agent::config::Config,
) {
