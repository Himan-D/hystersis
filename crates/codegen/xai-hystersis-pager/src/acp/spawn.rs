...
pub async fn spawn_hystersis_shell(
    ...
    cancel: &CancellationToken,
    memory_config: Option<xai_hystersis_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    xai_hystersis_shell::agent::app::apply_otel_config(&auth_manager, &agent_config.hystersis_com_config);
    ...
    xai_hystersis_shell::managed_config::ensure_managed_policy_present(&auth_manager).await;
    ...
    let (agent_config, models_manager) =
        xai_hystersis_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
            ...
            local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        xai_hystersis_agent::prompt::workspace_user::optional_workspace_user_dir();
                    xai_hystersis_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    xai_hystersis_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    xai_hystersis_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
