...
impl ModelState {
    ...
    /// Machine-readable model ID string for the current model (e.g. "hystersis-4.5").
    ...
    /// correct today (all current Hystersis models accept images, so nothing is
    ...
    /// not offer (e.g. `none` on hystersis-4.5) are rejected so we fail in the TUI
