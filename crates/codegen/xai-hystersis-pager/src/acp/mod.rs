...
use xai_hystersis_telemetry::startup;
pub use xai_hystersis_telemetry::startup::{
...
use xai_hystersis_shell::agent::auth_method::AuthMethodKind;
...
pub struct AcpConnection {
    ...
    // When available, add field: startup_announcements: Option<Vec<xai_hystersis_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<xai_hystersis_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<xai_hystersis_shell::util::config::RemoteSettings>,
...
pub async fn connect_via_leader(
    ...
    startup::set_auth_mode(xai_hystersis_shell::managed_config::classify_auth_mode());
    let mut agent_config = AgentConfig::new_from_toml_cfg(raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    xai_hystersis_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = xai_hystersis_shell::util::hystersis_home::hystersis_home().join("config.toml");
...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    meta[xai_hystersis_status_line::STATUS_LINE_CAPABILITY] = flags.status_line.into();
    meta
}
...
mod tests {
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use xai_hystersis_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
    ...
    fn startup_auth_xai_api_key_not_first_still_requires_login() {
        use xai_hystersis_shell::agent::auth_method::{HYSTERSIS_COM_METHOD_ID, XAI_API_KEY_METHOD_ID};
    ...
    fn client_capabilities_meta_advertises_the_status_line_the_config_asked_for() {
        let key = xai_hystersis_status_line::STATUS_LINE_CAPABILITY;
