//! Headless single-turn mode (`hystersis -p "prompt"`).
//!
//! Runs the agent in-process via `spawn_hystersis_shell`, drives the ACP lifecycle
...
use xai_hystersis_shell::extensions::task::{CancelSubagentRequest, KillTaskRequest};
...
use xai_hystersis_shell::util::config as cli_config;
use xai_hystersis_telemetry::startup::PendingStartup;

use crate::acp::model_state::{EffortTokenError, ModelState};
use crate::acp::spawn::{AgentShutdownGuard, spawn_hystersis_shell};
...
impl HeadlessEmitter {
    ...
    /// Render an `hystersis.com/*` lifecycle notification for the active format.
...
pub(crate) fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    xai_hystersis_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
fn mcp_server_names(cwd: &Path) -> Vec<McpServer> {
    let servers =
        cli_config::load_mcp_servers(cwd, &xai_hystersis_tools::types::compat::CompatConfig::default());
    servers
        .iter()
        .filter_map(|s| {
            let name = match s {
...
fn auth_required_message(interactive: bool) -> String {
    if interactive {
        "Not signed in. Run `hystersis login` to authenticate \
         (or `hystersis login --device-code` if no browser is available)."
            .to_string()
    } else {
        "Not signed in. To authenticate without a browser, run:\n  \
         hystersis login --device-code\n\n\
         Alternatively, set the XAI_API_KEY environment variable \
         or run `hystersis login` on a machine with a browser."
            .to_string()
    }
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !xai_hystersis_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !xai_hystersis_shell::util::clipboard::is_remote_session();
            ...
            acp::LoadSessionRequest::new(acp::SessionId::new(sid.to_string()), cwd.to_path_buf())
                ...
                .meta({
                    ...
                    if let Some(rc) = restore_code {
                        m.insert("hystersis.com/restore_code".into(), serde_json::Value::Bool(rc));
                    }
                    Some(m)
                }),
            acp_tx,
...
async fn fork_then_open(
    ...
    let req = acp::ExtRequest::new("hystersis.com/session/fork", fork_params.into());
...
async fn apply_headless_model_and_effort(
    ...
    .map_err(|e| {
        if let Some(name) = model_name {
            anyhow::anyhow!(
                "Couldn't set model '{}': {}. Run 'hystersis models' to see available models.",
                name,
                e
            )
        } else {
...
pub async fn run_single_turn(
    ...
    xai_hystersis_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = xai_hystersis_shell::agent::config::AgentMode::Headless;
    ...
    agent_config.cli_agent_overrides = xai_hystersis_shell::agent::config::CliAgentOverrides {
    ...
    if options.trust {
        xai_hystersis_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }
    ...
    let spawned = match spawn_hystersis_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    xai_hystersis_telemetry::startup::enter(crate::acp::StartupPhase::AcpInitialize);
    ...
    xai_hystersis_telemetry::startup::enter(crate::acp::StartupPhase::EagerAuth);
    ...
    xai_hystersis_telemetry::startup::enter(crate::acp::StartupPhase::SessionCreate);
    ...
    let track_active = std::env::var("HYSTERSIS_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = xai_hystersis_active_sessions::register(xai_hystersis_active_sessions::ActiveSession {
    ...
    let session_models = if needs_fresh_catalog {
        match xai_hystersis_shell::cli_models::fetch_model_state(&acp_tx).await {
    ...
    if track_active {
        // Non-blocking flock so a slow/network ~/.hystersis can't hang exit.
        let _ = xai_hystersis_active_sessions::try_unregister(&session_id);
    }
    ...
    let outcome: Result<()> = match prompt_result {
        Some(Ok(resp)) => {
            ...
            let is_max_turns = resp
                ...
                .and_then(|v| v.as_str())
                == Some(xai_hystersis_shell::session::commands::MAX_TURNS_REACHED_CATEGORY);
            if is_max_turns {
        ...
        Some(Err(err)) => {
            ...
            if let Some(usage) = xai_hystersis_shell::sampling::error::prompt_usage_from_error(&err) {
            ...
            let stop_reason_override =
                (xai_hystersis_shell::sampling::error::stop_reason_for_turn_error(&err) == "MaxTokens")
    ...
    let (method, params) = match work {
        ...
        BackgroundWork::Task(id) => (
            ...
            serde_json::value::to_raw_value(&KillTaskRequest {
                session_id: session_id.0.to_string(),
                task_id: id.clone(),
                source: xai_hystersis_shell::extensions::task::TaskKillSource::Teardown,
            })?,
        ),
    };
