...
//!
//! voice-api accepts both API keys and OAuth2 tokens directly at `api.hystersis.com`
...
use xai_hystersis_voice::{SharedVoiceAuth, VoiceAuthProvider};
...
///
/// Works for every auth method: OAuth / hystersis.com / OIDC session tokens and
/// `XAI_API_KEY` / per-model BYOK keys.
pub fn build_voice_auth(auth_manager: Arc<xai_hystersis_shell::auth::AuthManager>) -> SharedVoiceAuth {
    Arc::new(AuthManagerVoiceAuth(
        xai_hystersis_shell::auth::shared_api_key_provider(auth_manager),
    ))
}
