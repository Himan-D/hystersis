...
//! payloads come from `hystersis_test_support::sse`.
...
use hystersis_test_support::{SseEvent, sse};
    ...
    assert!(
        events.iter().any(|e| match e {
            SamplingEvent::ImagesStripped {
                stripped_urls,
                reason: hystersis_sampler::StripReason::ServerRejected,
                ..
    ...
    match events.last().unwrap() {
        SamplingEvent::Completed { response, .. } => {
            assert_eq!(
                ...
                Some(hystersis_sampling_types::StopReason::ContentFilter)
            );
        }
        other => panic!("expected Completed, got {other:?}"),
    }
...
async fn responses_doom_loop_does_not_resample_after_output_when_retry_only_before_output() {
    ...
    assert!(
        matches!(
            result,
            Err(hystersis_sampling_types::SamplingError::DoomLoopDetected { .. })
        ),
