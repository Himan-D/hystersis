...
//! `hystersis_sampling_types::SamplingConfig` so that the sampler crate
//! avoids transitive dependencies on shell-specific types
//! (`xai-hystersis-tools`, etc.).
...
/// submit.
///
/// # Construction in `xai-hystersis-shell`
///
...
/// composing chat-state's `hystersis_sampling_types::SamplingConfig`
...
pub struct SamplerConfig {
    ...
    /// client itself sends the opt-in `x-hystersis-doom-loop-check` header on
