...
enum AttemptOutcome {
    ...
    Failed {
        error: SamplingError,
        recovery_items: Vec<hystersis_sampling_types::ConversationItem>,
    },
...
async fn run_one_attempt(
    ...
    event_tx: &mpsc::UnboundedSender<SamplingEvent>,
    cancel_token: &CancellationToken,
    doom_check: Option<hystersis_sampling_types::DoomLoopRecoveryPolicy>,
        ...
        tokio::select! {
            ...
            next = l2.next() => match next {
                Some(SamplingEvent::Completed { response, metrics, .. }) => {
                    ...
                    if response.stop_reason == Some(hystersis_sampling_types::StopReason::Length) {
                        return AttemptOutcome::Failed {
...
fn build_empty_context(
    reason: hystersis_sampling_types::EmptyReason,
